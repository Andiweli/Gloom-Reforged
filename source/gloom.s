...
.loop	jsr	selmenu
	move	d0,d7
	and	#$0300,d7	;v43c: $0100=left, $0200=right in game menu
	and	#$00ff,d0
	tst	d7
...
.notlwin
	...
	cmp	#$0100,d7
	beq.s	.floor_left
	cmp	#-1,floorflag	;right/fire: YES -> SHADED -> NO, stop at NO
	ble.s	.floor_nochange
	subq	#1,floorflag
	bra.s	.floor_changed
.floor_left
	cmp	#1,floorflag	;left: NO -> SHADED -> YES, stop at YES
	bge.s	.floor_nochange
	addq	#1,floorflag
.floor_changed
...
.notfloor
	...
	cmp	#$0100,d7
	beq.s	.roof_left
	cmp	#-1,roofflag	;right/fire: YES -> SHADED -> NO, stop at NO
	ble.s	.roof_nochange
	subq	#1,roofflag
	bra.s	.roof_changed
.roof_left
	cmp	#1,roofflag	;left: NO -> SHADED -> YES, stop at YES
	bge.s	.roof_nochange
	addq	#1,roofflag
.roof_changed
...
.notroof
	...
	cmp	#$0100,d7
	beq.s	.inv_left
	bsr	trainer_inv_on_clamped
	bra	.loop
.inv_left
	bsr	trainer_inv_off_clamped
	bra	.loop
.notinv
	cmp	#9,d0
	bne.s	.notbouncy
	cmp	#$0100,d7
	beq.s	.bouncy_left
	bsr	trainer_bouncy_on_clamped
	bra	.loop
.bouncy_left
	bsr	trainer_bouncy_off_clamped
	bra	.loop
.notbouncy
	cmp	#10,d0
	bne.s	.notweapon
...
trainer_toggle_inv
	...
	beq.s	trainer_inv_on
	bra.s	trainer_inv_off_clamped
trainer_inv_on
	; fall through to trainer_inv_on_clamped

trainer_inv_on_clamped
	tst	trainer_invincible
	bne.s	.inv_done
...
trainer_toggle_bouncy
	tst	trainer_bouncy
	beq.s	trainer_bouncy_on
	bra.s	trainer_bouncy_off_clamped
trainer_bouncy_on
	; fall through to trainer_bouncy_on_clamped

trainer_bouncy_on_clamped
	tst	trainer_bouncy
	bne.s	.bouncy_done
...
trainer_bouncy_off_clamped
	tst	trainer_bouncy
	beq.s	.bouncy_off_done
	clr	trainer_bouncy
.bouncy_off_done
	bsr	trainer_update_bouncy_text
	bra	trainer_apply
...
trainer_next_weapon
	...
	bcc.s	trainer_weapon_ok
	addq	#1,trainer_weapon
trainer_weapon_ok
...
trainer_prev_weapon
	tst	trainer_weapon
	beq.s	trainer_weapon_prev_ok
	subq	#1,trainer_weapon
trainer_weapon_prev_ok
	bsr	trainer_update_weapon_text
	bra	trainer_apply

trainer_next_boost
	cmp	#5,trainer_boost
	bcc.s	trainer_boost_ok
	addq	#1,trainer_boost
trainer_boost_ok
...
trainer_prev_boost
	tst	trainer_boost
	beq.s	trainer_boost_prev_ok
	subq	#1,trainer_boost
trainer_boost_prev_ok
	bsr	trainer_update_boost_text
	bra	trainer_apply
trainer_apply
	move.l	player1(pc),d0
	beq.s	trainer_apply_p2
	move.l	d0,a5
	bsr	trainer_apply_one
trainer_apply_p2
	tst	twowins
	beq.s	trainer_apply_rts
...
trainer_apply_one
	...
	beq.s	trainer_apply_weapon_default
	subq	#1,d0
	move	d0,ob_weapon(a5)
trainer_apply_weapon_default
	move	trainer_boost,d0
	beq.s	trainer_apply_boost_default
	moveq	#6,d0
	sub	trainer_boost,d0
	move.b	d0,ob_reload(a5)
trainer_apply_boost_default
	tst	trainer_bouncy
	beq.s	trainer_apply_nobouncy
	move	#3,ob_bouncecnt(a5)
	bra.s	trainer_apply_bouncy_done
trainer_apply_nobouncy
	clr	ob_bouncecnt(a5)
trainer_apply_bouncy_done
	tst	trainer_invincible
	beq.s	trainer_apply_noinv
	move	#25,ob_hitpoints(a5)
trainer_apply_noinv
	st	ob_update(a5)
	rts

trainer_enforce_weapon_upgrade
	move	trainer_weapon,d0
	beq.s	trainer_enforce_weapon_done
	subq	#1,d0
	cmp	ob_weapon(a5),d0
	beq.s	trainer_enforce_weapon_done
	move	d0,ob_weapon(a5)
	st	ob_update(a5)
trainer_enforce_weapon_done
	move	trainer_boost,d0
	beq.s	trainer_enforce_boost_done
	moveq	#6,d0
	sub	trainer_boost,d0
	...
	move.b	d0,ob_reload(a5)
	st	ob_update(a5)
trainer_enforce_boost_done
	rts
trainer_update_inv_text
	lea	game_inv,a0
	lea	26(a0),a0
	tst	trainer_invincible
	beq.s	trainer_inv_txt_no
	move.b	#89,(a0)+	;Y
	move.b	#69,(a0)+	;E
	move.b	#83,(a0)+	;S
	rts
...
trainer_update_bouncy_text
	...
	lea	26(a0),a0
	tst	trainer_bouncy
	beq.s	trainer_bouncy_txt_no
	move.b	#89,(a0)+
	move.b	#69,(a0)+
	move.b	#83,(a0)+
	rts
trainer_bouncy_txt_no
	move.b	#78,(a0)+
	move.b	#79,(a0)+
	move.b	#32,(a0)+
	rts
...
trainer_update_wsize_text
	...
	mulu	d3,d2
	cmp	#318,d0
	bcs.s	.trainer_wsize_notfull
	cmp	#230,d2
	bcs.s	.trainer_wsize_notfull
	lea	trainer_txt_fullscreen,a1
	bra.s	trainer_copy_wsize
.trainer_wsize_notfull
	cmp	#150,d0
	bcs.s	.trainer_wsize_tiny
	cmp	#180,d0
	bcs.s	.trainer_wsize_small
	cmp	#210,d0
	bcs.s	.trainer_wsize_medium
	cmp	#240,d0
	bcs.s	.trainer_wsize_large
	cmp	#264,d0
	bcs.s	.trainer_wsize_xlarge
	cmp	#288,d0
	bcs.s	.trainer_wsize_huge
	cmp	#312,d0
	bcs.s	.trainer_wsize_vhuge
	lea	trainer_txt_almost,a1
	bra.s	trainer_copy_wsize
.trainer_wsize_tiny
	lea	trainer_txt_tiny,a1
	bra.s	trainer_copy_wsize
.trainer_wsize_small
	lea	trainer_txt_small,a1
	bra.s	trainer_copy_wsize
.trainer_wsize_medium
	lea	trainer_txt_medium,a1
	bra.s	trainer_copy_wsize
.trainer_wsize_large
	lea	trainer_txt_large,a1
	bra.s	trainer_copy_wsize
.trainer_wsize_xlarge
	lea	trainer_txt_xlarge,a1
	bra.s	trainer_copy_wsize
.trainer_wsize_huge
	lea	trainer_txt_huge,a1
	bra.s	trainer_copy_wsize
.trainer_wsize_vhuge
	lea	trainer_txt_vhuge,a1
trainer_copy_wsize
	lea	game_wsize,a0
	lea	26(a0),a0
	moveq	#15,d1
.trainer_wsize_blank
	move.b	#32,(a0)+
	dbf	d1,.trainer_wsize_blank
	lea	game_wsize,a0
	lea	26(a0),a0
.trainer_wsize_copy
	move.b	(a1)+,d0
	beq.s	.trainer_wsize_done
	move.b	d0,(a0)+
	bra.s	.trainer_wsize_copy
.trainer_wsize_done
	rts
...
trainer_update_floor_text
	...
	bmi.s	trainer_floor_no
	beq.s	trainer_floor_shaded
	move.b	#89,(a0)+	;Y
	move.b	#69,(a0)+	;E
	move.b	#83,(a0)+	;S
	move.b	#32,(a0)+
...
trainer_update_ceiling_text
	...
	lea	26(a0),a0
	move	roofflag(pc),d0
	bmi.s	trainer_ceil_no
	beq.s	trainer_ceil_shaded
	move.b	#89,(a0)+
	move.b	#69,(a0)+
	move.b	#83,(a0)+
	move.b	#32,(a0)+
...
largewin	move.l	player1,a5
	bsr	isfullwindow
	bne.s	.large_do
	rts	;already fullscreen - do not refresh on repeated click
.large_do
	move.l	player1,a5
	bsr	large
	tst	twowins
...
isfullwindow	move.l	ob_window(a5),a6
	move	wi_w(a6),d0
	move	wi_pw(a6),d1
	mulu	d1,d0
	move	wi_h(a6),d2
...
newwsize	;window size up, stops at fullscreen
	;
	move.l	player1,a5
	bsr	isfullwindow
	beq.s	.rts
...
prevwsize	;window size down, stops at tiny
	;
	move.l	player1,a5
	bsr	istinywindow
	beq.s	.rts
...
istinywindow	move.l	ob_window(a5),a6
	move	wi_w(a6),d0
	move	wi_pw(a6),d1
	mulu	d1,d0
	cmp	#132,d0
...
swapwsize	;increment window size without wrap
	;
	move.l	ob_window(a5),a6
...
backwsize	;decrement window size without wrap
	;
	move.l	ob_window(a5),a6
	movem	wi_w(a6),d0-d1	;w,h
	move	wi_pw(a6),d2
...
blithudtext	;a4=zero-terminated text, a6=window, d7=x, d6=y
	;
.loop	move.b	(a4)+,d2
	beq.s	.done
	cmp.b	#' ',d2
...
showstats	;a5=player
	...
	;elseif
	;
	;lagtime... disabled - leaked a lone 0 under HUD labels
;	move.l	ob_window(a5),a0
;	moveq	#2,d0
;	add	wi_x(a0),d0
;	move.l	wi_bmap(a0),a1
...
.nofade	rts
	...
	dc.l	readmodem
joytable	dc.l	readkeymouse,readkeys,readjoy1,readjoy0,readcd321,readcd320
...
vbhandler	movem.l	d2-d7/a2-a6,-(a7)
	...
	addq	#1,(a1)
	;
	;v28: sample/apply KEYBMOUSE mouse before drawing to reduce visible input lag
...
playertimers	tst	trainer_invincible
	beq.s	trainer_inv_keep_done
	move	#25,ob_hitpoints(a5)
	st	ob_update(a5)
trainer_inv_keep_done
	bsr	trainer_enforce_weapon_upgrade
	tst	ob_mega(a5)
	beq.s	.nomega
	subq	#1,ob_mega(a5)
	bne.s	.nomega
...
rotplayer	;return rot in d0, leave a0
	;
	cmp	#0,ob_cntrl(a5)	;KEYBMOUSE mouse rotation is applied in VBlank sampler
	bne.w	.notkmouse
...
moveplayer	;work out movement vector into d0/d1...check still/moving!
	;
	cmp	#0,ob_cntrl(a5)	;KEYBMOUSE: cursor movement + mouse turn
	bne	.normmove
...
checkfireb	move	joyb(pc),d0
	beq.s	.nofire
	cmp	#0,ob_cntrl(a5)	; KEYBMOUSE keeps firing while held
	beq.s	.kmfire
	tst	ob_lastbut(a5)
	bne.s	.skip
...
.blok	sub	camy(pc),d1
	...
	bge	.loop
	;
	;GLOOM_REFORGED_V45A_BLOOD_WALL_OCCLUSION
	;Blood is a single pixel poke after walls/shapes, so it must
	;use the same per-column wall Z test as normal object drawing.
	move	d0,d4
	add	midx(pc),d4	;screen column 0..width-1
	move.l	vertdraws(pc),a0
	mulu	#vd_size,d4
	lea	0(a0,d4),a0
	cmp	vd_z(a0),d2
	bcc	.loop	;hidden behind nearer wall/opaque strip
	;
	cmp	#40,d2
	bcc.s	.pix
	tst	bl_y(a5)
...
readkeymouse	;keyboard plus mouse into a0 block
	; v44o: KEYBMOUSE WAXD via rawkey event state, not a new CIA poller
	; cursor up/down or W/X = forward/back, A/D or cursor left/right = strafe
	; W/A/X/D are tracked in rawkeyread so key rollover can avoid the S+A/S+D rollover issue
	; mouse turn remains separate in sample_keymouse_vb; original KEYBOARD path unchanged
	movem.l	d1-d3/a1,-(a7)
	clr	keymouse_mx
	move.l	rawtable,a1
	; clear control block
	...
	clr	4(a0)
	clr	6(a0)
	; cursor up/numpad up/W = forward, cursor down/numpad down/S = backward
	moveq	#0,d0
	keya1	$63	; cursor up
	bne.s	.up
	keya1	$4c	; numpad up
	...
	beq.s	.nup
.up	moveq	#-1,d0
.nup	keya1	$60	; cursor down
	bne.s	.down
	keya1	$4d	; numpad down
	bne.s	.down
	btst	#3,wasd_state	; X held = backward
	beq.s	.ndown
.down	moveq	#1,d0
.ndown	move	d0,2(a0)
	; cursor left/A = strafe left, cursor right/D = strafe right
	moveq	#0,d0
...
.nrite	tst	d0
	...
	move	d0,(a0)
	move	#-1,6(a0)
.nostrafe	; keyboard fire remains available
	moveq	#0,d0
	keya1	$66
	beq.s	.nkeyfire
	moveq	#-1,d0
.nkeyfire	move	d0,4(a0)
	; mouse X is handled in sample_keymouse_vb once per VBlank.
	; Keep this routine focused on cursor movement/fire so movement remains stable.
	clr	keymouse_mx
.mousefire	; left mouse button = fire
	btst	#6,$bfe001
	bne.s	.nolmfire
	move	#-1,4(a0)
.nolmfire	movem.l	(a7)+,d1-d3/a1
	rts

sample_keymouse_vb	;v29a: VBlank direct mouse yaw for KEYBMOUSE, mid speed
	movem.l	d0-d2/a5,-(a7)
	move	$dff00a,d1
	move.b	d1,d0	;JOY0DAT low byte = mouse X counter
	tst	mousexinit
	bne.s	.vb_have_last
	move.b	d0,mousexlast
	move	#-1,mousexinit
	bra.s	.vb_done
.vb_have_last	move.b	mousexlast(pc),d2
	move.b	d0,mousexlast
	sub.b	d2,d0	;8-bit wrap-safe delta
	ext.w	d0
	beq.s	.vb_done
	cmp	#24,d0	;per-VBlank clamp, not direction memory
	ble.s	.vb_pos_ok
	moveq	#24,d0
	bra.s	.vb_apply
.vb_pos_ok	cmp	#-24,d0
...
.vb_apply	tst	paused
	...
	clr.w	d0
	move.l	d0,d1
	asr.l	#3,d0	;v29a: v28 base mouse sensitivity
	asr.l	#4,d1	;v29a: add half step for approx 1.5x v28
	add.l	d1,d0
	add.l	d0,ob_rot(a5)
	clr.l	ob_rotspeed(a5)
.vb_done	movem.l	(a7)+,d0-d2/a5
	rts
...
.solid	move.l	a3,vd_data(a0)	;start column
	...
	;elseif
	;
;	neg	d3	; disabled alternate quarter wall texture step
;	neg	d5
;	cmp	#-128,camy
;	sle	d4
;	ext	d4
...
window1	;
	...
	dc	160-159
	dc	cy-120
	dc	106	;fullscreen window default
	dc	80
	dc	3
	dc	3
	;
...
.use	lea	iffwindow,a6
	...
	add	d0,d0
	neg	d0
	move	d0,menu_yadjust
	jsr	initmenu
	clr	menu_yadjust
	bsr	print_reforged_footer
	;
.sel	jsr	selmenu
	;
	tst	linked
...
print_reforged_footer
	lea	iffwindow,a6
	move	wi_bh(a6),d6
	sub	fonth(pc),d6	;half-line up from previous footer position
	bra.s	print_reforged_footer_do

print_reforged_footer_about
	lea	iffwindow,a6
	move	menuy(pc),d6	;one blank line below last about line
	move	numopts(pc),d0
	addq	#1,d0
	mulu	fonth(pc),d0
	add	d0,d6
print_reforged_footer_do
	lea	reffooter_txt(pc),a4
	move.l	a4,a0
	moveq	#-1,d0
.len	addq	#1,d0
	tst.b	(a0)+
...
gamemenu	dc.b	14
	dc.b	'CONTINUE',0
	dc.b	92,0
; label columns padded so ':' lines up - v9 centered colon
game_res	dc.b	'              RESOLUTION: 3X3                   ',0
game_wsize	dc.b	'             WINDOW SIZE: FULLSCREEN            ',0
...
reffooter_txt	dc.b	'GLOOM REFORGED V0.1 BY ANDIWELI',0
	even
...
menuy	dc	0
menu_yadjust	dc	0	;main menu vertical adjustment
...
readmenujoy	;encode to d0!
	...
	beq.s	.noesc
	moveq	#$20,d0	;ESC = cancel/back to game in game menu
	rts
.noesc	qkey	$44
	bne.s	.fire
	bra.s	.encode
...
.encode	lea	joyx(pc),a0
	...
	and	#$1f,d0	;v43c game menu: left/right/up/down/fire
	rts
.normal_menu_mask
...
menuskip	;return EQ if current item is a visual spacer
	move	curropt(pc),d0
	lea	menustrips(pc),a0
	move.l	0(a0,d0*8),a0
	cmp.b	#92,(a0)
...
.joygot	move	d0,-(a7)
	...
	move	(a7)+,d0
.joygot2	;
	btst	#5,d0	;ESC cancel/back to game
	beq.s	.noescsel
	moveq	#0,d0
	rts
.noescsel	btst	#0,d0
	...
	rts
	;
.left	move	curropt(pc),d0
	or	#$0100,d0
	rts
.right	move	curropt(pc),d0
	or	#$0200,d0
	rts
	;
.up	subq	#1,curropt
...
wasd_keydown_update	;maintain KEYBMOUSE WASD state from raw keyboard events
	movem.l	d0-d3/a0,-(a7)
	move.w	d2,d3
	and.w	#$7f,d3
	...
	beq.s	.done
	bset	d0,wasd_state
.done	movem.l	(a7)+,d0-d3/a0
	rts

wasd_keyup_update	;clear KEYBMOUSE WASD state from raw keyboard events
	movem.l	d0-d3/a0,-(a7)
	move.w	d2,d3
	and.w	#$7f,d3
	bsr	wasd_map_rawcode
...
wasd_map_rawcode	;d3.w rawcode -> d0.w bit number, 0 if not WAXD
	; W/A/D use confirmed rawkeys. X is used as backward instead of S.
	cmp.w	#$11,d3	; W = forward
	beq.s	wasd_map_w
	cmp.w	#$20,d3	; A = strafe left
	beq.s	wasd_map_a
	cmp.w	#$32,d3	; X = backward, avoids S+A/S+D rollover issue
	beq.s	wasd_map_x
	cmp.w	#$22,d3	; D = strafe right
	beq.s	wasd_map_d
	; fallback: use Gloom's own rawmap so keyboard layout/init decides the letter
	lea	rawmap(pc),a0
	move.b	0(a0,d3.w),d0
	cmp.b	#'W',d0
	beq.s	wasd_map_w
	...
	moveq	#0,d0
	rts
wasd_map_w	moveq	#1,d0
	rts
wasd_map_a	moveq	#2,d0
	rts
wasd_map_x	moveq	#3,d0
	rts
wasd_map_d	moveq	#4,d0
	rts
...
abouttext	dc.b	16
	...
	dc.b	0
	dc.b	'GLOOM REFORGED V0.1 BY ANDIWELI',0
	even
