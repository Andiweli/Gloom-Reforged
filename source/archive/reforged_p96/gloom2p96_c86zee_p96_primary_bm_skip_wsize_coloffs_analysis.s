	...
	;gloom2p96_c86zee: BlackMagic single-blit/skip, window-size coloffs copy, audio test revert.
	;
...
.play	;
	...
	ifne	g2p96_overlay_enabled
	; c86zea: WINDOW SIZE is active again in P96-primary.  Do not force
	; fullscreen here; just clear the chunky frame once so smaller render
	; windows start with clean black borders.
...
.done	jsr	g2cfg_save	;v141: save ingame menu settings when leaving menu
	...
	ifne	g2p96_overlay_enabled
	; c86zea: preserve selected WINDOW SIZE after leaving ESC menu, but
	; clear the P96 chunky source so no stale menu/backdrop pixels survive.
	jsr	g2clearfullchunky
	endc
	jsr	predrawall
	jsr	dispon
...
drawall	;
	ifne	g2p96_overlay_enabled
	; c86zea: in P96-primary the RTG window pointer was already hidden; avoid
	; repeating Intuition pointer/DMA pokes every gameplay frame.
	tst	p96preview_gameactive
	bne.s	.g2p96_skip_gameptr_hide
	endc
	jsr	g2v36_hide_pointer	;v36: keep Intuition pointer hidden during gameplay
.g2p96_skip_gameptr_hide
.wait	tst	doneflag
	bne.s	.waitskip
	bsr	vwait
	bra.s	.wait
...
.waitskip2	; c86m: normal C2P/show path restored after c86l label regression
	...
	; c86zea: do not run ScreenToBack/ActivateWindow/SetPointer periodically
	; during gameplay.  The window is brought forward on entry/static pages;
	; repeated OS calls are unnecessary and may contribute to audio chatter.
...
.here	move.l	font(pc),a0
	...
	;
	ifne	g2p96_overlay_enabled
	; c86zdx: rolling intermission text is drawn character-by-character
	; while pdelay is active.  The old P96 menu bridge only flushed once
	; after the full line, so the crawl became visible only after ENTER.
	tst	p96preview_gameactive
...
.bwait	btst	#6,$dff002
	...
	bsr	disownblitter
	ifne	g2p96_overlay_enabled
	jsr	g2p96_present_planar_showbitmap_to_window	;c86zdt: menu flash-off to P96
	endc
	rts
...
.loop	addq	#1,d0
	...
	move	#13,flashdelay
	ifne	g2p96_overlay_enabled
	jsr	g2p96_present_planar_showbitmap_to_window	;c86zdt: menu flash-on to P96
	endc
	;
	rts
...
p96prevlog_name	dc.b	'RAM:gloom_p96_preview.txt',0
	even
p96prevlog_ok	dc.b	'P96 preview: c86zee blackmagic/window coloffs active',10
