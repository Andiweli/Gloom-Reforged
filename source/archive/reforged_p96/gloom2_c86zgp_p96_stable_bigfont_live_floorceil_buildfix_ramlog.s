...
g2release_marker	dc.b	'Gloom Reforged AGA/P96 probe (v1.4 c86zgp 09.07.2026) Andreas ',39,'Andiweli',39,' Stuermer',0
	even
...
p96gameplay_msg_ok	dc.b	'P96 gameplay: stable bigfont P96 menu live buildfix OK',10
...
; c86zgo: P96 in-game menu selection loop without periodic blink.
; The original AGA loop alternates optoff/opton every few frames.  With the
; real-bigfont P96 row bridge that causes expensive repeated rebuilds and looks
; like a broken lamp.  Under P96 keep the selected row stable and redraw only
; when the user actually moves or activates an option.
...
g2p96_menu_selmenu_stable
	jsr	opton		;c86zgo: make current P96 row visibly selected once
.wait
	jsr	vwait
	jsr	readmenusel
	beq.s	.wait
...
.right
	...
	rts
.up
	jsr	optoff		;c86zgo: unhighlight old row only on real navigation
.up_loop
	subq	#1,curropt
	bpl.s	.up_chk
	move	numopts,d0
...
.up_chk
	jsr	menuskip
	beq.s	.up_loop
	jsr	opton		;c86zgo: highlight new row immediately
	bra	.wait
.down
	jsr	optoff		;c86zgo: unhighlight old row only on real navigation
.down_loop
