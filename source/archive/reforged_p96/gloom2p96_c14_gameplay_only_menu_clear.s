...
p96prevlog_name	dc.b	'RAM:gloom_p96_preview.txt',0
	even
p96prevlog_ok	dc.b	'P96 preview: 80x48 gameplay-only/menu-clear active',10
...
.nolog	movem.l	d0-d7/a0-a6,-(a7)
	...
	beq.w	.restore
	lea	coloffs(pc),a3
	; gloom2p96_c10: c9 proved that a 40x24 block preview is stable.
	; Keep the confirmed 80x48 tonal grid with 4x4 blocks.  c11 only
	; raises update cadence again; the real-hardware probe now updates every
	; 2nd frame and still uses safe RectFill pens.  AGA output remains
	; untouched; this is still only a P96 preview probe.
	moveq	#0,d6		;preview grid row 0..23
.c7rowloop
	cmp	#48,d6
	bge	.c7done
	move	d6,d7
	mulu	#4,d7
	add	#20,d7		;sample source rows 20..208
	move	d7,d0
	mulu	chunkymodw(pc),d0
	move.l	a4,a2
...
.c7xloop
	...
	; Workbench/P96 pens only, but should preserve the scene's light/dark
	; structure across multiple preview bands.
	move.l	planar_palette(pc),a0
	tst.l	a0
	beq	.c7fallback
	move	d0,d1
	...
	bra	.c7paladdr
.c7ecs	add	d1,d1		;ECS palette entry = 1 word
.c7paladdr
...
.c7mapped
	move.l	50(a5),a1
	move.l	grbase(pc),a6
	jsr	-342(a6)	;SetAPen(mapped sample)
	move	d5,d0
	move	d6,d1
	mulu	#4,d1
	...
	move	d1,d3
	add	#3,d3
	jsr	-306(a6)	;RectFill 4x4 sample block
	add	#4,d5
	bra	.c7xloop
.c7nextrow
	addq	#1,d6
