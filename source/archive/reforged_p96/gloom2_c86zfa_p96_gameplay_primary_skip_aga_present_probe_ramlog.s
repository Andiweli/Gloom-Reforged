...
;gloom2_c86zdi: stronger near vertical oval, whole projectile/upgrade reflection Bayer-dithered, weapon-upgrade floor-touch pulse.
	...
	;c86zfa: P96 gameplay-primary probe; skip AGA doc2p/db while P96 present succeeds.
	;
...
.wait2	tst	showflag
	...
	bsr	vwait
	bra.s	.wait2
.waitskip2	; c86zfa: P96 gameplay-primary present probe.
	...
	jsr	g2p96_gameplay_present_probe	;c86zfa: primary P96 full-source AGA8 palette
...
; -----------------------------------------------------------------------------
; c86zfa: P96 gameplay-primary direct RGB565 full-source gameplay probe.
...
p96gameplay_msg_ok	dc.b	'P96 gameplay: primary skip AGA present OK',10
...
g2p96_gameplay_present_probe
	push
	clr	p96gameplay_skip_aga_present	;c86zfa: never skip AGA unless this exact frame copied OK
	; c86zfa: if the P96 gameplay-primary screen is already open, refresh it
	; once for this gameplay present.  The caller will skip doc2p/db only when
	; p96gameplay_skip_aga_present is set by a successful direct bitmap copy.
...
.not_active
	...
	bne	.done
	; Keep the confirmed c86zex/c86zey delay so wall columns are present before
	; opening the P96 screen.
	addq	#1,p96gameplay_delay_counter
	cmp	#12,p96gameplay_delay_counter
	blo	.done
...
.try_open
	...
	tst	p96gameplay_persist_active
	beq	.log_immediate
	; Draw the first P96 frame immediately.  If it succeeds, log the primary
	; P96 path once and let the caller skip this frame's AGA doc2p/db.
	jsr	g2p96_gameplay_persistent_update
	tst	p96gameplay_persist_active
	beq	.open_failed_after_update
	jsr	g2p96_gameplay_present_log
	bra	.done
.open_failed_after_update
	jsr	g2p96_gameplay_persistent_close
	jsr	g2p96_gameplay_present_log
	bra	.done
.log_immediate
...
g2p96_gameplay_persistent_open
	...
	move	#-1,p96gameplay_persist_frames	;c86zfa: primary gameplay path stays open
	bra	.done
.nomode
	move	#4,p96gameplay_state
	bra	.done
...
.frame_ok
	move	#1,p96gameplay_state
	move	#-1,p96gameplay_skip_aga_present	;c86zfa: caller may skip doc2p/db for this frame
.done
	pull
	rts
...
; a0 = Window.  c86zfa full-source P96-primary present.
