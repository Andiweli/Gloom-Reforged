...
.fskip	move	floorflag,floorflag2	;c86p: keep 2P swapped floor flag in sync for cfg/gameplay
	...
	bsr	refresh		;c86zft: redraw and keep P96 menu bridge visible after FLOOR toggle
	jsr	g2v17_wait_menu_release	;c86zft: do not auto-leave/re-trigger while fire is still held
	bra	.loop
.notfloor
	cmp	#5,d0
	bne.s	.notroof
...
.rskip	move	roofflag,roofflag2	;c86p: keep 2P swapped ceiling flag in sync for cfg/gameplay
	...
	bsr	refresh		;c86zft: redraw and keep P96 menu bridge visible after CEILING toggle
	jsr	g2v17_wait_menu_release	;c86zft: do not auto-leave/re-trigger while fire is still held
	bra	.loop
.notroof
	cmp	#7,d0
...
p96gameplay_msg_ok	dc.b	'P96 gameplay: ingame menu disabled rows return OK',10
...
.nextrow
	...
	bra.w	.rowloop
.postcopy
	jsr	g2p96_menu_dim_disabled_rows	;c86zft: grey VIEW SIZE/FULL SCREEN in visible P96 bridge
.done
	movem.l	(a7)+,d0-d7/a0-a6
	rts
