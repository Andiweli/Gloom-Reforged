	...
	;gloom2p96_c18: controlled P96 tonal pens, AGA unchanged.
	;
...
p96prevlog_name	dc.b	'RAM:gloom_p96_preview.txt',0
	even
p96prevlog_ok	dc.b	'P96 preview: 160x96 controlled tonal pens active',10
...
p96pens_set	dc	0	;gloom2p96_c18: controlled private preview pen ramp applied once
; gloom2p96_c18: RGB4 ramp for pens 8..15.  It keeps the low WB pens
; mostly alone and makes the tonal preview less dependent on random Workbench
; palette colours.  Values are dark-to-bright Gloom-like neutral/brown tones.
p96tonal_rgb4	dc	$000,$111,$221,$332,$443,$665,$997,$ccc
...
g2p96_test_window	push
	...
	clr	p96pens_set	;gloom2p96_c18: reapply tonal preview pens on new window
	...
	move.l	d7,a0
	bsr	g2p96_setup_tonal_pens	;gloom2p96_c18: make preview pens deterministic
	bra.s	.logonly
...
; gloom2p96_c18: configure pens 8..15 on the public P96/Workbench
; screen to a deterministic tonal ramp for the preview.  This is still only
; a preview/palette probe, not the final true-colour P96 path.  Failure is
; silent: if the screen pointer is missing, the old Workbench pens are used.
g2p96_setup_tonal_pens	push
	tst	p96pens_set
	bne.s	.done
	move.l	a0,d0
	beq.s	.done
	move.l	46(a0),d0	;Window.WScreen
	beq.s	.done
	move.l	d0,a4
	lea	44(a4),a0	;Screen.ViewPort
	move.l	grbase(pc),d0
	beq.s	.done
	move.l	d0,a6
	lea	p96tonal_rgb4(pc),a2
	moveq	#0,d4
.loop	cmp	#8,d4
	bge.s	.setdone
	move	0(a2,d4*2),d5
	move	d4,d0
	add	#8,d0		;use pens 8..15
	move	d5,d1
	and	#$0f00,d1
	lsr	#8,d1
	move	d5,d2
	...
	move	d5,d3
	and	#$000f,d3
	jsr	-288(a6)	;SetRGB4(ViewPort,pen,r,g,b)
	addq	#1,d4
	bra.s	.loop
.setdone
	move	#-1,p96pens_set
...
.nolog	movem.l	d0-d7/a0-a6,-(a7)
	...
	lsr	#3,d0
	and	#7,d0
	add	#8,d0		;gloom2p96_c18: heartbeat uses controlled pens 8..15
	jsr	-342(a6)	;SetAPen
	moveq	#0,d0
	moveq	#0,d1
	move	#319,d2
	...
	beq.w	.restore
	lea	coloffs(pc),a3
	; gloom2p96_c17: c16 proved that the 160x96 runfill grid is stable.
	; This doubles the preview grid to 160x96 with 2x2 destination blocks,
	; still merging adjacent same-pen blocks into horizontal runs.  The
	; update rate is raised to every 2nd frame as a load/smoothness test.
	; It should reduce graphics.library calls without changing the preview
	; semantics or touching the AGA game output.
	moveq	#0,d6		;preview grid row 0..47
.c15rowloop
	cmp	#96,d6
	bge	.c15done
	move	d6,d0
...
.c15xloop
	...
	; Convert the sampled palette index through the current Gloom RGB12
	; palette into a simple brightness bucket.  This still uses safe
	; Workbench/P96 pens only, but preserves the scene's light/dark
	; structure.  c15 only changes draw batching, not mapping.
	move.l	planar_palette(pc),a0
	tst.l	a0
	beq	.c15fallback
	move	d0,d1
	...
	bra	.c15paladdr
.c15ecs	add	d1,d1		;ECS palette entry = 1 word
.c15paladdr
	move	0(a0,d1.w),d2	;RGB12 high word
	move	d2,d1
	and	#$0f00,d1
	lsr	#8,d1		;R 0..15
	move	d2,d3
	...
	lsr	#2,d1		;bucket 0..11
	add	#8,d1		;gloom2p96_c18: use controlled preview pens 8..15
	cmp	#15,d1
	bls	.c15penok
	moveq	#15,d1
.c15penok
...
.c15fallback
	and	#7,d0
	add	#8,d0		;gloom2p96_c18 fallback: controlled pens 8..15
.c15mapped
	move	d0,d4		;new pen for this 4x4 block
	cmp	d7,d4
	beq	.c15samepen
	tst	d7
	bmi	.c15startrun
