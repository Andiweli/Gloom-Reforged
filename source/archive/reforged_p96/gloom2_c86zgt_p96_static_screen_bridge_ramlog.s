...
g2release_marker	dc.b	'Gloom Reforged AGA/P96 probe (v1.4 c86zgt 09.07.2026) Andreas ',39,'Andiweli',39,' Stuermer',0
	even
...
.show	movem.l	d0-d1,showbitmap
	move.l	todb,a0
	jsr	(a0)		;c86zgt: return here so static P96 bridge can present completed frames
	jsr	g2p96_static_present_showbitmap_if_bridge
	rts
...
p96gameplay_msg_ok	dc.b	'P96 gameplay: navguard bigfont + static bridge OK',10
...
g2p96_display_enter_title
	...
	; c86zgt: title/about/menu screens should now stay on the P96 screen.
	; Open or keep the existing P96 bridge screen and let db/showpic copy the
	; completed planar showbitmap to RTG after the old renderer finished it.
	jsr	g2p96_static_open_bridge_screen
...
g2p96_display_enter_intermission
	...
	moveq	#3,d0
	move	d0,p96display_event
	; c86zgt: keep/open P96 for intermission/script/static screens instead of
	; closing to AGA.  The picture/text is still prepared by the old planar code,
	; then copied to P96 only after it is complete.
	jsr	g2p96_static_open_bridge_screen
...
; -----------------------------------------------------------------------------
; c86zgt: P96 static-screen bridge for title/about/intermission/script screens.
; The old picture/text renderer still builds the complete planar showbitmap first.
; After db swaps the completed bitmap, this bridge converts that finished 320x240
; planar frame into the live P96 RGB565 screen.  This deliberately avoids any
; visible line-by-line decode on the P96 display and keeps AGA fallback intact.
; -----------------------------------------------------------------------------
...
g2p96_static_present_showbitmap
	...
	move.l	showbitmap,d0
	beq.w	.done
	move.l	d0,a4		; planar source frame
	move.l	p96winprobe_window_ptr,d0
	beq.w	.done
	move.l	d0,a5		; Window
	move.l	50(a5),d0	; Window.RPort
	...
	; Include Window offsets for completeness, although the custom P96 window is 0,0.
	moveq	#0,d0
	move	6(a5),d0		; Window.TopEdge
	mulu	d4,d0
	adda.l	d0,a3
...
; input: a4=planar showbitmap base, d6=y, d7=x. output: d1=palette index.
...
; Build a plain palette-index -> RGB565 LUT for complete static planar screens.
; Unlike the menu LUT, this must not override indices 0..3 with font colours.
