...
.waitskip2	; c86m: normal C2P/show path restored after c86l label regression
	jsr	doc2p
	jsr	db
	jsr	g2p96_gameplay_present_probe	;c86zej: mini-frame gameplay P96 diagnostic probe
	ifne	g2p96_overlay_enabled
...
; -----------------------------------------------------------------------------
; c86zej: first tiny blocky gameplay mini-frame after c86zei proved visible
...
; briefly from the gameplay context, draws a small 40x24 monochrome block map
...
p96gameplay_msg_ok	dc.b	'P96 gameplay: mini-frame OK',10
...
.have_p96
	...
	bra	.log
.try_marker
	jsr	g2p96_gameplay_mini_frame_probe
.log
...
g2p96_gameplay_mini_frame_probe
	push
	move.l	p96modeid,d0
	beq	.nomode
	move.l	d0,p96winprobe_displayid_tag+4
	...
	move.l	d4,a0
	bsr	g2p96_gameplay_draw_mini_frame
...
; mini-frame probe: a 40x24 blocky monochrome view sampled from the current
...
g2p96_gameplay_draw_mini_frame
	push
	move.l	a0,a5
	move.l	50(a5),a1	;Window.RPort
	move.l	a1,d0
	...
	moveq	#0,d7	;row 0..23
	move	#40,d5	;screen y start
.rowloop
	cmp	#24,d7
	bge	.done
	move	d7,d0
	mulu	#8,d0	;source y step, deliberately rough
	mulu	chunkymodw,d0
	lea	0(a3,d0.l),a2	;source row pointer
	moveq	#0,d6	;column 0..39
	move	#0,d4	;screen x offset in pixels
.colloop
	cmp	#40,d6
	bge	.nextrow
	move	d6,d0
	mulu	#8,d0	;source x step
	moveq	#0,d3
	...
	move	d5,d3
	add	#5,d3
	jsr	-306(a6)	;8x6 mini-frame cell
	add	#8,d4
	addq	#1,d6
	bra	.colloop
.nextrow
