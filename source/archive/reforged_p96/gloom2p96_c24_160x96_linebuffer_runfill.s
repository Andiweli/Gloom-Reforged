...
.osskipz	;
	...
	;
	;gloom2p96_c24: allocate one small row-staging buffer.  The visible
	;output still uses the proven RectFill/runfill path, but rows are now
	;copied through a linear 160-byte buffer first.  This is a safe bridge
	;towards a later WritePixelArray-style line output test.
	move.l	#160,d0
	moveq	#1,d1
...
p96prevlog_name	dc.b	'RAM:gloom_p96_preview.txt',0
	even
p96prevlog_ok	dc.b	'P96 preview: 160x96 linebuffer runfill active',10
...
p96preview_lineptr	dc.l	0	;gloom2p96_c24: one 160-byte row staging buffer
...
.nolog	movem.l	d0-d7/a0-a6,-(a7)
	...
	; gloom2p96_c24: line-staged buffered stage.  c17/c18a/c21 proved
...
.c22rowloop
	...
	move	d6,d0
	mulu	#160,d0
	;gloom2p96_c24: stage each finished preview row through a small
	;linear 160-byte buffer before drawing.  If allocation somehow failed,
	;fall back to the proven c22a direct-buffer row path instead of blocking
	;the game loop.
	move.l	p96preview_lineptr(pc),a3
	tst.l	a3
	beq.s	.c24directrow
	lea	0(a4,d0.l),a0	;source row in the 160x96 frame buffer
	move.l	a3,a2		;destination row staging buffer
	move	#159,d4
.c24copyrow
	move.b	(a0)+,(a2)+
	dbf	d4,.c24copyrow
	move.l	a3,a2		;draw from staged row
	bra.s	.c24rowready
.c24directrow
	lea	0(a4,d0.l),a2
.c24rowready
