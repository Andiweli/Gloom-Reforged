...
p96prevlog_name	dc.b	'RAM:gloom_p96_preview.txt',0
	even
p96prevlog_ok	dc.b	'P96 preview: c78 selective olive-green probe active',10
...
g2p96_c73_palready
	...
	; gloom2p96_c78: c77 hit the wrong texture spots.  Stay on the good
	; c75 colour anchor and only nudge a very small olive/green subset:
	; mid/dark entries where green is already dominant, blue is low and
	; the green lead over red/blue is small.  This aims at the slightly
	; warmer greenish Gloom texture tones without broad Shrek tinting.
	move	d6,d4
	move	d4,d0
	and	#$0f00,d0
	lsr	#8,d0		;R nibble
	...
	lsr	#4,d1		;G nibble
	move	d4,d2
	and	#$000f,d2		;B nibble
	cmp	#4,d1
	blo	.c78nogreen	;too dark
	cmp	#12,d1
	bhi	.c78nogreen	;too bright
	cmp	d0,d1
	ble	.c78nogreen	;G must already beat R
	cmp	d2,d1
	ble	.c78nogreen	;G must already beat B
	move	d1,d3
	sub	d0,d3
	cmp	#3,d3
	bhi	.c78nogreen	;too much green already
	move	d1,d3
	sub	d2,d3
	cmp	#2,d3
	blo	.c78nogreen	;avoid cyan-ish colours
	cmp	#5,d3
	bhi	.c78nogreen	
	cmp	#6,d2
	bhi	.c78nogreen	;blue must stay modest
	cmp	#$000f,d1
	beq	.c78nogreen	;already max green
	add	#$0010,d4
.c78nogreen
	move	d4,d6
