...
.waitskip2	; c86m: normal C2P/show path restored after c86l label regression
	jsr	doc2p
	jsr	db
	jsr	g2p96_gameplay_present_probe	;c86zek: 80x48 mini-frame gameplay P96 diagnostic probe
	ifne	g2p96_overlay_enabled
...
; -----------------------------------------------------------------------------
; c86zek: larger blocky 80x48 gameplay mini-frame after c86zej proved visible
; P96 drawing and live chunky-dependent stripes. This still does not skip the
; normal AGA doc2p/db present path. It opens the proven 320x240 P96 screen/window
; briefly from the gameplay context, draws a larger 80x48 monochrome block map
; sampled from the current chunky gameplay buffer, waits only a few video frames,
; closes everything, and logs the result to DH3.
; -----------------------------------------------------------------------------
...
p96gameplay_msg_ok	dc.b	'P96 gameplay: mini-frame 80x48 OK',10
...
.have_p96
	...
	bra	.log
.try_marker
	jsr	g2p96_gameplay_mini_frame_80x48_probe
.log
...
g2p96_gameplay_mini_frame_80x48_probe
	push
	move.l	p96modeid,d0
	beq	.nomode
	move.l	d0,p96winprobe_displayid_tag+4
	...
	move.l	d4,a0
	bsr	g2p96_gameplay_draw_mini_frame_80x48
	bsr	g2p96_gameplay_short_wait
	bra	.close_window
.nogfx
...
; a0 = Window. Deliberately primitive and pen-0/1 only.  This is the first
; mini-frame probe: an 80x48 blocky monochrome view sampled from the current
; chunky gameplay buffer.  It is intentionally one-shot and short-lived so the
; proven AGA present path, audio cadence and menu timing remain untouched.
g2p96_gameplay_draw_mini_frame_80x48
	push
	move.l	a0,a5
	move.l	50(a5),a1	;Window.RPort
	move.l	a1,d0
	...
	; fixed border/title/status bars so we can distinguish an empty chunky
	; sample from a failed P96 draw path.
	move.l	50(a5),a1
	move.l	p96gameplay_grbase,a6
	moveq	#1,d0
	jsr	-342(a6)	;SetAPen marker
	...
	move	#295,d2
	move	#207,d3
	jsr	-306(a6)	;bottom marker bar

	move.l	chunky,d0
	beq	.fallback
	move.l	d0,a3	;base chunky pointer
	moveq	#0,d7	;row 0..47
	move	#48,d5	;screen y start
.rowloop
	cmp	#48,d7
	bge	.done
	move	d7,d0
	mulu	#4,d0	;source y step, deliberately rough
	mulu	chunkymodw,d0
	lea	0(a3,d0.l),a2	;source row pointer
	moveq	#0,d6	;column 0..79
	move	#0,d4	;screen x offset in pixels
.colloop
	cmp	#80,d6
	bge	.nextrow
	move	d6,d0
	mulu	#4,d0	;source x step
	moveq	#0,d3
	move.b	0(a2,d0.w),d3	;live chunky pixel
	; high-bit/luma-ish bit plus coordinate xor so low-contrast scenes still show.
	lsr	#4,d3
	eor	d6,d3
	eor	d7,d3
	and	#1,d3
	...
	move	d5,d3
	add	#2,d3
	jsr	-306(a6)	;4x3 mini-frame cell
	add	#4,d4
	addq	#1,d6
	bra	.colloop
.nextrow
