...
; c86zfh: read HIRES=YES/NO and STRETCH=YES/NO from ToolTypes or CLI.
...
.have_p96
	...
	bne	.stretch_hires
	; STRETCH=YES + HIRES=NO/default: prefer 240-line widescreen modes
	; so the output can stay lowres while still filling a wide screen.
	move	#426,d0
	move	#240,d1
	move	#3,d2
	...
	tst.l	d0
	bne	.found
	; If no 240-line widescreen mode exists, fall back to 480-line stretch.
.stretch_hires
	move	#854,d0
	move	#480,d1
	move	#2,d2
...
; -----------------------------------------------------------------------------
; c86zfg: output mode ToolTypes for the now-confirmed P96 gameplay-primary path.
; Default stays 320x240.  HIRES=YES selects 640x480 with 2x nearest-neighbour
; output.  STRETCH=YES searches 240-line widescreen in lowres or 480-line widescreen in hires and stretches
; the same 320x240 game image horizontally to fill it.  Rendering, palette,
; lifecycle/menu/intermission handling and AGA fallback remain from c86zff.
; -----------------------------------------------------------------------------
...
p96gameplay_msg_ok	dc.b	'P96 gameplay: output modes LOWRES/HIRES/STRETCH OK',10
...
; c86zfb/c86zfg: black-fill the open direct RGB565 P96 bitmap during startup/menu handoff.
...
; c86zff/c86zfh: a0 = Window.  Overlay a small true 1:1 black/white pixel
...
.copy_stretch_480
	moveq	#0,d7	; destination x
	moveq	#0,d3	; source x accumulator index 0..319
	moveq	#0,d2	; fractional accumulator
.stretch480_loop
	cmp	p96target_width,d7
	bge.s	.nextrow
	move.l	0(a6,d3*4),d0
	...
	blo.s	.stretch480_next
	sub	p96target_width,d2
	addq	#1,d3
	cmp	#320,d3
	blo.s	.stretch480_next
	move	#319,d3
