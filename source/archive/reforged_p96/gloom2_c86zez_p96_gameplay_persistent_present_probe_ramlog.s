...
;gloom2_c86zdi: stronger near vertical oval, whole projectile/upgrade reflection Bayer-dithered, weapon-upgrade floor-touch pulse.
	...
	;c86zez: persistent bounded P96 gameplay present probe from c86zey anchor.
	;
...
.wait2	tst	showflag
	...
	bsr	vwait
	bra.s	.wait2
.waitskip2	; c86zez: bounded persistent P96 full-source gameplay present probe
	jsr	g2p96_gameplay_present_probe	;c86zez: persistent full-source AGA8 palette probe
	jsr	doc2p
	jsr	db
	ifne	g2p96_overlay_enabled
...
; -----------------------------------------------------------------------------
; c86zez: bounded persistent direct RGB565 full-source gameplay probe.
; Based on the confirmed c86zey anchor: delayed timing, full 320x240 source,
; old c86zds source-byte -> RGB565 LUT path, and AGA high+low palette precision.
; New in this step: keep the P96 screen/window open for a limited number of
; gameplay presents and refresh it once per hook, then close and log.  AGA
; presentation is still not skipped.
; -----------------------------------------------------------------------------
...
p96gameplay_delay_counter	dc	0	;c86zez: wait a few gameplay presents before starting persistent present
p96gameplay_persist_active	dc	0	;-1 while the bounded P96 gameplay screen is open
p96gameplay_persist_frames	dc	0	;remaining gameplay present updates before closing
...
p96gameplay_msg_ok	dc.b	'P96 gameplay: persistent present AGA8 palette OK',10
...
g2p96_gameplay_present_probe
	push
	; c86zez: if the bounded P96 screen is already open, refresh it once for
	; this gameplay present.  The AGA path still runs afterwards.
...
.try_open
	...
	tst	p96gameplay_persist_active
	beq	.log_immediate
	; Draw the first P96 frame immediately, then subsequent frames from later
	; gameplay hooks.  No blocking wait loop here.
	jsr	g2p96_gameplay_persistent_update
	bra	.done
.log_immediate
	jsr	g2p96_gameplay_present_log
...
g2p96_gameplay_persistent_open
	...
	move	#48,p96gameplay_persist_frames	;bounded probe: about a few seconds of gameplay presents
	bra	.done
.nomode
	move	#4,p96gameplay_state
	bra	.done
...
.fail_bitmap
	...
	bra	.done
.frame_ok
	subq	#1,p96gameplay_persist_frames
	bgt	.done
	move	#1,p96gameplay_state
	clr	p96gameplay_persist_active
.done
...
; a0 = Window.  c86zez full-source persistent present probe.
; Writes the full 320x240 chunky source through the c86zds source-byte ->
; RGB565 LUT directly into the open P96 bitmap. AGA presentation still runs
; afterwards; this is not yet the final P96-primary skip path.
