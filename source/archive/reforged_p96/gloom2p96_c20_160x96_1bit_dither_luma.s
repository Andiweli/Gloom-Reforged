	...
	;gloom2p96_c20: safe one-bit ordered-dither luma preview, no P96 pen/palette manipulation, AGA unchanged.
	;
...
p96prevlog_name	dc.b	'RAM:gloom_p96_preview.txt',0
	even
p96prevlog_ok	dc.b	'P96 preview: 160x96 1-bit dither luma active',10
...
p96dither4	dc.b	0,6,1,8,9,3,10,5,2,8,1,7,11,5,10,4	;gloom2p96_c20: 4x4 ordered dither thresholds, scaled to luma 0..11
	even
...
.c15fallback
	move	d0,d1
	and	#$000f,d1	;fallback pseudo-luma 0..15
	cmp	#11,d1
	ble	.c19luma
	moveq	#11,d1
.c19luma
	; gloom2p96_c20: still avoid all Workbench/P96 pen or palette changes,
	; but recover visible light/dark structure by ordered-dithering the
	; 0..11 luma value into a safe one-bit pen mask.  This uses only pen 0
	; and pen 1 like c19, so it should be as safe as the rollback, but with
	; far more perceived tonal steps.
	move	d6,d2
	and	#3,d2
	lsl	#2,d2
	move	d5,d3
	lsr	#1,d3		;grid x index, because each preview cell is 2 pixels wide
	and	#3,d3
	or	d3,d2		;4x4 Bayer-style threshold index
	lea	p96dither4(pc),a0
	moveq	#0,d3
	move.b	0(a0,d2.w),d3	;threshold 0..11
	cmp	d3,d1
	bge	.c20lit
	moveq	#0,d0
	bra	.c15mapped
.c20lit
	moveq	#1,d0
.c15mapped
	move	d0,d4		;new pen for this 2x2 block
	cmp	d7,d4
	beq	.c15samepen
	tst	d7
	bmi	.c15startrun
