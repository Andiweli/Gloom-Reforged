	...
	;gloom2p96_c86zeo: clean restart from c86zds.  Keep P96-primary gameplay, add a neutral static-buffer anchor only; no AGA fallback and no slow Planar->P96 menu bridge.
	;
...
.osskipz	;
	...
	move.l	d0,p96static_bufptr	;c86zeo: reuse the same 320x240 linear buffer outside gameplay for future static screens
	clr	p96static_state	;c86zeo: no static screen content yet; this patch is the clean base only
	;
	endc
	;
	move.l	#256*16,d0
...
p96prevlog_name	dc.b	'RAM:gloom_p96_preview.txt',0
	even
p96prevlog_ok	dc.b	'P96 preview: c86zes native 320x240 P96 screen active',10
...
p96preview_screen	dc.l	0	;c86zes: own native 320x240 P96/RTG screen pointer
p96static_state	dc	0	;c86zeo: future native P96 static-screen buffer state (0=empty)
p96static_bufptr	dc.l	0	;c86zeo: aliases the 320x240 mirror buffer for future menu/title rendering

; c86zes: real fullscreen attempt: own 320x240 custom Intuition screen, then
; a borderless 320x240 window on that screen.  This is no longer a Workbench
; overlay, so Workbench windows cannot cover the gameplay output.
p96test_screen	dc	0,0	;x,y
	dc	320,240	;w,h native target
	dc	16	;depth: force true-colour/RTG candidate instead of native AGA
	dc.b	0,1	;pens
	dc	0	;viewmode, let P96/intuition choose suitable monitor mode
	dc	15	;CUSTOMSCREEN, no CUSTOMBITMAP
	dc.l	0	;font
	dc.l	p96test_title	;title
	dc.l	0	;gadgets
	dc.l	0	;custombitmap

p96test_window	dc	0,0	;x,y on own P96 screen
	dc	320,240	;w,h
	dc.b	0,1	;pens
...
g2p96_test_window	push
	...
	clr	p96static_state	;c86zeo: static-buffer hooks are dormant in this clean base
	clr.l	p96preview_window
	clr.l	p96preview_screen
	clr.l	p96test_window_screen
	tst	p96present
	beq.w	g2p96_test_window_logonly
	move.l	int,d0
	beq.s	.fail
	move.l	d0,a6
	lea	p96test_screen(pc),a0
	jsr	-198(a6)	;c86zes: OpenScreen 320x240/16-bit custom RTG/P96 candidate
	move.l	d0,p96preview_screen
	beq.s	.fail
	move.l	d0,p96test_window_screen
	move.l	int,a6
	lea	p96test_window(pc),a0
	jsr	-204(a6)	;OpenWindow on own P96 custom screen
	move.l	d0,d7
	beq.s	.fail
	move	#1,p96window_state
	move	#1,p96preview_state
	move.l	d7,p96preview_window
	jsr	g2p96_hide_p96_pointer	;c86zdb: hide P96 mouse pointer inside output window
	jsr	g2p96_keep_preview_active	;c86zdc: activate P96 preview window immediately
	move.l	d7,a0
	bsr	g2p96_draw_testpattern
	jsr	g2p96_c71_init_bestpen_map	; c71: build RGB332->P96 best-pen map once
	jsr	g2p96_c73_init_palette_pen_map
	move.l	p96preview_window(pc),a0
	bsr	g2p96_draw_palette_hold_probe
	bra.w	g2p96_test_window_logonly
.fail	move.l	p96preview_screen,d0
	beq.s	.failnoscreen
	move.l	d0,a0
	move.l	int,d0
	beq.s	.failnoscreen
	move.l	d0,a6
	jsr	-66(a6)	;CloseScreen if OpenWindow failed
	clr.l	p96preview_screen
	clr.l	p96test_window_screen
.failnoscreen	move	#-1,p96window_state
g2p96_test_window_logonly	jsr	g2p96_window_log
...
.clearwin	clr.l	p96preview_window
	clr	p96preview_state
.checkscreen	move.l	p96preview_screen,d0
	beq.s	.done
	move.l	d0,a0
	move.l	int,d0
	beq.s	.clearscreen
	move.l	d0,a6
	jsr	-66(a6)	;c86zes: Close own P96 custom screen
.clearscreen	clr.l	p96preview_screen
	clr.l	p96test_window_screen
.done	pull
	rts
...
g2v36_hide_pointer	;force invisible pointer for the game screen/window
	...
	move.l	int,d1
	beq.s	.rts
	move.l	chipzero,a1	;v37/c86zet: pc-range fix, pointer image must live in chip RAM
	move.l	a1,d1
	beq.s	.rts
	move.l	d0,a0
...
db_ecs	move.l	coplist,a0	;c86zet: pc-range fix
	moveq	#40,d2
