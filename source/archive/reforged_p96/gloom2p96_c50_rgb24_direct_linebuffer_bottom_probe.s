...
p96prevlog_name	dc.b	'RAM:gloom_p96_preview.txt',0
	even
p96prevlog_ok	dc.b	'P96 preview: RGB24 direct linebuffer bottom probe active',10
...
p96preview_rgb24bottomptr	dc.l	0	;gloom2p96_c50: intentionally unused; no c48-style bottom buffer
...
.c43norgb24line
	...
	;gloom2p96_c50: direct RGB24 bottom probe without a dedicated 320x24x3 buffer.
	;Only every 4th sampled gameplay row is drawn into the 24-pixel mini-window
	;at the bottom.  The data source is the already prepared RGB24 scanline
	;buffer from c43, consumed immediately before it is overwritten by the next
	;row.  This keeps c49's safe allocation profile and avoids the c48 stale
	;bottom-buffer path that blocked after the Intermission screen.
	move	d6,d0
	and	#3,d0
...
.c50penok
	...
	move	d5,d0
	move	d6,d1
	lsr	#2,d1		;sample rows 0,4,8...92 -> bottom rows 0..23
	add	#216,d1		;bottom mini-window rows 216..239
	move	d5,d2
	addq	#1,d2
	move	d1,d3
...
.c22nextrow
	...
	bra	.c22rowloop
.c22done
	;gloom2p96_c50: no post-loop bottom redraw.  The 24-pixel mini-preview
	;is now drawn directly from the live RGB24 scanline buffer inside the
