	...
	;gloom2p96_c3: open preview window after Gloom screen + vwait heartbeat; AGA output unchanged.
	;
...
.osvwait	movem.l	d0-d1/a0-a1/a6,-(a7)
	...
	jsr	g2p96_preview_frame	;gloom2p96_c3: heartbeat from the reliable OS vwait path
	rts
...
p96prevlog_name	dc.b	'RAM:gloom_p96_preview.txt',0
	even
p96prevlog_ok	dc.b	'P96 preview: coarse chunky copy active',10
...
; gloom2p96_c3: open a persistent Workbench/P96 preview window after the Gloom screen.
...
g2p96_preview_frame	push
	...
	and	#3,d0		;update every 4th frame, still conservative
	bne.w	.done
...
.nolog	movem.l	d0-d7/a0-a6,-(a7)
	...
	move.l	chunky(pc),a4
	tst.l	a4
	beq.w	.heartbeat_only

	; Runtime heartbeat: if only this top strip changes colour, the frame
	; hook and window write path are OK, but the Chunky source sampling still
	; needs work.
.heartbeat_only
	move.l	50(a5),a1
	move.l	grbase(pc),a6
	...
	beq.w	.restore
	lea	coloffs(pc),a3
	moveq	#8,d6		;leave heartbeat visible in top 8 pixels
.yloop	cmp	#240,d6
	bge.w	.restore
	move	d6,d7
	mulu	chunkymodw(pc),d7
	move.l	a4,a2
	add.l	d7,a2		;row base in Chunky/C2P source layout
	moveq	#0,d5
.xloop	cmp	#320,d5
	bge.s	.nexty
	move.l	0(a3,d5*4),d4
	moveq	#0,d0
	move.b	0(a2,d4.l),d0	;sample source Chunky colour
	and	#7,d0
	addq	#1,d0		;map to visible pens 1..8
	move.l	50(a5),a1
	move.l	grbase(pc),a6
	...
	move.l	50(a5),a1
	move.l	grbase(pc),a6
	jsr	-306(a6)	;RectFill 8x8 sampled block
	addq	#8,d5
	bra.s	.xloop
.nexty	addq	#8,d6
	bra.s	.yloop
...
initdisplay	;
	...
	jsr	-408(a6)
	move.l	d0,int
	; gloom2p96_c3: open P96 preview window later, after the Gloom screen exists.
...
.agasc	move.l	int(pc),a6
	...
	move.l	d0,dbufinfo
	jsr	g2p96_test_window	;gloom2p96_c3: open preview window after game screen setup
	bra	db
	;
