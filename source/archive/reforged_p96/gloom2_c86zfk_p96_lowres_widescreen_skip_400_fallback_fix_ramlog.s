...
g2release_marker	dc.b	'Gloom Reforged AGA/P96 probe (v1.4 c86zfg 30.06.2026) Andreas ',39,'Andiweli',39,' Stuermer',0
	even
...
.nocd	;
	...
	;
.intro	jsr	g2p96_gameplay_force_close	;c86zfb: title/menu owns display again
...
.wmf	tst	fadevol
	...
	beq.s	.n22
	bsr	swaphflags
.n22	jsr	g2p96_gameplay_force_close	;c86zfb: do not leave last P96 frame over title/menu
	...
	bra	.intro
	;
exittoos	jsr	g2p96_gameplay_force_close	;c86zfb: close primary P96 screen before OS/display shutdown
...
dogamemenu	;
	jsr	g2p96_gameplay_force_close	;c86zfb: ESC menu must not stay behind frozen P96 gameplay
	move	#$20,$dff09a
	;
...
scripthide	; v190hy cleanup: log marker removed
	jsr	g2p96_gameplay_force_close	;c86zfc: script display owns screen
	clr	p96preview_gameactive
	bsr	dispoff
	bra	execscript

scriptshow	; v190hy cleanup: log marker removed
	jsr	g2p96_gameplay_force_close	;c86zfc: reveal AGA/intermission before dispon
	clr	p96preview_gameactive
...
scriptdraw	; v190hy cleanup: log marker removed
	jsr	g2p96_gameplay_force_close	;c86zfc: do not draw intermission behind P96 screen
	clr	p96preview_gameactive
...
scriptpict	;load an iff
	jsr	g2p96_gameplay_force_close	;c86zfc: picture scripts run on original display path
	clr	p96preview_gameactive
	bsr	freeiff
...
scriptdark	;
	jsr	g2p96_gameplay_force_close	;c86zfc: palette darken belongs to intermission/title display
	clr	p96preview_gameactive
...
scripttext	;print text on iff
	...
	;
	jsr	g2p96_gameplay_force_close	;c86zfc: text overlays must not remain behind P96 gameplay
	clr	p96preview_gameactive
...
.g2no_tele_black_hold
	...
	;
mainexit	st	paused
	jsr	g2p96_gameplay_force_close	;c86zfc: release P96 before script/intermission display
...
dointro	;
	jsr	g2p96_gameplay_force_close	;c86zfb: title menu must own visible display
...
.have_p96
	...
	bne	.stretch_hires
	; STRETCH=YES + HIRES=NO/default: try true 240-line widescreen,
	; but avoid the emulator-confirmed problematic 400x240 mode.
	; 426x240 is the closest 16:9-ish lowres target; 424/432 are aligned
	; fallbacks.  If none is available, use the already confirmed 480-line
	; widescreen path rather than the bad 400x240 output.
	move	#426,d0
	move	#240,d1
	move	#3,d2
	...
	tst.l	d0
	bne	.found
	; Do not try 400x240 here: it was detected, but produced repeated/striped
	; lowres stretch output.  Fall back to 480-line widescreen instead.
.stretch_hires
	move	#800,d0
	move	#480,d1
	move	#2,d2
...
finitdisplay	push
	;
	jsr	g2p96_gameplay_force_close	;c86zfb: safety close for primary P96 gameplay screen
...
inputoff	jsr	g2p96_gameplay_force_close	;c86zfb: close P96 gameplay on deactivate/menu/exit
	tst	active
	beq.s	.rts
	;
	move	#$4000,$dff09a
...
; -----------------------------------------------------------------------------
; c86zfk: lowres widescreen mode-order fix for the confirmed P96 gameplay path.
; Default stays 320x240.  HIRES=YES selects 640x480 with 2x nearest-neighbour
; output.  STRETCH=YES searches clean 240-line widescreen modes first in
; lowres, but skips the problematic 400x240 mode that produced repeated/striped
; output on emulator tests.  If no clean 240-line mode is found it falls back to
; the already confirmed 480-line widescreen path.
; -----------------------------------------------------------------------------
...
p96gameplay_msg_ok	dc.b	'P96 gameplay: lowres widescreen skip400/fallback OK',10
...
; c86zfc: close primary P96 gameplay display when title/menu/script/intermission paths take over.
