...
.osskipz	;
	...
	;
	;gloom2p96_c40: allocate a parallel 160x96 RGB12 word buffer.
	; This is only filled from the real Gloom palette for now; visible output
	; remains the proven Pen/Runfill path, so this first RGB step is safe.
	move.l	#160*96*2,d0
	moveq	#1,d1
...
p96prevlog_name	dc.b	'RAM:gloom_p96_preview.txt',0
	even
p96prevlog_ok	dc.b	'P96 preview: 320x192 rgb12 sample buffer prep active',10
...
p96preview_rgbptr	dc.l	0	;gloom2p96_c40: 160x96 RGB12 word sample buffer, no output yet
...
.c40skiprgbstore
	...
	move	d2,d3
	and	#$000f,d3	;B 0..15
	add	d3,d1		;brightness 0..45
	;gloom2p96_c39: keep the safe no-palette-init path and stretch
	;the sampled brightness over existing Workbench/P96 pens 1..255.
	;No SetRGB/pen allocation is used here; this remains the stable
	;visible 255-pen c39 path while c40 silently builds RGB12 samples.
	move	d1,d0		;brightness 0..45
	lsl	#2,d0		;*4
	add	d1,d0		;*5
	add	d1,d0		;*6 -> approx 0..270
	cmp	#254,d0
	ble.s	.c39rangeok
	move	#254,d0
.c39rangeok
	addq	#1,d0		;existing pen 1..255
	bra	.c22mapped
.c22fallback
