...
g2release_marker	dc.b	'Gloom Reforged AGA/P96 probe (v1.4 c86zgk 09.07.2026) Andreas ',39,'Andiweli',39,' Stuermer',0
	even
...
p96gameplay_msg_ok	dc.b	'P96 gameplay: turbo shadow P96 menu OK',10
...
; c86zgk: fast native P96 menu text renderer with a cheap shadow pass.
; c86zgi is the confirmed speed anchor, but its emergency chatfont path looked
; too flat.  c86zgj tried to parse the real bigfont2 shape records directly and
; produced unreadable enlarged garbage, so that experiment is rejected.  This
; version returns to the proven c86zgi chatfont renderer and adds only a tiny
; black drop-shadow/outline per glyph pixel.  It keeps the fast row/full-page
; cache flow and avoids the slow planar menubmap scanner.
...
; c86zgk: draw one 8x10 source-coordinate glyph from chatfont into the active
; P96 target mode.  Each 5-line chatfont glyph is doubled vertically like
; c86zgi, but every visible pixel first writes a one-pixel black shadow at
; x+1/y+1 before writing the real menu colour.  This makes the fast font read
; much closer to a chunky/bigfont menu without touching the failed c86zgj shape
; parser or the slow c86zge planar scanner.
...
.bit_loop
	...
	addq	#1,p96menu_native_px
	move	d1,p96menu_native_py
	addq	#1,p96menu_native_py
	cmp	#320,p96menu_native_px
	bge.s	.draw_main
	cmp	#240,p96menu_native_py
	bge.s	.draw_main
