...
p96prevlog_name	dc.b	'RAM:gloom_p96_preview.txt',0
	even
p96prevlog_ok	dc.b	'P96 preview: c86zeu7 native WPA band probe active',10
...
; ---- c86zeu7 native-screen p96WritePixelArray band probe ----
; This deliberately avoids both direct BitMap.Planes writes and p96LockBitMap.
; On real hardware, direct native-screen BitMap access reported BPR=$0028 and
; kept the 8 vertical test bars visible.  p96LockBitMap caused music whine and
; crashes around the in-game menu.  Therefore this probe writes only a small
; 320x16 gameplay band through the official P96 API.
;
...
p96wpalog_name	dc.b	'RAM:gloom_p96_wpa.txt',0
	even
p96wpalog_msg_ok	dc.b	'P96 WPA: c86zeu7 native 320x16 p96WritePixelArray band returned',10
p96wpalog_msg_ok_len	equ	*-p96wpalog_msg_ok
p96wpalog_msg_skip	dc.b	'P96 WPA: c86zeu7 skipped',10
...
p96wpa_desty	dc	112	;center-ish 16-line test band; leaves test bars visible around it
...
; Build a 320x16 RGB565 band from the real 320x240 chunky source, then ask
; P96 to copy it into the native window's RastPort.  The source buffer reuses
; p96preview_frameptr because it is already a large fast RAM allocation and is
; unused by the native direct writer path.
g2p96_native_wpa_band_probe
	movem.l	d0-d7/a0-a6,-(a7)
	; keep pressure low on real hardware: one small band every second hook
...
.lut_ready
	...
	; Fill 16 rows starting at p96wpa_desty into a compact 320*16*2 source.
	lea	coloffs,a6
	lea	p96rgb565_lut,a5
	move	p96wpa_desty,d6	;source/destination y base
	moveq	#0,d7		;band row 0..15
.row_loop
	cmp	#16,d7
	bge	.call_wpa
	move	d6,d0
...
.call_wpa
	; Prepare source RenderInfo in CPU memory.  Use the same RGB format that
	; the native P96 screen was requested with.  PC mode is expected first.
	move.l	a4,p96wpa_ri_memory
	move	#640,p96wpa_ri_bpr
	clr	p96wpa_ri_pad
	move.l	p96screen_rgbformat,p96wpa_ri_rgbformat
	move.l	p96base,d0
	beq	.skip
	move.l	d0,a6
	lea	p96wpa_ri_memory,a0	;RenderInfo source
	moveq	#0,d0		;SrcX
	moveq	#0,d1		;SrcY
	; a1 still holds Window.RPort from above
	moveq	#0,d2		;DestX
	move	p96wpa_desty,d3	;DestY
	move	#320,d4		;SizeX
	move	#16,d5		;SizeY
	jsr	-102(a6)	;p96WritePixelArray
	move	#1,p96wpa_state
