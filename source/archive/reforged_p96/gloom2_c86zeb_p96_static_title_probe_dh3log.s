...
;gloom2_c86zdi: stronger near vertical oval, whole projectile/upgrade reflection Bayer-dithered, weapon-upgrade floor-touch pulse.
	...
	;gloom2_c86zeb: P96 static title-marker probe on custom screen/window; no asset conversion/gameplay yet.
	;
...
.nocd	;
	...
	jsr	g2p96_static_buffer_probe	;c86zeb: static title-marker probe, then close
	move.l	4.w,a6
	moveq	#1,d1
	jsr	-216(a6)
...
; -----------------------------------------------------------------------------
; c86zeb: conservative static title-marker probe on the proven 320x240 P96
...
; title/status marker from RectFill blocks, waits briefly, closes everything,
; then logs DH3.
; -----------------------------------------------------------------------------
...
p96static_msg_ok	dc.b	'P96 static: title-marker OK',10
...
; a0 = Window. Draws a deliberately primitive title/static marker using only pens 0/1.
...
g2p96_static_wait
	push
	moveq	#90,d7	;about 1.5 seconds on PAL; long enough to verify the static-title marker
.wait
	move.l	p96static_grbase,d0
	beq	.done
	move.l	d0,a6
