	...
	;gloom2p96_c48: prepare/use a tiny 320x24 RGB24 2D bottom buffer from RGB12 samples.
	;
...
.osskipz	;
	...
	;
	;gloom2p96_c48: allocate a small 320x24 RGB24 2D bottom probe buffer.
	;It is filled from the true RGB12 samples, then read back for the visible
	;bottom test area.  Visible output is still Pen-based, not true-colour yet.
	move.l	#320*24*3,d0
	moveq	#1,d1
	allocmem	p96previewrgb24bottom
	move.l	d0,p96preview_rgb24bottomptr
	;
...
p96prevlog_name	dc.b	'RAM:gloom_p96_preview.txt',0
	even
p96prevlog_ok	dc.b	'P96 preview: RGB24 2D bottom buffer active',10
...
p96preview_rgb24bottomptr	dc.l	0	;gloom2p96_c48: tiny 320x24 RGB24 2D bottom probe buffer
...
.c22nextrow
	...
	bra	.c22rowloop
.c22done
	;gloom2p96_c48: dense 2D RGB24 bottom-buffer probe.  c47 proved
	;that the RGB12 samples form a recognizable 2D game image in the bottom
	;area.  This stage converts those same samples into a small 320x24 RGB24
	;buffer first, then draws the bottom mini-image from that RGB24 buffer.
	;The visible output is still Pen-based RGB332, so colours remain false;
	;the purpose is to prove a real 2D RGB24 staging buffer is live and spatially correct.
	move.l	p96preview_rgbptr(pc),d0
	beq	.restore
	move.l	d0,a4		;base of 160x96 RGB12 sample buffer
	move.l	p96preview_rgb24bottomptr(pc),d0
	beq	.restore
	move.l	d0,a3		;base of 320x24 RGB24 bottom probe buffer
	moveq	#0,d6		;bottom probe row 0..23
.c48rowloop
	cmp	#24,d6
	bge	.restore
	move	d6,d0
	mulu	#4,d0		;sample rows 0,4,8...92 from the RGB12 buffer
	mulu	#320,d0		;160 RGB12 words = 320 bytes per row
	lea	0(a4,d0.l),a2
	move	d6,d0
	mulu	#960,d0		;320 RGB24 pixels = 960 bytes per bottom row
	lea	0(a3,d0.l),a0
	moveq	#0,d5		;destination x = 0..319 in 2-pixel blocks
	moveq	#0,d7		;source byte offset, +2 per 2px block
.c48xloop
	cmp	#320,d5
	bge	.c48nextrow
	moveq	#0,d0
	move	0(a2,d7.w),d0	;RGB12 = 0000rrrrggggbbbb
	move	d0,d4
	and	#$0f00,d4
	lsr	#8,d4		;R nibble 0..15
	move	d4,d3
	lsl	#4,d4
	or	d3,d4		;R byte 0..255
	move	d0,d1
	and	#$00f0,d1
	lsr	#4,d1		;G nibble 0..15
	move	d1,d3
	lsl	#4,d1
	or	d3,d1		;G byte 0..255
	move	d0,d2
	and	#$000f,d2	;B nibble 0..15
	move	d2,d3
	lsl	#4,d2
	or	d3,d2		;B byte 0..255
	;store two RGB24 pixels for this 2px probe block
	move.b	d4,(a0)+
	move.b	d1,(a0)+
	move.b	d2,(a0)+
	move.b	d4,(a0)+
	move.b	d1,(a0)+
	move.b	d2,(a0)+
	;derive a visible RGB332 Pen candidate from the staged RGB24 values.
	;This is still not true-colour output; it only reads from the RGB24 buffer path.
	move	d4,d0
	and	#$00e0,d0	;R top 3 bits already in bits 7..5
	move	d1,d3
	and	#$00e0,d3
...
.c48penok
	move.l	50(a5),a1
	move.l	grbase(pc),a6
	jsr	-342(a6)	;SetAPen(RGB332 Pen candidate)
	move	d5,d0
	move	d6,d1
	add	#216,d1		;bottom mini-window rows 216..239, 1 row per RGB24 row
	move	d5,d2
	addq	#1,d2
	move	d1,d3
	jsr	-306(a6)	;RectFill 2x1 probe block
	addq	#2,d5
	addq	#2,d7		;next RGB12 sample per 2px block
	bra	.c48xloop
.c48nextrow
	addq	#1,d6
	bra	.c48rowloop

	;gloom2p96_c32: keep the framebuffer readback probe continuous again,
	;but make it compact and place it below the main 160x96 preview area.
	;c31 throttling caused rhythmic appearing/disappearing because the main
	;preview overwrote the lower probe area on frames where the probe was not
	;redrawn.  This version draws a smaller 24px bottom window every preview
	;update, using the same safe Runfill/RectFill path and no palette changes.
	move.l	p96preview_frameptr(pc),d0
	beq	.restore
	move.l	d0,a4		;base of hidden 320x192 framebuffer
	moveq	#0,d6		;window probe row 0..11 (24px high)
.c29bandloop
	cmp	#12,d6
	bge	.restore
	move	d6,d0
	mulu	#16,d0		;source framebuffer rows 0..176
	mulu	#320,d0
	lea	0(a4,d0.l),a2
	moveq	#0,d5		;destination x = 0..319
	moveq	#-1,d7		;current run pen, -1 = no run yet
	clr	p96run_start
.c29xloop
	cmp	#320,d5
	...
	move.l	grbase(pc),a6
	move	d7,d0
	jsr	-342(a6)	;SetAPen(previous band run pen)
	move	p96run_start(pc),d0
	move	d6,d1
	add	d1,d1
	add	#216,d1
	...
	move	d1,d3
	addq	#1,d3
	jsr	-306(a6)	;RectFill bottom framebuffer band run
.c29startrun
	move	d4,d7
	move	d5,p96run_start
.c29samepen
...
.c29flushrow
	...
	move.l	grbase(pc),a6
	move	d7,d0
	jsr	-342(a6)	;SetAPen(final band run pen)
	move	p96run_start(pc),d0
	move	d6,d1
	add	d1,d1
	add	#216,d1
	...
	move	d1,d3
	addq	#1,d3
	jsr	-306(a6)	;RectFill final bottom framebuffer band run
.c29nextband
	addq	#1,d6
	bra	.c29bandloop
