...
p96prevlog_name	dc.b	'RAM:gloom_p96_preview.txt',0
	even
p96prevlog_ok	dc.b	'P96 preview: c86zej zeg-audio noclick probe active',10
...
g2p96_workbench_hold_probe	push
	; c86zej: audio-clean c86zeg base, but remove only the early grey click hold.
	; Do one WaitTOF so the P96 window settles, then let the normal title path run.
	move.l	p96preview_window(pc),d0
	beq.s	.done
	move.l	grbase(pc),d0
	beq.s	.done
