...
;gloom2_c86zdi: stronger near vertical oval, whole projectile/upgrade reflection Bayer-dithered, weapon-upgrade floor-touch pulse.
	...
	;gloom2_c86zeg: safe rollback after failed c86zef holdscreen; one-shot frame marker only.
	;
...
g2release_marker	dc.b	'Gloom Reforged AGA/P96 probe (v1.4 c86zeg 29.06.2026) Andreas ',39,'Andiweli',39,' Stuermer',0
	even
...
.waitskip2	; c86m: normal C2P/show path restored after c86l label regression
	jsr	doc2p
	jsr	db
	jsr	g2p96_gameplay_present_probe	;c86zeg: safe one-shot visible gameplay P96 frame-marker probe
	ifne	g2p96_overlay_enabled
...
; -----------------------------------------------------------------------------
; c86zeg: safe rollback after failed c86zef holdscreen.  This keeps the
; renderer and it still does not skip the normal AGA doc2p/db present path.  It
; is called once from the one-player show path after doc2p/db, opens the proven
; 320x240 P96 screen/window briefly, draws a simple frame marker, waits only a
; few video frames, closes everything, and logs the result to DH3.
; -----------------------------------------------------------------------------
p96gameplay_state		dc	0	;0=fail, 1=frame marker OK, 2=skip AGA, 3=no P96 lib, 4=no ModeID, 5=no intuition, 6=screen fail, 7=window fail, 8=no graphics
...
p96gameplay_msg_ok	dc.b	'P96 gameplay: frame-marker safe rollback OK',10
...
.have_p96
	...
	bra	.log
.try_marker
	jsr	g2p96_gameplay_frame_marker_probe
.log
...
g2p96_gameplay_frame_marker_probe
	push
	move.l	p96modeid,d0
	beq	.nomode
	move.l	d0,p96winprobe_displayid_tag+4
