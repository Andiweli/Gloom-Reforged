...
.waitskip2	; c86m: normal C2P/show path restored after c86l label regression
	jsr	doc2p
	jsr	db
	jsr	g2p96_gameplay_present_probe	;c86zei: visible gameplay stripe diagnostic probe
	ifne	g2p96_overlay_enabled
...
; the result to DH3:gloom_displaymode.txt.  It does not open P96, does not
...
iconname	dc.b	'icon.library',0
	even
g2display_log_name	dc.b	'DH3:gloom_displaymode.txt',0
	even
...
p96log_name	dc.b	'DH3:gloom_p96_probe.txt',0
	even
...
p96modeid_log_name	dc.b	'DH3:gloom_p96_modeid.txt',0
	even
...
p96screen_log_name	dc.b	'DH3:gloom_p96_screen.txt',0
...
; Write one compact status line to DH3:gloom_displaymode.txt.  This is the
...
.got	move.l	d0,p96base
	move	#-1,p96present
.done	jsr	g2p96_probe_log	;c86zdw: write DH3:gloom_p96_probe.txt
	pull
	rts
...
; p96BestModeIDTagList() and writes DH3:gloom_p96_modeid.txt.
...
p96winprobe_log_name	dc.b	'DH3:gloom_p96_window.txt',0
...
p96input_log_name	dc.b	'DH3:gloom_p96_input.txt',0
...
p96input_msg_skipped_len	equ	*-p96input_msg_skipped
	even
p96static_log_name	dc.b	'DH3:gloom_p96_static.txt',0
...
; -----------------------------------------------------------------------------
; c86zei: visible stripe diagnostic after c86zeh only showed grey. This still
; does not skip the normal AGA doc2p/db present path.  It is called once from
; the one-player show path after doc2p/db, opens the proven 320x240 P96
; screen/window briefly, draws a small stripe sampled from the current chunky
; gameplay buffer plus fixed calibration stripes, waits only a few video frames, closes everything, and logs
; the result to DH3.
; -----------------------------------------------------------------------------
p96gameplay_state		dc	0	;0=fail, 1=visible stripe OK, 2=skip AGA, 3=no P96 lib, 4=no ModeID, 5=no intuition, 6=screen fail, 7=window fail, 8=no graphics
p96gameplay_latched		dc	0	;one-shot latch so disk logging happens only once
p96gameplay_grbase		dc.l	0
p96gameplay_log_name	dc.b	'DH3:gloom_p96_gameplay.txt',0
p96gameplay_msg_ok	dc.b	'P96 gameplay: visible-stripe OK',10
...
.have_p96
	...
	bra	.log
.try_marker
	jsr	g2p96_gameplay_chunky_stripe_probe
.log
...
g2p96_gameplay_chunky_stripe_probe
	push
	move.l	p96modeid,d0
	beq	.nomode
	move.l	d0,p96winprobe_displayid_tag+4
	...
	move.l	d4,a0
	bsr	g2p96_gameplay_draw_chunky_stripe
	push
	move.l	a0,a5
	move.l	50(a5),a1	;Window.RPort
	move.l	a1,d0
	...
	; fixed output calibration frame: proves RectFill/Pen drawing is visible
	move.l	50(a5),a1
	move.l	p96gameplay_grbase,a6
	moveq	#1,d0
	jsr	-342(a6)	;SetAPen marker
	...
	move	#319,d2
	move	#9,d3
	jsr	-306(a6)	;top calibration bar
	move.l	50(a5),a1
	move.l	p96gameplay_grbase,a6
	moveq	#0,d0
	move	#230,d1
	move	#319,d2
	move	#239,d3
	jsr	-306(a6)	;bottom calibration bar

	; fixed alternating calibration stripe at y=88..103.
	; If this is not visible, the problem is the P96 draw path, not chunky.
	moveq	#0,d6
	moveq	#0,d5
.calibloop
	cmp	#32,d6
	...
	move.l	p96gameplay_grbase,a6
	move	d4,d0
	jsr	-342(a6)	;SetAPen 0/1 alternating
	move.l	50(a5),a1
	move.l	p96gameplay_grbase,a6
	move	d5,d0
	move	#88,d1
...
.aftercalib
	...
	move	#120,d0
	mulu	chunkymodw,d0
	lea	0(a2,d0.l),a2	;sample one middle gameplay row

	; raw chunky parity stripe at y=112..127.
	; This can be invisible if the sampled row happens to have only even values.
	moveq	#0,d6
	moveq	#0,d5
.rawloop
	cmp	#32,d6
	...
	mulu	#10,d0
	moveq	#0,d4
	move.b	0(a2,d0.w),d4	;live chunky pixel
	and	#1,d4
	move.l	50(a5),a1
	move.l	p96gameplay_grbase,a6
	move	d4,d0
	...
	; chunky xor-index stripe at y=136..151.
	; This stays visible even when the raw sampled row is mostly zero/even,
	; while still touching the real chunky buffer.
.xorprep
	moveq	#0,d6
	moveq	#0,d5
.xorloop
...
.fallback
	...
	move	#279,d2
	move	#151,d3
	jsr	-306(a6)	;fallback visible stripe if no chunky pointer exists
.done
	pull
	rts

g2p96_gameplay_short_wait
	bra	.close_window
.nogfx
...
; a0 = Window.  Deliberately primitive, pen-0/1 only, so this tests the
; gameplay-time P96 path plus a real read from the current chunky frame without
; depending on palette conversion or the old Workbench overlay renderer.
g2p96_gameplay_draw_chunky_stripe
	push
	move.l	a0,a5
	move.l	50(a5),a1	;Window.RPort
	move.l	a1,d0
	...
	mulu	chunkymodw,d0
	lea	0(a2,d0.l),a2	;sample one middle gameplay row
	moveq	#0,d6		;sample index 0..31
	moveq	#0,d5		;screen x = sample index * 10
.sampleloop
	cmp	#32,d6
	bge	.done
	move	d6,d0
	mulu	#10,d0		;sample x roughly matches block start
	moveq	#0,d4
	move.b	0(a2,d0.w),d4	;live chunky pixel
	and	#1,d4		;pen 0/1 only, safe on any P96 depth
	move.l	50(a5),a1
	move.l	p96gameplay_grbase,a6
	move	d4,d0
	jsr	-342(a6)	;SetAPen sampled bit
	move.l	50(a5),a1
	move.l	p96gameplay_grbase,a6
	move	d5,d0
	move	#104,d1
	...
	add	#9,d2
	move	#135,d3
	jsr	-306(a6)	;10x32 block from chunky parity
	add	#10,d5
	addq	#1,d6
	bra	.sampleloop
.fallback
	...
	move	#215,d2
	move	#127,d3
	jsr	-306(a6)	;fallback stripe if no chunky pointer exists
.done
	pull
	rts
...
g2p96_gameplay_short_wait
	push
	moveq	#18,d7	;short visible gameplay-time P96 marker, then back to AGA
.wait
	move.l	p96gameplay_grbase,d0
	beq	.done
	move.l	d0,a6
