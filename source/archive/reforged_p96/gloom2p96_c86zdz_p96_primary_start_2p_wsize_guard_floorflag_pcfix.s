...
.play	;
	...
	ifne	g2p96_overlay_enabled
	jsr	g2p96_force_fullscreen_view	;c86zdy: stable P96-primary gameplay view
	endc
...
.done	jsr	g2cfg_save	;v141: save ingame menu settings when leaving menu
	...
	ifne	g2p96_overlay_enabled
	jsr	g2p96_force_fullscreen_view	;c86zdy: avoid displaced/doubled non-fullscreen P96 output
	endc
	jsr	predrawall
	jsr	dispon
...
; c86zdy: P96-primary is now a native 320x240 direct framebuffer path.
; Smaller AGA WINDOW SIZE modes still preview correctly inside the menu via
; the planar bridge, but the active direct RGB path uses full-width coloffs
; and a full 320x240 source.  Until a dedicated RTG crop/scaler exists, force
; gameplay back to fullscreen to avoid displaced/doubled output after ESC.
g2p96_force_fullscreen_view
	...
	move	#120,maxy
	clr.l	offset
	jsr	g2twop_restore_coloffs	; rebuild full 320-column lookup used by P96 copy
	jsr	trainer_update_display_texts	; keep next ESC menu honest: FULLSCREEN
.rts
	endc
	rts
