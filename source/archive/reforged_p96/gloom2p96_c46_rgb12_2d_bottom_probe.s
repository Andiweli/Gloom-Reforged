...
p96prevlog_name	dc.b	'RAM:gloom_p96_preview.txt',0
	even
p96prevlog_ok	dc.b	'P96 preview: RGB12 2D bottom probe active',10
...
.c22nextrow
	...
	bra	.c22rowloop
.c22done
	;gloom2p96_c46: replace the c45 single-line RGB24 composite smear with
	;a small true 2D probe built from multiple rows of the hidden RGB12 sample
	;buffer.  The output still uses existing Pens through SetAPen/RectFill, so
	;this is not true-colour yet, but it should no longer look like one floor
	;texture line wiped downward.  It reads 12 separate RGB12 source rows and
	;draws them as a 24px bottom mini-window, two display rows per source row.
	move.l	p96preview_rgbptr(pc),d0
	beq	.restore
	move.l	d0,a4		;base of 160x96 RGB12 sample buffer
	moveq	#0,d6		;bottom probe source row index 0..11
.c46rowloop
	cmp	#12,d6
	bge	.restore
	move	d6,d0
	mulu	#8,d0		;sample rows 0,8,16...88 from the RGB12 buffer
	mulu	#320,d0		;160 RGB12 words = 320 bytes per row
	lea	0(a4,d0.l),a2
	moveq	#0,d5		;destination x = 0..319 in 4-pixel blocks
	moveq	#0,d7		;source byte offset, +4 per 4px block
.c46xloop
	cmp	#320,d5
	bge	.c46nextrow
	moveq	#0,d0
...
.c46penok
	...
	move	d1,d3
	addq	#1,d3
	jsr	-306(a6)	;RectFill 4x2 probe block
	addq	#4,d5
	addq	#4,d7		;skip two RGB12 samples per 4px block
	bra	.c46xloop
.c46nextrow
	addq	#1,d6
	bra	.c46rowloop
