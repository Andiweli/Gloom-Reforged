...
.done	; v190hy cleanup: log marker removed
	jsr	printmess2
	jsr	g2p96_static_present_intermission_text_native_guard	;c86zjl: planar text changed, invalidate direct picture indices first
	;
	tst	pdelay
	bmi	execscript
	clr	pdelay
...
g2p96_intermission_typewriter_char_if_bridge
	...
	beq	.done
	move	pdelay,d0
	cmp	#1,d0	;c86zhg: only typewriter pdelay, not pdelay=0 text draws
	blt	.done
	; Wait for the character blit to finish before sampling showbitmap.
	btst	#6,$dff002
.wait	btst	#6,$dff002
	bne.s	.wait
