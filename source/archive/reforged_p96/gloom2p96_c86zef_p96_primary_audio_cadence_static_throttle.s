	...
	;gloom2p96_c86zef: restore c86zdx gameplay audio cadence; throttle static P96 menu/text flushes.
	;
...
.intro	clr	p96preview_gameactive	;gloom2p96_c14: title/menu phase, do not preview stale game buffer
	...
	; c86zea: do not blank the P96 window at the start of the title loop.
	; The early clear made the boot/splash phase appear as a grey/empty page
	; until the first menu input.  Real title/menu frames are flushed below.
	;jsr	g2p96_clear_preview_window
	endc
...
.play	;
	...
	; c86zef: restore c86zdx gameplay entry cadence for audio test.
	; Do not add an extra full chunky clear here.
...
swaphflags	lea	floorflag,a0	; c86zdz: GenAm range fix, avoid floorflag(pc)
	movem.l	(a0),d0-d1
	move.l	d1,floorflag
...
.done	jsr	g2cfg_save	;v141: save ingame menu settings when leaving menu
	...
	; c86zef: restore c86zdx menu-exit cadence for audio test.
	jsr	predrawall
	jsr	dispon
...
; c86zdy introduced a P96-primary fullscreen guard.  c86zea keeps the
; label for compatibility but turns it into a no-op: WINDOW SIZE is active
; again, while stale border pixels are handled by clearing the chunky backing
; store before/while non-fullscreen P96 frames are copied.
g2p96_force_fullscreen_view
	rts
...
drawall	;
	; c86zef: restore c86zdx cadence.  The last no-whine version still hid
	; the OS/hardware pointer every gameplay frame; do the same before blaming
	; the RGB transfer itself.
...
drawall_	tst	twowins		; c54/c86m: route 2P into split path, keep normal 1P untouched
	...
	ifne	g2p96_overlay_enabled
	; c86zea: if WINDOW SIZE is smaller than fullscreen, clear the whole
	; chunky source before rendering the smaller view.  The direct P96 copy
	; still transfers 320x240, so this prevents stale doubled/deplaced pixels.
	tst	p96preview_gameactive
	beq.s	.g2p96_wsize_clear_done
	cmp	#320,width
	bne.s	.g2p96_wsize_clear
	cmp	#240,hite
	beq.s	.g2p96_wsize_clear_done
.g2p96_wsize_clear
	jsr	g2clearfullchunky
.g2p96_wsize_clear_done
	endc
...
.waitskip2	; c86m: normal C2P/show path restored after c86l label regression
	...
	; c86zef: restore c86zdx front/window cadence for the audio comparison.
...
g2v15_clear_bottom16	;clear bottom 16 visible OS lines below the 240-line game render
	...
	move	bitplanes,d1	;c86zdu: pc-range fix
	subq	#1,d1
g2v15_cb16_plane
	move.l	a0,a1
	add.l	#40*240,a1
...
.here	move.l	font(pc),a0
	...
	;
	ifne	g2p96_overlay_enabled
	; c86zef: the full-screen planar->P96 blit is too slow on real Amiga.
	; Keep the intermission crawl alive, but flush only every 8th character
	; instead of after every glyph.
	tst	p96preview_gameactive
	bne.s	.g2p96_no_char_flush
	tst	pdelay
	ble.s	.g2p96_no_char_flush
	addq	#1,p96static_flush_ctr
	move	p96static_flush_ctr,d0
	and	#7,d0
	bne.s	.g2p96_no_char_flush
	jsr	g2p96_present_planar_showbitmap_to_window
.g2p96_no_char_flush
	endc
	;
	move	pdelay(pc),d2
...
pokepal2	;
	ifne	g2p96_overlay_enabled
	jsr	g2p96_update_active_palette	;c86zdv: mirror partial/full AGA palette updates for P96 menu/title blits
	endc
...
.loop	move.l	d0,(a1)+
	...
	bsr	db
	ifne	g2p96_overlay_enabled
	jsr	g2p96_present_planar_showbitmap_to_window	;c86zdt: mirror blank/menu planar frame to P96
	endc
	rts
...
.nopal	bsr	db
	ifne	g2p96_overlay_enabled
	jsr	g2p96_present_planar_showbitmap_to_window	;c86zdt: show IFF/title/intermission on P96
	endc
	bra	vwait
...
.nopal	bsr	db
	ifne	g2p96_overlay_enabled
	jsr	g2p96_present_planar_showbitmap_to_window	;c86zdt: show noclear IFF/title on P96
	endc
	bra	vwait
...
.agaa	bsr	loadfiles
	...
	bsr	dispon
	ifne	g2p96_overlay_enabled
	clr	p96preview_gameactive	;c86zdw: BlackMagic is a P96 static page, not gameplay
	jsr	g2p96_workbench_front_for_preview	;c86zdw: keep RTG/P96 front after custom screen setup
	jsr	g2p96_present_planar_showbitmap_to_window	;c86zee: one BlackMagic blit only
	; c86zee: on real Amiga the repeated Planar->P96 flush built the splash
	; line-by-line and could appear to hang.  Keep it briefly, but do not
	; redraw it inside the hold loop.  ENTER/fire/mouse can skip it.
	move	#36,d7	;about half the previous visible time
.g2p96_bm_hold
	jsr	checkany	;includes vwait; NE when RETURN/fire/mouse selects
	bne.s	.g2p96_bm_skip
	dbf	d7,.g2p96_bm_hold
.g2p96_bm_skip
	endc
	;
...
; c86zdy: temporary P96-primary guard.  The one-player RTG path is now solid,
; but two-player still uses the old split/C2P assumptions and can show black
; or crash while the P96 window stays frontmost.  Block it cleanly until a
; dedicated P96 split renderer is added.
g2p96_twopnokmenu	dc.b	1
	dc.b	'P96 TWO PLAYER NOT READY',0
	even

initnewgame	;
	ifne	g2p96_overlay_enabled
	; c86zea final safety net: P96-primary does not enter old 2P/combat init.
	tst.l	p96preview_window
	beq.s	.g2p96_init_2p_ok
	tst	gametype
	beq.s	.g2p96_init_2p_ok
	move	#-1,gametype
	rts
...
scriptshow	; v190hy cleanup: log marker removed
	...
	bsr	dispon
	ifne	g2p96_overlay_enabled
	clr	p96preview_gameactive	;c86zdw: script-show page is static P96, not gameplay
	jsr	g2p96_present_planar_showbitmap_to_window
	endc
	bra	execscript
...
.done	; v190hy cleanup: log marker removed
	jsr	printmess2
	ifne	g2p96_overlay_enabled
	jsr	g2p96_present_planar_showbitmap_to_window	;c86zdt: intermission text to P96
	endc
	;
	tst	pdelay
	bmi	execscript
...
.nolmed	;
	...
	bsr	dispon
	ifne	g2p96_overlay_enabled
	move	#-1,p96preview_gameactive	;c86zdw: each scriptplay re-enters P96 gameplay mode
	endc
	bsr	chaton
	;
...
mainexit	st	paused
	ifne	g2p96_overlay_enabled
	clr	p96preview_gameactive	;c86zdw: following scriptdraw/scripttext must blit intermission to P96
	endc
	bsr	chatoff
	bsr	dispoff
	bsr	qsync2
...
dointro	;
	...
	ifne	g2p96_overlay_enabled
	; c86zea: do not clear here; the title picture is flushed immediately
	; below, and clearing first caused a grey pre-click page in P96-primary.
	;jsr	g2p96_clear_preview_window
	endc
	...
	ifne	g2p96_overlay_enabled
	; c86zea: flush the pre-menu title page immediately.  Without this the
	; P96 window can remain grey/blank until the first mouse/keyboard click.
	jsr	g2p96_present_planar_showbitmap_to_window
	endc
...
.sel	jsr	selmenu
	;
	ifne	g2p96_overlay_enabled
	; c86zea: hard-block all title-menu 2P starts while P96-primary is active.
	; The earlier qmenu warning could re-enter with stale menu colours and one
	; path still reached the old split renderer.  For now, these rows simply
	; do nothing until a dedicated P96 split renderer exists.
	tst.l	p96preview_window
	beq.s	.g2p96_2p_sel_ok
	move	d0,d1
	and	#$00ff,d1
	tst	linked
	bne.s	.g2p96_2p_linked_check
	cmp	#1,d1
	beq	.g2p96_2p_block_msg
	cmp	#2,d1
	beq	.g2p96_2p_block_msg
	bra.s	.g2p96_2p_sel_ok
.g2p96_2p_linked_check
	cmp	#0,d1
	beq	.g2p96_2p_block_msg
	cmp	#1,d1
	beq	.g2p96_2p_block_msg
...
.newgame_maptest
	ifne	g2p96_overlay_enabled
	; c86zea fallback: if any 2P value reaches here, ignore it cleanly.
	tst.l	p96preview_window
	beq.s	.g2p96_2p_ok
	move	d0,d1
	cmp	#1,d1
...
.g2v158_no_title_credit
	...
	bsr	db
	ifne	g2p96_overlay_enabled
	jsr	g2p96_present_planar_showbitmap_to_window	;c86zdt: menu text/composite to P96
	endc
	bra	vwait
...
.bwait	btst	#6,$dff002
	...
	bsr	disownblitter
	ifne	g2p96_overlay_enabled
	; c86zef: do not full-blit the P96 menu on every flash-off tick.
	; Real hardware spent seconds in this path, so only refresh periodically.
	addq	#1,p96static_flush_ctr
	move	p96static_flush_ctr,d0
	and	#3,d0
	bne.s	.g2p96_skip_optoff_flush
	jsr	g2p96_present_planar_showbitmap_to_window
.g2p96_skip_optoff_flush
	endc
	rts
...
.loop	addq	#1,d0
	...
	move	#13,flashdelay
	ifne	g2p96_overlay_enabled
	; c86zef: throttle full-screen P96 menu flash-on blits too.
	addq	#1,p96static_flush_ctr
	move	p96static_flush_ctr,d0
	and	#3,d0
	bne.s	.g2p96_skip_opton_flush
	jsr	g2p96_present_planar_showbitmap_to_window
.g2p96_skip_opton_flush
	endc
	;
	rts
...
p96prevlog_name	dc.b	'RAM:gloom_p96_preview.txt',0
	even
p96prevlog_ok	dc.b	'P96 preview: c86zef audio-cadence/static-throttle active',10
...
p96static_flush_ctr	dc	0	;c86zef: throttle slow full-screen static/menu P96 flushes on real Amiga
...
g2p96_test_window	push
	...
	jsr	g2p96_hide_p96_pointer	;c86zdb: hide Workbench/P96 mouse pointer inside overlay window
	jsr	g2p96_keep_preview_active	;c86zdc: activate P96 preview window immediately
	move.l	d7,a0
	bsr	g2p96_draw_testpattern
	jsr	g2p96_c71_init_bestpen_map	; c71: build RGB332->Workbench/P96 best-pen map once
	; c86zd1: while the Workbench/P96 window is still visible before AGA,
	; try the exact Gloom palette -> Workbench/P96 BestPen map and draw it.
	; This is diagnostic only: no Live-WPA, no AGA output change.
	jsr	g2p96_c73_init_palette_pen_map
	; c86zdv: the old 16x16 diagnostic grid was useful while probing Pens,
	; but it can visually cover the early BlackMagic/title transition in P96-primary.
	; Keep the window/log/probe, then clear to neutral black instead of drawing cells.
	jsr	g2p96_clear_preview_window
	bra.s	.logonly
...
g2p96_workbench_front_for_preview	push
	...
	jsr	g2p96_keep_preview_active	;c86zdc: keep P96 overlay active even if mouse drifts outside
	jsr	g2p96_hide_p96_pointer	;c86zdb: hide RTG/Workbench pointer over the overlay
	jsr	inputon		;c86zdb: keep raw input active even though the custom screen is pushed back
.done	pull
	rts
...
g2p96_preview_frame	push
	...
	; c86zee: the every-2nd-frame audio test did not reduce the music whine,
	; so restore full-rate P96 presentation.  The whine is not a simple
	; frame-divider issue; see analysis notes for MED/RTG transfer contention.
...
; c86zdt: mirror the currently displayed AGA/planar showbitmap into the
; same P96/RTG RGB window used by the gameplay path.  This is deliberately
; only for non-gameplay/title/menu/intermission pages; gameplay keeps the
; fast chunky->P96 primary path from c86zds.  It solves the grey title/menu
; screens without re-enabling AGA C2P/db during gameplay.
g2p96_present_planar_showbitmap_to_window
	movem.l	d0-d7/a0-a6,-(a7)
	tst	p96preview_gameactive
	bne.w	.rts
	tst	p96preview_state
	...
	move.l	showbitmap,d0
	beq.w	.rts
	move.l	d0,a0		; source planar bitmap base
	; c86zec: BlackMagic is shown very early, before planar_palette may
	; be initialised.  c86zdv already mirrors the active pokepal palette,
	; so allow a blit if lastpal or p96activepal is available.
	move.l	planar_palette,d0
	bne.s	.g2p96_have_planar_palette
	move.l	lastpal,d0
	bne.s	.g2p96_have_planar_palette
	tst	p96activepal_state
	beq.w	.rts
.g2p96_have_planar_palette
	jsr	g2p96_build_direct_palette_lut
	move.l	50(a5),d0	; Window.RPort
	beq.w	.rts
	move.l	d0,a1
	...
	; Same format detection as c86zdp gameplay path: pitch relative to
	; screen width.  This keeps emulator and real Amiga/P96 in agreement.
...
.setup32
	...
	lsl.l	#2,d1
	add.l	d1,a3
	moveq	#4,d5		; bytes per target pixel
	bra.s	.common
.setup24
	moveq	#0,d1
	move	6(a5),d1
...
.setup16
	...
	moveq	#2,d5
.common
	move.l	a0,a6		; source planar base
	moveq	#0,d6		; y 0..239
.row
	cmp	#240,d6
	bge.w	.rts
	move	d6,d0
	mulu	#40,d0		; planar row byte offset
	move.l	a6,a0
	add.l	d0,a0		; a0 = plane0 row base
	move	d6,d0
	mulu	d4,d0
	move.l	a3,a4
	add.l	d0,a4		; destination row base
	moveq	#0,d7		; byte column 0..39
.bytecol
	cmp	#40,d7
	bge.s	.nextrow
	; c86zdt first version favours correctness over speed for static pages:
	; convert this group of 8 planar pixels one by one.
	bra.s	.pix_fallback_start
.nextrow
	addq	#1,d6
	bra.w	.row

.pix_fallback_start
	; Simpler proven path for c86zdt: per-pixel planar read, format-correct
	; RGB write.  d6=current y, d7 byte column.  It intentionally favours
	; correctness for static screens over speed.
	move	d7,d0
	lsl	#3,d0		; start x = bytecol*8
	moveq	#0,d1		; bit number 0..7
.pix8
	cmp	#8,d1
	bge.s	.byte_done
	move	d0,d2
	add	d1,d2		; x
	bsr.w	g2p96_planar_get_pixel_index	; returns d3 = palette index
	cmp	#2,d5
	beq.s	.write16
	cmp	#3,d5
	beq.s	.write24
...
; c86zdt helper: d6=y, d2=x, a6=planar bitmap base.  Returns d3.w palette index.
; Uses the original compact 320x240 planar layout: 40 bytes per row,
; 9600 bytes per plane.  Gloom/Gloom2 uses 8 planes in AGA mode here;
; for ECS the unused high planes should be zero in the allocated bitmap.
g2p96_planar_get_pixel_index
	; c86zdv: use the real runtime bitmap layout instead of a fixed 9600 stride.
	; Generic address is base + y*linemod + plane*bpmod + xbyte.
	; This supports OS/P96 plane-major (linemod=40,bpmod=9600) and native
	; row-interleaved AGA (linemod=320,bpmod=40). d6=y, d2=x, a6=base.
	; Returns d3.w palette index and preserves caller state except d3.
	movem.l	d0-d2/d4-d7/a0-a1,-(a7)
	move	d6,d0
	mulu	linemodw,d0		; row offset = y * visible row stride word
	move	d2,d1
	lsr	#3,d1			; x byte within one plane row
	add	d1,d0
	moveq	#7,d1
	and	d2,d1
	moveq	#7,d2
	sub	d1,d2			; bit number within byte, MSB first
	moveq	#1,d1
	lsl	d2,d1			; bit mask
	moveq	#0,d3
	move.l	a6,a0
	add.l	d0,a0			; plane 0 byte for this x/y
	move	bitplanes,d7
	beq.s	.done
	subq	#1,d7
	moveq	#1,d5			; palette bit value for current plane
.plane_loop
	move.b	(a0),d2
	and.b	d1,d2
	beq.s	.next_plane
...
.next_plane
	move.l	bpmod,d0
	adda.l	d0,a0			; next plane row for same y
	add	d5,d5
	dbf	d7,.plane_loop
.done	movem.l	(a7)+,d0-d2/d4-d7/a0-a1
	rts


; c86zdt: direct palette index -> target RGB lookup.  Unlike the gameplay
; LUT above this does NOT pass through g2_strip_invpal, because planar menu
; bitmaps already contain real palette indices.
; c86zdv: keep a P96-side mirror of the actually active AGA/ECS palette.
; This is needed because menu/font code often pokes only the first 4 colours
; through pokepal2 without changing lastpal.  P96 title/menu blits must see
; those partial font-colour updates, while the rest of the picture palette stays.
g2p96_update_active_palette
	movem.l	d0-d7/a0-a2,-(a7)
	move.l	a1,a0			; source palette passed to pokepal2
	lea	p96activepal,a2		; always stored as AGA-style hi/low pairs
	move	d0,d7			; number of colours
	beq.s	.done
	subq	#1,d7
	tst	aga
	beq.s	.ecs_copy
.aga_copy
	move	(a0)+,(a2)+		; high/normal word
	move	(a0)+,(a2)+		; low word
	dbf	d7,.aga_copy
	bra.s	.ready
.ecs_copy
	move	(a0)+,(a2)+		; ECS 12-bit word as high/normal word
	clr	(a2)+			; no AGA low nibble word
	dbf	d7,.ecs_copy
.ready	move	#-1,p96activepal_state
.done	movem.l	(a7)+,d0-d7/a0-a2
	rts

; c86zdv: direct active palette index -> target RGB lookup.
; Uses p96activepal, not just lastpal/planar_palette, so title/menu/intermission
; colours include partial font-palette pokes and true AGA high+low nibbles.
g2p96_build_direct_palette_lut
	movem.l	d0-d7/a0-a6,-(a7)
	lea	p96pal_rgb565_lut,a1
	lea	p96pal_rgb888_lut,a4
	lea	p96activepal,a3
	tst	p96activepal_state
	bne.s	.have_active
	; Fallback for very early calls before the first pokepal2 mirror exists.
	move.l	lastpal,d0
	bne.s	.fallback_ptr
	move.l	planar_palette,d0
.fallback_ptr
...
.lut_loop
	...
	tst	p96activepal_state
	bne.s	.from_active
	; Fallback source may be the original AGA paired palette or an ECS palette.
	tst	aga
	beq.s	.from_fallback_ecs
	lsl	#2,d6
	move	0(a3,d6.w),d0		; high/normal AGA word
	move	2(a3,d6.w),d1		; low AGA word
	bra.s	.combine_aga
.from_fallback_ecs
	add	d6,d6
	move	0(a3,d6.w),d0
...
.from_active
	lsl	#2,d6
	move	0(a3,d6.w),d0		; high/normal word
	move	2(a3,d6.w),d1		; low word, zero for ECS entries
	bra.s	.combine_aga
.lut_black
	moveq	#0,d0
	moveq	#0,d1
.combine_aga
	; Convert AGA hi/low words to 8-bit RGB like pokepal_os does.
	move	d0,d4
	and	#$0f00,d4
	lsr	#4,d4			; high R nibble -> bits 7..4
	move	d1,d5
	and	#$0f00,d5
	lsr	#8,d5			; low R nibble -> bits 3..0
	or	d5,d4
	move.b	d4,(a4)+
	move	d0,d4
	and	#$00f0,d4		; high G nibble already bits 7..4
	move	d1,d5
	and	#$00f0,d5
	lsr	#4,d5			; low G nibble -> bits 3..0
	or	d5,d4
	move.b	d4,(a4)+
	move	d0,d4
	and	#$000f,d4
	lsl	#4,d4			; high B nibble -> bits 7..4
	move	d1,d5
	and	#$000f,d5		; low B nibble -> bits 3..0
	or	d5,d4
	move.b	d4,(a4)+
	clr.b	(a4)+
	; Build byteswapped RGB565 from the same 8-bit R/G/B values just written.
	moveq	#0,d4
	move.b	-4(a4),d4
	lsr	#3,d4			; R 0..31
	move	d4,d1
...
.lut_ready
	...
	move	chunkymodw(pc),d5
	beq.w	g2p96_direct_full_rts
	; c86zec: detect non-fullscreen WINDOW SIZE.  The renderer stores
	; those frames as a packed width*hite chunky image starting at chunky.
	; The previous full 320x240 copier read past short rows, causing repeats.
	moveq	#0,d6
	cmp	#320,width
	bne.s	.g2p96_windowed_source
	cmp	#240,hite
	beq.s	.g2p96_source_mode_done
.g2p96_windowed_source
	moveq	#1,d6
.g2p96_source_mode_done
	move.l	50(a5),d0	; Window.RPort
	beq.w	g2p96_direct_full_rts
...
; c86zec: centred P96-primary WINDOW SIZE copy paths.  The old full-frame
; copier always walked 320 columns with coloffs, which is correct only when
; width=320/hite=240.  For smaller render windows the chunky source is packed
; at width bytes per row, so copy only width*hite and centre it in black.
.win_setup32
	bsr.w	g2p96_clear_window32
	move	#224,d1	;c86zee: centre 3D view in gameplay area, not under bottom panel
	sub	hite,d1
	lsr	#1,d1
	mulu	d4,d1
	add.l	d1,a3
	...
	add.l	d1,a3
	lea	p96rgb888_lut,a5
	lea	coloffs,a6	;c86zee: source chunky columns are C2P/coloffs-scrambled
	moveq	#0,d6
.win32_row
	cmp	hite,d6
	bge.w	g2p96_direct_full_rts
...
; clear exactly the 320x240 P96 game area at a3 with pitch d4.
g2p96_clear_window32
...
.cw24_row
	move.l	a2,a4
	move	#239,d2		; 960 bytes = 240 longwords
.cw24_pix
	move.l	d0,(a4)+
	dbf	d2,.cw24_pix
	add.l	d4,a2
...
.cw16_row
	move.l	a2,a4
	move	#159,d2		; 640 bytes = 160 longwords
.cw16_pix
	move.l	d0,(a4)+
	dbf	d2,.cw16_pix
	add.l	d4,a2
...
g2v36_hide_pointer	;force invisible pointer for the game screen/window
	...
	move.l	int,d1
	beq.s	.rts
	move.l	chipzero,a1	;c86zdu: pc-range fix / pointer image must live in chip RAM
	move.l	a1,d1
	beq.s	.rts
	move.l	d0,a0
...
initdisplay	;
	...
	; c86zdy: do not stop on the empty early Workbench/P96 window anymore.
	; The previous hold was useful while probing C790/FrameThrower visibility,
	; but in P96-primary it shows a grey/black pre-splash until mouse/fire.
	; BlackMagic is presented later from the real loaded picture instead.
...
db_ecs	move.l	coplist,a0	;c86zdu: pc-range fix
	moveq	#40,d2
...
db_aga	move.l	coplist,a0	;c86zdu: pc-range fix
	moveq	#40,d2
...
; c86zdt: direct planar palette index -> target RGB LUTs for title/menu/intermission pages.
p96pal_rgb565_lut	dcb.w	256,0
p96pal_rgb888_lut	dcb.b	1024,0
p96activepal_state	dc	0	;c86zdv: -1 after at least one pokepal2 mirror
p96activepal	dcb.w	512,0	;c86zdv: 256 AGA-style hi/low colour pairs
	even
