	...
	;gloom2p96_c86zd1: P96 Workbench-hold palette-penmap probe before AGA screen; based on c86zd0, keeps enemy reflections and P96 bridge.
	;
...
; c86zd1: draw a 16x16 grid of the current Gloom palette after it has
; been mapped to the Workbench/P96 ColorMap via ObtainBestPenA.  This is shown
; during the Workbench-hold phase before the custom AGA screen is opened, so it
; stays visible even with automatic AGA switching hardware.
; a0 = Window
g2p96_draw_palette_hold_probe	push
	...
	beq.s	.done
	moveq	#0,d7		;palette index 0..255
.row
	cmp	#256,d7
	bge.s	.done
	move	d7,d6
	...
	move	d6,d1
	mulu	#8,d1		;8 px high cells
	add	#56,d1		;start below top pen bars
	move	d1,d3
	add	#7,d3		;y2
	lea	p96palpen_map,a0
	moveq	#0,d0
	move.b	0(a0,d7.w),d0	;final Workbench/P96 Pen for this Gloom palette entry
	move.l	50(a5),a1
	move.l	grbase(pc),a6
	jsr	-342(a6)	;SetAPen
