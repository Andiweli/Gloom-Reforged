...
p96prevlog_name	dc.b	'RAM:gloom_p96_preview.txt',0
	even
p96prevlog_ok	dc.b	'P96 preview: coarse 20x12 tonal grid active',10
...
g2p96_preview_frame	push
	...
	and	#15,d0		;gloom2p96_c4: update only every 16th frame
	bne.w	.done
...
.nolog	movem.l	d0-d7/a0-a6,-(a7)
	...
	; deliberately tiny Chunky-source test: one scanline, 40 coarse
	; blocks, every 16th frame.  c3 proved that full-window sampling was
	...
	beq.w	.restore
	lea	coloffs(pc),a3
	; gloom2p96_c8: c7b proved that several live tonal bands are
	; stable.  Turn them into a small coarse grid: 20 columns by 12 rows,
	; still only every 16th frame and still using safe RectFill pens.  This
	; keeps the number of drawing operations near c7 while showing a real
	; low-resolution P96 preview instead of line samples only.
	moveq	#0,d6		;preview grid row 0..11
.c7rowloop
	cmp	#12,d6
	bge	.c7done
	move	d6,d7
	mulu	#16,d7
	add	#20,d7		;sample source rows 20..196
	move	d7,d0
	mulu	chunkymodw(pc),d0
	move.l	a4,a2
	add.l	d0,a2		;Chunky/C2P row base for this grid row
	moveq	#0,d5		;x = 0..304, step 16
.c7xloop
	cmp	#320,d5
	bge	.c7nextrow
	move.l	0(a3,d5*4),d4
...
.c7mapped
	...
	move	d6,d1
	mulu	#16,d1
	add	#24,d1		;destination rows 24..200
	move	d5,d2
	add	#15,d2
	move	d1,d3
	add	#15,d3
	jsr	-306(a6)	;RectFill 16x16 sample block
	add	#16,d5
	bra	.c7xloop
.c7nextrow
	addq	#1,d6
