...
p96gameplay_msg_ok	dc.b	'P96 gameplay: direct RGB565 c86zds-lut mini-frame OK',10
...
; a0 = Window.  c86zeq direct RGB565 gameplay mini-frame probe.
; Writes a direct RGB565 calibration ramp plus a 160x96 blocky mini-frame from
; the live chunky gameplay buffer.  This bypasses SetAPen/BestPen for the actual
; image, so the colour result is no longer limited by the Workbench/P96 pen map.
...
.colloop
	...
	; c86zet: old working c86zds colour path.  The source byte is
	; already paladjust/C2P-scrambled, so the 256-entry LUT was built with
	; g2_strip_invpal before reading planar_palette.
	move	d3,d1
	add	d1,d1
	move	0(a6,d1.w),d1
