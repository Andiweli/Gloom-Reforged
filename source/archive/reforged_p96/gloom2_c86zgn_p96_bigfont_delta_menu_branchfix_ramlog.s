...
p96gameplay_msg_ok	dc.b	'P96 gameplay: bigfont delta P96 menu branchfix OK',10
...
g2p96_ingame_menu_floorceil_live_refresh
	...
	; c86zgi: P96 menu stays visible on FLOOR/CEILING changes.  The old
	; c86zfw path rebuilt the gameplay backdrop with predrawall, rebuilt all
	; menu strips with initmenu2, copied a fresh gameplay frame and then rendered
	; every menu row again.  On real A1200 RTG hardware that hid the menu for
	; about two seconds.  Under P96 the changed text string is already updated
	; by trainer_update_floor_text/trainer_update_ceiling_text; opton below
	; redraws only the selected row.
...
.aga_refresh_full
	...
	jsr	g2v190aj_grey_menu_backdrop
.refresh_done
	jsr	opton		;c86zgi: P96 path refreshes only this selected row
	jsr	g2v17_wait_menu_release
	addq	#2,a7
	movem.l	(a7)+,d0/a4
	rts
