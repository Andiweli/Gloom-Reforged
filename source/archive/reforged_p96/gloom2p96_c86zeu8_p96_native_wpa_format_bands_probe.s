...
p96prevlog_name	dc.b	'RAM:gloom_p96_preview.txt',0
	even
p96prevlog_ok	dc.b	'P96 preview: c86zeu8 native WPA format bands probe active',10
...
; ---- c86zeu8 native-screen p96WritePixelArray format bands probe ----
; c86zeu7 proved the important part on real hardware: the official
; p96WritePixelArray path can show a gameplay band on the native 320x240
; P96 screen without bringing back the music whine.  The remaining real-Amiga
; problem was colour order: the band was recognisable but blue/dark-blue.
;
; Therefore this probe draws four tiny 320x12 gameplay bands using different
; 16-bit source encodings.  The user only has to report which band has the
; correct Gloom colours.  No direct BitMap writes and no p96LockBitMap.
;
; Bands:
;   y=060: R5G6B5PC, current c86zdk byte-swapped word data
;   y=090: R5G6B5PC, non-swapped word data
;   y=120: B5G6R5PC, byte-swapped word data
;   y=150: B5G6R5PC, non-swapped word data
;
; Register convention follows the public Picasso96API LVO/prototype order:
...
p96wpalog_name	dc.b	'RAM:gloom_p96_wpa.txt',0
	even
p96wpalog_msg_ok	dc.b	'P96 WPA: c86zeu8 native 4x 320x12 format bands returned',10
p96wpalog_msg_ok_len	equ	*-p96wpalog_msg_ok
p96wpalog_msg_skip	dc.b	'P96 WPA: c86zeu8 skipped',10
...
; Build one 320x12 16-bit band into the shared fast RAM staging buffer.
; Inputs:
;   a0 = chunky base
;   a4 = staging buffer base
;   a5 = p96rgb565_lut, currently c86zdk byte-swapped R5G6B5 data
;   a6 = coloffs
;   d5 = chunkymodw
;   d6 = source y / destination y
;   d2 = mode: 0 RGB swapped, 1 RGB unswapped, 2 BGR swapped, 3 BGR unswapped
; Output: p96wpa_ri_memory points to the ready source band.
g2p96_build_one_format_band
	movem.l	d0-d7/a0-a6,-(a7)
...
.row_loop
	...
	mulu	#640,d0
	move.l	a4,a3
	add.l	d0,a3		;RGB565 band row base
	moveq	#0,d4		;x 0..319
.pix_loop
	cmp	#320,d4
	bge	.next_row
	...
	move.b	0(a2,d0.l),d1
	add	d1,d1
	move	0(a5,d1.w),d0	;current c86zdk byte-swapped RGB565
	cmp	#1,d2
	beq	.rgb_unswap
	cmp	#2,d2
	beq	.bgr_swap
	cmp	#3,d2
	beq	.bgr_unswap
	; mode 0: RGB565PC, current swapped byte order
	move	d0,(a3)+
	bra	.next_pix
.rgb_unswap
	ror	#8,d0
	...
	bra	.next_pix
.bgr_swap
	; convert current swapped RGB565 -> unswapped RGB565 -> BGR565 -> swapped
	ror	#8,d0
	move	d0,d1
	and	#$f800,d1	;old R
	move	d0,d3
	and	#$07e0,d3	;G unchanged
	move	d0,d0
	and	#$001f,d0	;old B
	lsl	#8,d0
	lsl	#3,d0		;old B -> new high 5 bits
	lsr	#8,d1
	lsr	#3,d1		;old R -> new low 5 bits
	or	d3,d0
	or	d1,d0
	ror	#8,d0
	move	d0,(a3)+
	bra	.next_pix
.bgr_unswap
	; convert current swapped RGB565 -> unswapped RGB565 -> BGR565
	ror	#8,d0
	move	d0,d1
	and	#$f800,d1	;old R
	move	d0,d3
...
; Build four 320x12 bands and copy each through p96WritePixelArray.  The
; source buffer reuses p96preview_frameptr because it is a large fast RAM
...
g2p96_native_wpa_band_probe
	...
	beq	.skip
	move.l	d0,a4		;source RGB565 band buffer
	move.l	chunky,d0
	beq	.skip
	...
	; Build/refresh palette->RGB565 LUT lazily, same as the proven direct path.
	tst	p96rgb_lut_ready
	bne	.lut_ready
	jsr	g2p96_build_rgb565_lut_source
	move	#-1,p96rgb_lut_ready
.lut_ready
	...
	; Band 1: y=60, current RGB565PC swapped source, RI format 4.
	move	#60,d6
	moveq	#0,d2
	jsr	g2p96_build_one_format_band
	move.l	p96base,d0
	beq	.skip
	move.l	d0,a6
	...
	move	#320,d4
	move	#12,d5
	move.l	#4,p96wpa_ri_rgbformat
	move.l	50(a3),a1
	jsr	-102(a6)

	; Band 2: y=90, RGB565PC non-swapped source, RI format 4.
	move.l	chunky,a0
	move	chunkymodw,d5
	lea	coloffs,a6
	lea	p96rgb565_lut,a5
	...
	; Band 3: y=120, B5G6R5PC swapped source, RI format 12.
	move.l	chunky,a0
	move	chunkymodw,d5
	lea	coloffs,a6
	lea	p96rgb565_lut,a5
	...
	move	#320,d4
	move	#12,d5
	move.l	#12,p96wpa_ri_rgbformat
	jsr	-102(a6)

	; Band 4: y=150, B5G6R5PC non-swapped source, RI format 12.
	move.l	chunky,a0
	move	chunkymodw,d5
	lea	coloffs,a6
	lea	p96rgb565_lut,a5
