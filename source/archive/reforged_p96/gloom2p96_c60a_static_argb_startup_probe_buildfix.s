	...
	;gloom2p96_c60a: c60 buildfix, one-shot static P96 ARGB probe after testpattern, never from live hook.
	;
...
p96prevlog_name	dc.b	'RAM:gloom_p96_preview.txt',0
	even
p96prevlog_ok	dc.b	'P96 preview: c60a static ARGB startup probe buildfix active',10
...
g2p96_test_window	push
	...
	move.l	d7,a0
	bsr	g2p96_static_argb_probe	;gloom2p96_c60a: one-shot only, not live/vwait
	bra.s	.logonly
...
; gloom2p96_c60a: c60 branch buildfix, one-shot true-colour API probe outside gameplay/vwait.
; a0 = Window.  This intentionally writes only a few static rectangles after
; the old pen-based testpattern.  If this fails, the P96 RGB API call/format is
; wrong; if it works, later patches can feed live RGB24 data through the same
; call, but only from a safe non-intermission path.
g2p96_static_argb_probe	push
	move.l	a0,a5		;Window pointer
	tst	p96present
	beq	.done
	move.l	p96base(pc),d0
	beq	.done
	move.l	50(a5),a0	;Window.RPort
	move.l	a0,d1
	beq	.done
	move.l	d0,a6

	; Small bottom blocks, one-shot only.  These should show real RGB/ARGB
	; colours if p96RectFill is accepted on this P96 setup.
	moveq	#8,d0
	move	#216,d1
	move	#79,d2
	move	#235,d3
	move.l	#$00cc0000,d4	;red, RGB form
	jsr	-126(a6)	;p96RectFill(rp,x1,y1,x2,y2,RGB)
	...
	move	#155,d2
	move	#235,d3
	move.l	#$0000cc00,d4	;green, RGB form
	jsr	-126(a6)
	...
	move	#231,d2
	move	#235,d3
	move.l	#$000000cc,d4	;blue, RGB form
	jsr	-126(a6)
	...
	move	#311,d2
	move	#235,d3
	move.l	#$00909090,d4	;grey, RGB form
	jsr	-126(a6)
.done	pull
	rts
