...
; -----------------------------------------------------------------------------
; c86zep: direct RGB565 bitmap bars probe after c86zeo proved that the
; SetAPen/BestPen/ColorMap path collapses colours too much for a real colour
; gameplay renderer.  This still does not skip the normal AGA doc2p/db present
; path.  It opens the proven 320x240 P96 screen/window briefly from the gameplay
; context and writes fixed RGB565 colour bars directly into the screen bitmap.
; Top half uses unswapped RGB565 words, bottom half uses byte-swapped RGB565
; words, so emulator/real Amiga byte order can be judged visually.
; -----------------------------------------------------------------------------
...
p96gameplay_msg_ok	dc.b	'P96 gameplay: direct RGB565 bars OK',10
...
; a0 = Window.  c86zep direct RGB565 bitmap probe.
; Writes fixed colour bars directly into the P96 screen bitmap.  Top half uses
; normal RGB565 word values, bottom half uses byte-swapped RGB565 values.  This
; bypasses the proven-collapsing SetAPen/BestPen path completely.
g2p96_gameplay_draw_mini_frame_160x96
	...
	; Top half: plain RGB565 words as seen by the 68k.
	lea	p96gameplay_rgb565_normal,a4
	move.l	a3,a0	;current destination row
	move	#119,d7	;120 rows
.top_rows
	bsr	g2p96_gameplay_draw_rgb565_bar_row
	adda.l	d6,a0
	dbf	d7,.top_rows

	; Bottom half: byte-swapped RGB565 words.  Old P96 history showed that
	; RGB565 may need byte swap depending on card/emulator/bitmap format.
	lea	p96gameplay_rgb565_swapped,a4
	move	#119,d7	;120 rows
.bottom_rows
	bsr	g2p96_gameplay_draw_rgb565_bar_row
	adda.l	d6,a0
