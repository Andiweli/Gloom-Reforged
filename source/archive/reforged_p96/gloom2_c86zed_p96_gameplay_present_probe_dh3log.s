...
;gloom2_c86zdi: stronger near vertical oval, whole projectile/upgrade reflection Bayer-dithered, weapon-upgrade floor-touch pulse.
	...
	;gloom2_c86zed: first gameplay-present hook probe; one-shot DH3 log only, no P96 drawing yet.
	;
...
g2release_marker	dc.b	'Gloom Reforged AGA/P96 probe (v1.4 c86zed 29.06.2026) Andreas ',39,'Andiweli',39,' Stuermer',0
	even
...
.waitskip2	; c86m: normal C2P/show path restored after c86l label regression
	jsr	doc2p
	jsr	db
	jsr	g2p96_gameplay_present_probe	;c86zed: one-shot gameplay-present hook/log, no P96 drawing
	ifne	g2p96_overlay_enabled
...
; -----------------------------------------------------------------------------
; c86zed: first gameplay-present hook probe.  This is intentionally still not a
; renderer.  It is called from the normal one-player show path after doc2p/db,
; logs once when gameplay presentation was reached, and then stays silent.  No
; P96 screen/window is opened here, no AGA-present is skipped, and audio/screen
; cadence is untouched.
; -----------------------------------------------------------------------------
p96gameplay_state		dc	0	;0=not run/fail, 1=hit OK, 2=skip AGA, 3=no P96 lib, 4=no ModeID
...
p96gameplay_msg_ok	dc.b	'P96 gameplay: present hook OK',10
