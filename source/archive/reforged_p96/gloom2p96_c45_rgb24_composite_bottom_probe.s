...
p96prevlog_name	dc.b	'RAM:gloom_p96_preview.txt',0
	even
p96prevlog_ok	dc.b	'P96 preview: RGB24 composite bottom probe active',10
...
.c22nextrow
	...
	bra	.c22rowloop
.c22done
	;gloom2p96_c45: keep the proven main Pen/Runfill preview, but replace
	;the three separate R/G/B channel diagnostic bands with one compact
	;composite RGB24 probe band.  The probe still uses existing Pens through
	;SetAPen/RectFill, so it is NOT true-colour output yet, but the Pen value
	;is now built from the real RGB24 linebuffer as RGB332.  This checks that
	;the R/G/B bytes can be combined into a coherent live source before we
	;risk a direct P96 pixel-copy call.
	move.l	p96preview_rgb24lineptr(pc),d0
	beq	.restore
	move.l	d0,a2
	moveq	#0,d5		;x = 0..319 in 4-pixel blocks
	moveq	#0,d7		;RGB24 byte offset, +12 per 4 pixels
.c45mixloop
	cmp	#320,d5
	bge	.restore
	moveq	#0,d0
	move.b	0(a2,d7.w),d0	;R byte
	and	#$e0,d0		;R -> bits 7..5
	moveq	#0,d1
	move.b	1(a2,d7.w),d1	;G byte
	and	#$e0,d1
	lsr	#3,d1		;G -> bits 4..2
	or	d1,d0
	moveq	#0,d1
	move.b	2(a2,d7.w),d1	;B byte
	and	#$c0,d1
	lsr	#6,d1		;B -> bits 1..0
	or	d1,d0		;RGB332 Pen candidate from true RGB24 row
	bne.s	.c45penok
	moveq	#1,d0		;avoid invisible Pen 0
.c45penok
	move.l	50(a5),a1
	move.l	grbase(pc),a6
	jsr	-342(a6)	;SetAPen(composite RGB332 as existing Pen index)
	move	d5,d0
	move	#216,d1
	move	d5,d2
	addq	#3,d2
	move	#239,d3
	jsr	-306(a6)	;RectFill 4x24 composite block
	addq	#4,d5
	add	#12,d7
	bra	.c45mixloop
