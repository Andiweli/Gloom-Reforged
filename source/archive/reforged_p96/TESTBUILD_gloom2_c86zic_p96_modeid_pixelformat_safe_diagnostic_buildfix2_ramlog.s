...
;gloom2_c86zdi: stronger near vertical oval, whole projectile/upgrade reflection Bayer-dithered, weapon-upgrade floor-touch pulse.
	...
	;c86zia: safe P96 pixel-format diagnostic via p96GetModeIDAttr before any P96 screen/window opens; no live bitmap access.
	;c86zht: ignore disabled icon ToolTypes in parentheses, e.g. (DISPLAY=P96).
...
g2release_marker	dc.b	'Gloom Reforged AGA/P96 probe (v1.4 c86zia 11.07.2026) Andreas ',39,'Andiweli',39,' Stuermer',0
	even
...
P96IDA_BYTESPERPIXEL	equ	3
P96IDA_BITSPERPIXEL	equ	4
P96IDA_RGBFORMAT	equ	5
P96IDA_ISP96	equ	6
P96IDA_STDBYTESPERROW	equ	8
...
p96modeid_depth_hex	ds.b	4
	dc.b	' bytespp=$'
p96modeid_bytespp_hex	ds.b	8
	dc.b	' bitspp=$'
p96modeid_bitspp_hex	ds.b	8
	dc.b	' rgbformat=$'
p96modeid_rgbformat_hex	ds.b	8
	dc.b	' isp96=$'
p96modeid_isp96_hex	ds.b	8
	dc.b	' stdbpr=$'
p96modeid_stdbpr_hex	ds.b	8
	dc.b	10
	dc.b	'RGB legend: 04=R5G6B5PC 05=R5G5B5PC 0A=R5G6B5 0B=R5G5B5 0C=B5G6R5PC 0D=B5G5R5PC',10
...
.loop	move.l	(a2)+,d0
	beq	.free
	move.l	d0,a0
	bsr	g2tooltype_disabled	;c86zht: ignore icon ToolTypes disabled as (DISPLAY=...)
	tst	d0
	bne	.loop
...
; c86zht: Workbench icon ToolTypes wrapped in parentheses are disabled by
...
.got	move.l	d0,p96base
	move	#-1,p96present
.done	jsr	g2p96_probe_log	;c86zdw: write RAM:gloom_p96_probe.txt
	pull
	rts
...
.found	move.l	p96modeid,d0
	...
	; c86zia: query the selected ModeID only.  This occurs before any P96
	; screen/window exists, avoiding driver-sensitive live BitMap queries.
	move.l	p96base,d0
	beq.s	.modeattrs_done
	move.l	d0,a6
	move.l	p96modeid,d0
	moveq	#P96IDA_BYTESPERPIXEL,d1
	jsr	-84(a6)	;p96GetModeIDAttr
	lea	p96modeid_bytespp_hex,a0
	bsr	g2p96_long_to_hex8
	move.l	p96modeid,d0
	moveq	#P96IDA_BITSPERPIXEL,d1
	jsr	-84(a6)
	lea	p96modeid_bitspp_hex,a0
	bsr	g2p96_long_to_hex8
	move.l	p96modeid,d0
	moveq	#P96IDA_RGBFORMAT,d1
	jsr	-84(a6)
	lea	p96modeid_rgbformat_hex,a0
	bsr	g2p96_long_to_hex8
	move.l	p96modeid,d0
	moveq	#P96IDA_ISP96,d1
	jsr	-84(a6)
	lea	p96modeid_isp96_hex,a0
	bsr	g2p96_long_to_hex8
	move.l	p96modeid,d0
	moveq	#P96IDA_STDBYTESPERROW,d1
	jsr	-84(a6)
	lea	p96modeid_stdbpr_hex,a0
	bsr	g2p96_long_to_hex8
.modeattrs_done
	move.l	dosbase,a6
...
.logonly	jsr	g2p96_window_log
	jsr	g2p96_preview_log
	pull
	rts
...
g2p96_c73_next
	...
	jsr	g2p96_c83_log_penmap	; c83: write RAM:gloom_p96_penmap.txt
	bra	g2p96_c73_done
...
.active_ready
	...
	jsr	g2p96_gameplay_present_log
...
g2p96_gameplay_rgb12_to_rgb565_c86zds
	...
	or	d3,d2		;B 0..31
	or	d2,d1		;RGB565
	ror	#8,d1		;c86zdk/c86zds confirmed target byte order
	movem.l	(a7)+,d2-d4
	rts
