...
; -----------------------------------------------------------------------------
; c86zer: direct RGB565 gameplay mini-frame with active palette lookup.
; c86zeq proved direct RGB565 bitmap writes and a pseudo-colour gameplay image.
; This version keeps the same bounded one-shot P96 gameplay probe, but builds a
; 256-entry RGB565 LUT from planar_palette and maps chunky indices through it.
...
p96gameplay_msg_ok	dc.b	'P96 gameplay: direct RGB565 palette mini-frame OK',10
...
; a0 = Window.  c86zer direct RGB565 active-palette mini-frame probe.
; Writes a direct RGB565 calibration ramp plus a 160x96 blocky mini-frame from
; the live chunky gameplay buffer.  The image uses a 256-entry RGB565 LUT built
; from planar_palette, not the old SetAPen/BestPen pen map.
...
.colloop
	...
	bge	.nextrow
	move	d5,d4
	add	d4,d4	;source x * 2
	moveq	#0,d3
	move.b	0(a2,d4.w),d3	;live chunky pixel/index

	; Active Gloom palette -> direct RGB565.  c86zer maps the final chunky
	; index through a 256-word RGB565 LUT built from planar_palette.  If the
	; palette is not available, fall back to the earlier RGB332-ish probe colour.
