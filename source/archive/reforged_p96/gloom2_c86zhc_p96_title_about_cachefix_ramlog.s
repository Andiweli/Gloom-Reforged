...
;gloom2_c86zdi: stronger near vertical oval, whole projectile/upgrade reflection Bayer-dithered, weapon-upgrade floor-touch pulse.
	...
	;c86zhc: keep c86zhb safe qmenu restore; avoid redundant P96 title/about reloads and fix disabled in-game rows.
...
g2release_marker	dc.b	'Gloom Reforged AGA/P96 probe (v1.4 c86zhc 09.07.2026) Andreas ',39,'Andiweli',39,' Stuermer',0
	even
...
.menu_backdrop_done
	...
	lea	gamemenu,a4
	jsr	initmenu
	move	#-1,game_menu_active	;c86zhc: mark in-game menu before first P96 render so disabled rows are grey/white immediately
...
.km_nkeyfire	move	d0,4(a0)
	...
	bne.s	.km_nolmfire
	move	#-1,4(a0)
.km_nolmfire	move.l	(a7)+,d1-d3/a1
	rts
...
.redrawmenu
	...
	jsr	g2p96_title_skip_redundant_brush_if_bridge	;c86zhc: P96 cache already has title+brush, avoid 1s redecode/present
	tst	d0
	bne.s	.g2c86zhc_skip_redraw_brush
	jsr	g2v145_draw_menu_gloombrush
.g2c86zhc_skip_redraw_brush
	tst	linked
...
.linksel	bsr	finitpmenu
	bsr	linkup
	cmp	#4,curropt	;c86zhc: REMOTE LINK OPTIONS -> EXIT returns to cached title menu, no full title reload
	bne.s	.g2c86zhc_link_full_return
	jsr	g2p96_title_restore_staged_background_if_bridge
	bra	.redrawmenu
.g2c86zhc_link_full_return
	bra	dointro
	;
.vilesel	addq	#1,mode
	and	#1,mode
...
.noch	bsr	g2v147_finitmenu_soft
	...
	jsr	g2p96_title_about_return_fast_if_bridge	;c86zhc: ABOUT exit under P96 restores cached title instead of reloading IFF
	tst	d0
	bne.s	.g2c86zhc_about_return_fast
	jsr	g2v145_show_clean_title
	bsr	dispon
.g2c86zhc_about_return_fast
	bra	.redrawmenu
...
qmenu	;quick menu in a0
	...
	move.l	a0,-(a7)
	bsr	clspic
	jsr	g2p96_title_restore_staged_background_if_bridge	;c86zhb: P96 qmenu clears old title menu without blacking title image
	move.l	(a7)+,a4
	bsr	initmenu
	bra	dispon
...
g2v147_pmenu_soft	;a0=iff, a1=palette, a2=menu
	move.l	a2,-(a7)
	jsr	g2p96_title_about_fast_background_if_bridge	;c86zhc: ABOUT can reuse staged title background under P96
	tst	d0
	bne.s	.g2c86zhc_fast_about_bg
	bsr	showpic_noclear
.g2c86zhc_fast_about_bg
	move.l	(a7)+,a4
	bsr	initmenu
	bra	dispon
...
p96gameplay_msg_ok	dc.b	'P96 gameplay: title/about cachefix OK',10
...
; c86zhc: P96 title/about cache helpers.  They are deliberately conservative:
...
g2p96_title_about_fast_background_if_bridge
	move.l	d1,-(a7)
	jsr	g2p96_title_cache_ready_if_bridge
	tst	d0
	beq.s	.done
	jsr	g2p96_title_restore_staged_background_if_bridge
	moveq	#-1,d0
