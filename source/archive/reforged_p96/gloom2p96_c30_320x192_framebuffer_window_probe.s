	...
	;gloom2p96_c30: enlarge the framebuffer readback probe to a 64px bottom window.
	;
...
p96prevlog_name	dc.b	'RAM:gloom_p96_preview.txt',0
	even
p96prevlog_ok	dc.b	'P96 preview: 320x192 framebuffer window probe active',10
...
.c22done
	...
	;gloom2p96_c30: enlarge the proven c29 framebuffer band into a 64px
	;bottom window. It still consumes only the hidden 320x192 framebuffer
	;mirror and still uses the proven Runfill/RectFill path. This is not a
	;full output switch yet; it just proves a larger framebuffer region can
	;be read back and displayed safely.
	move.l	p96preview_frameptr(pc),d0
	beq	.restore
