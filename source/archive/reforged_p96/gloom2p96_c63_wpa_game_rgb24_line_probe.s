	...
	;gloom2p96_c63: one live RGB24 game scanline via p96WritePixelArray, guarded and tiny.
	;
...
p96prevlog_name	dc.b	'RAM:gloom_p96_preview.txt',0
	even
p96prevlog_ok	dc.b	'P96 preview: c63 WPA game RGB24 line probe active',10
...
p96wpalog_name	dc.b	'RAM:gloom_p96_wpa.txt',0
	even
p96wpalog_ok	dc.b	'P96 WPA: game RGB24 line p96WritePixelArray returned',10
...
p96wpa_game_logged	dc	0	;gloom2p96_c63: one-shot WPA game-line marker
...
g2p96_test_window	push
	...
	;gloom2p96_c63: no startup WPA draw here; game-line WPA is tried only from the gated gameplay preview path.
	bra.s	.logonly
...
; gloom2p96_c63: one tiny live-game p96WritePixelArray test.  c62b proved
; that p96WritePixelArray itself can return safely against the P96 window.
; This step uses the already prepared RGB24 game scanline, but only for one
; 320x1 row in the bottom probe area and only from the gameplay-gated preview
; path.  No startup/intermission/menu WPA drawing is performed.
g2p96_writepixelarray_game_line_probe	push
	...
	moveq	#0,d1		;SrcY
	moveq	#0,d2		;DestX
	move	#216,d3		;DestY: inside old bottom probe area
	move	#320,d4		;SizeX
	moveq	#1,d5		;SizeY: exactly one scanline
	move.l	p96base(pc),a6
	jsr	-138(a6)	;p96WritePixelArray(ri,0,0,rp,0,216,320,1)
...
; gloom2p96_c63: log first returned live-game WPA call.
...
p96wpa_ri	dc.l	0
	dc	0
	dc	0
	dc.l	2	;RGBFB_R8G8B8 for c63 live game-line probe
	even
...
.c50penok
	...
	bra	.c50xloop
.c50skipdirectrgb24
	;gloom2p96_c63: after the RGB24 linebuffer for sampled row 48 is ready,
	;copy exactly one real RGB24 game scanline through p96WritePixelArray.
	;This is still tiny and gameplay-gated by the outer preview function; no
	;Intermission/menu/startup WPA drawing is attempted here.
	cmp	#48,d6
	bne	.c63nowpa
