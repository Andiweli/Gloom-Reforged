	...
	;gloom2p96_c86zde: larger 640x480 borderless P96 shell; center the 320x240 preview stage to keep focus/pointer inside the overlay.
	;
...
p96prevlog_name	dc.b	'RAM:gloom_p96_preview.txt',0
	even
p96prevlog_ok	dc.b	'P96 preview: c86zde fullscreen shell live preview active',10
...
.nolog	movem.l	d0-d7/a0-a6,-(a7)
	...
	; c86zdc: no heartbeat strip in live mode.  The active preview is a
	; clean 320x192 image centered inside a 320x240 stage. c86zde centers
	; that stage inside a larger 640x480 borderless P96 shell to keep focus/pointer inside.
	move.l	50(a5),a1
	move.l	grbase(pc),a6
	moveq	#0,d0
	...
	move	#479,d2
	move	#143,d3
	jsr	-306(a6)	;c86zde RectFill centered top letterbox
	move.l	50(a5),a1
	move.l	grbase(pc),a6
	moveq	#0,d0
	jsr	-342(a6)	;SetAPen black
	...
	move	#479,d2
	move	#359,d3
	jsr	-306(a6)	;c86zde RectFill centered bottom letterbox
...
.c26noframemirror
	...
	;c86zdc: draw the complete 96-row sampled preview into the main area.
	;Destination is 320x192 at x=160..479, y=144..335 inside the 640x480 shell.
	move.l	p96preview_frameptr(pc),d0
...
.c22xloop
	...
	add	#160,d0		;c86zde centered preview x offset
	move	d6,d1
	add	d1,d1
	add	#144,d1		;c86zde destination rows 144..335
	move	d5,d2
	subq	#1,d2
	add	#160,d2		;c86zde centered preview x offset
	move	d1,d3
	addq	#1,d3
