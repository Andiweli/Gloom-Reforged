...
p96prevlog_name	dc.b	'RAM:gloom_p96_preview.txt',0
	even
p96prevlog_ok	dc.b	'P96 preview: c74 green palette bestpen bottommatch probe active',10
...
g2p96_c73_palready
	...
	; gloom2p96_c74: the direct palette map is now the correct colour path,
	; but the RTG preview still looks slightly too red/grey compared with
	; the AGA game.  For this probe, bias the requested best-pen colour
	; gently toward the original green/brown mood: reduce red and blue by
	; one 4-bit step when non-zero, and raise green by one step when not
	; already maxed.  This only affects the P96 preview pen map.
	move	d6,d4
	move	d4,d0
	and	#$0f00,d0
	beq	.c74norred
