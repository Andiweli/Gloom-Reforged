...
p96prevlog_name	dc.b	'RAM:gloom_p96_preview.txt',0
	even
p96prevlog_ok	dc.b	'P96 preview: multi tonal scanlines active',10
...
.nolog	movem.l	d0-d7/a0-a6,-(a7)
	...
	beq.w	.restore
	lea	coloffs(pc),a3
	; gloom2p96_c7: c6 proved that one tonal scanline follows the
	; game image.  Expand this very carefully to six coarse horizontal
	; bands, still only every 16th frame and still using safe RectFill
	; pens instead of replacing AGA output.
	moveq	#0,d6		;preview band index 0..5
.c7rowloop
	cmp	#6,d6
	bge.s	.c7done
	move	d6,d7
	mulu	#32,d7
	add	#32,d7		;sample source rows 32,64,...,192
	move	d7,d0
	mulu	chunkymodw(pc),d0
	move.l	a4,a2
	add.l	d0,a2		;Chunky/C2P row base for this band
...
.c7mapped
	...
	move	d6,d1
	mulu	#32,d1
	add	#24,d1		;destination rows 24,56,...,184
	move	d5,d2
	addq	#7,d2
	move	d1,d3
	addq	#7,d3
