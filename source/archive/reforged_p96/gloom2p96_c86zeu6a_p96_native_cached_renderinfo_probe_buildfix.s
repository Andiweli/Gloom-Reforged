...
p96prevlog_name	dc.b	'RAM:gloom_p96_preview.txt',0
	even
p96prevlog_ok	dc.b	'P96 preview: c86zeu6a native cached RenderInfo probe buildfix active',10
...
g2p96_test_window	push
	...
	clr.l	p96mode_id
	jsr	g2p96_clear_cached_renderinfo	;c86zeu6: cached P96 RenderInfo target starts invalid
	tst	p96present
	beq	.logonly
...
g2p96_close_native_probe_only
	push
	jsr	g2p96_clear_cached_renderinfo	;c86zeu6: native target no longer valid after close
...
.opened
	move	#1,p96window_state
	move	#1,p96preview_state
	jsr	g2p96_native_cache_renderinfo_once	;c86zeu6: one-shot p96LockBitMap, no per-frame lock
...
.nolog	movem.l	d0-d7/a0-a6,-(a7)
	...
	; c86zdl: stop the 160x120 sampling path here.  c86zdk proved that
	; RGB565 byte order is correct, but the doubled 160x120 source made HUD
	; text unreadable.  Copy the real 320x240 chunky source through a small
	; palette->RGB565 LUT directly into the RTG bitmap instead.
...
.lut_ready
	...
	; c86zeu6: On real P96 hardware the classic BitMap fields on a native
	; 16-bit screen can still look planar (bpr=40), while the same path works
	; in the emulator.  c86zeu5 proved that per-frame p96LockBitMap is too
	; intrusive on real hardware (music whine/crash).  This probe uses a
	; one-shot RenderInfo captured when the native gameplay screen opens, then
	; writes without locking every frame.  If the cached target is absent or
	; implausible, fall back to the old BitMap fields unchanged.
	tst	p96native_state
	beq.s	.no_cached_native_target
	tst	p96cache_valid
	beq.s	.no_cached_native_target
	move.l	p96cache_memory,d0
	beq.s	.no_cached_native_target
	move	p96cache_bpr,d1
	cmp	#640,d1
	blo.s	.no_cached_native_target
	move.l	d0,a3
	move	d1,d4
	move	d4,p96bitmap_bpr
	move.l	a3,d0
	move	d0,p96bitmap_plane0lo
.no_cached_native_target
...
; c86zeu6: cached RenderInfo from one short p96LockBitMap during native
; gameplay-screen opening.  We immediately unlock again, then use the cached
; Memory/BytesPerRow as a diagnostic direct target.  This avoids the c86zeu5
; per-frame lock/unlock path that brought back music whine and crashed the
; real Amiga when opening the ingame menu.
p96cache_ri_memory	dc.l	0
p96cache_ri_bpr	dc	0
p96cache_ri_pad	dc	0
p96cache_ri_rgbformat	dc.l	0
p96cache_valid	dc	0
p96cache_bpr	dc	0
p96cache_memory	dc.l	0
p96cache_rgbformat	dc.l	0
	even

g2p96_clear_cached_renderinfo
	clr	p96cache_valid
	clr	p96cache_bpr
	clr.l	p96cache_memory
	clr.l	p96cache_rgbformat
	clr.l	p96cache_ri_memory
	clr	p96cache_ri_bpr
	clr	p96cache_ri_pad
	clr.l	p96cache_ri_rgbformat
	rts

; Capture RenderInfo once after the native P96 gameplay screen/window opens.
; a0-a6/d0-d7 are preserved.  No lock is kept beyond this routine.
g2p96_native_cache_renderinfo_once
	movem.l	d0-d7/a0-a6,-(a7)
	jsr	g2p96_clear_cached_renderinfo
	tst	p96native_state
	beq	.done
	move.l	p96preview_window,d0
	beq	.done
	...
	beq.s	.done
	move.l	d0,a6
	lea	p96cache_ri_memory,a1
	move.l	#12,d0		; sizeof(RenderInfo)
	jsr	-48(a6)		;p96LockBitMap(BitMap, RenderInfo, size)
	move.l	d0,d7
	beq.s	.done
	move.l	p96cache_ri_memory,d0
	beq.s	.unlock_done
	move.l	d0,p96cache_memory
	move	p96cache_ri_bpr,p96cache_bpr
	move.l	p96cache_ri_rgbformat,p96cache_rgbformat
	move	#-1,p96cache_valid
.unlock_done
	move.l	p96preview_window,d0
	beq.s	.done
	move.l	d0,a5
	...
	move.l	d0,a6
	move.l	d7,d0
	jsr	-54(a6)		;p96UnlockBitMap(BitMap, lock)
.done
	movem.l	(a7)+,d0-d7/a0-a6
	rts
