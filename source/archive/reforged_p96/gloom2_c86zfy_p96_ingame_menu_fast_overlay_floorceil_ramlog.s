...
dogamemenu	;
	...
	lea	gamemenu,a4
	jsr	initmenu
	jsr	g2p96_menu_bridge_all_rows_present	;c86zfy: overlay only menu rows, keep existing P96 gameplay backdrop
...
p96gameplay_msg_ok	dc.b	'P96 gameplay: ingame menu fast overlay OK',10
...
.no_p96_state
	jsr	g2p96_menu_bridge_all_rows_present	;c86zfy: rows only, no full-screen planar bridge
...
; c86zfy: initial/menu-rebuild bridge for real hardware.  Keep the current
; P96 gameplay frame as the backdrop and copy only the menu row strips from the
; original planar menu bitmap.  This avoids the slow full 320x240 planar->RGB565
; bridge that visibly built the whole screen line-by-line on A1200 RTG.
