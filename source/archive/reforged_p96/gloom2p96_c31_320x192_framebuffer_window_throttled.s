	...
	;gloom2p96_c31: keep the c30 64px framebuffer window but draw it only when the hidden mirror refreshes.
	;
...
p96prevlog_name	dc.b	'RAM:gloom_p96_preview.txt',0
	even
p96prevlog_ok	dc.b	'P96 preview: 320x192 framebuffer window throttled active',10
...
.c22done
	...
	;gloom2p96_c31: keep the proven c30 64px framebuffer window, but
	;draw it only on the same frames where the hidden 320x192 mirror is
	;refreshed.  c30 redrew the same stale mirror data every 2nd frame while
	;the mirror itself updated only every 8th frame, adding visible load
	;without fresher framebuffer-window content.
	move	p96preview_frame(pc),d0
	and	#7,d0
	bne	.restore
	move.l	p96preview_frameptr(pc),d0
	...
	moveq	#0,d6		;window probe row 0..31
.c29bandloop
	cmp	#32,d6
	bge	.restore
	move	d6,d0
	mulu	#6,d0
	add	#0,d0		;source framebuffer rows 0..186
	mulu	#320,d0
	lea	0(a4,d0.l),a2
