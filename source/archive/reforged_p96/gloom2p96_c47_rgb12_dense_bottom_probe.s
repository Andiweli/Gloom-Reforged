...
p96prevlog_name	dc.b	'RAM:gloom_p96_preview.txt',0
	even
p96prevlog_ok	dc.b	'P96 preview: RGB12 dense bottom probe active',10
...
.c22nextrow
	...
	bra	.c22rowloop
.c22done
	;gloom2p96_c47: denser true 2D RGB12 bottom probe.  c46 proved that
	;the hidden RGB12 buffer can drive a live 2D mini-window, but it used
	;only 12 source rows and 4x2 probe blocks.  This version keeps the
	;probe inside the safe bottom 24px area, but samples 24 different
	;RGB12 rows and draws 2x1 blocks.  The visible output is still Pen-
	;based RGB332, so this is not true-colour yet; it only gives the next
	;more detailed proof that the RGB12 buffer is spatially correct.
	move.l	p96preview_rgbptr(pc),d0
	beq	.restore
	move.l	d0,a4		;base of 160x96 RGB12 sample buffer
	moveq	#0,d6		;bottom probe source row index 0..23
.c47rowloop
	cmp	#24,d6
	bge	.restore
	move	d6,d0
...
.c47xloop
	...
	move	d0,d1
	and	#$000f,d1
	lsr	#2,d1		;B 0..3 -> bits 1..0
	or	d1,d4
	move	d4,d0		;RGB332 Pen candidate from true RGB12 row
	bne.s	.c47penok
