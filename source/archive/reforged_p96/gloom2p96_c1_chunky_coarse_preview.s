	...
	;gloom2p96_c1: P96 probe + persistent preview window; AGA output unchanged.
	;
...
; gloom2p96_c1: live coarse Chunky preview into the P96/Workbench
; window.  This is deliberately conservative for the first real-hardware
; stage: it samples one 8x8 block and draws it via graphics.library
; SetAPen/RectFill.  It is slower than final P96 WritePixelArray output,
; but safe and independent of P96 API LVO details.  AGA remains the real
; display path.
g2p96_preview_frame	push
	...
	and	#7,d0		;update only every 8th frame for safety
	bne.w	.done
	move.l	dosbase,d0
	; no DOS needed here; this is just a cheap keep-alive null check slot
	movem.l	d0-d7/a0-a6,-(a7)
