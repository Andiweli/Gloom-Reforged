...
.nocd	;
	...
	jsr	g2p96_static_buffer_probe	;c86zea: neutral static-buffer/base visual probe, then close
	move.l	4.w,a6
	moveq	#1,d1
	jsr	-216(a6)
...
; -----------------------------------------------------------------------------
; c86zea: neutral static-buffer/base visual probe on the proven 320x240 P96
; screen/window path.  This still does not enable gameplay P96, does not sample
; the chunky buffer, and does not use the old Workbench overlay.  It opens a
; fresh P96 screen/window, draws a neutral 0/1-pen frame/cross via graphics.library
; RectFill, waits briefly so it can be seen, closes everything, then logs DH3.
; -----------------------------------------------------------------------------
...
p96static_msg_ok	dc.b	'P96 static: neutral static-buffer OK',10
...
; a0 = Window. Draws a deliberately neutral frame/cross using only pens 0/1.
g2p96_static_draw_neutral
	push
	move.l	a0,a5
	move.l	50(a5),a1	;Window.RPort
	move.l	a1,d0
	...
	move	#169,d2
	move	#239,d3
	jsr	-306(a6)	;centre vertical bar
	move.l	50(a5),a1
	move.l	p96static_grbase,a6
	moveq	#0,d0
	move	#110,d1
	move	#319,d2
	move	#129,d3
	jsr	-306(a6)	;centre horizontal bar
.done
	pull
	rts
...
g2p96_static_wait
	push
	moveq	#30,d7	;about half a second on PAL; enough to see, short enough to stay non-invasive
.wait
	move.l	p96static_grbase,d0
	beq	.done
	move.l	d0,a6
