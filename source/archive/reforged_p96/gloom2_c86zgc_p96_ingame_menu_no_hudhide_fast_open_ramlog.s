...
p96gameplay_msg_ok	dc.b	'P96 gameplay: ingame menu clean overlay no-HUDhide fast OK',10
...
; c86zga: overlay all visible menu rows by repeatedly using the fast row-copy
; path.  Every row restores its gameplay backdrop first, then writes only the
; glyph pixels that differ from the saved pre-text menu strip.
g2p96_menu_bridge_all_rows_present
	movem.l	d0-d7,-(a7)
	move	curropt,-(a7)
	move	#-1,p96menu_bridge_glyph_only	;c86zgc: initial menu build writes glyph pixels only
	moveq	#0,d7
.loop
	move	numopts,d0
	cmp	d0,d7
	...
	bra.w	.loop
.done
	clr	p96menu_bridge_glyph_only
	move	(a7)+,curropt
	movem.l	(a7)+,d0-d7
	rts

; c86zfx/c86zga: fast row-only bridge for real hardware menu navigation.
