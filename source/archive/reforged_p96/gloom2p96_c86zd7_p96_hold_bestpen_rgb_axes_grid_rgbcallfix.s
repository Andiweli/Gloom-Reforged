	...
	;gloom2p96_c86zd7: buildfix for c86zd6 rgb-axis BestPen hold test; P96 Workbench-hold BestPen RGB axes grid; c86zd3 raw pen grid confirmed, c86zd5 BestPen map worked but repeated by RGB332 bit layout.
	;
...
p96prevlog_name	dc.b	'RAM:gloom_p96_preview.txt',0
	even
p96prevlog_ok	dc.b	'P96 preview: c86zd6 BestPen RGB axes grid active',10
...
; c86zd6: draw a more intuitive direct BestPen RGB-axis grid over the
; whole hold window.  c86zd5's RGB332 candidate order naturally repeated every
; 4 columns because the low bits were red and the next bit was green.  This
; diagnostic asks ObtainBestPenA directly for each visible cell instead:
;   X axis  = red 0..255
;   Y axis  = green 0..255
;   4 bands = blue 0/85/170/255
; So a healthy ColorMap/BestPen path should look like a red-green gradient
; with blue increasing in four horizontal bands, not like a 4-column repeat.
; a0 = Window
g2p96_draw_palette_hold_probe	push
	move.l	a0,a5
	move.l	a5,d0
	beq	.done
	...
	moveq	#0,d7		;cell 0..255
.cell_loop
	cmp	#256,d7
	bge	.done

	; Red from X cell: 0..15 -> 0..255.
	move	d7,d5
	and	#15,d5
	move	d5,d0
	lsl	#4,d0
	...
	; Blue in four horizontal bands: 0,85,170,255.
	move	d6,d0
	lsr	#2,d0
	lea	p96c71_tab2,a3
	moveq	#0,d3
	...
	cmp.l	#-1,d0
	bne	.pen_ok
	move	d7,d0		;fallback should still make geometry visible
	and	#$00ff,d0
.pen_ok
	and.l	#$000000ff,d0
	...
	move.l	50(a5),a1
	move.l	grbase,a6
	jsr	-342(a6)	;SetAPen(BestPen)

	; Calculate RectFill coordinates after SetAPen, like the c86zd3 fix.
	move	d7,d6
	...
	move	d6,d1
	mulu	#15,d1		;y1 = cell*15, 16 rows fill 240px
	move	d1,d3
	add	#14,d3		;y2
	move.l	50(a5),a1
	move.l	grbase,a6
