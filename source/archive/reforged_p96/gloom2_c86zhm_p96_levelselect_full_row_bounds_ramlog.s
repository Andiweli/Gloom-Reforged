...
g2release_marker	dc.b	'Gloom Reforged AGA/P96 probe (v1.4 c86zhm 09.07.2026) Andreas ',39,'Andiweli',39,' Stuermer',0
	even
...
.state_ok
	...
	move.l	a0,d0
	beq.w	.done
	; c86zhm: the optional title level selector replaces the first
	; menu row text at draw time with "ONE PLAYER GAME: MAPx.y".
	; The P96 bounded scan must use that same replacement text for
	; its x_start/x_end, otherwise it scans only the old "ONE
	; PLAYER GAME" bounds and clips the MAP suffix to "MA".
	move.l	a0,a4
	jsr	g2v190hx_level_menu_a4
	move.l	a4,a0
	move.l	a0,a1
