	...
	;gloom2p96_b1: P96 probe + safe Workbench/P96 testpattern window; AGA output unchanged.
	;
...
; gloom2p96_b1: draw a short-lived Workbench/P96 testpattern window.
; This deliberately does NOT replace AGA output yet.  If the window cannot
; be opened, the normal AGA path continues and RAM:gloom_p96_window.txt
; records the failure.
g2p96_test_window	push
	...
	moveq	#50,d6	;about one second on PAL; enough for visual proof
.wait	move.l	grbase,a6
	jsr	-270(a6)	;WaitTOF
	dbf	d6,.wait
	move.l	d7,a0
	move.l	int,a6
...
initdisplay	;
	...
	jsr	-408(a6)
	move.l	d0,int
	jsr	g2p96_test_window	;gloom2p96_b1: visible P96/Workbench testpattern, AGA path unchanged
	;
