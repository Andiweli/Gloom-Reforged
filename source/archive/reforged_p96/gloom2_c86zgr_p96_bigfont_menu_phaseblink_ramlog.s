...
;gloom2_c86zdi: stronger near vertical oval, whole projectile/upgrade reflection Bayer-dithered, weapon-upgrade floor-touch pulse.
	...
	;c86zgr: keep c86zgq slow P96-only Bigfont blink, preserve blink
	;phase on row navigation so selected rows do not flash on menu moves.
...
g2release_marker	dc.b	'Gloom Reforged AGA/P96 probe (v1.4 c86zgr 09.07.2026) Andreas ',39,'Andiweli',39,' Stuermer',0
	even
...
p96gameplay_msg_ok	dc.b	'P96 gameplay: phase bigfont P96 menu blink OK',10
...
.blink_off
	...
	bra	.wait
.got_input
	; c86zgr: do not force the selected row visible on input.  For up/down
	; navigation that caused a one-frame flash when the blink phase was OFF.
	; The current blink phase is preserved and applied to the new row below.
	btst	#5,d0	;v115 ESC cancel/back in game menu
	beq	.noesc
	moveq	#0,d0
	rts
...
.right
	...
	rts
.up
	jsr	optoff		;c86zgq: old row becomes normal only on real navigation
.up_loop
	subq	#1,curropt
	bpl	.up_chk
	move	numopts,d0
...
.up_chk
	...
	tst	p96menu_blink_visible
	bne	.up_show_new
	move	#-1,p96menu_row_present_suppress	;c86zgr: build new selected row, but do not flash it ON
	jsr	opton
	jsr	g2p96_menu_blink_current_row_off	;c86zgr: preserve OFF phase on new row
	bra	.wait
.up_show_new
	jsr	opton		;c86zgr: preserve ON phase, no blink counter reset
	bra	.wait
.down
	jsr	optoff		;c86zgq: old row becomes normal only on real navigation
.down_loop
...
.down_chk
	...
	tst	p96menu_blink_visible
	bne	.down_show_new
	move	#-1,p96menu_row_present_suppress	;c86zgr: build new selected row, but keep blink OFF
	jsr	opton
	jsr	g2p96_menu_blink_current_row_off	;c86zgr: preserve OFF phase on new row
	bra	.wait
.down_show_new
...
g2p96_menu_bridge_current_row_present
	tst	p96menu_row_present_suppress	;c86zgr: opton may rebuild row without immediate P96 flash
	beq	.no_suppress
	clr	p96menu_row_present_suppress
	rts
.no_suppress
	movem.l	d0-d1,-(a7)
	move	curropt,d0
...
p96menu_row_present_suppress	dc	0	;c86zgr -1=suppress next opton P96 row present
