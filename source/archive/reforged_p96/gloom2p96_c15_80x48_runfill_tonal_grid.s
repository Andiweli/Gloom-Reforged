	...
	;gloom2p96_c7: multiple tonal scanline preview bands, AGA unchanged.
	;
...
p96prevlog_name	dc.b	'RAM:gloom_p96_preview.txt',0
	even
p96prevlog_ok	dc.b	'P96 preview: 80x48 runfill tonal grid active',10
...
g2p96_preview_frame	push
	...
	and	#1,d0		;gloom2p96_c13: update every 2nd frame
	bne.w	.done
...
.nolog	movem.l	d0-d7/a0-a6,-(a7)
	...
	; deliberately tiny Chunky-source test: an 80x48 coarse
	...
	beq.w	.restore
	lea	coloffs(pc),a3
	; gloom2p96_c15: c14 proved that the 80x48 2-frame tonal grid is
	; stable and useful, but it still performs one RectFill per 4x4 block.
	; This keeps the exact same 80x48 source sampling and safe pen mapping,
	; but merges adjacent blocks with the same pen into one horizontal run.
...
.c15rowloop
	...
	move	d6,d0
	mulu	#4,d0
	add	#20,d0		;sample source rows 20..208
	mulu	chunkymodw(pc),d0
	move.l	a4,a2
	add.l	d0,a2		;Chunky/C2P row base for this grid row
	moveq	#0,d5		;x = 0..316, step 4
