...
p96prevlog_name	dc.b	'RAM:gloom_p96_preview.txt',0
	even
p96prevlog_ok	dc.b	'P96 preview: c86zeu5a native locked bitmap writer buildfix active',10
...
.lut_ready
	...
	; c86zeu5: real Amiga/P96 does not expose useful linear framebuffer
	; values in the classic BitMap fields for native 16-bit screens.  The
	; previous diagnostic saw Depth=16 but BytesPerRow=40, i.e. the old planar
	; row size, so the RGB writer correctly skipped and only the 8 test bars
	; stayed visible.  Lock the P96 bitmap and use the RenderInfo Memory/BPR
	; returned by Picasso96 instead.
	jsr	g2p96_lock_bitmap_for_direct
	tst.l	d0
	bne.s	.locked_bitmap
	move	#$0006,p96bitmap_state	;c86zeu5: p96LockBitMap failed
	jsr	g2p96_bitmap_log_once
	bra.w	.full_rts
.locked_bitmap
	move.l	p96lock_memory,a3	; destination memory from RenderInfo
	moveq	#0,d4
	move	p96lock_bpr,d4	; pitch from RenderInfo

	; c86zeu5: capture the locked RenderInfo target, not the stale BitMap fields.
...
.fmt_fallback16
	...
	cmp	#640,d4
	bhs.w	.full_setup16
	move	#$0005,p96bitmap_state	;c86zeu5: pitch too small/unknown, direct writer skipped
	bra.w	.full_unlock_rts

.full_setup32
	move	#$0030,p96bitmap_state	;c86zeu5: selected 32-bit direct writer
	moveq	#0,d1
...
.full_setup24
	move	#$0020,p96bitmap_state	;c86zeu5: selected 24-bit direct writer
	moveq	#0,d1
	move	6(a5),d1		; Window.TopEdge
	mulu	d4,d1
	add.l	d1,a3
...
.full_setup16
	move	#$0010,p96bitmap_state	;c86zeu5: selected 16-bit direct writer
	moveq	#0,d1
	move	6(a5),d1		; Window.TopEdge
	mulu	d4,d1
	add.l	d1,a3
...
.full16_nextrow
	...
	bra.w	.full16_row
.full_unlock_rts
	; c86zeu5: every successful p96LockBitMap must be released before
	; any DOS logging or later OS/P96 activity.  The P96 docs warn that
	; locks must be paired and should be held only briefly.
	jsr	g2p96_unlock_bitmap_after_direct
	jsr	g2p96_bitmap_log_once
.full_rts
	movem.l	(a7)+,d0-d7/a0-a6
	rts
...
p96bitmap_state	dc	0	;0010=16-bit writer, 0020=24-bit, 0030=32-bit, 0005=pitch small, 0006=lock fail
...
; c86zeu5: P96 locked bitmap RenderInfo for direct native-screen writes.
; The classic BitMap fields may report planar-looking values on real RTG
; hardware.  p96LockBitMap fills this RenderInfo with the actual CPU-visible
; memory pointer, row pitch and RGB format.  The lock is held only for the
; duration of the direct frame copy, then released before logging.
p96lock_ri_memory	dc.l	0
p96lock_ri_bpr	dc	0
p96lock_ri_pad	dc	0
p96lock_ri_rgbformat	dc.l	0
p96lock_handle	dc.l	0
p96lock_bitmap	dc.l	0
p96lock_memory	dc.l	0
p96lock_bpr	dc	0
p96lock_rgbformat	dc.l	0
	even

g2p96_lock_bitmap_for_direct
	movem.l	d1-d7/a0-a6,-(a7)
	clr.l	p96lock_handle
	clr.l	p96lock_bitmap
	clr.l	p96lock_memory
	clr	p96lock_bpr
	clr.l	p96lock_rgbformat
	move.l	a2,p96lock_bitmap
	move.l	p96base,d0
	beq.s	.fail
	move.l	d0,a6
	move.l	a2,a0		; BitMap
	lea	p96lock_ri_memory,a1	; RenderInfo buffer
	move.l	#12,d0		; sizeof(RenderInfo)
	jsr	-48(a6)		;p96LockBitMap(BitMap, RenderInfo, size)
	move.l	d0,p96lock_handle
	beq.s	.fail
	move.l	p96lock_ri_memory,d1
	beq.s	.unlock_fail
	move.l	d1,p96lock_memory
	move	p96lock_ri_bpr,p96lock_bpr
	move.l	p96lock_ri_rgbformat,p96lock_rgbformat
	moveq	#1,d0
	bra.s	.done
.unlock_fail
	move.l	p96lock_bitmap,a0
	move.l	p96lock_handle,d0
	move.l	p96base,a6
	jsr	-54(a6)		;p96UnlockBitMap
	clr.l	p96lock_handle
.fail
	moveq	#0,d0
.done
...
g2p96_unlock_bitmap_after_direct
