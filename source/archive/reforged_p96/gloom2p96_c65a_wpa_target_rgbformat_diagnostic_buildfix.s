	...
	;gloom2p96_c65a: buildfix for target P96 RGBFORMAT diagnostic; WPA band still active.
	;
...
p96prevlog_name	dc.b	'RAM:gloom_p96_preview.txt',0
	even
p96prevlog_ok	dc.b	'P96 preview: c65a WPA target RGBFORMAT diagnostic buildfix active',10
...
p96wpalog_name	dc.b	'RAM:gloom_p96_wpa.txt',0
	even
p96wpalog_ok	dc.b	'P96 WPA: game RGB24 band p96WritePixelArray returned',10
...
; gloom2p96_c64: small visible live-game p96WritePixelArray band.  c62b proved
; that p96WritePixelArray itself can return safely against the P96 window,
; and c63 proved that a live game RGB24 row returns safely but was too thin
; to judge.  This version writes selected even game rows as a 16px band in
; the bottom probe area, still only from the gameplay-gated preview path.
; No startup/intermission/menu WPA drawing is performed.
g2p96_writepixelarray_game_line_probe	push
	...
	lea	p96wpa_ri(pc),a0
	move.l	d0,(a0)		;RenderInfo.Memory = current 320x1 RGB24 line
