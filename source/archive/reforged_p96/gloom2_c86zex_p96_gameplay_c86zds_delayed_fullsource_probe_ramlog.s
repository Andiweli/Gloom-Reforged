...
.wait2	tst	showflag
	...
	bsr	vwait
	bra.s	.wait2
.waitskip2	; c86zex: delayed P96 full-source probe after several gameplay frames, before doc2p/db
	jsr	g2p96_gameplay_present_probe	;c86zex: delayed c86zds full-source pre-AGA-present probe
	jsr	doc2p
	jsr	db
	ifne	g2p96_overlay_enabled
...
; -----------------------------------------------------------------------------
; c86zex: delayed direct RGB565 full-source gameplay probe using the old proven c86zds
...
p96gameplay_delay_counter	dc	0	;c86zex: wait a few gameplay presents before sampling final wall columns
...
p96gameplay_msg_ok	dc.b	'P96 gameplay: c86zds delayed full-source OK',10
...
g2p96_gameplay_present_probe
	...
	bne	.done
	; c86zex: do not sample the very first gameplay present.  The earlier
	; full-source probe showed floor/ceiling but no wall columns; delay the
	; one-shot P96 copy for a few real gameplay frames so the final wall draw
	; buffer has settled before the diagnostic screen opens.
...
; a0 = Window.  c86zew full-source pre-AGA-present probe.
; This intentionally moves the probe timing before doc2p/db, matching the old
; c86zds primary path more closely. It writes the full 320x240 chunky source
; through the c86zds source-byte -> RGB565 LUT directly into the fresh P96
; bitmap. AGA presentation still runs afterwards; this is not yet the final
; P96-primary skip path.
