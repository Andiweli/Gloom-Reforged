...
p96prevlog_name	dc.b	'RAM:gloom_p96_preview.txt',0
	even
p96prevlog_ok	dc.b	'P96 preview: 320x192 framebuffer stripe probe active',10
...
.c22done
	...
	;gloom2p96_c28: prove that the hidden 320x192 framebuffer mirror can
	;be consumed independently from the live sampler.  Draw only one small
	;bottom diagnostic stripe from mirrored row 96, using the already proven
	;Runfill/RectFill path.  This is intentionally not a full output switch.
	move.l	p96preview_frameptr(pc),d0
	beq	.restore
	move.l	d0,a2
	move	#96,d0
...
.c28xloop
	...
	move.l	grbase(pc),a6
	move	d7,d0
	jsr	-342(a6)	;SetAPen(previous stripe run pen)
	move	p96run_start(pc),d0
	move	#220,d1
	move	d5,d2
	subq	#1,d2
	move	#231,d3
	jsr	-306(a6)	;RectFill bottom proof stripe run
.c28startrun
	move	d4,d7
	move	d5,p96run_start
.c28samepen
...
.c28flushrow
	...
	move.l	grbase(pc),a6
	move	d7,d0
	jsr	-342(a6)	;SetAPen(final stripe run pen)
	move	p96run_start(pc),d0
	move	#220,d1
	move	#319,d2
	move	#231,d3
	jsr	-306(a6)	;RectFill final bottom proof stripe run
