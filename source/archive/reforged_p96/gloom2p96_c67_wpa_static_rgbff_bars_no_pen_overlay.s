	...
	;gloom2p96_c61: P96 bitmap attribute guard probe; no true-colour drawing yet.
	;gloom2p96_c62: isolated static p96WritePixelArray probe after testpattern.
	;gloom2p96_c67: static RGBFF bars via p96WritePixelArray, no pen overlay.
	;
...
; overwrote the entire frame.  This probe renders player1 into the top 320x120
; half and player2 into the bottom 320x120 half by temporarily rebasing the
; chunky pointer for player2.  c55 additionally draws the existing top HUD
; into each half; first-person gun placement is still intentionally left out.
...
p96attr_state	dc	0	;gloom2p96_c61: -1 skip/no bm, 0 non-P96, 1 P96 bitmap
p96attr_rgbformat	dc.l	0	;gloom2p96_c61: raw P96BMA_RGBFORMAT value for later use
...
p96prevlog_name	dc.b	'RAM:gloom_p96_preview.txt',0
	even
p96prevlog_ok	dc.b	'P96 preview: c67 WPA static RGBFF bars no pen overlay active',10
...
; gloom2p96_c61: one-shot bitmap capability log.  This deliberately
; does not draw RGB pixels.  It only checks whether the preview window's
; RastPort BitMap is a P96 bitmap before any later true-colour API call.
p96attrlog_name	dc.b	'RAM:gloom_p96_attr.txt',0
	even
p96attrlog_skip	dc.b	'P96 attr: skipped, no window or bitmap',10
p96attrlog_skip_len	equ	*-p96attrlog_skip
p96attrlog_nonp96	dc.b	'P96 attr: window bitmap is NOT P96, RGB writes disabled',10
p96attrlog_nonp96_len	equ	*-p96attrlog_nonp96
p96attrlog_p96	dc.b	'P96 attr: window bitmap is P96, WPA guard passed, RGBFORMAT other/unknown',10
p96attrlog_p96_len	equ	*-p96attrlog_p96
p96attrlog_clut	dc.b	'P96 attr: target RGBFORMAT=CLUT, WPA true RGB is quantized to pens',10
p96attrlog_clut_len	equ	*-p96attrlog_clut
p96attrlog_r8g8b8	dc.b	'P96 attr: target RGBFORMAT=R8G8B8',10
p96attrlog_r8g8b8_len	equ	*-p96attrlog_r8g8b8
p96attrlog_b8g8r8	dc.b	'P96 attr: target RGBFORMAT=B8G8R8',10
p96attrlog_b8g8r8_len	equ	*-p96attrlog_b8g8r8
p96attrlog_a8r8g8b8	dc.b	'P96 attr: target RGBFORMAT=A8R8G8B8',10
p96attrlog_a8r8g8b8_len	equ	*-p96attrlog_a8r8g8b8
p96attrlog_a8b8g8r8	dc.b	'P96 attr: target RGBFORMAT=A8B8G8R8',10
p96attrlog_a8b8g8r8_len	equ	*-p96attrlog_a8b8g8r8
	even

; gloom2p96_c62: p96WritePixelArray one-shot probe log.  This is
; deliberately separate from the capability log so we can tell whether
; the call returned at all on real hardware.
p96wpalog_name	dc.b	'RAM:gloom_p96_wpa.txt',0
	even
p96wpalog_ok	dc.b	'P96 WPA: static RGBFF bars p96WritePixelArray returned',10
p96wpalog_ok_len	equ	*-p96wpalog_ok
p96wpalog_skip	dc.b	'P96 WPA: skipped by attr guard',10
p96wpalog_skip_len	equ	*-p96wpalog_skip
	even
...
p96wpa_game_logged	dc	0	;gloom2p96_c64: one-shot WPA game-band marker
...
g2p96_test_window	push
	...
	move.l	d7,a0
	bsr	g2p96_bitmap_attr_probe	;gloom2p96_c61: no drawing, only capability guard log
	;gloom2p96_c64: no startup WPA draw here; game-band WPA is tried only from the gated gameplay preview path.
	bra.s	.logonly
...
; gloom2p96_c61: safe one-shot capability probe for the preview window.
; c60/c60a proved that calling true-colour P96 drawing blindly can crash even
; directly after the testpattern.  Therefore this version only asks P96 whether
; the Window.RPort BitMap is actually a P96 bitmap.  No p96RectFill, no
; p96WritePixelArray, no p96WriteTrueColorData is called here.
g2p96_bitmap_attr_probe	push
	clr	p96attr_state
	clr.l	p96attr_rgbformat
	tst	p96present
	beq	.skip
	move.l	p96base(pc),d0
	beq	.skip
	move.l	a0,a5		;Window pointer
	...
	beq	.skip
	move.l	d0,a0		;BitMap
	moveq	#8,d0		;P96BMA_ISP96
	move.l	p96base(pc),a6
	jsr	-42(a6)	;p96GetBitMapAttr(BitMap,P96BMA_ISP96)
	tst.l	d0
	beq	.notp96
	move	#1,p96attr_state
	move.l	50(a5),a1
	move.l	4(a1),a0	;RastPort.BitMap again
	moveq	#7,d0		;P96BMA_RGBFORMAT, only stored for later diagnostics
	move.l	p96base(pc),a6
	jsr	-42(a6)	;p96GetBitMapAttr(BitMap,P96BMA_RGBFORMAT)
	move.l	d0,p96attr_rgbformat
	bra	.log
.notp96
	clr	p96attr_state
	bra	.log
.skip
	move	#-1,p96attr_state
.log
	jsr	g2p96_bitmap_attr_log
	pull
	rts

; gloom2p96_c61: write the bitmap attribute probe result.
g2p96_bitmap_attr_log	push
	move.l	dosbase,d0
	beq	.done
	move.l	d0,a6
	lea	p96attrlog_name(pc),a0
	move.l	a0,d1
	move.l	#1006,d2	;MODE_NEWFILE
	jsr	-30(a6)	;Open
	move.l	d0,d7
	beq	.done
	move	p96attr_state(pc),d0
	bmi	.skip
	beq.s	.nonp96
	move.l	p96attr_rgbformat(pc),d0
	cmp.l	#1,d0		;RGBFB_CLUT
	beq.s	.clut
	cmp.l	#2,d0		;RGBFB_R8G8B8
	beq.s	.r8g8b8
	cmp.l	#3,d0		;RGBFB_B8G8R8
	beq.s	.b8g8r8
	cmp.l	#6,d0		;RGBFB_A8R8G8B8
	beq.s	.a8r8g8b8
	cmp.l	#7,d0		;RGBFB_A8B8G8R8
	beq.s	.a8b8g8r8
	lea	p96attrlog_p96(pc),a0
	move.l	#p96attrlog_p96_len,d3
	bra.s	.write
.clut
	lea	p96attrlog_clut(pc),a0
	move.l	#p96attrlog_clut_len,d3
	bra.s	.write
.r8g8b8
	lea	p96attrlog_r8g8b8(pc),a0
	move.l	#p96attrlog_r8g8b8_len,d3
	bra.s	.write
.b8g8r8
	lea	p96attrlog_b8g8r8(pc),a0
	move.l	#p96attrlog_b8g8r8_len,d3
	bra.s	.write
.a8r8g8b8
	lea	p96attrlog_a8r8g8b8(pc),a0
	move.l	#p96attrlog_a8r8g8b8_len,d3
	bra.s	.write
.a8b8g8r8
	lea	p96attrlog_a8b8g8r8(pc),a0
	move.l	#p96attrlog_a8b8g8r8_len,d3
	bra.s	.write
.nonp96
	lea	p96attrlog_nonp96(pc),a0
	move.l	#p96attrlog_nonp96_len,d3
	bra.s	.write
.skip
	lea	p96attrlog_skip(pc),a0
	move.l	#p96attrlog_skip_len,d3
.write
	move.l	d7,d1
	move.l	a0,d2
	jsr	-48(a6)	;Write
...
; gloom2p96_c67: static RGB-format probe via p96WritePixelArray.
; c64d proved that live WPA writes reach the P96 window, but the band still
; looked like the old false-colour preview.  This test deliberately ignores
; game pixels and fills the existing 320x1 RGB24 linebuffer with fixed bars:
; red, green, blue, white.  If these bars appear correctly, WPA/RGBFORMAT is
; fine and the game RGB source is wrong.  If red/blue are swapped, source byte
; order is wrong.  Still only called from the gameplay-gated preview path, not
; during startup/menu/intermission.
g2p96_writepixelarray_game_line_probe	push
	move	p96attr_state(pc),d0
	cmp	#1,d0
	bne	p96c66_done
	tst	p96present
	...
	beq	p96c66_done
	move.l	p96preview_window(pc),d1
	beq	p96c66_done

	; Fill one 320-pixel RGB24 source row with unambiguous test colours.
	move.l	d0,a2
	moveq	#79,d7
p96c66_red_loop
	move.b	#$ff,(a2)+	;R
	move.b	#0,(a2)+	;G
	move.b	#0,(a2)+	;B
	dbf	d7,p96c66_red_loop
	moveq	#79,d7
p96c66_green_loop
	move.b	#0,(a2)+	;R
	move.b	#$ff,(a2)+	;G
	move.b	#0,(a2)+	;B
	dbf	d7,p96c66_green_loop
	moveq	#79,d7
p96c66_blue_loop
	move.b	#0,(a2)+	;R
	move.b	#0,(a2)+	;G
	move.b	#$ff,(a2)+	;B
	dbf	d7,p96c66_blue_loop
	moveq	#79,d7
p96c66_white_loop
	move.b	#$ff,(a2)+	;R
	move.b	#$ff,(a2)+	;G
	move.b	#$ff,(a2)+	;B
	dbf	d7,p96c66_white_loop

	move.l	d1,a5		;Window pointer
	move.l	50(a5),d1	;Window.RPort
	beq	p96c66_done
	move.l	d1,a1		;RastPort for p96WritePixelArray
	lea	p96wpa_ri(pc),a0
	move.l	p96preview_rgb24lineptr(pc),d0
	move.l	d0,(a0)		;RenderInfo.Memory = static 320x1 RGB24 bars
	move	#960,4(a0)	;BytesPerRow = 320 * 3
	clr	6(a0)		;pad
	move.l	#4,8(a0)	;RGBFF_R8G8B8 source data (1<<RGBFB_R8G8B8)
	moveq	#0,d0		;SrcX
	moveq	#0,d1		;SrcY
	moveq	#0,d2		;DestX
	move	d6,d3		;DestY follows current sampled row
	lsr	#1,d3		;even rows 32..62 -> band rows 208..223
	add	#192,d3		;inside bottom probe area
	move	#320,d4		;SizeX
	moveq	#1,d5		;SizeY: exactly one scanline
	move.l	p96base(pc),a6
	jsr	-138(a6)	;p96WritePixelArray(ri,0,0,rp,0,destY,320,1)
	tst	p96wpa_game_logged
	bne	p96c66_done
	move	#-1,p96wpa_game_logged
	jsr	g2p96_writepixelarray_log
p96c66_done
	pull
	rts

; gloom2p96_c64: log first returned live-game WPA call.
g2p96_writepixelarray_log	push
	move.l	dosbase,d1
	beq	.done
	move.l	d1,a6
	lea	p96wpalog_name(pc),a0
	move.l	a0,d1
	move.l	#1006,d2	;MODE_NEWFILE
	jsr	-30(a6)	;Open
	move.l	d0,d7
	...
	cmp	#1,d0
	bne.s	.skipped
	lea	p96wpalog_ok(pc),a0
	move.l	#p96wpalog_ok_len,d3
	bra.s	.write
.skipped
	lea	p96wpalog_skip(pc),a0
	move.l	#p96wpalog_skip_len,d3
.write
	move.l	d7,d1
	move.l	a0,d2
	jsr	-48(a6)	;Write
...
p96wpa_ri	dc.l	0
	dc	0
	dc	0
	dc.l	4	;RGBFF_R8G8B8 for WPA source data
	even
...
.c43norgb24line
		...
		;gloom2p96_c67: skip the old pen/RectFill bottom probe entirely for
		;this format test. The bottom band must be driven only by WPA now;
		;if the colours still do not appear, the problem is WPA source format
		;or call setup, not the old false-colour overlay.
		bra.w	.c50skipdirectrgb24
	;gloom2p96_c51: taller direct RGB24 bottom probe without a dedicated
	;320x24x3/320x48x3 buffer.  Every 2nd sampled gameplay row is drawn
	;into a 48-pixel bottom window.  The data source is still the already
	;prepared RGB24 scanline buffer from c43, consumed immediately before
	;it is overwritten by the next row.  This keeps c49/c50's safe allocation
	;profile and avoids the c48 stale bottom-buffer path that blocked after
	;the Intermission screen.
	move	d6,d0
	and	#1,d0
	bne.w	.c50skipdirectrgb24
	move.l	p96preview_rgb24lineptr(pc),d0
	beq.w	.c50skipdirectrgb24
	move.l	p96preview_rgbptr(pc),d1
	beq.w	.c50skipdirectrgb24
	move.l	d0,a2		;base of current 320-pixel RGB24 scanline
	moveq	#0,d5		;destination x = 0..319, 2 pixels per source sample
.c50xloop
	cmp	#320,d5
	bge.w	.c50skipdirectrgb24
	moveq	#0,d4
	move.b	(a2)+,d4	;R byte
	moveq	#0,d1
	move.b	(a2)+,d1	;G byte
	moveq	#0,d2
	move.b	(a2)+,d2	;B byte
	addq.l	#3,a2		;skip duplicated RGB24 pixel for this 2px block

	;Visible output is still Workbench/P96 Pen-based RGB332, not true RGB yet.
	;The important proof here is that the visible mini-window is now driven
	;directly from the true RGB24 linebuffer and not from RGB12 or a staged
	;bottom buffer.
	move	d4,d0
	and	#$00e0,d0	;R top 3 bits already at bits 7..5
	move	d1,d3
	and	#$00e0,d3
	lsr	#3,d3		;G top 3 bits -> bits 4..2
	or	d3,d0
	move	d2,d3
	lsr	#6,d3		;B top 2 bits -> bits 1..0
	or	d3,d0
	bne.s	.c50penok
...
.c50penok
	move.l	50(a5),a1
	move.l	grbase(pc),a6
	jsr	-342(a6)	;SetAPen(RGB332 Pen candidate from RGB24 linebuffer)
	move	d5,d0
	move	d6,d1
	...
	addq	#1,d2
	move	d1,d3
	jsr	-306(a6)	;RectFill 2x1 direct RGB24 probe block
	addq	#2,d5
	bra.w	.c50xloop
.c50skipdirectrgb24
		;gloom2p96_c64: write a small visible WPA RGB24 band instead of
		;the c63 single 1px row.  Only even sampled rows 32..62 are used,
		;mapping to bottom rows 208..223.  This stays gameplay-gated by
		;the outer preview function; no Intermission/menu/startup WPA.
		move	d6,d0
		and	#1,d0
		bne.w	.c64nowpa
		cmp	#32,d6
		...
		cmp	#64,d6
		bhs.w	.c64nowpa
		jsr	g2p96_writepixelarray_game_line_probe
.c64nowpa
...
initdisplay	;
	...
	bsr	initbitmap
	;
	; c62b: GenAm range fix after c62 WPA growth - avoid far PC-relative read
...
db	;double buffering
	;
	; c62a: keep this old DB block build-safe after the c62 WPA code growth.
	; GenAm ran out of PC-relative reach here, so use relocatable absolute reads.
	move.l	bitmaps,d0
	move.l	bitmaps2,d1
	cmp.l	drawbitmap,d0
	beq.s	.show
...
.g2_title_has_palette
	...
	jsr	g2fileexists
	beq	.done		; keep profile 0: Gloom Deluxe, visible title menu stays classic
