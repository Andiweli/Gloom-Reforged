...
dogamemenu	;
	...
	jsr	g2p96_ingame_menu_filter_wsize	;c86zfs: hide/skips WINDOW SIZE only under P96
	lea	gamemenu,a4
	jsr	initmenu
	jsr	g2p96_menu_bridge_present	;c86zfr/c86zfs: copy freshly drawn ESC menu into live P96 screen
...
.game_menu_adjust
	cmp	#2,d0
	bne.s	.notwsize
	jsr	g2p96_display_should_lock_fullwin	;c86zfq: P96 primary expects full 320x240 source until P96 menu exists
	tst	d0
	beq.s	.wsize_normal
	bsr	largewin
	bra	.loop
...
.fskip	move	floorflag,floorflag2	;c86p: keep 2P swapped floor flag in sync for cfg/gameplay
	...
	bsr	refresh	;c86zfs: P96-aware refresh keeps menu bridge visible after FLOOR toggle
	bra	.loop
.notfloor
	cmp	#5,d0
	bne.s	.notroof
...
.rskip	move	roofflag,roofflag2	;c86p: keep 2P swapped ceiling flag in sync for cfg/gameplay
	...
	bsr	refresh	;c86zfs: P96-aware refresh keeps menu bridge visible after CEILING toggle
	bra	.loop
.notroof
	cmp	#7,d0
	bne.s	.notblob
...
.g2v190aj_refresh_nogrey
	jsr	g2p96_ingame_menu_filter_wsize	;c86zfs: keep WINDOW SIZE hidden/skipped after full menu refresh
	lea	gamemenu,a4
	jsr	initmenu2
	jsr	dispon
	jsr	g2p96_menu_bridge_present	;c86zfs: FLOOR/CEILING refresh stays visible in P96 menu bridge
	rts
...
p96gameplay_msg_ok	dc.b	'P96 gameplay: ingame menu hide wsize/floor-ceiling OK',10
...
; c86zfs: hide the old AGA WINDOW SIZE row while the in-game menu is shown
; through the P96 bridge.  The row stays in the menu table so existing option
; indices remain unchanged, but a zero first byte makes menuskip treat it as an
; empty/non-selectable row.  DISPLAY=AGA restores the original leading space.
g2p96_ingame_menu_filter_wsize
	move.b	#32,game_wsize
