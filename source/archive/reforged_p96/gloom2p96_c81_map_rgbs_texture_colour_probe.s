...
p96prevlog_name	dc.b	'RAM:gloom_p96_preview.txt',0
	even
p96prevlog_ok	dc.b	'P96 preview: c81 map_rgbs texture colour probe active',10
...
.c22bufxloop
	...
	move.b	0(a6,d0.w),d0
	; gloom2p96_c81: d0 after invpal is a colour index from the live
	; map_rgbs pool produced by remapanim/addpal/calcpalettes, not a direct
	; index into misc/palette_8.  Using planar_palette made sprites partly
	; plausible but flattened many wall/floor texture tones to grey.  Sample
	; the same live RGB pool that the AGA shade tables are generated from.
	move.l	map_rgbs(pc),a6
	tst.l	a6
	beq	.c22fallback
	move	d0,d1
	tst	d1
	bne	.c81rgbok
	moveq	#0,d2		;index 0/transparent -> black sample
	bra	.c22gotrgb
.c81rgbok
	add	d1,d1		;map_rgbs is RGB12 word pool
	move	0(a6,d1.w),d2	;RGB12 from live texture/object RGB pool
.c22gotrgb
...
g2p96_c73_init_palette_pen_map
	movem.l	d0-d7/a0-a6,-(a7)
	; c81: cache/rebuild against the live map_rgbs end pointer.  The base
	; pointer is static, but the colour pool contents/end can change when
	; assets are remapped.  This map must therefore follow map_rgbsat, not
	; the static planar_palette pointer.
...
g2p96_c73_loop
	; c81: build pen map from the live map_rgbs RGB12 pool.  This is the
	; colour source used when the game remaps texture/object palettes into
	; its shade tables, so texture greens should now land in the right places.
	move	d7,d0
	tst	d0
	bne	g2p96_c81_maprgb
	moveq	#0,d6		;transparent/zero stays black for pen matching
