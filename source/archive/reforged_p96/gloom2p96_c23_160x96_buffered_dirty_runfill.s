...
.osskipz	;
	...
	;
	;gloom2p96_c23: allocate a second 160x96 previous-frame buffer.
	;It allows dirty-run output: unchanged 2x2 cells are not redrawn,
	;while the first frame after menu/title clear still redraws everything.
	move.l	#160*96,d0
	moveq	#1,d1
	allocmem	p96previewprevbuf
	move.l	d0,p96preview_prevptr
	clr	p96preview_prevvalid
	;
	move.l	#256*16,d0
	moveq	#1,d1
	allocmem	palettes
...
p96prevlog_name	dc.b	'RAM:gloom_p96_preview.txt',0
	even
p96prevlog_ok	dc.b	'P96 preview: 160x96 buffered dirty-runfill active',10
...
p96preview_prevptr	dc.l	0	;gloom2p96_c23: previous 160x96 preview buffer for dirty redraw
p96preview_prevvalid	dc	0	;0=force full redraw, -1=dirty compare allowed
...
.nolog	movem.l	d0-d7/a0-a6,-(a7)
	...
	; gloom2p96_c22a: first safe buffered stage.  c17/c18a/c21 proved
...
.c22drawbuffer
	...
	;gloom2p96_c23: if the previous-frame buffer exists and is valid,
	;draw only dirty horizontal runs.  If it is missing or invalidated by
	;title/menu clear, fall back to the proven full runfill draw and then
	;refresh the previous buffer.  This keeps the P96 output safe while
	;moving one step closer to a real buffered line/array path.
	move.l	p96preview_prevptr(pc),d0
	beq	.c23full_draw
	tst	p96preview_prevvalid
	beq	.c23full_draw

	move.l	d0,a0		;previous 160x96 pen buffer
...
.c23dirty_rowloop
	...
	move	d6,d0
	mulu	#160,d0
	lea	0(a4,d0.l),a2	;current row
	move.l	p96preview_prevptr(pc),a0
	lea	0(a0,d0.l),a0	;previous row
	moveq	#0,d5		;destination x = 0..318, step 2
	moveq	#-1,d7		;current changed-run pen, -1 = no run yet
	clr	p96run_start
.c23dirty_xloop
	cmp	#320,d5
	bge	.c23dirty_flushrow
	moveq	#0,d4
	move.b	(a2)+,d4	;current pen/luma from current buffer
	moveq	#0,d0
	move.b	(a0)+,d0	;previous pen/luma
	cmp.b	d0,d4
	beq	.c23dirty_unchanged
	move.b	d4,-1(a0)	;update previous buffer only for changed cells
	cmp	d7,d4
	beq	.c23dirty_samepen
	tst	d7
	bmi	.c23dirty_startrun
	; Changed pen: flush previous dirty run [p96run_start..d5-1].
	move.l	50(a5),a1
	move.l	grbase(pc),a6
	move	d7,d0
	...
	move	d1,d3
	addq	#1,d3
	jsr	-306(a6)	;RectFill changed horizontal run
.c23dirty_startrun
	move	d4,d7
...
.c23dirty_samepen
	...
	bra	.c23dirty_xloop
.c23dirty_unchanged
	; A clean cell breaks a dirty run because the already visible P96 cell
	; can stay as it was.  Flush any active dirty run before skipping it.
	tst	d7
	bmi	.c23dirty_skipclean
	move.l	50(a5),a1
	move.l	grbase(pc),a6
	move	d7,d0
	jsr	-342(a6)	;SetAPen(active dirty run pen)
	move	p96run_start(pc),d0
	move	d6,d1
	add	d1,d1
	add	#24,d1
...
.c23dirty_flushrow
	tst	d7
	bmi	.c23dirty_nextrow
	move.l	50(a5),a1
	move.l	grbase(pc),a6
	move	d7,d0
	jsr	-342(a6)	;SetAPen(final dirty run pen)
	move	p96run_start(pc),d0
	move	d6,d1
	add	d1,d1
	add	#24,d1
	...
	move	d1,d3
	addq	#1,d3
	jsr	-306(a6)	;RectFill final dirty run
.c23dirty_nextrow
	addq	#1,d6
	bra	.c23dirty_rowloop
.c23dirty_done
	bra	.c22done

.c23full_draw
	; First frame or after menu/title clear: draw the full 160x96 buffer
	; through the same proven c22a runfill path, then seed prev-buffer.
	moveq	#0,d6		;buffer row 0..95
.c22rowloop
	cmp	#96,d6
	bge	.c23full_done
...
.c22xloop
	...
	bge	.c22flushrow
	moveq	#0,d4
	move.b	(a2)+,d4	;pen/luma value from the internal buffer
	cmp	d7,d4
	beq	.c22samepen
	tst	d7
	bmi	.c22startrun
...
.c22nextrow
	...
	bra	.c22rowloop
.c23full_done
	; Seed the previous buffer after a full redraw, if it was allocated.
	move.l	p96preview_prevptr(pc),a0
	tst.l	a0
	beq	.c22done
	move.l	p96preview_bufptr(pc),a1
	tst.l	a1
	beq	.c22done
	move	#3839,d0	;160*96/4 longs - 1
.c23copyloop
	move.l	(a1)+,(a0)+
	dbf	d0,.c23copyloop
	move	#-1,p96preview_prevvalid
.c22done
...
g2p96_clear_preview_window	push
	clr	p96preview_prevvalid	;gloom2p96_c23: force full redraw when gameplay resumes
	move.l	p96preview_window(pc),d0
	beq.s	.done
	move.l	d0,a5
	move.l	50(a5),d0	;Window.RPort
