	...
	;gloom2p96_c86zd3: P96 Workbench-hold raw pen grid coordinate fix; reload RectFill coords after SetAPen so the 16x16 Pen map is actually drawn.
	;
...
p96prevlog_name	dc.b	'RAM:gloom_p96_preview.txt',0
	even
p96prevlog_ok	dc.b	'P96 preview: c86zd3 raw pen grid coordfix active',10
...
; c86zd2: draw a raw 16x16 Workbench/P96 Pen grid over the whole
; hold window.  c86zd1 still showed only the old 8 vertical pen stripes,
; which means the palette-mapped probe either was not valid yet or was hidden
; by the simple startup test pattern.  This version does not depend on
; lastpal/planar_palette/p96palpen_state at all: it asks graphics.library to
; draw pens 0..255 directly.  If this still collapses to only a few colours,
; the Workbench/P96 ColorMap itself is the limiting factor for the RectFill
; pen path and we should switch the next probe to true RGB/WPA instead.
; a0 = Window
g2p96_draw_palette_hold_probe	push
	...
	beq.s	.done
	moveq	#0,d7		;raw Workbench/P96 pen index 0..255
.cell_loop
	cmp	#256,d7
	bge.s	.done
	; c86zd3: SetAPen may trash d0/d1, so set the pen first and
	; calculate/reload all RectFill coordinates after it.
	move	d7,d0		;SetAPen raw pen number
	move.l	50(a5),a1
	move.l	grbase(pc),a6
	jsr	-342(a6)	;SetAPen
