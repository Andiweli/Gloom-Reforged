...
;gloom2_c86zdi: stronger near vertical oval, whole projectile/upgrade reflection Bayer-dithered, weapon-upgrade floor-touch pulse.
	...
	;c86zhh: P96 intermission presents picture immediately and full yellow text instantly; no typewriter/fullscreen text palette bug.
...
g2release_marker	dc.b	'Gloom Reforged AGA/P96 probe (v1.4 c86zhh 09.07.2026) Andreas ',39,'Andiweli',39,' Stuermer',0
	even
...
.wmf	tst	fadevol
	...
	beq.s	.n22
	bsr	swaphflags
.n22	jsr	g2p96_display_enter_title	;c86zfq: title/menu AGA-bridge owns display
...
.g2v190t_show_text
	move	#2,pdelay
	; c86zhh: On P96, do not use the original typewriter delay.
	; The picture is already visible from scriptdraw; draw the complete
	; text into showbitmap offscreen, then present once with the P96
	; intermission font LUT.  This avoids the minute-long RTG redraw and
	; the pink/dark text caused by repeated fullscreen conversions.
	cmp	#P96DSP_INTERMISSION_BRIDGE,p96display_state
	bne	.g2c86zhh_keep_typewriter
	tst	p96gameplay_persist_active
	beq	.g2c86zhh_keep_typewriter
	move	#-1,pdelay
.g2c86zhh_keep_typewriter
...
g2display_mode	dc	0	;0=AUTO, 1=AGA, 2=P96
...
g2display_log_name	dc.b	'RAM:gloom_displaymode.txt',0
	even
g2display_msg_auto_default	dc.b	'DISPLAY probe: AUTO (default)',10
...
; the default AUTO mode remains active and the normal game startup continues.
...
p96gameplay_msg_ok	dc.b	'P96 gameplay: intermission instant yellow text OK',10
...
; c86zhh: printmess2 still calls this hook, but P96 intermission text is
; no longer presented one character at a time.  scripttext now draws the full
; text immediately into showbitmap and presents once afterwards.
...
g2p96_static_present_showbitmap
	...
	beq	.plain_lut
	; c86zhh: Font/text mode must still use the intermission picture
	; palette for the backdrop, then override only the font colour slots
	; from bigfont.bin/bigfont2.bin.  The old code built the menu LUT but
	; the fullscreen converter read p96static_rgb565_lut afterwards, which
	; caused pink/brown intermission text.
...
; c86zhh: Build a P96 static-screen LUT for intermission text.
; Base colours come from the current intermission picture palette (picpal/gloompal),
; not from lastpal, because initfontpal may have poked only the first four font
; colours into lastpal.  Then override entries 0..3 with the currently selected
; bigfont.bin/bigfont2.bin palette so P96 intermission text is yellow like AGA.
...
.font_loop
	...
	tst	d7
	bne	.font_colour
	moveq	#0,d0	; initfontpal clears font colour 0 before poking palette
.font_colour
	moveq	#0,d2
	tst	aga
	beq	.font_ecs
