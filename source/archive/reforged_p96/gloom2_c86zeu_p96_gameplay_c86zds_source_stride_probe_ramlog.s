...
p96gameplay_msg_ok	dc.b	'P96 gameplay: c86zds source-stride mini-frame OK',10
...
; a0 = Window.  c86zeu c86zds source/stride mini-frame probe.
; Writes a direct RGB565 calibration ramp plus a 160x96 mini-frame from
; the live chunky gameplay buffer.  Compared with c86zet this uses the old
; working c86zds source column mapping through coloffs instead of linear x*2
; sampling.  This should remove repeated white boxes caused by sampling the
; wrong chunky source positions.
...
.cal_rows
	...
	; Draw the 160x96 gameplay mini-frame as 2x2 cells at y=24.
	; c86zeu: source addressing now follows the old working c86zds direct
	; present path: each visible screen x is first mapped through coloffs before
	; reading the chunky source byte.  c86zet used a simple linear x*2 sample,
	; which could hit invalid/repeated source columns and show repeated white boxes.
	move.l	chunky,d0
	beq	.fallback
	move.l	d0,a3	;base chunky pointer
	move	chunkymodw,d2	;source chunky row stride, as in c86zds
	beq	.fallback
	lea	p96gameplay_rgb565_source_lut,a6
	lea	coloffs,a5	;old c86zds column remap table
	moveq	#0,d0
	move	#24,d0
	mulu	d6,d0	;destination y offset in bytes
	move.l	8(a2),a0	;restore P96 bitmap base
	adda.l	d0,a0
	moveq	#0,d7	;row 0..95
.rowloop
	cmp	#96,d7
	bge	.done
	move	d7,d0
	add	d0,d0	;source y * 2
	mulu	d2,d0
	move.l	a3,a2
	lea	0(a2,d0.l),a2	;source row pointer
	move.l	a0,a1	;destination row 1
	move.l	a0,a4
	adda.l	d6,a4	;destination row 2
	moveq	#0,d5	;column 0..159
.colloop
	cmp	#160,d5
	bge	.nextrow
	move	d5,d4
	add	d4,d4	;full source/display x = mini x * 2
	move.l	0(a5,d4*4),d0	;c86zds: remapped chunky source column offset
	moveq	#0,d3
	move.b	0(a2,d0.l),d3	;live chunky pixel/index

	; c86zet/c86zds colour path.  The source byte is already
	; paladjust/C2P-scrambled, so the LUT was built with g2_strip_invpal
	; before reading planar_palette.
	move	d3,d1
	add	d1,d1
	move	0(a6,d1.w),d1
