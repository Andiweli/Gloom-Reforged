...
p96prevlog_name	dc.b	'RAM:gloom_p96_preview.txt',0
	even
p96prevlog_ok	dc.b	'P96 preview: RGB24 bottom channel probe active',10
...
.c22nextrow
	...
	bra	.c22rowloop
.c22done
	;gloom2p96_c44: keep the proven main Pen/Runfill preview, but add a very
	;small diagnostic RGB24 channel probe in the free bottom 24 pixels.  This
	;does NOT use true-colour output yet; it simply verifies that the RGB24
	;linebuffer contains live R/G/B byte data by drawing three 8px-high bands
	;from the last prepared scanline.  No SetRGB, no palette changes.
	move.l	p96preview_rgb24lineptr(pc),d0
	beq	.restore
	move.l	d0,a2
	moveq	#0,d5		;x = 0..319 in 8-pixel blocks
	moveq	#0,d7		;RGB24 byte offset, +24 per 8 pixels
.c44rloop
	cmp	#320,d5
	bge	.c44gstart
	moveq	#0,d0
	move.b	0(a2,d7.w),d0	;R byte from RGB24 line
	bne.s	.c44rpenok
	moveq	#1,d0
.c44rpenok
	move.l	50(a5),a1
	move.l	grbase(pc),a6
	jsr	-342(a6)	;SetAPen(R intensity as existing Pen index)
	move	d5,d0
	move	#216,d1
	move	d5,d2
	addq	#7,d2
	move	#223,d3
	jsr	-306(a6)	;RectFill 8x8 R block
	addq	#8,d5
	add	#24,d7
	bra	.c44rloop
.c44gstart
	moveq	#0,d5
	moveq	#1,d7		;G byte offset
.c44gloop
	cmp	#320,d5
	bge	.c44bstart
	moveq	#0,d0
	move.b	0(a2,d7.w),d0	;G byte from RGB24 line
	bne.s	.c44gpenok
	moveq	#1,d0
.c44gpenok
	move.l	50(a5),a1
	...
	addq	#7,d2
	move	#231,d3
	jsr	-306(a6)	;RectFill 8x8 G block
	addq	#8,d5
	add	#24,d7
	bra	.c44gloop
.c44bstart
	moveq	#0,d5
	moveq	#2,d7		;B byte offset
.c44bloop
	cmp	#320,d5
	bge	.restore
	moveq	#0,d0
	move.b	0(a2,d7.w),d0	;B byte from RGB24 line
	bne.s	.c44bpenok
	moveq	#1,d0
.c44bpenok
	move.l	50(a5),a1
	...
	addq	#7,d2
	move	#239,d3
	jsr	-306(a6)	;RectFill 8x8 B block
	addq	#8,d5
	add	#24,d7
	bra	.c44bloop
