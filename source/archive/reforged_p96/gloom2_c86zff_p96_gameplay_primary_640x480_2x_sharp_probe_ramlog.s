...
g2release_marker	dc.b	'Gloom Reforged AGA/P96 probe (v1.4 c86zff 30.06.2026) Andreas ',39,'Andiweli',39,' Stuermer',0
	even
...
p96modeid_msg_notfound	dc.b	'P96 modeid: not found 640x480',10
...
p96screen_depth_tag	dc.l	SA_Depth,16
	...
	cnop	0,4
p96modeid_tags
	dc.l	P96BIDTAG_NominalWidth,640
	dc.l	P96BIDTAG_NominalHeight,480
p96modeid_depth_tag
...
; -----------------------------------------------------------------------------
; c86zfe: P96 gameplay-primary sharpness/pixel-probe anchor.
; Based on the confirmed c86zfd real-hardware smoke-test anchor: P96-primary
; gameplay with delayed timing, full 320x240 source, old c86zds source-byte ->
; RGB565 LUT path, AGA high+low palette precision, lifecycle/menu/intermission
; force-close, and AGA fallback.  This step intentionally keeps the renderer
; unchanged and overlays a tiny 1:1 black/white pixel checker in the top-left
; corner of each P96 gameplay frame, so real hardware can reveal whether the
; apparent softness is monitor/RTG scaling rather than renderer blur.
; -----------------------------------------------------------------------------
...
p96gameplay_msg_ok	dc.b	'P96 gameplay: primary 640x480 2x sharp OK',10
...
.fail_bitmap
	...
	bra	.done
.frame_ok
	bsr	g2p96_gameplay_draw_sharpness_probe	;c86zfe: visible 1:1 pixel sharpness calibration overlay
	move	#1,p96gameplay_state
...
; c86zfb: black-fill the open direct RGB565 P96 bitmap during startup/menu handoff.
...
; c86zff: a0 = Window.  Overlay a small true 1:1 black/white pixel
...
; a0 = Window.  c86zff full-source P96-primary present.
; Writes the full 320x240 chunky source through the c86zds source-byte ->
; RGB565 LUT into a 640x480 P96 bitmap as nearest-neighbour 2x pixels.  The caller skips the old
; AGA doc2p/db present only after this copy reports success.
g2p96_gameplay_draw_mini_frame_160x96
	...
	move	0(a2),d4	; BitMap.BytesPerRow
	and.l	#$0000ffff,d4
	cmp	#1280,d4		; c86zff: 640x480 16-bit RGB565 target
	blo.w	.done
	...
	moveq	#0,d6	; source y 0..239, each row doubled to 480
.rowloop
	cmp	#240,d6
	bge.w	.done
	move	d6,d0
	...
	move	d6,d0
	add	d0,d0		; destination y = source y * 2
	mulu	d4,d0
	move.l	a3,a1
	adda.l	d0,a1	; destination row 1
	move.l	a1,a0
	adda.l	d4,a0	; destination row 2
	moveq	#0,d7	; source x 0..319, each pixel doubled to 640
.pixloop
	cmp	#320,d7
	bge.s	.nextrow
	move.l	0(a6,d7*4),d0	; old c86zds column offset
	moveq	#0,d1
	move.b	0(a2,d0.l),d1	; final freshly rendered chunky pixel
	add	d1,d1
	move	0(a5,d1.w),d2	; RGB565 from c86zds/AGA8 LUT
	move	d2,(a1)+
	move	d2,(a1)+
	move	d2,(a0)+
	move	d2,(a0)+
