...
.here	move.l	font(pc),a0
	...
	jsr	g2p96_present_planar_strip_y_d6_to_window
.g2p96_no_char_flush
	endc
	;
	move	pdelay(pc),d2
...
.agaa	bsr	loadfiles
	...
	jsr	g2p96_present_planar_showbitmap_to_window	;c86zeg: single BlackMagic blit only
	; c86zeg: keep the known-good c86zdx audio/gameplay cadence, but do not
	; repeatedly re-convert the whole BlackMagic screen on real Amiga/P96.
	; The old repeated full blits caused line-by-line buildup and apparent hangs.
	move	#12,d7
.g2p96_bm_hold
	btst	#6,$bfe001	; left mouse/fire skips early (active low)
	beq.s	.g2p96_bm_done
	btst	#7,$bfe001	; joystick fire skips early (active low)
	beq.s	.g2p96_bm_done
	jsr	vwait
...
.done	; v190hy cleanup: log marker removed
	jsr	printmess2
	ifne	g2p96_overlay_enabled
	jsr	g2p96_present_planar_strip_y_d6_to_window	;c86zeg: dirty intermission text strip only
	endc
	;
	tst	pdelay
	bmi	execscript
...
.bwait	btst	#6,$dff002
	...
	bsr	disownblitter
	ifne	g2p96_overlay_enabled
	jsr	g2p96_present_planar_menuitem_to_window	;c86zeg: dirty selected menu strip only
	endc
	rts
...
p96prevlog_name	dc.b	'RAM:gloom_p96_preview.txt',0
	even
p96prevlog_ok	dc.b	'P96 preview: c86zeh fast-static/wsize/noclick active',10
...
g2p96_rect_active	dc	0	;c86zeg: nonzero = planar->P96 dirty strip mode
g2p96_rect_y	dc	0
g2p96_rect_h	dc	0
g2p96_rect_yend	dc	0
g2p96_rect_beg	dc	0
g2p96_rect_end	dc	40
...
g2p96_workbench_hold_probe	push
	; c86zeh: do not hold a grey P96 screen until the user clicks.
	; The window is still opened early for C790/FrameThrower safety, but startup
	; continues after one WaitTOF so BlackMagic/title handling owns the screen.
	move.l	p96preview_window(pc),d0
	beq.s	.done
	move.l	grbase(pc),d0
	beq.s	.done
...
; c86zei: restored dirty-strip wrapper helpers that c86zeh calls.
; They set the clipping rectangle and reuse the fast full planar bridge below.
g2p96_present_planar_menuitem_to_window
	movem.l	d0-d1,-(a7)
	move	curropt,d0
	mulu	fonth,d0
	add	menuy,d0
	move	d0,g2p96_rect_y
	move	fonth,g2p96_rect_h
	clr	g2p96_rect_beg
	move	#40,g2p96_rect_end
	move	#-1,g2p96_rect_active
	jsr	g2p96_present_planar_showbitmap_to_window
	clr	g2p96_rect_active
	movem.l	(a7)+,d0-d1
	rts

g2p96_present_planar_strip_y_d6_to_window
	movem.l	d0-d1,-(a7)
	move	d6,d0
	cmp	#0,d0
	bge.s	.y_ok_low
...
g2p96_present_planar_showbitmap_to_window
	...
	; Same format detection as the proven gameplay path.
...
.common
	...
	tst	g2p96_rect_active
	beq.s	.full_area
	move	g2p96_rect_y,d6
	move	d6,d0
	add	g2p96_rect_h,d0
	cmp	#240,d0
	ble.s	.yend_ok
	move	#240,d0
.yend_ok
	move	d0,g2p96_rect_yend
	bra.s	.row
.full_area
	moveq	#0,d6		; y 0..239
	move	#240,g2p96_rect_yend
	clr	g2p96_rect_beg
	move	#40,g2p96_rect_end
.row
	cmp	g2p96_rect_yend,d6
	bge.w	.rts
	; c86zeh: fast static planar bridge.  Compute the source row once and
	; read the 8 plane bytes inline for each group of 8 pixels instead of
	; calling the old per-pixel helper 76,800 times on a full screen.
	move	d6,d0
	mulu	linemodw,d0
	move.l	a6,a0
	...
	move	g2p96_rect_beg,d7	; byte column start
.bytecol
	cmp	g2p96_rect_end,d7
	bge.w	.nextrow
	move.l	a0,a1
	moveq	#0,d0
	move	d7,d0
	adda.l	d0,a1		; a1 = plane0 byte for this 8-pixel group
	moveq	#7,d1		; 8 pixels, masks $80..$01
	move	#$0080,d2
.pix_fast
...
.win_setup32
	...
	move	#240,d1	;c86zeh: centre selected WINDOW SIZE in full 320x240 P96 screen
	sub	hite,d1
	lsr	#1,d1
	mulu	d4,d1
	add.l	d1,a3
	...
	add.l	d1,a3
	lea	p96rgb888_lut,a5
	lea	coloffs,a6	;c86zeh: windowed source is read linearly, no coloffs repeat
	moveq	#0,d6
.win32_row
	cmp	hite,d6
	bge.w	g2p96_direct_full_rts
