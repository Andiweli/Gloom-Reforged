	...
	;gloom2p96_c4: safe P96 heartbeat-only window update; no Chunky sampling yet.
	;
...
p96prevlog_name	dc.b	'RAM:gloom_p96_preview.txt',0
	even
p96prevlog_ok	dc.b	'P96 preview: heartbeat only active',10
...
.nolog	movem.l	d0-d7/a0-a6,-(a7)
	...
	; gloom2p96_c4: heartbeat-only safety stage.  c3 proved that the
	; window can be updated, but the coarse Chunky sampling caused a
	; colourful mosaic and then a real-hardware crash.  Do not touch
	; chunky/coloffs here; only draw one visible strip so the next test
	; can prove the persistent P96-window write path is stable.
	move.l	50(a5),a1
	move.l	grbase(pc),a6
	move	p96preview_frame(pc),d0
	lsr	#4,d0
	...
	move	#319,d2
	move	#15,d3
	jsr	-306(a6)	;RectFill heartbeat strip only
