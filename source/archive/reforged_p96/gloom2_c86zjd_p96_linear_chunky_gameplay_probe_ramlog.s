...
g2release_marker	dc.b	'Gloom Reforged AGA/P96 (v1.6 c86zjd 12.07.2026) Andreas ',39,'Andiweli',39,' Stuermer',0
	even
...
.fail_bitmap
	...
	clr	p96gameplay_persist_active
	clr	p96gameplay_persist_frames
	; c86zjd: a frame rendered with linear coloffs is not valid C2P input.
	; Keep the last good P96 frame for this one failed present, restore the old
	; table, and let the following frame return cleanly to AGA.
	tst	p96gameplay_linear_active
	beq.s	.legacy_fail
	move	#-1,p96gameplay_skip_aga_present
...
.frame_ok
	move	#1,p96gameplay_state
	move	#-1,p96gameplay_skip_aga_present	;c86zfb: caller may skip doc2p/db for this frame
	; The first successful frame deliberately still uses the proven C2P-style
	; column table.  Arm linear rendering only for the next complete frame.
	tst	p96gameplay_linear_active
	bne.s	.linear_mark_done
