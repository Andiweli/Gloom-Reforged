	...
	;gloom2p96_c86zdu: passive 320x240 P96 ModeID probe only; no native screen, no new writer, OK path preserved.
	;
...
p96mode_state	dc	0	;c86zdu: 0=skipped/no p96, 1=ModeID found, -1=not found
p96mode_try	dc	0	;c86zdu: 0=none, 1=16-bit PC, 2=16-bit BGR
p96mode_id	dc.l	0	;c86zdu: DisplayID returned by p96BestModeIDTagList

; c86zdu P96 tag constants used numerically to keep Devpac/GenAm standalone.
; TAG_USER=$80000000, P96BIDTAG_Dummy=TAG_USER+96.
p96bestmode_tags
...
; c86zdu: passive ModeID-only probe. This deliberately does not open
; a native screen and does not change the existing Workbench/P96 OK path.
...
p96prevlog_name	dc.b	'RAM:gloom_p96_preview.txt',0
	even
p96prevlog_ok	dc.b	'P96 preview: c86zdu passive 320x240 ModeID probe on OK path active',10
...
; c86zdu: passive 320x240 16-bit ModeID query.
; Important: this routine only asks Picasso96 for a suitable DisplayID.
; It does not open a native screen and therefore cannot disturb title music,
; static screens, or the proven Workbench/P96 OK path.
g2p96_find_320x240_modeid
	...
	beq	.fail
	move.l	d0,a6
	move.l	#16,p96bestmode_formatsallowed	;RGBFF_R5G6B5PC, preferred first
	lea	p96bestmode_tags,a0
	...
	bra	.done
.try_bgr
	move.l	#4096,p96bestmode_formatsallowed	;RGBFF_B5G6R5PC fallback
	lea	p96bestmode_tags,a0
	jsr	-60(a6)	;p96BestModeIDTagList(tags)
	cmp.l	#-1,d0	;INVALID_ID
	beq	.fail
...
; c86zdu: write passive ModeID result to RAM:gloom_p96_modeid.txt.
g2p96_mode_log
	push
	move.l	dosbase,d0
	beq.s	.done
	move.l	d0,a6
	lea	p96modelog_name,a0
	move.l	a0,d1
	move.l	#1006,d2	;MODE_NEWFILE
	jsr	-30(a6)	;Open
	move.l	d0,d7
	...
	cmp	#2,d0
	beq.s	.ok_bgr
	lea	p96modelog_ok_pc,a0
	move.l	#p96modelog_ok_pc_len,d3
	bra.s	.write
.ok_bgr
	lea	p96modelog_ok_bgr,a0
	move.l	#p96modelog_ok_bgr_len,d3
	bra.s	.write
.fail
	lea	p96modelog_fail,a0
	move.l	#p96modelog_fail_len,d3
	bra.s	.write
.skip
	lea	p96modelog_skip,a0
...
g2p96_test_window	push
	...
	clr	p96mode_state	;c86zdu: passive ModeID probe state
	clr	p96mode_try
	clr.l	p96mode_id
	...
	tst	p96present
	beq.s	.logonly
	jsr	g2p96_find_320x240_modeid	;c86zdu: passive only, do not open native screen
	move.l	int,d0
	beq.s	.fail
	move.l	d0,a6
...
.logonly	jsr	g2p96_mode_log	;c86zdu: extra passive ModeID marker
