	...
	;gloom2p96_c86zdf: larger 640x384 scaled live preview inside 640x480 borderless P96 shell; keeps throttle8 and focus shell from c86zde.
	;
...
p96prevlog_name	dc.b	'RAM:gloom_p96_preview.txt',0
	even
p96prevlog_ok	dc.b	'P96 preview: c86zdf 640x384 scaled shell active',10
...
p96test_window	dc	0,0	;c86zde: cover Workbench/P96 from top-left
	dc	640,480	;c86zde: large borderless shell, 320x240 preview centered inside
	dc.b	0,1	;pens
...
g2p96_draw_testpattern	push
	...
	move	#639,d2
	move	#479,d3
	jsr	-306(a6)	;c86zde RectFill full 640x480 shell
	moveq	#0,d4
...
.set_pen
	...
	add	#160,d0		;c86zde center 320-wide hold grid in 640-wide shell
	move	d0,d2
	add	#19,d2		;x2
	move	d7,d6
	...
	add	#120,d1		;c86zde center 240-high hold grid in 480-high shell
	move	d1,d3
	add	#14,d3		;y2
	move.l	50(a5),a1
	move.l	grbase,a6
...
.nolog	movem.l	d0-d7/a0-a6,-(a7)
	...
	; c86zdf: scale the proven 320x192 live preview up to a 640x384
	; visible play area inside the 640x480 shell.  This is still the safe
	; Pen/RectFill path, not yet a true RGB/WPA game mode.  Keep only a
	; 48px black letterbox above and below the larger play area.
	move.l	50(a5),a1
	move.l	grbase(pc),a6
	moveq	#0,d0
	jsr	-342(a6)	;SetAPen black
	moveq	#0,d0
	moveq	#0,d1
	move	#639,d2
	move	#47,d3
	jsr	-306(a6)	;c86zdf RectFill top 48px letterbox
	move.l	50(a5),a1
	move.l	grbase(pc),a6
	moveq	#0,d0
	jsr	-342(a6)	;SetAPen black
	...
	move	#639,d2
	move	#479,d3
	jsr	-306(a6)	;c86zdf RectFill bottom 48px letterbox
...
.c26noframemirror
	...
	;c86zdf: draw the complete 96-row sampled preview as 640x384.
	;Each 320-wide staged row is doubled horizontally and expanded to 4
	;destination rows.  Destination is x=0..639, y=48..431.
	move.l	p96preview_frameptr(pc),d0
...
.c33drawfromready
	cmp	#96,d6		;c86zdc: draw all 96 sampled rows -> 320x192 image at y=24..215
	bge	.c22nextrow
...
.c22xloop
	...
	add	d0,d0		;c86zdf x1 = run_start * 2
	move	d6,d1
	lsl	#2,d1
	add	#48,d1		;c86zdf y = source_row * 4 + 48
	move	d5,d2
	add	d2,d2
	subq	#1,d2		;c86zdf x2 = d5*2-1
	move	d1,d3
	addq	#3,d3		;c86zdf 4 destination rows per source row
	jsr	-306(a6)	;RectFill merged scaled horizontal run
.c22startrun
	move	d4,d7
...
.c22flushrow
	...
	move	d1,d3
	addq	#3,d3
	jsr	-306(a6)	;RectFill final scaled run
.c22nextrow
	addq	#1,d6
	bra	.c22rowloop
.c22done
...
g2p96_clear_preview_window	push
	...
	move	#639,d2
	move	#479,d3
	jsr	-306(a6)	;c86zde RectFill full shell/clear stale preview
.done	pull
	rts
