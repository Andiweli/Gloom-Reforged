...
p96prevlog_name	dc.b	'RAM:gloom_p96_preview.txt',0
	even
p96prevlog_ok	dc.b	'P96 preview: RGB12 dense bottom safe rollback active',10
...
p96preview_rgb24bottomptr	dc.l	0	;gloom2p96_c49: unused rollback marker; c48 bottom buffer disabled
...
.c22nextrow
	...
	bra	.c22rowloop
.c22done
	;gloom2p96_c49: safe rollback to the c47-style RGB12 dense bottom probe.
	;c48's dedicated RGB24 bottom buffer blocked the Intermission/continue path
	;on real hardware.  This keeps the proven RGB12 2D mini-image, avoids the
	;extra 23 KB allocation, and still does not touch P96 pens/palette/SetRGB.
...
.c49rowloop
	...
	bge	.restore
	move	d6,d0
	mulu	#4,d0		;sample rows 0,4,8...92 from RGB12 buffer
...
.c49xloop
	...
	;Derive the same simple RGB332-style Pen candidate directly from RGB12.
	;This remains false-colour Workbench/P96 Pen output, but it avoids the
	;problematic c48 RGB24 bottom staging buffer completely.
	move	d0,d4
	and	#$0e00,d4	;R top 3 bits
	lsr	#4,d4		;-> bits 7..5
	move	d0,d3
	and	#$00e0,d3	;G top 3 bits
	lsr	#3,d3		;-> bits 4..2
	or	d3,d4
	move	d0,d3
	and	#$000c,d3	;B top 2 bits
	lsr	#2,d3		;-> bits 1..0
	or	d3,d4
	move	d4,d0
	bne.s	.c49penok
...
.c49penok
	...
	move	d5,d0
	move	d6,d1
	add	#216,d1		;bottom mini-window rows 216..239, 1 row per RGB12 row
	move	d5,d2
	addq	#1,d2
	move	d1,d3
	jsr	-306(a6)	;RectFill 2x1 RGB12 dense probe block
	addq	#2,d5
