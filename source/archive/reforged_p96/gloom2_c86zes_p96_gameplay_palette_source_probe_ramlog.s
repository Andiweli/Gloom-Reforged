...
g2release_marker	dc.b	'Gloom Reforged AGA/P96 probe (v1.4 c86zei 29.06.2026) Andreas ',39,'Andiweli',39,' Stuermer',0
	even
...
.waitskip2	; c86m: normal C2P/show path restored after c86l label regression
	jsr	doc2p
	jsr	db
	jsr	g2p96_gameplay_present_probe	;c86zem: 160x96 burst gameplay P96 diagnostic probe
	ifne	g2p96_overlay_enabled
...
; -----------------------------------------------------------------------------
; c86zes: direct RGB565 palette-source probe.
; c86zer proved that direct RGB565 + gameplay chunky data works, but
; planar_palette high-word lookup produced green/black/violet wrong colours.
; This version keeps the same bounded P96 probe and draws four small source
; panels side-by-side/stacked so we can identify the correct palette source.
; It still does not skip the normal AGA doc2p/db present path.
; -----------------------------------------------------------------------------
...
p96gameplay_latched		dc	0	;one-shot latch so disk logging happens only once
...
p96gameplay_rgb565_pal_state	dc	0	;0=not built, -1=palette LUT valid
p96gameplay_pen64_tab2	dc.b	0,85,170,255
p96gameplay_burst_phase	dc	0
p96gameplay_bpr		dc.l	0
p96gameplay_bitmapbase	dc.l	0
p96gameplay_panel_mode	dc	0
p96gameplay_rgb565_pair_state	dc	0
p96gameplay_rgb565_lastpal_state	dc	0
p96gameplay_log_name	dc.b	'RAM:gloom_p96_gameplay.txt',0
p96gameplay_msg_ok	dc.b	'P96 gameplay: direct RGB565 palette-source OK',10
...
.have_p96
	...
	bra	.log
.try_marker
	jsr	g2p96_gameplay_direct_rgb565_bars_probe
.log
...
g2p96_gameplay_direct_rgb565_bars_probe
	push
	move.l	p96modeid,d0
	beq	.nomode
	move.l	d0,p96winprobe_displayid_tag+4
	...
	move.l	4.w,a6
	lea	intname,a1
	jsr	-408(a6)	;OldOpenLibrary intuition.library for this one-shot marker
	move.l	d0,d6
	beq	.noint
	move.l	d6,a6
	sub.l	a0,a0
	...
	moveq	#2,d7	;3 bounded direct RGB565 refreshes, about 3 seconds total
.burst_loop
	move	d7,p96gameplay_burst_phase
	move.l	d4,a0
	bsr	g2p96_gameplay_draw_mini_frame_160x96
	bsr	g2p96_gameplay_tiny_wait
	dbf	d7,.burst_loop
...
; a0 = Window.  c86zes direct RGB565 palette-source probe.
; Draws fixed RGB565 bars plus four 80x48 gameplay mini-panels:
;   top-left     = pseudo/index RGB565 reference from c86zeq
;   top-right    = planar_palette high-word-only lookup from c86zer
;   bottom-left  = planar_palette AGA high+low pair lookup
;   bottom-right = lastpal AGA high+low pair lookup
; All panels use the same live chunky gameplay source and direct P96 bitmap writes.
g2p96_gameplay_draw_mini_frame_160x96
	...
	move.l	a1,d0
	beq	.done
	move.l	4(a1),a2	;RastPort.BitMap
	move.l	a2,d0
	beq	.done
	moveq	#0,d6
	move	0(a2),d6	;BitMap.BytesPerRow
	and.l	#$0000ffff,d6
	beq	.done
	move.l	8(a2),a3	;BitMap.Planes[0] / chunky truecolour base on P96
	move.l	a3,d0
	beq	.done
	move	#-1,p96gameplay_direct_state
	move.l	d6,p96gameplay_bpr
	move.l	a3,p96gameplay_bitmapbase
	bsr	g2p96_gameplay_build_rgb565_palette
	bsr	g2p96_gameplay_build_rgb565_palette_pair
	bsr	g2p96_gameplay_build_rgb565_lastpal_pair

	; Top calibration ramp: 16 rows of 8 fixed direct RGB565 colour bars.
	lea	p96gameplay_rgb565_normal,a4
	move.l	a3,a0
	move	#15,d7
.cal_rows
	bsr	g2p96_gameplay_draw_rgb565_bar_row
	adda.l	d6,a0
	dbf	d7,.cal_rows

	; Four comparative palette-source panels below the calibration ramp.
	; Panel 0: pseudo/reference, x=0 y=24.
	clr	p96gameplay_panel_mode
	moveq	#0,d0
	move	#24,d1
	bsr	g2p96_gameplay_draw_source_panel_80x48

	; Panel 1: planar_palette high word only, x=160 y=24.
	move	#1,p96gameplay_panel_mode
	move	#160,d0
	move	#24,d1
	bsr	g2p96_gameplay_draw_source_panel_80x48

	; Panel 2: planar_palette AGA high+low pair, x=0 y=132.
	move	#2,p96gameplay_panel_mode
	moveq	#0,d0
	move	#132,d1
	bsr	g2p96_gameplay_draw_source_panel_80x48

	; Panel 3: lastpal AGA high+low pair, x=160 y=132.
	move	#3,p96gameplay_panel_mode
	move	#160,d0
	move	#132,d1
	bsr	g2p96_gameplay_draw_source_panel_80x48
.done
...
; d0 = destination X in pixels, d1 = destination Y in pixels.
; p96gameplay_panel_mode selects pseudo/high-only/paired/lastpal palette source.
; Draws an 80x48 sample field scaled to 160x96 pixels.
g2p96_gameplay_draw_source_panel_80x48
	push
	move.l	p96gameplay_bitmapbase,a0
	move.l	a0,d2
	beq	.done
	move.l	p96gameplay_bpr,d6
	move.l	d6,d2
	beq	.done
	move	d1,d7
	mulu	d6,d7
	adda.l	d7,a0
	move	d0,d7
	add	d7,d7	; X * 2 bytes per RGB565 pixel
	adda.w	d7,a0
	move.l	chunky,d0
	beq	.done
	moveq	#0,d7	; row 0..47
.rowloop
	...
	bge	.done
	move	d7,d0
	lsl	#2,d0	; source y = row * 4
	mulu	chunkymodw,d0
	move.l	chunky,a2
	lea	0(a2,d0.l),a2
	move.l	a0,a1	; destination row 1
	move.l	a0,a4
	adda.l	d6,a4	; destination row 2
	moveq	#0,d5	; column 0..79
.colloop
	cmp	#80,d5
	bge	.nextrow
	move	d5,d4
	lsl	#2,d4	; source x = col * 4
	moveq	#0,d3
	move.b	0(a2,d4.w),d3	; live chunky pixel/index
	bsr	g2p96_gameplay_index_to_rgb565_source
	move	d1,(a1)+
	move	d1,(a1)+
	move	d1,(a4)+
	move	d1,(a4)+
...
.nextrow
	adda.l	d6,a0
	adda.l	d6,a0	; advance destination by 2 rows
	addq	#1,d7
	bra	.rowloop
.done
	pull
...
; d3 = chunky index.  Returns d1 = RGB565 colour from selected source.
g2p96_gameplay_index_to_rgb565_source
	movem.l	d0/d2/a6,-(a7)
	move	p96gameplay_panel_mode,d0
	cmp	#1,d0
	beq	.highonly
	cmp	#2,d0
	beq	.paired
	...
	bra	.pseudo
.highonly
	tst	p96gameplay_rgb565_pal_state
	beq	.pseudo
	lea	p96gameplay_rgb565_palette,a6
	move	d3,d2
	add	d2,d2
	move	0(a6,d2.w),d1
	bra	.done
.paired
	tst	p96gameplay_rgb565_pair_state
	beq	.pseudo
	lea	p96gameplay_rgb565_palette_pair,a6
	move	d3,d2
	add	d2,d2
	move	0(a6,d2.w),d1
	bra	.done
.lastpal
	tst	p96gameplay_rgb565_lastpal_state
	beq	.pseudo
	lea	p96gameplay_rgb565_lastpal_pair,a6
	move	d3,d2
	add	d2,d2
	move	0(a6,d2.w),d1
	bra	.done
.pseudo
	move	d3,d1
	and	#$00e0,d1
	lsl	#8,d1	;red contribution -> RGB565 high bits
	move	d3,d2
	and	#$001c,d2
	lsl	#6,d2	;green contribution
	or	d2,d1
	move	d3,d2
	and	#$0003,d2
	lsl	#3,d2	;blue contribution
	or	d2,d1
.done
	movem.l	(a7)+,d0/d2/a6
	rts

; a0 = destination row start, a4 = 8 RGB565 words.
; Draws eight 40-pixel-wide 16-bit colour bars = 320 pixels.
g2p96_gameplay_draw_rgb565_bar_row
	push
	move.l	a0,a1
	moveq	#0,d5	;bar index
.bar_loop
	cmp	#8,d5
	bge	.done
	move	d5,d0
...
; Build a 256-entry chunky-index -> RGB565 LUT from the active 12-bit planar
; palette.  planar_palette entries are addressed as words at index*4 in the
; existing renderer/tint code, with RGB444 layout $0RGB.  If no palette is
; available, the caller falls back to the older pseudo-colour mapping.
g2p96_gameplay_build_rgb565_palette
	push
	clr	p96gameplay_rgb565_pal_state
	move.l	planar_palette,d0
	beq	.done
	move.l	d0,a0
	lea	p96gameplay_rgb565_palette,a1
	moveq	#0,d7
.loop
	cmp	#256,d7
	bge	.valid
	move	d7,d6
	lsl	#2,d6
	move	0(a0,d6.w),d0	;RGB444 word, same source used by tint helpers

	; Red: 4-bit -> 5-bit -> RGB565 bits 15..11
	move	d0,d1
	lsr	#8,d1
	and	#$000f,d1
	move	d1,d2
	...
	; Green: 4-bit -> 6-bit -> RGB565 bits 10..5
	move	d0,d2
	lsr	#4,d2
	and	#$000f,d2
	move	d2,d3
	...
	; Blue: 4-bit -> 5-bit -> RGB565 bits 4..0
	move	d0,d2
	and	#$000f,d2
	move	d2,d3
	add	d2,d2		;b4 << 1
	...
	bra	.loop
.valid
	move	#-1,p96gameplay_rgb565_pal_state
.done
	pull
	rts


; Build RGB565 LUT from AGA-style paired palette words at planar_palette.
; This combines the high and low nibble words exactly like pokepal_os does,
; then reduces 8-bit RGB to RGB565.  This is the most likely correct source
; for actual AGA-loaded Gloom palette data.
g2p96_gameplay_build_rgb565_palette_pair
	push
	clr	p96gameplay_rgb565_pair_state
	move.l	planar_palette,d0
	beq	.done
	move.l	d0,a0
	lea	p96gameplay_rgb565_palette_pair,a1
	moveq	#0,d7
.loop
	cmp	#256,d7
	bge	.valid
	move	d7,d6
	lsl	#2,d6
	move	0(a0,d6.w),d0	;AGA high nibble word
	move	2(a0,d6.w),d4	;AGA low nibble word
	bsr	g2p96_gameplay_agapair_to_rgb565
	move	d7,d6
	add	d6,d6
	move	d1,0(a1,d6.w)
	addq	#1,d7
	bra	.loop
.valid
	move	#-1,p96gameplay_rgb565_pair_state
.done
	pull
	rts

; Build RGB565 LUT from the currently poked display palette pointer.
; lastpal is set by pokepal whenever the active Amiga display palette changes.
g2p96_gameplay_build_rgb565_lastpal_pair
	push
	clr	p96gameplay_rgb565_lastpal_state
	move.l	lastpal,d0
	beq	.done
	move.l	d0,a0
	lea	p96gameplay_rgb565_lastpal_pair,a1
	moveq	#0,d7
.loop
	cmp	#256,d7
	bge	.valid
	...
	bra	.loop
.valid
	move	#-1,p96gameplay_rgb565_lastpal_state
.done
	pull
	rts

; d0 = AGA high-nibble RGB word, d4 = AGA low-nibble RGB word.
; Return d1 = RGB565.  Mirrors pokepal_os channel reconstruction:
; R = high $f00 >> 4 | low $f00 >> 8, G = high $0f0 | low $0f0 >> 4,
; B = high $00f << 4 | low $00f.
g2p96_gameplay_agapair_to_rgb565
	movem.l	d2-d5,-(a7)
	; Red 8-bit -> RGB565 red 5-bit at bits 15..11
	move	d0,d1
	and	#$0f00,d1
	lsr	#4,d1
	move	d4,d2
	...
	lsl	#8,d1
	lsl	#3,d1
	; Green 8-bit -> RGB565 green 6-bit at bits 10..5
	move	d0,d2
	and	#$00f0,d2
	move	d4,d3
	and	#$00f0,d3
	...
	lsl	#5,d2
	or	d2,d1
	; Blue 8-bit -> RGB565 blue 5-bit at bits 4..0
	move	d0,d2
	and	#$000f,d2
	lsl	#4,d2
	move	d4,d3
...
p96gameplay_rgb565_palette
	dcb.w	256,0
p96gameplay_rgb565_palette_pair
	dcb.w	256,0
p96gameplay_rgb565_lastpal_pair
	dcb.w	256,0
	even
