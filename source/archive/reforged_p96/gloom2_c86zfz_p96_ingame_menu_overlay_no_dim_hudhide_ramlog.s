...
dogamemenu	;
	...
	move	#$8020,$dff09a
	;
	jsr	g2p96_display_should_lock_fullwin	;c86zfz: P96 menu overlays live game, no grey backdrop
	tst	d0
	bne.s	.p96_no_grey_menu_backdrop
	jsr	g2v190aj_grey_menu_backdrop	; AGA/original path keeps grey current chunky frame
.p96_no_grey_menu_backdrop
	bsr	trainer_update_display_texts
	lea	gamemenu,a4
	jsr	initmenu
	jsr	g2p96_menu_hide_hud_regions	;c86zfz: hide HUD left/right behind P96 menu overlay
	jsr	g2p96_menu_bridge_all_rows_present	;c86zfz: text-only overlay, keep live P96 gameplay backdrop
...
refresh	jsr	dispoff
	...
	jsr	g2p96_display_should_lock_fullwin	;c86zfz: P96 menu keeps live backdrop, AGA keeps grey backdrop
	tst	d0
	bne.s	.g2v190aj_refresh_nogrey
...
.bwait	btst	#6,$dff002
	...
	;
	bsr	disownblitter
	jsr	g2p96_menu_bridge_current_row_present	;c86zfx: only copy restored row on real hardware
	rts
...
.loop	addq	#1,d0
	...
	;
	move	#13,flashdelay
	jsr	g2p96_menu_bridge_current_row_present	;c86zfx: only copy selected row on real hardware
	;
	rts
...
p96gameplay_msg_ok	dc.b	'P96 gameplay: ingame menu overlay no-dim HUD-hide OK',10
...
g2p96_ingame_menu_floorceil_live_refresh
	...
	jsr	finitmenu
	jsr	predrawall
	; c86zfy: redraw the changed gameplay backdrop through the fast P96
	; gameplay copy path, then overlay only the menu text rows.  The old c86zfw
	; full planar-menu bridge copied 320x240 pixel-by-pixel and visibly built
	; from top to bottom on real A1200 RTG hardware.
	jsr	g2p96_gameplay_persistent_update
	clr	p96gameplay_skip_aga_present
	;c86zfz: no grey backdrop under P96. Keep the updated gameplay frame visible.
	move	(a7),curropt
	lea	gamemenu,a4
	jsr	initmenu2
	move	(a7),curropt
...
.no_p96_state
	jsr	g2p96_menu_hide_hud_regions	;c86zfz: hide HUD before redrawing menu overlay
	jsr	g2p96_menu_bridge_all_rows_present	;c86zfz: rows only, text over live game backdrop
...
; c86zfz: hide the gameplay HUD under the P96 in-game menu without greying the
; whole screen.  This blanks only top-left and top-right HUD regions in the live
; direct RGB565 P96 framebuffer.  The gameplay image remains visible elsewhere.
g2p96_menu_hide_hud_regions
	movem.l	d0-d7/a0-a3,-(a7)
	...
	; HUD source area is roughly 80x40 at 320x240.  Scale the clear regions
	; according to the selected P96 output mode.
	move	#80,d2		; clear width
	move	#40,d3		; clear height
	move	p96target_mode,d0
	beq.s	.size_ready	; 320x240
	cmp	#1,d0
	beq.s	.hires_2x
	cmp	#2,d0
	beq.s	.stretch_480
...
.stretch_480
	...
	lsr	#2,d2		; left/right quarters correspond to 80/320 source width
	move	#80,d3
	bra.s	.size_ready
.stretch_240
	move	p96target_width,d2
...
.width_ok
	...
	moveq	#0,d5
	bsr	.clear_rect
	; right HUD: x=target_width-clear_width
...
.clear_rect
	; input: d2=width, d3=height, d5=x start, a3=base, d4=BPR
	movem.l	d0-d1/d6-d7/a0-a1,-(a7)
	moveq	#0,d6		; y
.row_loop
	cmp	d3,d6
	bge.s	.rect_done
...
.next_row
	...
	bra.s	.row_loop
.rect_done
	movem.l	(a7)+,d0-d1/d6-d7/a0-a1
	rts
.done
	movem.l	(a7)+,d0-d7/a0-a3
	rts

; c86zfz: initial/menu-rebuild bridge for real hardware.  Keep the current
; P96 gameplay frame as the backdrop and copy only non-background menu glyph
; pixels from the original planar menu bitmap.  This avoids grey/dim full-screen
; backdrop work and overlays the menu directly over the paused gameplay image.
...
.loop
	...
	bge.w	.done
	move	d7,curropt
	jsr	g2p96_menu_bridge_current_row_present
	addq	#1,d7
	bra.w	.loop
.done
	move	(a7)+,curropt
...
; c86zfx: fast row-only bridge for real hardware menu navigation.  opton/optoff
; only redraw one menu strip in the original menu bitmap, so copying the whole
; 320x240 P96 bridge on every cursor movement is unnecessarily slow on A1200 RTG.
g2p96_menu_bridge_current_row_present
	...
	move	#1,p96menu_bridge_row_only
	jsr	g2p96_menu_bridge_present
	clr	p96menu_bridge_row_only
	movem.l	(a7)+,d0-d1
	rts

; c86zfr: copy the current original in-game menu planar bitmap into the live
; P96 framebuffer.  This is deliberately a bridge, not the final P96-native menu:
; the existing menu code still owns drawing/input, but P96 stays the visible
; screen and receives a full 320x240/HIRES/STRETCH copy after menu row changes.
g2p96_menu_bridge_present
	...
	move.l	lastpal,d0	;c86zfr: active menu/backdrop palette first
	bne.s	.have_palette
	move.l	planar_palette,d0
	beq.w	.done
.have_palette
	...
	cmp.l	d0,d4
	blo.w	.done
	bsr	g2p96_menu_build_rgb565_lut
	...
	tst	p96menu_bridge_row_only	;c86zfx: row-only fast path for menu highlight changes
	beq.s	.fullcopy
...
.fullcopy
	moveq	#0,d6		; source y 0..239
.rowloop
	tst	p96menu_bridge_row_only
	beq.s	.check_fullcopy
...
.loop1
	cmp	#320,d7
	bge.w	.nextrow
	bsr	g2p96_menu_planar_pixel
	tst	d1		;c86zfz: palette index 0 = transparent menu background
	beq.s	.skip1
	bsr	g2p96_menu_pixel_rgb565	;c86zfv: grey disabled text pixels only
	move	d2,(a1)+
	bra.s	.adv1
.skip1	addq.l	#2,a1
.adv1	addq	#1,d7
...
.loop2
	...
	bge.w	.nextrow
	bsr	g2p96_menu_planar_pixel
	tst	d1		;c86zfz: transparent menu background
	beq.s	.skip2
	bsr	g2p96_menu_pixel_rgb565	;c86zfv: grey disabled text pixels only
	move	d2,(a1)+
	move	d2,(a1)+
...
.stretch480_src_loop
	...
	bge.w	.nextrow
	bsr	g2p96_menu_planar_pixel
	move	d1,d5		;c86zfz: remember palette index for transparent background
	move	d2,-(a7)	; preserve destination-x count
	tst	d5
	beq.s	.stretch480_transparent
	bsr	g2p96_menu_pixel_rgb565	;c86zfv: grey disabled text pixels only
	move	d2,d1		; emitted RGB565 colour
	bra.s	.stretch480_have_colour
.stretch480_transparent
	moveq	#0,d1
.stretch480_have_colour
...
.stretch480_emit
	...
	bge.w	.nextrow
	tst	d5
	beq.s	.stretch480_skip_write
	move	d1,(a1)+
	move	d1,(a0)+
	bra.s	.stretch480_advance
.stretch480_skip_write
	addq.l	#2,a1
	addq.l	#2,a0
.stretch480_advance
	addq	#1,d2
...
.stretch240_src_loop
	...
	move	d2,-(a7)	; preserve destination-x count
	tst	d5
	beq.s	.stretch240_transparent
	bsr	g2p96_menu_pixel_rgb565	;c86zfv: grey disabled text pixels only
	move	d2,d1		; emitted RGB565 colour
	bra.s	.stretch240_have_colour
.stretch240_transparent
	moveq	#0,d1
.stretch240_have_colour
	move	(a7)+,d2	; restore destination-x count
	add	p96target_width,d3
.stretch240_emit
	cmp	p96target_width,d2
	bge.w	.nextrow
	tst	d5
	beq.s	.stretch240_skip_write
	move	d1,(a1)+
	bra.s	.stretch240_advance
.stretch240_skip_write
	addq.l	#2,a1
.stretch240_advance
	addq	#1,d2
	sub	#320,d3
...
.nextrow
	...
	bra.w	.rowloop
.postcopy
	; c86zfv: no full-row dim pass. Disabled rows are greyed per text pixel
	; during copy, so the menu background stays unchanged and does not flicker.
.done
	movem.l	(a7)+,d0-d7/a0-a6
	rts

; c86zfv: convert one menu palette index to RGB565 for the P96 bridge.
; VIEW SIZE and FULL SCREEN stay visible/selectable in P96, but their glyph
; pixels use neutral greys.  Background pixels are copied unchanged.
...
g2p96_menu_pixel_rgb565
	...
	cmp	#2,d1
	beq.s	.grey2
	move	#$79ce,d2	; light grey via confirmed RGB565 byte order
	bra.s	.done
.grey2	move	#$518c,d2	; mid grey via confirmed RGB565 byte order
	bra.s	.done
.grey1	move	#$2842,d2	; dark grey via confirmed RGB565 byte order
	bra.s	.done
.normal	add	d1,d1
	move	0(a5,d1.w),d2
...
; c86zft: visually grey/dim the disabled VIEW SIZE and FULL SCREEN rows in
; the P96 bridge.  The original AGA menu table is left intact, so option
; indices, EXIT row and DISPLAY=AGA behaviour remain unchanged.
; input from g2p96_menu_bridge_present: a3=P96 base, d4=BytesPerRow.
g2p96_menu_dim_disabled_rows
	movem.l	d0-d7/a0-a1,-(a7)
	jsr	g2p96_display_should_lock_fullwin
	tst	d0
	beq.w	.done
	...
	move	fonth,d1
	add	d1,d0
	add	d1,d0		; source row index 2: VIEW SIZE
	move	d1,d3
	add	d1,d3		; two menu rows: VIEW SIZE + FULL SCREEN
...
.pixel_loop
	...
	move	d0,d6
	and	#$07e0,d6
	lsr	#5,d6		; green 0..63 as brightness base
	move	d6,d7
	lsr	#2,d7		; dim grey 5-bit 0..15
	move	d7,d0
	lsl	#8,d0
	lsl	#3,d0		; R bits 11..15
	move	d6,d2
	lsr	#1,d2		; dim grey 6-bit 0..31
	lsl	#5,d2		; G bits 5..10
	or	d2,d0
	or	d7,d0		; B bits 0..4
	move	d0,(a1)+
	dbf	d5,.pixel_loop
...
; Use lastpal first because the ESC menu has already poked the grey backdrop
; palette; planar_palette is only a fallback.
...
p96menu_bridge_row_only
	dc	0	;c86zfx: nonzero = copy only selected menu row range
