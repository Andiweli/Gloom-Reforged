...
; -----------------------------------------------------------------------------
; c86zet: direct RGB565 gameplay mini-frame using the old proven c86zds
; source-byte -> RGB565 LUT path.  c86zer/c86zes proved that direct RGB565
; output is real P96 but that naïve planar_palette/lastpal lookup gives
; green/violet false colours.  The old working c86zds path first inverted the
; paladjust/C2P-scrambled chunky byte through g2_strip_invpal, then read the
; active planar_palette and converted RGB12 to RGB565.  This probe ports only
; that colour conversion into the current clean ModeID/OpenScreenTagList path.
; -----------------------------------------------------------------------------
...
p96gameplay_msg_ok	dc.b	'P96 gameplay: source-region probe OK',10
...
; a0 = Window.  c86zev source-region probe.
; Writes a direct RGB565 calibration ramp plus four small 80x48 sampled
; gameplay-region panels scaled to 160x96 each.  c86zeu proved the old c86zds
; colour path and coloffs sampling can show floor/ceiling with sane colours,
; but not the full 1:1 wall image.  This diagnostic keeps the same direct
; RGB565/LUT path and compares four source addressing variants on one screen:
; TL = coloffs x*4, y*4; TR = linear x*4, y*4;
; BL = coloffs x*4, y*5 full-height; BR = coloffs x*4, centre y*2+48.
; The goal is to identify which source region contains the wall columns before
; re-enabling the full old P96-present path.
...
.cal_rows
	...
	move.l	chunky,d0
	beq	.fallback
	move.l	d0,a3	;base chunky pointer
	move	chunkymodw,d2	;source chunky row stride
	beq	.fallback
	lea	p96gameplay_rgb565_source_lut,a6
	lea	coloffs,a5
	move.l	8(a2),a0	;P96 bitmap base

	; TL: current-style coloffs, broad y*4 coverage.
	move	#0,p96gameplay_region_dstx
	move	#24,p96gameplay_region_dsty
	move	#0,p96gameplay_region_mode
	bsr	g2p96_gameplay_draw_region_panel_80x48

	; TR: same y range but linear source x, useful to catch coloffs mismatch.
	move	#160,p96gameplay_region_dstx
	move	#24,p96gameplay_region_dsty
	move	#1,p96gameplay_region_mode
	bsr	g2p96_gameplay_draw_region_panel_80x48

	; BL: coloffs with y*5 to cover almost the full 240 source height.
	move	#0,p96gameplay_region_dstx
	move	#120,p96gameplay_region_dsty
	move	#3,p96gameplay_region_mode
	bsr	g2p96_gameplay_draw_region_panel_80x48

	; BR: coloffs but centred vertically around the usual wall band.
	move	#160,p96gameplay_region_dstx
	move	#120,p96gameplay_region_dsty
	move	#2,p96gameplay_region_mode
	bsr	g2p96_gameplay_draw_region_panel_80x48
	bra	.done

.fallback
	; If chunky is unexpectedly unavailable, keep the failure visible with the
	; direct RGB565 bars instead of silently showing a blank screen.
	move.l	8(a2),a0
	lea	p96gameplay_rgb565_normal,a4
	move	#239,d7
.fallback_rows
	bsr	g2p96_gameplay_draw_rgb565_bar_row
	adda.l	d6,a0
	dbf	d7,.fallback_rows
.done
	pull
	rts

; Draw one 80x48 source panel scaled 2x2 into a 160x96 screen quadrant.
; Inputs:
;   a0 = P96 bitmap base, d6 = P96 bytes/row,
;   a3 = chunky source base, d2 = source row stride,
;   a5 = coloffs, a6 = RGB565 source LUT.
; Globals select destination and source mode.
g2p96_gameplay_draw_region_panel_80x48
	push
	move	p96gameplay_region_dsty,d0
	mulu	d6,d0
	adda.l	d0,a0
	move	p96gameplay_region_dstx,d0
	add	d0,d0	;16-bit destination x -> byte offset
	adda.l	d0,a0
	moveq	#0,d7	;panel row 0..47
.rowloop
	cmp	#48,d7
	bge	.done
	move	p96gameplay_region_mode,d0
	cmp	#2,d0
	beq	.mode_center
	cmp	#3,d0
	beq	.mode_fullheight
...
.mode_center
	move	d7,d0
	add	d0,d0	;y = row*2 + 48, centre-heavy wall band probe
	add	#48,d0
	bra	.have_y
.mode_fullheight
	move	d7,d0
	mulu	#5,d0	;y = row*5, almost full 0..235 source range
.have_y
	mulu	d2,d0
	move.l	a3,a2
	adda.l	d0,a2	;source row pointer
	move.l	a0,a1	;destination row 1
	move.l	a0,a4
	adda.l	d6,a4	;destination row 2
	moveq	#0,d5	;panel column 0..79
.colloop
	cmp	#80,d5
	bge	.nextrow
	move	d5,d4
	lsl	#2,d4	;source x = column*4
	move	p96gameplay_region_mode,d0
	cmp	#1,d0
	beq	.linear_x
	move.l	0(a5,d4*4),d0	;c86zds coloffs source column
	bra	.have_x
.linear_x
	move.l	d4,d0	;plain linear chunky column offset
.have_x
	moveq	#0,d3
	move.b	0(a2,d0.l),d3	;live chunky pixel/index
	move	d3,d1
	add	d1,d1
	move	0(a6,d1.w),d1
	move	d1,(a1)+
...
.nextrow
	adda.l	d6,a0
	adda.l	d6,a0	;advance destination by two output rows
	addq	#1,d7
	bra	.rowloop
.done
	pull
...
; c86zet: build source-byte -> RGB565 LUT using the old c86zds logic.
...
.ecs
	add	d6,d6
.paladdr
	move	0(a3,d6.w),d0	;RGB12 high word, exactly as c86zds used
