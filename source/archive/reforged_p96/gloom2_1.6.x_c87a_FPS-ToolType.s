...
;gloom2_c86zdi: stronger near vertical oval, whole projectile/upgrade reflection Bayer-dithered, weapon-upgrade floor-touch pulse.
	...
	;c87a: optional 5-pixel-high FPS counter selected by bare FPS ToolType.
	;No in-game menu entry and no gloom.cfg persistence; AGA and P96 draw at
	;the final output resolution, so HIRES also keeps the counter exactly 5px high.
...
debugser	equ	0
debugmem	equ	0
g2p96_overlay_enabled	equ	0	;c86o: 0 disables P96 overlay, -1 re-enables old c85 P96 preview path
	;
...
exhalf	equ	exone>>1
	...
	jmp	entrypoint
	; c86zdq / GitHub 1.4: early executable ASCII release marker for hex/text viewers.
g2release_marker	dc.b	'Gloom Reforged AGA/P96 (v1.6 c87a FPS ToolType) Andreas ',39,'Andiweli',39,' Stuermer',0
	even
...
.nocd	;
	...
	jsr	g2displaymode_validate_stage1
	tst	d0
	bne	nomem
	jsr	g2p96_probe	;c86zdw: passive Picasso96API.library probe/log only, no output
	jsr	g2p96_modeid_probe	;c86zdx: passive 320x240 P96 ModeID probe/log only
	jsr	g2p96_screen_probe	;c86zdy: passive P96 screen open/close probe/log only
	jsr	g2p96_window_probe	;c86zdz: passive borderless window on P96 screen, close immediately
	; c86zhg: remove the visible startup P96 test/input screen.
	; Keep passive library/mode/screen/window logs above, but do not open
	; the old user-visible marker screen before the game starts.
	move.l	4.w,a6
	moveq	#1,d1
	jsr	-216(a6)
	...
	;
.intro	jsr	g2p96_display_enter_title	;c86zfq: title/menu AGA-bridge owns display
	clr	p96preview_gameactive	;gloom2p96_c14: title/menu phase, do not preview stale game buffer
	ifne	g2p96_overlay_enabled
	jsr	g2p96_clear_preview_window	;gloom2p96_c14: remove stale game preview from P96 menu/title
	endc
...
.intro2	jsr	g2v36_clear_title_buffers	;v36: clear both OS bitmaps before returning to title menu
	...
	bra.s	exittoos
.play	;
	jsr	g2p96_transition_clear_p96_if_open	;c86zhj: clear title/stale game frame before new game/intermission starts
	jsr	initnewgame
	tst	gametype
	bmi	.intro2
	move	#-1,p96preview_gameactive	;gloom2p96_c14: gameplay is now the visible source
	...
	jsr	g2p96_transition_clear_p96_if_open
	jsr	g2p96_gameplay_present_probe	;c86zfb: pre-open black P96 screen before first AGA gameplay present
...
.wmf	tst	fadevol
	...
	beq.s	.n22
	bsr	swaphflags
.n22	jsr	g2p96_transition_clear_p96_if_open	;c86zhj: remove stale gameplay frame immediately on game->title handoff
	jsr	g2p96_display_enter_title	;c86zfq: title/menu AGA-bridge owns display
	clr	p96preview_gameactive	;gloom2p96_c14: leaving gameplay for title/menu
	ifne	g2p96_overlay_enabled
	jsr	g2p96_clear_preview_window	;gloom2p96_c14: P96 window must not keep stale gameplay behind menus
	endc
	bsr	g2v190ct_titlefont
	bra	.intro
	;
...
dogamemenu	;
	jsr	g2p96_display_enter_ingame_menu	;c86zfq: ESC menu AGA-bridge owns display
	move	#$20,$dff09a
	;
	clr	p96preview_gameactive	;gloom2p96_c14: in-game menu overlay, hide stale gameplay preview
	ifne	g2p96_overlay_enabled
	jsr	g2p96_clear_preview_window	;gloom2p96_c14: clear stale P96 game preview before ESC menu
	endc
	st	paused
	move	framecnt,-(a7)
	move	linked,-(a7)
...
.menu_backdrop_done
	...
	lea	gamemenu,a4
	jsr	initmenu
	move	#-1,game_menu_active	;c86zhd: mark in-game menu before first P96 render so disabled rows are grey/white immediately
	...
	jsr	g2p96_menu_bridge_all_rows_present	;c86zgc: fast glyph-only menu overlay
...
.game_menu_adjust
	cmp	#2,d0
	bne.s	.notwsize
	jsr	g2p96_display_should_lock_fullwin	;c86zft: P96 keeps VIEW SIZE selectable but disabled/no-op
	tst	d0
	beq.s	.wsize_normal
	jsr	g2p96_ingame_menu_disabled_noop
	bra	.loop
.wsize_normal
	cmp	#$0100,d7
	beq.s	.wsize_left
...
.notwsize
	cmp	#3,d0
	bne.s	.notlwin
	jsr	g2p96_display_should_lock_fullwin	;c86zft: P96 keeps FULL SCREEN selectable but disabled/no-op
	tst	d0
	beq.s	.lwin_normal
	jsr	g2p96_ingame_menu_disabled_noop
	bra	.loop
...
.fskip	move	floorflag,floorflag2	;c86p: keep 2P swapped floor flag in sync for cfg/gameplay
	bsr	trainer_update_floor_text
	jsr	g2cfg_save	;c86p: persist FLOOR YES/NO immediately
	jsr	g2p96_ingame_menu_floorceil_live_refresh	;c86zfw: redraw live backdrop + menu, stay open
	bra	.loop
.notfloor
	cmp	#5,d0
	bne.s	.notroof
...
.rskip	move	roofflag,roofflag2	;c86p: keep 2P swapped ceiling flag in sync for cfg/gameplay
	bsr	trainer_update_ceiling_text
	jsr	g2cfg_save	;c86p: persist CEILING YES/NO immediately
	jsr	g2p96_ingame_menu_floorceil_live_refresh	;c86zfw: redraw live backdrop + menu, stay open
	bra	.loop
.notroof
	cmp	#7,d0
...
.done	jsr	g2cfg_save	;v141: save ingame menu settings when leaving menu
	...
	jsr	predrawall
	jsr	dispon
	move	#-1,p96preview_gameactive	;gloom2p96_c14: CONTINUE returns to live gameplay preview
	bra.s	.findone
.finish_exit
...
.g2v190aj_refresh_nogrey
	...
	jsr	initmenu2
	jsr	dispon
	jsr	g2p96_menu_bridge_present	;c86zft: refresh P96 bridge after FLOOR/CEILING redraw
	rts
...
.waitskip	clr	doneflag
	;
drawall_	tst	twowins		; c54/c86m: route 2P into split path, keep normal 1P untouched
	bne	g2twop_drawall_split
	move.l	player1(pc),player_
...
.wait2	tst	showflag
	...
	jsr	vwait
	bra	.wait2
.waitskip2	; c86zfb: P96 gameplay-primary lifecycle/pointer/menu probe.
	; If this frame was copied successfully to the P96 screen, skip the old
	; AGA doc2p/db present. Fallback remains unchanged for DISPLAY=AGA, no P96,
	; TWO PLAYER, or any P96 copy failure.
	jsr	g2p96_gameplay_present_probe	;c86zfb: primary P96 lifecycle/menu-safe AGA8 palette
...
.g2c86zfa_do_aga_present
	jsr	g2fps_draw_chunky	;c87a: AGA or P96 fallback, final 320x240 source before C2P
	jsr	doc2p
	jsr	db
	jsr	g2fps_frame_presented	;c87a: count the completed AGA/fallback display swap
	ifne	g2p96_overlay_enabled
	tst	twowins		;c55: skip P96 preview while diagnosing/restoring 2P split
	bne.w	.g2c55_no_p96_preview
	jsr	g2p96_preview_frame	;old c85 P96 preview path, disabled by default in c86m
.g2c55_no_p96_preview
	endc
...
g2twop_drawall_split
	...
	move	width(pc),g2twop_saved_width
	move	hite(pc),g2twop_saved_hite
	move	chunkymodw(pc),g2twop_saved_chunkymodw
	move	minx(pc),g2twop_saved_minx
	move	maxx(pc),g2twop_saved_maxx
	move	miny(pc),g2twop_saved_miny
	move	maxy(pc),g2twop_saved_maxy
	move.l	offset(pc),g2twop_saved_offset
	;
	; c86m: mimic gloom.s two-player window behaviour.  Do not render a
	; 320x120 full-FOV image and squeeze it vertically.  Instead render a
	; centred crop window whose width follows WINDOW SIZE up to the full
	; split width.  X/Y projection scaling is disabled while this flag is
	; active, so the split behaves like gloom.s usewindow/castwalls: smaller
	; windows crop/zoom, FULLSCREEN uses the full -160..+160 cast range.
	jsr	g2twop_prepare_view_width
	jsr	g2clearfullchunky	;clear both split halves from the real full-frame base
	jsr	g2twop_center_coloffs	;centre selected 2P view inside each 320px split half
	...
	; teleport starts the black hold, flush the whole 320x240 split frame to
...
.g2twop_restore_full
	...
	jsr	g2twop_restore_coloffs	;restore normal 320-column C2P table before full-frame conversion
...
g2twop_prepare_view_width
	; c86l: translate the existing one-player WINDOW SIZE values into a
	; gloom.s-style 2P crop width.  TINY starts at the proven 132px split
	; baseline; each larger menu step shows more horizontal field.  FULLSCREEN
	; reaches the full 320px split width without stretch because scaling helpers
	; and castwalls use crop mode while rendering each half.
	move	g2twop_saved_width(pc),d0
	cmp	#96,d0
	bcs.s	.tiny
	cmp	#128,d0
	bcs.s	.small
...
.vhuge	move	#276,d0
	bra.s	.store
.full	move	#320,d0
.store	move	d0,g2twop_view_width
	rts
...
g2twop_set_half_view
	...
	move	d0,width
	move	#120,hite
	move	#320,chunkymodw	;keep row stride at the real full-frame width
	lsr	#1,d0
	move	d0,maxx
	neg	d0
	move	d0,minx
...
; c86l: map logical render columns 0..selected_width-1 to a centred physical
; region inside the 320-pixel split half.  Smaller windows leave black side
; borders; FULLSCREEN copies coloffs onto itself and uses the whole half.
...
; c86l: rebuild the original full-width coloffs/paladjust tables before the
; C2P pass, HUD edge drawing, menu preview, and normal one-player restore.
...
; c86n: C2P helper for TWO PLAYER in-game menu preview.
; The chunky frame already contains both split halves.  Convert it as one
; full 320x240 image, then restore the user's WINDOW SIZE globals.
...
.here	move.l	font(pc),a0
	...
	move	d6,d1
	bsr	blit
	; c86zhg: in P96 intermission mode, publish each freshly blitted
	; character during the normal pdelay typewriter cadence.  Menus keep
	; pdelay=-1 and are not affected.
	jsr	g2p96_intermission_typewriter_char_if_bridge
	;
	move	pdelay(pc),d2
	subq	#1,d2
	bmi.s	.spc
...
g2_draw_wall_reflection_lite
	movem.l	d0-d7/a0-a6,-(a7)
	; c86zco: wall reflections obey the same REFLECTIONS toggle as
	; enemy/player/projectile reflections.
	tst	g2_reflections
	ble.w	.done
	move.l	chunky(pc),a2
	tst.l	a2
	...
	; fog, so wall reflections can safely sit on that surface just like
	; projectile/weapon reflections do.
	move.l	vertdraws(pc),a0
	lea	coloffs(pc),a1
	move	width(pc),d7
	beq.w	.done
	subq	#1,d7
	clr	d4		; screen X for stable Bayer phase
.col_loop
	tst.l	vd_data(a0)
	beq	.col_next
	move	vd_y(a0),d0
	...
	ble	.col_next
	add	d0,d1
	subq	#1,d1		; visible wall foot/bottom row
	cmp	midy(pc),d1
	blt	.col_next		; only mirror walls reaching the floor half
	move	hite(pc),d2
	sub	#8,d2		; safe last destination row
	move	d1,d3
	addq	#1,d3		; first destination row below wall foot
	cmp	d2,d3
	bge	.col_next		; no visible floor room below the wall foot
	; c86zc2: distance-building wall reflection height.  The previous full-weak
	; test mirrored too tall at every distance.  Now the wanted depth is driven
	; by the visible wall height, but with strong distance caps: far walls only
	; get a very shallow reflection, approaching walls can build higher.
	move	vd_h(a0),d6
	asr	#3,d6		; visible wall height / 8 -> DBF-ish reflection depth
	cmp	#1,d6
	bge.s	.depth_min_ok
	moveq	#1,d6		; minimum 2 visible rows when reflection exists
.depth_min_ok
	move	vd_z(a0),d5
	tst	g2_visibility
	bgt.s	.depth_adv
	...
	cmp	#(2<<grdshft),d5
	blo.s	.depth_mid
	moveq	#2,d5		; far: max 3 rows
	bra.s	.depth_cap_apply
.depth_mid
	moveq	#8,d5		; medium: max 9 rows
	bra.s	.depth_cap_apply
.depth_near
	moveq	#19,d5		; near: max 20 rows
	bra.s	.depth_cap_apply
.depth_adv
	cmp	#(2<<grdshft),d5
	blo.s	.depth_near
	cmp	#(4<<grdshft),d5
	blo.s	.depth_mid
	moveq	#2,d5		; far advanced: max 3 rows
.depth_cap_apply
	cmp	d5,d6
	ble.s	.depth_cap_ok
	move	d5,d6
.depth_cap_ok
	cmp	#1,d6
	bge.s	.depth_floor_ok
	moveq	#1,d6
.depth_floor_ok
	move	d2,d5
	sub	d3,d5		; available DBF count below wall foot
	cmp	d5,d6
	ble.s	.depth_avail_ok
	move	d5,d6
.depth_avail_ok
	cmp	#0,d6
	blt	.col_next
	move.l	(a1),d5		; chunky X offset for this column
	move	d1,d2
	mulu	chunkymodw(pc),d2
	move.l	a2,a3
	add.l	d5,a3
	add.l	d2,a3		; source starts at rendered wall foot
	move	d3,d2
	mulu	chunkymodw(pc),d2
	move.l	a2,a4
	add.l	d5,a4
	add.l	d2,a4		; destination starts below wall foot
	moveq	#0,d5		; reflection row index
.row_loop
	; c86zco: use the same opacity bands as the stable enemy/player mirror
	; reflection path: fuller near the contact point, then Bayer-faded tail.
	moveq	#8,d3		; top: stable near the wall/floor contact
	cmp	#3,d5
	blt.s	.have_thr
	moveq	#5,d3		; early mid
	cmp	#8,d5
	blt.s	.have_thr
	moveq	#3,d3		; lower: coarse fade
	cmp	#14,d5
	blt.s	.have_thr
	moveq	#1,d3		; sparse tail
.have_thr
	; distance/fog also thins the reflected wall columns.
	move	vd_z(a0),d1
	tst	g2_visibility
	bgt.s	.fog_adv
	cmp	#(4<<grdshft),d1
...
drawshapes	bsr	g2_build_reflect_coverz
	lea	shapelist(pc),a6
	;
...
.osvwait	movem.l	d0-d1/a0-a1/a6,-(a7)
	...
	ifne	g2p96_overlay_enabled
	tst	twowins		;gloom2p96_c59: keep P96 live hook out of TWO PLAYER split-screen
	bne.w	.g2c59_no_p96_vwait
	jsr	g2p96_preview_frame	;gloom2p96_c4/c59: safe P96 update from OS vwait path
.g2c59_no_p96_vwait
	endc
	rts
...
; c86n menu-preview full-C2P temporary globals
...
.nopic	movem.l	(a7)+,a0-a1
	...
	bsr	pokepal
.nopal	jsr	db
	jsr	g2p96_static_present_showbitmap_plain_if_bridge	;c86zgv: targeted staged P96 static present
	bra	vwait
...
.g2c86zgv_no_staticbuf
	;
	ifne	g2p96_overlay_enabled
	;gloom2p96_c22a: allocate the 160x96 preview staging buffer at runtime
	;instead of reserving it inline in the late P96 code block.  This keeps
	;Devpac/68000 PC-relative branches and data references in range.
	move.l	#160*96,d0
	moveq	#1,d1
	allocmem	p96previewbuf
	move.l	d0,p96preview_bufptr
	;
	;gloom2p96_c40/c41: allocate a parallel 160x96 RGB12 word buffer.
	; c40 only filled it silently.  c41 uses the same true RGB12 samples to
	; build an RGB332-style Pen index, without touching the P96/Workbench palette.
	move.l	#160*96*2,d0
	moveq	#1,d1
	allocmem	p96previewrgb12
	move.l	d0,p96preview_rgbptr
	;
	;gloom2p96_c43: allocate one 320-pixel RGB24 scanline buffer.  It is
	;filled from the true RGB12 sample buffer, but not yet pushed to P96 by
	;API.  The visible preview still uses the proven Pen/Runfill path.
	move.l	#320*3,d0
	moveq	#1,d1
	allocmem	p96previewrgb24line
	move.l	d0,p96preview_rgb24lineptr
	;
	;gloom2p96_c49: c48's extra 320x24 RGB24 bottom buffer could block
	;the Intermission/continue flow on real hardware.  Keep the RGB24 linebuffer
	;prep, but do not allocate or use the larger bottom buffer in this rollback.
	clr.l	p96preview_rgb24bottomptr
	;
	;gloom2p96_c25: allocate one row-staging buffer large enough for a
	;full 320-pixel window scanline.  The sampled 160-byte preview row is
	;expanded 2x into this linear buffer before the proven Runfill/RectFill
	;output.  This is still safe preview output, but matches the shape a
	;later WritePixelArray-style line path will need.
	move.l	#320,d0
	moveq	#1,d1
	allocmem	p96previewline
	move.l	d0,p96preview_lineptr
	;
	;gloom2p96_c26: allocate a full 320x192 linear mirror buffer.
	;For this patch it is filled from the expanded linebuffer but not yet used
	;as the visible output source; the proven Runfill path stays active.
	move.l	#320*192,d0
	moveq	#1,d1
	allocmem	p96previewframe
	move.l	d0,p96preview_frameptr
	;
	endc
	;
	move.l	#256*16,d0
...
scripthide	; v190hy cleanup: log marker removed
	jsr	g2p96_display_enter_intermission	;c86zfq: script/intermission AGA-bridge owns display
	clr	p96preview_gameactive
	bsr	dispoff
	bra	execscript
...
scriptdraw	; v190hy cleanup: log marker removed
	jsr	g2p96_display_enter_intermission	;c86zfq: intermission AGA-bridge before drawing
	clr	p96preview_gameactive
...
.use	move.l	d0,a0
	...
	jsr	g2p96_static_present_showbitmap_if_intermission_bridge
...
scriptpict	;load an iff
	jsr	g2p96_display_enter_intermission	;c86zfq: picture scripts use AGA-bridge for now
	clr	p96preview_gameactive
	bsr	freeiff
...
scriptdark	;
	jsr	g2p96_display_enter_intermission	;c86zfq: palette darken in AGA-bridge for now
	clr	p96preview_gameactive
...
scripttext	;print text on iff
	...
	;
	jsr	g2p96_display_enter_intermission	;c86zfq: text overlays in AGA-bridge for now
	clr	p96preview_gameactive
...
.g2v190t_show_text
	...
	cmp	#1,g2display_mode	; DISPLAY=AGA: do not alter pdelay
	beq	.g2c86zhi_keep_typewriter
	cmp	#P96DSP_INTERMISSION_BRIDGE,p96display_state
	bne	.g2c86zhi_keep_typewriter
	tst	p96gameplay_persist_active
	beq	.g2c86zhi_keep_typewriter
	jsr	g2p96_intermission_native_typewriter_prepare
...
.done	; v190hy cleanup: log marker removed
	jsr	printmess2
	jsr	g2p96_intermission_native_typewriter_finish	;c86zjn: native typewriter already updated P96; fallback keeps c86zjl full present
	;
	tst	pdelay
	bmi	execscript
	clr	pdelay
...
checkany	movem.l	d0-d7/a0-a6,-(a7)
	jsr	vwait
	jsr	readmenusel	; v188: restore original RETURN/ENTER title-menu opener
...
.nolmed	;
	...
	clr	paused
	jsr	predrawall
	jsr	g2fps_reset	;c87a: exclude the two predraw buffer-prime frames from measurement
	bsr	dispon
	bsr	chaton
	;
mainloop	; v190hy cleanup: log frame counter removed
	...
	beq.s	.noesc
	jsr	dogamemenu
	jsr	g2fps_restart_window	;c87a: menu pause must not lower the next FPS sample
	clr	escape
...
mainexit	st	paused
	...
	clr	p96preview_gameactive	;c86zfc: script/intermission owns visible display
	bsr	chatoff
	bsr	dispoff
	bsr	qsync2
	bsr	linkswap
...
.g2v190ct_brush_y_ok
	...
	jsr	g2p96_static_decode_gloombrush_best	;c86zjl: update planar compatibility bitmap + direct P96 composition
	movem.l	(a7)+,d0/a0-a1
	rts
...
.draw	jsr	g2v142_draw_gloombrush_safe	; no brush loaded = safe no-op for classic Gloom
	jsr	g2p96_static_present_showbitmap_plain_if_bridge	;c86zgv: refresh P96 title brush after full offscreen draw
.rts	rts
...
dointro	;
	jsr	g2p96_display_enter_title	;c86zfq: title menu AGA-bridge owns display
	clr	p96preview_gameactive	;gloom2p96_c14: title menu must not preview last game frame
	ifne	g2p96_overlay_enabled
	jsr	g2p96_clear_preview_window	;gloom2p96_c14: title menu starts with neutral P96 preview window
	endc
	...
	move	#-1,p96static_defer_present	;c86zgz: do not show title before brush is composed
	jsr	g2v145_show_clean_title
	...
	jsr	g2p96_static_present_showbitmap_plain_if_bridge
	bsr	dispon
...
.redrawmenu
	...
	jsr	g2p96_title_skip_redundant_brush_if_bridge	;c86zhd: P96 cache already has title+brush, avoid 1s redecode/present
	tst	d0
...
.linksel	bsr	finitpmenu
	bsr	linkup
	cmp	#4,curropt	;c86zhd: REMOTE LINK OPTIONS -> EXIT returns to cached title menu, no full title reload
	bne.s	.g2c86zhd_link_full_return
	jsr	g2p96_title_restore_staged_background_if_bridge
	bra	.redrawmenu
...
.noch	bsr	g2v147_finitmenu_soft
	...
	jsr	g2v145_show_clean_title
	; c86zhg: after ABOUT returns to the normal title menu, restore the
	; pics/gloom brush too.  The P96 cache can otherwise skip the redundant
	; brush draw and leave the title menu without the logo.
	jsr	g2v145_draw_menu_gloombrush
	bsr	dispon
	bra	.redrawmenu
...
qmenu	;quick menu in a0
	...
	move.l	a0,-(a7)
	bsr	clspic
	jsr	g2p96_title_qmenu_background_if_bridge	;c86zhe: safe restore, or rebuild after gameplay return
	move.l	(a7)+,a4
	bsr	initmenu
	bra	dispon
...
initmenu2	;
	...
	bsr	initfontpal
	; c86zjj: once the exact Bigfont glyph cache is available, P96 no longer
	; needs printmess2 to compose every menu row into menubmap first.  AGA and
	; any cache/build failure keep the original planar path below unchanged.
	clr	p96menu_native_init_active
	jsr	g2p96_menu_glyph_mode_active
	tst	d0
	beq	.g2c86zjj_planar_init
	; Do not suppress the proven planar composition unless the currently
	; visible P96 bitmap can also be resolved.  This keeps title/menu setup
	; fail-safe if a driver changes its RastPort/BitMap during a transition.
	jsr	g2p96_menu_glyph_prepare_target
	tst	d0
	beq	.g2c86zjj_planar_init
	move	#-1,p96menu_native_init_active
.g2c86zjj_planar_init
	;
...
.g2v158_no_title_credit
	bsr	swapshow
	bsr	db
	jsr	g2p96_static_present_title_menu_bigfont_if_bridge	;c86zgx: title menu text via real bigfont delta only
	bra	vwait
...
optoff	;
	...
	jsr	g2p96_menu_restore_current_row_best
	move	#13,flashdelay
	rts
...
.bwait	btst	#6,$dff002
	...
	jsr	g2p96_menu_bridge_current_row_present	;c86zgd: native row restore + redraw
	rts
...
.roof_ok	move	floorflag,floorflag2	;c86p: cfg-loaded floor applies to both 2P flag banks
	move	roofflag,roofflag2	;c86p: cfg-loaded ceiling applies to both 2P flag banks
	move	g2_blobshadow,d0
	bgt	.blob_yes
	move	#-1,g2_blobshadow
	bra	.blob_ok
...
g2early_noaga_cli_text
	dc.b	10,'Gloom Reforged requires an AGA Amiga.',10
	dc.b	'The current P96 version still uses AGA internally.',10,10
...
g2early_req_body	dc.b	'Gloom Reforged requires an AGA Amiga.',10,'The current P96 version still uses AGA internally.',0
...
; c86zdv: display-mode selector probe.  This stage deliberately only reads
; DISPLAY=AGA/P96/AUTO from Workbench ToolTypes or the CLI line and writes
; the result to RAM:gloom_displaymode.txt.  It does not open P96, does not
; alter AGA, and does not pick a ModeID yet.  Later P96 stages will resolve
...
g2p96_wide_mode	dc	0	;reserved native wide rendering, P96 only
...
; c86zdw: DH3 marker for emulator/real-hardware tests.  This file is
; written immediately after the passive P96 probe, before any later crash can
; hide the result.  Failure to write the marker is silent.
p96log_name	dc.b	'RAM:gloom_p96_probe.txt',0
	even
p96log_ok1	dc.b	'P96 probe: Picasso96API.library OK',10
p96log_ok1_len	equ	*-p96log_ok1
p96log_ok2	dc.b	'P96 probe: p96api.library OK',10
p96log_ok2_len	equ	*-p96log_ok2
p96log_fail	dc.b	'P96 probe: not found',10
p96log_fail_len	equ	*-p96log_fail
p96log_skip_aga	dc.b	'P96 probe: skipped (DISPLAY=AGA)',10
p96log_skip_aga_len	equ	*-p96log_skip_aga
	even

; c86zdx: passive 320x240 P96/RTG ModeID probe.  This only asks
; Picasso96API.library for a suitable DisplayID; it does not open a screen.
TAG_DONE	equ	0
...
p96modeid_state	dc	0	;0=not found, 1=found, 2=skip AGA, 3=no P96 lib
p96modeid_rgbformat	dc.l	RGBFB_R5G6B5PC	;c86zjc: safe fallback keeps old byte-swap
p96modeid_log_name	dc.b	'RAM:gloom_p96_modeid.txt',0
	even
p96modeid_msg_found	dc.b	'P96 modeid: found $'
...
p96modeid_msg_skip_aga	dc.b	'P96 modeid: skipped (DISPLAY=AGA)',10
...
p96screen_state	dc	0	;0=failed/not run, 1=OK, 2=skip AGA, 3=no P96 lib, 4=no ModeID, 5=no intuition
p96screen_ptr	dc.l	0
p96screen_log_name	dc.b	'RAM:gloom_p96_screen.txt',0
p96screen_msg_ok	dc.b	'P96 screen: OpenScreenTagList OK',10
p96screen_msg_ok_len	equ	*-p96screen_msg_ok
p96screen_msg_fail	dc.b	'P96 screen: OpenScreenTagList failed',10
p96screen_msg_fail_len	equ	*-p96screen_msg_fail
p96screen_msg_skip_aga	dc.b	'P96 screen: skipped (DISPLAY=AGA)',10
p96screen_msg_skip_aga_len	equ	*-p96screen_msg_skip_aga
p96screen_msg_skip_nolib	dc.b	'P96 screen: skipped, no P96 library',10
p96screen_msg_skip_nolib_len	equ	*-p96screen_msg_skip_nolib
p96screen_msg_skip_nomode	dc.b	'P96 screen: skipped, no ModeID',10
p96screen_msg_skip_nomode_len	equ	*-p96screen_msg_skip_nomode
p96screen_msg_skip_noint	dc.b	'P96 screen: skipped, no intuition.library',10
p96screen_msg_skip_noint_len	equ	*-p96screen_msg_skip_noint
	cnop	0,4
p96screen_tags
p96screen_displayid_tag	dc.l	SA_DisplayID,0
	dc.l	SA_Left,0
	dc.l	SA_Top,0
p96screen_width_tag	dc.l	SA_Width,320
p96screen_height_tag	dc.l	SA_Height,240
p96screen_depth_tag	dc.l	SA_Depth,16
	dc.l	SA_Type,15	;CUSTOMSCREEN
	dc.l	SA_ShowTitle,0
	dc.l	SA_Quiet,-1
	...
	cnop	0,4
p96modeid_tags
p96modeid_width_tag	dc.l	P96BIDTAG_NominalWidth,320
p96modeid_height_tag	dc.l	P96BIDTAG_NominalHeight,240
p96modeid_depth_tag
	dc.l	P96BIDTAG_Depth,16
	dc.l	TAG_DONE,0

; gloom2p96_b1: second RAM marker for the optional Workbench/P96
; testpattern window.  It is separate from the probe log so both results
; remain visible after a real-hardware crash.
p96winlog_name	dc.b	'RAM:gloom_p96_window.txt',0
	even
p96winlog_ok	dc.b	'P96 window: testpattern OK',10
p96winlog_ok_len	equ	*-p96winlog_ok
p96winlog_fail	dc.b	'P96 window: OpenWindow failed',10
p96winlog_fail_len	equ	*-p96winlog_fail
p96winlog_skip	dc.b	'P96 window: skipped, no P96 library',10
p96winlog_skip_len	equ	*-p96winlog_skip
	even

; gloom2p96_c1: third RAM marker for the first live-preview stage.
; This stage keeps AGA output untouched and draws a coarse sampled Chunky
; preview into the already opened Workbench/P96 window every few frames.
p96prevlog_name	dc.b	'RAM:gloom_p96_preview.txt',0
	even
p96prevlog_ok	dc.b	'P96 preview: c86zc1 P96 off full weak reflections test active',10
p96prevlog_ok_len	equ	*-p96prevlog_ok
p96prevlog_skip	dc.b	'P96 preview: skipped, no window',10
p96prevlog_skip_len	equ	*-p96prevlog_skip
	even

; gloom2p96_c2: written once when the per-frame preview hook really
; runs.  If this exists but the window stays unchanged, the hook is live
; and the remaining problem is the runtime window/RastPort write path.
p96livelog_name	dc.b	'RAM:gloom_p96_live.txt',0
	even
p96livelog_ok	dc.b	'P96 live: vwait/frame hook hit',10
p96livelog_ok_len	equ	*-p96livelog_ok
	even

; gloom2p96_c83: diagnostic marker for palette-index -> Workbench/P96 Pen collapse.
; Written when the exact palette BestPen map is rebuilt.  It does not change
; any visible colours; it only tells us whether many Gloom palette colours are
; collapsing onto the same screen Pens in the current Workbench/P96 window.
p96penlog_name	dc.b	'RAM:gloom_p96_penmap.txt',0
	even
p96penlog_msg	dc.b	'P96 penmap: unique256=$'
p96penlog_unique_hex	ds.b	4
	dc.b	' collapsed256=$'
p96penlog_collapsed_hex	ds.b	4
	dc.b	' colours=$'
p96penlog_colours_hex	ds.b	4
	dc.b	' uniqueActive=$'
p96penlog_active_hex	ds.b	4
	dc.b	10
p96penlog_msg_len	equ	*-p96penlog_msg
p96penlog_hexchars	dc.b	'0123456789ABCDEF'
	even

p96window_state	dc	0	;0=skipped/no p96, 1=ok, -1=open failed
p96preview_state	dc	0	;0=not active, 1=coarse preview active
p96preview_frame	dc	0	;frame divider for safe real-hardware preview
p96preview_live_logged	dc	0	;gloom2p96_c2: one-shot frame-hook RAM marker
p96preview_gameactive	dc	0	;gloom2p96_c14: preview only during real gameplay, not title/menus
p96run_start	dc	0	;gloom2p96_c15: x start for horizontal same-pen runfill
	;gloom2p96_c22a: buffer is allocated at runtime.  Keeping a 15 KB ds.b
	;inside this late code/data block pushes many old 68000 PC-relative
	;references beyond +/-32 KB on Devpac, so only the pointer lives here.
p96preview_bufptr	dc.l	0
p96preview_lineptr	dc.l	0	;gloom2p96_c25: one 320-byte expanded row staging buffer
p96preview_frameptr	dc.l	0	;gloom2p96_c26: optional 320x192 full-frame mirror buffer
p96preview_rgbptr	dc.l	0	;gloom2p96_c40/c41: 160x96 RGB12 word sample buffer
p96preview_rgb24lineptr	dc.l	0	;gloom2p96_c44: 320-pixel RGB24 linebuffer, 960 bytes, plus bottom channel probe
p96preview_rgb24bottomptr	dc.l	0	;gloom2p96_c51: intentionally unused; no c48-style bottom buffer
p96preview_window	dc.l	0	;persistent Workbench/P96 preview window pointer
p96test_window	dc	20,20	;x,y on Workbench/public native screen
	dc	320,240	;w,h
	dc.b	0,1	;pens
	dc.l	0	;no IDCMP needed for this short self-closing probe
	dc.l	$11840	;ACTIVATE|BORDERLESS|SIMPLE_REFRESH|RMBTRAP, no gadgets
	dc.l	0	;gadgets
	dc.l	0	;checkmark
	dc.l	p96test_title
	dc.l	0	;screen = Workbench/default
	dc.l	0	;bitmap
	dc	-1,-1,-1,-1	;mins/maxs
	dc	1	;WBENCHSCREEN
p96test_title	dc.b	'Gloom P96 test',0
	even
...
g2displaymode_probe
	...
	clr	g2p96_wide_mode
	clr	g2fps_enabled	;c87a: FPS is opt-in on every program start, never loaded from gloom.cfg
...
; c86zfk: read HIRES=YES/NO and STRETCH=YES/NO from ToolTypes or CLI.
; These options only affect the P96 ModeID/output size.  The original game
; render remains 320x240; HIRES doubles it, STRETCH widens it.
; STRETCH=YES + HIRES=NO/default now searches 240-line widescreen modes first;
; STRETCH=YES + HIRES=YES searches 480-line widescreen modes.
g2p96_options_scan_line
	jmp	g2display_tokens_scan
...
; c86zdw: safe second-stage probe.  DISPLAY=AGA deliberately skips the
; library open and logs that choice.  AUTO/P96 only try OpenLibrary and still
; leave the normal AGA path exactly as before.
g2p96_probe	push
	clr.l	p96base
	clr	p96present
	clr	p96found
	move	g2display_mode(pc),d0
	cmp	#1,d0	;DISPLAY=AGA: do not touch P96 at all
	bne.s	.open
	move	#3,p96found	;log explicit AGA skip
	bra.s	.done
.open	move.l	4.w,a6
	lea	p96name(pc),a1
...
; v190p96a2: write passive probe result to RAM:gloom_p96_probe.txt.
g2p96_probe_log	push
	move.l	dosbase,d0
	beq.s	.done
	move.l	d0,a6
	lea	p96log_name(pc),a0
	move.l	a0,d1
	move.l	#1006,d2	;MODE_NEWFILE
	jsr	-30(a6)	;Open
	move.l	d0,d7
	...
	cmp	#2,d0
	beq.s	.ok2
	lea	p96log_ok1(pc),a0
	move.l	#p96log_ok1_len,d3
	bra.s	.write
.ok2	lea	p96log_ok2(pc),a0
	move.l	#p96log_ok2_len,d3
	bra.s	.write
.skipaga	lea	p96log_skip_aga(pc),a0
	move.l	#p96log_skip_aga_len,d3
	bra.s	.write
.fail	lea	p96log_fail(pc),a0
	move.l	#p96log_fail_len,d3
.write	move.l	d7,d1
	move.l	a0,d2
	jsr	-48(a6)	;Write
	move.l	d7,d1
...
; c86zdx: passive 320x240 ModeID probe.  It calls only
; p96BestModeIDTagList() and writes RAM:gloom_p96_modeid.txt.
; No P96 screen/window/output is opened here.
g2p96_modeid_probe	push
	clr.l	p96modeid
	clr	p96modeid_depth
	clr	p96modeid_state
	move.l	#RGBFB_R5G6B5PC,p96modeid_rgbformat	;c86zjc fallback
	move	g2display_mode(pc),d0
	cmp	#1,d0
	bne.s	.have_nonaga
...
.have_nonaga
	tst	p96present
	bne.s	.have_p96
	move	#3,p96modeid_state
	bra	.log
.have_p96
	tst	g2p96_stretch_mode
	beq	.not_stretch
	; STRETCH=YES + HIRES=YES: prefer common 480-line widescreen RTG modes.
	tst	g2p96_hires_mode
	bne	.stretch_hires
	; STRETCH=YES + HIRES=NO/default: use confirmed custom 240-line
	; widescreen P96 modes first.  These must exist as proper ScreenModes in
	; P96Prefs/monitor settings; otherwise Picasso96 falls through to 480-line
	; widescreen fallback.
	move	#426,d0
	move	#240,d1
	move	#3,d2
	bsr	g2p96_set_target_mode
	bsr	g2p96_modeid_try_all_depths
	tst.l	d0
	bne	.found
	move	#400,d0
	move	#240,d1
...
.lowres
	...
	bne	.found
	bra	.log
.found	move	#1,p96modeid_state
	move.l	p96base,d0
	beq.s	.log
	move.l	d0,a6
	move.l	p96modeid,d0
	moveq	#P96IDA_RGBFORMAT,d1
	jsr	-84(a6)	;p96GetModeIDAttr(modeid, RGBFORMAT)
	move.l	d0,p96modeid_rgbformat
.log
	pull
	rts

; d0.w width, d1.w height, d2.w mode.  Patch every P96 tag/NewWindow
; that must agree with the live ModeID resolution.
g2p96_set_target_mode
	...
	and.l	#$0000ffff,d0
	and.l	#$0000ffff,d1
	move.l	d0,p96modeid_width_tag+4
	move.l	d1,p96modeid_height_tag+4
	move.l	d0,p96screen_width_tag+4
	move.l	d1,p96screen_height_tag+4
...
g2p96_modeid_try_all_depths
	moveq	#16,d0
	bsr	g2p96_modeid_try_depth
	tst.l	d0
	bne.s	.rts
	moveq	#24,d0
	bsr	g2p96_modeid_try_depth
...
; d0.w = requested depth.  Returns d0.l = 0 if no valid mode, else ModeID.
g2p96_modeid_try_depth	push
	and.l	#$0000ffff,d0
	move	d0,d6
	move.l	d0,p96modeid_depth_tag+4
	move.l	p96base,d0
	beq.s	.fail
	move.l	d0,a6
	lea	p96modeid_tags(pc),a0
	jsr	-60(a6)	;p96BestModeIDTagList(a0)
	cmp.l	#P96_INVALID_ID,d0
	beq.s	.fail
	tst.l	d0
	beq.s	.fail
	move.l	d0,p96modeid
	move	d6,p96modeid_depth
	pull
	rts
.fail	clr.l	d0
	pull
...
g2p96_modeid_log	push
	move.l	dosbase,d0
	beq	.done
	move.l	d0,a6
	lea	p96modeid_log_name(pc),a0
	move.l	a0,d1
	move.l	#1006,d2	;MODE_NEWFILE
	jsr	-30(a6)	;Open
	move.l	d0,d7
	...
	cmp	#3,d0
	beq.s	.skipnolib
	lea	p96modeid_msg_notfound(pc),a0
	move.l	#p96modeid_msg_notfound_len,d3
	bra.s	.write
.skipaga	lea	p96modeid_msg_skip_aga(pc),a0
	move.l	#p96modeid_msg_skip_aga_len,d3
	bra.s	.write
.skipnolib	lea	p96modeid_msg_skip_nolib(pc),a0
	move.l	#p96modeid_msg_skip_nolib_len,d3
	bra.s	.write
.found	move.l	p96modeid,d0
	lea	p96modeid_hex,a0
	bsr	g2p96_long_to_hex8
	move	p96target_width,d0
	lea	p96modeid_width_hex,a0
	bsr	g2p96_word_to_hex4
...
; c86zdy: Open and immediately close a 320x240 P96 screen using the
; live ModeID from c86zdx.  This is intentionally only a screen probe.
g2p96_screen_probe	push
	clr	p96screen_state
	clr.l	p96screen_ptr
	move	g2display_mode,d0
	cmp	#1,d0
	bne.s	.have_nonaga
	move	#2,p96screen_state
	bra	.log
.have_nonaga
	tst	p96present
	bne.s	.have_p96
	move	#3,p96screen_state
	bra	.log
.have_p96
	cmp	#1,p96modeid_state
	beq.s	.have_mode
	move	#4,p96screen_state
	bra	.log
.have_mode
	move.l	p96modeid,d0
	beq.s	.nomode
	move.l	d0,p96screen_displayid_tag+4
	move	p96modeid_depth,d0
	and.l	#$0000ffff,d0
	move.l	d0,p96screen_depth_tag+4
	move.l	4.w,a6
	lea	intname(pc),a1
	jsr	-408(a6)	;OldOpenLibrary intuition.library for this probe only
	move.l	d0,d6
	beq.s	.noint
	move.l	d6,a6
	sub.l	a0,a0	;no NewScreen struct, tags drive the probe
	lea	p96screen_tags(pc),a1
	jsr	-612(a6)	;OpenScreenTagList
	move.l	d0,p96screen_ptr
	beq.s	.closeint
	move	#1,p96screen_state
	move.l	d0,a0
	move.l	d6,a6
	jsr	-66(a6)	;CloseScreen immediately, no visible P96 path yet
.closeint
	move.l	d6,a1
	move.l	4.w,a6
	jsr	-414(a6)	;CloseLibrary
...
.nomode
	...
	bra.s	.log
.noint
	move	#5,p96screen_state
.log
	pull
	rts

g2p96_screen_log	push
	move.l	dosbase,d0
	beq	.done
	move.l	d0,a6
	lea	p96screen_log_name(pc),a0
	move.l	a0,d1
	move.l	#1006,d2	;MODE_NEWFILE
	jsr	-30(a6)	;Open
	move.l	d0,d7
	beq.s	.done
	move	p96screen_state,d0
	cmp	#1,d0
	beq.s	.ok
	cmp	#2,d0
	beq.s	.skipaga
	...
	cmp	#5,d0
	beq.s	.skipnoint
	lea	p96screen_msg_fail(pc),a0
	move.l	#p96screen_msg_fail_len,d3
	bra.s	.write
.ok	lea	p96screen_msg_ok(pc),a0
	move.l	#p96screen_msg_ok_len,d3
	bra.s	.write
.skipaga
	lea	p96screen_msg_skip_aga(pc),a0
	move.l	#p96screen_msg_skip_aga_len,d3
	bra.s	.write
.skipnolib
	lea	p96screen_msg_skip_nolib(pc),a0
	move.l	#p96screen_msg_skip_nolib_len,d3
	bra.s	.write
.skipnomode
	lea	p96screen_msg_skip_nomode(pc),a0
	move.l	#p96screen_msg_skip_nomode_len,d3
	bra.s	.write
.skipnoint
	lea	p96screen_msg_skip_noint(pc),a0
	move.l	#p96screen_msg_skip_noint_len,d3
.write	move.l	d7,d1
	move.l	a0,d2
	jsr	-48(a6)	;Write
	move.l	d7,d1
...
; gloom2p96_c4: open a persistent Workbench/P96 preview window after the Gloom screen.
; This still does NOT replace AGA output.  The window first shows the
; old testpattern, then g2p96_preview_frame/vwait samples the Chunky buffer into
; it every few frames.  If opening fails, the normal AGA path continues.
g2p96_test_window	push
	clr	p96window_state
	clr	p96preview_state
	clr	p96preview_frame
	clr	p96preview_live_logged
	clr.l	p96preview_window
	tst	p96present
	beq.s	.logonly
	move.l	int,d0
	beq.s	.fail
	move.l	d0,a6
	lea	p96test_window(pc),a0
	jsr	-204(a6)	;OpenWindow on Workbench/default screen
	move.l	d0,d7
	beq.s	.fail
	move	#1,p96window_state
	move	#1,p96preview_state
	move.l	d7,p96preview_window
	move.l	d7,a0
	bsr	g2p96_draw_testpattern
	jsr	g2p96_c71_init_bestpen_map	; c71: build RGB332->Workbench/P96 best-pen map once
	bra.s	.logonly
.fail	move	#-1,p96window_state
.logonly
	pull
	rts

; a0 = Window.  Uses normal graphics.library calls against the window
; RastPort, so the first P96 visible test stays OS-safe and non-invasive.
g2p96_draw_testpattern	push
	move.l	a0,a5	;keep Window pointer safe across graphics calls
	move.l	50(a5),a1	;Window.RPort
	move.l	a1,d0
	beq.s	.done
	move.l	grbase,a6
	moveq	#0,d0
	jsr	-342(a6)	;SetAPen black/background
	moveq	#0,d0
	moveq	#0,d1
	move	#319,d2
	move	#239,d3
	jsr	-306(a6)	;RectFill full window
	moveq	#0,d4
.barloop	move.l	50(a5),a1
	move.l	grbase,a6
	move	d4,d0
	addq	#1,d0
	and	#15,d0
	...
	add	#39,d2
	move	#199,d3
	jsr	-306(a6)	;RectFill vertical colour bar
	addq	#1,d4
	cmp	#8,d4
	bcs.s	.barloop
	move.l	50(a5),a1
	move.l	grbase,a6
	moveq	#15,d0
	jsr	-342(a6)	;SetAPen, bottom marker strip
	moveq	#0,d0
	move	#205,d1
	move	#319,d2
	move	#239,d3
...
; gloom2p96_b1: write Workbench/P96 window result marker.
g2p96_window_log	push
	move.l	dosbase,d0
	beq.s	.done
	move.l	d0,a6
	lea	p96winlog_name(pc),a0
	move.l	a0,d1
	move.l	#1006,d2	;MODE_NEWFILE
	jsr	-30(a6)	;Open
	move.l	d0,d7
	beq.s	.done
	move	p96window_state,d0
	bmi.s	.fail
	beq.s	.skip
	lea	p96winlog_ok(pc),a0
	move.l	#p96winlog_ok_len,d3
	bra.s	.write
.fail	lea	p96winlog_fail(pc),a0
	move.l	#p96winlog_fail_len,d3
	bra.s	.write
.skip	lea	p96winlog_skip(pc),a0
	move.l	#p96winlog_skip_len,d3
.write	move.l	d7,d1
	move.l	a0,d2
	jsr	-48(a6)	;Write
	move.l	d7,d1
...
; gloom2p96_c2: live coarse Chunky preview into the P96/Workbench
; window, plus a very visible heartbeat strip.  c1 reported "active" but
; the window stayed on the testpattern.  This version proves the runtime
; hook with RAM:gloom_p96_live.txt and maps sampled Chunky colours down to
; the already proven low Workbench pens 1..8, instead of feeding raw 0..255
; palette indices to SetAPen.
g2p96_preview_frame	push
	move.l	p96preview_window(pc),d0
	beq.w	.done
	tst	p96preview_state
	beq.w	.done
	tst	p96preview_gameactive	;gloom2p96_c14: title/ingame menu must not show stale 3D buffer
	beq.w	.done
	addq	#1,p96preview_frame
	move	p96preview_frame(pc),d0
	and	#1,d0		;gloom2p96_c17: update every 2nd frame
	bne.w	.done
	tst	p96preview_live_logged
	bne.s	.nolog
	move	#-1,p96preview_live_logged
.nolog	movem.l	d0-d7/a0-a6,-(a7)
	move.l	p96preview_window(pc),a5
	move.l	50(a5),d0	;Window.RPort
	beq.w	.restore
	move.l	grbase(pc),d0
	beq.w	.restore

	; gloom2p96_c5: keep the proven heartbeat strip, then perform a
	; deliberately tiny Chunky-source test: a 160x96 coarse
	; block grid, every 2nd frame.  c3 proved that full-window sampling was
	; too aggressive on real hardware; this stage checks whether the
	; actual Chunky source can be read safely without replacing AGA.
	move.l	50(a5),a1
	move.l	grbase(pc),a6
	move	p96preview_frame(pc),d0
	lsr	#3,d0
	and	#7,d0
	addq	#1,d0		;use visible pens 1..8, proven by testpattern
	jsr	-342(a6)	;SetAPen
	moveq	#0,d0
	moveq	#0,d1
	move	#319,d2
	move	#15,d3
	jsr	-306(a6)	;RectFill heartbeat strip
	...
	; gloom2p96_c73: build an exact current-palette-index -> best screen Pen
	; map lazily inside gameplay preview. c71 only mapped RGB332 candidates,
	; which made some sprites/projectiles better but collapsed subtle texture
	; palette ranges to red/grey.  This direct map keeps the proven Pen/RectFill
	; path, but bypasses RGB332 quantisation for the main preview rows.
	jsr	g2p96_c73_init_palette_pen_map

	; gloom2p96_c25: expanded-linebuffer stage based on c24.  c17/c18a/c21 proved
	; that 160x96 2-frame runfill output is stable, but still directly
	; mixed source sampling with drawing.  This version first builds a
	; compact 160x96 pen/luma buffer, then draws that buffer through the
	; exact same proven RectFill/runfill output path.  It does not change
	; AGA output and does not touch P96 pens or palettes.
	move.l	p96preview_bufptr(pc),a0
	tst.l	a0
	beq.w	.restore
	moveq	#0,d6		;preview grid row 0..95
.c22bufrow
	cmp	#96,d6
	bge	.c22drawbuffer
	move	d6,d0
	add	d0,d0
	add	#20,d0		;sample source rows 20..210
	mulu	chunkymodw,d0
	move.l	a4,a2
	add.l	d0,a2		;Chunky/C2P row base for this grid row
	move	d6,d0
	mulu	#160,d0
	lea	0(a0,d0.l),a1	;destination row inside 160x96 pen buffer
	;gloom2p96_c40: set up matching RGB12 row pointer.  a5 is safe to
	;reuse during sampling; the window pointer is reloaded before drawing.
	move.l	p96preview_rgbptr(pc),d7
	beq	.c40norgbrow
	move.l	d7,a5
	move	d6,d7
	mulu	#320,d7		;160 RGB12 words per row
	lea	0(a5,d7.l),a5
	bra	.c40rgbrowready
.c40norgbrow
	moveq	#0,d7
	move.l	d7,a5
.c40rgbrowready
	moveq	#0,d5		;x = 0..318, step 2
.c22bufxloop
	cmp	#320,d5
	bge	.c22nextbufrow
	move.l	0(a3,d5*4),d4
	moveq	#0,d0
	move.b	0(a2,d4.l),d0	;sample one Chunky pixel
	; c72: chunky pixels are paladjust/C2P-scrambled. Undo that
	; before looking up planar_palette, otherwise walls/floors use the
	; wrong RGB entry and collapse to red/grey false colours.
	lea	g2_strip_invpal,a6
	move.b	0(a6,d0.w),d0
	move.l	planar_palette,a6	;c86zgw: GenAm pc-relative range buildfix
	tst.l	a6
	beq	.c22fallback
	move	d0,d1
	tst	aga
	beq	.c22ecs
	lsl	#2,d1		;AGA palette entry = 2 words
	bra	.c22paladdr
.c22ecs	add	d1,d1		;ECS palette entry = 1 word
.c22paladdr
	move	0(a6,d1.w),d2	;RGB12 high word
	;gloom2p96_c40: keep the true RGB12 sample in a hidden buffer for
	;the upcoming real-colour output experiments.
	move.l	a5,d7
	beq	.c40skiprgbstore
	move	d2,(a5)+
.c40skiprgbstore
	;gloom2p96_c73: main preview colour test: map the real palette index
	; directly to the closest P96/Workbench Pen.  This avoids the old RGB332
	; reduction for wall/floor/ceiling texture pixels.  The RGB12 sample is
	; still stored above for the existing bottom probe.
	tst	p96palpen_state
	beq	.c73oldrgb332
	and	#$00ff,d0
	lea	p96palpen_map,a6
	move.b	0(a6,d0.w),d0
	bra	.c73storepen
.c73oldrgb332
	;gloom2p96_c42/c43: visible output is still BGR332 Penindex; c43 additionally prepares RGB24.
	;This is still Pen-index output only: no SetRGB, no palette changes.
	;Some RTG/Workbench palettes arrange useful colour ramps differently,
	;so this tests whether blue-high / green-mid / red-low gives a more
	;natural false-colour preview than c41a's RGB332 order.
	move	d2,d0
	and	#$000f,d0
	lsr	#1,d0		;B 0..7
	lsl	#5,d0		;B bits -> 7..5
	...
	move	d2,d1
	and	#$0f00,d1
	lsr	#8,d1		;R 0..15, GenAm/68000 immediate shift max is 8
	lsr	#2,d1		;R 0..3 -> bits 1..0
	or	d1,d0		;BGR332 Pen candidate 0..255
	bne.s	.c41penok
	moveq	#1,d0		;avoid invisible Pen 0 except via clear/menu path
.c41penok
	bra	.c22mapped
.c22fallback
	;gloom2p96_c40: no palette pointer, so write a black RGB12 sample if the
	;hidden RGB buffer exists.  Visible fallback Pen output stays unchanged.
	move.l	a5,d7
	beq	.c40skiprgbfallback
	clr	(a5)+
.c40skiprgbfallback
	and	#15,d0		;fallback only if palette pointer is missing
	bne	.c22mapped
	moveq	#1,d0
.c22mapped
	tst	p96bestpen_state	;c71: if BestPen map exists, translate RGB332 candidate to real screen pen
	beq.s	.c71nomap
	and	#$00ff,d0
	lea	p96bestpen_map,a6
	move.b	0(a6,d0.w),d0
.c71nomap
.c73storepen
	move.b	d0,(a1)+	;store final pen/luma value in the new buffer
	addq	#2,d5
	bra	.c22bufxloop
.c22nextbufrow
	addq	#1,d6
...
.c22drawbuffer
	move.l	p96preview_window(pc),a5	;gloom2p96_c40: restore Window pointer after RGB sampling
	move.l	p96preview_bufptr(pc),a4
	tst.l	a4
	beq.w	.restore
	moveq	#0,d6		;buffer row 0..95
.c22rowloop
	cmp	#96,d6
	bge	.c22done
	move	d6,d0
	mulu	#160,d0
	;gloom2p96_c25: expand the 160-byte sampled preview row into a real
	;320-byte linear scanline buffer.  The current output still uses the
	;proven RectFill/runfill path, but the staging shape is now identical to
	;what a later WritePixelArray-style row copy would consume.  If the tiny
	;linebuffer allocation failed, skip only this P96 frame and keep AGA safe.
	move.l	p96preview_lineptr(pc),a3
	tst.l	a3
	beq.w	.restore
	lea	0(a4,d0.l),a0	;source row in the 160x96 frame buffer
	move.l	a3,a2		;destination 320-byte expanded row staging buffer
	move	#159,d3
.c25expandrow
	moveq	#0,d4
	move.b	(a0)+,d4
	...
	;gloom2p96_c43: prepare one true RGB24 320-pixel line from the hidden
	;RGB12 sample row.  This does not affect visible output yet; it only
	;builds the exact row shape a later P96 RGB/WritePixelArray-style path
	;can consume.  RGB12 nibbles are expanded to 8-bit as n*17.
	move.l	p96preview_rgb24lineptr(pc),d0
	beq	.c43norgb24line
	move.l	p96preview_rgbptr(pc),d1
	beq	.c43norgb24line
	move.l	d0,a2		;destination RGB24 row, 320*3 bytes
	move.l	d1,a0		;base of 160x96 RGB12 buffer
	move	d6,d0
	mulu	#320,d0		;160 RGB12 words per sampled row
	lea	0(a0,d0.l),a0
	move	#159,d3
.c43rgb24loop
	move	(a0)+,d0		;RGB12 = 0000rrrrggggbbbb
	move	d0,d4
	and	#$0f00,d4
	lsr	#8,d4		;R nibble 0..15
	move	d4,d7
	lsl	#4,d4
	or	d7,d4		;R byte 0..255
	move	d0,d5
	and	#$00f0,d5
	lsr	#4,d5		;G nibble 0..15
	move	d5,d7
	lsl	#4,d5
	or	d7,d5		;G byte 0..255
	move	d0,d2
	and	#$000f,d2	;B nibble 0..15
	move	d2,d7
	lsl	#4,d2
	or	d7,d2		;B byte 0..255
	move.b	d4,(a2)+
	move.b	d5,(a2)+
	move.b	d2,(a2)+
	move.b	d4,(a2)+
...
.c43norgb24line
	...
	;gloom2p96_c74: bottom mini-window now uses the same final direct
	;palette->BestPen scanline as the big preview.  c73 made the main
	;preview much more realistic, while the old RGB24/RGB332 bottom probe
	;still showed red/blue false colours.  Keep the safe Pen/RectFill path,
	;but stop using the old RGB332 bottom conversion.
	move	d6,d0
	and	#1,d0
	bne	.c50skipdirectrgb24
	move.l	p96preview_lineptr(pc),d0
	beq	.c50skipdirectrgb24
	move.l	d0,a2		;base of current 320-pixel final-Pen scanline
	moveq	#0,d5		;destination x = 0..319, 2 pixels per source sample
.c50xloop
	cmp	#320,d5
	bge	.c50skipdirectrgb24
	moveq	#0,d0
	move.b	0(a2,d5.w),d0	;already c73/c74 palette BestPen output
	bne	.c50penok
	moveq	#1,d0		;avoid invisible Pen 0
.c50penok
	move.l	50(a5),a1
	move.l	grbase(pc),a6
	jsr	-342(a6)	;SetAPen(final palette BestPen)
	move	d5,d0
	move	d6,d1
	lsr	#1,d1		;sample rows 0,2,4...94 -> bottom rows 0..47
	add	#192,d1		;bottom mini-window rows 192..239
	move	d5,d2
	addq	#1,d2
	move	d1,d3
	jsr	-306(a6)	;RectFill 2x1 c74 bottom preview block
	addq	#2,d5
	bra	.c50xloop
.c50skipdirectrgb24

	;gloom2p96_c34: based on c33 main-source; mirror the expanded 320-byte row into the optional
	;full 320x192 linear frame buffer on every visible preview update.
	;The main Runfill preview below will read from this framebuffer instead
	;of directly from the temporary expanded linebuffer.  This keeps the
	;old safe RectFill output, but makes the hidden 320x192 buffer the real
	;source of the visible preview for the first time.
	move.l	p96preview_frameptr(pc),d0
	beq.s	.c26noframemirror
	move.l	d0,a0
	move	d6,d0
	mulu	#640,d0
	lea	0(a0,d0.l),a0
...
.c26copyrow2
	...
	dbf	d3,.c26copyrow2
.c26noframemirror
	;gloom2p96_c34: draw the main preview from the mirrored 320x192
	;framebuffer when available.  If allocation ever failed, safely fall
	;back to the proven c25 expanded linebuffer for this row.
	;gloom2p96_c51: keep the taller RGB24 bottom probe visible by not
	;drawing the main pen-preview into rows 192..239.  Rows 0..83 still
	;fill the normal main area 24..191; rows 84..95 are only used by the
	;RGB24 bottom probe above.
	move.l	p96preview_frameptr(pc),d0
	beq.s	.c33linefallback
	move.l	d0,a2
	move	d6,d0
	mulu	#640,d0		;two 320-byte framebuffer rows per 160x96 row
	lea	0(a2,d0.l),a2
	bra.s	.c33drawfromready
.c33linefallback
	move.l	a3,a2		;draw from expanded 320-byte staged row
.c33drawfromready
	cmp	#84,d6		;gloom2p96_c51: reserve window rows 192..239 for RGB24 probe
	bge	.c22nextrow
	moveq	#0,d5		;destination x = 0..319, step 1
	moveq	#-1,d7		;current run pen, -1 = no run yet
	clr	p96run_start
.c22xloop
	cmp	#320,d5
	bge	.c22flushrow
	moveq	#0,d4
	move.b	(a2)+,d4	;pen/luma value from the expanded linebuffer
	cmp	d7,d4
	beq	.c22samepen
	tst	d7
	bmi	.c22startrun
	; Pen changed: flush the previous horizontal run [p96run_start..d5-1].
	move.l	50(a5),a1
	move.l	grbase(pc),a6
	move	d7,d0
	jsr	-342(a6)	;SetAPen(previous run pen)
	move	p96run_start(pc),d0
	move	d6,d1
	add	d1,d1
	add	#24,d1		;destination rows 24..215
	move	d5,d2
	subq	#1,d2
	move	d1,d3
	addq	#1,d3
	jsr	-306(a6)	;RectFill merged horizontal run
.c22startrun
	move	d4,d7
	move	d5,p96run_start
.c22samepen
	addq	#1,d5
	bra	.c22xloop
.c22flushrow
	tst	d7
	bmi	.c22nextrow
	; End of row: flush the final run [p96run_start..319].
	move.l	50(a5),a1
	move.l	grbase(pc),a6
	move	d7,d0
	jsr	-342(a6)	;SetAPen(final run pen)
	move	p96run_start(pc),d0
	move	d6,d1
	add	d1,d1
	add	#24,d1
	...
	move	d1,d3
	addq	#1,d3
	jsr	-306(a6)	;RectFill final run
.c22nextrow
	addq	#1,d6
	bra	.c22rowloop
.c22done
	;gloom2p96_c51: no post-loop bottom redraw.  The 48-pixel mini-preview
	;is still drawn directly from the live RGB24 scanline buffer inside the
	;row loop above, so there is no persistent RGB24 bottom buffer and no
	;extra Intermission/continue-state staging after the main preview pass.
...
; gloom2p96_c2: one-shot marker proving that g2p96_preview_frame is
; reached during the live game loop, not only during startup.
g2p96_preview_live_log	push
	move.l	dosbase,d0
	beq.s	.done
	move.l	d0,a6
	lea	p96livelog_name(pc),a0
	move.l	a0,d1
	move.l	#1006,d2	;MODE_NEWFILE
	jsr	-30(a6)	;Open
	move.l	d0,d7
	beq.s	.done
	lea	p96livelog_ok(pc),a0
	move.l	#p96livelog_ok_len,d3
	move.l	d7,d1
	move.l	a0,d2
	jsr	-48(a6)	;Write
	move.l	d7,d1
...
; gloom2p96_c14: neutralize the P96 preview window while title/ingame
; menus are displayed on AGA.  The live preview reads the 3D Chunky
; source only; menu overlays are not in that buffer, so leaving it visible
; during menus looks like a stale gameplay layer.  Clear the window instead.
g2p96_clear_preview_window	push
	move.l	p96preview_window(pc),d0
	beq.s	.done
	move.l	d0,a5
	move.l	50(a5),d0	;Window.RPort
	...
	move	#319,d2
	move	#239,d3
	jsr	-306(a6)	;RectFill full preview window
.done	pull
	rts

; gloom2p96_c1: write live-preview status marker.
g2p96_preview_log	push
	move.l	dosbase,d0
	beq.s	.done
	move.l	d0,a6
	lea	p96prevlog_name(pc),a0
	move.l	a0,d1
	move.l	#1006,d2	;MODE_NEWFILE
	jsr	-30(a6)	;Open
	move.l	d0,d7
	...
	tst	p96preview_state
	beq.s	.skip
	lea	p96prevlog_ok(pc),a0
	move.l	#p96prevlog_ok_len,d3
	bra.s	.write
.skip	lea	p96prevlog_skip(pc),a0
	move.l	#p96prevlog_skip_len,d3
.write	move.l	d7,d1
	move.l	a0,d2
	jsr	-48(a6)	;Write
	move.l	d7,d1
...
; Close persistent preview window before the normal Intuition/screen cleanup.
g2p96_close_window	push
	move.l	p96preview_window(pc),d0
	beq.s	.done
	move.l	d0,a0
	move.l	int,d0
...
.clear	clr.l	p96preview_window
	clr	p96preview_state
.done	pull
	rts

; v190p96a: matching close for the passive probe library.
g2p96_close	push
	move.l	p96base,d0
	beq.s	.done
	move.l	d0,a1
...
finitdisplay	push
	...
	ifne	g2p96_overlay_enabled
	jsr	g2p96_close_window	;gloom2p96_c1: close persistent P96 preview window first
	jsr	g2p96_close	;v190p96a: close optional P96 probe library if it was opened
	endc
	;
	tst	os
	beq.w	.noos
...
windowtask	move.l	int(pc),a6	;intuition base
	...
	move.l	d0,oswindow
	beq.s	.rts	;c86zdc: avoid dereferencing failed OpenWindow (grey screen + pointer guard only)
...
initdisplay	;
	...
	move.l	d0,grbase
	ifne	g2p96_overlay_enabled
	jsr	g2p96_probe	;v190p96a: passive P96 library probe, no output change
	endc
	;
	;allocate 2 bitmaps!
	;
	...
	jsr	-408(a6)
	move.l	d0,int
	; gloom2p96_c4: open P96 preview window later, after the Gloom screen exists.
	; Opening it before OpenScreen leaves the visible Workbench window stuck on the
	; startup testpattern while the game screen takes over.
	;
	move.l	bitmaps,d0
	lea	osbitmap1(pc),a0
	bsr	initbitmap
...
.g2c86zdc_screen_ok
	...
	move.l	d0,dbufinfo
	ifne	g2p96_overlay_enabled
	jsr	g2p96_test_window	;gloom2p96_c4: open preview window after game screen setup
	endc
	bra	db
	;
...
; ---------------------------------------------------------------------------
; gloom2p96_c71: one-time colourmap pen mapping for the smooth Pen preview.
; This deliberately avoids all unsafe true-colour paths that crashed or
; slowed down earlier: no p96RectFill/ARGB, no p96WritePixelArray in the
; live hook, no startup WPA.  Instead we keep the proven SetAPen/RectFill
; renderer and ask graphics.library for the closest pen to each RGB332
; candidate on the preview window's own ViewPort/ColorMap.
;
; State: 0 = old raw Pen candidates / fallback, -1 = p96bestpen_map valid.
; ---------------------------------------------------------------------------
p96bestpen_state	dc	0
p96bestpen_tags	dc.l	0	;TAG_DONE
p96bestpen_map	dcb.b	256,0
p96c71_tab2	dc.b	0,85,170,255
p96c71_tab3	dc.b	0,36,72,108,144,180,216,252
	even

; gloom2p96_c73: exact current Gloom palette index -> best screen Pen.
; This is separate from c71's RGB332 candidate map.  It keeps the safe
; Pen/RectFill output path, but lets the main preview use real palette
; colours for texture pixels instead of reducing them to RGB332 first.
p96palpen_state	dc	0	;0=inactive, -1=map valid
p96palpen_palette	dc.l	0	;planar_palette pointer used for the current map
p96palpen_map	dcb.b	256,0
	even

g2p96_c71_init_bestpen_map
	movem.l	d0-d7/a0-a6,-(a7)
	clr	p96bestpen_state
	move.l	p96preview_window,d0
	beq	g2p96_c71_done
	move.l	d0,a0
	move.l	int,d0
	beq	g2p96_c71_done
	move.l	d0,a6
	jsr	-300(a6)	;intuition.library/ViewPortAddress(Window*)
	move.l	d0,a4
	tst.l	a4
	beq	g2p96_c71_done
	move.l	4(a4),a4	;ViewPort.ColorMap
	tst.l	a4
	beq	g2p96_c71_done
	move.l	grbase,d0
	beq	g2p96_c71_done
	move.l	d0,a6
	lea	p96bestpen_map,a2
	moveq	#0,d7

g2p96_c71_loop
	; Current preview candidate layout is BGR332:
	; bits 7..5 = blue, 4..2 = green, 1..0 = red.
	move	d7,d5
	and	#3,d5
	lea	p96c71_tab2,a3
	moveq	#0,d0
	move.b	0(a3,d5.w),d0	;red byte 0/85/170/255
	bsr	g2p96_c71_byte_to_rgb32
	move.l	d0,d1
	...
	lea	p96c71_tab3,a3
	moveq	#0,d0
	move.b	0(a3,d5.w),d0	;green byte 0..252
	bsr	g2p96_c71_byte_to_rgb32
	move.l	d0,d2
	...
	lea	p96c71_tab3,a3
	moveq	#0,d0
	move.b	0(a3,d5.w),d0	;blue byte 0..252
	bsr	g2p96_c71_byte_to_rgb32
	move.l	d0,d3

	move.l	a4,a0	;ColorMap
	lea	p96bestpen_tags,a1
	jsr	-840(a6)	;graphics.library/ObtainBestPenA(cm,R,G,B,tags)
	cmp.l	#-1,d0
	beq	g2p96_c71_fallback
	and.l	#$000000ff,d0
	move.b	d0,0(a2,d7.w)
	bra	g2p96_c71_next
...
g2p96_c71_next
	...
	cmp	#256,d7
	blo	g2p96_c71_loop
	move	#-1,p96bestpen_state

g2p96_c71_done
	movem.l	(a7)+,d0-d7/a0-a6
	rts

; c73: build/refresh exact palette-index -> best Pen map.
; Called lazily from the preview frame after planar_palette exists.
g2p96_c73_init_palette_pen_map
	movem.l	d0-d7/a0-a6,-(a7)
	; gloom2p96_c82: use the actually poked AGA/game palette first.
	; c75 used planar_palette, which is close but can miss map-specific
	; green/olive tones visible in the real AGA output.  lastpal is set by
	; pokepal whenever the displayed game palette changes.  Fall back to
	; planar_palette if no active palette has been poked yet.
	move.l	lastpal,d0
	bne	g2p96_c82_havepal
	move.l	planar_palette,d0
g2p96_c82_havepal
	beq	g2p96_c73_fail
	cmp.l	p96palpen_palette,d0
	bne	g2p96_c73_rebuild
	tst	p96palpen_state
	bne	g2p96_c73_done

g2p96_c73_rebuild
	move.l	d0,p96palpen_palette
	clr	p96palpen_state
	move.l	p96preview_window,d0
	beq	g2p96_c73_done
	move.l	d0,a0
	...
	beq	g2p96_c73_done
	move.l	d0,a6
	move.l	p96palpen_palette,a5	; c82: selected active lastpal/planar fallback
	lea	p96palpen_map,a2
	moveq	#0,d7
...
g2p96_c73_loop
	...
	tst	aga
	beq	g2p96_c73_ecs
	lsl	#2,d0		;AGA palette entry uses 2 words, high RGB12 first
	bra	g2p96_c73_palready
g2p96_c73_ecs
	add	d0,d0
g2p96_c73_palready
	move	0(a5,d0.w),d6	;RGB12 = 0000rrrrggggbbbb

	; gloom2p96_c75: c74 green-bias reverted.  Keep the useful part
	; from c74 only: the bottom mini-window uses the same final
	; direct palette->BestPen scanline as the big preview.
	...
	or	d3,d0
	bsr	g2p96_c71_byte_to_rgb32
	move.l	d0,d3	;B 32-bit

	move.l	a4,a0	;ColorMap
	lea	p96bestpen_tags,a1
	jsr	-840(a6)	;graphics.library/ObtainBestPenA(cm,R,G,B,tags)
	cmp.l	#-1,d0
	beq	g2p96_c73_fallback
	and.l	#$000000ff,d0
	move.b	d0,0(a2,d7.w)
	bra	g2p96_c73_next
...
g2p96_c73_next
	...
	cmp	#256,d7
	blo	g2p96_c73_loop
	move	#-1,p96palpen_state
	bra	g2p96_c73_done

g2p96_c73_fail
	clr	p96palpen_state
	clr.l	p96palpen_palette

g2p96_c73_done
	movem.l	(a7)+,d0-d7/a0-a6
	rts

; c83: Count how many distinct screen Pens the 256-entry palette map really
; uses.  If this number is low, the grey/flat texture look comes from the
; Workbench/P96 Pen/ColorMap path, not from the Gloom source palette.
g2p96_c83_log_penmap
	movem.l	d0-d7/a0-a6,-(a7)
	tst	p96palpen_state
	beq	g2p96_c83_done

	; unique count over the full 256-entry map
	lea	p96diag_seen,a0
	moveq	#0,d0
	move	#255,d7
.c83clr1
	move.b	d0,(a0)+
	dbf	d7,.c83clr1
	lea	p96palpen_map,a1
	lea	p96diag_seen,a2
	moveq	#0,d6		;unique256
	moveq	#0,d7		;palette index
.c83loop1
	cmp	#256,d7
	bge	.c83full_done
	moveq	#0,d0
...
.c83full_done
	move	d6,d0
	lea	p96penlog_unique_hex,a0
	bsr	g2p96_c83_word_to_hex4
	move	#256,d0
	sub	d6,d0
	lea	p96penlog_collapsed_hex,a0
	bsr	g2p96_c83_word_to_hex4

	; active colour count from Gloom's current palette size, capped at 256
	move	colours,d5
	cmp	#256,d5
	bls	.c83col_ok
	move	#256,d5
.c83col_ok
	move	d5,d0
	lea	p96penlog_colours_hex,a0
	bsr	g2p96_c83_word_to_hex4

	; unique count over entries 0..colours-1
	lea	p96diag_seen,a0
	moveq	#0,d0
	move	#255,d7
.c83clr2
...
.c83active_done
	move	d6,d0
	lea	p96penlog_active_hex,a0
	bsr	g2p96_c83_word_to_hex4

	move.l	dosbase,d0
	beq	g2p96_c83_done
	move.l	d0,a6
	lea	p96penlog_name,a0
	move.l	a0,d1
	move.l	#1006,d2	;MODE_NEWFILE
	jsr	-30(a6)	;Open
	move.l	d0,d7
	beq	g2p96_c83_done
	move.l	d7,d1
	lea	p96penlog_msg,a0
	move.l	a0,d2
	move.l	#p96penlog_msg_len,d3
	jsr	-48(a6)	;Write
	move.l	d7,d1
	jsr	-36(a6)	;Close
...
p96diag_seen	ds.b	256
	even

g2p96_c71_byte_to_rgb32
	and.l	#$000000ff,d0
	move.l	d0,d4
	lsl.l	#8,d0
	or.l	d4,d0
...
; -----------------------------------------------------------------------------
; c86zdz: passive borderless Window probe on the live-found 320x240 P96 screen.
; This deliberately opens a fresh P96 screen, opens a borderless 320x240 window
; at 0,0 on that screen, closes the window, closes the screen, and logs only.
; No drawing, no overlay, no renderer switch and no AGA-present changes happen.
; -----------------------------------------------------------------------------
p96winprobe_state		dc	0	;0=failed/not run, 1=OK, 2=skip AGA, 3=no P96 lib, 4=no ModeID, 5=no intuition, 6=screen failed
p96winprobe_screen_ptr	dc.l	0
p96winprobe_window_ptr	dc.l	0
p96winprobe_log_name	dc.b	'RAM:gloom_p96_window.txt',0
p96winprobe_msg_ok	dc.b	'P96 window: OpenWindow OK',10
p96winprobe_msg_ok_len	equ	*-p96winprobe_msg_ok
p96winprobe_msg_fail	dc.b	'P96 window: OpenWindow failed',10
p96winprobe_msg_fail_len	equ	*-p96winprobe_msg_fail
p96winprobe_msg_skip_aga	dc.b	'P96 window: skipped (DISPLAY=AGA)',10
p96winprobe_msg_skip_aga_len	equ	*-p96winprobe_msg_skip_aga
p96winprobe_msg_skip_nolib	dc.b	'P96 window: skipped, no P96 library',10
p96winprobe_msg_skip_nolib_len	equ	*-p96winprobe_msg_skip_nolib
p96winprobe_msg_skip_nomode	dc.b	'P96 window: skipped, no ModeID',10
p96winprobe_msg_skip_nomode_len	equ	*-p96winprobe_msg_skip_nomode
p96winprobe_msg_skip_noint	dc.b	'P96 window: skipped, no intuition.library',10
p96winprobe_msg_skip_noint_len	equ	*-p96winprobe_msg_skip_noint
p96winprobe_msg_screenfail	dc.b	'P96 window: skipped, OpenScreenTagList failed',10
p96winprobe_msg_screenfail_len	equ	*-p96winprobe_msg_screenfail
	even

	cnop	0,4
p96winprobe_empty_title	dc.b	0
	even

p96winprobe_tags
p96winprobe_displayid_tag	dc.l	SA_DisplayID,0
	dc.l	SA_Title,p96winprobe_empty_title
	dc.l	SA_Left,0
	dc.l	SA_Top,0
p96winprobe_width_tag	dc.l	SA_Width,320
p96winprobe_height_tag	dc.l	SA_Height,240
p96winprobe_depth_tag	dc.l	SA_Depth,16
	dc.l	SA_Type,15	;CUSTOMSCREEN
	dc.l	SA_ShowTitle,0
	dc.l	SA_Quiet,-1
	dc.l	SA_AutoScroll,0
	...
	cnop	0,4
p96winprobe_newwindow
	dc	0,0		;LeftEdge,TopEdge
p96winprobe_window_size
	dc	320,240		;Width,Height
	dc.b	0,1		;DetailPen,BlockPen
	dc.l	$00000408	;IDCMP_RAWKEY|IDCMP_MOUSEBUTTONS for c86zec input probe
	dc.l	$11840		;ACTIVATE|BORDERLESS|SIMPLE_REFRESH|RMBTRAP, no gadgets
	dc.l	0		;FirstGadget
	dc.l	0		;CheckMark
	dc.l	p96winprobe_title
p96winprobe_window_screenptr
	dc.l	0		;Screen, patched after OpenScreenTagList OK
	dc.l	0		;BitMap
	dc	-1,-1,-1,-1	;Min/Max
	dc	15		;CUSTOMSCREEN
p96winprobe_title	dc.b	'Gloom P96 window probe',0
	even

g2p96_window_probe
	push
	clr	p96winprobe_state
	clr.l	p96winprobe_screen_ptr
	clr.l	p96winprobe_window_ptr
	move	g2display_mode,d0
	cmp	#1,d0
	bne	.have_nonaga
	move	#2,p96winprobe_state
	bra	.log
.have_nonaga
	tst	p96present
	bne	.have_p96
	move	#3,p96winprobe_state
	bra	.log
.have_p96
	cmp	#1,p96modeid_state
	beq	.have_mode
	move	#4,p96winprobe_state
	bra	.log
.have_mode
	move.l	p96modeid,d0
	beq	.nomode
	...
	jsr	-612(a6)	;OpenScreenTagList
	move.l	d0,d5
	move.l	d0,p96winprobe_screen_ptr
	beq	.screenfail
	move.l	d5,p96winprobe_window_screenptr
	move.l	d6,a6
	lea	p96winprobe_newwindow,a0
	jsr	-204(a6)	;OpenWindow on the fresh P96 custom screen
	move.l	d0,d4
	move.l	d0,p96winprobe_window_ptr
	beq	.close_screen_fail
	move	#1,p96winprobe_state
	move.l	d4,a0
	move.l	d6,a6
	jsr	-72(a6)	;CloseWindow immediately, no visible output path yet
	clr.l	p96winprobe_window_ptr
	bra	.close_screen
.close_screen_fail
	clr	p96winprobe_state	;state 0 = OpenWindow failed
.close_screen
	move.l	d5,a0
	move.l	d6,a6
	jsr	-66(a6)	;CloseScreen immediately
	clr.l	p96winprobe_screen_ptr
	bra	.close_int
.screenfail
	move	#6,p96winprobe_state
.close_int
	move.l	d6,a1
	move.l	4.w,a6
	jsr	-414(a6)	;CloseLibrary
...
.nomode
	...
	bra	.log
.noint
	move	#5,p96winprobe_state
.log
	pull
	rts

g2p96_window_probe_log
	push
	move.l	dosbase,d0
	beq	.done
	move.l	d0,a6
	lea	p96winprobe_log_name,a0
	move.l	a0,d1
	move.l	#1006,d2	;MODE_NEWFILE
	jsr	-30(a6)	;Open
	move.l	d0,d7
	beq	.done
	move	p96winprobe_state,d0
	cmp	#1,d0
	beq	.ok
	cmp	#2,d0
	beq	.skipaga
	...
	cmp	#6,d0
	beq	.screenfail
	lea	p96winprobe_msg_fail,a0
	move.l	#p96winprobe_msg_fail_len,d3
	bra	.write
.ok
	lea	p96winprobe_msg_ok,a0
	move.l	#p96winprobe_msg_ok_len,d3
	bra	.write
.skipaga
	lea	p96winprobe_msg_skip_aga,a0
	move.l	#p96winprobe_msg_skip_aga_len,d3
	bra	.write
.skipnolib
	lea	p96winprobe_msg_skip_nolib,a0
	move.l	#p96winprobe_msg_skip_nolib_len,d3
	bra	.write
.skipnomode
	lea	p96winprobe_msg_skip_nomode,a0
	move.l	#p96winprobe_msg_skip_nomode_len,d3
	bra	.write
.skipnoint
	lea	p96winprobe_msg_skip_noint,a0
	move.l	#p96winprobe_msg_skip_noint_len,d3
	bra	.write
.screenfail
	lea	p96winprobe_msg_screenfail,a0
	move.l	#p96winprobe_msg_screenfail_len,d3
.write
	move.l	d7,d1
	move.l	a0,d2
	jsr	-48(a6)	;Write
...
; -----------------------------------------------------------------------------
; c86zec: conservative menu/input probe on the proven 320x240 P96
; screen/window path.  This still does not enable gameplay P96, does not sample
; the chunky buffer, does not decode real assets, and does not use the old
; Workbench overlay.  It opens a fresh P96 screen/window, draws a simple
; menu/status marker from RectFill blocks, waits for input or timeout, closes
; everything, then logs DH3.
; -----------------------------------------------------------------------------
p96static_state		dc	0	;0=draw/window failed, 1=OK, 2=skip AGA, 3=no P96 lib, 4=no ModeID, 5=no intuition, 6=screen failed, 7=window failed, 8=no graphics
p96static_grbase	dc.l	0
p96input_state		dc	0	;0=not run, 1=input event, 2=timeout
p96input_log_name	dc.b	'RAM:gloom_p96_input.txt',0
p96input_msg_event	dc.b	'P96 input: event OK',10
p96input_msg_event_len	equ	*-p96input_msg_event
p96input_msg_timeout	dc.b	'P96 input: timeout OK',10
p96input_msg_timeout_len	equ	*-p96input_msg_timeout
p96input_msg_skipped	dc.b	'P96 input: skipped',10
p96input_msg_skipped_len	equ	*-p96input_msg_skipped
	even
p96static_log_name	dc.b	'RAM:gloom_p96_static.txt',0
p96static_msg_ok	dc.b	'P96 static: input-marker OK',10
p96static_msg_ok_len	equ	*-p96static_msg_ok
p96static_msg_fail	dc.b	'P96 static: failed',10
p96static_msg_fail_len	equ	*-p96static_msg_fail
p96static_msg_skip_aga	dc.b	'P96 static: skipped (DISPLAY=AGA)',10
p96static_msg_skip_aga_len	equ	*-p96static_msg_skip_aga
p96static_msg_skip_nolib	dc.b	'P96 static: skipped, no P96 library',10
p96static_msg_skip_nolib_len	equ	*-p96static_msg_skip_nolib
p96static_msg_skip_nomode	dc.b	'P96 static: skipped, no ModeID',10
p96static_msg_skip_nomode_len	equ	*-p96static_msg_skip_nomode
p96static_msg_skip_noint	dc.b	'P96 static: skipped, no intuition.library',10
p96static_msg_skip_noint_len	equ	*-p96static_msg_skip_noint
p96static_msg_screenfail	dc.b	'P96 static: skipped, OpenScreenTagList failed',10
p96static_msg_screenfail_len	equ	*-p96static_msg_screenfail
p96static_msg_windowfail	dc.b	'P96 static: skipped, OpenWindow failed',10
p96static_msg_windowfail_len	equ	*-p96static_msg_windowfail
p96static_msg_skip_nogfx	dc.b	'P96 static: skipped, no graphics.library',10
p96static_msg_skip_nogfx_len	equ	*-p96static_msg_skip_nogfx
	even

g2p96_static_buffer_probe
	push
	clr	p96static_state
	clr.l	p96static_grbase
	move	g2display_mode,d0
	cmp	#1,d0
	bne	.have_nonaga
	move	#2,p96static_state
	bra	.log
.have_nonaga
	tst	p96present
	bne	.have_p96
	move	#3,p96static_state
	bra	.log
.have_p96
	cmp	#1,p96modeid_state
	beq	.have_mode
	move	#4,p96static_state
	bra	.log
.have_mode
	move.l	p96modeid,d0
	beq	.nomode
	...
	move.l	4.w,a6
	lea	grname,a1
	jsr	-408(a6)	;OldOpenLibrary graphics.library for this early standalone probe
	move.l	d0,p96static_grbase
	beq	.nogfx
	move	#1,p96static_state
	move.l	d4,a0
	bsr	g2p96_static_draw_title
	move.l	d4,a0
	bsr	g2p96_static_wait
	bra	.close_window
.nogfx
	move	#8,p96static_state
.close_window
	move.l	d4,a0
	move.l	d6,a6
	jsr	-72(a6)	;CloseWindow
...
.close_screen
	...
	move.l	p96static_grbase,d0
	beq	.close_int
	move.l	d0,a1
	move.l	4.w,a6
	jsr	-414(a6)	;CloseLibrary graphics.library
	clr.l	p96static_grbase
	bra	.close_int
.windowfail
	move	#7,p96static_state
	move.l	d5,a0
	move.l	d6,a6
	jsr	-66(a6)	;CloseScreen after failed OpenWindow
	clr.l	p96winprobe_screen_ptr
	clr.l	p96winprobe_window_screenptr
	bra	.close_int
.screenfail
	move	#6,p96static_state
.close_int
	move.l	d6,a1
	move.l	4.w,a6
	jsr	-414(a6)	;CloseLibrary intuition.library
	clr.l	p96gameplay_intbase
	bra	.log
.nomode
	move	#4,p96static_state
	bra	.log
.noint
	move	#5,p96static_state
.log
	pull
	rts

; a0 = Window. Draws a deliberately primitive menu/input marker using only pens 0/1.
g2p96_static_draw_title
	push
	move.l	a0,a5
	move.l	50(a5),a1	;Window.RPort
	move.l	a1,d0
	...
	move.l	d0,a6
	moveq	#0,d0
	jsr	-342(a6)	;SetAPen background
	move.l	50(a5),a1
	move.l	p96static_grbase,a6
	moveq	#0,d0
	moveq	#0,d1
	move	#319,d2
	move	#239,d3
	jsr	-306(a6)	;RectFill full background
	move.l	50(a5),a1
	move.l	p96static_grbase,a6
	moveq	#1,d0
	jsr	-342(a6)	;SetAPen marker
	move.l	50(a5),a1
	move.l	p96static_grbase,a6
	moveq	#0,d0
	moveq	#0,d1
	...
	move	#279,d2
	move	#71,d3
	jsr	-306(a6)	;upper title marker bar
	move.l	50(a5),a1
	move.l	p96static_grbase,a6
	move	#72,d0
	move	#154,d1
	move	#247,d2
	move	#173,d3
	jsr	-306(a6)	;lower status marker bar
.done
	pull
	rts
...
g2p96_static_wait
	push
	clr	p96input_state
	move.l	a0,a5	;Window pointer.  Wait for IDCMP input, mouse/fire, or timeout.
	move	#600,d7	;about 10 seconds PAL max, so the test cannot trap the user
.wait
	move.l	a5,d0
	beq	.timeout
	move.l	86(a5),d0	;Window.UserPort
	beq	.check_hw
	move.l	d0,a0
	move.l	4.w,a6
	...
	move.l	d0,a1
	move.l	4.w,a6
	jsr	-378(a6)	;ReplyMsg(IntuiMessage)
.drain
	move.l	86(a5),d0	;drain any already queued input before CloseWindow
	beq	.event
	move.l	d0,a0
	move.l	4.w,a6
	jsr	-372(a6)	;GetMsg(UserPort)
	...
	bra	.drain
.check_hw
	btst	#6,$bfe001	;left mouse / port 1 fire, active low
	beq	.event
	btst	#7,$bfe001	;right mouse / port 0 fire, active low
	beq	.event
	move.l	p96static_grbase,d0
	beq	.timeout
	move.l	d0,a6
	...
	dbf	d7,.wait
.timeout
	move	#2,p96input_state
	bra	.done
.event
	move	#1,p96input_state
.done
	pull
	rts

g2p96_static_buffer_log
	push
	move.l	dosbase,d0
	beq	.done
	move.l	d0,a6
	lea	p96static_log_name,a0
	move.l	a0,d1
	move.l	#1006,d2	;MODE_NEWFILE
	jsr	-30(a6)	;Open
	move.l	d0,d7
	beq	.done
	move	p96static_state,d0
	cmp	#1,d0
	beq	.ok
	cmp	#2,d0
	beq	.skipaga
	...
	cmp	#8,d0
	beq	.skipnogfx
	lea	p96static_msg_fail,a0
	move.l	#p96static_msg_fail_len,d3
	bra	.write
.ok
	lea	p96static_msg_ok,a0
	move.l	#p96static_msg_ok_len,d3
	bra	.write
.skipaga
	lea	p96static_msg_skip_aga,a0
	move.l	#p96static_msg_skip_aga_len,d3
	bra	.write
.skipnolib
	lea	p96static_msg_skip_nolib,a0
	move.l	#p96static_msg_skip_nolib_len,d3
	bra	.write
.skipnomode
	lea	p96static_msg_skip_nomode,a0
	move.l	#p96static_msg_skip_nomode_len,d3
	bra	.write
.skipnoint
	lea	p96static_msg_skip_noint,a0
	move.l	#p96static_msg_skip_noint_len,d3
	bra	.write
.screenfail
	lea	p96static_msg_screenfail,a0
	move.l	#p96static_msg_screenfail_len,d3
	bra	.write
.windowfail
	lea	p96static_msg_windowfail,a0
	move.l	#p96static_msg_windowfail_len,d3
	bra	.write
.skipnogfx
	lea	p96static_msg_skip_nogfx,a0
	move.l	#p96static_msg_skip_nogfx_len,d3
.write
	move.l	d7,d1
	move.l	a0,d2
	jsr	-48(a6)	;Write
...
g2p96_input_log
	...
	beq	.done
	move.l	d0,a6
	lea	p96input_log_name,a0
	move.l	a0,d1
	move.l	#1006,d2	;MODE_NEWFILE
	jsr	-30(a6)	;Open
	move.l	d0,d7
	beq	.done
	move	p96input_state,d0
	cmp	#1,d0
	beq	.event
	cmp	#2,d0
	beq	.timeout
	lea	p96input_msg_skipped,a0
	move.l	#p96input_msg_skipped_len,d3
	bra	.write
.event
	lea	p96input_msg_event,a0
	move.l	#p96input_msg_event_len,d3
	bra	.write
.timeout
	lea	p96input_msg_timeout,a0
	move.l	#p96input_msg_timeout_len,d3
.write
	move.l	d7,d1
	move.l	a0,d2
	jsr	-48(a6)	;Write
...
; -----------------------------------------------------------------------------
; c86zfr: P96 in-game menu bridge visible probe on top of display-state foundation.
; Default stays 320x240.  HIRES=YES selects 640x480 with 2x nearest-neighbour
; output.  STRETCH=YES searches confirmed 240-line widescreen modes first
; (426x240, 400x240, 424x240, 432x240) and falls back to confirmed
; 480-line widescreen modes if no 240-line mode is available.
; -----------------------------------------------------------------------------
p96gameplay_state		dc	0	;0=fail, 1=direct RGB565 bars OK, 2=skip AGA, 3=no P96 lib, 4=no ModeID, 5=no intuition, 6=screen fail, 7=window fail, 8=no graphics, 9=no direct bitmap
p96gameplay_latched		dc	0	;c86zfb 0=not opened, 1=open/waiting for first good copy, 2=logged/failed
p96gameplay_delay_counter	dc	0	;c86zfa: wait a few gameplay presents before starting P96 primary
p96gameplay_persist_active	dc	0	;-1 while the P96 gameplay-primary screen is open
p96gameplay_persist_frames	dc	0	;unused in c86zfa primary mode; kept for rollback/build continuity
p96gameplay_skip_aga_present	dc	0	;-1 when this frame was copied to P96 and AGA present may be skipped
p96gameplay_grbase		dc.l	0
p96gameplay_intbase		dc.l	0
p96gameplay_pen64_state	dc	0	;0=raw pens, -1=64-entry BestPen map valid
p96gameplay_pen64_tags	dc.l	0	;TAG_DONE
p96gameplay_pen64_map	dcb.b	64,0
p96gameplay_direct_state	dc	0	;set by direct bitmap writer when planes[0]/BPR are usable
p96gameplay_rgb565_source_state	dc	0	;0=not valid, -1=c86zds source-byte RGB565 LUT valid
p96gameplay_linear_ready	dc	0	;c86zjd -1 after one confirmed legacy-layout P96 copy
p96gameplay_linear_active	dc	0	;c86zjd -1 while coloffs is the linear 0..319 table
p96gameplay_linear_logstate	dc	0	;c86zjd 1=active, 2=fallback restored C2P
p96gameplay_pen64_tab2	dc.b	0,85,170,255
p96gameplay_burst_phase	dc	0
p96gameplay_region_dstx	dc	0
p96gameplay_region_dsty	dc	0
p96gameplay_region_mode	dc	0	;0=coloffs y*4, 1=linear y*4, 2=coloffs centre y*2+48, 3=coloffs y*5
P96DSP_AGA		equ	0
P96DSP_GAMEPLAY	equ	1
P96DSP_MENU_BRIDGE	equ	2
P96DSP_INTERMISSION_BRIDGE	equ	3
P96DSP_TITLE_BRIDGE	equ	4
P96DSP_TRANSITION_BLACK	equ	5
P96DSP_SHUTDOWN	equ	6
p96display_state	dc	0	;c86zfq: current display owner/state
p96display_prev_state	dc	0	;c86zfq: previous display owner/state
p96display_event	dc	0	;c86zfq: last transition event marker
p96title_qmenu_rebuild_needed	dc	0	;c86zhe: rebuild title cache before first qmenu after gameplay/menu close
p96gameplay_log_name	dc.b	'RAM:gloom_p96_gameplay.txt',0
p96gameplay_msg_ok	dc.b	'P96 gameplay: newgame stale frame clear OK',10
p96gameplay_msg_ok_len	equ	*-p96gameplay_msg_ok
p96gameplay_msg_skip_aga	dc.b	'P96 gameplay: skipped (DISPLAY=AGA)',10
p96gameplay_msg_skip_aga_len	equ	*-p96gameplay_msg_skip_aga
p96gameplay_msg_skip_nolib	dc.b	'P96 gameplay: skipped, no P96 library',10
p96gameplay_msg_skip_nolib_len	equ	*-p96gameplay_msg_skip_nolib
p96gameplay_msg_skip_nomode	dc.b	'P96 gameplay: skipped, no ModeID',10
p96gameplay_msg_skip_nomode_len	equ	*-p96gameplay_msg_skip_nomode
p96gameplay_msg_skip_noint	dc.b	'P96 gameplay: skipped, no intuition.library',10
p96gameplay_msg_skip_noint_len	equ	*-p96gameplay_msg_skip_noint
p96gameplay_msg_screenfail	dc.b	'P96 gameplay: skipped, OpenScreenTagList failed',10
p96gameplay_msg_screenfail_len	equ	*-p96gameplay_msg_screenfail
p96gameplay_msg_windowfail	dc.b	'P96 gameplay: skipped, OpenWindow failed',10
p96gameplay_msg_windowfail_len	equ	*-p96gameplay_msg_windowfail
p96gameplay_msg_skip_nogfx	dc.b	'P96 gameplay: skipped, no graphics.library',10
p96gameplay_msg_skip_nogfx_len	equ	*-p96gameplay_msg_skip_nogfx
p96gameplay_msg_nobitmap	dc.b	'P96 gameplay: skipped, no direct bitmap',10
p96gameplay_msg_nobitmap_len	equ	*-p96gameplay_msg_nobitmap
p96gameplay_msg_fail	dc.b	'P96 gameplay: failed',10
p96gameplay_msg_fail_len	equ	*-p96gameplay_msg_fail
p96linear_log_name	dc.b	'RAM:gloom_p96_linear.txt',0
p96linear_msg_active	dc.b	'P96 linear chunky: active after confirmed legacy frame',10
p96linear_msg_active_len	equ	*-p96linear_msg_active
p96linear_msg_fallback	dc.b	'P96 linear chunky: bitmap failure, C2P layout restored',10
p96linear_msg_fallback_len	equ	*-p96linear_msg_fallback
	even

g2p96_gameplay_present_probe
	push
	clr	p96gameplay_skip_aga_present	;c86zfb: never skip AGA unless this exact frame copied OK
	; c86zfb lifecycle:
	; - open the P96 screen as soon as gameplay starts, to hide the short AGA handoff
	; - keep the confirmed c86zex/c86zey delay before copying/skipping, so walls exist
	; - log only after the first successful delayed P96 copy
	tst	p96gameplay_persist_active
	beq	.not_active
	move	p96gameplay_delay_counter,d0
	cmp	#12,d0
	bhs	.active_ready
	addq	#1,p96gameplay_delay_counter
	; c86zjd: after linear mode has already been activated (for example after
	; a level/intermission handoff), the visible P96 screen stays black during
	; the guarded delay.  Do not run AGA C2P on a linear source frame.
...
.active_ready
	...
	move	#2,p96gameplay_latched
	bra	.done
.active_ok
	jsr	g2p96_display_enter_gameplay	;c86zfr: returning from menu bridge gives gameplay ownership back to P96
	cmp	#1,p96gameplay_latched
	bne	.done
	move	#2,p96gameplay_latched
	bra	.done
.not_active
	tst	p96gameplay_latched
	bne	.done
	move	#1,p96gameplay_latched	;c86zfb: one open attempt per gameplay/menu cycle
	clr	p96gameplay_delay_counter
	clr	p96gameplay_state
	jsr	g2p96_gameplay_restore_c2p_layout	;c86zjd new/open attempt always starts from known legacy layout
	clr.l	p96gameplay_grbase
	clr.l	p96gameplay_intbase
	move	g2display_mode,d0
	cmp	#1,d0
	bne	.have_nonaga
	move	#2,p96gameplay_state
	bra	.log_immediate
.have_nonaga
	tst	p96present
	bne	.have_p96
	move	#3,p96gameplay_state
	bra	.log_immediate
.have_p96
	cmp	#1,p96modeid_state
	beq	.try_open
	move	#4,p96gameplay_state
	bra	.log_immediate
.try_open
	jsr	g2p96_gameplay_persistent_open
	tst	p96gameplay_persist_active
	beq	.log_immediate
	; Do not copy yet: first frames before the old confirmed delay miss wall columns.
	; Clear the P96 bitmap so the player no longer sees a brief AGA gameplay frame.
...
.done
	pull
	rts
g2p96_gameplay_persistent_open
	push
	...
	move.l	d0,p96winprobe_screen_ptr
	beq	.screenfail
	move.l	d0,p96winprobe_window_screenptr
	jsr	g2p96_force_hide_screen_title	;c86zjp: some P96/Intuition setups reveal the screen bar despite SA_ShowTitle=FALSE
	move.l	p96gameplay_intbase,a6
	lea	p96winprobe_newwindow,a0
	jsr	-204(a6)	;OpenWindow on the fresh P96 custom screen
	move.l	d0,p96winprobe_window_ptr
	beq	.windowfail
	jsr	g2p96_gameplay_hide_pointer	;c86zfb: P96 window must not show OS mouse pointer
	jsr	g2p96_gameplay_clear_p96_screen	;c86zfb: neutral black screen during startup delay
	move.l	4.w,a6
	lea	grname,a1
	jsr	-408(a6)	;OldOpenLibrary graphics.library
	move.l	d0,p96gameplay_grbase
	beq	.nogfx
	move	#1,p96gameplay_state
	move	#-1,p96gameplay_persist_active
	move	#-1,p96gameplay_persist_frames	;c86zfb: primary gameplay path stays open
	jsr	g2p96_gameplay_dbuf_init	;c86zje: optional real ScreenBuffer double buffering
	jsr	g2p96_display_enter_gameplay	;c86zfq: central owner state for live P96 gameplay
	bra	.done
.nomode
	move	#4,p96gameplay_state
	bra	.done
.noint
	move	#5,p96gameplay_state
	bra	.done
.screenfail
	move	#6,p96gameplay_state
	bra	.close_int_only
.windowfail
	move	#7,p96gameplay_state
...
g2p96_gameplay_persistent_update
	push
...
.draw
	...
	bsr	g2p96_gameplay_draw_mini_frame_160x96
...
.frame_ok
	...
	bne.s	.no_fps_count	;c87a: menu-backdrop copies are not gameplay frames
	jsr	g2fps_frame_presented	;c87a: count only the successfully copied/flipped P96 frame
.no_fps_count
	move	#1,p96gameplay_state
	move	#-1,p96gameplay_skip_aga_present
	tst	p96gameplay_linear_active
...
.fail_dbuf
	...
	move	#3,p96gameplay_dbuf_logstate
	jsr	g2p96_gameplay_dbuf_log
	bra.s	.fail_common
.fail_bitmap
...
.fail_common
	...
	beq.s	.legacy_fail
	move	#-1,p96gameplay_skip_aga_present
	move	#2,p96gameplay_linear_logstate
	jsr	g2p96_gameplay_linear_log
...
g2p96_gameplay_persistent_close
	push
	jsr	g2p96_gameplay_restore_c2p_layout	;c86zjd never leave linear coloffs behind a closed P96 owner
	jsr	g2p96_gameplay_dbuf_suspend_primary	;c86zje: restore original bitmap before close
	; c86zfo: if a live P96 gameplay screen is being handed back to the
	; script/intermission/menu path, blank it first so the old rendered frame is
	; not visible as a stale standstill during the transition.
	tst	p96gameplay_persist_active
	beq.s	.no_preclear
	jsr	g2p96_gameplay_clear_p96_screen
.no_preclear
	...
	clr.l	p96winprobe_window_ptr
.no_window
	jsr	g2p96_gameplay_dbuf_free	;c86zje: after window, before screen
	move.l	p96winprobe_screen_ptr,d0
	beq	.no_screen
	move.l	d0,a0
	move.l	p96gameplay_intbase,a6
...
; c86zjd: guarded row-major gameplay source probe.
; The existing renderer already sends every visible x through coloffs.  Replacing
; that table with 0..319 therefore makes walls, objects, gun, HUD and effects write
; into a normal linear 320-byte row without touching each renderer separately.
; Activation is delayed until one old-layout frame has reached the direct P96
; bitmap successfully.  AGA/TWO PLAYER/static-screen paths keep the original table.
g2p96_gameplay_prepare_linear_frame
	movem.l	d0-d2/a0,-(a7)
	tst	twowins
	bne.s	.legacy
	cmp	#1,g2display_mode
	beq.s	.legacy
	tst	p96gameplay_persist_active
	beq.s	.legacy
...
.linear_loop
	...
	move	#1,p96gameplay_linear_logstate
	jsr	g2p96_gameplay_linear_log
	jsr	g2p96_perf_snapshot_once	;c86zjm: diagnostic only, one gameplay memory snapshot per run
	bra.s	.done
.legacy
	tst	p96gameplay_linear_active
	beq.s	.clear_ready
	...
	bra.s	.done
.clear_ready
	clr	p96gameplay_linear_ready
.done
	movem.l	(a7)+,d0-d2/a0
	rts

; Rebuild the exact original blackmagic_1-compatible column table.  This routine
; is intentionally safe to call more than once and also clears the linear arm.
g2p96_gameplay_restore_c2p_layout
	movem.l	d0-d2/a0-a1,-(a7)
	tst	p96gameplay_linear_active
	beq.s	.clear
	lea	coloffs,a0
	...
	jsr	g2_inline_c2p_init
.clear
	clr	p96gameplay_linear_active
	clr	p96gameplay_linear_ready
	movem.l	(a7)+,d0-d2/a0-a1
	rts

; One tiny state marker, written only when linear mode activates or a direct
; bitmap failure forces the C2P rollback.  No per-frame logging is performed.
g2p96_gameplay_linear_log
	movem.l	d0-d7/a0-a1/a6,-(a7)
	move.l	dosbase,d0
	beq.s	.done
	move.l	d0,a6
	lea	p96linear_log_name,a0
	move.l	a0,d1
	move.l	#1006,d2	;MODE_NEWFILE
	jsr	-30(a6)	;Open
	move.l	d0,d7
	beq.s	.done
	cmp	#2,p96gameplay_linear_logstate
	beq.s	.fallback
	lea	p96linear_msg_active,a0
	move.l	#p96linear_msg_active_len,d3
	bra.s	.write
.fallback
	lea	p96linear_msg_fallback,a0
	move.l	#p96linear_msg_fallback_len,d3
.write
	move.l	d7,d1
	move.l	a0,d2
	jsr	-48(a6)	;Write
...
; c86zfq: central display-state manager foundation.  For now only gameplay is
; P96-native.  Title, in-game menu and intermission are explicit AGA-bridge
; states, so all transitions go through one place instead of scattered ad-hoc
; closes.  Later P96 menu/intermission ports can replace these bridge states
; without touching every caller again.
; c86zft: VIEW SIZE and FULL SCREEN stay visible/selectable in P96 menus,
; but they are disabled no-ops until P96-native viewport/window-size support exists.
g2p96_ingame_menu_disabled_noop
	jsr	g2p96_menu_bridge_current_row_present	;c86zfx: disabled rows are no-op, refresh only highlight row
	jsr	g2v17_wait_menu_release
	rts

; c86zfw: FLOOR/CEILING must stay inside the P96 in-game menu and update
; the visible grey gameplay backdrop immediately, like the original AGA path.
; Rebuild the menu over a freshly rendered gameplay frame, then copy the bridge
; back to P96.  VIEW SIZE/FULL SCREEN remain no-op rows under P96.
g2p96_ingame_menu_floorceil_live_refresh
	movem.l	d0/a4,-(a7)
	move	curropt,-(a7)
	jsr	g2p96_display_should_lock_fullwin
	tst	d0
	beq.s	.aga_refresh_full
	; c86zgo: P96 must keep the menu visible, but FLOOR/CEILING also has to
	; update the live gameplay view underneath.  Do one gameplay source redraw
	; without rebuilding/freeing the menu strips, then overlay the already-built
	; real-bigfont menu again.  This avoids the old initmenu2/predrawall 2-second
	; blackout while restoring the expected live FLOOR/CEILING preview.
	moveq	#P96DSP_MENU_BRIDGE,d0
	jsr	g2p96_display_state_set
	move	#-1,p96menu_native_selected
	jsr	g2p96_menu_glyph_mode_active
	tst	d0
	bne	.g2c86zjj_floorceil_native
	jsr	g2p96_menu_recompose_current_row_planar
.g2c86zjj_floorceil_native
	jsr	g2p96_menu_redraw_gameplay_source_once
	moveq	#P96DSP_MENU_BRIDGE,d0
	jsr	g2p96_display_state_set
	jsr	g2p96_menu_bridge_all_rows_present
	move	(a7),curropt
	jsr	opton
	bra.s	.refresh_wait_release
.aga_refresh_full
	; Keep the original AGA behaviour unchanged: rebuild backdrop/menu there.
	jsr	g2v190aj_restore_game_palette
	jsr	finitmenu
	jsr	predrawall
	move	(a7),curropt
	...
	jsr	g2v190aj_grey_menu_backdrop
.refresh_done
	jsr	opton		;c86zgi: AGA path refreshes selected row after full rebuild
...
; c86zgq: P96 in-game menu selection loop with slow P96-only blink.
; c86zgo/c86zgp kept the row stable to avoid the broken-lamp effect caused by
; repeated optoff/opton real-Bigfont rebuilds.  This version keeps that anchor,
; but blinks only the already-built P96 row: OFF restores the gameplay row under
; the selected item, ON redraws the existing real-bigfont glyph delta.  No live
; printmess2/menubmap rebuild is done on the blink cadence.
g2p96_menu_should_use_stable_select
	moveq	#0,d0
	; c86zhi: DISPLAY=AGA must never enter the P96 blink loop.
	; This preserves the original AGA title/in-game menu blinking.
	jsr	g2p96_display_is_p96_capable
	tst	d0
	beq	.done
	moveq	#0,d0
	tst	game_menu_active
	bne	.check_ingame
	cmp	#P96DSP_TITLE_BRIDGE,p96display_state	;c86zgy: title menu uses same P96 blink loop
	bne	.done
	tst	p96gameplay_persist_active
	beq	.done
	moveq	#-1,d0
	rts
.check_ingame
	cmp	#P96DSP_MENU_BRIDGE,p96display_state
	bne	.done
	tst	p96gameplay_persist_active
	beq	.done
	moveq	#-1,d0
...
g2p96_menu_selmenu_stable
	jsr	g2p96_menu_native_nav_select	;c86zji: cached P96 row, no planar redraw/blitter
	move	#-1,p96menu_blink_visible
	move	#22,p96menu_blink_count	;c86zgq: slower, deliberate blink cadence
.wait
	jsr	vwait
	jsr	readmenusel
	bne	.got_input
	subq	#1,p96menu_blink_count
	bgt	.wait
	move	#22,p96menu_blink_count
...
.got_input
	btst	#5,d0	;v115 ESC cancel/back in game menu
	beq	.noesc
	jsr	g2p96_menu_blink_ensure_visible	;c86zgs: leave menu with row restored ON
	moveq	#0,d0
	rts
.noesc
	btst	#0,d0
	...
	btst	#3,d0
	bne	.down
	jsr	g2p96_menu_blink_ensure_visible	;c86zgs: accept/activate uses stable visible row
	jsr	menuskip
	beq	.down
	move	curropt,d0
	rts
.left
	tst	game_menu_active	;c86zgy: title menus only allow left on player/level rows
	bne	.left_ok
	jsr	g2v166_title_player_lr
	beq	.wait
.left_ok
	jsr	g2p96_menu_blink_ensure_visible	;c86zgs: option refresh starts from visible text
	move	curropt,d0
	or	#$0100,d0
	rts
.right
	tst	game_menu_active	;c86zgy: title menus only allow right on player/level rows
	bne	.right_ok
	jsr	g2v166_title_player_lr
	beq	.wait
.right_ok
	...
	rts
.up
	move	p96menu_blink_visible,p96menu_nav_old_visible	;c86zgs: preserve blink phase
	jsr	g2p96_menu_blink_ensure_visible	;c86zgs: old selected row can be normalised safely
	jsr	g2p96_menu_native_nav_normal	;c86zji: normal cached P96 row, no planar restore
.up_loop
	subq	#1,curropt
	bpl	.up_chk
	move	numopts,d0
...
.up_chk
	jsr	menuskip
	beq	.up_loop
	jsr	g2p96_menu_native_nav_select	;c86zji: selected cached P96 row, no planar redraw
...
; -----------------------------------------------------------------------------
p96menu_nav_log_state	dc	0
p96menu_nav_log_name	dc.b	'RAM:gloom_p96_menunav.txt',0
p96menu_nav_log_msg	dc.b	'P96 menu: native navigation without AGA blitter active',10
p96menu_nav_log_msg_len	equ	*-p96menu_nav_log_msg
	even
...
g2p96_menu_native_nav_select
	...
	jsr	g2p96_menu_native_nav_log_once
	movem.l	(a7)+,d0
	rts
...
g2p96_menu_native_nav_present
	movem.l	d0-d1,-(a7)
	cmp	#P96DSP_TITLE_BRIDGE,p96display_state
	bne.s	.not_title
	move	curropt,d0
	move	fonth,d1
	mulu	d1,d0
	add	menuy,d0
	move	d0,p96menu_bridge_y_start
	add	d1,d0
	move	d0,p96menu_bridge_y_end
	jsr	g2p96_menu_restore_current_row_best
	jsr	g2p96_menu_draw_current_row_best_no_restore
	bra.s	.done
.not_title
...
g2p96_menu_native_nav_log_once
	movem.l	d0-d3/d7/a0/a6,-(a7)
	tst	p96menu_nav_log_state
	bne.s	.done
	move	#-1,p96menu_nav_log_state
	move.l	dosbase,d0
	beq.s	.done
	move.l	d0,a6
	lea	p96menu_nav_log_name,a0
	move.l	a0,d1
	move.l	#1006,d2
	jsr	-30(a6)
	move.l	d0,d7
	beq.s	.done
	move.l	d7,d1
	lea	p96menu_nav_log_msg,a0
	move.l	a0,d2
	move.l	#p96menu_nav_log_msg_len,d3
	jsr	-48(a6)
	move.l	d7,d1
	jsr	-36(a6)
.done
...
; c86zgq: blink helpers.  They only touch the P96 visible row/cache, not the
; original menubmap/font renderer.  That keeps Bigfont correct while avoiding
; the c86zgn optoff/opton flicker and repeated printmess2 rebuild cost.
g2p96_menu_blink_current_row_on
	movem.l	d0,-(a7)
	cmp	#P96DSP_TITLE_BRIDGE,p96display_state	;c86zgy: title menu blinks against staged title background
	bne	.not_title_on
...
g2p96_menu_blink_current_row_off
	movem.l	d0,-(a7)
	cmp	#P96DSP_TITLE_BRIDGE,p96display_state	;c86zgy: title menu OFF restores clean staged title row
	bne	.not_title_off
...
g2p96_display_is_p96_capable
	...
	cmp	#1,d1		;DISPLAY=AGA: pure old path
	beq.s	.done
	tst	p96present
	beq.s	.done
	cmp	#1,p96modeid_state
...
g2p96_display_enter_title
	...
	jsr	g2p96_gameplay_restore_c2p_layout	;c86zjd title/static bridge stays on the proven legacy layout
	moveq	#1,d0
...
.g2c86zhe_no_qmenu_mark
	jsr	g2p96_static_open_bridge_screen	;c86zgv: keep/open P96 while title is built offscreen
	moveq	#P96DSP_TITLE_BRIDGE,d0
	jsr	g2p96_display_state_set
	pull
	rts
...
g2p96_display_enter_ingame_menu
	...
	moveq	#2,d0
	move	d0,p96display_event
	; c86zfr: keep the live P96 screen open for the ESC menu bridge.
	; The original menu still draws into the normal planar menu bitmap, then
	; g2p96_menu_bridge_present copies that bitmap to the P96 framebuffer.
	clr	p96gameplay_skip_aga_present
	moveq	#P96DSP_MENU_BRIDGE,d0
	jsr	g2p96_display_state_set
	pull
...
g2p96_display_enter_intermission
	push
	jsr	g2p96_gameplay_dbuf_suspend_visible	;c86zjf: static bridge writes to the visible buffer
	jsr	g2p96_gameplay_restore_c2p_layout	;c86zjd intermission/static bridge stays on the proven legacy layout
	moveq	#3,d0
	move	d0,p96display_event
	jsr	g2p96_static_open_bridge_screen	;c86zgv: keep/open P96 while intermission is built offscreen
	moveq	#P96DSP_INTERMISSION_BRIDGE,d0
	jsr	g2p96_display_state_set
	pull
	rts
...
; P96 gameplay is the only P96-native owner in c86zfq.  Until the in-game menu
; itself is moved to P96, WINDOW SIZE must not change the 320x240 source frame
; shape when P96 is available; DISPLAY=AGA keeps the original behaviour.
g2p96_display_should_lock_fullwin
	jsr	g2p96_display_is_p96_capable
	rts

; One controlled bridge out of P96 ownership.  It blanks the P96 bitmap, waits a
; refresh if graphics.library is open, then closes the P96 screen/window.  This
; is the foundation for replacing AGA-bridge title/menu/intermission with true
; P96-native screens later.
...
; c86zfc compatibility entry: old callers now also pass through the central
; state manager close path.
g2p96_gameplay_force_close
	push
	jsr	g2p96_display_black_close_if_open
	pull
	rts
...
g2p96_gameplay_hide_pointer
	...
	moveq	#0,d3
	move.l	p96gameplay_intbase,a6
	jsr	-270(a6)	;Intuition SetPointer, 1-line blank chip pointer
.done
	movem.l	(a7)+,d0-d3/a0-a1/a6
	rts

; c86zfb/c86zfj: black-fill the open direct RGB565 P96 bitmap during startup/menu handoff.
g2p96_gameplay_clear_p96_screen
	...
	beq.w	.done
	move.l	d0,a1
	move.l	4(a1),d0	;RastPort.BitMap
	beq.w	.done
	move.l	d0,a2
	move.l	8(a2),d0	;BitMap.Planes[0] / direct P96 base
	beq.w	.done
	move.l	d0,a3
	moveq	#0,d4
	move	0(a2),d4	;BitMap.BytesPerRow
	and.l	#$0000ffff,d4
	move	p96target_width,d0
	add	d0,d0		;minimum bytes per row = width*2
	and.l	#$0000ffff,d0
	cmp.l	d0,d4
	blo.w	.done
	moveq	#0,d6
...
; c86zff/c86zfj: a0 = Window.  Overlay a small true 1:1 black/white pixel
; checker in the top-left corner of the already copied P96 gameplay frame.
; This is a real-hardware sharpness probe only; black/white are RGB565 values
; that are identical in both byte orders, so any visible softness comes from
; RTG/monitor scaling rather than colour conversion.
g2p96_gameplay_draw_sharpness_probe
	movem.l	d0-d7/a0-a3,-(a7)
	move.l	a0,a3		; Window
	move.l	50(a3),d0	; Window.RPort
	beq.w	.done
	...
	move.l	8(a2),d0	; BitMap.Planes[0] / direct P96 base
	beq.w	.done
	move.l	d0,a0		; destination bitmap base
	moveq	#0,d4
	move	0(a2),d4	; BitMap.BytesPerRow
	and.l	#$0000ffff,d4
	move	p96target_width,d0
	add	d0,d0		; minimum BytesPerRow = target width * 2
	and.l	#$0000ffff,d0
	cmp.l	d0,d4
	blo.w	.done

	; Include Window offsets for completeness, although the gameplay window is 0,0.
	moveq	#0,d0
	move	6(a3),d0	; Window.TopEdge
	mulu	d4,d0
	adda.l	d0,a0
	moveq	#0,d0
	move	4(a3),d0	; Window.LeftEdge
	add.l	d0,d0		; 16-bit pixels: x*2
	adda.l	d0,a0
...
.pixloop
	...
	and	#1,d0
	beq.s	.black
	move	#$ffff,(a1)+	; white RGB565
	bra.s	.nextpix
.black
	clr.w	(a1)+		; black RGB565
.nextpix
	addq	#1,d7
	bra.s	.pixloop
.nextrow
...
; a0 = Window.  c86zfg full-source P96-primary present.
; The source is always the original 320x240 gameplay chunky frame.  Output is
; selected by ToolType/CLI: 320x240 1x, 640x480 2x nearest-neighbour, or a
; 480-line widescreen stretch where each destination x chooses the nearest
; source x by an integer accumulator.
g2p96_gameplay_draw_mini_frame_160x96
	movem.l	d0-d7/a0-a6,-(a7)
	move.l	a0,a5		; Window
	move.l	chunky,d0
	beq.w	.done
	move.l	d0,a4		; source chunky base
	move	chunkymodw,d5
	beq.w	.done
	tst	p96gameplay_dbuf_active
	beq.s	.window_bitmap
...
.window_bitmap
	...
	move.l	d0,a2
.have_bitmap
	move.l	8(a2),d0	; BitMap.Planes[0] / truecolour base on P96
	beq.w	.done
	move.l	d0,a3		; destination bitmap base
	moveq	#0,d4
	move	0(a2),d4	; BitMap.BytesPerRow
	and.l	#$0000ffff,d4
	move	p96target_width,d0
...
.nextrow
	...
	bra.w	.rowloop
.fps_overlay
	move.l	a3,a0		; c87a: final P96 bitmap origin after Window offsets
	move.l	d4,d0		; BytesPerRow
...
; c86zga: prepare the P96 in-game menu backdrop.  This is not a direct
; P96 rectangle cover: the HUD is removed from the chunky source, then the
; normal confirmed P96 gameplay-copy path redraws the paused frame.  The chunky
; top strip is restored immediately afterwards so the real gameplay state is not
; changed by the menu.
g2p96_menu_prepare_backdrop_nohud
	movem.l	d0,-(a7)
	cmp	#P96DSP_MENU_BRIDGE,p96display_state
	bne.w	.done
	tst	p96gameplay_persist_active
	beq.w	.done
	jsr	g2hud_save_top_chunky
	jsr	g2p96_menu_clear_hud_source_regions
	jsr	g2p96_gameplay_persistent_update
	clr	p96gameplay_skip_aga_present
	jsr	g2hud_restore_top_chunky
.done
...
; c86zga: clear only the top-left and top-right HUD source areas in chunky.
; This lets the existing P96 scaler handle 320/640/stretch output consistently,
; instead of drawing target-side black rectangles with mode-specific geometry.
g2p96_menu_clear_hud_source_regions
	movem.l	d0-d7/a0-a2,-(a7)
	move.l	chunky,d0
	beq.w	.done
	move.l	d0,a2
...
.row_loop
	...
	move.l	a2,a0
	adda.l	d0,a0
	; left HUD/source text area, 96 chunky pixels = 24 longwords
	move.l	a0,a1
	moveq	#23,d7
.left_loop
	clr.l	(a1)+
	dbf	d7,.left_loop
	; right HUD/lives area, x=232..319, 88 chunky pixels = 22 longwords
	move.l	a0,a1
	adda.w	#232,a1
	moveq	#21,d7
.right_loop
...
; -----------------------------------------------------------------------------
; c86zjh: fast real-Bigfont menu delta path for initial menu composition.
;
; initmenu has already rendered the exact original bigfont.bin/bigfont2.bin text
; into menubmap and saved the untouched background of every row in menustrips.
; Older P96 bridges re-read individual planar pixels and recalculated all plane
; addresses repeatedly.  This path expands the current planar row and its saved
; background row once to two linear 320-byte index rows, compares them linearly,
; and writes only changed glyph pixels straight into the existing offscreen P96
; menu-page batch.  Blink/navigation updates deliberately keep their proven row
; path; only the expensive first title/in-game menu build is replaced here.
; -----------------------------------------------------------------------------
p96menu_fast_log_state	dc	0
p96menu_fast_log_name	dc.b	'RAM:gloom_p96_menufast.txt',0
p96menu_fast_log_msg	dc.b	'P96 menu: fast planar row delta active',10
p96menu_fast_log_msg_len	equ	*-p96menu_fast_log_msg
	even
p96menu_fast_strip	dc.l	0
p96menu_fast_rowoff	dc	0
p96menu_fast_plane_size	dc.l	0
p96menu_fast_plane_bytes	ds.b	8
p96menu_fast_bg_indexrow	ds.b	320
	even
p96menu_fast_row_lut	dcb.w	256,0
	even

; curropt selects both the composed source row and its saved pre-text strip.
; Requires an active p96menu_batch_* cache prepared by the existing menu/title
; batch begin routines.
g2p96_menu_fast_delta_current_row_to_batch
	movem.l	d0-d7/a0-a6,-(a7)
	...
	move.l	4(a0,d0*8),d1
	beq.w	.done
	move.l	d1,p96menu_fast_strip
	move	fonth,d0
	mulu	#40,d0
	move.l	d0,p96menu_fast_plane_size
	jsr	g2p96_menu_fast_build_row_lut
	jsr	g2p96_menu_fast_log_once

	move	curropt,d6
	move	fonth,d0
	mulu	d0,d6
	add	menuy,d6
	move	d6,p96menu_bridge_y_start
	move	d6,d0
	add	fonth,d0
	move	d0,p96menu_bridge_y_end
	clr	p96menu_fast_rowoff
.row_loop
	move	p96menu_fast_rowoff,d5
	cmp	fonth,d5
	bge.w	.done
	move	p96menu_bridge_y_start,d6
	add	d5,d6
	cmp	#0,d6
	blt.w	.next_row
	cmp	#240,d6
	bge.w	.done
	; Expand the composed menu row to p96static_indexrow (a6).
	jsr	g2p96_static_decode_index_row_fast
	; Expand the matching saved pre-text row to p96menu_fast_bg_indexrow (a3).
	jsr	g2p96_menu_fast_decode_bg_row
	; Compare both linear rows and modify only glyph pixels in the RAM batch.
	jsr	g2p96_menu_fast_blit_delta_row_to_batch
.next_row
	addq	#1,p96menu_fast_rowoff
	bra.w	.row_loop
.done
	movem.l	(a7)+,d0-d7/a0-a6
	rts

; Build one exact menu-row LUT.  Normal/title rows use the current real font
; palette unchanged.  The two disabled in-game rows retain the established
; neutral greys from g2p96_menu_pixel_rgb565.
g2p96_menu_fast_build_row_lut
	movem.l	d0-d2/a0-a1,-(a7)
	lea	p96menu_rgb565_lut,a0
	lea	p96menu_fast_row_lut,a1
	move	#127,d0
.copy_lut
	move.l	(a0)+,(a1)+
	dbf	d0,.copy_lut
...
.disabled
	lea	p96menu_fast_row_lut,a1
	move	#$eb5a,2(a1)	; palette index 1: dark grey
	move	#$718c,4(a1)	; palette index 2: mid grey
	move	#$f7bd,6(a1)	; palette index 3: light grey
.done
	movem.l	(a7)+,d0-d2/a0-a1
	rts

; Decode one saved menustrip row.  The strip is plane-major: every plane owns
; fonth*40 bytes, while one row inside a plane is exactly 40 bytes.
; input: d5=row offset 0..fonth-1, p96menu_fast_strip/plane_size set
; output: a3=p96menu_fast_bg_indexrow
g2p96_menu_fast_decode_bg_row
	movem.l	d0-d7/a0-a2/a4-a6,-(a7)
	lea	p96menu_fast_bg_indexrow,a3
	move.l	p96menu_fast_strip,d0
	beq.w	.black_row
	move.l	d0,a0
	move	d5,d0
	mulu	#40,d0
...
.byte_loop
	move.l	a0,a1
	lea	p96menu_fast_plane_bytes,a2
	move	bitplanes,d4
	beq.w	.black_row
	subq	#1,d4
.load_planes
	move.b	(a1),(a2)+
	adda.l	p96menu_fast_plane_size,a1
	dbf	d4,.load_planes
	moveq	#7,d3
.bit_loop
	moveq	#0,d1
...
; a6=current 320-byte source-index row, a3=background-index row,
; d6=absolute source y.  Writes directly into the active menu batch cache.
g2p96_menu_fast_blit_delta_row_to_batch
	movem.l	d0-d7/a0-a5,-(a7)
...
.have_y
	...
	bge.w	.done
	move	d1,d0
	mulu	p96menu_batch_rowbytes,d0
	adda.l	d0,a1
	move.l	a1,a2
	cmp	#1,d2
	beq.s	.add_second_row
	cmp	#2,d2
	beq.s	.add_second_row
	bra.s	.dispatch
.add_second_row
	moveq	#0,d0
	move	p96menu_batch_rowbytes,d0
	adda.l	d0,a2
.dispatch
	lea	p96menu_fast_row_lut,a5
	cmp	#0,d2
	beq.w	.mode_1x
	cmp	#1,d2
	beq.w	.mode_2x
...
g2p96_menu_fast_log_once
	movem.l	d0-d3/d7/a0/a6,-(a7)
	tst	p96menu_fast_log_state
	bne.s	.done
	move	#-1,p96menu_fast_log_state
	move.l	dosbase,d0
	beq.s	.done
	move.l	d0,a6
	lea	p96menu_fast_log_name,a0
	move.l	a0,d1
	move.l	#1006,d2
	jsr	-30(a6)
	move.l	d0,d7
	beq.s	.done
	move.l	d7,d1
	lea	p96menu_fast_log_msg,a0
	move.l	a0,d2
	move.l	#p96menu_fast_log_msg_len,d3
	jsr	-48(a6)
	move.l	d7,d1
	jsr	-36(a6)
.done
...
; c86zgl: native P96 in-game menu renderer.
; Keep the c86zgi fast P96 menu lifecycle, but do not use the temporary
; chatfont renderer.  The original game renders real bigfont.bin/bigfont2.bin
; into menubmap; the P96 bridge scans only the changed glyph pixels versus the
; saved menustrip background and writes those pixels into an offscreen P96 cache.
g2p96_menu_bridge_all_rows_present
	jsr	g2p96_menu_glyph_mode_active
	tst	d0
	beq.w	.g2c86zjj_planar_fallback
	movem.l	d0-d7,-(a7)
	move	curropt,-(a7)
	jsr	g2p96_menu_build_rgb565_lut
	jsr	g2p96_menu_batch_begin
	clr	p96menu_native_selected
	moveq	#0,d7
...
.g2c86zjj_native_flush
	jsr	g2p96_menu_batch_flush
	jsr	g2p96_menu_batch_end
	move	(a7)+,curropt
	movem.l	(a7)+,d0-d7
	rts
.g2c86zjj_planar_fallback
	movem.l	d0-d7,-(a7)
	move	curropt,-(a7)
	; c86zgi: draw all rows through the compact native glyph path.  c86zgh
	; solved the visible row-by-row build by batching, but still spent about two
	; seconds scanning the planar menu bitmap pixel-by-pixel before the final
	; flush.  This path writes only real glyph pixels into the batch cache.
	jsr	g2p96_menu_build_rgb565_lut	;c86zjh: build exact font/menu colours once for the whole batch
	jsr	g2p96_menu_batch_begin
	clr	p96menu_native_selected
	moveq	#0,d7
.loop
	...
	bge.w	.flush
	move	d7,curropt
	jsr	g2p96_menu_fast_delta_current_row_to_batch	;c86zjh: row-wise source/background decode, direct RAM batch delta
	addq	#1,d7
	bra.w	.loop
.flush
	jsr	g2p96_menu_batch_flush
...
g2p96_menu_bridge_current_row_present
	movem.l	d0-d1,-(a7)
	; c86zhj: opton/optoff is also used by DISPLAY=AGA.  Do not touch
	; menubmap/row caches unless a real P96 bridge window is active; otherwise
	; the original AGA blink collapses into a tiny flicker.
	jsr	g2p96_display_is_p96_capable
	tst	d0
	beq	.done
	tst	p96gameplay_persist_active
	...
	move.l	p96winprobe_window_ptr,d0
	beq	.done
	cmp	#P96DSP_TITLE_BRIDGE,p96display_state	;c86zgy: title menu row refresh uses staged title background
	bne	.not_title
	jsr	g2p96_title_menu_bridge_current_row_present
	bra	.done
.not_title
	move	curropt,d0
	move	fonth,d1
	...
	jsr	g2p96_menu_glyph_mode_active
	tst	d0
	bne	.g2c86zjj_row_native
	jsr	g2p96_menu_recompose_current_row_planar	;c86zgl fallback
.g2c86zjj_row_native
	jsr	g2p96_menu_restore_current_row_best
	jsr	g2p96_menu_draw_current_row_best_no_restore
.done
...
; but limited to p96menu_bridge_y_start..end so menu blink stays cheap.
...
; c86zgi: fast native P96 menu text renderer.
; This intentionally avoids the slow c86zge planar-menubmap scanner for normal
; P96 menu presentation.  It draws compact 8x10 glyphs from chatfont directly
; into the active row cache or full-menu batch cache.  That means the menu is
; responsive on real A1200 RTG hardware, while the older exact planar scanner is
; kept below as a rollback/reference routine.
g2p96_menu_native_draw_current_row_fasttext_no_restore
	movem.l	d0-d7/a0-a6,-(a7)
	cmp	#P96DSP_MENU_BRIDGE,p96display_state
	bne.w	.done
	tst	p96gameplay_persist_active
	...
	move	0(a2),d4
	and.l	#$0000ffff,d4
	move.l	d4,p96menu_native_bpr
	moveq	#0,d0
	move	6(a5),d0
	mulu	d4,d0
	add.l	d0,p96menu_native_dstbase
	moveq	#0,d0
	move	4(a5),d0
	add.l	d0,d0
	add.l	d0,p96menu_native_dstbase

	; Current row Y range in source coordinates.
	move	curropt,d6
	move	fonth,d0
	mulu	d0,d6
	add	menuy,d6
	...
	; Fetch the original menu text pointer for this row.
	move	curropt,d0
	lea	menustrips,a0
	move.l	0(a0,d0*8),a4
	move.l	a4,d0
	...
	; Calculate the same centred X position as printmess2.
	move.l	a4,a0
	moveq	#-1,d3
.len_loop
	addq	#1,d3
...
.x_ok
	...
	jsr	g2p96_menu_cache_begin
	move	#-1,p96menu_cache_active
	jsr	g2p96_menu_native_pick_color
...
.char_loop
	...
	cmp.b	#"'",d0
	beq.s	.advance
	jsr	g2p96_menu_native_calcchar
	cmp	#-1,d0
	beq.s	.advance
	move	d0,d2
	jsr	g2p96_menu_native_draw_chatglyph
.advance
	add	fontw,d7
	bra.s	.char_loop
.draw_done
	clr	p96menu_cache_active
	jsr	g2p96_menu_cache_flush
.done
	clr	p96menu_cache_active
	movem.l	(a7)+,d0-d7/a0-a6
	rts
...
; c86zgl: rebuild the current menu row in the original planar menubmap using
; the already loaded real font (bigfont.bin/bigfont2.bin through font/bigfont_).
; This is used for P96 row refreshes after optoff/opton so the P96 bridge can
; sample the true Bigfont output instead of the temporary chatfont renderer.
g2p96_menu_recompose_current_row_planar
	movem.l	d0-d7/a0-a6,-(a7)
	cmp	#P96DSP_MENU_BRIDGE,p96display_state
	beq	.state_ok
	cmp	#P96DSP_TITLE_BRIDGE,p96display_state	;c86zgy: title row refresh also recomposes from real bigfont
	bne.w	.done
.state_ok
	move.l	menubmap,d0
	beq.w	.done
	; Restore the saved background strip for this row first.
	move	curropt,d6
	lea	menustrips,a0
	move.l	4(a0,d6*8),a0	; saved strip/background
	move.l	a0,d0
	beq	.skip_restore
	mulu	fonth,d6
	add	menuy,d6
...
.bwait	btst	#6,$dff002
	...
	dbf	d1,.restore_loop
.skip_restore
	; Draw the current menu text with the original printmess2/font path.
	move	curropt,d6
	lea	menustrips,a0
	move.l	0(a0,d6*8),a4	;text pointer
	move.l	a4,d0
	beq.w	.done
	jsr	g2v190hx_level_menu_a4
	move.l	a4,a0
...
.len_loop
	...
	mulu	fonth,d6
	add	menuy,d6
	move.l	menuwindow,a6
	jsr	printmess2	; real bigfont.bin/bigfont2.bin renderer
.done
	movem.l	(a7)+,d0-d7/a0-a6
	rts

; c86zgl: fast enough real-Bigfont P96 row renderer.
; It does NOT parse chatfont and does NOT guess the shape format.  The original
; game has already drawn the row with bigfont.bin/bigfont2.bin into menubmap.
; This routine compares that row against the saved menustrip background byte by
; byte across bitplanes, and emits only changed/glyph pixels into the P96 row or
; full-menu batch cache.
g2p96_menu_native_draw_current_row_bigfont_delta_no_restore
	movem.l	d0-d7/a0-a6,-(a7)
	cmp	#P96DSP_MENU_BRIDGE,p96display_state
	bne.w	.done
	tst	p96gameplay_persist_active
	...
	move.l	menubmap,d0
	beq.w	.done
	move.l	d0,p96menu_delta_menubmap
	move	curropt,d0
	lea	menustrips,a0
	move.l	4(a0,d0*8),d1
	beq.w	.done
	move.l	d1,p96menu_delta_strip
	move.l	p96winprobe_window_ptr,a5
	move.l	50(a5),d0
	beq.w	.done
	move.l	d0,a1
	...
	; background strip plane size = fonth * 40 bytes
	move	fonth,d0
	mulu	#40,d0
	move.l	d0,p96menu_delta_bg_planesize

	jsr	g2p96_menu_cache_begin
	move	#-1,p96menu_cache_active
	lea	p96menu_rgb565_lut,a5
	clr	p96menu_delta_rowoff
.row_loop
	move	p96menu_delta_rowoff,d5
	cmp	fonth,d5
	bge.w	.scan_done
	move	p96menu_bridge_y_start,d6
	add	d5,d6
	cmp	#240,d6
	bge.w	.scan_done
	move	d6,p96menu_delta_source_y
	; src row base = menubmap + source_y * linemodw
	move.l	p96menu_delta_menubmap,a0
	move	d6,d0
	mulu	linemodw,d0
	adda.l	d0,a0
	move.l	a0,p96menu_delta_srcrow
	; background row base = strip + rowoff * 40
	move.l	p96menu_delta_strip,a1
	move	d5,d0
	mulu	#40,d0
	adda.l	d0,a1
	move.l	a1,p96menu_delta_bgrow
	clr	p96menu_delta_xbyte
.byte_loop
	move	p96menu_delta_xbyte,d7
	cmp	#40,d7
	bge.w	.next_row
	; Build diff mask for 8 pixels in this byte across all active bitplanes.
	move.l	p96menu_delta_srcrow,a0
	adda.w	d7,a0
	move.l	p96menu_delta_bgrow,a1
	adda.w	d7,a1
	moveq	#0,d3	; diff mask byte
	move	bitplanes,d4
	beq.w	.next_byte
	subq	#1,d4
.diff_plane_loop
	...
	or.b	d0,d3
	adda.l	bpmod,a0
	adda.l	p96menu_delta_bg_planesize,a1
	dbf	d4,.diff_plane_loop
	and	#$00ff,d3
	move	d3,p96menu_delta_diffmask
	beq.w	.next_byte
	clr	p96menu_delta_bitidx
.bit_loop
	move	p96menu_delta_bitidx,d0
	cmp	#8,d0
	bge.w	.next_byte
	moveq	#7,d2
	sub	d0,d2	; bit number inside planar byte
	move	p96menu_delta_diffmask,d3
	btst	d2,d3
	beq.w	.next_bit
	; Build current planar colour index for this changed pixel.
	moveq	#0,d1	; palette index
	moveq	#0,d4	; plane index
	move.l	p96menu_delta_srcrow,a0
	move	p96menu_delta_xbyte,d0
	adda.w	d0,a0
.plane_index_loop
	move.b	(a0),d3
	btst	d2,d3
...
.no_plane_bit
	...
	addq	#1,d4
	cmp	bitplanes,d4
	blt.s	.plane_index_loop
	move	p96menu_delta_source_y,d6
	jsr	g2p96_menu_pixel_rgb565	; d1 index -> d2 RGB565
	move	p96menu_delta_xbyte,d0
	lsl	#3,d0
	add	p96menu_delta_bitidx,d0
	move	d0,p96menu_native_px
	move	p96menu_delta_source_y,p96menu_native_py
	move	d2,p96menu_native_colour
	jsr	g2p96_menu_native_put_pixel
.next_bit
	addq	#1,p96menu_delta_bitidx
	bra.w	.bit_loop
.next_byte
	addq	#1,p96menu_delta_xbyte
	bra.w	.byte_loop
.next_row
	addq	#1,p96menu_delta_rowoff
	bra.w	.row_loop
.scan_done
	clr	p96menu_cache_active
	jsr	g2p96_menu_cache_flush
...
p96menu_delta_menubmap	dc.l	0
p96menu_delta_strip	dc.l	0
p96menu_delta_srcrow	dc.l	0
p96menu_delta_bgrow	dc.l	0
p96menu_delta_bg_planesize	dc.l	0
p96menu_delta_source_y	dc	0
p96menu_delta_rowoff	dc	0
p96menu_delta_xbyte	dc	0
p96menu_delta_bitidx	dc	0
p96menu_delta_diffmask	dc	0
	even

; c86zge: draw current menu row as native RGB565 bigfont2 text.
; Instead of the temporary chatfont renderer from c86zgd, this reads only the
; actual changed glyph pixels from the already composed original menu bitmap.
; The important difference versus the rejected bridge is that we scan only the
; text bounding box, never the full 320-pixel row/background.  This preserves
; the real bigfont2 look while keeping the P96 path compact enough for real RTG
; hardware.
g2p96_menu_native_draw_current_row_no_restore
	movem.l	d0-d7/a0-a6,-(a7)
	cmp	#P96DSP_MENU_BRIDGE,p96display_state
	beq	.state_ok
	cmp	#P96DSP_TITLE_BRIDGE,p96display_state	;c86zgy: reuse bounded text scan for title menu
	bne.w	.done
.state_ok
	tst	p96gameplay_persist_active
	beq.w	.done
	...
	move.l	menubmap,d0
	beq.w	.done
	move.l	d0,a4		; source planar menu bitmap containing real bigfont2
	move.l	p96winprobe_window_ptr,a5
	move.l	50(a5),d0
	beq.w	.done
	move.l	d0,a1
	...
	; Determine source row range for current menu item.
	move	curropt,d6
	move	fonth,d0
	mulu	d0,d6
	add	menuy,d6
	...
	; Determine exact text bounds using the same centering rule as printmess2.
	move	curropt,d0
	lea	menustrips,a0
	move.l	0(a0,d0*8),a0
	move.l	a0,d0
...
.len_done
	...
	move	fontw,d5
	move	d3,d0
	mulu	d5,d0		; full text pixel width
	move	d0,d2
	lsr	#1,d0		; half width for centred x
	move	#160,d7
	sub	d0,d7
	bpl.s	.x_start_ok
	moveq	#0,d7
.x_start_ok
	move	d7,p96menu_native_x_start
	add	d2,d7
	cmp	#320,d7
	ble.s	.x_end_ok
	move	#320,d7
.x_end_ok
	move	d7,p96menu_native_x_end

	; c86zhn: safe P96 level-select fix.  Do not call the dynamic
	; level-menu helper here (that corrupts the title text pointer state).
	; When the title menu row 0 is selected, AGA may already have drawn the
	; dynamic "MAP x.x" suffix into menubmap, so just widen the P96 scan
	; bounds for this row and let the existing glyph-vs-background delta
	; scanner pick up the extra pixels.  All other rows keep the tight
	; text bounds from the original menu string.
	cmp	#P96DSP_TITLE_BRIDGE,p96display_state
	bne.s	.c86zhn_bounds_done
	tst	curropt
	bne.s	.c86zhn_bounds_done
	clr	p96menu_native_x_start
	move	#320,p96menu_native_x_end
.c86zhn_bounds_done

	; Force glyph-only mode while sampling menubmap/menustrip pixels.  Equal
	; pixels are transparent and are skipped by this routine, so the paused P96
	; gameplay frame remains visible behind the menu text.
	; c86zgf: build into an offscreen RGB565 row cache first, then
	; flush the completed rows to visible RTG memory.  This keeps the
	; real bigfont2 look from c86zge but avoids visible pixel-by-pixel
	; drawing on A1200 RTG hardware.
	jsr	g2p96_menu_cache_begin
	move	p96menu_bridge_glyph_only,-(a7)
	move	#-1,p96menu_bridge_glyph_only
	move	#-1,p96menu_cache_active
	lea	p96menu_rgb565_lut,a5
	move	p96menu_bridge_y_start,d6
.y_loop
	...
	cmp	#240,d6
	bge.w	.scan_done
	move	p96menu_native_x_start,d7
.x_loop
	cmp	p96menu_native_x_end,d7
	bge.s	.next_y
	jsr	g2p96_menu_source_pixel_rgb565
	tst	p96menu_pixel_is_background
	bne.s	.skip_pixel
...
.scan_done
	clr	p96menu_cache_active
	jsr	g2p96_menu_cache_flush
	move	(a7)+,p96menu_bridge_glyph_only
.done
	clr	p96menu_cache_active
	movem.l	(a7)+,d0-d7/a0-a6
	rts

p96menu_native_x_start	dc	0
p96menu_native_x_end	dc	0
	even

; c86zgd: P96 native menu colours.  Rows VIEW SIZE and FULL SCREEN stay visible
; and selectable under P96 but are disabled/no-op, so draw them in neutral grey.
g2p96_menu_native_pick_color
	move	curropt,d0
	cmp	#2,d0
	beq.s	.disabled
	cmp	#3,d0
	beq.s	.disabled
	tst	p96menu_native_selected
	beq.s	.normal
	move	#$ffff,d2	; selected: white, byteswap-safe
	rts
.normal
	move	#$e0ff,d2	; normal: yellow RGB565 byteswapped
	rts
.disabled
	tst	p96menu_native_selected
	beq.s	.disabled_normal
	move	#$f7bd,d2	; selected disabled: light grey
	rts
.disabled_normal
	move	#$1084,d2	; disabled normal: dark grey
	rts

; c86zgd: map menu text character to the same compact 40-glyph chatfont range.
g2p96_menu_native_calcchar
	ext	d0
	cmp	#'0',d0
	bcs.s	.notnum
	cmp	#'9',d0
...
; c86zgd: draw one 8x10 source-coordinate glyph from chatfont into the active P96
; target mode.  Each of the 5 chatfont rows is doubled vertically to match the
; normal ingame menu font height.
g2p96_menu_native_draw_chatglyph
	movem.l	d0-d7/a0-a4,-(a7)
	lea	chatfont,a0
	adda.w	d2,a0
	moveq	#0,d5	; glyph source row 0..4
.glyph_row
	cmp	#5,d5
	bge.w	.done
	move.b	(a0),d3
	beq.w	.next_glyph_row
	moveq	#0,d4	; vertical duplicate 0..1
.vdup_loop
	move	d5,d1
	add	d1,d1
	add	d4,d1
...
.bit_loop
	...
	cmp	#320,d2
	bge.s	.no_pixel
	move	d2,p96menu_native_px
	move	d1,p96menu_native_py
	jsr	g2p96_menu_native_put_pixel
.no_pixel
	addq	#1,d0
	bra.s	.bit_loop
...
g2p96_menu_native_put_pixel
	...
	jsr	g2p96_menu_native_put_pixel_batch
	bra.w	.done
.check_row_cache
	tst	p96menu_cache_active
	beq.s	.visible
	jsr	g2p96_menu_native_put_pixel_cache
	bra.w	.done
.visible
...
; c86zgf: cache one menu text row offscreen before presenting it.  The cache is
; a full-width RGB565 slice of the current visible P96 frame for the active menu
; row.  Bigfont2 glyph pixels are written into this RAM cache, then the completed
; slice is copied sequentially to the visible RTG bitmap.  This trades slow
; visible random pixel writes for fast linear row copies on real A1200 hardware.
g2p96_menu_cache_begin
	movem.l	d0-d7/a0-a2,-(a7)
	clr	p96menu_cache_active
	; c86zgh: during full-menu batching, row-cache begin/flush are intentionally
	; inert.  Pixel writes are redirected by g2p96_menu_native_put_pixel.
	tst	p96menu_batch_active
	bne.w	.done
	move	p96target_width,d0
	beq.w	.done
	add	d0,d0
	move	d0,p96menu_cache_rowbytes
	move	p96target_mode,d1
	move	p96menu_bridge_y_start,d2
	cmp	#1,d1
	beq.s	.double_y
	cmp	#2,d1
	beq.s	.double_y
...
g2p96_menu_cache_flush
	...
	move.l	p96menu_native_dstbase,a1
	moveq	#0,d0
	move	p96menu_cache_dsty,d0
	move.l	p96menu_native_bpr,d4
	mulu	d4,d0
	adda.l	d0,a1
	lea	p96menu_row_cache,a0
	move	p96menu_cache_height,d7
	beq.w	.done
	subq	#1,d7
.row_loop
	move	p96target_width,d6
...
; c86zgh: full-menu batch cache.  This cache covers only the active menu text
; region, not the full screen.  It is lazily allocated on first P96 menu use and
; tracked by the normal memlist via allocmem_, so exit cleanup remains unchanged.
; The region is copied from visible RTG memory into RAM, all glyph pixels are
; rendered into RAM, then the finished region is copied back linearly once.
g2p96_menu_batch_begin
	movem.l	d0-d7/a0-a6,-(a7)
	...
	move.l	p96menu_page_cache_ptr,d0
	bne.s	.have_cache
	move.l	#900000,d0	; enough for 854x480x2 plus safety margin
	moveq	#1,d1		; public/fast if available, same style as existing buffers
	lea	p96menu_page_cache_name,a0
	jsr	allocmem_
	move.l	d0,p96menu_page_cache_ptr
	beq.w	.done
.have_cache
	move.l	p96winprobe_window_ptr,a3
	move.l	50(a3),d0
	beq.w	.done
	move.l	d0,a1
	move.l	4(a1),d0
	...
	move.l	8(a2),d0
	beq.w	.done
	move.l	d0,p96menu_batch_dstbase
	moveq	#0,d4
	move	0(a2),d4
	and.l	#$0000ffff,d4
	...
	beq.w	.done
	add	d0,d0
	move	d0,p96menu_batch_rowbytes
	moveq	#0,d0
	move	6(a3),d0
	mulu	d4,d0
	add.l	d0,p96menu_batch_dstbase
	moveq	#0,d0
	move	4(a3),d0
	add.l	d0,d0
	add.l	d0,p96menu_batch_dstbase

	; Source-coordinate bounds: from first menu row to one row past last option.
	move	menuy,d0
	bpl.s	.y0_ok
	moveq	#0,d0
.y0_ok
...
.have_dest_y
	...
	; Copy the current visible menu region into the RAM cache.
	move.l	p96menu_batch_dstbase,a1
	moveq	#0,d0
	move	p96menu_batch_dst_y0,d0
	move.l	p96menu_batch_bpr,d4
	mulu	d4,d0
	adda.l	d0,a1
	move.l	p96menu_page_cache_ptr,a0
	move	p96menu_batch_height,d7
	subq	#1,d7
.copy_row
	move	p96target_width,d6
	subq	#1,d6
...
; c86zga: P96 in-game menu bridge without grey/dim background.
; For each menu row pixel:
;   - if current menubmap pixel equals the saved strip background, restore the
;     gameplay pixel underneath;
;   - otherwise write the menu glyph pixel (WINDOW SIZE/FULL SCREEN greys kept).
; This removes the rejected dim-panel/black-rectangle experiments while still
; making opton/optoff row updates erase correctly on P96.
g2p96_menu_bridge_present
	movem.l	d0-d7/a0-a6,-(a7)
	cmp	#P96DSP_MENU_BRIDGE,p96display_state
	bne.w	.done
	tst	p96gameplay_persist_active
	...
	move.l	menubmap,d0
	beq.w	.done
	move.l	d0,a4		; source planar menu bitmap
	move.l	lastpal,d0
	bne.s	.have_palette
	move.l	planar_palette,d0
	beq.w	.done
.have_palette
	...
	; Include Window offsets for completeness, although this window is 0,0.
	moveq	#0,d0
	move	6(a5),d0	; Window.TopEdge
	mulu	d4,d0
	adda.l	d0,a3
...
.stretch480_src_loop
	...
	move	d2,-(a7)
	jsr	g2p96_menu_source_pixel_rgb565
	move	d2,d1		; emitted RGB565 colour
	move	p96menu_pixel_is_background,d5
	move	(a7)+,d2	; restore destination-x count
	add	p96target_width,d3
.stretch480_emit
	cmp	p96target_width,d2
	bge.w	.nextrow
...
; c86zga: choose one output pixel for a P96 menu row.  The saved menu strip is
; the original row background captured by initmenu before text was printed.
; Equal current/background pixels mean transparent menu background, so restore
; the corresponding gameplay pixel.  Different pixels are menu glyphs.
; input: a4=menubmap, a5=menu RGB565 LUT, d6=y, d7=x. output: d2=RGB565.
g2p96_menu_source_pixel_rgb565
	movem.l	d0-d1/d3-d5/a0-a2,-(a7)
	jsr	g2p96_menu_planar_pixel
	move	d1,d5		; current menu pixel index
	jsr	g2p96_menu_planar_bg_pixel
	clr	p96menu_pixel_is_background
	cmp	d1,d5
	bne.s	.menu_pixel
	move	#-1,p96menu_pixel_is_background
	tst	p96menu_bridge_glyph_only
	beq.s	.restore_gameplay_pixel
	clr	d2		;c86zgc: glyph-only full menu build: skip background write in caller
	bra.s	.done
.restore_gameplay_pixel
	; background: restore gameplay pixel under this menu row
	move.l	chunky,d0
	beq.s	.black_pixel
	move.l	d0,a0
	move	chunkymodw,d0
...
.done
	movem.l	(a7)+,d0-d1/d3-d5/a0-a2
	rts

; c86zfv/c86zfw: convert one menu palette index to RGB565 for the P96 bridge.
; VIEW SIZE and FULL SCREEN stay visible/selectable in P96, but their glyph
; pixels use neutral greys.  Background is handled separately by c86zga.
; input: d1=palette index, d6=source y, a5=RGB565 LUT. output: d2=RGB565.
g2p96_menu_pixel_rgb565
	movem.l	d0-d1/d3,-(a7)
	tst	game_menu_active	;c86zgz: disabled grey rows apply only to in-game menu, not title/about
	beq.s	.normal
	move	d6,d0
	move	menuy,d3
	move	fonth,d2
	add	d2,d3
	add	d2,d3		; first disabled text row: VIEW SIZE
	cmp	d3,d0
	bcs.s	.normal
	add	d2,d3
	add	d2,d3		; after FULL SCREEN row
	cmp	d3,d0
	bcc.s	.normal
	cmp	#0,d1
	beq.s	.normal
	...
	cmp	#2,d1
	beq.s	.grey2
	move	#$f7bd,d2	; light grey, byteswapped RGB565
	bra.s	.done
.grey2	move	#$718c,d2	; mid grey, byteswapped RGB565
	bra.s	.done
.grey1	move	#$eb5a,d2	; dark grey, byteswapped RGB565
	bra.s	.done
.normal	add	d1,d1
	move	0(a5,d1.w),d2
.done	movem.l	(a7)+,d0-d1/d3
	rts

; c86zfr: read one planar menu pixel from menubmap.
; input: a4 = planar base, d6 = y, d7 = x.  output: d1 = palette index.
g2p96_menu_planar_pixel
	movem.l	d0/d2-d5/a0,-(a7)
	moveq	#0,d1
	move	d6,d0
	mulu	linemodw,d0	; row byte offset
	move	d7,d2
	lsr	#3,d2		; x byte
	add	d2,d0
	moveq	#7,d3
	move	d7,d2
	and	#7,d2
	sub	d2,d3		; bit number within source byte
	moveq	#0,d5		; plane index / palette bit
	move	bitplanes,d4
	beq.s	.done
	subq	#1,d4
.plane_loop
...
; c86zga: read the matching pixel from the saved pre-text menu strip for the
; current menu row.  This lets the P96 bridge tell text pixels from background
; without guessing palette colours.
; input: d6=absolute y, d7=x, curropt=row. output: d1=background palette index.
g2p96_menu_planar_bg_pixel
	movem.l	d0/d2-d5/a0-a2,-(a7)
	moveq	#0,d1
	lea	menustrips,a0
	moveq	#0,d0
	move	curropt,d0
	move.l	4(a0,d0*8),a2	; saved strip address for current row
	move.l	a2,d0
	beq.s	.done
	move	d6,d0
	sub	p96menu_bridge_y_start,d0
	bmi.s	.done
	cmp	fonth,d0
	bcc.s	.done
	mulu	#40,d0		; row byte offset inside strip
	move	d7,d2
	lsr	#3,d2
	add	d2,d0		; byte offset within one plane
	moveq	#7,d3
	move	d7,d2
	and	#7,d2
	sub	d2,d3		; bit number within source byte
...
.plane_loop
	move.l	a2,a0
	move	fonth,d2
	mulu	#40,d2		; bytes per plane in saved strip
	mulu	d5,d2
	adda.l	d2,a0
	adda.l	d0,a0
	btst	d3,(a0)
...
.no_bit
	...
	dbf	d4,.plane_loop
.done
	movem.l	(a7)+,d0/d2-d5/a0-a2
	rts

; c86zfr: direct palette-index -> RGB565 LUT for planar menu bitmaps.
; Unlike gameplay Chunky bytes, menu pixels are not passed through g2_strip_invpal.
...
p96menu_rgb565_lut
	dcb.w	256,0
p96menu_bridge_row_only
	dc	0	;c86zfx/c86zga: nonzero = copy only selected menu row range
p96menu_bridge_glyph_only
	dc	0	;c86zgc: all-row menu open writes only glyph pixels, not background
p96menu_pixel_is_background
	dc	0	;c86zgc: set by source_pixel so caller can skip background writes
p96menu_bridge_y_start
	dc	0
p96menu_bridge_y_end
	dc	0
	even
...
p96static_dstbase		dc.l	0
p96static_dstbpr		dc.l	0
...
; -----------------------------------------------------------------------------
g2p96_title_menu_bridge_current_row_present
	movem.l	d0-d1,-(a7)
	move	curropt,d0
	move	fonth,d1
	mulu	d1,d0
	...
	jsr	g2p96_menu_glyph_mode_active
	tst	d0
	bne	.g2c86zjj_title_native
	jsr	g2p96_menu_recompose_current_row_planar
.g2c86zjj_title_native
	jsr	g2p96_menu_restore_current_row_best
	jsr	g2p96_menu_draw_current_row_best_no_restore
	movem.l	(a7)+,d0-d1
	rts
...
; they only claim success while the P96 title bridge is open and a staged RGB565
; title buffer exists.  AGA keeps the original slow but safe showpic path.
g2p96_title_cache_ready_if_bridge
	move.l	d1,-(a7)
	moveq	#0,d0
	cmp	#P96DSP_TITLE_BRIDGE,p96display_state
	bne.s	.done
...
g2p96_title_skip_redundant_brush_if_bridge
	...
	bra	g2p96_title_cache_ready_if_bridge

g2p96_title_about_fast_background_if_bridge
	; c86zhe: disabled.  c86zhd's ABOUT cache shortcut corrupted ABOUT/title text.
	moveq	#0,d0
	rts

g2p96_title_about_return_fast_if_bridge
	; c86zhe: disabled.  Use the proven clean-title ABOUT return path instead.
	moveq	#0,d0
	rts
...
g2p96_title_qmenu_background_if_bridge
	movem.l	d0-d1/a0-a1,-(a7)
	cmp	#P96DSP_TITLE_BRIDGE,p96display_state
	bne.s	.done
...
.rebuild
	...
	move	#P96DSP_TITLE_BRIDGE,p96display_prev_state
	bra.s	.done
.restore
	jsr	g2p96_title_restore_staged_background_if_bridge
.done
...
g2p96_title_restore_staged_background_if_bridge
	movem.l	d0-d7/a0-a6,-(a7)
	cmp	#P96DSP_TITLE_BRIDGE,p96display_state
	bne	.done
	tst	p96gameplay_persist_active
...
g2p96_static_present_showbitmap_if_intermission_bridge
	movem.l	d0-d1,-(a7)
	cmp	#P96DSP_INTERMISSION_BRIDGE,p96display_state
	bne	.done
	tst	p96gameplay_persist_active
...
g2p96_intermission_typewriter_char_if_bridge
	movem.l	d0-d2,-(a7)
	cmp	#P96DSP_INTERMISSION_BRIDGE,p96display_state
	bne	.done
	tst	p96gameplay_persist_active
	...
	beq	.legacy
	jsr	g2p96_intermission_native_typewriter_char
	bra	.done
.legacy
...
g2p96_static_present_intermission_text_skip_if_bridge
...
g2p96_static_present_title_menu_bigfont_if_bridge
	jsr	g2p96_menu_glyph_mode_active
	tst	d0
	beq.w	.g2c86zjj_title_planar_fallback
	movem.l	d0-d7/a0-a6,-(a7)
	cmp	#P96DSP_TITLE_BRIDGE,p96display_state
	...
	jsr	g2p96_menu_build_rgb565_lut
	jsr	g2p96_static_title_menu_batch_begin
...
.g2c86zjj_title_native_done
	...
	movem.l	(a7)+,d0-d7/a0-a6
	rts
.g2c86zjj_title_planar_fallback
	movem.l	d0-d7/a0-a6,-(a7)
	cmp	#P96DSP_TITLE_BRIDGE,p96display_state
	bne	.done
	tst	p96gameplay_persist_active
...
.row_loop
	...
	bge	.flush
	move	d7,curropt
	jsr	g2p96_menu_fast_delta_current_row_to_batch	;c86zjh: exact Bigfont delta via fast planar row decode
	addq	#1,d7
	bra	.row_loop
.flush
	jsr	g2p96_title_menu_draw_separator_lines_if_needed	;c86zgz: restore AGA dotted title separators
	move	(a7)+,curropt
	jsr	g2p96_menu_batch_flush
	jsr	g2p96_menu_batch_end
.done
...
; c86zgz: P96 title menu dotted separator lines.  The AGA path draws two
; 80px dotted lines in the spacer rows (between TWO PLAYER COMBAT and PLAYER 1,
; and between PLAYER 2 and REMOTE LINK OPTIONS).  The title P96 bridge draws
...
g2p96_title_menu_draw_one_separator_line	; d0 = source Y
	...
	cmp	#240,d6
	bge	.done
	move	#$e0ff,p96menu_native_colour	; yellow RGB565 byteswapped
	move	#120,d7
.loop
	cmp	#200,d7
	bge	.done
...
g2p96_static_title_menu_batch_begin
	movem.l	d0-d7/a0-a6,-(a7)
	clr	p96menu_batch_active
	cmp	#P96DSP_TITLE_BRIDGE,p96display_state
	bne	.done
...
g2p96_static_title_menu_draw_current_row_delta
	movem.l	d0-d7/a0-a6,-(a7)
	move.l	menubmap,d0
	beq	.done
	move.l	d0,p96menu_delta_menubmap
...
g2p96_static_open_bridge_screen
...
g2p96_static_present_showbitmap_plain_if_bridge
	clr	p96static_palette_mode
	bra	g2p96_static_present_showbitmap_if_bridge

g2p96_static_present_showbitmap_font_if_bridge
	move	#-1,p96static_palette_mode
	bra	g2p96_static_present_showbitmap_if_bridge

g2p96_static_present_showbitmap_if_bridge
	movem.l	d0-d1,-(a7)
	cmp	#P96DSP_TITLE_BRIDGE,p96display_state
	beq	.present
	cmp	#P96DSP_INTERMISSION_BRIDGE,p96display_state
...
g2p96_static_present_showbitmap
	...
	move.l	showbitmap,d0
	beq	.done
	move.l	d0,a4		; planar source
	move.l	p96static_rgbbufptr,d0
	beq	.done
	move.l	d0,a3		; staged RGB565 target buffer
	move.l	p96winprobe_window_ptr,d0
	beq	.done
	move.l	d0,a5		; Window
	move.l	50(a5),d0	; Window.RPort
	...
	move.l	8(a2),d0	; BitMap.Planes[0] / truecolour base on P96
	beq	.done
	move.l	d0,p96static_dstbase
	moveq	#0,d4
	move	0(a2),d4	; BitMap.BytesPerRow
	and.l	#$0000ffff,d4
	move.l	d4,p96static_dstbpr
	moveq	#0,d0
	move	p96target_width,d0
	add.l	d0,d0
...
.have_lut
	...
	jsr	g2p96_static_copy_rgb_buffer_to_p96
.done
	movem.l	(a7)+,d0-d7/a0-a6
	rts
...
g2p96_static_build_rgb_buffer
	movem.l	d0-d7/a0-a6,-(a7)
	move.l	showbitmap,a4
	move.l	p96static_rgbbufptr,a3
	lea	p96static_rgb565_lut,a5
	jsr	g2p96_static_row_source_log_once	;c86zjl: log direct composition or planar fallback accurately
	moveq	#0,d6		; source y 0..239
.rowloop
	cmp	#240,d6
	bge	.done
...
.low_y
	...
	bra	.dispatch
.add_row2
	adda.l	d4,a0		; dest row 2
.dispatch
	jsr	g2p96_static_decode_index_row_best	;c86zjl: direct IFF composition row, planar fallback
	move	p96target_mode,d0
	beq	.copy_1x
	cmp	#1,d0
	beq	.copy_2x
...
g2p96_static_copy_rgb_buffer_to_p96
	...
	beq	.done
	move.l	d0,a3
	move.l	p96static_dstbase,d0
	beq	.done
	move.l	d0,a4
	move.l	p96winprobe_window_ptr,d0
	beq	.done
	move.l	d0,a5
	move.l	p96static_dstbpr,d4
	move.l	p96static_rgbbpr,d5
	moveq	#0,d0
	move	6(a5),d0	; Window.TopEdge
	mulu	d4,d0
	adda.l	d0,a4
...
.rowloop
	...
	adda.l	d0,a1
	move	p96target_width,d7
	lsr	#1,d7		; two 16-bit pixels per long
	subq	#1,d7
.copyloop
	move.l	(a0)+,(a1)+
	dbf	d7,.copyloop
...
; input: a4=planar source, d6=y, d7=x. output: d1=palette index.
g2p96_static_planar_pixel
	movem.l	d0/d2-d5/a0,-(a7)
	moveq	#0,d1
	move	d6,d0
	mulu	linemodw,d0
...
; c86zey: build source-byte -> RGB565 LUT using the old c86zds logic.
; Chunky gameplay bytes are paladjust/C2P-scrambled, therefore each source
; byte is first mapped through g2_strip_invpal before reading planar_palette.
...
.paladdr
	...
	bsr	g2p96_gameplay_rgb12_pair_to_rgb565_aga8
	bra.s	.store_lut_word
.ecs_convert_high_only
	bsr	g2p96_gameplay_rgb12_to_rgb565_c86zds
.store_lut_word
	move	d7,d6
	add	d6,d6
	move	d1,0(a1,d6.w)
...
p96gameplay_rgb565_normal
	dc.w	$0000,$f800,$07e0,$001f,$ffe0,$f81f,$07ff,$ffff
p96gameplay_rgb565_swapped
	dc.w	$0000,$00f8,$e007,$1f00,$e0ff,$1ff8,$ff07,$ffff
	even

; Draw 8 fixed colour/pen stripes from the same 64-entry BestPen map used by
; the mini-frame.  This is a visual calibration aid for the first colour probe.
g2p96_gameplay_draw_pen64_ramp
	push
	tst	p96gameplay_pen64_state
	beq	.done
	move.l	p96gameplay_grbase,d0
	beq	.done
	lea	p96gameplay_pen64_map,a4
	moveq	#0,d7
	move	#0,d4
.ramploop
	cmp	#8,d7
	bge	.done
	move	d7,d3
	mulu	#9,d3	;0,9,18,27,36,45,54,63
	moveq	#0,d0
	move.b	0(a4,d3.w),d0
	move.l	50(a5),a1
	move.l	p96gameplay_grbase,a6
	jsr	-342(a6)	;SetAPen ramp colour
	move.l	50(a5),a1
	move.l	p96gameplay_grbase,a6
	move	d4,d0
	move	#8,d1
	...
	add	#39,d2
	move	#15,d3
	jsr	-306(a6)	;40px wide ramp stripe
	add	#40,d4
	addq	#1,d7
	bra	.ramploop
.done
...
; Build a small 64-entry RGB222 -> screen Pen map on the freshly opened P96
; custom screen/window.  This keeps the test conservative: still SetAPen/RectFill,
; no p96WritePixelArray and no AGA-present skip.
g2p96_gameplay_init_pen64_map
	push
	clr	p96gameplay_pen64_state
	move.l	p96gameplay_intbase,d0
	beq	.done
	move.l	d0,a6
	move.l	p96winprobe_window_ptr,d0
	...
	beq	.done
	move.l	d0,a6
	lea	p96gameplay_pen64_map,a2
	moveq	#0,d7
.maploop
	cmp	#64,d7
	bge	.mapdone

	; RGB222 candidate: bits 0..1 red, 2..3 green, 4..5 blue.
	move	d7,d5
	and	#3,d5
	lea	p96gameplay_pen64_tab2,a3
	moveq	#0,d0
	move.b	0(a3,d5.w),d0
	jsr	g2p96_c71_byte_to_rgb32
	move.l	d0,d1	;R 32-bit
...
.next
	...
	bra	.maploop
.mapdone
	move	#-1,p96gameplay_pen64_state
.done
	pull
	rts

g2p96_gameplay_tiny_wait
	push
	move	#49,d7	;about 1 second PAL wait between bounded mini-frame updates
.wait
	move.l	p96gameplay_grbase,d0
	beq	.done
	move.l	d0,a6
...
g2p96_gameplay_present_log
	push
	move.l	dosbase,d0
	beq	.done
	move.l	d0,a6
	lea	p96gameplay_log_name,a0
	move.l	a0,d1
	move.l	#1006,d2	;MODE_NEWFILE
	jsr	-30(a6)	;Open
	move.l	d0,d7
	beq	.done
	move	p96gameplay_state,d0
	cmp	#1,d0
	beq	.ok
	cmp	#2,d0
	beq	.skipaga
	...
	cmp	#9,d0
	beq	.skipnobitmap
	lea	p96gameplay_msg_fail,a0
	move.l	#p96gameplay_msg_fail_len,d3
	bra	.write
.ok
	lea	p96gameplay_msg_ok,a0
	move.l	#p96gameplay_msg_ok_len,d3
	bra	.write
.skipaga
	lea	p96gameplay_msg_skip_aga,a0
	move.l	#p96gameplay_msg_skip_aga_len,d3
	bra	.write
.skipnolib
	lea	p96gameplay_msg_skip_nolib,a0
	move.l	#p96gameplay_msg_skip_nolib_len,d3
	bra	.write
.skipnomode
	lea	p96gameplay_msg_skip_nomode,a0
	move.l	#p96gameplay_msg_skip_nomode_len,d3
	bra	.write
.skipnoint
	lea	p96gameplay_msg_skip_noint,a0
	move.l	#p96gameplay_msg_skip_noint_len,d3
	bra	.write
.screenfail
	lea	p96gameplay_msg_screenfail,a0
	move.l	#p96gameplay_msg_screenfail_len,d3
	bra	.write
.windowfail
	lea	p96gameplay_msg_windowfail,a0
	move.l	#p96gameplay_msg_windowfail_len,d3
	bra	.write
.skipnogfx
	lea	p96gameplay_msg_skip_nogfx,a0
	move.l	#p96gameplay_msg_skip_nogfx_len,d3
	bra	.write
.skipnobitmap
	lea	p96gameplay_msg_nobitmap,a0
	move.l	#p96gameplay_msg_nobitmap_len,d3
	bra	.write
.write
	move.l	d7,d1
	move.l	a0,d2
...
; -----------------------------------------------------------------------------
; c86zje appended P96 ScreenBuffer state/data.  Appended deliberately so the
; proven c86zjd code/data layout and its PC-relative ranges stay nearly intact.
; -----------------------------------------------------------------------------
...
p96gameplay_dbuf_logstate	dc	0
...
p96dbuf_log_name	dc.b	'RAM:gloom_p96_doublebuffer.txt',0
p96dbuf_msg_active	dc.b	'P96 doublebuffer: ScreenBuffer swap active',10
p96dbuf_msg_active_len	equ	*-p96dbuf_msg_active
p96dbuf_msg_fallback	dc.b	'P96 doublebuffer: allocation failed, stable single buffer used',10
p96dbuf_msg_fallback_len	equ	*-p96dbuf_msg_fallback
p96dbuf_msg_syncfail	dc.b	'P96 doublebuffer: sync or ChangeScreenBuffer failed',10
p96dbuf_msg_syncfail_len	equ	*-p96dbuf_msg_syncfail
p96dbuf_msg_bridge	dc.b	'P96 doublebuffer: visible bridge bitmap rebound',10
p96dbuf_msg_bridge_len	equ	*-p96dbuf_msg_bridge
	even

; c86zjp: force the P96 custom-screen title bar off at runtime.
; SA_ShowTitle/SA_Quiet are kept in the tag list, but a few P96/Intuition
; combinations can still reveal the screen title ("Gloom") during buffer swaps.
; ShowTitle(screen,FALSE) is therefore repeated after opening and front-buffer
; changes.  Safe no-op when screen or intuition.library is unavailable.
...
g2p96_gameplay_dbuf_init
	...
	move	#1,p96gameplay_dbuf_logstate
	jsr	g2p96_gameplay_dbuf_log
	jsr	g2p96_dbuf_diag_capture_init	;c86zjn: capture actual P96 ScreenBuffer backing placement
	moveq	#-1,d0
	bra	.done
.fail
	move	#2,p96gameplay_dbuf_logstate
...
; c86zjf: menus and static bridges must not assume the original screen bitmap
...
.have_sb
	...
	jsr	g2p96_force_hide_screen_title	;c86zjp: menus/static bridges must not expose Intuition screen title
	move	#4,p96gameplay_dbuf_logstate
	jsr	g2p96_gameplay_dbuf_log
	bra.s	.done
.fail
	move	#3,p96gameplay_dbuf_logstate
...
g2p96_gameplay_dbuf_free_partial
	movem.l	d0/a0-a1/a6,-(a7)
	jsr	g2p96_dbuf_diag_mark_free_begin	;c86zjn: remember pending state before resources disappear
	move.l	p96gameplay_intbase,d0
	beq	.no_buffers
	move.l	p96gameplay_dbuf_sb1,d0
	beq	.no_sb1
...
.no_safe
	...
	clr	p96gameplay_dbuf_current
	move	#1,p96gameplay_dbuf_draw
	jsr	g2p96_dbuf_diag_write_free_done	;c86zjn: proves FreeScreenBuffer/port teardown was reached
	movem.l	(a7)+,d0/a0-a1/a6
	rts
...
g2p96_gameplay_dbuf_log
	movem.l	d0-d7/a0-a1/a6,-(a7)
	move.l	dosbase,d0
	beq	.done
	move.l	d0,a6
	lea	p96dbuf_log_name,a0
	move.l	a0,d1
	move.l	#1006,d2
	jsr	-30(a6)
	move.l	d0,d7
	beq	.done
	cmp	#2,p96gameplay_dbuf_logstate
	beq	.fallback
	cmp	#3,p96gameplay_dbuf_logstate
	beq	.syncfail
	cmp	#4,p96gameplay_dbuf_logstate
	beq	.bridge
	lea	p96dbuf_msg_active,a0
	move.l	#p96dbuf_msg_active_len,d3
	bra	.write
.fallback
	lea	p96dbuf_msg_fallback,a0
	move.l	#p96dbuf_msg_fallback_len,d3
	bra	.write
.syncfail
	lea	p96dbuf_msg_syncfail,a0
	move.l	#p96dbuf_msg_syncfail_len,d3
	bra	.write
.bridge
	lea	p96dbuf_msg_bridge,a0
	move.l	#p96dbuf_msg_bridge_len,d3
.write
	move.l	d7,d1
	move.l	a0,d2
	jsr	-48(a6)
...
; -----------------------------------------------------------------------------
; c86zjg: fast static planar row decoder.
;
; The old static presenter called g2p96_static_planar_pixel once for every
; source pixel.  That recalculated y*linemodw, x/8 and every plane address
; 76,800 times per 320x240 frame.  This decoder reads each group of eight
; planar pixels once, expands one complete row to 320 palette indices, and
; lets the existing RGB565/scaling loops consume the linear row through a6.
; The legacy pixel routine remains below its original label as a safe reference,
; but title/intermission conversion no longer calls it.
; -----------------------------------------------------------------------------
p96static_fast_log_state	dc	0
p96static_fast_log_name	dc.b	'RAM:gloom_p96_staticfast.txt',0
p96static_fast_log_msg	dc.b	'P96 static: fast planar row decoder active',10
p96static_fast_log_msg_len	equ	*-p96static_fast_log_msg
	even
...
g2p96_static_fast_log_once
	movem.l	d0-d3/d7/a0/a6,-(a7)
	tst	p96static_fast_log_state
	bne.s	.done
	move	#-1,p96static_fast_log_state
	move.l	dosbase,d0
	beq.s	.done
	move.l	d0,a6
	lea	p96static_fast_log_name,a0
	move.l	a0,d1
	move.l	#1006,d2
	jsr	-30(a6)		; Open
	move.l	d0,d7
	beq.s	.done
	move.l	d7,d1
	lea	p96static_fast_log_msg,a0
	move.l	a0,d2
	move.l	#p96static_fast_log_msg_len,d3
	jsr	-48(a6)		; Write
	move.l	d7,d1
	jsr	-36(a6)		; Close
.done
...
; c86zjj appended near EOF to avoid shifting mature PC-relative code ranges.
; -----------------------------------------------------------------------------
; c86zjj: exact native P96 Bigfont glyph cache.
;
; The loaded bigfont.bin/bigfont2.bin remains the single source of truth.  Once
; per font pointer, render all 40 glyph slots into a temporary CHIP bitmap,
; expand the resulting 8x10 planar glyphs into 3,200 linear palette indices,
; then free the CHIP scratch.  Every P96 menu composition after that uses only
; this linear cache.  AGA and any allocation/cache failure retain the complete
; c86zjh planar-delta fallback below.
; -----------------------------------------------------------------------------
...
p96menu_glyph_log_state	dc	0
p96menu_glyph_cache_name	dc.b	'P96 Bigfont glyph cache',0
p96menu_glyph_scratch_name	dc.b	'P96 Bigfont CHIP scratch',0
p96menu_glyph_log_name	dc.b	'RAM:gloom_p96_glyphcache.txt',0
p96menu_glyph_log_msg	dc.b	'P96 menu: native Bigfont glyph cache active',10
p96menu_glyph_log_msg_len	equ	*-p96menu_glyph_log_msg
	even
...
.state_ok
	...
	tst	d0
	beq	.done_zero
	; Cache validity alone is not enough: only suppress/repace the planar menu
	; while the currently visible P96 bitmap can be resolved safely.
	jsr	g2p96_menu_glyph_prepare_target
	bra	.done
.done_zero
	moveq	#0,d0
...
; Set direct visible target fields for row-cache updates.  Full-menu batches do
; not need these fields, but preparing them is harmless and keeps one renderer.
g2p96_menu_glyph_prepare_target
	; d0 is the return value and must not be restored from the stack.
	...
	move	6(a0),d1
	mulu	d4,d1
	add.l	d1,p96menu_native_dstbase
	moveq	#0,d1
	move	4(a0),d1
	add.l	d1,d1
	add.l	d1,p96menu_native_dstbase
...
; Select native glyph renderer when available, otherwise preserve the exact
; c86zjh/c86zji planar-delta routines as rollback.
...
.fallback
	cmp	#P96DSP_TITLE_BRIDGE,p96display_state
	beq	.title
	jmp	g2p96_menu_native_draw_current_row_bigfont_delta_no_restore
.title
	jmp	g2p96_menu_native_draw_current_row_no_restore
...
g2p96_menu_glyph_log_once
	movem.l	d0-d3/d7/a0/a6,-(a7)
	tst	p96menu_glyph_log_state
	bne	.done
	move	#-1,p96menu_glyph_log_state
	move.l	dosbase,d0
	beq	.done
	move.l	d0,a6
	lea	p96menu_glyph_log_name,a0
	move.l	a0,d1
	move.l	#1006,d2
	jsr	-30(a6)
	move.l	d0,d7
	beq	.done
	move.l	d7,d1
	lea	p96menu_glyph_log_msg,a0
	move.l	a0,d2
	move.l	#p96menu_glyph_log_msg_len,d3
	jsr	-48(a6)
	move.l	d7,d1
	jsr	-36(a6)
.done
...
; -----------------------------------------------------------------------------
; c86zjl: direct linear P96 trimmed-IFF composition.
;
; showpic/showpic_noclear now decode ByteRun1 picture data once.  The same pass
; writes the original planar compatibility bitmap (so AGA-era title/script code
; remains intact) and a private 320x240 chunky palette-index composition buffer.
; Static P96 RGB565 conversion consumes that linear buffer directly and no
; longer reconstructs every pixel by reading all Amiga bitplanes back.
;
; The optional title brush is decoded into showbitmap and the same composition
; buffer during its first of the existing two compatibility decodes.  Any path
; which changes only planar pixels (currently intermission printmess2 text)
; invalidates the direct buffer and automatically uses the proven c86zjg planar
; row decoder.  Allocation, format or bounds failure therefore remains safe.
; -----------------------------------------------------------------------------
...
p96static_direct_log_state	dc	0
p96static_direct_name	dc.b	'P96 direct IFF index composition',0
p96static_direct_log_name	dc.b	'RAM:gloom_p96_staticnative.txt',0
p96static_direct_log_msg	dc.b	'P96 static: direct IFF index composition active',10
p96static_direct_log_msg_len	equ	*-p96static_direct_log_msg
	even
...
g2p96_static_showpic_native_entry
	...
	move.l	d0,a1
	move.l	(a7),a0
	jsr	g2p96_static_decode_iff_dual_clear_if_bridge
	tst	d0
	beq.w	.legacy_clear
	; Direct decoder already cleared and filled drawbitmap without an AGA-visible
	; blank swap.  Keep the original palette/db/vwait tail exactly.
	movem.l	(a7)+,a0-a1
	tst.l	a1
	beq	.direct_nopal
	jsr	pokepal
...
g2p96_static_showpic_noclear_native_entry
	...
	move.l	d0,a1
	move.l	(a7),a0
	jsr	g2p96_static_decode_iff_dual_noclear_if_bridge
	tst	d0
	bne	.decoded
.planar_only
	clr	p96static_direct_valid
...
; current P96 title/intermission bridge.
...
; Clear the complete compatibility draw bitmap and direct index buffer, then
; decode an optional picture into both in one pass.
g2p96_static_decode_iff_dual_clear_if_bridge
	movem.l	d1-d7/a0-a6,-(a7)
	move.l	a0,a5
	move.l	a1,a4
	...
	tst	d0
	beq.w	.fail
	; Clear all active planar compatibility memory without clspic/db, so P96
	; does not wait for or expose the intermediate AGA blank frame.
	move.l	a4,a0
	move.l	bmapmem,d7
	lsr.l	#2,d7
	beq	.planar_cleared
...
.clear_index_long
	...
	beq	.fail
.blank_success
	jsr	g2p96_static_direct_log_once
	moveq	#-1,d0
	bra	.done
.fail
	clr	p96static_direct_valid
...
g2p96_static_decode_iff_dual_noclear_if_bridge
	movem.l	d1-d7/a0-a6,-(a7)
	move.l	a0,a5
	move.l	a1,a4
	jsr	g2p96_static_direct_ensure_buffer
...
g2p96_static_row_source_log_once
	tst	p96static_direct_valid
	bne	.done
	jmp	g2p96_static_fast_log_once
.done
	rts
...
; printmess2 has modified showbitmap without updating the direct picture index
; composition.  Force the exact c86zjg planar decoder for this final text frame.
g2p96_static_present_intermission_text_native_guard
	clr	p96static_direct_valid
	jmp	g2p96_static_present_intermission_text_skip_if_bridge

g2p96_static_direct_log_once
	movem.l	d0-d3/d7/a0/a6,-(a7)
	tst	p96static_direct_log_state
	bne	.done
	move	#-1,p96static_direct_log_state
	move.l	dosbase,d0
	beq	.done
	move.l	d0,a6
	lea	p96static_direct_log_name,a0
	move.l	a0,d1
	move.l	#1006,d2
	jsr	-30(a6)
	move.l	d0,d7
	beq	.done
	move.l	d7,d1
	lea	p96static_direct_log_msg,a0
	move.l	a0,d2
	move.l	#p96static_direct_log_msg_len,d3
	jsr	-48(a6)
	move.l	d7,d1
	jsr	-36(a6)
.done
...
; -----------------------------------------------------------------------------
; c86zjm: P96/Classic performance memory-placement diagnostic.
;
; The c86zjl direct static compositor itself does not run during gameplay, but
; it keeps a target-sized RGB565 staging buffer and a 320x240 index buffer alive.
; Classic Gloom also loads several embedded compatibility assets.  This snapshot
; records whether important render/static buffers ended up in FAST or CHIP RAM
; and how large the remaining contiguous blocks are when the first linear P96
; gameplay frame becomes active.  No rendering, allocation or lifecycle logic
; is changed by this diagnostic.
; -----------------------------------------------------------------------------
p96perf_log_state	dc	0
p96perf_name_classic	dc.b	'RAM:gloom_p96_perf_classic.txt',0
p96perf_name_deluxe	dc.b	'RAM:gloom_p96_perf_deluxe.txt',0
p96perf_name_other	dc.b	'RAM:gloom_p96_perf_other.txt',0
	even

p96perf_buffer
	dc.b	'PROFILE='
p96perf_profile_hex	ds.b	8
	dc.b	10,'TARGET_W='
p96perf_target_w_hex	ds.b	8
	dc.b	' TARGET_H='
p96perf_target_h_hex	ds.b	8
	dc.b	' MODE='
p96perf_target_mode_hex	ds.b	8
	dc.b	10,'BITPLANES='
p96perf_bitplanes_hex	ds.b	8
	dc.b	' LINEMOD='
p96perf_linemod_hex	ds.b	8
	dc.b	' BPMOD='
p96perf_bpmod_hex	ds.b	8
	dc.b	10,'CHUNKY='
p96perf_chunky_ptr_hex	ds.b	8
	dc.b	' TYPE='
p96perf_chunky_type_hex	ds.b	8
	dc.b	10,'MEMORY='
p96perf_memory_ptr_hex	ds.b	8
	dc.b	' TYPE='
p96perf_memory_type_hex	ds.b	8
	dc.b	10,'STATIC_RGB='
p96perf_staticrgb_ptr_hex	ds.b	8
	dc.b	' TYPE='
p96perf_staticrgb_type_hex	ds.b	8
	dc.b	' SIZE='
p96perf_staticrgb_size_hex	ds.b	8
	dc.b	10,'DIRECT_INDEX='
p96perf_direct_ptr_hex	ds.b	8
	dc.b	' TYPE='
p96perf_direct_type_hex	ds.b	8
	dc.b	' VALID='
p96perf_direct_valid_hex	ds.b	8
	dc.b	10,'MENU_PAGE='
p96perf_menupage_ptr_hex	ds.b	8
	dc.b	' TYPE='
p96perf_menupage_type_hex	ds.b	8
	dc.b	10,'GLYPH_CACHE='
p96perf_glyph_ptr_hex	ds.b	8
	dc.b	' TYPE='
p96perf_glyph_type_hex	ds.b	8
	dc.b	10,'PLANAR_PALETTE='
p96perf_palette_ptr_hex	ds.b	8
	dc.b	' TYPE='
p96perf_palette_type_hex	ds.b	8
	dc.b	10,'PLANAR_REMAP='
p96perf_remap_ptr_hex	ds.b	8
	dc.b	' TYPE='
p96perf_remap_type_hex	ds.b	8
	dc.b	10,'FREE_PUBLIC='
p96perf_free_public_hex	ds.b	8
	dc.b	' LARGEST_PUBLIC='
p96perf_largest_public_hex	ds.b	8
	dc.b	10,'FREE_FAST='
p96perf_free_fast_hex	ds.b	8
	dc.b	' LARGEST_FAST='
p96perf_largest_fast_hex	ds.b	8
	dc.b	10,'FREE_CHIP='
p96perf_free_chip_hex	ds.b	8
	dc.b	' LARGEST_CHIP='
p96perf_largest_chip_hex	ds.b	8
	dc.b	10
p96perf_buffer_len	equ	*-p96perf_buffer
	even

; Called once when c86zjd linear gameplay is first armed.  Separate filenames
; preserve Classic and Deluxe snapshots when both are run without rebooting.
g2p96_perf_snapshot_once
	movem.l	d0-d7/a0-a6,-(a7)
	tst	p96perf_log_state
	bne.w	.done
	move	#-1,p96perf_log_state

	moveq	#0,d0
	move	g2_game_profile,d0
	lea	p96perf_profile_hex,a0
	jsr	g2p96_long_to_hex8
	moveq	#0,d0
	move	p96target_width,d0
	lea	p96perf_target_w_hex,a0
	jsr	g2p96_long_to_hex8
	moveq	#0,d0
	move	p96target_height,d0
	lea	p96perf_target_h_hex,a0
	jsr	g2p96_long_to_hex8
	moveq	#0,d0
	move	p96target_mode,d0
	lea	p96perf_target_mode_hex,a0
	jsr	g2p96_long_to_hex8
	moveq	#0,d0
	move	bitplanes,d0
	lea	p96perf_bitplanes_hex,a0
	jsr	g2p96_long_to_hex8
	move.l	linemod,d0
	lea	p96perf_linemod_hex,a0
	jsr	g2p96_long_to_hex8
	move.l	bpmod,d0
	lea	p96perf_bpmod_hex,a0
	jsr	g2p96_long_to_hex8

	move.l	chunky,d0
	lea	p96perf_chunky_ptr_hex,a0
	lea	p96perf_chunky_type_hex,a1
	jsr	g2p96_perf_store_ptr_type
	move.l	memory,d0
	lea	p96perf_memory_ptr_hex,a0
	lea	p96perf_memory_type_hex,a1
	jsr	g2p96_perf_store_ptr_type
	move.l	p96static_rgbbufptr,d0
	lea	p96perf_staticrgb_ptr_hex,a0
	lea	p96perf_staticrgb_type_hex,a1
	jsr	g2p96_perf_store_ptr_type
	move.l	p96static_rgbbufsize,d0
	lea	p96perf_staticrgb_size_hex,a0
	jsr	g2p96_long_to_hex8
	move.l	p96static_direct_index_ptr,d0
	lea	p96perf_direct_ptr_hex,a0
	lea	p96perf_direct_type_hex,a1
	jsr	g2p96_perf_store_ptr_type
	moveq	#0,d0
	move	p96static_direct_valid,d0
	and.l	#$0000ffff,d0
	lea	p96perf_direct_valid_hex,a0
	jsr	g2p96_long_to_hex8
	move.l	p96menu_page_cache_ptr,d0
	lea	p96perf_menupage_ptr_hex,a0
	lea	p96perf_menupage_type_hex,a1
	jsr	g2p96_perf_store_ptr_type
	move.l	p96menu_glyph_cache_ptr,d0
	lea	p96perf_glyph_ptr_hex,a0
	lea	p96perf_glyph_type_hex,a1
	jsr	g2p96_perf_store_ptr_type
	move.l	planar_palette,d0
	lea	p96perf_palette_ptr_hex,a0
	lea	p96perf_palette_type_hex,a1
	jsr	g2p96_perf_store_ptr_type
	move.l	planar_remap,d0
	lea	p96perf_remap_ptr_hex,a0
	lea	p96perf_remap_type_hex,a1
	jsr	g2p96_perf_store_ptr_type

	move.l	#1,d1		; MEMF_PUBLIC
	lea	p96perf_free_public_hex,a0
	jsr	g2p96_perf_store_avail
	move.l	#$00020001,d1	; MEMF_PUBLIC|MEMF_LARGEST
	lea	p96perf_largest_public_hex,a0
	jsr	g2p96_perf_store_avail
	move.l	#4,d1		; MEMF_FAST
	lea	p96perf_free_fast_hex,a0
	jsr	g2p96_perf_store_avail
	move.l	#$00020004,d1	; MEMF_FAST|MEMF_LARGEST
	lea	p96perf_largest_fast_hex,a0
	jsr	g2p96_perf_store_avail
	move.l	#2,d1		; MEMF_CHIP
	lea	p96perf_free_chip_hex,a0
	jsr	g2p96_perf_store_avail
	move.l	#$00020002,d1	; MEMF_CHIP|MEMF_LARGEST
	lea	p96perf_largest_chip_hex,a0
	jsr	g2p96_perf_store_avail

	move.l	dosbase,d0
	beq.w	.done
	move.l	d0,a6
	lea	p96perf_name_other,a0
	cmp	#1,g2_game_profile
	bne.s	.not_classic
	lea	p96perf_name_classic,a0
	bra.s	.have_name
.not_classic
	tst	g2_game_profile
	bne.s	.have_name
	lea	p96perf_name_deluxe,a0
.have_name
	move.l	a0,d1
	move.l	#1006,d2	; MODE_NEWFILE
	jsr	-30(a6)
	...
	beq.s	.done
	move.l	d7,d1
	lea	p96perf_buffer,a0
	move.l	a0,d2
	move.l	#p96perf_buffer_len,d3
	jsr	-48(a6)
	move.l	d7,d1
	jsr	-36(a6)
.done
...
; d0=pointer, a0=address-field destination, a1=type-field destination.
g2p96_perf_store_ptr_type
	...
	move.l	d2,a1
	move.l	4.w,a6
	jsr	-534(a6)	; Exec TypeOfMem
	move.l	a3,a0
	jsr	g2p96_long_to_hex8
	movem.l	(a7)+,d0-d2/a0-a3/a6
	rts

; d1=AvailMem requirements, a0=8-character destination.
g2p96_perf_store_avail
...
; -----------------------------------------------------------------------------
; c86zjn: native P96 intermission typewriter.
;
; The original P96 bridge forced pdelay=0 because converting all 320x240 planar
; pixels after every character was too slow.  c86zjl now keeps the decoded IFF
; picture as a linear 320x240 palette-index composition and c86zjk provides the
; exact Bigfont glyph cache.  This path overlays each newly blitted character
; directly into that composition, the staged RGB565 buffer and the visible P96
; bitmap.  Therefore the original pdelay=2 cadence and ENTER-to-skip behaviour
; can be restored without repeated full-screen conversion.
; -----------------------------------------------------------------------------
p96inter_typewriter_active	dc	0
p96inter_visible_base	dc.l	0
p96inter_visible_bpr	dc.l	0
p96inter_log_state	dc	0
p96inter_log_name	dc.b	'RAM:gloom_p96_intertype.txt',0
p96inter_log_msg	dc.b	'P96 intermission: native Bigfont typewriter active',10
p96inter_log_msg_len	equ	*-p96inter_log_msg
	even

; Called from scripttext in the exact old clr pdelay slot.  Keep pdelay=2 only
; when every native dependency is available; otherwise restore c86zhi pdelay=0.
g2p96_intermission_native_typewriter_prepare
	...
	tst	d0
	beq.w	.fail
	move.l	p96menu_native_dstbase,d0
	move.l	d0,p96inter_visible_base
	move.l	p96menu_native_bpr,d0
	move.l	d0,p96inter_visible_bpr
	moveq	#0,d0
	move	p96target_width,d0
	add.l	d0,d0
	move.l	d0,p96static_rgbbpr
	...
	jsr	g2p96_intermission_native_typewriter_log_once
	moveq	#-1,d0
	bra.s	.done
.fail
	clr	pdelay	;c86zhi safe fallback: complete static text, one final full-screen present
	moveq	#0,d0
.done
	movem.l	(a7)+,d1-d7/a0-a6
	rts
...
.col
	...
	; First update the offscreen staged RGB565 composition.
	move.l	p96static_rgbbufptr,d6
	move.l	d6,p96menu_native_dstbase
	move.l	p96static_rgbbpr,d6
	move.l	d6,p96menu_native_bpr
	jsr	g2p96_menu_native_put_pixel
	; Then publish only this glyph pixel to the visible P96 bitmap.
	move.l	p96inter_visible_base,d6
	move.l	d6,p96menu_native_dstbase
	move.l	p96inter_visible_bpr,d6
	move.l	d6,p96menu_native_bpr
	jsr	g2p96_menu_native_put_pixel
.next_col
	addq	#1,d2
...
.next_row
	...
	bra.w	.row
.done
	move.l	p96inter_visible_base,d0
	move.l	d0,p96menu_native_dstbase
	move.l	p96inter_visible_bpr,d0
	move.l	d0,p96menu_native_bpr
	movem.l	(a7)+,d0-d7/a0-a6
	rts

; Native characters already updated all P96 representations.  The old full
; planar row-decoder present is retained only when preparation failed.
...
g2p96_intermission_native_typewriter_log_once
	movem.l	d0-d3/d7/a0/a6,-(a7)
	tst	p96inter_log_state
	bne.s	.done
	move	#-1,p96inter_log_state
	move.l	dosbase,d0
	beq.s	.done
	move.l	d0,a6
	lea	p96inter_log_name,a0
	move.l	a0,d1
	move.l	#1006,d2
	jsr	-30(a6)
	move.l	d0,d7
	beq.s	.done
	move.l	d7,d1
	lea	p96inter_log_msg,a0
	move.l	a0,d2
	move.l	#p96inter_log_msg_len,d3
	jsr	-48(a6)
	move.l	d7,d1
	jsr	-36(a6)
.done
...
; -----------------------------------------------------------------------------
; c86zjn: P96 ScreenBuffer lifecycle diagnostic for the unresolved
; Classic->Deluxe slowdown.  c86zjm ruled out placement changes in all ordinary
; Exec buffers; these fields capture the two Intuition ScreenBuffers and prove
; whether their teardown path is actually reached.
; -----------------------------------------------------------------------------
p96dbdiag_sb0	dc.l	0
p96dbdiag_sb1	dc.l	0
p96dbdiag_bm0	dc.l	0
p96dbdiag_bm1	dc.l	0
p96dbdiag_base0	dc.l	0
p96dbdiag_base1	dc.l	0
p96dbdiag_type0	dc.l	0
p96dbdiag_type1	dc.l	0
p96dbdiag_bpr0	dc.l	0
p96dbdiag_bpr1	dc.l	0
p96dbdiag_free_begin	dc.l	0
p96dbdiag_free_done	dc.l	0
p96dbdiag_safe_before	dc.l	0
p96dbdiag_disp_before	dc.l	0
p96dbdiag_name_classic	dc.b	'RAM:gloom_p96_dbuf_classic.txt',0
p96dbdiag_name_deluxe	dc.b	'RAM:gloom_p96_dbuf_deluxe.txt',0
p96dbdiag_name_other	dc.b	'RAM:gloom_p96_dbuf_other.txt',0
	even
p96dbdiag_buffer
	dc.b	'PROFILE='
p96dbdiag_profile_hex	ds.b	8
	dc.b	10,'SB0='
p96dbdiag_sb0_hex	ds.b	8
	dc.b	' BM0='
p96dbdiag_bm0_hex	ds.b	8
	dc.b	' BASE0='
p96dbdiag_base0_hex	ds.b	8
	dc.b	' TYPE0='
p96dbdiag_type0_hex	ds.b	8
	dc.b	' BPR0='
p96dbdiag_bpr0_hex	ds.b	8
	dc.b	10,'SB1='
p96dbdiag_sb1_hex	ds.b	8
	dc.b	' BM1='
p96dbdiag_bm1_hex	ds.b	8
	dc.b	' BASE1='
p96dbdiag_base1_hex	ds.b	8
	dc.b	' TYPE1='
p96dbdiag_type1_hex	ds.b	8
	dc.b	' BPR1='
p96dbdiag_bpr1_hex	ds.b	8
	dc.b	10,'FREE_BEGIN='
p96dbdiag_free_begin_hex	ds.b	8
	dc.b	' FREE_DONE='
p96dbdiag_free_done_hex	ds.b	8
	dc.b	' SAFE_BEFORE='
p96dbdiag_safe_hex	ds.b	8
	dc.b	' DISP_BEFORE='
p96dbdiag_disp_hex	ds.b	8
	dc.b	10
p96dbdiag_buffer_len	equ	*-p96dbdiag_buffer
	even

g2p96_dbuf_diag_capture_init
	movem.l	d0-d2/a0-a2/a6,-(a7)
	move.l	p96gameplay_dbuf_sb0,d0
	move.l	d0,p96dbdiag_sb0
	move.l	p96gameplay_dbuf_sb1,d0
	move.l	d0,p96dbdiag_sb1
	move.l	p96gameplay_dbuf_sb0,d0
	beq.s	.sb1
	move.l	d0,a0
	move.l	SB_BitMap(a0),d0
	move.l	d0,p96dbdiag_bm0
	beq.s	.sb1
	move.l	d0,a1
	moveq	#0,d1
	move	0(a1),d1
	move.l	d1,p96dbdiag_bpr0
	move.l	8(a1),d0
	move.l	d0,p96dbdiag_base0
	move.l	d0,a1
	move.l	4.w,a6
	jsr	-534(a6)
	move.l	d0,p96dbdiag_type0
.sb1
	move.l	p96gameplay_dbuf_sb1,d0
	beq.s	.done
	move.l	d0,a0
	move.l	SB_BitMap(a0),d0
	move.l	d0,p96dbdiag_bm1
	beq.s	.done
	move.l	d0,a1
	moveq	#0,d1
	move	0(a1),d1
	move.l	d1,p96dbdiag_bpr1
	move.l	8(a1),d0
	move.l	d0,p96dbdiag_base1
	move.l	d0,a1
	move.l	4.w,a6
	jsr	-534(a6)
	move.l	d0,p96dbdiag_type1
.done
	movem.l	(a7)+,d0-d2/a0-a2/a6
	rts

g2p96_dbuf_diag_mark_free_begin
	movem.l	d0,-(a7)
	move.l	#-1,p96dbdiag_free_begin
	moveq	#0,d0
	move	p96gameplay_dbuf_safe_pending,d0
	move.l	d0,p96dbdiag_safe_before
	moveq	#0,d0
	move	p96gameplay_dbuf_disp_pending,d0
	move.l	d0,p96dbdiag_disp_before
	movem.l	(a7)+,d0
	rts

g2p96_dbuf_diag_write_free_done
	movem.l	d0-d3/d7/a0-a1/a6,-(a7)
	move.l	#-1,p96dbdiag_free_done
	moveq	#0,d0
	move	g2_game_profile,d0
	lea	p96dbdiag_profile_hex,a0
	jsr	g2p96_long_to_hex8
	move.l	p96dbdiag_sb0,d0
	lea	p96dbdiag_sb0_hex,a0
	jsr	g2p96_long_to_hex8
	move.l	p96dbdiag_bm0,d0
	lea	p96dbdiag_bm0_hex,a0
	jsr	g2p96_long_to_hex8
	move.l	p96dbdiag_base0,d0
	lea	p96dbdiag_base0_hex,a0
	jsr	g2p96_long_to_hex8
	move.l	p96dbdiag_type0,d0
	lea	p96dbdiag_type0_hex,a0
	jsr	g2p96_long_to_hex8
	move.l	p96dbdiag_bpr0,d0
	lea	p96dbdiag_bpr0_hex,a0
	jsr	g2p96_long_to_hex8
	move.l	p96dbdiag_sb1,d0
	lea	p96dbdiag_sb1_hex,a0
	jsr	g2p96_long_to_hex8
	move.l	p96dbdiag_bm1,d0
	lea	p96dbdiag_bm1_hex,a0
	jsr	g2p96_long_to_hex8
	move.l	p96dbdiag_base1,d0
	lea	p96dbdiag_base1_hex,a0
	jsr	g2p96_long_to_hex8
	move.l	p96dbdiag_type1,d0
	lea	p96dbdiag_type1_hex,a0
	jsr	g2p96_long_to_hex8
	move.l	p96dbdiag_bpr1,d0
	lea	p96dbdiag_bpr1_hex,a0
	jsr	g2p96_long_to_hex8
	move.l	p96dbdiag_free_begin,d0
	lea	p96dbdiag_free_begin_hex,a0
	jsr	g2p96_long_to_hex8
	move.l	p96dbdiag_free_done,d0
	lea	p96dbdiag_free_done_hex,a0
	jsr	g2p96_long_to_hex8
	move.l	p96dbdiag_safe_before,d0
	lea	p96dbdiag_safe_hex,a0
	jsr	g2p96_long_to_hex8
	move.l	p96dbdiag_disp_before,d0
	lea	p96dbdiag_disp_hex,a0
	jsr	g2p96_long_to_hex8
	move.l	dosbase,d0
	beq.w	.done
	move.l	d0,a6
	lea	p96dbdiag_name_other,a0
	cmp	#1,g2_game_profile
	bne.s	.not_classic
	lea	p96dbdiag_name_classic,a0
	bra.s	.have_name
.not_classic
	tst	g2_game_profile
	bne.s	.have_name
	lea	p96dbdiag_name_deluxe,a0
.have_name
	move.l	a0,d1
	move.l	#1006,d2
	jsr	-30(a6)
	...
	beq.s	.done
	move.l	d7,d1
	lea	p96dbdiag_buffer,a0
	move.l	a0,d2
	move.l	#p96dbdiag_buffer_len,d3
	jsr	-48(a6)
	move.l	d7,d1
	jsr	-36(a6)
.done
...
; -----------------------------------------------------------------------------
; c86zkm - clean bare-token ToolType / CLI parser
;
; Accepted, case-insensitive tokens:
;   AGA  ECS  P96  FPS  HIRES  STRETCH  WIDE
;
; Separators:
;   space, TAB, comma, CR, LF
;
; DISPLAY=..., HIRES=YES and STRETCH=YES are deliberately not recognized.
; d1 = source: 1 ToolType, 2 CLI
; -----------------------------------------------------------------------------
...
g2tok_set_fps
	move	#-1,g2fps_enabled	;c87a: runtime-only; deliberately absent from gloom.cfg
	bra	g2tok_next
...
g2tok_p96	dc.b	'P96'
g2tok_fps	dc.b	'FPS'
g2tok_wide	dc.b	'WIDE'
g2tok_hires	dc.b	'HIRES'
g2tok_stretch	dc.b	'STRETCH'
	even
...
; Parser-only stage safety:
; - AGA/P96 on ECS: existing AGA-required notice
; - ECS on either chipset: selected correctly, but renderer not included yet
g2displaymode_validate_stage1
	movem.l	d1-d7/a0-a6,-(a7)
	clr	d7
	move	g2display_mode,d0
	cmp	#3,d0
	beq.s	g2validate_ecs_pending
	tst	g2chipset_aga
	bne.s	g2validate_done
	moveq	#-1,d7
	jsr	g2early_show_noaga_notice
	bra.s	g2validate_done

g2validate_ecs_pending
	moveq	#-1,d7
	jsr	g2show_ecs_stage1_notice
...
g2ecs_stage1_cli
	dc.b	10,'ECS mode selected successfully.',10
	dc.b	'The ECS renderer is not included in this ToolType test build yet.',10,10
g2ecs_stage1_cli_end
g2ecs_stage1_cli_len	equ	g2ecs_stage1_cli_end-g2ecs_stage1_cli

g2ecs_stage1_req_body
	dc.b	'ECS mode selected successfully.',10
	dc.b	'The ECS renderer is not included in this ToolType test build yet.',0
	even

g2ecs_stage1_easy
	dc.l	20,0,g2early_req_title,g2ecs_stage1_req_body,g2early_req_gadget

g2show_ecs_stage1_notice
	movem.l	d0-d4/a0-a3/a6,-(a7)
	tst.l	wbmess
	bne.s	g2ecs_notice_wb
	move.l	dosbase,d0
	beq.s	g2ecs_notice_done
	move.l	outhand,d1
	beq.s	g2ecs_notice_done
	lea	g2ecs_stage1_cli(pc),a0
	move.l	a0,d2
	move.l	#g2ecs_stage1_cli_len,d3
	move.l	dosbase,a6
	jsr	-48(a6)
	bra.s	g2ecs_notice_done
...
; -----------------------------------------------------------------------------
; c87a - optional FPS ToolType counter
;
; Bare ToolType FPS enables the counter.  It is never written to gloom.cfg.
; The measurement counts only completed gameplay presents and uses ExecBase's
; readable VBlankFrequency byte (normally 50 or 60) as the real timebase.
; AGA/fallback is drawn into the final 320x240 chunky frame before C2P.
; P96 is drawn after scaling directly into the RGB565 target, preserving the
; requested physical height of exactly five pixels in 240p and 480p modes.
; -----------------------------------------------------------------------------
EB_VBLANKFREQUENCY	equ	530

g2fps_enabled		dc	0	;0=off, -1=FPS ToolType present
g2fps_value		dc	0	;last completed sample shown on screen
g2fps_frames		dc	0	;successful presents in current sample window
g2fps_vbl_start		dc	0	;framecnt at sample-window start
g2fps_vbl_rate		dc	50	;read from ExecBase, safe PAL fallback

; New gameplay session/level: clear the displayed value and acquire the
; system's actual vertical-blank frequency.  Invalid values fall back to 50.
g2fps_reset
	movem.l	d0-d1/a0,-(a7)
	clr	g2fps_value
	clr	g2fps_frames
	move	framecnt,g2fps_vbl_start
	moveq	#50,d1
	move.l	4.w,a0
	moveq	#0,d0
	...
	move	d0,d1
.fallback
	move	d1,g2fps_vbl_rate
	movem.l	(a7)+,d0-d1/a0
	rts

; Return from the paused in-game menu: keep the last visible number, but begin
; a fresh timing window so menu dwell time never appears as a false FPS drop.
...
; Called only after a confirmed P96 present or completed AGA db swap.
g2fps_frame_presented
	tst	g2fps_enabled
	beq.s	.done
	movem.l	d0-d3,-(a7)
	addq	#1,g2fps_frames
	...
	move	framecnt,d0
	moveq	#0,d1
	move	g2fps_vbl_start,d1
	sub	d1,d0		;word arithmetic intentionally handles framecnt wrap
	and.l	#$0000ffff,d0
	moveq	#0,d1
	move	g2fps_vbl_rate,d1
	cmp	d1,d0
	bcs.s	.exit
	; A gap longer than two seconds is a pause/transition, not a useful sample.
	move	d1,d2
	add	d1,d2
	cmp	d2,d0
	bhi.s	.restart_with_current
	moveq	#0,d3
	move	g2fps_frames,d3
	mulu	d1,d3		;frames * VBlankFrequency
	divu	d0,d3		;/ elapsed VBlanks = real frames per second
	cmp	#99,d3
	bls.s	.store
	moveq	#99,d3
.store
...
; AGA and emergency P96 fallback: draw 20x5 pixels at x=298, y=234.
; The foreground uses the bright smallfont/panel source colour remapped through
; the active gameplay palette; the five-pixel black backing keeps it readable.
g2fps_draw_chunky
	tst	g2fps_enabled
	beq.w	.done
	tst	g2teleport_blackout
	bne.w	.done		;keep teleport/intermission handoff completely black
	movem.l	d0-d7/a0-a4,-(a7)
	move.l	chunky,d0
	beq.w	.exit
	move.l	palettes,d1	;first active 256-byte shade/remap table
	beq.w	.exit
	move.l	d1,a3
	moveq	#0,d4
	move.b	3(a3),d4	;bright 4-colour font source pen
	bne.s	.have_colour
	move.b	2(a3),d4
	bne.s	.have_colour
	move.b	1(a3),d4
...
.have_colour
	move.l	d0,a4
	adda.l	#(234*320)+298,a4
	move.l	a4,a1
	moveq	#4,d6		;five rows
.clear_row
	move.l	a1,a2
	moveq	#4,d7		;20 bytes = five longwords
.clear_pixels
	clr.l	(a2)+
	dbf	d7,.clear_pixels
	lea	320(a1),a1
	dbf	d6,.clear_row
	move	#320,d5
	bsr	g2fps_draw_fixed_chunky
.exit
	movem.l	(a7)+,d0-d7/a0-a4
.done
	rts

; P96: a0=target RGB565 origin, d0=BytesPerRow.  Black and white are endian
; invariant in RGB565, so both supported Picasso96 word formats stay correct.
g2fps_draw_p96_target
	...
	cmp	#22,d0
	bcs.w	.exit
	sub	#22,d0		;20-pixel counter plus two-pixel right margin
	...
	cmp	#6,d0
	bcs.w	.exit
	subq	#6,d0		;five pixels plus one-pixel bottom margin
	mulu	d5,d0
	adda.l	d0,a4
	move.l	a4,a1
	moveq	#4,d6
...
.clear_pixels
	...
	adda.l	d5,a1
	dbf	d6,.clear_row
	bsr	g2fps_draw_fixed_rgb565
.exit
	movem.l	(a7)+,d0-d7/a0-a4
.done
	rts

; Draw the fixed "FPS nn" layout into chunky.  a4=top-left, d4.b=colour,
; d5=row stride.  Leading zero is suppressed; the ones digit always appears.
g2fps_draw_fixed_chunky
	move.l	a4,a1
	lea	g2fps_glyph_f(pc),a0
	bsr	g2fps_draw_glyph_chunky
	move.l	a4,a1
	addq.l	#4,a1
	lea	g2fps_glyph_p(pc),a0
	bsr	g2fps_draw_glyph_chunky
	move.l	a4,a1
	addq.l	#8,a1
	lea	g2fps_glyph_s(pc),a0
	bsr	g2fps_draw_glyph_chunky
...
; Same fixed layout for the final RGB565 target.  a4=top-left, d5=stride.
g2fps_draw_fixed_rgb565
	move.l	a4,a1
	lea	g2fps_glyph_f(pc),a0
	bsr	g2fps_draw_glyph_rgb565
	move.l	a4,a1
	adda.w	#8,a1		;four pixels * two bytes
	lea	g2fps_glyph_p(pc),a0
	bsr	g2fps_draw_glyph_rgb565
	move.l	a4,a1
	adda.w	#16,a1
	...
	adda.l	d0,a0
	move.l	a4,a1
	adda.w	#26,a1		;13 pixels * two bytes
	bsr	g2fps_draw_glyph_rgb565
.ones
	move	d7,d0
	lsl	#2,d0
	...
	adda.l	d0,a0
	move.l	a4,a1
	adda.w	#34,a1		;17 pixels * two bytes
	bsr	g2fps_draw_glyph_rgb565
	rts

; Return d6=tens, d7=ones, clamped to the fixed two-digit display.
g2fps_split_digits
	moveq	#0,d0
	move	g2fps_value,d0
	cmp	#99,d0
...
; Five rows, three columns, bit 2=left pixel ... bit 0=right pixel.
...
; 3x5 glyph data.  The counter itself is exactly five physical pixels high.
g2fps_glyph_f	dc.b	7,4,6,4,4
g2fps_glyph_p	dc.b	6,5,6,4,4
g2fps_glyph_s	dc.b	7,4,7,1,7
g2fps_digits
	dc.b	7,5,5,5,7	;0
	dc.b	2,6,2,2,7	;1
	dc.b	7,1,7,4,7	;2
