...
p96prevlog_name	dc.b	'RAM:gloom_p96_preview.txt',0
	even
p96prevlog_ok	dc.b	'P96 preview: finer 40x24 tonal grid active',10
...
.nolog	movem.l	d0-d7/a0-a6,-(a7)
	...
	beq.w	.restore
	lea	coloffs(pc),a3
	; gloom2p96_c9: c8 proved that a 20x12 block preview is stable.  Step
	; up to a finer 40x24 tonal grid with 8x8 blocks, still only every
	; 16th frame and still using safe RectFill pens.  AGA output remains
	; untouched; this is only a progressively safer P96 preview probe.
...
.c7rowloop
	...
	move	d6,d7
	mulu	#8,d7
	add	#20,d7		;sample source rows 20..204
	move	d7,d0
	mulu	chunkymodw(pc),d0
	move.l	a4,a2
	add.l	d0,a2		;Chunky/C2P row base for this grid row
	moveq	#0,d5		;x = 0..312, step 8
.c7xloop
	cmp	#320,d5
	bge	.c7nextrow
	move.l	0(a3,d5*4),d4
...
.c7mapped
	...
	move	d1,d3
	add	#7,d3
	jsr	-306(a6)	;RectFill 8x8 sample block
	add	#8,d5
	bra	.c7xloop
.c7nextrow
	addq	#1,d6
