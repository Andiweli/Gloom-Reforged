...
;gloom2_c86zdi: stronger near vertical oval, whole projectile/upgrade reflection Bayer-dithered, weapon-upgrade floor-touch pulse.
	...
	;c86zgj: keep the fast c86zgi row/batch refresh, but render menu glyphs from
	;the real loaded bigfont2 shape table instead of the temporary flat chatfont.
	;
...
g2release_marker	dc.b	'Gloom Reforged AGA/P96 probe (v1.4 c86zgj 09.07.2026) Andreas ',39,'Andiweli',39,' Stuermer',0
	even
...
p96gameplay_msg_ok	dc.b	'P96 gameplay: fast bigfont2 P96 menu OK',10
...
; c86zgd: native P96 in-game menu renderer.
; The rejected bridge read the AGA/planar menu bitmap pixel-by-pixel and wrote it
; directly to the visible RTG bitmap.  That works in emulators but on real A1200
; RTG hardware the menu visibly builds from top to bottom.  The P96 path now keeps
; the paused gameplay frame as-is and draws compact native RGB565 text directly
; from chatfont.  No grey backdrop, no HUD masking, no planar bridge.
g2p96_menu_bridge_all_rows_present
	movem.l	d0-d7,-(a7)
	move	curropt,-(a7)
	; c86zgj: draw all rows through the fast native bigfont2 shape path.
	; c86zgi solved the two-second delay with chatfont glyphs; this keeps the
	; same batch flow but uses the real loaded bigfont2 mask/image data.
...
.loop
	...
	bge.w	.flush
	move	d7,curropt
	jsr	g2p96_menu_native_draw_current_row_fasttext_no_restore
	addq	#1,d7
	bra.w	.loop
.flush
...
; c86zgj: fast native P96 bigfont2 renderer.
; c86zgi proved that the menu must avoid the slow planar menubmap scanner, but
; its emergency chatfont path looked too flat.  This routine keeps the same fast
; row-cache/full-page batch flow and renders glyph masks directly from the real
; loaded bigfont2 shape table instead of scanning the already composed planar
; menu bitmap.
...
.char_loop
	...
	beq.s	.advance
	move	d0,d2
	jsr	g2p96_menu_native_draw_bigfontglyph
.advance
	add	fontw,d7
	bra.s	.char_loop
.draw_done
...
p96menu_native_saved_colour	dc	0
	even
...
; c86zgj: map menu text characters to the same glyph numbers as printmess2.
...
; c86zgj: draw one real bigfont2 glyph directly from the loaded Gloom2 font
; shape table into the active P96 row/full-page cache.  This avoids the expensive
; c86zge planar menubmap readback while restoring the original bigfont2 glyph
; silhouettes.  The cookie/mask word is used as a dark outline and the image word
; as the normal selected/normal row colour.
g2p96_menu_native_draw_bigfontglyph
	movem.l	d0-d7/a0-a4,-(a7)
	move.l	font,d0
	beq.w	.done
	move.l	d0,a0
	...
	beq.w	.done
	adda.l	d0,a0
	move.l	a0,a3		; glyph header
	move.l	(a3),d0
	move.l	a3,a4
	adda.l	d0,a4		; cookie/mask source
	lea	10(a3),a1	; image source
	move	6(a3),d5	; blitter size: height in upper bits
	lsr	#6,d5
	cmp	fonth,d5
	ble.s	.height_ok
	move	fonth,d5
.height_ok
	...
	bmi.s	.done
	move.l	a4,a2
	moveq	#0,d4	; glyph y offset
.row_loop
	move	(a1)+,d3	; image/fill bits
	move	(a2)+,d0	; cookie/mask bits
	moveq	#0,d2	; glyph x offset 0..7
.bit_loop
	cmp	#8,d2
	bge.s	.next_row
	moveq	#15,d1
	...
	bra.s	.no_pixel
.outline_pixel
	move	p96menu_native_colour,p96menu_native_saved_colour
	clr	p96menu_native_colour
	jsr	g2p96_menu_native_put_pixel
	move	p96menu_native_saved_colour,p96menu_native_colour
.no_pixel
	addq	#1,d2
	bra.s	.bit_loop
.next_row
