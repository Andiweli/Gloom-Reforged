...
.wait2	tst	showflag
	...
	bsr	vwait
	bra.s	.wait2
.waitskip2	; c86zey: delayed P96 full-source probe, AGA high+low palette test
	jsr	g2p96_gameplay_present_probe	;c86zey: delayed full-source, AGA high+low palette probe
	jsr	doc2p
	jsr	db
	ifne	g2p96_overlay_enabled
...
; -----------------------------------------------------------------------------
; c86zey: delayed direct RGB565 full-source gameplay probe using the old proven c86zds
; source-byte -> RGB565 LUT path, now testing AGA high+low palette precision.
; It keeps the confirmed delayed full-source timing from c86zex, but for AGA
; palette entries it now reads both the high and low RGB12 words before
; reducing to RGB565.  This should be closer to the warm AGA palette than
; the earlier high-word-only conversion.
; -----------------------------------------------------------------------------
...
p96gameplay_delay_counter	dc	0	;c86zey: wait a few gameplay presents before sampling final wall columns
...
p96gameplay_msg_ok	dc.b	'P96 gameplay: delayed full-source AGA8 palette OK',10
...
g2p96_gameplay_present_probe
	...
	bne	.done
	; c86zey: do not sample the very first gameplay present.  The earlier
