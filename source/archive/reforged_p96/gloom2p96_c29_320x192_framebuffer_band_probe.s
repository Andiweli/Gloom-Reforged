...
p96prevlog_name	dc.b	'RAM:gloom_p96_preview.txt',0
	even
p96prevlog_ok	dc.b	'P96 preview: 320x192 framebuffer band probe active',10
...
.c22done
	...
	;gloom2p96_c29: extend the proven c28 bottom stripe into a small
	;multi-line diagnostic band. It still consumes only the hidden 320x192
	;framebuffer mirror and still uses the proven Runfill/RectFill path.
	;This is not a full output switch yet; it just proves more framebuffer
	;rows can be read back and displayed safely.
	move.l	p96preview_frameptr(pc),d0
	beq	.restore
	move.l	d0,a4		;base of hidden 320x192 framebuffer
	moveq	#0,d6		;band row 0..7
.c29bandloop
	cmp	#8,d6
	bge	.restore
	move	d6,d0
	mulu	#12,d0
	add	#48,d0		;source framebuffer rows 48..132
	mulu	#320,d0
	lea	0(a4,d0.l),a2
