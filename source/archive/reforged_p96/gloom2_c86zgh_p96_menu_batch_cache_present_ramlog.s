...
g2release_marker	dc.b	'Gloom Reforged AGA/P96 probe (v1.4 c86zgh 09.07.2026) Andreas ',39,'Andiweli',39,' Stuermer',0
	even
...
p96gameplay_msg_ok	dc.b	'P96 gameplay: native bigfont2 cached menu buildfix OK',10
...
g2p96_ingame_menu_floorceil_live_refresh
	...
	jsr	g2p96_gameplay_persistent_update	;c86zgc: refresh paused gameplay frame with HUD intact
	clr	p96gameplay_skip_aga_present
	jsr	g2p96_menu_bridge_all_rows_present	;c86zgc: fast glyph-only menu overlay
	bra.s	.refresh_done
.aga_refresh
	jsr	g2v190aj_grey_menu_backdrop
.refresh_done
	jsr	opton		;c86zfx: opton itself now copies only the selected row
	jsr	g2v17_wait_menu_release
	addq	#2,a7
	movem.l	(a7)+,d0/a4
	rts
...
g2p96_menu_bridge_all_rows_present
	movem.l	d0-d7,-(a7)
	move	curropt,-(a7)
	; c86zgh: build the whole visible menu text region in a RAM cache first.
	; Earlier c86zgf cached only one row at a time, so the initial ESC menu and
	; FLOOR/CEILING refresh still appeared row-by-row on real A1200 RTG hardware.
	; If the large cache cannot be allocated, the old safe row-cache path remains.
