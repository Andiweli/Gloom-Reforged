...
; -----------------------------------------------------------------------------
; c86zen: bounded 160x96 gameplay palette/colour probe after c86zem proved burst updates
; on emulator and real Amiga. This still does not skip the normal AGA doc2p/db
; present path. It opens the proven 320x240 P96 screen/window briefly from the
; gameplay context, draws the 160x96 monochrome block map a few times with a
; tiny frame phase change, then closes everything and logs the result to RAM.
; -----------------------------------------------------------------------------
p96gameplay_state		dc	0	;0=fail, 1=160x96 palette OK, 2=skip AGA, 3=no P96 lib, 4=no ModeID, 5=no intuition, 6=screen fail, 7=window fail, 8=no graphics
...
p96gameplay_msg_ok	dc.b	'P96 gameplay: 160x96 palette 3sec OK',10
...
.have_p96
	...
	bra	.log
.try_marker
	jsr	g2p96_gameplay_160x96_palette_probe
.log
...
g2p96_gameplay_160x96_palette_probe
	push
	move.l	p96modeid,d0
	beq	.nomode
	move.l	d0,p96winprobe_displayid_tag+4
	...
	beq	.nogfx
	bsr	g2p96_gameplay_init_pen64_map
	move	#1,p96gameplay_state
	moveq	#2,d7	;3 bounded colour mini-frame updates, about 3 seconds total
.burst_loop
...
; a0 = Window. Deliberately primitive and pen-0/1 only.  This is still a
; 160x96 blocky monochrome view sampled from the current chunky gameplay buffer.
; c86zem calls it a few times inside one bounded burst to prove repeated P96
; gameplay updates without turning this into the old long HoldScreen path.
g2p96_gameplay_draw_mini_frame_160x96
	...
	; c86zen: fixed colour calibration ramp.  If this is still grey/black only,
	; the Pen/ColorMap path is collapsing and the next step must be true RGB.
	bsr	g2p96_gameplay_draw_pen64_ramp

	move.l	chunky,d0
	beq	.fallback
	move.l	d0,a3	;base chunky pointer
	moveq	#0,d7	;row 0..95
	move	#32,d5	;screen y start, leaves top ramp visible
.rowloop
	cmp	#96,d7
	bge	.done
	move	d7,d0
	mulu	#2,d0	;source y step, deliberately rough
	mulu	chunkymodw,d0
	lea	0(a3,d0.l),a2	;source row pointer
	moveq	#0,d6	;column 0..159
	move	#0,d4	;screen x offset in pixels
.colloop
	cmp	#160,d6
	bge	.nextrow
	move	d6,d0
	mulu	#2,d0	;source x step
	moveq	#0,d3
	move.b	0(a2,d0.w),d3	;live chunky pixel
	lsr	#2,d3	;64 colour-index candidates from current chunky data
	move	p96gameplay_burst_phase,d0
	eor	d0,d3	;small phase change per mini-frame
	and	#$003f,d3

	; If BestPen map is valid, translate the 64 candidates to real screen pens.
	; Fallback keeps raw low pens visible instead of failing silently.
	moveq	#0,d0
...
.havepen
	move.l	50(a5),a1
	move.l	p96gameplay_grbase,a6
	jsr	-342(a6)	;SetAPen selected colour/pen
	move.l	50(a5),a1
	move.l	p96gameplay_grbase,a6
	move	d4,d0
	move	d5,d1
	...
	move	d5,d3
	add	#1,d3
	jsr	-306(a6)	;2x2 mini-frame cell
	add	#2,d4
	addq	#1,d6
	bra	.colloop
.nextrow
...
.fallback
	...
	move.l	p96gameplay_grbase,a6
	moveq	#1,d0
	jsr	-342(a6)	;SetAPen visible fallback
	move.l	50(a5),a1
	move.l	p96gameplay_grbase,a6
	move	#40,d0
	move	#80,d1
	move	#279,d2
	move	#159,d3
	jsr	-306(a6)	;fallback if no chunky pointer exists
.done
	pull
	rts
