...
; -----------------------------------------------------------------------------
; c86zeq: first direct RGB565 gameplay mini-frame probe.  c86zep proved that
; direct 16-bit P96 bitmap writes show real colours, while SetAPen/BestPen
; collapsed.  This still does not skip the normal AGA doc2p/db present path.
; It opens the proven 320x240 P96 screen/window briefly from the gameplay
; context, writes a tiny RGB565 calibration ramp, then draws a 160x96 blocky
; mini-frame directly from the current chunky gameplay buffer into the P96 bitmap.
; -----------------------------------------------------------------------------
...
p96gameplay_msg_ok	dc.b	'P96 gameplay: direct RGB565 mini-frame OK',10
...
.colloop
	...
	; RGB332-ish direct RGB565 pseudo-colour from the chunky byte.
	; This is not the final Gloom palette yet; it is only to prove direct colour
	; gameplay image transport through the P96 bitmap.
	move	d3,d1
	and	#$00e0,d1
