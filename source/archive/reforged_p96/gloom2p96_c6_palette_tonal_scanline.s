	...
	;gloom2p96_c6: one-scanline preview with Gloom palette tonal mapping.
	;
...
p96prevlog_name	dc.b	'RAM:gloom_p96_preview.txt',0
	even
p96prevlog_ok	dc.b	'P96 preview: palette tonal scanline active',10
...
.nolog	movem.l	d0-d7/a0-a6,-(a7)
	...
	beq.w	.restore
	lea	coloffs(pc),a3
	move	#120,d6		;sample stable mid-screen source row
	move	d6,d7
	mulu	chunkymodw(pc),d7
	move.l	a4,a2
	add.l	d7,a2		;Chunky/C2P row base
...
.c5xloop
	...
	; gloom2p96_c6: c5 proved that the live Chunky source is stable,
	; but raw indices mapped to Workbench pens looked unrelated to the
	; game.  Convert the sampled palette index through the current Gloom
	; RGB12 palette into a simple brightness bucket.  This still uses safe
	; RectFill pens only, but the scanline should now follow the game's
	; light/dark structure instead of random low-bit colours.
	move.l	planar_palette(pc),a0
	tst.l	a0
	beq.s	.c6fallback
	move	d0,d1
	...
	bra.s	.c6paladdr
.c6ecs	add	d1,d1		;ECS palette entry = 1 word
.c6paladdr
...
.c6mapped
	...
	move	d5,d0		;draw only one coarse scanline band
	move	#32,d1
	move	d5,d2
	addq	#7,d2
	move	#39,d3
