...
p96prevlog_name	dc.b	'RAM:gloom_p96_preview.txt',0
	even
p96prevlog_ok	dc.b	'P96 preview: c77 selective green-only probe active',10
...
g2p96_c73_palready
	...
	; gloom2p96_c77: c76 blue-cut made green areas brown.  Go back to
	; the c75 colour anchor and only nudge entries which are already
	; clearly green-dominant.  No red/blue reduction and no global green
	; boost, so grey/brown textures should not turn into Shrek mode.
	move	d6,d4
	move	d4,d0
	and	#$0f00,d0
	lsr	#8,d0		;R nibble
	...
	cmp	d0,d1
	ble	.c77nogreen	;only if G > R
	cmp	d2,d1
	ble	.c77nogreen	;only if G > B
	cmp	#$000f,d1
	beq	.c77nogreen	;already max green
	add	#$0010,d4
.c77nogreen
	move	d4,d6
