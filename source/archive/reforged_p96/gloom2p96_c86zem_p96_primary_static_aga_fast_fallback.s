...
p96prevlog_name	dc.b	'RAM:gloom_p96_preview.txt',0
	even
p96prevlog_ok	dc.b	'P96 preview: c86zem static AGA fast fallback',10
...
; c86zeg: update only the text strip whose Y is already in d6.
; Used by slow intermission/menu text paths so real Amiga input is not blocked
; by a complete 320x240 planar conversion after every small text change.
...
g2p96_present_planar_showbitmap_to_window
	; c86zem: real-Amiga static screens were too slow when converted
	; Planar->RGB line-by-line into RTG.  For now use a fast hybrid:
	; static title/menu/intermission pages stay on the original AGA screen,
	; while gameplay remains P96-primary.  Keep the c86zek audio/hold cadence.
	movem.l	d0/a0/a6,-(a7)
	clr	p96preview_gameactive
	move.l	int,d0
	beq.s	.g2p96zem_static_done
	move.l	screen,d0
	beq.s	.g2p96zem_static_done
	move.l	d0,a0
	move.l	int,a6
	jsr	-252(a6)	; Intuition ScreenToFront(screen): show AGA static page fast
.g2p96zem_static_done
	movem.l	(a7)+,d0/a0/a6
	rts

; c86zem: old P96 planar bridge kept below as dormant reference.
g2p96_present_planar_showbitmap_to_window_slow_p96_reference
	movem.l	d0-d7/a0-a6,-(a7)
	tst	p96preview_gameactive
	bne.w	.rts
	tst	p96preview_state
...
.common
	...
	tst	g2p96_rect_active	;c86zeg: optional dirty strip mode
	beq.s	.full_area
