...
;gloom2_c86zdi: stronger near vertical oval, whole projectile/upgrade reflection Bayer-dithered, weapon-upgrade floor-touch pulse.
	...
	;c86zha: P96 title qmenu black clear, faster title Bigfont delta, and skip duplicate brush decode.
...
g2release_marker	dc.b	'Gloom Reforged AGA/P96 probe (v1.4 c86zha 09.07.2026) Andreas ',39,'Andiweli',39,' Stuermer',0
	even
...
.nocd	;
	...
	jsr	g2p96_static_buffer_probe	;c86zec: static input probe, closes by input or timeout
	move.l	4.w,a6
	moveq	#1,d1
	jsr	-216(a6)
...
.draw	jsr	g2v142_draw_gloombrush_safe	; no brush loaded = safe no-op for classic Gloom
	move	#-1,g2p96_title_brush_composed	;c86zha: avoid decoding/presenting brush again on first menu draw
...
g2v145_show_clean_title
	clr	g2p96_title_brush_composed	;c86zha: clean title invalidates composed brush cache
	move.l	gloom(pc),a0
	move.l	gloompal(pc),a1
...
dointro	;
	clr	p96title_qmenu_black	;c86zha: normal title menu uses title background, not quick-menu black
...
.redrawmenu
	...
	tst	g2p96_title_brush_composed	;c86zha: initial title path already drew/presented brush
	bne	.g2c86zha_brush_ready
	jsr	g2v145_draw_menu_gloombrush
.g2c86zha_brush_ready
	tst	linked
...
qmenu	;quick menu in a0
	;on black screen
	;
	move	#-1,p96title_qmenu_black	;c86zha: P96 quick menus must clear title/menu background first
	move.l	a0,-(a7)
	bsr	clspic
	move.l	(a7)+,a4
	bsr	initmenu
...
finitpmenu	;
finitqmenu	clr	p96title_qmenu_black	;c86zha: quick-menu black applies only while the quick menu is active
	bsr	dispoff
	bra	finitmenu
...
g2p96_title_brush_composed	dc	0	;c86zha: title+brush already composed for first menu draw
...
p96gameplay_msg_ok	dc.b	'P96 gameplay: navguard bigfont + staged static/title row OK',10
...
p96title_qmenu_black	dc	0	;c86zha: quick/remote menus use a black P96 backdrop, not the title cache
	cnop	0,4
...
g2p96_title_menu_bridge_current_row_present
	...
	jsr	g2p96_title_menu_draw_current_row_delta_cached	;c86zha: faster real-Bigfont title row redraw
	movem.l	(a7)+,d0-d1
	rts
...
g2p96_title_menu_blink_current_row_on
	...
	move	#-1,p96menu_native_selected
	jsr	g2p96_title_menu_native_restore_current_row
	jsr	g2p96_title_menu_draw_current_row_delta_cached	;c86zha: keep blink fast and Bigfont-accurate
	movem.l	(a7)+,d0
	rts
...
g2p96_title_menu_draw_current_row_delta_cached
	movem.l	d0-d7/a0-a6,-(a7)
	...
	jsr	g2p96_menu_cache_begin
	move	#-1,p96menu_cache_active
	jsr	g2p96_static_title_menu_draw_current_row_delta
	clr	p96menu_cache_active
	jsr	g2p96_menu_cache_flush
.done
	clr	p96menu_cache_active
...
g2p96_static_present_intermission_text_skip_if_bridge
	; c86zgx: intermission text is deliberately not sent through the broken
	; fullscreen font converter.  Picture staging remains active through showpic;
	; intermission text will get its own native text bridge in the next pass.
	rts
...
g2p96_static_present_title_menu_bigfont_if_bridge
	...
	beq	.done
	jsr	g2p96_menu_build_rgb565_lut
	jsr	g2p96_title_menu_clear_black_if_qmenu	;c86zha: remote/link quick menu must not overlap title menu
...
.row_loop
	...
	bge	.flush
	move	d7,curropt
	jsr	g2p96_static_title_menu_draw_current_row_delta	;c86zha: byte-delta Bigfont scan is faster than per-pixel bounded scan
	addq	#1,d7
	bra	.row_loop
.flush
...
; c86zha: Clear the visible P96 title surface and the staged row-restore
; buffer for black quick menus such as REMOTE LINK OPTIONS.  Without this,
; qmenu text is drawn over the previous title menu because the title bridge
; normally restores rows from the cached title picture.
g2p96_title_menu_clear_black_if_qmenu
	movem.l	d0-d7/a0-a6,-(a7)
	tst	p96title_qmenu_black
	beq	.done
	move.l	p96static_rgbbufptr,d0
	beq	.skip_buf
	move.l	d0,a0
	...
	mulu	d2,d1
	move.l	p96static_rgbbufsize,d3
	cmp.l	d1,d3
	blo	.skip_buf
	lsr.l	#2,d1
	beq	.skip_buf
