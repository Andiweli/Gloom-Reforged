	...
	;gloom2p96_c86zd8: P96 Workbench-hold true Gloom palette BestPen grid after confirmed RGB-axis BestPen gradient.
	;gloom2p96_c86zda: first confirmed visible/playable P96 live-preview bridge, but with pointer, bottom band and heavy stutter.
	;gloom2p96_c86zdb: stabilize live preview: throttle updates, hide P96 pointer, force input active, remove duplicate bottom mini-band.
	;gloom2p96_c86zdc: keep P96 preview window active and use clean black letterbox instead of grey unused band.
	;gloom2p96_c86zdd: keep raw game input active while P96/Workbench live preview is frontmost; throttle P96 preview to every 8th frame.
	;gloom2p96_c86zdp: fix RGB target autodetect: use Workbench Screen.Width with BytesPerRow, not pitch alone.
	;gloom2p96_c86zdq: performance pass 1 for real P96 RGB path: one live hook only, throttle Workbench-front calls, lazy RGB LUT.
	;
...
debugser	equ	0
debugmem	equ	0
g2p96_overlay_enabled	equ	-1	;c86zc8: re-enable old safe c85/c59 P96 preview path on the c86zc7 gameplay/reflection base
g2p96_workbench_hold_enabled	equ	-1	;c86zd0: open/hold P96 window on Workbench before custom AGA screen opens
g2p96_workbench_front_enabled	equ	-1	;c86zdb: Workbench-front P96 live preview, stabilized after c86zda
	;
...
.nocd	;
	...
	;
.intro	clr	p96preview_gameactive	;gloom2p96_c14: title/menu phase, do not preview stale game buffer
...
.play	;
	...
	ifne	g2p96_overlay_enabled
	jsr	g2p96_open_native_for_gameplay	;c86zeu3: late native P96 screen only after title/static phase
	endc
...
.wmf	tst	fadevol
	...
	beq.s	.n22
	bsr	swaphflags
.n22	clr	p96preview_gameactive	;gloom2p96_c14: leaving gameplay for title/menu
	ifne	g2p96_overlay_enabled
	jsr	g2p96_return_to_workbench_for_static	;c86zeu3: close native P96 before title/music/static screens
...
dogamemenu	;
	...
	ifne	g2p96_overlay_enabled
	jsr	g2p96_return_to_workbench_for_static	;c86zeu3: close native P96 before in-game menu/static backdrop
...
.done	jsr	g2cfg_save	;v141: save ingame menu settings when leaving menu
	...
	ifne	g2p96_overlay_enabled
	jsr	g2p96_open_native_for_gameplay	;c86zeu3: restore native P96 gameplay screen after menu
	endc
	bra.s	.findone
.finish_exit
	jsr	dispoff
...
.wait2	tst	showflag
	...
	bsr	vwait
	bra.s	.wait2
.waitskip2	; c86m: normal C2P/show path restored after c86l label regression
	ifne	g2p96_overlay_enabled
	; c86zds: P96-primary performance probe.  Once the live P96 window
	; is active during gameplay, skip the old AGA C2P/db presentation and
	; send the freshly rendered chunky frame directly to RTG.  The AGA
	; path is still kept for title/menu/fallback and for two-player.
	tst	twowins
	bne.s	.g2c86zds_do_aga_present
	tst.l	p96preview_window
	beq.s	.g2c86zds_do_aga_present
	tst	p96preview_state
	beq.s	.g2c86zds_do_aga_present
	tst	p96preview_gameactive
	beq.s	.g2c86zds_do_aga_present
	jsr	g2p96_preview_frame
	ifne	g2p96_workbench_front_enabled
	move	p96preview_frame,d0
	and	#31,d0
	bne.s	.g2c86zds_no_front_refresh
	jsr	g2p96_workbench_front_for_preview
.g2c86zds_no_front_refresh
	endc
	jsr	g2twop_restore_after_c2p
	clr	showflag
	rts
.g2c86zds_do_aga_present
	endc
	jsr	doc2p
	jsr	db
	ifne	g2p96_overlay_enabled
	...
	ifne	g2p96_workbench_front_enabled
	; c86zdq: do not force ScreenToBack/ActivateWindow/SetPointer every
	; game frame. That OS churn was harmless for tiny probes but very
	; expensive once the live path became a full RGB framebuffer copy.
	move	p96preview_frame,d0	;c86zdr: absolute, GenAm PC-range fix
	and	#31,d0		;refresh Workbench/P96 front roughly every 32 preview calls
	bne.s	.g2c86zdq_no_front_refresh
	jsr	g2p96_workbench_front_for_preview	;keep Workbench/P96 visible, but not every frame
.g2c86zdq_no_front_refresh
	endc
...
.row_loop
	; about 80 percent weaker than a 1:1 copy: sparse top, sparser tail.
	moveq	#4,d3		; top: clearer at the wall foot
	cmp	#4,d5
	blt.s	.have_thr
	moveq	#2,d3		; early fade down
	cmp	#10,d5
	blt.s	.have_thr
	moveq	#1,d3		; coarse tail
.have_thr
...
g2_draw_reflection_column_after_sprite
	...
	bne.s	.no_projectile_reflect
	bsr	g2_draw_enemy_blob_column
.no_projectile_reflect
	tst	g2_enemy_ref_active
	ble.s	.rts
...
.osvwait	movem.l	d0-d1/a0-a1/a6,-(a7)
	...
	ifne	g2p96_overlay_enabled
	; c86zdq: the direct RGB P96 path is driven only from drawall after
	; doc2p/db. Keeping this old OS-vwait hook active doubles the number
	; of preview attempts and creates heavy real-hardware stutter.
	; The label stays so the surrounding old control flow remains stable.
.g2c59_no_p96_vwait
	endc
	rts
...
.osskipz	;
	...
	;
	ifne	g2p96_overlay_enabled
	;c86zdg: allocate the 160x120 preview staging buffer at runtime
	...
	;
	;c86zdg: allocate a parallel 160x120 RGB12 word buffer.
	...
	;
	;c86zdg: allocate a full 320x240 linear mirror buffer.
...
; v190p96a2: RAM marker for real-hardware tests.  This file is written
; immediately after the passive P96 probe, before any later crash can hide
; the result.  Failure to write the marker is silent.
...
p96winlog_native_ok	dc.b	'P96 window: native 320x240 OK',10
p96winlog_native_ok_len	equ	*-p96winlog_native_ok
p96winlog_fallback_ok	dc.b	'P96 window: Workbench fallback OK',10
p96winlog_fallback_ok_len	equ	*-p96winlog_fallback_ok
...
; c86zeu: native 320x240 P96 screen probe logs.  This first probe only
; asks Picasso96 for a matching ModeID, then tries p96OpenScreenTagList.
; If either step fails, the old Workbench/P96 window path is used unchanged.
p96modelog_name	dc.b	'RAM:gloom_p96_modeid.txt',0
	even
p96modelog_ok_pc	dc.b	'P96 mode: 320x240 16-bit PC found',10
p96modelog_ok_pc_len	equ	*-p96modelog_ok_pc
p96modelog_ok_bgr	dc.b	'P96 mode: 320x240 16-bit BGR found',10
p96modelog_ok_bgr_len	equ	*-p96modelog_ok_bgr
p96modelog_fail	dc.b	'P96 mode: 320x240 16-bit not found',10
p96modelog_fail_len	equ	*-p96modelog_fail
p96modelog_skip	dc.b	'P96 mode: skipped, no P96 library',10
p96modelog_skip_len	equ	*-p96modelog_skip
	even
p96screenlog_name	dc.b	'RAM:gloom_p96_screen.txt',0
	even
p96screenlog_ok	dc.b	'P96 screen: p96OpenScreenTagList OK',10
p96screenlog_ok_len	equ	*-p96screenlog_ok
p96screenlog_fail	dc.b	'P96 screen: p96OpenScreenTagList failed',10
p96screenlog_fail_len	equ	*-p96screenlog_fail
p96screenlog_skip	dc.b	'P96 screen: skipped, no ModeID',10
p96screenlog_skip_len	equ	*-p96screenlog_skip
	even
...
p96prevlog_name	dc.b	'RAM:gloom_p96_preview.txt',0
	even
p96prevlog_ok	dc.b	'P96 preview: c86zeu9 native WPA pen bands probe active',10
...
p96mode_state	dc	0	;0=skipped/no p96, 1=ModeID found, -1=not found
p96mode_try	dc	0	;0=none, 1=16-bit PC, 2=16-bit BGR
p96native_late_failed	dc	0	;c86zeu3: do not repeatedly try native gameplay screen after fail
p96screen_state	dc	0	;0=not tried/skipped, 1=own P96 screen OK, -1=failed
p96native_state	dc	0	;0=Workbench fallback window, 1=own native P96 screen active
...
p96rgb_lut_ready	dc	0	;c86zdq: build RGB565/RGB888 source LUT lazily once during live gameplay
...
p96preview_frameptr	dc.l	0	;c86zdg: optional 320x240 full-frame mirror buffer
p96preview_rgbptr	dc.l	0	;c86zdg: 160x120 RGB12 word sample buffer
...
p96preview_window	dc.l	0	;persistent P96 preview window pointer
p96preview_screen	dc.l	0	;c86zeu: own p96OpenScreenTagList screen pointer, if opened
p96mode_id	dc.l	0	;c86zeu: DisplayID returned by p96BestModeIDTagList

; c86zeu P96 tag constants used numerically to keep Devpac/GenAm standalone.
; TAG_USER=$80000000, P96BIDTAG_Dummy=TAG_USER+96, P96SA_Dummy=TAG_USER+$20000+96.
p96bestmode_tags
	dc.l	$80000061	;P96BIDTAG_FormatsAllowed
p96bestmode_formatsallowed	dc.l	0
	dc.l	$80000063	;P96BIDTAG_NominalWidth
	dc.l	320
	dc.l	$80000064	;P96BIDTAG_NominalHeight
	dc.l	240
	dc.l	$80000065	;P96BIDTAG_Depth
	dc.l	16
	dc.l	0,0	;TAG_DONE

p96screen_tags
	dc.l	$80020061	;P96SA_Left
	dc.l	0
	dc.l	$80020062	;P96SA_Top
	dc.l	0
	dc.l	$80020063	;P96SA_Width
	dc.l	320
	dc.l	$80020064	;P96SA_Height
	dc.l	240
	dc.l	$80020065	;P96SA_Depth
	dc.l	16
	dc.l	$80020068	;P96SA_Title
	dc.l	p96test_title
	dc.l	$8002006d	;P96SA_Type
	dc.l	15	;CUSTOMSCREEN
	dc.l	$80020072	;P96SA_DisplayID
p96screen_displayid	dc.l	0
	dc.l	$80020076	;P96SA_Quiet
	dc.l	1
	dc.l	$8002007d	;P96SA_RGBFormat
p96screen_rgbformat	dc.l	4	;RGBFB_R5G6B5PC by default
	dc.l	0,0	;TAG_DONE

p96test_window_wb	dc	20,20	;x,y on Workbench/public native screen
	dc	320,240	;w,h
	dc.b	0,1	;pens
	dc.l	0	;no IDCMP needed for this P96 output window
...
p96test_window_native	dc	0,0	;x,y on own P96 screen
	dc	320,240	;w,h
	dc.b	0,1	;pens
	dc.l	0	;no IDCMP needed for this P96 output window
	dc.l	$1840	;ACTIVATE|BORDERLESS|SIMPLE_REFRESH, no gadgets
	...
	dc.l	0	;checkmark
	dc.l	p96test_title
p96test_window_screen	dc.l	0	;runtime: own P96 screen pointer
	dc.l	0	;bitmap
	dc	-1,-1,-1,-1	;mins/maxs
	dc	15	;CUSTOMSCREEN

p96test_title	dc.b	'Gloom P96 320x240',0
	even

; v190p96a: safe first-stage probe.  OpenLibrary failure is silent and
; leaves the normal AGA path exactly as before.
g2p96_probe	push
	clr.l	p96base
	clr	p96present
	clr	p96found
...
.got	move.l	d0,p96base
	move	#-1,p96present
.done	jsr	g2p96_probe_log	;v190p96a2: write RAM:gloom_p96_probe.txt
	pull
	rts
...
g2p96_test_window	push
	...
	clr	p96mode_state
	clr	p96mode_try
	clr	p96native_late_failed
	clr	p96screen_state
	clr	p96native_state
	clr	p96preview_state
	clr	p96preview_frame
	clr	p96rgb_lut_ready	;c86zdq: force first live frame to build RGB LUT
	clr	p96preview_live_logged
	clr	p96bitmap_logged	;c86zeu4: allow one fresh native bitmap diagnostic per run
	clr.l	p96preview_window
	clr.l	p96preview_screen
	clr.l	p96mode_id
	tst	p96present
	beq	.logonly
	jsr	g2p96_find_320x240_modeid
	tst	p96mode_state
	ble	.fallback
	; c86zeu2: probe the native P96 screen/window only, then close it
	; before the title/music/gameplay path starts.  c86zeu1 proved that the
	; native screen opens, but keeping it as the live output hid all static
	; AGA screens and brought back the music whine on real hardware.
	jsr	g2p96_open_native_screen_window
	tst.l	p96preview_window
	beq	.fallback
	jsr	g2p96_close_native_probe_only
.fallback
	jsr	g2p96_open_workbench_fallback_window
	tst.l	p96preview_window
	bne	.opened
	move	#-1,p96window_state
	bra	.logonly
.opened
	clr	p96bitmap_logged	;c86zeu4: log actual gameplay target after native/fallback switch
	move	#1,p96window_state
	move	#1,p96preview_state
	jsr	g2p96_hide_p96_pointer	;c86zdb: hide mouse pointer inside output window
	jsr	g2p96_keep_preview_active	;c86zdc: activate P96 output window immediately
	move.l	p96preview_window(pc),a0
	bsr	g2p96_draw_testpattern
	jsr	g2p96_c71_init_bestpen_map	; c71: build RGB332->Workbench/P96 best-pen map once
	; c86zd1: while the P96 window is still visible before AGA,
	; try the exact Gloom palette -> BestPen map and draw it.
	; This is diagnostic only: no Live-WPA, no AGA output change.
	jsr	g2p96_c73_init_palette_pen_map
	move.l	p96preview_window(pc),a0
	bsr	g2p96_draw_palette_hold_probe
.logonly
	jsr	g2p96_mode_log
	jsr	g2p96_screen_log
	jsr	g2p96_window_log
	jsr	g2p96_preview_log
	pull
	rts

; c86zeu: ask Picasso96 for a 320x240 16-bit ModeID.  Prefer 16-bit
; PC byte order first, then BGR as fallback.  No requester, no user choice.
g2p96_find_320x240_modeid
	push
	clr	p96mode_state
	clr	p96mode_try
	clr.l	p96mode_id
	...
	beq	.fail
	move.l	d0,a6
	move.l	#16,p96bestmode_formatsallowed	;RGBFF_R5G6B5PC
	lea	p96bestmode_tags(pc),a0
	jsr	-60(a6)	;p96BestModeIDTagList(tags)
	cmp.l	#-1,d0	;INVALID_ID
	beq	.try_bgr
	move.l	d0,p96mode_id
	move	#1,p96mode_try
	move	#1,p96mode_state
	bra	.done
.try_bgr
	move.l	#4096,p96bestmode_formatsallowed	;RGBFF_B5G6R5PC
	lea	p96bestmode_tags(pc),a0
	jsr	-60(a6)	;p96BestModeIDTagList(tags)
	cmp.l	#-1,d0	;INVALID_ID
	beq	.fail
...
; c86zeu: open a real P96API screen, then a borderless 320x240 window
; on that screen.  On any failure, leaves p96preview_window=0 so the caller
; can fall back to the old Workbench/P96 window path.
g2p96_open_native_screen_window
	push
	clr	p96screen_state
	clr	p96native_state
	clr.l	p96preview_screen
	clr.l	p96preview_window
	move.l	p96mode_id(pc),d0
	beq	.fail_screen
	move.l	d0,p96screen_displayid
	move	p96mode_try(pc),d0
	cmp	#2,d0
	beq	.bgr
	move.l	#4,p96screen_rgbformat	;RGBFB_R5G6B5PC
	bra	.open_screen
.bgr
	move.l	#12,p96screen_rgbformat	;RGBFB_B5G6R5PC
.open_screen
	move.l	p96base,d0
	beq	.fail_screen
	move.l	d0,a6
	lea	p96screen_tags(pc),a0
	jsr	-90(a6)	;p96OpenScreenTagList(tags)
	move.l	d0,d7
	beq	.fail_screen
	move	#1,p96screen_state
	move.l	d7,p96preview_screen
	move.l	d7,p96test_window_screen
	move.l	int,d0
	beq	.fail_window
	move.l	d0,a6
	lea	p96test_window_native(pc),a0
	jsr	-204(a6)	;OpenWindow on own P96 screen
	move.l	d0,d7
	beq	.fail_window
	move.l	d7,p96preview_window
	move	#1,p96native_state
	bra	.done
.fail_window
	; Window failed after screen opened. Close the P96API screen before fallback.
	move.l	p96preview_screen(pc),d0
	beq	.fail_screen
	move.l	p96base,d1
	beq	.clear_screen
	move.l	d0,a0
	move.l	d1,a6
	jsr	-96(a6)	;p96CloseScreen, not intuition CloseScreen
.clear_screen
	clr.l	p96preview_screen
	clr.l	p96test_window_screen
.fail_screen
	move	#-1,p96screen_state
.done
	pull
...
; c86zeu: old proven Workbench/P96 window path, kept as automatic fallback.
g2p96_open_workbench_fallback_window
	push
	clr	p96native_state
	move.l	int,d0
	beq	.done
	move.l	d0,a6
	lea	p96test_window_wb(pc),a0
...
; c86zeu2: close the temporary native P96 probe screen/window before
; normal startup continues.  Keep p96mode_state/p96screen_state intact so
; RAM:gloom_p96_modeid.txt and RAM:gloom_p96_screen.txt still report whether
; the native fullscreen probe itself worked.
g2p96_close_native_probe_only
	push
...
.checkscreen
	...
	move.l	d0,a0
	move.l	d1,a6
	jsr	-96(a6)	;p96CloseScreen for p96OpenScreenTagList screens
.clearscreen
	clr.l	p96preview_screen
	clr.l	p96test_window_screen
.clearstate
...
; c86zeu3: switch from the safe Workbench/P96 fallback window to the real
; native 320x240 P96 screen only after gameplay is active.  c86zeu1 proved
; that keeping the native screen open during title/music/static screens brings
; the high music whine back and hides the old static screens.  Therefore this
; routine is called late, after initnewgame/menu selection, and never from the
; boot/title path.
g2p96_open_native_for_gameplay
	push
	tst	p96present
	beq	.done
	tst	p96preview_gameactive
	...
	tst	p96native_state
	bne	.done
	tst	p96native_late_failed
	bne	.done
	tst	p96mode_state
	ble	.done
	; Close the Workbench fallback preview window before opening the native
	; screen, otherwise both RTG outputs fight for visibility/input.
	move.l	p96preview_window(pc),d0
	beq.s	.open_native
	move.l	d0,a0
	move.l	int,d0
	beq.s	.clear_old_window
	move.l	d0,a6
	jsr	-72(a6)	;CloseWindow
.clear_old_window
	clr.l	p96preview_window
.open_native
	...
	tst.l	p96preview_window
	bne.s	.opened
	move	#-1,p96native_late_failed
	; If native gameplay open failed, restore the proven Workbench path.
	jsr	g2p96_open_workbench_fallback_window
	tst.l	p96preview_window
	beq.s	.done
.opened
	move	#1,p96window_state
	move	#1,p96preview_state
	jsr	g2p96_hide_p96_pointer
	jsr	g2p96_keep_preview_active
	move.l	p96preview_window(pc),a0
	tst.l	a0
	beq.s	.done
	bsr	g2p96_draw_testpattern
	; c86zeu9: native-screen p96WritePixelArray writes pen values on
	; real P96.  Rebuild the exact palette->BestPen map once for the
	; currently active native screen, not the earlier Workbench fallback.
	clr	p96palpen_state
...
; c86zeu3: before title/menu/static phases, close the native P96 screen again
; and return to the old safe Workbench preview window.  This keeps music/static
; timing on the known-good c86zeu2 path while allowing gameplay-only native P96
; tests.
g2p96_return_to_workbench_for_static
	push
	tst	p96native_state
	beq.s	.ensure_fallback
	jsr	g2p96_close_native_probe_only
.ensure_fallback
	move.l	p96preview_window(pc),d0
	bne.s	.have_window
...
; c86zd8: draw the real current Gloom palette as a 16x16 BestPen grid.
; c86zd7 proved that the Workbench/P96 ColorMap + ObtainBestPenA path can
; produce a readable RGB gradient.  This step switches the hold-window test
; back to the real Gloom palette-index map instead of synthetic RGB axes.
; If the active game palette is not available this early, the routine falls
; back to raw Pen index geometry so the window still shows a visible grid.
; a0 = Window
g2p96_draw_palette_hold_probe
	push
	move.l	a0,a5
	move.l	a5,d0
	beq	.done
	...
	; Rebuild/refresh the exact palette-index -> Workbench/P96 Pen map.
	; The caller already did this once, but doing it here makes the visible
	; hold probe self-contained and robust against later call-order changes.
	jsr	g2p96_c73_init_palette_pen_map

	moveq	#0,d7		;palette cell/index 0..255
.cell_loop
	cmp	#256,d7
	bge	.done
	...
	moveq	#0,d0
	move.b	0(a2,d7.w),d0	;screen Pen closest to Gloom palette index
	bne	.set_pen
	moveq	#1,d0		;avoid invisible black holes in the diagnostic grid
	bra	.set_pen
.raw_pen
	move	d7,d0		;fallback: visible raw Pen geometry
	and	#$00ff,d0
	beq	.raw_nonzero
	bra	.set_pen
.raw_nonzero
...
.set_pen
	...
	; Coordinates are calculated after SetAPen, like the confirmed c86zd3 fix.
	move	d7,d6
	and	#15,d6		;x cell 0..15
	move	d6,d0
	mulu	#20,d0		;x1 = cell*20
	move	d0,d2
	add	#19,d2		;x2
	move	d7,d6
	lsr	#4,d6		;y cell 0..15
	move	d6,d1
	mulu	#15,d1		;y1 = cell*15
	move	d1,d3
	add	#14,d3		;y2
	move.l	50(a5),a1
	move.l	grbase,a6
	jsr	-306(a6)	;RectFill cell
...
; c86zdb: hide the Workbench/P96 mouse pointer inside the P96 preview
; window.  The existing g2v36_hide_pointer only targets the hidden game
; oswindow, so the RTG/Workbench pointer remained visible and moved with
; the overlay.
g2p96_hide_p96_pointer
	movem.l	d0-d3/a0-a1/a6,-(a7)
	move.l	p96preview_window(pc),d0
	beq.s	.rts
	move.l	int(pc),d1
	beq.s	.rts
	move.l	chipzero(pc),a1
	move.l	a1,d1
	beq.s	.rts
	move.l	d0,a0
	moveq	#1,d0
	...
	moveq	#0,d3
	move.l	int(pc),a6
	jsr	-270(a6)	;Intuition SetPointer, invisible 1x1 pointer for P96 window
.rts	movem.l	(a7)+,d0-d3/a0-a1/a6
	rts

; c86zdc: keep the P96 preview window active.  On Workbench/RTG setups
; the user can otherwise move the mouse outside the 320x240 preview window
; and Workbench becomes the active input target again.  This is intentionally
; separate from ScreenToBack so it can be removed independently if needed.
g2p96_keep_preview_active
	movem.l	d0/a0/a6,-(a7)
	move.l	p96preview_window(pc),d0
	beq.s	.rts
	move.l	int(pc),d1
	...
	move.l	d0,a0
	move.l	int(pc),a6
	jsr	-450(a6)	;Intuition ActivateWindow(window) on OS3/V36+
.rts	movem.l	(a7)+,d0/a0/a6
	rts

; c86zd9: after the game custom AGA screen exists, push that screen behind
; Workbench again so the already-open P96 preview window remains visible on
; RTG/FrameThrower setups.  The game still renders normally into its hidden
; buffers; g2p96_preview_frame copies the chunky gameplay into the Workbench
; P96 window.  This is a visibility/diagnostic bridge, not a true RTG game mode.
g2p96_workbench_front_for_preview	push
	tst	p96native_state	;c86zeu: own P96 screen uses ScreenToFront instead of Workbench bridge
	bne.s	.native
	move.l	p96preview_window(pc),d0
	beq.s	.done
	move.l	int(pc),d0
	...
	move.l	d0,a0
	move.l	int(pc),a6
	jsr	-246(a6)	;Intuition ScreenToBack(screen): Workbench/P96 should become visible again
	bra.s	.keep
.native
	move.l	p96preview_screen(pc),d0
	beq.s	.done
	...
	move.l	d0,a0
	move.l	d1,a6
	jsr	-252(a6)	;Intuition ScreenToFront(own P96 screen)
.keep
	jsr	g2p96_keep_preview_active	;c86zdc: keep P96 output window active
	jsr	g2p96_hide_p96_pointer	;c86zdb: hide RTG/Workbench pointer over the output window
	jsr	inputon		;c86zdb: keep raw input active even though screen depth changed
.done	pull
	rts

; c86zd0: keep the P96/Workbench window visible before the game opens its
; custom AGA screen.  Continue with normal startup when left mouse button or
; joystick/fire is pressed.  Uses graphics.library WaitTOF to avoid a busy loop.
g2p96_workbench_hold_probe	push
	move.l	p96preview_window(pc),d0
	beq.s	.done
	move.l	grbase(pc),d0
	beq.s	.done
.loop	move.l	grbase(pc),a6
	jsr	-270(a6)	;graphics.library WaitTOF
	btst	#6,$bfe001	;left mouse / port 1 fire, active low
	beq.s	.done
	btst	#7,$bfe001	;joystick port 2 fire, active low
	beq.s	.done
	bra.s	.loop
.done	pull
	rts
...
g2p96_mode_log	push
	...
	move	p96present,d0
	beq.s	.skip
	move	p96mode_state,d0
	bmi.s	.fail
	beq.s	.skip
	move	p96mode_try,d0
	cmp	#2,d0
	beq.s	.ok_bgr
	lea	p96modelog_ok_pc(pc),a0
	move.l	#p96modelog_ok_pc_len,d3
	bra.s	.write
.ok_bgr
	lea	p96modelog_ok_bgr(pc),a0
	move.l	#p96modelog_ok_bgr_len,d3
	bra.s	.write
.fail
	lea	p96modelog_fail(pc),a0
	move.l	#p96modelog_fail_len,d3
	bra.s	.write
.skip
	lea	p96modelog_skip(pc),a0
	move.l	#p96modelog_skip_len,d3
.write
	move.l	d7,d1
	move.l	a0,d2
	jsr	-48(a6)	;Write
...
g2p96_screen_log	push
	...
	bmi.s	.fail
	beq.s	.skip
	lea	p96screenlog_ok(pc),a0
	move.l	#p96screenlog_ok_len,d3
	bra.s	.write
.fail
	lea	p96screenlog_fail(pc),a0
	move.l	#p96screenlog_fail_len,d3
	bra.s	.write
.skip
	lea	p96screenlog_skip(pc),a0
	move.l	#p96screenlog_skip_len,d3
.write
	move.l	d7,d1
	move.l	a0,d2
	jsr	-48(a6)	;Write
...
; gloom2p96_b1/c86zeu: write P96 window result marker.
g2p96_window_log	push
	...
	tst	p96native_state
	bne.s	.native
	lea	p96winlog_fallback_ok(pc),a0
	move.l	#p96winlog_fallback_ok_len,d3
	bra.s	.write
.native
	lea	p96winlog_native_ok(pc),a0
	move.l	#p96winlog_native_ok_len,d3
	bra.s	.write
...
g2p96_preview_frame	push
	...
	and	#0,d0		;c86zds: P96-primary path updates every frame; AGA C2P/db is skipped
	bne.w	.done
...
.nolog	movem.l	d0-d7/a0-a6,-(a7)
	...
	; c86zdg: native 320x240 P96 preview test.  Do not upscale to 640x384
	; and do not clear letterbox rows: the visible preview now covers the whole
	; 320x240 window by sampling 160x120 from the Chunky source and doubling to
	; 320x240.  This also avoids extra RectFill clears every live frame.

	move.l	chunky(pc),a4
	tst.l	a4
	beq.w	.restore
	move	chunkymodw(pc),d0
	beq.w	.restore
	lea	coloffs(pc),a3

	; c86zeu7: on the native 320x240 P96 screen, do not touch BitMap.Planes
	; and do not use p96LockBitMap.  Real hardware reports planar-looking
	; Planes/BPR values there and p96LockBitMap brought back music whine.
	; Use a tiny official p96WritePixelArray band probe instead.  Workbench
	; fallback keeps the old direct writer.
	jsr	g2p96_native_wpa_band_or_direct
	bra.w	.restore
	...
	; that 160x96 2-frame runfill output was stable, but still directly
	; mixed source sampling with drawing.  This version first builds a
	; compact 160x120 pen/luma buffer, then draws that buffer through the
	...
	tst.l	a0
	beq.w	.restore
	moveq	#0,d6		;preview grid row 0..119
.c22bufrow
	cmp	#120,d6
	bge	.c22drawbuffer
	move	d6,d0
	add	d0,d0
	; c86zdg: sample source rows 0..238 for a native 320x240 P96 target
	mulu	chunkymodw(pc),d0
	move.l	a4,a2
	add.l	d0,a2		;Chunky/C2P row base for this grid row
	move	d6,d0
	mulu	#160,d0
	lea	0(a0,d0.l),a1	;destination row inside 160x120 pen buffer
...
.c22drawbuffer
	; c86zdi: c86zdh proved that direct memory copy reaches the visible RTG
	; bitmap, but writing 8-bit pen bytes into the target produced half-width
	; red output.  That means the visible P96 bitmap is almost certainly a
	; 16-bit RGB bitmap.  Use the already-built 160x120 RGB12 buffer and copy
	; it as doubled RGB565 pixels to a native 320x240 destination.  This skips
	; the old Pen/RectFill row renderer entirely.
	jsr	g2p96_direct_copy_rgb565_to_window
	bra.w	.restore

	; old c86zdg/c86zdh Pen/RectFill/direct-pen code kept below as dead
	; reference only.
	move.l	p96preview_window(pc),a5	;gloom2p96_c40: restore Window pointer after RGB sampling
	move.l	p96preview_bufptr(pc),a4
	tst.l	a4
	beq.w	.restore
	moveq	#0,d6		;buffer row 0..119
.c22rowloop
	cmp	#120,d6
	bge	.c22done
	move	d6,d0
	...
	tst.l	a3
	beq.w	.restore
	lea	0(a4,d0.l),a0	;source row in the 160x120 frame buffer
...
.c25expandrow
	...
	move.l	d1,a0		;base of 160x120 RGB12 buffer
	move	d6,d0
...
.c43norgb24line
	...
	; c86zdb: disable the old bottom mini-preview/band.  It duplicates the
	; same gameplay content below the main P96 image and costs extra RectFills.
	bra	.c50skipdirectrgb24
...
.c50skipdirectrgb24
	...
	;full 320x240 linear frame buffer on every visible preview update.
	...
	;old safe RectFill output, but makes the hidden 320x240 buffer the real
...
.c26copyrow2
	...
	dbf	d3,.c26copyrow2
.c26noframemirror
	; c86zdh: direct-bitmap probe.  From here on this live pass no longer
	; draws every row through SetAPen/RectFill.  The row has already been
	; mirrored into p96preview_frameptr above.  Finish building all 240 rows,
	; then one direct copy writes the full 320x240 Pen framebuffer into the
	; P96/Workbench bitmap.  This is the first real candidate away from the
	; slow OS-rectangle preview path.
	bra	.c22nextrow

	; old c86zdg SetAPen/RectFill renderer kept below as dead fallback code for
	; easy rollback/reference, but intentionally skipped in c86zdh.
	move.l	p96preview_frameptr(pc),d0
	beq.s	.c33linefallback
	move.l	d0,a2
	move	d6,d0
	mulu	#640,d0		;two 320-byte framebuffer rows per 160x120 row
	lea	0(a2,d0.l),a2
	bra.s	.c33drawfromready
.c33linefallback
	move.l	a3,a2		;draw from expanded 320-byte staged row
.c33drawfromready
	cmp	#120,d6		;c86zdg: draw all 120 sampled rows -> 320x240 image
	bge	.c22nextrow
...
.c22xloop
	...
	move	d6,d1
	add	d1,d1
	; no y offset in native 320x240 mode
	move	d5,d2
	subq	#1,d2
	move	d1,d3
	addq	#1,d3
...
.c22nextrow
	...
	bra	.c22rowloop
.c22done
	; c86zdh: output the completed 320x240 Pen framebuffer in one direct
	; bitmap-memory copy instead of thousands of SetAPen/RectFill calls.
	jsr	g2p96_direct_copy_frame_to_window
...
; c86zdn: build direct source-byte -> target RGB lookup tables.
; The chunky source bytes are still C2P/paladjust-scrambled, so each source
; byte first goes through g2_strip_invpal before reading planar_palette.
; Outputs:
; - p96rgb565_lut: byteswapped RGB565 word, confirmed good on 16-bit targets.
; - p96rgb888_lut: R,G,B,0 bytes for packed 24/32-bit RTG targets.
g2p96_build_rgb565_lut_source
	movem.l	d0-d7/a0-a6,-(a7)
	lea	p96rgb565_lut,a1
	lea	p96rgb888_lut,a4
	lea	g2_strip_invpal,a2
	move.l	planar_palette(pc),a3
	moveq	#0,d7
.lut_loop
	moveq	#0,d0
	move.b	0(a2,d7.w),d0
...
.lut_black
	moveq	#0,d0
.lut_convert
	; prepare RGB888 bytes from RGB12 nibbles: byte = nibble * 17
	move	d0,d4
	and	#$0f00,d4
	lsr	#8,d4		; R nibble
	move	d4,d5
	...
	move.b	d4,(a4)+
	move	d0,d4
	and	#$000f,d4	; B nibble
	move	d4,d5
	lsl	#4,d4
	or	d5,d4		; B byte
	move.b	d4,(a4)+
	clr.b	(a4)+		; pad/alpha byte for 32-bit fallback

	; prepare byteswapped RGB565 word for 16-bit targets
	move	d0,d1
	and	#$0f00,d1
	lsr	#8,d1		; R 0..15
	move	d1,d2
	...
	or	d3,d2		; B 0..31
	or	d2,d1		; RGB565
	ror	#8,d1		; c86zdk confirmed target byte order
	move	d1,(a1)+
	addq	#1,d7
	cmp	#256,d7
	blo.w	.lut_loop
...
; c86zdn: direct full-resolution 320x240 chunky -> RTG copy.
; Real Amiga/P96 test showed the 16-bit-only c86zdm path could land on a
; packed 24-bit screen: image was 2/3 width and grey-ish.  This routine now
; chooses the writer from BitMap.BytesPerRow:
; - >=1280: 32-bit RGBx
; - >= 960: 24-bit RGB
; - >= 640: 16-bit RGB565, c86zdk byte order
; It is still direct bitmap RAM, but no longer assumes one fixed pixel size.
g2p96_direct_copy_chunky_rgb565_full_to_window
	movem.l	d0-d7/a0-a6,-(a7)
	; c86zdq: building the 256-entry RGB565/RGB888 LUT every P96 frame
	; costs CPU and can cause visible hitching. Build it once after the
	; first real gameplay frame has a valid palette, then reuse it.
	tst	p96rgb_lut_ready
	bne.s	.lut_ready
	jsr	g2p96_build_rgb565_lut_source
	move	#-1,p96rgb_lut_ready
.lut_ready
	move.l	p96preview_window(pc),d0
	beq.w	.full_rts
	move.l	d0,a5		; Window
	move.l	chunky(pc),d0
	beq.w	.full_rts
	move.l	d0,a0		; source chunky base
	move	chunkymodw(pc),d5
	beq.w	.full_rts
	move.l	50(a5),d0	; Window.RPort
	beq.w	.full_rts
	...
	beq.w	.full_rts
	move.l	d0,a2		; BitMap
	move.l	8(a2),d0		; BitMap.Planes[0]
	beq.w	.full_rts
	move.l	d0,a3		; destination bitmap memory
	moveq	#0,d4
	move	0(a2),d4		; BytesPerRow / pitch

	; c86zeu4: capture bitmap/window facts in an EOF helper so old PC-relative
	; GenAm-sensitive code below this block is shifted as little as possible.
	jsr	g2p96_bitmap_capture_target

	; c86zdp: do not infer pixel size from pitch alone.  A 640 wide
	; 16-bit Workbench also has 1280 bytes/row, which c86zdn wrongly
	; treated as 32-bit.  Use the parent Screen.Width to classify the
	; bitmap row size: width*2=RGB565, width*3=RGB24, width*4=RGB32.
	move.l	46(a5),d0		; Window.WScreen
	beq.s	.fmt_fallback16
	move.l	d0,a4
	moveq	#0,d3
	move	12(a4),d3		; Screen.Width
	beq.s	.fmt_fallback16
	move	d3,d0
	lsl	#2,d0		; width*4 threshold
	cmp	d0,d4
	bhs.w	.full_setup32
	move	d3,d0
	add	d3,d0
	add	d3,d0		; width*3 threshold
	cmp	d0,d4
	bhs.w	.full_setup24
	move	d3,d0
	add	d0,d0		; width*2 threshold
	cmp	d0,d4
	bhs.w	.full_setup16
.fmt_fallback16
	; Conservative fallback: keep the c86zdm/c86zdk-confirmed 16-bit path
	; instead of guessing 24/32-bit from a large padded pitch.
	cmp	#640,d4
	bhs.w	.full_setup16
	move	#$0005,p96bitmap_state	;c86zeu4: pitch too small/unknown, direct writer skipped
	jsr	g2p96_bitmap_log_once
	bra.w	.full_rts

.full_setup32
	move	#$0030,p96bitmap_state	;c86zeu4: selected 32-bit direct writer
	jsr	g2p96_bitmap_log_once
	moveq	#0,d1
	move	6(a5),d1		; Window.TopEdge
	mulu	d4,d1
	add.l	d1,a3
	moveq	#0,d1
	move	4(a5),d1		; Window.LeftEdge
	lsl.l	#2,d1		; x*4
	add.l	d1,a3
	lea	coloffs,a6
	lea	p96rgb888_lut,a5
	moveq	#0,d6		; y 0..239
.full32_row
	cmp	#240,d6
	bge.w	.full_rts
	...
	mulu	d5,d0
	move.l	a0,a2
	add.l	d0,a2		; chunky row base
	move	d6,d0
	mulu	d4,d0
	move.l	a3,a4
	add.l	d0,a4		; dest row base
	moveq	#0,d7		; x 0..319
.full32_pix
	cmp	#320,d7
	bge.s	.full32_nextrow
	move.l	0(a6,d7*4),d0
	moveq	#0,d1
	move.b	0(a2,d0.l),d1
	lsl	#2,d1
	move.b	0(a5,d1.w),(a4)+	; R
	move.b	1(a5,d1.w),(a4)+	; G
	move.b	2(a5,d1.w),(a4)+	; B
	clr.b	(a4)+			; pad/alpha
	addq	#1,d7
	bra.s	.full32_pix
.full32_nextrow
...
.full_setup24
	move	#$0020,p96bitmap_state	;c86zeu4: selected 24-bit direct writer
	jsr	g2p96_bitmap_log_once
	moveq	#0,d1
	move	6(a5),d1		; Window.TopEdge
	mulu	d4,d1
...
.full_setup16
	move	#$0010,p96bitmap_state	;c86zeu4: selected 16-bit direct writer
	jsr	g2p96_bitmap_log_once
	moveq	#0,d1
	move	6(a5),d1		; Window.TopEdge
	mulu	d4,d1
	...
	moveq	#0,d1
	move	4(a5),d1		; Window.LeftEdge
	add.l	d1,d1		; 16-bit pixels: x*2
	add.l	d1,a3
	lea	coloffs,a6
	lea	p96rgb565_lut,a5
	moveq	#0,d6		; y 0..239
.full16_row
	cmp	#240,d6
	bge.w	.full_rts
...
; c86zdi: direct 160x120 RGB12 -> 320x240 RGB565 copy into the visible
; P96/Workbench bitmap.  This is the direct-colour successor to c86zdh.
; c86zdh wrote 8-bit pen bytes into what visually behaved like a 16-bit RTG
; bitmap, causing half-width red output.  This routine writes two 16-bit
; RGB565 pixels horizontally and vertically for every RGB12 source sample.
g2p96_direct_copy_rgb565_to_window
	movem.l	d0-d7/a0-a6,-(a7)
	move.l	p96preview_window(pc),d0
	beq.w	.rts
	move.l	d0,a5		;Window
	move.l	p96preview_rgbptr(pc),d0
	beq.w	.rts
	move.l	d0,a0		;source 160x120 RGB12 buffer
	move.l	50(a5),d0	;Window.RPort
	beq.w	.rts
	move.l	d0,a1		;RastPort
	move.l	4(a1),d0		;RastPort.BitMap
	...
	move.l	d0,a3		;destination bitmap memory
	moveq	#0,d4
	move	0(a2),d4		;BytesPerRow
	cmp	#640,d4
	blo.w	.rts		;need at least 320*2 bytes for RGB565
	moveq	#0,d1
	move	6(a5),d1		;Window.TopEdge
	mulu	d4,d1
	add.l	d1,a3
	...
	add.l	d1,d1		;16-bit pixels: x*2
	add.l	d1,a3
	moveq	#0,d6		;source row 0..119
.row
	cmp	#120,d6
	bge.w	.rts
	move	d6,d1
	mulu	#320,d1		;160 RGB12 words per row = 320 bytes
	move.l	p96preview_rgbptr(pc),a0
	lea	0(a0,d1.l),a0
	move	d6,d1
	add	d1,d1		;dest row = source row * 2
	mulu	d4,d1
	move.l	a3,a4
	lea	0(a4,d1.l),a4	;dest row 1
	move.l	a4,a5
	move.l	d4,d1
	add.l	d1,a5		;dest row 2
	move	#159,d7
.pix
	move	(a0)+,d0		;RGB12: 0000rrrrggggbbbb
	move	d0,d1
	...
	lsr	#3,d2
	or	d2,d1		;R 0..31
	lsl	#8,d1		; GenAm/68000 shiftfix: #11 = #8 + #3
	lsl	#3,d1
	move	d0,d2
	and	#$00f0,d2
	lsr	#4,d2		;G 0..15
	...
	lsr	#3,d3
	or	d3,d2		;B 0..31
	or	d2,d1		;RGB565 word
	; c86zdk: c86zdj filled the full P96 target but appeared green/magenta.
	; Try swapped 16-bit RTG byte order before writing the word to bitmap RAM.
	ror	#8,d1
	move	d1,(a4)+
	move	d1,(a4)+
	move	d1,(a5)+
...
; c86zdh: direct 320x240 Pen-framebuffer copy into the visible P96/Workbench
; bitmap.  This intentionally bypasses SetAPen/RectFill completely.  It is a
; test for the next architecture step: one complete frame transfer instead of
; many small OS drawing calls.  Safe assumptions for this probe:
; - the Workbench/P96 screen is 8-bit/CLUT (confirmed by 256-colour pen tests)
; - p96preview_frameptr contains 320x240 final screen Pen indices
; - the preview window is borderless at Window.LeftEdge/TopEdge
; If the target bitmap is not 8-bit or a plane pointer is missing, the routine
; returns without touching memory.
g2p96_direct_copy_frame_to_window
	movem.l	d0-d7/a0-a6,-(a7)
	move.l	p96preview_window(pc),d0
	beq.w	.rts
	move.l	d0,a5		;Window
	move.l	p96preview_frameptr(pc),d0
	beq.w	.rts
	move.l	d0,a0		;source 320x240 Pen framebuffer
	move.l	50(a5),d0	;Window.RPort
	beq.w	.rts
	move.l	d0,a1		;RastPort
	move.l	4(a1),d0		;RastPort.BitMap
	...
	move.l	d0,a2		;BitMap
	moveq	#0,d0
	move.b	5(a2),d0		;BitMap.Depth
	cmp	#8,d0
	bne.w	.rts		;this direct test only targets 8-bit P96 screens
	move.l	8(a2),d0		;BitMap.Planes[0]
	beq.w	.rts
	move.l	d0,a3		;destination bitmap memory
	moveq	#0,d0
	move	0(a2),d0		;BytesPerRow
	beq.w	.rts
	move	d0,d4		;dest modulo/rowbytes
	moveq	#0,d1
	move	6(a5),d1		;Window.TopEdge
	mulu	d4,d1
	add.l	d1,a3
	...
	add.l	d1,a3
	move	d4,d5
	sub	#320,d5		;destination add after each 320-byte row
	move	#239,d7
.row
	move	#79,d6		;320 bytes = 80 longwords
.copy
	move.l	(a0)+,(a3)+
	dbf	d6,.copy
	add	d5,a3
...
initbitmap	;a0=bitmap struct, d0=bitplane 0
	;
	move	bitplanes,d1	;c86zeu1: GenAm pc-range buildfix
	move	#40,(a0)	;linemod
...
g2v36_hide_pointer	;force invisible pointer for the game screen/window
	...
	move.l	int,d1
	beq.s	.rts
	move.l	chipzero,a1	;v37/c86zeu1: pointer image must live in chip RAM, pc-range buildfix
	move.l	a1,d1
	beq.s	.rts
	move.l	d0,a0
...
.loop	move.l	msgport(pc),a0
	...
	bne.s	.skip
	;
	; c86zdd: when the Workbench/P96 live-preview bridge is active, the
	; mouse can leave the 320x240 overlay and Workbench may send a deactivate
	; for the hidden game window.  Do not drop raw keyboard/mouse/joy input in
	; that case; otherwise controls stop until the user clicks the overlay.
	ifne	g2p96_workbench_front_enabled
	tst.l	p96preview_window
	bne.s	.skip
	endc
...
initdisplay	;
	...
	mulu	bitplanes,d2	;c86zdo: pc-range fix
	move.l	d2,bmapmem
	move.l	d2,d0
	...
	jsr	-408(a6)
	move.l	d0,int
	; c86zd0: FrameThrower/C790 workaround.  Open the P96/Workbench preview
	; window before the custom AGA screen is opened, draw the test pattern, and
	; hold here until left mouse/fire is pressed.  This lets the RTG/P96 output
	; be inspected before any visible AGA signal can force an external switch.
	ifne	g2p96_overlay_enabled
	ifne	g2p96_workbench_hold_enabled
	jsr	g2p96_test_window
	jsr	g2p96_workbench_hold_probe
	endc
	endc
	;
	move.l	bitmaps,d0	;c86zdo: pc-range fix
	lea	osbitmap1(pc),a0
	bsr	initbitmap
	move.l	bitmaps2,d0	;c86zdo: pc-range fix
	lea	osbitmap2(pc),a0
	bsr	initbitmap
	;
	move	bitplanes,newscreen_d	;c86zdm: pc-range fix
	tst	aga
	bne.s	.agasc
	move	#$80,newscreen_v
	;
.agasc	move.l	int(pc),a6
	...
	move.l	d0,dbufinfo
	ifne	g2p96_overlay_enabled
	ifeq	g2p96_workbench_hold_enabled
	jsr	g2p96_test_window	;gloom2p96_c4: open preview window after game screen setup
	endc
	endc
	; c86zd0: with Workbench-hold enabled, the P96 window was already opened
	; before OpenScreen and is deliberately not reopened here.
	bsr	db
	ifne	g2p96_overlay_enabled
	ifne	g2p96_workbench_front_enabled
	jsr	g2p96_workbench_front_for_preview	;c86zd9: keep RTG/P96 visible after hidden AGA screen setup
	endc
	endc
	rts
	;
...
db	;double buffering
	;
	lea	bitmaps,a0	;c86zdm: pc-range fix
	movem.l	(a0),d0-d1
	cmp.l	drawbitmap,d0	;c86zdm: pc-range fix
	beq.s	.show
	exg	d0,d1
.show	movem.l	d0-d1,showbitmap
	move.l	todb,a0	;c86zdm: pc-range fix
	jmp	(a0)

db_ecs	move.l	coplist,a0	;c86zeu1: GenAm pc-range buildfix
	moveq	#40,d2
...
db_aga	move.l	coplist,a0	;c86zeu1: GenAm pc-range buildfix
	moveq	#40,d2
...
freemem_	;
	...
	lea	memlist,a1	;c86zdo: pc-range fix
	;
...
p96palpen_map	dcb.b	256,0
	even
; c86zdl/c86zdn: source chunky byte -> target RGB LUTs.
; RGB565 word keeps c86zdk-confirmed byte order for 16-bit targets.
; RGB888 bytes are stored as R,G,B,0 for packed 24/32-bit RTG targets.
p96rgb565_lut	dcb.w	256,0
p96rgb888_lut	dcb.b	1024,0
	even
...
; ---- c86zeu4 appended diagnostics kept at EOF to avoid shifting old GenAm PC-relative code ----
; c86zeu4: one-shot diagnostic for the native/fullscreen RTG bitmap that
; the direct 320x240 RGB writer sees during gameplay.  Real Amiga showed the
; vertical test bars only, while emulator accepted the same direct writer.
; This log tells us whether the real P96 bitmap exposes usable linear memory,
; which depth/pitch it reports, and which writer branch was selected.
p96bmplog_name	dc.b	'RAM:gloom_p96_bitmap.txt',0
	even
p96bmplog_msg	dc.b	'P96 bitmap: state=$'
p96bmplog_state_hex	ds.b	4
	dc.b	' depth=$'
p96bmplog_depth_hex	ds.b	4
	dc.b	' bpr=$'
p96bmplog_bpr_hex	ds.b	4
	dc.b	' sw=$'
p96bmplog_sw_hex	ds.b	4
	dc.b	' ww=$'
p96bmplog_ww_hex	ds.b	4
	dc.b	' wh=$'
p96bmplog_wh_hex	ds.b	4
	dc.b	' left=$'
p96bmplog_left_hex	ds.b	4
	dc.b	' top=$'
p96bmplog_top_hex	ds.b	4
	dc.b	' plane0lo=$'
p96bmplog_plane0lo_hex	ds.b	4
	dc.b	10
p96bmplog_msg_len	equ	*-p96bmplog_msg
	even
p96bitmap_logged	dc	0
p96bitmap_state	dc	0	;0010=16-bit writer, 0020=24-bit, 0030=32-bit, 0005=too small/unknown
p96bitmap_depth	dc	0
p96bitmap_bpr	dc	0
p96bitmap_sw	dc	0
p96bitmap_ww	dc	0
p96bitmap_wh	dc	0
p96bitmap_left	dc	0
p96bitmap_top	dc	0
p96bitmap_plane0lo	dc	0
	even

; c86zeu4: write one diagnostic line about the native/fullscreen bitmap.
; Uses the existing c83 hex helper to keep the patch small.
g2p96_bitmap_capture_target
	movem.l	d0-d4/a0-a4,-(a7)
	clr	p96bitmap_sw
	moveq	#0,d0
	move.b	5(a2),d0		; BitMap.Depth
	move	d0,p96bitmap_depth
	move	d4,p96bitmap_bpr
	move.l	a3,d0
	move	d0,p96bitmap_plane0lo
	moveq	#0,d0
	move	4(a5),d0		; Window.LeftEdge
	move	d0,p96bitmap_left
	moveq	#0,d0
	move	6(a5),d0		; Window.TopEdge
	move	d0,p96bitmap_top
	moveq	#0,d0
	move	8(a5),d0		; Window.Width
	move	d0,p96bitmap_ww
	moveq	#0,d0
	move	10(a5),d0		; Window.Height
	move	d0,p96bitmap_wh
	move.l	46(a5),d0		; Window.WScreen
	beq.s	.capdone
	move.l	d0,a4
	moveq	#0,d0
	move	12(a4),d0		; Screen.Width
	move	d0,p96bitmap_sw
.capdone
	movem.l	(a7)+,d0-d4/a0-a4
	rts

g2p96_bitmap_log_once
	movem.l	d0-d7/a0-a6,-(a7)
	tst	p96bitmap_logged
	bne	.bmpdone
	move	#-1,p96bitmap_logged
	move	p96bitmap_state,d0
	lea	p96bmplog_state_hex,a0
	jsr	g2p96_c83_word_to_hex4
	move	p96bitmap_depth,d0
	lea	p96bmplog_depth_hex,a0
	jsr	g2p96_c83_word_to_hex4
	move	p96bitmap_bpr,d0
	lea	p96bmplog_bpr_hex,a0
	jsr	g2p96_c83_word_to_hex4
	move	p96bitmap_sw,d0
	lea	p96bmplog_sw_hex,a0
	jsr	g2p96_c83_word_to_hex4
	move	p96bitmap_ww,d0
	lea	p96bmplog_ww_hex,a0
	jsr	g2p96_c83_word_to_hex4
	move	p96bitmap_wh,d0
	lea	p96bmplog_wh_hex,a0
	jsr	g2p96_c83_word_to_hex4
	move	p96bitmap_left,d0
	lea	p96bmplog_left_hex,a0
	jsr	g2p96_c83_word_to_hex4
	move	p96bitmap_top,d0
	lea	p96bmplog_top_hex,a0
	jsr	g2p96_c83_word_to_hex4
	move	p96bitmap_plane0lo,d0
	lea	p96bmplog_plane0lo_hex,a0
	jsr	g2p96_c83_word_to_hex4
	move.l	dosbase,d0
	beq	.bmpdone
	move.l	d0,a6
	lea	p96bmplog_name,a0
	move.l	a0,d1
	move.l	#1006,d2	;MODE_NEWFILE
	jsr	-30(a6)	;Open
	move.l	d0,d7
	beq	.bmpdone
	move.l	d7,d1
	lea	p96bmplog_msg,a0
	move.l	a0,d2
	move.l	#p96bmplog_msg_len,d3
	jsr	-48(a6)	;Write
	move.l	d7,d1
	jsr	-36(a6)	;Close
.bmpdone
...
; ---- c86zeu9 native-screen p96WritePixelArray pen bands probe ----
; c86zeu8 proved that changing RGB565/BGR565 source words and RGBFormat values
; makes no visible difference on the real Amiga: all four bands stayed blue/green.
; That means the official p96WritePixelArray route is behaving as the P96 docs
; describe: it writes pen values, not our arbitrary 16-bit RGB words, on this
; real-hardware path.
;
; Therefore this probe switches the staging buffer to one byte per pixel and
; feeds p96WritePixelArray real pen values.  Four tiny bands compare the safest
; candidate mappings without touching direct BitMap memory and without
; p96LockBitMap:
;
;   y=060: raw chunky byte as pen value
;   y=090: exact Gloom palette index -> ObtainBestPen pen map
;   y=120: old RGB332 candidate -> ObtainBestPen pen map
;   y=150: luma/contrast fallback pen 1..8
;
; The user only has to report which band has the most correct colours.
; Register convention:
;   a0=RenderInfo, d0=SrcX, d1=SrcY, a1=RastPort,
;   d2=DestX, d3=DestY, d4=SizeX, d5=SizeY, a6=Picasso96APIBase
; p96WritePixelArray is LVO -102.

g2p96_native_wpa_band_or_direct
	tst	p96native_state
	bne	.native
	jsr	g2p96_direct_copy_chunky_rgb565_full_to_window
	rts
.native
	jsr	g2p96_native_wpa_band_probe
	rts

p96wpalog_name	dc.b	'RAM:gloom_p96_wpa.txt',0
	even
p96wpalog_msg_ok	dc.b	'P96 WPA: c86zeu9 native 4x 320x12 pen bands returned',10
p96wpalog_msg_ok_len	equ	*-p96wpalog_msg_ok
p96wpalog_msg_skip	dc.b	'P96 WPA: c86zeu9 skipped',10
p96wpalog_msg_skip_len	equ	*-p96wpalog_msg_skip
	even
p96wpa_logged	dc	0
p96wpa_state	dc	0	;0=not run, 1=returned, -1=skipped/fail
p96wpa_divider	dc	0	;throttle tiny band copy to reduce real-hardware stress
p96wpa_ri_memory	dc.l	0
p96wpa_ri_bpr	dc	0
p96wpa_ri_pad	dc	0
p96wpa_ri_rgbformat	dc.l	0
	even

g2p96_native_wpa_log_once
	movem.l	d0-d7/a0-a6,-(a7)
	tst	p96wpa_logged
	bne	.done
	move	#-1,p96wpa_logged
	move.l	dosbase,d0
	beq	.done
	move.l	d0,a6
	lea	p96wpalog_name,a0
	move.l	a0,d1
	move.l	#1006,d2	;MODE_NEWFILE
	jsr	-30(a6)	;Open
	move.l	d0,d7
	...
	tst	p96wpa_state
	bmi	.skip
	lea	p96wpalog_msg_ok,a0
	move.l	#p96wpalog_msg_ok_len,d3
	bra	.write
.skip
	lea	p96wpalog_msg_skip,a0
	move.l	#p96wpalog_msg_skip_len,d3
.write
	move.l	d7,d1
	move.l	a0,d2
	jsr	-48(a6)	;Write
...
; Build one 320x12 8-bit pen-value band into the shared fast RAM staging buffer.
; Inputs:
;   a0 = chunky base
;   a4 = staging buffer base
;   a5 = p96palpen_map
;   a6 = coloffs
;   d5 = chunkymodw
;   d6 = source y / destination y
;   d2 = mode: 0 raw, 1 exact palette BestPen, 2 RGB332 BestPen, 3 luma pens
; Output: p96wpa_ri_memory points to the ready source band.
g2p96_build_one_pen_band
	movem.l	d0-d7/a0-a6,-(a7)
	move.l	a4,p96wpa_ri_memory
	move	#320,p96wpa_ri_bpr	;one byte per pixel for pen values
	clr	p96wpa_ri_pad
	clr.l	p96wpa_ri_rgbformat	;pen/CLUT-style source for p96WritePixelArray
	moveq	#0,d7		;band row 0..11
.row_loop
	cmp	#12,d7
	bge	.done
	move	d6,d0
	add	d7,d0		;actual source y
	mulu	d5,d0
	move.l	a0,a2
	add.l	d0,a2		;chunky row base
	move	d7,d0
	mulu	#320,d0
	move.l	a4,a3
	add.l	d0,a3		;8-bit pen band row base
	moveq	#0,d4		;x 0..319
.pix_loop
	cmp	#320,d4
	bge	.next_row
	move.l	0(a6,d4*4),d0
	moveq	#0,d1
	move.b	0(a2,d0.l),d1	;chunky palette/index byte
	cmp	#1,d2
	beq	.exact_bestpen
	cmp	#2,d2
	beq	.rgb332_bestpen
	cmp	#3,d2
	beq	.luma_pen
	; mode 0: raw chunky byte as pen value
	move.b	d1,(a3)+
	bra	.next_pix
.exact_bestpen
	move.b	0(a5,d1.w),(a3)+
	bra	.next_pix
.rgb332_bestpen
	lea	p96bestpen_map,a5
	move.b	0(a5,d1.w),(a3)+
	lea	p96palpen_map,a5
	bra	.next_pix
.luma_pen
	; quick safe fallback: spread source index over pens 1..8.
	move	d1,d0
	lsr	#5,d0
	and	#7,d0
	addq	#1,d0
...
; Build four 320x12 8-bit pen bands and copy each through p96WritePixelArray.
; The source buffer reuses p96preview_frameptr because it is a large fast RAM
; allocation and unused by the native direct writer path.
g2p96_native_wpa_band_probe
	movem.l	d0-d7/a0-a6,-(a7)
	; keep pressure low on real hardware: four small bands every second hook
	addq	#1,p96wpa_divider
	move	p96wpa_divider,d0
	and	#1,d0
	bne	.skip_no_log
	...
	beq	.skip
	move.l	d0,a1		;RastPort for p96WritePixelArray
	move.l	p96preview_frameptr,d0
	beq	.skip
	move.l	d0,a4		;source pen band buffer
	move.l	chunky,d0
	beq	.skip
	move.l	d0,a0		;chunky base
	move	chunkymodw,d5
	beq	.skip
	move.l	p96base,d0
	beq	.skip

	; Make sure both pen maps exist.  g2p96_open_native_for_gameplay already
	; invalidates/rebuilds the exact native palette map once, but this keeps the
	; probe robust if the hook runs after a palette change.
	tst	p96palpen_state
	bne	.pal_ok
	jsr	g2p96_c73_init_palette_pen_map
.pal_ok
	tst	p96bestpen_state
	bne	.best_ok
	jsr	g2p96_c71_init_bestpen_map
.best_ok

	lea	coloffs,a6
	lea	p96palpen_map,a5

	; Band 1: y=60, raw chunky pen values.
	move	#60,d6
	moveq	#0,d2
	jsr	g2p96_build_one_pen_band
	move.l	p96base,d0
	beq	.skip
	move.l	d0,a6
	lea	p96wpa_ri_memory,a0
	moveq	#0,d0
	moveq	#0,d1
	move.l	50(a3),a1
	moveq	#0,d2
	...
	; Band 2: y=90, exact Gloom palette index -> BestPen.
	move.l	chunky,a0
	move	chunkymodw,d5
	lea	coloffs,a6
	lea	p96palpen_map,a5
	...
	; Band 3: y=120, old RGB332 candidate -> BestPen.
	move.l	chunky,a0
	move	chunkymodw,d5
	lea	coloffs,a6
	lea	p96palpen_map,a5
	...
	; Band 4: y=150, simple luma pens 1..8.
	move.l	chunky,a0
	move	chunkymodw,d5
	lea	coloffs,a6
	lea	p96palpen_map,a5
