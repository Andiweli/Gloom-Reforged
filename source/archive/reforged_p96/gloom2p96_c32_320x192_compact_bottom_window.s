...
p96prevlog_name	dc.b	'RAM:gloom_p96_preview.txt',0
	even
p96prevlog_ok	dc.b	'P96 preview: 320x192 compact bottom window active',10
...
.c25expandrow
	...
	;gloom2p96_c27: mirror the expanded 320-byte row into the optional
	;full 320x192 linear frame buffer only every 8th real frame.  c26 proved
	;that the full mirror works, but it added a little visible stutter.  The
	;visible Runfill preview still updates every 2nd frame; only this hidden
	;future-output buffer is throttled to reduce load while keeping it alive.
	move	p96preview_frame(pc),d0
	and	#7,d0
	bne.s	.c26noframemirror
