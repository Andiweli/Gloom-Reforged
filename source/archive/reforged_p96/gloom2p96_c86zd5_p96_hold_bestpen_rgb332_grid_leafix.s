	...
	;gloom2p96_c86zd5: buildfix for c86zd4, absolute p96bestpen_map addressing; P96 Workbench-hold BestPen RGB332 grid; raw pen grid confirmed, now test ObtainBestPenA colour mapping before AGA.
	;
...
p96prevlog_name	dc.b	'RAM:gloom_p96_preview.txt',0
	even
p96prevlog_ok	dc.b	'P96 preview: c86zd4 BestPen RGB332 grid active',10
...
; c86zd4: draw the 256-entry RGB332 -> Workbench/P96 BestPen map over
; the whole hold window.  c86zd3 proved that raw SetAPen/RectFill and the
; 16x16 geometry are correct.  This version keeps the same geometry but uses
; the c71 ObtainBestPenA table, so we can see whether RGB requests collapse
; or produce a useful colour cube before AGA ever opens.
; a0 = Window
g2p96_draw_palette_hold_probe	push
	...
	beq.s	.done
	moveq	#0,d7		;RGB332 candidate index 0..255
.cell_loop
	cmp	#256,d7
	bge.s	.done
	; Set the mapped BestPen first; SetAPen may trash d0/d1, so all
	; RectFill coordinates are calculated afterwards, like the c86zd3 fix.
	move	d7,d0
...
.raw_pen
	move.l	50(a5),a1
	move.l	grbase(pc),a6
	jsr	-342(a6)	;SetAPen(BestPen or raw fallback)
