...
solidstrip	set	0
drawstrip2	; v195 chunky-safe transparent wall strip overlay with 50% green glass tint
	...
	; texels use a true checkerboard 50% tint based on screen X/Y parity, so
	; the background stays visible through the green glass without the old
	; horizontal stripe artefact.
	movem.l	d0-d7/a0-a5,-(a7)
	move	d0,d1
	and	#1,d1		; screen-X parity from caller for 50% checker transparency
	move.l	vd_data(a4),d0
	beq	.rts
	move.l	d0,a0		; source texture column
	; v195: force a darker green glass tint for transparent texels.  The
	; legacy strip colour selector is not reliable for these doors in chunky
	; mode, so use a fixed remapped green instead.
	moveq	#2,d6		; safe dark-green-ish fallback palette index
...
.glass	tst	d6
	bmi.s	.next
	move	d4,d5
	eor	d1,d5		; real X/Y checkerboard for 50% transparency
	btst	#0,d5
	bne.s	.next
	move.b	d6,(a1)
.next	addx.l	d3,d0
