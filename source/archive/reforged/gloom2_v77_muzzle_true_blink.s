...
.g2dm_have_weapon
	...
	; v77: draw a true blinking flash.  The small phase and the large phase
	; are now mutually exclusive, so the large frame no longer keeps the
	; smaller stamp underneath it.  Because the whole scene is redrawn every
	; frame, this makes the muzzle appear to really blink: small>large>small.
	move	g2gun_firetimer(pc),d5
	btst	#0,d5
	beq.w	.g2dm_bigflash		;125%,150%,125%,150% over the 4 fire frames
	; small flash phase ~= 125%
	bsr	g2drawgun_coloffs_shape
	subq	#3,d1
	bsr	g2drawgun_coloffs_shape
	addq	#6,d1
