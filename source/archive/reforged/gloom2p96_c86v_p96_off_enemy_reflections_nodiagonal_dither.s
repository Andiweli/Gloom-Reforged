	...
	;gloom2p96_c86v: enemy reflections no-diagonal bitmap dither fade.
	;
...
.yloop
	cmp.l	a1,a0
	bhi	.rts
	; c86v: no diagonal hatch.  Do not mix X and Y into the dither
	; phase.  The reflected enemy keeps the current sprite bitmap columns:
	; top rows are mostly intact, lower rows become coarser and fade out.
	move	g2_enemy_ref_curx(pc),d0
	move	d5,d1
	cmp	#3,d1
	blt.s	.dither_dense75
	cmp	#9,d1
	blt.s	.dither_half50
	cmp	#15,d1
	blt.s	.dither_quarter25
	; far end: very sparse vertical-column dither, no row phase shift
	and	#7,d0
	bne	.next
	bra	.sample
.dither_quarter25
...
p96prevlog_name	dc.b	'RAM:gloom_p96_preview.txt',0
	even
p96prevlog_ok	dc.b	'P96 preview: c86v P96 off enemy reflections no-diagonal dither active',10
