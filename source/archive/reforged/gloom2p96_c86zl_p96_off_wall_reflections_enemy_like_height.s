...
.col_loop
	...
	; c86zl: rebuild wall-reflection height like the confirmed enemy-reflection
	; logic.  Use the actually visible wall-column height as the driver: far
	; walls (small vd_h) get a short reflection, near walls (large vd_h) get
	; a longer one.  Do not derive the primary reflection height from the free
	; screen space below the wall-foot; that created the inverted behaviour.
	move	vd_h(a0),d6
...
.depth_min_ok
	cmp	#5,d6
	ble.s	.depth_cap_ok
	moveq	#5,d6		; max 6 visible rows (0..5 in DBF)
.depth_cap_ok
	; now only clip this desired height to the actually available floor rows
	; below the wall-foot.  This is the same idea as enemies: desired size
	; first, screen clipping second.
	move	hite(pc),d2
	sub	#3,d2		; safe last destination row before HUD/gun edge
	sub	d1,d2
	subq	#2,d2		; rows available below the wall-foot start
	cmp	d2,d6
