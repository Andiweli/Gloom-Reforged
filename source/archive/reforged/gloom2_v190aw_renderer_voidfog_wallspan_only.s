...
g2fill_void_fog
	; v190aw: copy far portal/corridor fog only across the neighbouring wall
	; span, not across the whole screen column.  This keeps distant openings
	; shaded like nearby far walls, while avoiding the broad horizontal smear
	; artefacts that appeared when sprites/blood/projectiles were present in
	; the dark region.
	movem.l	d0-d7/a0-a6,-(a7)
	move.l	chunky(pc),a0
	tst.l	a0
	...
	tst.l	a6
	beq.w	.g2fv_done
	move	chunkymodw(pc),d4
	move	hite(pc),d3
	ble.w	.g2fv_done
	;
	; pass 1: left -> right, fill empty wall columns from the previous
	; neighbouring solid wall column, but only over that wall's vertical span.
	lea	coloffs(pc),a1
	move	width(pc),d7
	ble.w	.g2fv_pass2
	subq	#1,d7
...
.g2fv_lx
	...
	move.l	a0,a3
	add.l	d0,a3
	move	-vd_size+vd_y(a6),d0
	add	midy(pc),d0
	move	-vd_size+vd_h(a6),d5
	ble.s	.g2fv_lnext
	bsr	g2fv_copy_wallspan
.g2fv_lnext
	lea	vd_size(a6),a6
	addq.l	#4,a1
	addq	#1,d6
	dbf	d7,.g2fv_lx
	;
	; pass 2: right -> left, same idea using the next solid wall column.
.g2fv_pass2
	move	width(pc),d7
	ble.w	.g2fv_done
	subq	#1,d7
...
; a2 = dest column, a3 = source column
; d0 = top Y (screen relative), d3 = visible chunky height, d4 = chunkymodw
; d5 = wall span height
; Copies only within the clipped vertical wall span and only into still-empty
; pixels, leaving floors/ceilings/sprites far less affected than the full-
; column fill used before.
g2fv_copy_wallspan
	tst	d0
	bpl.s	.g2fv_ws_notopclip
	add	d0,d5
	ble.s	.g2fv_ws_done
	neg	d0
	move	d0,d1
	...
	add.l	d1,a3
	moveq	#0,d0
	bra.s	.g2fv_ws_clipbot
.g2fv_ws_notopclip
	beq.s	.g2fv_ws_clipbot
	move	d0,d1
	mulu	d4,d1
	add.l	d1,a2
	add.l	d1,a3
...
.noroof	;
	...
	jsr	g2log_drawstep
.nofloor	;
	; v190av: apply far-corridor fog before sprites/blood.
	; v190au did it after sprites/blood, which could smear through
	; transparent projectile/blood pixels and make them look like lines.
	jsr	g2fill_void_fog
