	...
	;gloom2p96_c86y: clearer enemy reflections + visible horizon wall reflection lite.
	;
...
; c86y: simple wall reflection version A.
; The previous c86x probe looked at the lower viewport edge, so it had almost
; no floor area left to draw into.  Gloom's floor is drawn from the horizon
; line (midy/maxy) downward, so mirror a few rows around that seam instead.
; This is intentionally stylised and weak, not a second physical wall renderer.
g2_draw_wall_reflection_lite
	movem.l	d0-d7/a0-a6,-(a7)
	move.l	chunky(pc),a0
	tst.l	a0
	beq.w	.done
	move	midy(pc),d0	; screen horizon/floor seam, not bottom viewport edge
	cmp	#3,d0
	blt.w	.done
	move	hite(pc),d1
	sub	#24,d1
	...
	move	#23,d6		; 24-pixel maximum wall-reflection depth
.yloop
	move	d6,d5
	neg	d5
	add	#23,d5		; y = 0..23 from horizon downward
	; visible but still subtle fade: strong only near horizon, quickly sparse
	moveq	#9,d4		; top: 9/16
	cmp	#3,d5
	blt.s	.have_thr
	moveq	#6,d4		; early mid: 6/16
	cmp	#8,d5
	blt.s	.have_thr
	moveq	#3,d4		; lower: 3/16
	cmp	#15,d5
	blt.s	.have_thr
...
.have_thr
	move	d0,d2
	subq	#1,d2
	sub	d5,d2		; source row above horizon
	blt.s	.next_y
	move	d0,d3
	addq	#1,d3
	add	d5,d3		; destination row below horizon
	move	hite(pc),d1
	sub	#4,d1
	cmp	d1,d3
	bgt.s	.next_y
...
.g2v190dm_floor_fog
	...
	jsr	g2fill_disabled_flat_fog
.nofloor	;
	; c86y: simple wall-reflection lite.  After flats are drawn, mirror
	; a few rows from above the horizon seam into the floor with sparse fade.
	; REFLECTIONS NO disables it.
	tst	g2_reflections
...
.rh_max_ok
	...
	cmp	d1,d0
	bgt.s	.no_ref
	; c86u: airborne/floating enemies mirror on the floor, not directly under
	; the body.  Move the reflection start down by the projected air gap and
	; shorten the reflection by the same amount so the lower fade end stays
	; aligned with grounded enemies.
	move	ob_y(a2),d1
	bpl.s	.airgap_done
	neg	d1
	ext.l	d1
