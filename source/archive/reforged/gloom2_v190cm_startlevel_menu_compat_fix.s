...
dointro	;
	...
	jsr	inputon
	tst	g2_game_profile	; v190cl: compatibility profiles go straight to menu
	bne.s	.g2compat_nowait
	jsr	waitany
.g2compat_nowait
	;
.redrawmenu
	jsr	g2v145_draw_menu_gloombrush
	lea	startmenu,a4	; v190cm: all profiles use START LEVEL title menu
	tst	linked
	beq.s	.use
	lea	startmenu2,a4
.use	jsr	initmenu
	tst	linked		; v190cm: default title selection is ONE PLAYER GAME, not START LEVEL
	bne.s	.sel
	move	#1,curropt	; rows: 0 START LEVEL, 1 ONE PLAYER
	;
.sel	jsr	selmenu
	;
	tst	linked
...
.notlinked
	move	d0,d7
	and	#$0300,d7	; $0100=left, $0200=right for START LEVEL / PLAYER rows
	and	#$00ff,d0
	tst	d7
...
.g2v166_notlinked_fire
	...
	bcs	.sel
	cmp	#4,d0
	bcs	.g2v190cm_newgame
	; v190cm: no spacer below START LEVEL.
	; rows: 1/2/3 game, 5/6 player, 8 link, 9 violence, 10 about, 11 exit.
	cmp	#5,d0
	beq	.g2v166_p1sel
	cmp	#6,d0
	beq	.g2v166_p2sel
...
.g2compat_notlinked
	...
	beq.s	.g2compat_fire
	cmp	#4,d0
	beq.s	.g2compat_p1lrsel
	cmp	#5,d0
	beq.s	.g2compat_p2lrsel
	bra	.sel
.g2compat_fire
	cmp	#3,d0
	bcs.s	.g2compat_newgame
	cmp	#4,d0
	beq.s	.g2compat_p1sel
	cmp	#5,d0
	beq.s	.g2compat_p2sel
	cmp	#7,d0
	beq	.linksel
	cmp	#8,d0
	beq	.vilesel
...
g2v190f_level_text	dc.b	'MAP1.1',0	;0 v190i script-built direct map selector, display uses dot not underscore
	...
	dc.b	0			;4 v190cm spacer/separator above PLAYER 1
	dc.b	'PLAYER 1 '
p1ctype
	ifne	cd32
...
p2ctype
	...
	endc
	;
	dc.b	0			;7 v190cm spacer/separator above REMOTE LINK OPTIONS
	dc.b	'REMOTE LINK OPTIONS',0	;8
	;
modetxt	dc.b	'MEATY VIOLENCE MODE',0	;9
	dc.b	'ABOUT GLOOM',0		;10
	dc.b	'EXIT GLOOM',0		;11
	even

	; v190cl: compatibility title menu for Gloom/Gloom3/Zombie Massacre.
	; It deliberately removes the Reforged START LEVEL row so classic title
	; screens do not feel one option shifted.
g2compat_startmenu dc.b 11
...
g2compat_modetxt dc.b	'MEATY VIOLENCE MODE',0	;8
...
initfontpal	cmp	#1,g2_game_profile	; v190cm: original Gloom blackmagic uses its own palette
	beq.s	.g2v190cm_keep_titlepal
	move.l	font(pc),a1
	add.l	(a1),a1
	move	(a1),-(a7)
	clr	(a1)
...
calcpalettes	;generate 16 versions of palette
	...
	; v190cj: original Gloom lacks misc/remap_8; use safe identity shade
	; tables through paladjust so it does not crash immediately.
	lea	palettes(pc),a2
	moveq	#15,d7
.g2v190cj_shade_loop
	move.l	(a2)+,a3
...
g2_game_profile	dc	0	;0=gloom2/gloom3, 1=gloom original, 2=zombie massacre
