...
g2fill_void_fog
	; v190fw/v190fv2/v190ba: safe real-Amiga void fog after flats.
	; Keep the corridor/room openings dark, but do not sample texture pixels
	; from neighbouring wall columns anymore.  v190ba/v190fv could still pick
	; a slightly different wall colour while walking and show vertical flicker
	; at long corridor ends.  Use one stable neutral far-fog colour instead.
	movem.l	d0-d7/a0-a6,-(a7)
	move.l	chunky(pc),a0
	tst.l	a0
	beq.w	.g2fv_done
	...
	; v190fw: build one stable neutral dark fog colour for this frame.
	; If palette/remap is unavailable, the fill routine falls back to the old
	; neighbouring-column sample instead of leaving holes.
	clr	g2fv_global_ok
	bsr.w	g2fv_build_default_global
	;
	; pass 1: left -> right, only for columns where castwalls found no wall
	lea	coloffs(pc),a1
	move	width(pc),d7
	ble.w	.g2fv_pass2
	subq	#1,d7
	moveq	#0,d6	; current x
.g2fv_lx
	tst.l	vd_data(a6)
	bne.s	.g2fv_lnext
	tst	d6
	...
	move.l	(a1),d0
	move.l	a0,a2
	add.l	d0,a2	; current column top
	move.l	-4(a1),d0
	move.l	a0,a3
	add.l	d0,a3	; previous column top
	bsr	g2fv_sample_and_fill
.g2fv_lnext
	lea	vd_size(a6),a6
	addq.l	#4,a1
	addq	#1,d6
	dbf	d7,.g2fv_lx
	;
	; pass 2: right -> left, catches openings at the left edge of the view
.g2fv_pass2
	move	width(pc),d7
	ble.w	.g2fv_done
	subq	#1,d7
	move	d7,d6	; current x, starting at last column
	move	d7,d3	; last column index
	lea	coloffs(pc),a1
	move	d7,d0
	ext.l	d0
	lsl.l	#2,d0
...
; a2 = empty destination column top
; a3 = neighbouring solid source column top fallback
; d4 = chunkymod
; Uses one stable neutral far-fog colour for the whole empty column.  This
; avoids the v190ba/v190fv vertical colour flicker at long corridor ends.
; If the neutral colour cannot be built, fall back to the old neighbour sample.
g2fv_sample_and_fill
	move	hite(pc),d5
	ble.s	.g2fv_sf_done
	tst	g2fv_global_ok
	beq.s	.g2fv_use_neighbour
	move	g2fv_global_col(pc),d1
	bra.s	.g2fv_fill_start
.g2fv_use_neighbour
	subq	#1,d5
	move.l	a3,a4
.g2fv_find
...
drawscene	;a5=player
	...
	jsr	g2log_drawstep
	; v190fv2: real-Amiga freeze fix - do not fill void fog before roof/floor.
	; v190bb introduced the early wallspan fill here; keep this path empty.
	; The safe v190ba uniform fill runs after flats below.
	;
	move	roofflag(pc),d0
...
.g2v190dm_floor_fog
	...
	jsr	g2fill_disabled_flat_fog
.nofloor	;
	; v190fv2: v190ba-safe order - fill void/far corridor fog only after
	; walls and flats, using the simple uniform sampled fill from v190ba.
	jsr	g2fill_void_fog
