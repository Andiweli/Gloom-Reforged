...
dogamemenu	;
	...
	move	#$8020,$dff09a
	;
	jsr	g2v190ai_ingamemenu_grey_backdrop	; v190ai: redraw current game view through grey shade tables for ESC menu
...
; v190ai: ESC in-game menu greyscale backdrop, derived from the original
; gloom.s palette-switch idea but adapted to the Gloom2 chunky/remap path.
; We do not poke a temporary wall palette here.  Instead, each shade table maps
; the current map RGB list to greyscale entries via planar_remap/paladjust.
; This avoids the bigfont2 yellow palette entries recolouring live wall pixels.
g2v190ai_ingamemenu_grey_backdrop
	movem.l	d0-d7/a0-a6,-(a7)
	jsr	g2v190ai_make_grey_tables
	clr.l	g2v190ai_savedpal1
	clr.l	g2v190ai_savedpal2
	move.l	player1,d0
	beq.s	.g2v190ai_draw
	move.l	d0,a5
	move.l	ob_palette(a5),g2v190ai_savedpal1
	move.l	#g2v190ai_palettesw,ob_palette(a5)
	move.l	player2,d0
	beq.s	.g2v190ai_draw
	tst	gametype
	beq.s	.g2v190ai_draw
	move.l	d0,a5
	move.l	ob_palette(a5),g2v190ai_savedpal2
	move.l	#g2v190ai_palettesw,ob_palette(a5)
.g2v190ai_draw
	jsr	drawall		; full render + C2P + db, so initmenu copies an actually grey backdrop
	move.l	player1,d0
	beq.s	.g2v190ai_restore2
	move.l	d0,a5
	move.l	g2v190ai_savedpal1,d0
	beq.s	.g2v190ai_restore2
	move.l	d0,ob_palette(a5)
.g2v190ai_restore2
	move.l	g2v190ai_savedpal2,d0
	beq.s	.g2v190ai_done
	move.l	player2,a5
	move.l	d0,ob_palette(a5)
.g2v190ai_done
...
; Build 16 greyscale remap tables in the same form as calcpalettes produces:
; source colour index -> planar_remap(grey RGB) -> paladjust.  Colours 1..3
; are avoided for the backdrop because initfontpal temporarily replaces them
; with bigfont2 colours while the menu is visible.
g2v190ai_make_grey_tables
	...
	beq.w	.g2v190ai_done
	lea	g2v190ai_palettesw,a2
	lea	g2v190ai_paldat,a3
	moveq	#15,d7
.g2v190ai_ptr_loop
	move.l	a3,(a2)+
	adda.l	#256,a3
	dbf	d7,.g2v190ai_ptr_loop
	lea	paladjust,a5
	move.l	planar_remap,a1
	lea	g2v190ai_palettesw,a2
	moveq	#0,d7		; shade/darkness level
.g2v190ai_shade_loop
	move.l	(a2)+,a3
	move.l	map_rgbs,a0
	move.l	map_rgbsat,a4
.g2v190ai_col_loop
	move	(a0)+,d0		; 12-bit RGB
	move	d0,d1
	move	d0,d2
	and	#$0f00,d0
	lsr	#8,d0
	...
	sub	d7,d0
	bpl.s	.g2v190ai_bright_ok
	moveq	#0,d0
.g2v190ai_bright_ok
	cmp	#15,d0
	bls.s	.g2v190ai_bright_clamped
	moveq	#15,d0
.g2v190ai_bright_clamped
	move	d0,d6
.g2v190ai_remap_try
	move	d6,d0
	move	d0,d1
	...
	lsl	#4,d1
	or	d1,d0
	or	d6,d0		; RGB = grey/grey/grey
	move.b	0(a1,d0.w),d0
	and	#$00ff,d0
	beq.s	.g2v190ai_remap_ok	; colour 0 stays black and is safe
	cmp	#4,d0
	bcc.s	.g2v190ai_remap_ok	; avoid colours 1..3 while bigfont2 is active
	cmp	#15,d6
	bcc.s	.g2v190ai_remap_ok
	addq	#1,d6
	bra.s	.g2v190ai_remap_try
.g2v190ai_remap_ok
	move.b	0(a5,d0.w),(a3)+
	cmp.l	a4,a0
	bcs.s	.g2v190ai_col_loop
	addq	#1,d7
	cmp	#16,d7
	bcs.s	.g2v190ai_shade_loop
.g2v190ai_done
...
refresh	jsr	dispoff
	jsr	finitmenu
	tst	game_menu_active	; v190ai: option refresh while ESC menu is open keeps the backdrop grey
	beq.s	.g2v190ai_refresh_normal
	jsr	g2v190ai_ingamemenu_grey_backdrop
	bra.s	.g2v190ai_refresh_menu
.g2v190ai_refresh_normal
	jsr	predrawall
.g2v190ai_refresh_menu
	lea	gamemenu,a4
	jsr	initmenu2
	jsr	dispon
	rts
...
g2v190i_level_offsets ds.l g2v190i_max_levels	; v190r: script command offsets for START LEVEL chain mode
	...
	even
; v190ai storage for ESC-menu greyscale backdrop.
g2v190ai_savedpal1	dc.l	0
g2v190ai_savedpal2	dc.l	0
g2v190ai_palettesw	ds.l	16
g2v190ai_paldat	ds.b	4096
	even
