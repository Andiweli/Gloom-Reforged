...
; Build a palette-correct fallback colour for long no-wall corridors.
; This is only used when no dark wall column was available to seed the
; frame-local global fog colour.
g2fv_build_default_global
	...
	beq.s	.g2fv_bdg_done
	lea	paladjust,a3
	; very dark warm/neutral RGB, remapped through the active game palette
	; and the chunky/C2P adjust table so it uses a valid screen index.
	move	#$211,d0
	moveq	#0,d1
	move.b	0(a4,d0.w),d1
	and	#$00ff,d1
...
; using the dark global fog colour sampled from a far wall earlier in the
; frame.  This is the fallback for very long corridor openings that have no
; nearby dark wall column to carry a local span.
g2fill_void_fog_remaining
	movem.l	d0-d7/a0-a6,-(a7)
	; v190bf: if no very-far wall was visible in this frame, build a
	; palette-correct default fog colour.  Long corridors with no end wall
	; inside the 6-7 texture-width range otherwise stayed pure black until
	; the end wall became visible enough to seed the global fog colour.
	tst	g2fv_global_ok
	bne.s	.g2fvr_haveglobal
