...
.redrawmenu
	...
	; v190he: one single title menu for all local profiles.
	; Profile-specific data loading (misc/script or stuf/stages) is kept
	; in g2v190i_levelselect_loadscripts; only the visible menu and
	; index/action mapping are unified here.
	lea	startmenu,a4
	jsr	initmenu
...
.notlinked
	...
	and	#$00ff,d0
	; v190he: all local title profiles use the shared Gloom-Deluxe
	; START LEVEL menu and therefore the same index/action mapping.
	tst	d7
...
.g2compat_notlinked
	tst	d7
	beq.s	.g2compat_fire
	cmp	#0,d0		; v190hc: START LEVEL left/right
	beq	.g2v190f_level_lrsel
	cmp	#6,d0		; v190hc: PLAYER 1 row after START LEVEL/spacer
	beq	.g2compat_p1lrsel
	cmp	#7,d0		; v190hc: PLAYER 2 row after START LEVEL/spacer
	beq	.g2compat_p2lrsel
	bra	.sel
.g2compat_fire
	cmp	#0,d0		; v190hc: START LEVEL launch
	beq	.g2v190f_level_start
	cmp	#2,d0
	bcs	.sel		; spacer row below START LEVEL
...
compat_startmenu	dc.b	13		; v190hc: compatibility profiles now include START LEVEL
	dc.b	'START LEVEL '
...
.g2v190i_copy_done
	clr.b	(a1)
	lea	g2v190f_level_text(pc),a0	; v190hc: keep compat START LEVEL text in sync
	lea	g2v190hc_level_text(pc),a1
	moveq	#5,d1
.g2v190hc_copy
	move.b	(a0)+,(a1)+
	beq.s	.g2v190hc_copy_done
	dbf	d1,.g2v190hc_copy
	clr.b	(a1)
.g2v190hc_copy_done
	movem.l	(a7)+,d0-d2/a0-a1
	rts
...
g2v190i_levelselect_loadscripts
	...
	clr	g2v190f_level_index
	bsr	permit
	; v190hc: profile-specific START LEVEL source:
	; Gloom/Gloom Deluxe/Gloom3 -> misc/script, Zombie Massacre -> stuf/stages.
	cmp	#2,g2_game_profile
	beq.s	.g2v190hc_try_zm_stages
.g2v190hc_try_misc_script
...
g2v190i_try_script
	; v190hc: use the normal loadfile path so compressed CrM2 scripts
...
.titlemenu_main	move	#-1,g2v154_titlemenu_lines
	...
	bra.s	.titlemenu_y_done
.titlemenu_compat	move	#-1,g2v154_titlemenu_lines	; v190hc: compat START LEVEL menu uses Deluxe separator rows
	sub	fonth(pc),d6	; v190hc: keep START LEVEL at Deluxe top position
	sub	fonth(pc),d6
	sub	fonth(pc),d6
.titlemenu_y_done
	move	d6,menuy
