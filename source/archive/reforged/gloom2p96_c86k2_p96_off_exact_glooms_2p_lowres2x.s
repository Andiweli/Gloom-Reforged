	...
	;gloom2p96_c86k2: P96 overlay off; TWO PLAYER exact gloom.s lowres 2x split window and screen-edge HUD.
	;
...
debugser	equ	0
debugmem	equ	0
g2p96_overlay_enabled	equ	0	; c86i: 0 hides/disables P96 overlay, -1 re-enables old c85 P96 preview path
	;
...
g2view_scale_x_d0
	; c86f: in TWO PLAYER the split height is fixed at 120.
	; Scale X by the same vertical factor so objects/walls keep their
	; proportions; wider WINDOW SIZE then becomes wider FOV, not stretching.
	tst	g2twop_aspect_mode
	bne	.g2twop_aspect
	cmp	#320,width
	beq	.rts
	movem.l	d1,-(a7)
...
g2view_scale_x_d2
	; c86f: same aspect-safe X scale for secondary wall endpoint.
	tst	g2twop_aspect_mode
	bne	.g2twop_aspect
	cmp	#320,width
	beq	.rts
...
g2view_scale_x_d3
	; c86f: sprite/shape widths must follow vertical scale in 2P,
	; otherwise FULLSCREEN becomes horizontally stretched.
	tst	g2twop_aspect_mode
	bne	.g2twop_aspect
	cmp	#320,width
	beq	.rts
...
.waitskip	clr	doneflag
	;
drawall_	tst	twowins		; c54: gloom2 never had real 2P split; avoid P2 fullscreen overwrite
...
.waitskip2	; v190hy cleanup: log marker removed
	...
	jsr	g2p96_preview_frame	;gloom2p96_c1: coarse live Chunky preview window, AGA unchanged
.g2c55_no_p96_preview
	endc
	jsr	g2twop_restore_after_c2p	;c54: restore user view globals after split C2P
	clr	showflag
	rts

; c54: first real TWO PLAYER split restore for gloom2.s.  The original
; gloom.s rendered two independent windows.  gloom2's chunky renderer rendered
; player1 and then player2 into the same full-screen chunky buffer, so player2
; overwrote the entire frame.  c86g keeps each physical split half at 320x120 and maps WINDOW SIZE to
; distinct safe widths.  TINY/SMALL/MEDIUM/LARGE now differ, while
; EXTRA LARGE..FULLSCREEN are capped at the largest wall-caster-safe width so
; the broken side-wall zones from c86f do not appear.  The HUD is drawn at
; normal split-half coordinates and is no longer scaled/anchored to the 3D view
; width.  First-person gun placement is still intentionally left out.
g2twop_drawall_split
	...
	;
	jsr	g2twop_prepare_view_width	;c86g: map WINDOW SIZE to safe 2P widths
	jsr	g2clearfullchunky	;clear both split halves from the real full-frame base
	jsr	g2twop_center_coloffs	;c86f: center selected split width inside each 320px half
	jsr	g2twop_set_half_view	;c86h sets aspect mode: height-locked up to 160, safe full-FOV above
	;
	...
	jsr	calcscene
	jsr	drawscene
	jsr	g2twop_expand_lowres_2x	;c86k: expand logical 2P render to physical split width
	jsr	g2twop_draw_half_hud	;c55: restore HUD inside player 1 split half
	;
	...
	jsr	drawscene
	jsr	g2twop_expand_lowres_2x	;c86k: expand logical 2P render to physical split width
	jsr	g2twop_draw_half_hud	;c55: restore HUD inside player 2 split half
.g2twop_restore_full
	clr	g2twop_aspect_mode	;c86f: normal full-frame C2P/menu code uses normal scale again
	jsr	g2twop_restore_coloffs	;c86: restore normal 320-column C2P table before full-frame conversion
	jsr	g2twop_set_full_c2p_view
	bra	g2drawall_show

; c55: draw the existing top HUD directly into the currently selected split half.
; Do not call the normal blitscene/gun path here: the first-person gun still uses
; full-screen Y placement and would either be invisible or write below the half.
g2twop_draw_half_hud
	movem.l	d0-d7/a0-a6,-(a7)
	move.l	player_(pc),a5
	tst.l	a5
	beq.s	.done
	; c86h: HUD must be anchored to the physical split-screen edges,
	; not to the centered 3D render window.  Temporarily restore the
	; normal 320-column coloffs table, draw HEALTH/WEAPON at the left
	; edge and LIVES at the right edge, then rebuild the centered 3D
	; coloffs table for the next player/render pass.
...
g2twop_prepare_view_width
	; c86k2: exact original-style TWO PLAYER sizing from gloom.s.
	; gloom.s does NOT render 168/180/320 real columns in 2P.
	; It uses pw/ph=2: 66..90 logical columns x 60 rows, then
	; the window is physically doubled to 132..180 x 120.
	; thinkwin also caps 2P highres width at 180, so larger menu
	; labels must stay at the same safe maximum instead of stretching.
	clr	g2twop_wide_mode
	...
	bcs.s	.small
	bra.s	.full
.tiny	move	#132,d0	; gloom.s: 66 logical columns, pw=2
	bra.s	.store
.small	move	#156,d0	; one original +24px step, 78 logical columns
	bra.s	.store
.full	move	#180,d0	; gloom.s max for 2P highres/pw=2, 90 logical columns
.store	move	d0,g2twop_view_width
	move	d0,d1
	lsr	#1,d1
	move	d1,g2twop_src_width
	rts

g2twop_set_half_view
	; c86k2: render a lowres logical 2P view and expand it to the
	; physical split width afterwards.  This matches the important part
	; of gloom.s: 2P uses pw/ph=2, not a stretched highres render.
	move	g2twop_src_width(pc),d0
	move	d0,width
	move	#60,hite
	move	#320,chunkymodw	;row stride remains the full split/frame width
	lsr	#1,d0
	move	d0,maxx
	neg	d0
	move	d0,minx
	...
	move	#30,maxy
	clr.l	offset
	move	#-1,g2twop_aspect_mode
	rts

	; c86k2: map logical render columns to the left edge of the physical
	; centred 2P window.  After drawscene, g2twop_expand_lowres_2x expands
	; those logical columns/rows to the full physical width/height.
g2twop_center_coloffs
	...
	move	#320,d1
	sub	d0,d1
	lsr	#1,d1		;x start in physical split pixels
	lsl	#2,d1
	ext.l	d1
	add.l	d1,a1
	move	g2twop_src_width(pc),d0
...
g2twop_expand_lowres_2x
	; c86k2: expand the just-rendered logical 2P view in-place.
	; Source: current chunky + xstart, size src_width x 60.
	; Dest:   current chunky + xstart, size view_width x 120.
	; Copy bottom-to-top and right-to-left so source pixels are not overwritten.
	movem.l	d0-d7/a0-a5,-(a7)
	move.l	chunky(pc),a4
	tst.l	a4
	beq.s	.done
	...
	lsr	#1,d1		;x start
	ext.l	d1
	add.l	d1,a4		;base + xstart
	move	g2twop_src_width(pc),d4
	ble.s	.done
	move	g2twop_view_width(pc),d5
	ble.s	.done
	lea	0(a4),a0
	add.l	#320*59,a0
	move	d4,d0
	ext.l	d0
	add.l	d0,a0		;source row 59, one past logical end
	lea	0(a4),a5
	add.l	#320*119,a5
	move	d5,d0
	ext.l	d0
	add.l	d0,a5		;dest row 119, one past physical end
	moveq	#59,d6
.row	move.l	a0,a1
	move.l	a5,a2
	move.l	a5,a3
	sub.l	#320,a3		;upper expanded row
	move	d4,d7
	subq	#1,d7
.pix	move.b	-(a1),d0
	move.b	d0,-(a2)
...
; c86a: rebuild the original full-width coloffs/paladjust tables before the
; C2P pass and before the normal one-player/fullscreen globals are restored.
...
.g2hud_draw_now
	...
	bsr	g2hud_draw_text_top
	;
	; LIVES label and icons at the top-right.  In TWO PLAYER the visible
	; field may be narrower than 320, so anchor the right-side HUD to the
	; selected split width instead of the black side area.
	tst	g2hud_twop_mode
	beq.s	.g2hud_lives_full_x
	move	width(pc),d7
	sub	#84,d7
	cmp	#44,d7
	bge.s	.g2hud_lives_x_ok
	move	#44,d7
	bra.s	.g2hud_lives_x_ok
.g2hud_lives_full_x
	move	#236,d7
.g2hud_lives_x_ok
	moveq	#4,d6		;v190hm: HUD 2px lower
...
.g2hud_lives_ok
	subq	#1,d7
	tst	g2hud_twop_mode
	beq.s	.g2hud_icons_full_x
	move	width(pc),d6
	sub	#44,d6
	cmp	#84,d6
	bge.s	.g2hud_icons_x_ok
	move	#84,d6
	bra.s	.g2hud_icons_x_ok
.g2hud_icons_full_x
	move	#276,d6
.g2hud_icons_x_ok
.g2hud_lives_loop
...
castwalls	;process 'walls' list
	...
	; c86f: TWO PLAYER is different: height stays 120 and X scale is
	; height-locked, so larger WINDOW SIZE must cast a wider raw X range
	; instead of stretching the same 4:3 view.
	move	minx(pc),d7
	tst	g2twop_aspect_mode
	bne	.g2twop_cast
...
.g2twop_cast
	...
	ext.l	d0
	asl.l	#8,d0		; minx << 8
	add.l	d0,d0		; *2, because 120/240 = 1/2
	move.l	d0,g2view_cast_xfp
	move.l	#512,g2view_cast_step	; raw step 2.0 per output column
.g2cast_ready
...
.loop	;do this vert line!
	...
	asr.l	#8,d6
	; c86j: height-locked 2P widths above 160 can ask for angles just
	; outside the original castrots table.  Clamp the lookup so wider
	; test modes do not read garbage wall vectors at the left/right edge.
	cmp	#-160,d6
	bge.s	.g2c86j_cast_min_ok
	move	#-160,d6
.g2c86j_cast_min_ok
	cmp	#159,d6
	ble.s	.g2c86j_cast_max_ok
	move	#159,d6
.g2c86j_cast_max_ok
...
g2hud_twop_mode	dc	0	;c86d anchor right-side HUD inside selected two-player view width
...
g2twop_view_width	dc	0	;c86k physical split width after 2x expand
g2twop_src_width	dc	0	;c86k logical lowres render width before 2x expand
g2twop_wide_mode	dc	0	;c86h nonzero when 2P uses safe full-FOV wide mode (>160)
g2twop_restore_pending	dc	0
g2twop_aspect_mode	dc	0	;c86e nonzero while 2P projection uses height-locked X scale
g2menu_saved_offset	dc.l	0	;c86e ESC-menu full-frame C2P restore values
...
p96prevlog_name	dc.b	'RAM:gloom_p96_preview.txt',0
	even
p96prevlog_ok	dc.b	'P96 preview: c86j p96 off twop no-stretch HUD edge active',10
...
g2v190aj_grey_menu_backdrop
	...
	jsr	pokepal
	tst	twowins
	beq	.g2grey_normal_c2p
	jsr	g2v190aj_twop_full_c2p	;c86f: split preview must be C2P'ed as full 320x240
	bra	.g2grey_db
.g2grey_normal_c2p
	jsr	doc2p
.g2grey_db
...
; c86f: The ESC-menu grey backdrop works on the already rendered chunky
; frame.  In TWO PLAYER that frame contains two 320x120 split views.  The
; normal restored WINDOW SIZE globals would C2P only a centred subwindow and
; make the menu preview look distorted until gameplay resumes.  Convert the
; full 320x240 chunky buffer once, then restore the user's globals.
g2v190aj_twop_full_c2p
	movem.l	d0-d1/a0-a1,-(a7)
