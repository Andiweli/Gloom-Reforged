	...
	;gloom2p96_c86a: P96 overlay disabled by default; TWO PLAYER matches gloom.s 132x120 centered split framing.
	;
...
debugser	equ	0
debugmem	equ	0
g2p96_overlay_enabled	equ	0	; c86a: 0 hides/disables P96 overlay, -1 re-enables old c85 P96 preview path
	;
...
; overwrote the entire frame.  c86a keeps the physical split halves at 320x120,
; but matches the proven gloom.s two-player framing more closely: the visible
; 3D view is centred and only 132x120 pixels wide.  This mirrors gloom.s
; defwindow1_2p/defwindow2_2p (66x60 chixels at 2x2) without physically
; scaling the chunky buffer after rendering.  c55 additionally draws the
; existing top HUD inside each split half; first-person gun placement is still
...
g2twop_drawall_split
	...
	jsr	g2twop_center_coloffs	;c86a: render 132px gloom.s-style view centered in each 320px split half
...
g2twop_set_half_view
	; c86a: physical split half remains 320x120, but the visible 3D
	; projection now follows the proven gloom.s two-player geometry.
	; gloom.s uses 66x60 chixels at 2x2, i.e. 132x120 physical pixels.
	move	#132,width
	move	#120,hite
...
; c86a: temporarily map logical render columns 0..131 to physical screen
; columns 94..225.  These are the same visible bounds as gloom.s 2P
; defwindows: x = 160-(33*2), width = 66*2.  The remaining side areas stay
; black from g2clearfullchunky.
