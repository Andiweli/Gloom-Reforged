...
dotelesfx	move.l	telesfx(pc),a0
	beq.s	.rts		;v105e: no silent/null sample access if teleport.bin was not loaded
	; v105e: force teleport.bin onto SFX channel 0.  The normal playsfx
	; priority/queue path could still lose the teleport sound when all four
	; channels were busy or queued during the level-exit transition.
	lea	sfxs(pc),a1
	moveq	#64,d0
	moveq	#120,d1
	move	d1,fx_priority(a1)
