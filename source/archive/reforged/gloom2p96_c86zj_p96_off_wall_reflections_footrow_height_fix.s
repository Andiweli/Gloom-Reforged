...
.col_loop
	...
	sub	#3,d2
	cmp	d2,d1
	bge	.col_next		; almost no room left below wall bottom
	moveq	#11,d6		; up to 12 reflection rows
	move	vd_h(a0),d3
	lsr	#1,d3
	subq	#1,d3
	cmp	d3,d6
	ble.s	.depth_from_height_ok
	move	d3,d6
.depth_from_height_ok
	move	hite(pc),d2
	sub	#3,d2
	sub	d1,d2
	subq	#1,d2		; rows available below wall bottom
	cmp	d2,d6
	ble.s	.depth_from_screen_ok
	move	d2,d6
.depth_from_screen_ok
	; c86zj: cap the visible reflection height by the wall-foot screen row,
	; not just by free screen space below the wall.  The old version let far
	; walls with a high foot row use a tall reflected tail, while close walls
	; near the bottom had almost no room and looked shorter.  This makes the
	; reflection grow as the wall-foot moves downward on screen.
	move	d1,d3		; d1 = wall bottom / wall-foot screen row
	sub	midy(pc),d3
	bgt.s	.foot_positive
	moveq	#0,d3
.foot_positive
	lsr	#2,d3		; about one reflected row per 4 screen rows below horizon
	addq	#1,d3
	cmp	#11,d3
	ble.s	.foot_cap_ok
	moveq	#11,d3
