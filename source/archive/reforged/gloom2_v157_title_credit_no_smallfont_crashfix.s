...
; v137: dark centre, lighter outside.
g2_reflect_dark_rgb
	dc	$630,$060,$600,$066,$606,$630,$060,$600
g2_reflect_rgb
	dc	$a60,$0a0,$a00,$0aa,$a0a,$a60,$0a0,$a00
...
.rabs_ok	move	g2_shadow_rx(pc),d2
	cmp	d2,d1
	bgt	.rts
	; v135: softer/feathered reflection edge.  Keep the basic ellipse,
	; but mark the outer horizontal band so it can be dithered against the
	; floor instead of ending as a hard coloured wall.  Blob shadows stay hard.
...
.outer_dither
	...
	bra	.ryloop
.centre_dither
	; centre is denser/darker, but still leaves some floor pixels visible.
	move	g2_shadow_curx(pc),d3
	add	d6,d3
	and	#3,d3
	cmp	#3,d3
...
; v157: draw the optional pics/gloom logo plus the title/footer credit
; only for the main title menu.  ABOUT deliberately uses the clean title
; background without these overlays.
g2v145_draw_menu_gloombrush
	tst	aga
	beq.s	.credit
	jsr	g2v142_draw_gloombrush_safe
.credit	jsr	g2v156_draw_title_credit
.rts	rts
...
.notlinked	cmp	#3,d0
	...
	bra	.sel
	;
.p1sel	lea	p1ctype(pc),a1
	lea	p1_ob_cntrl,a2
	lea	p2_ob_cntrl,a3
	bsr	inccntrl
	bra	.sel
	;
.p2sel	lea	p2ctype(pc),a1
	lea	p2_ob_cntrl,a2
	lea	p1_ob_cntrl,a3
	bsr	inccntrl
	bra	.sel
...
initmenu2	;
	...
	mulu	d2,d0
	sub	d0,d6	;Y
	; v154: title/main menus one more text row higher than v153.
	; Detect startmenu/startmenu2 after the count byte has been consumed.
...
; v156: draw two thin 80px medium-dark yellow separators into the blank
; title-menu rows, 1px higher than v154: one between TWO PLAYER COMBAT
; and PLAYER 1, one between PLAYER 2 and REMOTE LINK OPTIONS.
; Colour index 2 is used: still dark/dirty yellow, but brighter than v155.
...
.plane	move.l	a1,a2
	...
	add.l	d0,a2
	cmp	#1,d2
	beq.s	.mediumyellowplane
	moveq	#0,d0
	bra.s	.fill
.mediumyellowplane	moveq	#-1,d0
.fill	moveq	#9,d1
.byte	move.b	d0,(a2)+
	dbf	d1,.byte
	addq	#1,d2
...
; v157: title/footer credit in the black area below the title art.
; Do NOT switch to smallfont here: mixing smallfont with the active title
; font on this screen can crash.  Use the already-active title/menu font
; and draw into both title buffers so it remains stable after menu copies.
g2v156_draw_title_credit
	movem.l	d0-d7/a0-a6,-(a7)
	bsr.s	.draw_current_show
	move.l	showbitmap,d7
	move.l	drawbitmap,d0
	beq.s	.nodraw
	move.l	d0,showbitmap
	bsr.s	.draw_current_show
	move.l	d7,showbitmap
.nodraw	movem.l	(a7)+,d0-d7/a0-a6
	rts
.draw_current_show
	lea	g2v156_title_credit(pc),a4
	move.l	a4,a0
	moveq	#-1,d0
.len	addq	#1,d0
	tst.b	(a0)+
...
g2v156_title_credit	dc.b	'GLOOM REFORGED IDEA BY ANDIWELI',0
	even
