...
.g2dm_have_weapon
	...
	; v78: use only the selected muzzle shape itself and scale that same
	; graphic larger twice in succession.  No more halo layering behind a
	; smaller core.  The 3-frame sequence is now 1x -> 2x -> 3x.
...
.g2dm_scale3
	sub	d0,d1		;centre 3x flash around the same muzzle point
	sub	d3,d4
	moveq	#3,d7
	bsr	g2drawgun_coloffs_shape_scaled
	bra.s	.g2dm_done
.g2dm_scale2
	move	d0,d6
	asr	#1,d6
	sub	d6,d1		;centre 2x flash
	move	d3,d6
	asr	#1,d6
	sub	d6,d4
	moveq	#2,d7
...
.nomega	bsr	shoot
	;
.shdone	move.l	(a7)+,a0
	move	#3,g2gun_firetimer	;v78: 3-frame single-shape flash 1x->2x->3x
	move.l	(a0),a0
	moveq	#32,d0
	moveq	#0,d1
	bsr	playsfx
