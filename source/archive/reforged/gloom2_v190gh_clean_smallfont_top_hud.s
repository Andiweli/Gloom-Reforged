...
g2draw_statusbar_base
	; v190gh: bottom smallfont2/statusbar background is no longer used.
	rts

g2draw_statusbar_clear
	; v190gh: no lower statusbar clear strip.
	rts
...
g2drawgun
	...
	cmp	#1,g2_game_profile	; v190gh: Classic Gloom has no gun overlay
	beq.w	.g2dg_done
	move.l	gunpic(pc),d0
	beq.w	.g2dg_done
...
.g2dg_have_shape_index
	...
	move	6(a0),d3		;height
	move	#240,d4
	sub	d3,d4		;v190gh: bottom-align gun after removing the statusbar
...
predrawall	;draw up everything....
	...
	bsr	clspic
	bsr	calcoffset
	; v190gh: no lower statusbar background.
...
showstats
	;a5=player
	; v190gh: clean top HUD using the same chunky/C2P render path as the game.
	; Source is panel=smallfont.bin (not smallfont2), loaded through the normal
	; data-file path and remapped by the existing remapanim block.
	movem.l	d0-d7/a0-a4,-(a7)
	move.l	panel(pc),a0
	tst.l	a0
	beq.w	.g2hud_done
	cmp.l	#50,(a0)		; anim/shape count sanity, avoids drawing from wrong data
	bcs.w	.g2hud_done
	cmp	#320,4(a0)		; smallfont.bin full strip width
	bne.w	.g2hud_done
	cmp	#8,6(a0)
	bcs.w	.g2hud_done
	;
	...
	bsr	g2hud_draw_text
	;
	; top-right lives label and hearts
	move	#236,d7
	moveq	#2,d6
	lea	hud_lives(pc),a4
	bsr	g2hud_draw_text
...
.g2hud_lives_loop
	move	d5,d0
	moveq	#2,d1
	moveq	#43,d2		; smallfont heart/life icon
	bsr	g2hud_draw_shape
	addq	#8,d5
	dbf	d7,.g2hud_lives_loop
.g2hud_no_lives
	;
	; health bar, using the proven Gloom2 colour-cell shapes from smallfont.bin
...
.g2hud_hp_ok
	...
	subq	#1,d7
.g2hud_hploop
	moveq	#45,d2		; red/danger cell
	cmp	#10,d3
	blt.s	.g2hud_hpcol_ok
	moveq	#46,d2		; middle cell
	cmp	#18,d3
	blt.s	.g2hud_hpcol_ok
	moveq	#47,d2		; green cell
.g2hud_hpcol_ok
	move	d6,d0
	moveq	#2,d1
	bsr	g2hud_draw_shape
...
.g2hud_nohp
	;
	; weapon/upgrade bar, same shape family as the old Gloom2 statusbar but at top-left
	moveq	#5,d7
	sub.b	ob_reload(a5),d7
	blt.s	.g2hud_done
	move	ob_weapon(a5),d2
	add	#39,d2		; weapon upgrade cell 39..43
	moveq	#44,d6
.g2hud_wploop
	move	d6,d0
	moveq	#12,d1
...
g2hud_draw_text	;a4=zero-terminated text, d7=x, d6=y
...
g2hud_draw_shape	;d0=x, d1=y, d2=shape#; source=panel/smallfont.bin; writes chunky top area
	movem.l	d0-d7/a0-a4,-(a7)
	move.l	panel(pc),a0
	tst.l	a0
	beq.w	.g2hds_done
	...
	beq.w	.g2hds_done
	add.l	d3,a0
	addq	#4,a0		; skip handles, as drawchunky does
	movem	(a0)+,d2-d3	;width,height
	tst	d2
	ble.w	.g2hds_done
	tst	d3
...
; v190gh: lower statusbar message scroller removed with the bottom panel.
...
.g2bs_not_blackout
	;
	; v190gh: no lower statusbar/panel.  Draw gun at the bottom of the
	; expanded 240-line playfield, then draw the smallfont top HUD.
	bclr	#7,ob_update(a5)
	bsr	g2drawgun
	bsr	showstats
	clr	panelcnt
...
hite	dc	240	;v190gh: fullscreen default uses former statusbar area
minx	dc	-160	;v44: fullscreen default
midx	;
maxx	dc	160	;v44: fullscreen default
miny	dc	-120	;v190gh: fullscreen default
midy	;
maxy	dc	120	;v190gh: fullscreen default
wdiv32	dc	0
wrem32	dc	0
...
progfiles	dc.l	gloomcfg
	...
	even
	dc.l	panel
	dc.b	'misc/smallfont.bin',0	; v190gh: top HUD source, no lower smallfont2 panel
	even
	dc.l	gunpic
...
g1progfiles	dc.l	gloomcfg
	...
	even
	; v190gh: Classic Gloom has no gun overlay; do not require/load misc/gun.bin here.
	dc.l	titlemed+1
	dc.b	'sfxs/med1',0
	even
	dc.l	loadingmed+1
...
zmprogfiles	dc.l	gloomcfg
	...
	even
	dc.l	panel
	dc.b	'stuf/smallfont.bin',0	; v190gh: top HUD source, no lower smallfont2 panel
	even
	dc.l	gunpic
	dc.b	'stuf/gun.bin',0
	even
