...
; v190fl: original Gloom/Gloom2 sound-effect pointer restore.
; The mixer and event calls above are the v58/original path again.  This small
; loader fixes the other half of the problem: after the BlackMagic profile layer
; has loaded its program table, force the core SFX pointers back to the working
; sfxs/ files when those files are present.  Existing profile sounds are kept if
; an original file is absent.
g2v190fl_load_original_sfx
	movem.l	d0-d2/a0-a3,-(a7)
	lea	g2v190fl_sfxfiles(pc),a2
.g2v190fl_loop
	move.l	(a2)+,d0
	beq.s	.g2v190fl_done
	move.l	d0,a3
	move.l	a2,a0
	moveq	#2,d1	; original progfiles used +1 => chip memory for SFX/MED data
	jsr	loadfile
	tst.l	d0
	beq.s	.g2v190fl_skip_store
	move.l	d0,(a3)
.g2v190fl_skip_store
.g2v190fl_z
	tst.b	(a2)+
...
.agaa	bsr	loadfiles
	...
	bsr	loadfiles
	jsr	g2v190fl_load_original_sfx	; v190fl: restore original Gloom/Gloom2 SFX pointers after profile load
