	...
	;gloom2_c86zdt: diagnose/guard missing player2 after execevent in TWO PLAYER/COMBAT; no ESC/dogamemenu hacks.
	;
...
g2zcs_player1_present_before_forbid
	...
	jsr	g2combat_stage_log
	; c86zdt: player1 alone is not enough for TWO PLAYER COMBAT.
	; If player2 is missing after execevent, the later resetplayer path
	; writes through a null/stale a5 and can Guru before any later logger.
	tst	gametype
	beq.s	g2zdt_no_player2_required_before_forbid
	tst.l	player2
	bne.s	g2zdt_player2_present_before_forbid
	lea	g2combat_stage_player2_missing,a0
	move.l	#g2combat_stage_player2_missing_len,d3
	jsr	g2combat_stage_log
	bra.s	g2zcs_after_player1_stage
g2zdt_player2_present_before_forbid
	lea	g2combat_stage_player2_ok,a0
	move.l	#g2combat_stage_player2_ok_len,d3
	jsr	g2combat_stage_log
g2zdt_no_player2_required_before_forbid
g2zcs_after_player1_stage
	...
	bra	gameover
g2v10_player1_ok
	; c86zdt: hard guard for two-player modes.  If player2 was not
	; spawned by execevent, do not continue into resetplayer with a5=0.
	tst	gametype
	beq.s	g2zdt_player2_ok_after_forbid
	tst.l	player2
	bne.s	g2zdt_player2_ok_after_forbid
	lea	g2v10_playerfailmenu(pc),a0
	jsr	g2v10_showmsg
	bra	gameover
g2zdt_player2_ok_after_forbid
	;
...
.doit	lea	joyx,a0
	jsr	readjoy1
	lea	joyx0,a0
	jsr	readkeys	; c86zdt build safety: logger growth can put readkeys out of BSR range
	move.l	joyx0(pc),d0
	or.l	d0,joyx
	move.l	joyb0(pc),d0
	or.l	d0,joyb
...
g2combat_stage_player2_missing	dc.b	'COMBAT: PLAYER2 MISSING after execevent',10
g2combat_stage_player2_missing_len	equ	*-g2combat_stage_player2_missing
g2combat_stage_player2_ok	dc.b	'COMBAT: player2 OK after execevent',10
g2combat_stage_player2_ok_len	equ	*-g2combat_stage_player2_ok
	even
