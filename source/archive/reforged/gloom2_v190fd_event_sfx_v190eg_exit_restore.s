...
execevent	;d0=event number to execute...1,2...
	;
	sf	doorsfxflag	; v190fd: restore v190eg delayed local-event door SFX path
...
exec_loop	move	(a6)+,d0
	...
	warn	#$f0f
	;
.rts	tst	doorsfxflag	; v190fd: v190eg-style delayed guaranteed door/switch SFX
	beq.s	.nodoor
	clr	doorsfxflag
	movem.l	d0-d1/a0,-(a7)
	move.l	doorsfx(pc),a0
	moveq	#64,d0
	moveq	#90,d1	; v190fd: restored stable v190eg event priority
...
exec_changetxt	;
	...
	move	d0,changedtxt
	move.b	d0,zo_t(a1)
	st	doorsfxflag	; v190fd: switch/texture uses delayed guaranteed event SFX
	bra	exec_loop

g2playdoorsfx_once	; v190fd: immediate door SFX path disabled; stable v190eg path plays at event end
	rts
...
exec_rotpolys	;
	...
	bra	exec_loop
	;
.ok	st	doorsfxflag	; v190fd: rotating/morphing doors use delayed guaranteed event SFX
