...
solidstrip	set	0
drawstrip2	; v203 chunky-safe transparent wall strip overlay with darker, more transparent green glass
	...
	; texels now keep more of the already-rendered background detail (about
	; 75% background / 25% dark-green tint), which should preserve the door's
	; inner horizontal detail better as you approach while keeping the glass
	; visibly dark-green and transparent.
	movem.l	d0-d7/a0-a5,-(a7)
	move.l	vd_data(a4),d0
	beq	.rts
	move.l	d0,a0		; source texture column
	move	#$080,d6		; dark green RGB12 tint for 25% glass contribution
	move.l	chunkymod(pc),d7
	move	vd_y(a4),d2
...
.loop
	...
	bra.s	.next
.glass	addx.l	d3,d0		; advance before blend work clobbers CCR/X
	moveq	#0,d1
