...
; v190ap: renderer one shade step darker only, no distance dither
...
.vloop	;
	...
	move	0(a5,d6*2),d5
	; v190ap: make flats one shade step darker only.
	; No distance dithering here, to avoid stripe/line artefacts.
	cmp	#15,d5
	bcc.s	.g2v190ap_flatshade_ok
	addq	#1,d5
.g2v190ap_flatshade_ok
...
.solid	move.l	a3,vd_data(a0)	;start column
	...
	move	0(a2,d2*2),d3
	; v190ap: make walls one shade step darker only.
	; Keeps original hard shade lookup, just globally darker.
	cmp	#15,d3
	bcc.s	.g2v190ap_wallshade_ok
	addq	#1,d3
.g2v190ap_wallshade_ok
	movem	d2-d3,vd_z(a0)
	;
	move	#-256,d3
	sub	camy(pc),d3
