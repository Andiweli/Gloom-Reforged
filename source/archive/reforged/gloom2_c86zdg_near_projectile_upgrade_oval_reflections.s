	...
	;gloom2_c86zdg: projectiles/weapon-upgrades grow wider and thicker into a fuller oval reflection at close range.
	;
...
.bul_min_ok
	...
	moveq	#16,d0
.rx_ok
	; c86zdg: make projectile/weapon-upgrade reflections read as a real
	; oval when close.  The old path kept almost the same thin footprint
	; at every distance.  Grow only the existing working reflection owners;
	; non-weapon pickup/token reflections stay removed.
	cmp	#(2<<grdshft),d2
	bge.s	.rx_store
	move	d0,d1
	lsr	#1,d1		; <2 map widths: +50% width
	add	d1,d0
	cmp	#(1<<grdshft),d2
	bge.s	.rx_cap
	move	d0,d1
	lsr	#2,d1		; very close: a little extra swell
	add	d1,d0
.rx_cap
	cmp	#24,d0
	ble.s	.rx_store
...
.rband_ok
	; c86zdg: when the radius has grown because the shot/upgrade is near,
	; thicken the mid/body bands too.  Outer edge remains sparse so the
	; footprint is oval instead of a filled rectangle.
	move	g2_shadow_rx(pc),d3
	cmp	#16,d3
	blt.s	.oval_thick_done
	move	g2_reflect_softedge(pc),d0
	cmp	#2,d0
	beq.s	.oval_thick_done	; keep far outer rim 1 row
	cmp	#1,d0
	bne.s	.oval_body_band
	moveq	#2,d6		; near mid band: 3 rows
	bra.s	.oval_thick_done
.oval_body_band
	moveq	#3,d6		; near centre: 4 rows
	cmp	#20,d3
	blt.s	.oval_thick_done
	moveq	#4,d6		; very close centre: 5 rows
.oval_thick_done
	move.l	chunkymod(pc),d7
	tst	g2_reflect_pickup
	bne.s	.pick_anchor_a
