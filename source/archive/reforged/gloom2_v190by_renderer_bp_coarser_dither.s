...
.g2dither_loop
	...
	cmp	#4,d6
	beq.s	.g2thr4
	; threshold 1: sparse coarse 4x4-style block pattern
	tst.b	g2_cur_xmod
	bne.s	.g2basepix
	btst	#2,d2
	bne.s	.g2basepix
	...
	bra.s	.g2altpix
.g2thr4
	; about 25 percent alternate pixels, coarser vertical phase
	tst.b	g2_cur_xmod
	bne.s	.g2basepix
	btst	#2,d2
	bne.s	.g2basepix
	bra.s	.g2altpix
.g2thr8
	; about 50 percent coarse checker/crosshatch
	tst.b	g2_cur_xmod
	beq.s	.g2thr8_x0
	btst	#2,d2
	beq.s	.g2altpix
...
.g2thr8_x0
	...
	bra.s	.g2basepix
.g2thr12
	; about 75 percent alternate pixels, inverse of sparse coarse pattern
	tst.b	g2_cur_xmod
	bne.s	.g2altpix
	btst	#2,d2
	bne.s	.g2altpix
...
; v190by: Build the same wider clean transition map as v190bx, but
; use a coarser ordered pattern in the wall draw loop.  The transition
; width stays 32px per side; only the visible dither grain is enlarged.
g2build_wallblend
	movem.l	d0-d7/a0-a6,-(a7)
...
; d0 = left palette, d1 = right palette, d3 = boundary x.
; 32px each side: 75/50/25/sparse ordered mix as distance increases.
g2blend_boundary20
	movem.l	d0-d7/a0-a2,-(a7)
...
.loop	move.l	(a6)+,a1
	...
	lsr	#1,d6	; v190by: coarser 2-column dither cells
	and	#3,d6
