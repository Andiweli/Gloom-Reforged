...
.notboost
	...
	tst	d7
	bne	.loop
	cmp	#2,gametype	; c86zdf: remember source of finished=1 in TWO PLAYER COMBAT
	bne.s	.g2zdf_no_menu_finish1_src
	move	#1,g2combat_finish1_source
.g2zdf_no_menu_finish1_src
	move	#1,finished
	;
...
.serhere	bsr	serget
	...
	beq.s	.djoy
	and	#$ff,d0
	cmp	#1,d0		; c86zdf: linked/serial finished=1 source marker
	bne.s	.g2zdf_no_serial_finish1_src
	cmp	#2,gametype
	bne.s	.g2zdf_no_serial_finish1_src
	move	#2,g2combat_finish1_source
.g2zdf_no_serial_finish1_src
	move	d0,finished
	moveq	#0,d0
	;bra.s	.sgot
	;
...
.nolmed	;
	...
	clr	finished
	clr	finished2
	clr	g2combat_finish1_source	; c86zdf: clear diagnostic source marker
	clr	g2combat_escape_latch	; c86zdl: reset combat ESC edge latch
	clr	g2combat_escape_cooldown	; c86zdl: reset combat startup ESC cooldown
	cmp	#2,gametype	; c86zdl: TWO PLAYER COMBAT must not inherit stale ESC/menu input
	bne.s	g2zdl_no_combat_esc_start_guard
	move	#-1,g2combat_escape_latch	; require one raw ESC release before accepting ESC
	move	#50,g2combat_escape_cooldown	; ignore ESC for first ~50 frames after start
g2zdl_no_combat_esc_start_guard
...
mainloop	; v190hy cleanup: log frame counter removed
	jsr	drawall
	; c86zdl: In TWO PLAYER COMBAT, only accept ESC as a fresh raw ESC
	; edge after startup cooldown.  This prevents the intermission/menu start
	; input path from immediately opening dogamemenu and auto-selecting QUIT GAME.
	cmp	#2,gametype
	bne.s	g2zdl_noncombat_escape_path
	tst	g2combat_escape_cooldown
	beq.s	g2zdl_combat_check_raw_escape
	subq	#1,g2combat_escape_cooldown
	clr	escape
	bra.s	g2zdl_noesc
g2zdl_combat_check_raw_escape
	move.l	rawtable,a0
	key	$45
	beq.s	g2zdl_combat_escape_released
	clr	escape
	tst	g2combat_escape_latch
	bne.s	g2zdl_noesc
	move	#-1,g2combat_escape_latch
	jsr	dogamemenu
	clr	escape
	bra.s	g2zdl_noesc
g2zdl_combat_escape_released
	clr	g2combat_escape_latch
	clr	escape
	bra.s	g2zdl_noesc
g2zdl_noncombat_escape_path
	move	escape(pc),d0
	beq.s	g2zdl_noesc
	jsr	dogamemenu
	clr	escape
g2zdl_noesc	; c86zdl: global no-ESC label
...
.g2no_tele_black_hold
	move	finished(pc),d0
	beq	mainloop
	; c86zdf: before leaving TWO PLAYER COMBAT, log where finished=1 came from.
	cmp	#2,gametype
	bne.s	g2zdf_no_finished_source_log
	move	finished(pc),d0
	and	#127,d0
	cmp	#1,d0
	bne.s	g2zdf_finished_not_quit
	move	g2combat_finish1_source,d0
	cmp	#1,d0
	beq.s	g2zdf_finished_quit_menu
	cmp	#2,d0
	beq.s	g2zdf_finished_quit_serial
	lea	g2combat_stage_finished1_unknownsrc,a0
	move.l	#g2combat_stage_finished1_unknownsrc_len,d3
	bra.s	g2zdf_write_finished_source
g2zdf_finished_quit_menu
	lea	g2combat_stage_finished1_menu,a0
	move.l	#g2combat_stage_finished1_menu_len,d3
	bra.s	g2zdf_write_finished_source
g2zdf_finished_quit_serial
	lea	g2combat_stage_finished1_serial,a0
	move.l	#g2combat_stage_finished1_serial_len,d3
	bra.s	g2zdf_write_finished_source
g2zdf_finished_not_quit
	lea	g2combat_stage_finished_not1,a0
	move.l	#g2combat_stage_finished_not1_len,d3
g2zdf_write_finished_source
	jsr	g2combat_stage_log_safe_forbid
g2zdf_no_finished_source_log
	;
mainexit	st	paused
	bsr	chatoff
	bsr	dispoff
...
; c86zdf: diagnostic DOS log while gameplay is in Forbid region.
; Briefly permits, writes latest stage, then forbids again. Preserves all registers.
...
g2combat_stage_finished1_menu	dc.b	'COMBAT: finished=1 source dogamemenu EXIT',10
g2combat_stage_finished1_menu_len	equ	*-g2combat_stage_finished1_menu
g2combat_stage_finished1_serial	dc.b	'COMBAT: finished=1 source serial/link',10
g2combat_stage_finished1_serial_len	equ	*-g2combat_stage_finished1_serial
g2combat_stage_finished1_unknownsrc	dc.b	'COMBAT: finished=1 source UNKNOWN',10
g2combat_stage_finished1_unknownsrc_len	equ	*-g2combat_stage_finished1_unknownsrc
g2combat_stage_finished_not1	dc.b	'COMBAT: finished non-quit code',10
g2combat_stage_finished_not1_len	equ	*-g2combat_stage_finished_not1
	even
g2combat_finish1_source	dc.w	0
g2combat_escape_latch	dc.w	0	; c86zdl: raw ESC has to be released before next combat menu open
g2combat_escape_cooldown	dc.w	0	; c86zdl: startup frames where combat ignores ESC
	even
