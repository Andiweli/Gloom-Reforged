...
g2draw_statusbar_base
	; v190fz: bottom Gloom2/smallfont2 statusbar background is no longer used.
	rts

g2draw_statusbar_clear
	; v190fz: no lower statusbar clear strip.
	rts
...
g2drawgun
	...
	cmp	#1,g2_game_profile	; v190fz: Classic Gloom has no first-person gun overlay
	beq.w	.g2dg_done
	move.l	gunpic(pc),d0
	beq.w	.g2dg_done
...
.g2dg_have_shape_index
	...
	sub	d0,d1		;x
	move	6(a0),d3		;height
	move	#266,d4		;v190ga: old lower statusbar was about 26px high
	sub	d3,d4		; draw gun 26px lower, cropped at the screen bottom
...
.g2dm_have_weapon
	...
	move	6(a0),d3		;height
	move	#237,d4		;v190ga: keep muzzle aligned with gun moved 26px down
...
; Try the shared smallfont.bin HUD font for all Black-Magic profiles.
; Menus still use bigfont2 on Gloom Deluxe/Gloom3/ZM; this is only for the
; top-left HEALTH/WEAPON HUD after the bottom statusbar was removed.
; v190gd: use a dedicated HUD font pointer.  smallfont_ is often already
; occupied by Gloom2 smallfont2.bin, so do not treat that as a valid HUD font.
g2loadsmallfontfallback
	movem.l	d0-d1/a0,-(a7)
	tst.l	g2hudsmallfont
	bne.s	.g2lsf_done
	lea	g2sf_name_miscbin(pc),a0
	moveq	#1,d1
	jsr	loadfile
	move.l	d0,g2hudsmallfont
	bne.s	.g2lsf_done
	lea	g2sf_name_stufbin(pc),a0
	moveq	#1,d1
	jsr	loadfile
	move.l	d0,g2hudsmallfont
	bne.s	.g2lsf_done
	lea	g2sf_name_miscraw(pc),a0
	moveq	#1,d1
	jsr	loadfile
	move.l	d0,g2hudsmallfont
	bne.s	.g2lsf_done
	lea	g2sf_name_stufraw(pc),a0
	moveq	#1,d1
	jsr	loadfile
	move.l	d0,g2hudsmallfont
	bne.s	.g2lsf_done
	; Last resort for Classic Gloom, where smallfont_ already points to
	; misc/smallfont.bin from the profile progfiles table.
	move.l	smallfont_(pc),d0
	move.l	d0,g2hudsmallfont
.g2lsf_done
	movem.l	(a7)+,d0-d1/a0
	rts

g2hudsmallfont	dc.l	0

g2sf_name_miscbin	dc.b	'misc/smallfont.bin',0
	even
g2sf_name_stufbin	dc.b	'stuf/smallfont.bin',0
	even
g2sf_name_miscraw	dc.b	'misc/smallfont',0
	even
g2sf_name_stufraw	dc.b	'stuf/smallfont',0
	even
...
predrawall	;draw up everything....
	...
	bsr	clspic
	bsr	calcoffset
	; v190fz: no bottom statusbar background in the new full-height view.
...
.waitskip2	lea	g2log_msg_da_wait2_ok,a0
	...
	jsr	g2log_drawstep
	jsr	doc2p
	; v190gd: draw classic Gloom-style top-left HUD after C2P, before display swap.
...
showstats	;a5=player, draw classic Gloom HUD after C2P
	; v190gf: draw old smallfont.bin HUD with CPU into the current planar layout.
	; The v190ge blitter path used the old interleaved Gloom layout and corrupts
	; plane-major/RTG bitmaps, causing top-left dither garbage instead of HUD.
	movem.l	d0-d7/a0-a6,-(a7)
	move.l	g2hudsmallfont(pc),d0
	bne.s	.g2hud_have_font
	move.l	smallfont_(pc),d0
	beq.w	.g2hud_done
.g2hud_have_font
	move.l	d0,a6		; a6 = real misc/stuf smallfont.bin shapetable
	; v190gg: do not clear a HUD rectangle; shape masks draw only HUD pixels.
	;
	; labels: same text/positions as gloom.s initstats, byte-aligned for safe CPU draw
	moveq	#0,d7
	moveq	#2,d6
	lea	hud_health(pc),a4
	bsr	g2hud_blittext_planar
	moveq	#0,d7
	moveq	#12,d6
	lea	hud_weapon(pc),a4
	bsr	g2hud_blittext_planar
	;
	; health bar: same shape numbers and loop logic as gloom.s showstats
	moveq	#48,d5
	moveq	#40,d4
	moveq	#0,d6
...
.g2hud_hpskip
	...
	moveq	#2,d1
	move	d4,d2
	bsr	g2hud_blit_oldsmallfont
	addq	#2,d5
	addq	#1,d6
	bra.s	.g2hud_hploop
.g2hud_hpdone
	;
	; weapon bar: same shape numbers and reload comparison as gloom.s showstats
	moveq	#48,d5
	moveq	#49,d4
	sub	ob_weapon(a5),d4	; 0..4 => 49..45 active cell
	moveq	#1,d7
.g2hud_wploop
	move.l	a6,a0
	move.l	drawbitmap(pc),a1
...
g2hud_blittext_planar	;a4=zero-terminated text, d7=x, d6=y, a6=font
	movem.l	d0-d2/d6-d7/a0-a1/a4,-(a7)
.g2hudt_loop
	move.b	(a4)+,d2
	beq.s	.g2hudt_done
	cmp.b	#' ',d2
...
.g2hudt_space
	...
	bra.s	.g2hudt_loop
.g2hudt_done
	movem.l	(a7)+,d0-d2/d6-d7/a0-a1/a4
	rts


g2hud_clear_top_area
	; v190gg: disabled.  v190gf cleared rows using the wrong stride and produced
	; horizontal black bands across the top of the screen.  The old smallfont
	; shapes have their own cookie/mask, so a separate rectangle clear is not needed.
	rts


g2hud_blit_oldsmallfont	;a0=old smallfont shapetable, a1=bitmap, d0=X, d1=Y, d2=char
	; v190gg: CPU copy for the original Black-Magic smallfont.bin layout.
	; Important: the first header word is NOT widthwords.  It is the old blitter
	; modulo helper: (widthwords-1)*2.  The real widthwords is stored in the
	; low 6 bits of the blitter size word at +6.  v190gf used the helper word as
	; width and therefore skipped most glyphs, while also reading source rows
	; without the old 40-byte stride.
	movem.l	d0-d7/a0-a6,-(a7)
	tst.l	a0
	beq.w	.g2hudb_done
	tst.l	a1
	...
	move.l	4(a0,d2*4),d3
	beq.w	.g2hudb_done
	add.l	d3,a0		; a0 = old shape record
	move.l	a0,a2
	add.l	(a0),a2		; a2 = cookie/mask rows
	lea	8(a0),a3		; a3 = image rows, old smallfont image starts at +8
	move	6(a0),d2		; old blitter size: (rows*planes)<<6 | widthwords
	move	d2,d3
	and	#$003f,d2		; d2 = real width in words
	beq.w	.g2hudb_done
	cmp	#8,d2
	bhi.w	.g2hudb_done	; defensive: HUD glyphs/cells are small
	lsr	#6,d3			; total source rows = oldHeight * 7 source planes
	ble.w	.g2hudb_done
	moveq	#7,d4
	divu	d4,d3			; d3.w = pixel height (source rows / 7)
	and.l	#$ffff,d3
	ble.w	.g2hudb_done
	cmp	#24,d3
	bhi.w	.g2hudb_done
	; source row stride in old smallfont is always 40 bytes.  After copying
	; d2 words, skip 40-(d2*2) bytes in both cookie and image sources.
	moveq	#40,d4
	move	d2,d5
	add	d5,d5
	sub	d5,d4			; d4 = source skip after each source plane row
	move.l	a1,a4		; a4 = destination row/plane0 base
	mulu	linemodw(pc),d1
	add.l	d1,a4
	lsr	#3,d0			; byte-align x.  HUD callers use byte-aligned positions.
	add.w	d0,a4
	subq	#1,d3			; row loop count
.g2hudb_row
	move.l	a4,a5		; plane0 dest for this y
	moveq	#7-1,d7		; old smallfont source planes
	moveq	#0,d6			; destination/source plane index
.g2hudb_plane
	move.l	a5,a1
	move	d2,d5
	subq	#1,d5
.g2hudb_word
	...
	move	(a3)+,d1		; image bits
	not	d0
	and	d0,(a1)		; clear shape pixels in this destination plane
	not	d0
	and	d0,d1
	or	d1,(a1)+		; draw image bits
	dbf	d5,.g2hudb_word
	; skip the unused remainder of the old 40-byte source row
	move	d4,d0
	ext.l	d0
	add.l	d0,a2
	add.l	d0,a3
	add.l	bpmod(pc),a5		; next destination plane
	addq	#1,d6
	cmp	bitplanes(pc),d6
	bcc.s	.g2hudb_skip_remaining_src
	dbf	d7,.g2hudb_plane
	bra.s	.g2hudb_nextrow
.g2hudb_skip_remaining_src
	; Target has fewer planes than the old 7-plane source: consume remaining
	; source rows without writing, so the next pixel row stays aligned.
	dbf	d7,.g2hudb_skip_plane
	bra.s	.g2hudb_nextrow
.g2hudb_skip_plane
	move	d2,d5
	subq	#1,d5
...
; v190fz: lower statusbar message scroller removed with the bottom panel.
...
.g2bs_not_blackout
	;
	; v190fz: no lower statusbar/panel. Draw optional gun at the bottom
	; of the expanded 240-line playfield, then top-left smallfont HUD.
	bclr	#7,ob_update(a5)
	bsr	g2drawgun
	; v190gd: top-left smallfont HUD is drawn after doc2p on the planar bitmap.
	clr	panelcnt
	rts
...
hite	dc	240	;v190fz: fullscreen default uses former 16-line panel area
minx	dc	-160	;v44: fullscreen default
midx	;
maxx	dc	160	;v44: fullscreen default
miny	dc	-120	;v190fz: fullscreen default
midy	;
maxy	dc	120	;v190fz: fullscreen default
wdiv32	dc	0
wrem32	dc	0
...
.lf	bsr	loadfiles
	...
	jsr	g2loadsmallfontfallback	;v190fz: optional smallfont.bin HUD source
	;
...
g1progfiles	dc.l	gloomcfg
	...
	even
 	; v190fz: Classic Gloom has no gun overlay; do not require/load misc/gun.bin here.
	dc.l	titlemed+1
	dc.b	'sfxs/med1',0
	even
	dc.l	loadingmed+1
