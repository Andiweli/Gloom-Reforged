...
.rx_ok
	move	d0,g2_shadow_rx
	; v174: apply the projected ob_y compensation to pickups too.  a4 starts
	; at the bottom of the drawn/bobbing sprite column, so without this the
	; floor oval only lines up when the upgrade is near the ground.  With the
	; compensation it stays on the floor for the whole bobbing cycle.
	clr	g2_shadow_yoff
	move	ob_y(a2),d0
	neg	d0
	ext.l	d0
