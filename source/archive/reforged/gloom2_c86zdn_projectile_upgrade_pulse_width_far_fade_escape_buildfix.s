...
.wp_pulse
	...
	; c86zdm: weapon-upgrade pulse now grows with distance correctly:
	; far = no pulse, mid = small pulse, near = up to 2x.
	; The draw path also paints side pixels, so the pulse is visible in width.
	move	ob_y(a2),d1
	cmp	#-32,d1
	blt	.pick_pulse_done
	cmp	#-20,d1
	...
	move	d6,d0
	add	d6,d0		;level 3: 2.0x radius when very near
...
.ryloop
		...
		bge	.ryskip
		move.b	d4,(a0)
		; c86zdm: weapon-upgrade pulse also expands horizontally.
		; The sprite-column loop cannot naturally draw outside the owner
		; bitmap width, so paint a few side pixels during pulse frames.
		tst	g2_reflect_pickup
		beq	.ryskip
		move	g2_reflect_pulse,d0
		beq	.ryskip
