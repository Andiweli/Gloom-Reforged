...
; -----------------------------------------------------------------------------
; v137 enemy blob shadows + floor-anchored multi-colour reflections.
; Blob shadows keep the confirmed hard-edged foot shadow path.  Reflections are
; palette/dither based: dark centre, lighter outside, and pickup rings try to
; stay on the floor while pulsing with the pickup bob phase.
; -----------------------------------------------------------------------------
g2_setup_enemy_blob_column
	...
	beq	.setup_reflection
	; v139: stationary powerups/tokens use rts-style logic, not weaponlogic.
	; Detect them by hit routine so the floor reflection is drawn for map upgrades too.
	bsr	g2_is_reflect_pickup
	tst	d0
...
; v139: true for stationary map pickups/powerups that do not run weaponlogic.
; a2=object, returns d0=1 if it should receive a floor reflection.
g2_is_reflect_pickup
...
g2_prepare_reflection_column
	...
	; v136: keep reflections in the same near range as blob shadows.  Do not
	; suppress very near side shots anymore; the later HUD/gun clamp protects
	; the weapon area while still allowing side reflections near the muzzle.
	cmp	#3<<grdshft,d2
	bgt	.no_reflect
	move	d7,g2_shadow_curx
	move	d7,d0
	...
	add	d1,d0
	move	d0,g2_shadow_cx
	move.l	ob_logic(a2),d5
	clr	g2_reflect_pickup
	move	d3,d0
	cmp.l	#weaponlogic,d5
	beq	.pickup_size
	move	d0,-(a7)
	bsr	g2_is_reflect_pickup
	tst	d0
...
.pickup_size
	...
	lsr	#2,d0	;pickup ring: a little wider
	cmp	#5,d0
	bge	.up_min_ok
	moveq	#5,d0
.up_min_ok
	...
	bra	.rx_ok
.bullet_size
	; v138: projectile reflection diameter ~= 2/3 of projectile width,
	; so radius ~= visible width / 3.  Keep upgrade pulse/ring sizing unchanged.
	ext.l	d0
	divu	#3,d0
	cmp	#2,d0
	bge	.bul_min_ok
...
.bul_min_ok
	...
	moveq	#10,d0
.rx_ok
	; v137: pickups pulse on the floor instead of following the bobbing
	; sprite.  Use the current vertical bob value as a size hint: lower sprite
	; (near the floor) = slightly larger ring, high sprite = smaller ring.
...
.pick_low	lsr	#5,d1
	addq	#1,d1
.pick_pulse_ok	cmp	#3,d1
	ble	.pick_pulse_cap
	moveq	#3,d1
.pick_pulse_cap	add	d1,d0
.store_rx	move	d0,g2_shadow_rx
	; v137: reflection belongs on the floor, not glued to the bottom of
	; the floating sprite.  Projectiles keep the previous approximate height
	; compensation.  Weapon/upgrade pickups use the opposite sign and a wider
	; clamp so their floor ring stays put while the sprite bobs.
	clr	g2_shadow_yoff
	tst	g2_reflect_pickup
	bne	.pickup_ycalc
	move	ob_y(a2),d0
	...
	moveq	#30,d0
	bra	.ymax_ok
.pickup_ycalc	move	ob_y(a2),d0
	; v139: cancel the sprite bob in the opposite direction so the ring stays
	; near floor level while only its radius pulses with the bob phase.
	neg	d0
	ext.l	d0
	asl.l	#8,d0
	divs	d2,d0
...
g2_reflection_colour
	; return centre palette index in d0 and store lighter edge index in
	; g2_reflect_edge_col.  This fakes a multi-colour/alpha reflection in
	; the 8-bit chunky palette: dark centre, brighter outside + dithering.
	movem.l	d1-d3/a0,-(a7)
	moveq	#0,d2
	...
	; v139: token/powerup pickups do not have bullet shapes.  Choose a stable
	; colour family from their pickup routine so stationary upgrades can glow too.
	tst	g2_reflect_pickup
	beq	.got_weapon
	moveq	#0,d2
	move.l	ob_hit(a2),d1
...
.got_weapon
	...
	tst.l	a0
	beq	.store_edge
	; v139: bright centre from projectile/upgrade core, darker edge outward.
...
; weapon 1..5 / projectile 1..5: yellow, green, red, cyan, magenta.
; v139: bright centre from sprite core, darker colour toward soft edge.
g2_reflect_dark_rgb
	dc	$630,$060,$600,$066,$606,$630,$060,$600
g2_reflect_rgb
	dc	$ff0,$0f0,$f00,$0ff,$f0f,$ff0,$0f0,$f00

g2_draw_enemy_blob_column
	; called from drawobjnorm after the current sprite column was drawn.
...
.rabs_ok	move	g2_shadow_rx(pc),d2
	cmp	d2,d1
	bgt	.rts
	; v139: softer, more oval reflection.  Width is still the projectile/
	; pickup footprint, but vertical thickness now falls off in more bands.
	; Centre uses bright core colour; edges use darker colour with sparse writes
	; to approximate about 80% transparency in the 8-bit palette renderer.
	clr	g2_reflect_softedge
	moveq	#3,d5	;outer edge vertical offset
	moveq	#0,d6	;1 row
	move	d2,d3
	mulu	#7,d3
	lsr	#3,d3	;7/8 radius: very soft thin edge
	cmp	d3,d1
	bgt	.rsoft_outer
	moveq	#2,d5
	moveq	#1,d6	;2 rows
	move	d2,d3
	mulu	#5,d3
	lsr	#3,d3	;5/8 radius: dark mid edge
	cmp	d3,d1
	bgt	.rsoft_mid
	moveq	#1,d5
	moveq	#2,d6	;3 rows
	move	d2,d3
	mulu	#3,d3
	lsr	#3,d3	;3/8 radius: bright inner oval
	cmp	d3,d1
	bgt	.rband_ok
	moveq	#0,d5
	moveq	#3,d6	;4-row bright centre for a rounder ellipse
	bra	.rband_ok
.rsoft_outer
	move	#2,g2_reflect_softedge	;outer edge: very sparse dark dither
	bra	.rband_ok
.rsoft_mid
	move	#1,g2_reflect_softedge	;mid edge: sparse dark dither
.rband_ok
	move.l	a4,a0
	move.l	chunkymod(pc),d7
	sub.l	d7,a0
...
.rdraw	move	g2_shadow_col(pc),d4
	; v139 multi-colour pseudo-alpha: bright centre, darker edge.  Real
	; 80% alpha is unavailable here, so most floor pixels are left untouched.
	...
	cmp	#2,d0
	beq	.outer_dither
	and	#3,d3	;mid edge: about 25% coloured samples
	bne	.rts
	bra	.ryloop
.outer_dither
	and	#7,d3	;outer edge: about 12.5% coloured samples
	bne	.rts
	bra	.ryloop
.centre_dither
	move	g2_shadow_curx(pc),d3
	add	d6,d3
	and	#3,d3	;bright centre: about 25% coloured samples
	bne	.rts
.ryloop	move.b	d4,(a0)
	adda.l	d7,a0
	dbf	d6,.ryloop
...
g2_reflect_edge_col	dc	0	;v137/v139 outer reflection colour
g2_reflect_pickup	dc	0	;v139 reflection source is pickup/powerup, not projectile
...
.encode	lea	joyx(pc),a0
	...
	rts
.normal_menu_mask	and	#$1c,d0	;v115 keep title menu behaviour from v107
	rts
...
.noescsel	btst	#0,d0
	...
	rts
	;
.left	tst	game_menu_active
	beq	selmenu
	move	curropt(pc),d0
	or	#$0100,d0
	rts
.right	tst	game_menu_active
	beq	selmenu
	move	curropt(pc),d0
	or	#$0200,d0
	rts
