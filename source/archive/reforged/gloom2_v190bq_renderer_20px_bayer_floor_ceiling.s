...
.vloop	;
	...
	move	0(a5,d6*2),d5
	; v190bq: floor/ceiling participate in the ordered dither too.
	; Keep it conservative: only mid/far shades get a sparse next-darker
	; Bayer mix, so floors/ceilings remain crisp but less banded.
	clr.b	g2_flat_thr
	move.l	palette(pc),a5
	move.l	0(a5,d5*4),a6
	cmp	#8,d5
	blo.s	.g2flat_no_dither
	cmp	#14,d5
	bhs.s	.g2flat_no_dither
	move	d5,d2
	addq	#1,d2
	cmp	#15,d2
	bls.s	.g2flat_alt_ok
...
.hloop	;get next column
	...
	addx	d2,d1
	tst.b	g2_flat_thr
	beq.s	.g2flat_basepix
	move.l	a2,d2
	eor	d5,d2
	and	#15,d2
	move.b	g2_bayer4(pc,d2.w),d2
	cmp.b	g2_flat_thr(pc),d2
	bge.s	.g2flat_basepix
	move.b	0(a6,d3),0(a2,a4.l)
	bra.s	.g2flat_pixdone
.g2flat_basepix
...
.hiteok	;
	...
	move.l	0(a2,d5*4),a4
	moveq	#0,d6
	move.b	g2_cur_thr(pc),d6
	beq.s	.g2nodither_setup
	moveq	#0,d5
	move.b	g2_cur_altpal(pc),d5
	cmp	#255,d5
	beq.s	.g2nodither_setup
	move.l	0(a2,d5*4),a5
	sub	d3,d2
...
.g2dither_loop
	move.b	0(a3,d2),d5
	; v190bq: real 4x4 Bayer/ordered lookup.  Threshold is now
	; 0..15 instead of four coarse buckets, so the 20px transition
	; ramps more gradually and looks less blocky.
	move	d2,d6
	and	#3,d6
	lsl	#2,d6
	add.b	g2_cur_xmod(pc),d6
	move.b	g2_bayer4(pc,d6.w),d6
	cmp.b	g2_cur_thr(pc),d6
	bge.s	.g2basepix
.g2altpix
	move.b	0(a5,d5),(a1)
	bra.s	.g2pixdone
...
; v190bo: Build a 20px ordered-dither transition map for wall shade
; boundaries.  This does NOT replace the stable v190bi fog; it only tells
; renderwalls where to mix the neighbouring palette in a broad transition.
g2build_wallblend
	movem.l	d0-d7/a0-a6,-(a7)
	lea	g2_wallblend_alt(pc),a1
	lea	g2_wallblend_thr(pc),a2
	move	width(pc),d7
	ble.w	.g2bwb_done
	subq	#1,d7
.g2bwb_init
	...
	blt.w	.g2bwb_done
	move.l	vertdraws(pc),a0
	moveq	#0,d6	; boundary x, between d6 and d6+1
.g2bwb_boundary
	tst.l	vd_data(a0)
	beq.s	.g2bwb_next
	tst.l	vd_size+vd_data(a0)
	beq.s	.g2bwb_next
	move	vd_pal(a0),d0
	move	vd_size+vd_pal(a0),d1
	move	d1,d2
	sub	d0,d2
	cmp	#1,d2
	beq.s	.g2bwb_blend
...
.g2bwb_blend
	move	d6,d3
	bsr	g2blend_boundary20
	movem.l	d0-d7/a0-a3,-(a7)
	lea	g2_wallblend_alt(pc),a0
	lea	g2_wallblend_thr(pc),a1
	lea	g2_transition_thr20(pc),a2
	move	d3,d4
	sub	#19,d4
	bpl.s	.g2bb_lstart_ok
	moveq	#0,d4
.g2bb_lstart_ok
	move	d4,d5
.g2bb_left_loop
	cmp	d3,d5
	bgt.s	.g2bb_right_setup
	move	d3,d2
	sub	d5,d2	; distance from boundary, 0..19
	moveq	#0,d6
	move.b	0(a2,d2.w),d6
	beq.s	.g2bb_left_next
	move.b	d1,0(a0,d5.w)
	move.b	d6,0(a1,d5.w)
.g2bb_left_next
	addq	#1,d5
	bra.s	.g2bb_left_loop
.g2bb_right_setup
	move	d3,d4
	addq	#1,d4
	move	width(pc),d7
...
.g2bb_right_loop
	...
	bgt.s	.g2bb_done
	move	d5,d2
	sub	d4,d2	; distance from boundary, 0..19
	cmp	#19,d2
	bgt.s	.g2bb_done
	moveq	#0,d6
	move.b	0(a2,d2.w),d6
	beq.s	.g2bb_right_next
	move.b	d0,0(a0,d5.w)
	move.b	d6,0(a1,d5.w)
.g2bb_right_next
	addq	#1,d5
	bra.s	.g2bb_right_loop
.g2bb_done
	movem.l	(a7)+,d0-d7/a0-a3
	rts

; 20px ramp: near boundary almost fully neighbour, fading to sparse mix.
g2_transition_thr20
	dc.b	15,14,13,13,12,11,10,10,9,8
	dc.b	7,6,6,5,4,3,2,2,1,0

; 4x4 Bayer matrix values 0..15, row-major.  Used for both walls and flats.
g2_bayer4
	dc.b	0,8,2,10
	dc.b	12,4,14,6
	dc.b	3,11,1,9
...
g2_wallblend_alt	ds.b	320
g2_wallblend_thr	ds.b	320
g2_cur_altpal	dc.b	0
g2_cur_thr	dc.b	0
g2_cur_xmod	dc.b	0
	even
g2_render_x	dc	0
...
renderwalls	;
	bsr	g2build_wallblend
	move.l	chunkymod(pc),d0
	move.l	vertdraws(pc),a0
	move.l	palette(pc),a2
	lea	palettes,a2
	lea	coloffs(pc),a6
	lea	g2_wallblend_alt(pc),a5
	lea	g2_wallblend_thr(pc),a3
	clr	g2_render_x
	;
	move	width(pc),d7
	subq	#1,d7
...
.loop	move.l	(a6)+,a1
	add.l	chunky(pc),a1
	move.b	(a5)+,d6
	move.b	d6,g2_cur_altpal
	move.b	(a3)+,d6
	move.b	d6,g2_cur_thr
	move	g2_render_x(pc),d6
	and	#3,d6
	move.b	d6,g2_cur_xmod
	move.l	a3,-(a7)
	move.l	a5,-(a7)
	;
	drawsolidstrip
