...
flatyadd	dc	0
flatyadd2	dc	0
render_lod_x	dc	0	;v92: current wall x phase for LOD/depth crossfade
render_lod_thr	dc	0	;v92: smoothstep threshold 0..15 toward next darker shade

; v92: 16-entry smoothstep-ish threshold table.  This makes the dithered
; transition start/end gently instead of linearly flipping whole columns.
smoothstep16	dc.b	0,0,1,2,3,4,6,7,8,10,11,12,13,14,15,15
even
...
.hiteok	;
	...
	move.l	0(a2,d5*4),a4	;base shade palette
	moveq	#0,d6
	move.l	d6,a5		;optional next darker palette, 68000-safe zeroing
	move	render_lod_thr(pc),d6
	beq.s	.nolodpal
	cmp	#15,d5
	bge.s	.nolodpal
	move.l	4(a2,d5*4),a5	;next darker shade palette
.nolodpal	sub	d3,d2
	add.l	d3,d2
	;
.vertloop	move.b	0(a3,d2),d5
	...
	tst.l	a5
	beq.s	.baseplot
	move	render_lod_x(pc),d6
	add	d4,d6		;mix column phase with pixel row to avoid column ridges
	and	#15,d6
	cmp	render_lod_thr(pc),d6
	bge.s	.baseplot
	move.b	0(a5,d5),(a1)
	bra.s	.plotted
...
renderwalls	;
	...
	move	width(pc),d7
	subq	#1,d7
	moveq	#0,d6		;v92 x phase for ordered LOD/depth crossfade
	;
.loop	move.l	(a6)+,a1
	add.l	chunky(pc),a1
	;
	move	d6,render_lod_x
	move	vd_z(a0),d1
	and	#127,d1		;fraction inside the current original 128-unit depth band
	lsr	#3,d1		;0..15
	lea	smoothstep16(pc),a5
	moveq	#0,d2
	move.b	0(a5,d1.w),d2
	move	d2,render_lod_thr
	drawsolidstrip
	;
	lea	vd_size(a0),a0
	move	render_lod_x(pc),d6	;drawsolidstrip uses d6 internally, restore x phase
	addq	#1,d6
	and	#15,d6
	dbf	d7,.loop
	;
