...
; -----------------------------------------------------------------------------
; v137/v174 enemy blob shadows + floor-anchored multi-colour reflections.
...
; Projectiles use a wide oval, and pickup rings are locked to the floor plane.
; -----------------------------------------------------------------------------
...
.wp_min_ok
	...
	moveq	#18,d0
.wp_pulse
	; v174: weapon pickups always keep a floor reflection.  When the bobbing
	; object reaches the low/touch part of the cycle, enlarge the footprint
	; visibly instead of merely showing it for a short moment.
	move	ob_y(a2),d1
	bpl.s	.wp_y_abs
	neg	d1
.wp_y_abs	cmp	#8,d1
...
.up_min_ok
	...
	moveq	#18,d0
.pick_pulse
	; v174: token/powerup reflections are always visible at their base size.
	; The low/touch part of the bobbing motion only increases the oval.
	move	ob_y(a2),d1
	bpl.s	.pick_y_abs
	neg	d1
.pick_y_abs	cmp	#8,d1
...
.rx_ok
	move	d0,g2_shadow_rx
	; v175: keep pickup/upgrade reflections fixed on the floor plane.
	; a4 follows the bobbing sprite bottom.  For stationary pickups, add the
	; projected ob_y offset back in the opposite direction so the floor oval
	; remains on the same screen row while the object moves up/down.  Bullets
	; keep the old direct underside anchor.
	clr	g2_shadow_yoff
