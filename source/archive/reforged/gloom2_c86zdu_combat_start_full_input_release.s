...
.play	add	#49,d0
	...
	move	d0,p2lives
	;
	; c86zdu: combat keeps its selected lives/collision, but mirrors
	; the persistent player-state reset from the normal TWO PLAYER GAME path.
	move	#25,p1health
	move	#25,p2health
	move	#0,p1weapon
	move	#0,p2weapon
...
; c86zdu: TWO PLAYER COMBAT must not inherit any held menu/game input
; from a previous game or from the combat selection menus.  This is not an
; ESC block: it only waits for a fully neutral input state before the first
; gameplay frame, then ESC works normally again.
g2combat_wait_all_controls_released	movem.l	d0-d7/a0-a6,-(a7)
g2zdu_wait_release	bsr	vwait
	clr	escape
	lea	joyx0(pc),a0
	moveq	#11,d0
g2zdu_clear_controls	clr.l	(a0)+
	dbf	d0,g2zdu_clear_controls
	jsr	readjoys
	tst	escape
	bne.s	g2zdu_wait_release
	lea	joyx0(pc),a0
	moveq	#11,d0
g2zdu_scan_controls	tst.l	(a0)+
	bne.s	g2zdu_wait_release
	dbf	d0,g2zdu_scan_controls
	qkey	$45
	bne.s	g2zdu_wait_release
	qkey	$44
	bne.s	g2zdu_wait_release
...
g2zdu_no_p1_lastbut
	...
	move.l	d0,a5
	clr	ob_lastbut(a5)
g2zdu_done	movem.l	(a7)+,d0-d7/a0-a6
	rts
...
.nolmed	;
	...
	clr	escape
	clr	frame
	cmp	#2,gametype	; c86zdu: only combat needs the pre-start neutral-input gate
	bne.s	g2zdu_skip_combat_release
	jsr	g2combat_wait_all_controls_released
g2zdu_skip_combat_release
	;
	bsr	syncup
	;
	clr	framecnt
