...
playsfx	;sfx file in a0, vol in d0, priority in d1
	;
	tst.l	a0	;v190cj: missing cross-game SFX is silent, not a Guru
	beq.s	.rts
...
drawchunky	;draw a chunky shape on a 320 wide chunkymap
	...
	;
	move.l	panel(pc),a0
	beq.s	g2dc_no_panel	; v190db: classic Gloom has no smallfont2 status panel
...
g2draw_statusbar_base
	movem.l	d0-d2/a0,-(a7)
	move.l	panel(pc),a0
	beq.s	.g2dsb_done	; v190da: no Gloom2 panel in classic Gloom data
	moveq	#47,d2
...
g2draw_statusbar_clear
	movem.l	d0-d2/a0,-(a7)
	move.l	panel(pc),a0
	beq.s	.g2dsc_done	; v190da: no Gloom2 panel in classic Gloom data
	moveq	#46,d2
...
g2drawgun
	...
	cmp	#1,g2_game_profile
	beq.w	.g2dg_done	; v190da: original Gloom has no first-person gun graphic
	move.l	gunpic(pc),d0
	beq.w	.g2dg_done
...
showstats	;
	;a5=player
	tst.l	panel
	beq	.rts		; v190da: classic Gloom has no smallfont2 status panel
...
g2draw_statusbar_scroll
	...
	tst.l	panel
	beq.w	.g2sms_done	; v190da: classic Gloom skips panel-font scroller
...
.g2bs_not_blackout
	...
	bclr	#7,ob_update(a5)
	cmp	#1,g2_game_profile	; v190de: original Gloom has top-left HUD, no bottom panel
	beq.s	.g2v190de_g1_blitscene
	...
	move	#2,panelcnt
	rts
.g2v190de_g1_blitscene
	jsr	g2v190de_g1_draw_classic_hud
	clr	panelcnt
	rts
...
.g2amo_do
	...
	bra.s	.g2amo_done
.g2amo_g1_classic
	; v190da: classic Gloom has no gun sprite; keep the shot visually lower
	; and do not apply the modern weapon-muzzle X/Z origin shift.
	add	#32,ob_y(a0)
.g2amo_done
	movem.l	(a7)+,d0-d2/a1
	rts
...
.lf	bsr	loadfiles
	cmp	#1,g2_game_profile
	beq.s	.g2v190da_no_gun_fallback
	jsr	g2loadgunfallback	;v61: optional misc/stuf gun.bin fallback
	bra.s	.g2v190de_after_g1_fontpal_boot
.g2v190da_no_gun_fallback
	jsr	g2v190de_g1_initbmappal	; v190de: same early smallfont palette poke as gloom.s
.g2v190de_after_g1_fontpal_boot
	;
...
.g2v190cj_no_panel_remap
	cmp	#1,g2_game_profile
	beq.s	g2v61_no_gun_remap	; v190da: no gun remap for classic Gloom
...
g2v142_draw_gloombrush_safe
	...
	; v190da: Zombie Massacre g3-dc keeps the menu-only timing, but sits
	; 1px lower than v190cx/v190cz.
	move	d2,-(a7)
	move	#72,d0
...
.g2v142_drawone
	movem.l	d0/a0-a1,-(a7)
	move	#168,d0
	cmp	#2,g2_game_profile	; v190da: Zombie Massacre brush only 1px above base
...
.about	;about text...
	...
	tst	g2_game_profile	; v190cp: compat ABOUT fully redraws clean title, no leftover gloombrush
...
.noch	bsr	g2v147_finitmenu_soft
	move	(a7)+,numopts
	cmp	#2,g2_game_profile
	beq.s	.g2v190da_about_return_zm
	jsr	g2v145_show_clean_title
	bsr	dispon
	bra	.redrawmenu
.g2v190da_about_return_zm
	bsr	dispoff		; precompose title+g3-dc before showing the menu again
	jsr	g2v145_show_clean_title
	jsr	g2v142_draw_gloombrush_safe
	bsr	dispon
	bra	.redrawmenu
...
initmenu2	;
	...
	bsr	swapshow
	cmp	#1,g2_game_profile	; v190de: original Gloom uses the old smallfont palette like gloom.s
	bne.s	.g2v190de_normal_initfontpal
	jsr	g2v190de_g1_initbmappal
	bra.s	.g2v190de_fontpal_done
.g2v190de_normal_initfontpal
	bsr	initfontpal
.g2v190de_fontpal_done
	;
...
calcpalettes	;generate 16 versions of palette
	...
	cmp	#1,g2_game_profile	; v190de: original Gloom needs real distance shade tables
	bne.s	.g2v190de_not_g1_palette
	move.l	planar_remap(pc),a1	;synthetic G1 remap/table must exist after map load
	tst.l	a1
	beq.s	.g2v190cj_no_remap
	jsr	g2v190de_g1_calcpalettes_nearest
	rts
.g2v190de_not_g1_palette
...
g1progfiles	dc.l	gloomcfg
	...
	even
	; v190da: original Gloom does not use a first-person gun graphic.
	; Do not load misc/gun.bin here; the profile also skips the fallback loader.
	dc.l	titlemed+1
	dc.b	'sfxs/med1',0
	even
	dc.l	loadingmed+1
...
g2setup_g1_profile
	...
	move	#240,hite		; v190de: original Gloom has no bottom statusbar, render full height
	clr.l	gunpic		; v190da: force classic no-gun state
	clr.l	panel		; v190da: no Gloom2 smallfont2 status panel in classic data
...
.g2v190cv_clearplanes
	...
	; v190de: after poking the original smallfont palette, use the same
	; colour plane convention as gloom.s chatprintout colour 1 (bp1).
	move.l	a1,a2
	add.l	d4,a2
	or.b	d0,(a2)
.g2v190cv_nextline
...
.g2v190cx_clear
	...
	tst.l	a0
	beq	.g2v190cx_restore_done
	tst.l	a1
	beq	.g2v190cx_restore_done
	move.l	a0,d4
	cmp.l	a1,d4
	bcc	.g2v190cx_restore_done
...
.g2v190cx_loop
	...
	move	(a0)+,d0
	and	#$0fff,d0
	tst	d3		; v190de: source slot 0 is a clear/unused slot in chunky, keep it black
	bne.s	.g2v190de_g1_not_slot0
	moveq	#0,d0
.g2v190de_g1_not_slot0
...
; -----------------------------------------------------------------------------
; v190de: Original Gloom dedicated classic renderer support
; -----------------------------------------------------------------------------
; This block is intentionally separate from the Gloom2/Gloom3/ZM paths.  It
; uses the old Gloom smallfont palette for menus and builds real distance-shade
; tables for the original Gloom data, which has no misc/palette_8/remap_8.

; Poke the original smallfont palette like gloom.s:initbmappal.  For AGA/OS-AGA
; the old 12-bit colours are expanded to the high/low-word format expected by
; the existing AGA palette poker.
g2v190de_g1_initbmappal
	movem.l	d0-d1/a0-a1,-(a7)
	move.l	smallfont_,a1
	tst.l	a1
	beq	.done
	add.l	(a1),a1
	addq	#2,a1
	tst	aga
	beq	.ecs_or_ocs
	lea	g2v190de_g1_fontpal_aga,a0
	moveq	#22,d1		; 23 colours
.aga_loop
	move	(a1)+,(a0)+
	clr	(a0)+
	dbf	d1,.aga_loop
	lea	g2v190de_g1_fontpal_aga,a1
	moveq	#23,d0
	jsr	pokepal2
	bra	.done
.ecs_or_ocs
...
even
g2v190de_g1_fontpal_aga	ds.w	46

; Build chunky shade tables for original Gloom.  The classic renderer could
; append darkened RGB palettes directly; this chunky renderer instead needs
; 16 byte lookup tables.  For each darkened RGB we choose the nearest live
; original map colour, giving back the missing distance fog/gradation.
g2v190de_g1_calcpalettes_nearest
	movem.l	d0-d7/a0-a6,-(a7)
	lea	paladjust,a5
	lea	palettes,a2
	moveq	#0,d7		; shade 0..15
.shade_loop
	move.l	(a2)+,a3		; destination byte table
	move.l	map_rgbs,a0
	move.l	map_rgbsat,a4
	moveq	#0,d6		; source palette index
.colour_loop
	cmp	#256,d6
	bcc	.tail_done
	cmp.l	a4,a0
	...
	tst	d6
	bne	.not_slot0
	moveq	#0,d0		; clear/background slot must stay black
.not_slot0
	tst	d7
	beq	.direct_index
	move	d0,d1
...
.b_ok
	...
	or	d3,d1
	move	d1,d0
	jsr	g2v190de_g1_find_nearest_rgb
	bra	.store
.direct_index
	move	d6,d0
.store
...
; d0.w = RGB12 target, returns nearest palette index in d0.w.  Slot 0 is
; reserved for clear/black, so normal searches start at slot 1.
g2v190de_g1_find_nearest_rgb
	movem.l	d1-d7/a0-a2,-(a7)
	and	#$0fff,d0
	tst	d0
	beq	.black
	move	d0,d4		; target RGB12
	move.l	map_rgbs,a0
	addq.l	#2,a0		; skip slot 0 (-1/clear in original Gloom data)
	move.l	map_rgbsat,a1
	moveq	#1,d7		; candidate index
	moveq	#1,d5		; best index
	move	#$7fff,d6	; best distance
.loop
	cmp	#256,d7
	bcc	.done
	cmp.l	a1,a0
...
; Convert an RGB12 HUD colour to an active chunky screen index using the same
; nearest-palette match as the G1 fog tables.
g2v190de_g1_rgb_to_chunky
	jsr	g2v190de_g1_find_nearest_rgb
	lea	paladjust,a0
	and	#$00ff,d0
	moveq	#0,d1
...
; Original Gloom HUD: HEALTH and WEAPON live at the top-left, not in a bottom
; Gloom2 status panel.  Positions mirror gloom.s:initstats/showstats.
g2v190de_g1_draw_classic_hud
	movem.l	d0-d7/a0-a5,-(a7)
	move.l	player_,a5
	tst.l	a5
	beq	.done
	move	#$04f,d0
	jsr	g2v190de_g1_rgb_to_chunky
	move	d0,g2v190de_g1_hud_col_text
	move	#$0f0,d0
	jsr	g2v190de_g1_rgb_to_chunky
	move	d0,g2v190de_g1_hud_col_green
	move	#$ff0,d0
	jsr	g2v190de_g1_rgb_to_chunky
	move	d0,g2v190de_g1_hud_col_yellow
	move	#$f00,d0
	jsr	g2v190de_g1_rgb_to_chunky
	move	d0,g2v190de_g1_hud_col_red
	move	#$0cf,d0
	jsr	g2v190de_g1_rgb_to_chunky
	move	d0,g2v190de_g1_hud_col_blue
	lea	g2v190de_g1_hud_health(pc),a4
	moveq	#2,d0
	moveq	#2,d1
	move	g2v190de_g1_hud_col_text(pc),d2
	bsr	g2v190de_g1_draw_hud_text
	lea	g2v190de_g1_hud_weapon(pc),a4
	moveq	#2,d0
	moveq	#12,d1
	move	g2v190de_g1_hud_col_text(pc),d2
	bsr	g2v190de_g1_draw_hud_text
	; health bar: classic 25 two-pixel cells starting at x=44,y=2
	move	ob_hitpoints(a5),d7
	ble	.no_health
	cmp	#25,d7
	bls	.hp_ok
...
.hp_loop
	...
	moveq	#2,d2
	moveq	#7,d3
	move	g2v190de_g1_hud_col_red(pc),d4
	cmp	#10,d6
	blt	.hp_col_ok
	move	g2v190de_g1_hud_col_yellow(pc),d4
	cmp	#18,d6
	blt	.hp_col_ok
	move	g2v190de_g1_hud_col_green(pc),d4
.hp_col_ok
	bsr	g2v190de_g1_fill_rect
	addq	#1,d6
	cmp	d7,d6
	blt	.hp_loop
.no_health
	; weapon bar: five classic 10px blocks starting at x=44,y=12
	move	ob_weapon(a5),d7
	addq	#1,d7
	cmp	#5,d7
	bls	.wp_ok
...
.wp_loop
	...
	moveq	#8,d2
	moveq	#7,d3
	move	g2v190de_g1_hud_col_blue(pc),d4
	bsr	g2v190de_g1_fill_rect
	addq	#1,d6
	cmp	d7,d6
	blt	.wp_loop
...
g2v190de_g1_draw_hud_text	; a4=0-term text, d0=x, d1=y, d2=colour
	movem.l	d0-d7/a0-a4,-(a7)
	move	d0,d6
	move	d1,d7
	move	d2,d5
.loop
	...
	bhi	.space
	move	d0,d2
	bsr	g2v190de_g1_draw_hud_glyph
.space
	addq	#6,d6
	bra	.loop
.done
...
g2v190de_g1_draw_hud_glyph	; d2=glyph, d6=x, d7=y, d5=colour
...
g2v190de_g1_fill_rect	; d0=x,d1=y,d2=w,d3=h,d4=colour
	movem.l	d0-d7/a0-a1,-(a7)
	move.l	chunky,a0
	mulu	#320,d1
	add.l	d1,a0
...
g2v190de_g1_hud_health	dc.b	'HEALTH',0
g2v190de_g1_hud_weapon	dc.b	'WEAPON',0
	even
g2v190de_g1_hud_col_text	dc	0
g2v190de_g1_hud_col_green	dc	0
g2v190de_g1_hud_col_yellow	dc	0
g2v190de_g1_hud_col_red	dc	0
g2v190de_g1_hud_col_blue	dc	0
	even
