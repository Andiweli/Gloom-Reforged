...
g2v190cx_build_g1_tables
	...
	bne.s	.g2v190cx_done
	; c86zdx: rebuild the synthetic Gloom1 palette/remap tables for every
	; level/combat map.  The old test skipped the rebuild as soon as
	; planar_remap was non-zero, but after the first Classic Gloom level that
	; pointer is our own g2v190cx_g1_remap table.  Reusing that stale table on
	; the next combat/level can leave the renderer using the previous map's
	; palette mapping.  Only skip if planar_remap is a real external/embedded
	; remap table, not our synthetic one.
	movem.l	d0-d1/a0,-(a7)
	move.l	planar_remap,d0
	beq.s	.g2v190cx_restore_and_build
	lea	g2v190cx_g1_remap(pc),a0
	move.l	a0,d1
	cmp.l	d1,d0
.g2v190cx_restore_and_build
	movem.l	(a7)+,d0-d1/a0
	bne.s	.g2v190cx_done
	movem.l	d0-d7/a0-a4,-(a7)
