...
g2v190i_levelselect_loadscripts
	...
	clr	g2v190f_level_index
	bsr	permit
	; v190gt: START LEVEL uses one shared menu, with decrunched profile-specific script source:
...
.g2v190gs_try_misc_script
	...
	; v190gt: use the normal loadfile path so compressed CrM2 scripts
	; (especially Classic Gloom misc/script) are decrunched before parsing.
	; The previous bounded raw Read() parser only saw the CrM2 header and
	; therefore left Classic Gloom stuck at the fallback MAP1.1 entry.
	movem.l	d0-d1/a0-a1,-(a7)
