...
solidstrip	set	0
drawstrip2	; v190c chunky-safe transparent wall strip overlay, map1_5 texcoord guard
	; a1=top of destination column, a2=palettes base, a4=strip/vd data
	; This is intentionally still the v190 behaviour: non-zero texels are drawn
	; through the current wall palette, zero texels leave the already-rendered
	; scene/floor behind untouched.  Only the texture row lookup is guarded to
	; stay inside the 64-pixel wall column, because map1_5 can drive transparent
	; strips close enough that the old indexed read can leave the column.
	movem.l	d0-d7/a0-a5,-(a7)
...
.loop
	move	d0,d5
	and	#63,d5		; v190c: transparent wall textures are 64px high
	move.b	0(a0,d5.w),d5
	beq.s	.skip
	move.b	0(a5,d5.w),(a1)
.skip	addx.l	d3,d0
