...
drawstrip2	; v191 chunky-safe transparent wall strip overlay with colour tint
	...
	; texels keep the scene visible, but now receive a forced green 50% checker
	; tint for glass doors.
	movem.l	d0-d7/a0-a5,-(a7)
	move.l	vd_data(a4),d0
	beq	.rts
	move.l	d0,a0		; source texture column
	; v192: force green glass tint for transparent texels.  The legacy
	; strip colour selector is not reliable for these doors in chunky mode,
	; so do not depend on -1(a0) here.
...
.glass	tst	d6
	bmi.s	.next
	btst	#0,d4		; 50% green glass checker, background stays visible
	bne.s	.next
	move.b	d6,(a1)
.next	addx.l	d3,d0
	add.l	d7,a1
