...
renderwalls	;
	...
	move	width(pc),d7
	subq	#1,d7
	moveq	#0,d6		;v88 column phase 0..7 for dithered depth crossfade
	;
.loop	move.l	(a6)+,a1
	add.l	chunky(pc),a1
	;
...
initdarktable	;
	...
	;
.loop	move	#maxz-1,d3
	sub	d2,d3		;v86: actual distance 0..maxz-1 for this table entry
	lsr	#6,d3		;v88: spread the 32 shade levels across the full visible depth
				;range again so near/mid distance stays readable and far distance
				;still falls off.  The softer transitions now come from column
				;dithered crossfade instead of crushing depth too early.
	cmp	#darkmax,d3
	bls.s	.ok
	move	#darkmax,d3
.ok	move	d3,(a1)+
