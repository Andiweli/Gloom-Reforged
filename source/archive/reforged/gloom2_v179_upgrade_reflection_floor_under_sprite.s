...
.wp_min_ok
	...
	moveq	#18,d0
.wp_pulse
	; v176: weapon pickups always keep a floor reflection.  The bobbing
	; object itself may move up/down, but only the down-moving half of the
	; cycle makes the floor oval swell.
	move	ob_movspeed(a2),d1
	and	#127,d1
	cmp	#64,d1
	bcs.s	.pick_pulse_done
	move	d0,d1
	lsr	#1,d1
	add	d1,d0
	cmp	#24,d0
...
.rx_ok
	move	d0,g2_shadow_rx
	; v179: pickups/upgrades use the same projected floor compensation as
	; the stable v174 path again.  Their reflection must stay under the
	; bouncing pickup on the floor texture, not inside the sprite bitmap.
	; Only the pulse size changes with movement; the floor anchor itself uses
	; the projected inverse ob_y offset like the projectile/old floor path.
	clr	g2_shadow_yoff
	move	ob_y(a2),d0
	neg	d0
.scale_yoff	ext.l	d0
