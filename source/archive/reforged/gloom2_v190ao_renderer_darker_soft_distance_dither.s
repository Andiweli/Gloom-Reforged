...
; v190ao: renderer half-step distance dithering + slightly darker wall/floor shading
...
.vloop	;
	...
	move	0(a5,d6*2),d5
	; v190ao: distance-dither flats as a cheap half-step between shade bands.
	; Alternating by scanline/Z parity darkens the floor/ceiling slightly while
	; breaking up the old hard horizontal distance steps.
	move	d6,d3
	add	d7,d3
	btst	#0,d3
	beq.s	.g2v190ao_flatshade_ok
	cmp	#15,d5
	bcc.s	.g2v190ao_flatshade_ok
	addq	#1,d5
.g2v190ao_flatshade_ok
...
.solid	move.l	a3,vd_data(a0)	;start column
	...
	move	0(a2,d2*2),d3
	; v190ao: distance-dither the wall shade one half-step darker.
	; This keeps the original 16 shade palettes, but alternates adjacent
	; shade levels by screen column/Z parity so the depth bands no longer
	; cut as hard across the wall texture.
	move	d2,d6
	add	d7,d6
	btst	#0,d6
	beq.s	.g2v190ao_wallshade_ok
	cmp	#15,d3
	bcc.s	.g2v190ao_wallshade_ok
	addq	#1,d3
.g2v190ao_wallshade_ok
	movem	d2-d3,vd_z(a0)
	;
	move	#-256,d3
	sub	camy(pc),d3
