...
.drawloop	move.l	(a6),d0
	...
	move	sh_x(a6),d0
	add	midx(pc),d0
	; v190b: transparent wall strips can be queued just outside the visible
	; screen on some maps.  Guard coloffs access before indexing the column
	; table, otherwise map1_5 can crash while entering/loading the level.
	tst	d0
	blt	.drawloop
	cmp	width(pc),d0
	bge	.drawloop
...
; -----------------------------------------------------------------------------
; v137/v176/v190b enemy blob shadows + floor-anchored multi-colour reflections.
; Blob shadows keep the confirmed hard-edged foot shadow path.  Reflections are
; now dark/dithered only: no brighter edge tint is used.  Projectiles use a
; wide oval, and pickup rings stay on the floor all the time.
; -----------------------------------------------------------------------------
...
g2_reflection_colour
	; v190b: use only the darker projectile/upgrade colour for the whole
	; reflection.  Edge pixels are still dithered, but no brighter edge tint is
	; stored/used anymore.
	movem.l	d1-d3/a0,-(a7)
	moveq	#0,d2
...
.edge_colour	lea	g2_reflect_rgb(pc),a0
	...
	bne	.store_edge
	moveq	#15,d3
.store_edge	move	d0,g2_reflect_edge_col	; v190b: edge stays dark, no bright rim
	movem.l	(a7)+,d1-d3/a0
	rts

; RGB12 colours, remapped to the active Gloom palette at runtime.
; v174/v190b weapon/projectile colours: 1 yellow, 2 green, 3 green/white,
; 4 blue/white, 5 magenta.  Dark table is the visible reflection body.
; The bright table remains only as unused data for compatibility/reference.
g2_reflect_dark_rgb
...
.rdraw	move	g2_shadow_col(pc),d4
	; v190b: all reflection bands use the dark reflection colour only.
	; Outer/mid bands are still dithered against the floor, but no lighter
	; edge colour is applied anymore.
...
solidstrip	set	0
drawstrip2	; v190b chunky-safe transparent wall strip overlay, map1_5 safe
	...
	; already-rendered scene/floor behind untouched.  Palette/texel indices are
	; cleaned before table lookups to avoid stale-register crashes.
	movem.l	d0-d7/a0-a5,-(a7)
