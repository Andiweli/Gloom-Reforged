...
.g2sms_timer_ok
	...
	ble.w	.g2sms_done
	; v55: original statusbar scroller uses red text.  Pick the
	; already-remapped red health-cell colour from panel shape #45 so
	; the text follows the active chunky/C2P palette instead of using a
	; generic bright/yellow colour.
	move.l	panel(pc),a0
...
g2draw_statusbar_char	;d0.b=character, d7.w=x, d2.b=chunky colour
	; v55: clip to the real black message slot in the final statusbar.
	; The full 8px character must fit inside x=111..208, otherwise it
	; would scroll over the green/weapon/life graphics.
	cmp	#111,d7
	blt.s	.g2smc_rts
	cmp	#201,d7
	bgt.s	.g2smc_rts
	...
	adda.l	d0,a0
	move.l	chunky(pc),a1
	adda.l	#320*230,a1	;statusbar message row inside 224..239 panel
	move	d7,d1
	ext.l	d1
	adda.l	d1,a1
	moveq	#4,d5		;5 rows
