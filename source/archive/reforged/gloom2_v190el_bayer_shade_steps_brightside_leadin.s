...
.g2v190dw_flat_shade
	...
	move.l	0(a5,d5*4),a5
	; v190el: put the dotted lead-in on the brighter side of each
	; visible shade transition.  The first half of the local depth band
	; mixes the current shade toward the next darker palette, so the
	; player-facing/bright side already gets dark Bayer points before
	; the hard fog step.  The second half then tails back with lighter
	; points after the step.  Geometry and projection stay untouched.
	clr	g2_bayer_thresh
	cmp	#(2<<grdshft),d0
	blo.s	.g2v190ej_flatblend_done
	move	d0,d1
	and	#$3f,d1
	cmp	#32,d1
	blo.s	.g2v190el_flatblend_darker_leadin
	; darker side: current shade fades back toward the previous lighter shade
	tst	d5
	beq.s	.g2v190ej_flatblend_done
	sub	#32,d1
	lsr	#2,d1
	moveq	#8,d2
	sub	d1,d2
	...
	bra.s	.g2v190ej_flatblend_done
.g2v190el_flatblend_darker_leadin
	; brighter side: add dark Bayer points before the fog step
	cmp	#14,d5
	bcc.s	.g2v190ej_flatblend_done
	lsr	#2,d1
	addq	#1,d1
	cmp	#8,d1
	bls.s	.g2v190el_flatblend_dark_thresh_ok
	moveq	#8,d1
.g2v190el_flatblend_dark_thresh_ok
	move	d1,g2_bayer_thresh
	move	d5,d1
	addq	#1,d1
	cmp	#14,d1
	bls.s	.g2v190ej_flatblend_next_ok
	moveq	#14,d1
.g2v190ej_flatblend_next_ok
	move.l	palette(pc),a4
	move.l	0(a4,d1*4),g2_bayer_nextpal
...
.hiteok	;
	...
	move.l	0(a2,d5*4),a4
	; v190el: dither the shade-step with the dark lead-in on the
	; brighter/player-facing side.  No wall column or texel is skipped.
	clr	g2_bayer_thresh
	move	vd_z(a0),d6
	tst	g2_visibility
	ble.s	.g2v190ej_wallblend_dist_ok
	lsr	#1,d6
.g2v190ej_wallblend_dist_ok
	cmp	#(2<<grdshft),d6
	blo.s	.g2v190ej_wallblend_setup_done
	movem.l	d1/d7/a2,-(a7)
	move	d6,d1
	and	#$3f,d1
	cmp	#32,d1
	blo.s	.g2v190el_wallblend_darker_leadin
	; darker side: current shade fades back toward the previous lighter shade
	tst	d5
	beq.s	.g2v190el_wallblend_restore
	sub	#32,d1
	lsr	#2,d1
	moveq	#8,d7
	sub	d1,d7
	...
	subq	#1,d5
	move.l	0(a2,d5*4),a5
	bra.s	.g2v190el_wallblend_restore
.g2v190el_wallblend_darker_leadin
	; brighter side: add dark Bayer points before the fog step
	cmp	#14,d5
	bcc.s	.g2v190el_wallblend_restore
	lsr	#2,d1
	addq	#1,d1
	cmp	#8,d1
	bls.s	.g2v190el_wallblend_dark_thresh_ok
	moveq	#8,d1
.g2v190el_wallblend_dark_thresh_ok
	move	d1,g2_bayer_thresh
	addq	#1,d5
	cmp	#14,d5
	bls.s	.g2v190ej_wallblend_next_ok
	moveq	#14,d5
.g2v190ej_wallblend_next_ok
	move.l	0(a2,d5*4),a5
.g2v190el_wallblend_restore
	movem.l	(a7)+,d1/d7/a2
...
g2_visibility	dc	-1	;v190ek -1=DEFAULT, +1=ADVANCED; saved in gloom.cfg
...
g2_bayer_thresh	dc	0	;v190el 0=no blend, 1..8 bright-side shade-step lead-in
