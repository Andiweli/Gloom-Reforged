...
.osskipz	;
	...
	move.l	d0,maptable
	;
	; v190d: transient scene memory must be larger than original gloom.s.
	; The chunky 320-column path can allocate more per-frame wall/transparent
	; strip draw items than the original lower-width copper path.  Complex maps
	; such as map5_1 could overrun the old 32 KB scratch block during the first
	; calculated frame and crash while appearing to load.
	move.l	#65536,d0
	moveq	#1,d1
	allocmem	memory
	move.l	d0,memory
