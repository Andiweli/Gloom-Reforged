...
solidstrip	set	0
drawstrip2	; v197 chunky-safe transparent wall strip overlay with true green alpha
	...
	; texels are blended 50/50 against the already-rendered chunky pixel using
	; planar_palette + planar_remap, the same RGB blend path used by drawobjtrans.
	movem.l	d0-d7/a0-a5,-(a7)
	move.l	vd_data(a4),d0
	beq	.rts
	move.l	d0,a0		; source texture column
	move	#$080,d6		; green glass RGB12 tint before 50% blend
	move.l	chunkymod(pc),d7
	move	vd_y(a4),d2
