...
.hiteok	;
	...
	; v190bj: conservative broad wall dither.  Only in the mid/far shade
	; range, alternate every couple of columns to the next lighter palette.
	; This softens hard shade bands without touching the deepest tunnel fog.
	move.l	a1,d6
	cmp	#8,d5
	blo.s	.g2v190bj_nodither
	cmp	#14,d5
	bhs.s	.g2v190bj_nodither
	btst	#1,d6
	beq.s	.g2v190bj_nodither
	subq	#1,d5
.g2v190bj_nodither
	move.l	0(a2,d5*4),a4
	sub	d3,d2
...
.hiteok
	...
	add.l	d3,d0
	move	vd_pal(a4),d5
	; v190bj: same broad dither for transparent wall strips so they blend
	; with the solid wall shading transitions.
	move.l	a1,d6
	cmp	#8,d5
	blo.s	.g2v190bj_strip_nodither
	cmp	#14,d5
	bhs.s	.g2v190bj_strip_nodither
	btst	#1,d6
	beq.s	.g2v190bj_strip_nodither
	subq	#1,d5
.g2v190bj_strip_nodither
	move.l	0(a2,d5*4),a5
.loop
	move.b	0(a0,d0),d5
	beq.s	.skip
