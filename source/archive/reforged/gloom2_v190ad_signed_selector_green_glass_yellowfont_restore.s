...
solidstrip	set	0
drawstrip2	; v190ad chunky transparent wall strip with original negative selector tint
	; a1=top of destination column, a2=palettes base, a4=strip/vd data
	; Original gloom.s does not use selector values 0..6 here.  The byte
	; directly before the texture column is signed: -7..-1 select the old
	; stripands masks (red, green, blue, yellow, purple, cyan, white).  The
	; map1_3/map1_7 green glass strips use that signed selector, so emulate
	; it in chunky mode.  Selector -1/white remains fully clear/transparent.
	movem.l	d0-d7/a0-a5,-(a7)
	move.l	vd_data(a4),d0
	beq	.rts
	move.l	d0,a0		; source texture column
	moveq	#-1,d6		; no colour tint / pure transparency by default
	move.b	-1(a0),d6	; signed original strip selector byte
	ext	d6
	cmp	#-1,d6		; original white mask: leave background unchanged
	beq.s	.notint
	cmp	#-7,d6
	blt.s	.notint
	cmp	#-2,d6
	...
	move	0(a3,d6*2),d6	; RGB12 tint matching original strip selector
.notint
	move.l	chunkymod(pc),d7
	move	vd_y(a4),d2
...
.hiteok
	...
	move	vd_pal(a4),d5
	move.l	0(a2,d5*4),a5
	move	#$0eee,d2		; packed-RGB safe average mask, like drawobjtrans
...
.loop
	...
	bra.s	.next
.glass	addx.l	d3,d0		; advance before RGB blend clobbers flags/X
	tst	d6
	bmi.s	.next		; selector -1/invalid: pure transparency
	move	d6,d5
