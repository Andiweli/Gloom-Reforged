	...
	;gloom2_c86zdh: TWO PLAYER COMBAT start ESC guard; ignore stale escape until released before opening dogamemenu.
	;gloom2_c86zdj: diagnostic: keep guard, log actual finished=1 source at mainexit.
...
.notboost
	...
	tst	d7
	bne	.loop
	cmp	#2,gametype	;c86zdj: remember source of finished=1 in TWO PLAYER COMBAT
	bne.s	g2zdj_no_menu_finish1_src
	move	#1,g2combat_finish1_source
g2zdj_no_menu_finish1_src
	move	#1,finished
	;
...
.serhere	bsr	serget
	...
	beq.s	.djoy
	and	#$ff,d0
	cmp	#1,d0		;c86zdj: linked/serial finished=1 source marker
	bne.s	g2zdj_no_serial_finish1_src
	cmp	#2,gametype
	bne.s	g2zdj_no_serial_finish1_src
	move	#2,g2combat_finish1_source
g2zdj_no_serial_finish1_src
	move	d0,finished
	moveq	#0,d0
	;bra.s	.sgot
	;
...
finished	dc	0
finished2	dc	0
g2combat_start_escape_guard	dc	0	;c86zdh: active until startup ESC is released once
	;
...
.nolmed	;
	...
	clr	finished
	clr	finished2
	clr	g2combat_finish1_source	;c86zdj: clear diagnostic source marker
	...
	clr	showflag
	clr	escape
	clr	g2combat_start_escape_guard	;c86zdh: reset stale-ESC guard
	cmp	#2,gametype	;c86zdh: only TWO PLAYER COMBAT gets startup ESC guard
	bne.s	g2zdh_no_start_esc_guard
	move	#-1,g2combat_start_escape_guard
g2zdh_no_start_esc_guard
	clr	frame
	;
	bsr	syncup
	;
...
mainloop	; v190hy cleanup: log frame counter removed
	jsr	drawall
	; c86zdh: TWO PLAYER COMBAT may inherit a stale ESC state from the
	; previous menu/start path.  Do not open dogamemenu until ESC has
	; been seen released once after gameplay is already running.
	cmp	#2,gametype
	bne.s	g2zdh_check_escape
	tst	g2combat_start_escape_guard
	beq.s	g2zdh_check_escape
	tst	escape
	beq.s	g2zdh_release_escape_guard
	clr	escape
	bra.s	g2zdh_noesc
g2zdh_release_escape_guard
	clr	g2combat_start_escape_guard
g2zdh_check_escape
	move	escape,d0
	beq.s	g2zdh_noesc
	jsr	dogamemenu
	clr	escape
g2zdh_noesc	; c86zdi: global no-ESC label, fixes GenAm local-label scope
...
.doit	lea	joyx,a0
	jsr	readjoy1
	lea	joyx0,a0
	jsr	readkeys	;c86zdh build safety: GenAm BSR range can overflow after guard code
	move.l	joyx0(pc),d0
	or.l	d0,joyx
	move.l	joyb0(pc),d0
	or.l	d0,joyb
...
; c86zdj: safe wrapper for diagnostic logging after forbid.
; Diagnostic only: temporarily permits DOS file IO, logs, then forbids again.
