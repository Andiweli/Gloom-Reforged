...
; v142/v144: safe title gloombrush overlay.
; Draws optional pics/gloom at Y=168 using linemod, not 40*bitplanes.
; The old offset worked only for the old interleaved 7-plane layout and
; can overrun the current compact title bitmap.
...
dointro	;
	...
	;
	; v142/v144: safe optional pics/gloom overlay.  The old code used
	; 168*40*bitplanes as destination offset, which is wrong for the
	; current bitmap layout and can decode beyond the bitmap.  Use
	; 168*linemod, clamp the brush height to the remaining 72 lines,
	; and draw before the menu copies the title bitmap.
	tst	aga
	beq	.noaga
...
.noaga	;
	...
	bsr	chaton
	;
	; v143: restore original title flow.  After blackmagic/title/gloombrush
	; the title screen stays visible until the player presses fire/key.
	; Only then the menu is drawn over the title.  Keep input enabled before
	; waitany so OS/rawkey paths can see the first press.
	jsr	inputon
	jsr	waitany
	;
	lea	startmenu,a4
