...
dotelesfx	move.l	telesfx(pc),a0
	beq.s	.rts		;v105f: no silent/null sample access if teleport.bin was not loaded
	; v105f: play teleport.bin on both first audio sides.  The original
	; playsfx path normally picks sfx0 first; on some real/WinUAE stereo setups
	; that can make this one-shot effectively disappear.  Starting sfx0+sfx1
	; keeps the original sample but makes the teleport clearly audible.
	lea	sfxs(pc),a1
	moveq	#64,d0
	moveq	#120,d1
	move	d1,fx_priority(a1)
