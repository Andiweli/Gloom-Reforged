...
trainer_maintain_one	;v111: keep enabled trainer options permanent across pickups/levels
	moveq	#0,d7
	move	trainer_weapon,d0
	beq.s	.weapon_done
	subq	#1,d0
...
.inc2	add	#32,width
	;
sizedone	;v111: non-fullscreen view sizes stay clear of the statusbar area.
	; The old 288x224 ALMOST FULL mode could leave bottom artefacts just above
	; the statusbar; only real FULLSCREEN uses the full 224-line playfield.
	cmp	#320,width
	bcc.s	.size_ok
	cmp	#192,hite
	bls.s	.size_ok
	move	#192,hite
.size_ok	move	width(pc),d0
	move	d0,chunkymodw
	lsr	#1,d0
	move	d0,maxx
	neg	d0
	...
	neg	d0
	move	d0,miny
	bsr	trainer_update_display_texts	;v110: rebuild menu with the new VIEW SIZE text immediately
	bra	refresh
...
playerdie	;v111: unlimited health cheat also blocks lethal damage paths
	tst	trainer_invincible
	beq.s	.die
	move	#25,ob_hitpoints(a5)
	st	ob_update(a5)
...
p2ctype
	...
	dc.b	'CD32 PAD 2',0	;5
	elseif
	dc.b	' KEYBOARD',0		;5 v111: PLAYER 2 default must not duplicate PLAYER 1
	endc
	;
	dc.b	92,0			;6 spacer
