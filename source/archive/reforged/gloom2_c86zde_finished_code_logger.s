	...
	;gloom2_c86zcn: clean basis from c86zc7; keep only confirmed TWO PLAYER COMBAT fresh-input waitfix for combat picture/intermission. No P96, no speculative cleanup fixes.
	;gloom2_c86zcs: restore other-player reflection in TWO PLAYER modes; add non-invasive DH3 combat stage logger only around combat map/event path.
	;
...
g2_setup_enemy_blob_column
	...
	move.l	player1(pc),d0
	cmp.l	d0,a2
	beq	g2zcs_player_shadow_owner
	move.l	player2(pc),d0
	cmp.l	d0,a2
	beq	g2zcs_player_shadow_owner
	bra.s	g2zcs_not_player_owner
g2zcs_player_shadow_owner
	...
	cmp.l	player_(pc),a2
	beq	g2zcs_blob_done
	bra	g2zcs_player_try_blob
g2zcs_not_player_owner
	;
	...
	bra	g2zcs_try_blob
	;
g2zcs_setup_reflection
	bsr	g2_prepare_reflection_column
	bra	g2zcs_blob_done
	;
g2zcs_player_try_blob
	; c86zcs: TWO PLAYER visible other-player reflection.
	; c86zc7 still jumped player-owned shapes directly to blob shadow only;
	; restore the same safe floor-anchored reflection setup used by enemies.
	tst	g2_reflections
	ble	g2zcs_player_blob_only
	tst	ob_hitpoints(a2)
	ble	g2zcs_player_blob_only
	cmp	#3<<grdshft,d2
	bgt	g2zcs_player_blob_only
	bsr	g2_prepare_enemy_reflection_safe
g2zcs_player_blob_only
	tst	g2_blobshadow
	ble	g2zcs_blob_done
	bra	g2zcs_blob_after_colltype
	;
g2zcs_try_blob
	tst	g2_blobshadow
	ble	g2zcs_blob_done
...
g2zcs_rx_max_ok
	...
	bne	g2zcs_col_ok
	moveq	#1,d0
g2zcs_col_ok	move	d0,g2_shadow_col
	move	#1,g2_shadow_active
g2zcs_blob_done	movem.l	(a7)+,d0-d7/a0-a2
	rts
...
; c86zcn: TWO PLAYER COMBAT picture/intermission must not be skipped by
; the Fire/Return press that selected the menu item or ended the previous round.
; First require a full release, then a fresh press, then release again.
g2waitfreshany	movem.l	d0-d7/a0-a6,-(a7)
g2zcn_release	bsr	checkany
	bne.s	g2zcn_release
g2zcn_press	bsr	checkany
	beq.s	g2zcn_press
g2zcn_release2	bsr	checkany
	bne.s	g2zcn_release2
	movem.l	(a7)+,d0-d7/a0-a6
	rts
...
.loop	move.b	(a0)+,(a1)+
	...
	move.l	combatpal,a1
	jsr	showpic
	bsr	g2waitfreshany	; c86zcn: wait for a new Fire/Return before loading combat map
...
scriptplay	;
	...
	;
	bsr	pickcombat
	lea	g2combat_stage_pick_done,a0
	move.l	#g2combat_stage_pick_done_len,d3
	jsr	g2combat_stage_log
	bra.s	.gotname
	;
.notcombat	;
...
.p2	;
	bsr	permit
	;
	; c86zcs: logger-safe map load.  The logger preserves registers,
	; but set a0 to the real map path only after logging.
	lea	g2combat_stage_mapload_begin,a0
	move.l	#g2combat_stage_mapload_begin_len,d3
	jsr	g2combat_stage_log
	move.l	map_test(pc),d0
	bne.s	g2zcs_use_maptest
	move.l	#mappath,d0
g2zcs_use_maptest
	move.l	d0,a0
	moveq	#1,d1
	bsr	loadfile
	move.l	d0,map_map
	lea	g2combat_stage_mapload_done,a0
	move.l	#g2combat_stage_mapload_done_len,d3
	jsr	g2combat_stage_log
	tst.l	map_map
	bne.s	g2v10_map_loaded
	lea	g2combat_stage_mapload_fail,a0
	move.l	#g2combat_stage_mapload_fail_len,d3
	jsr	g2combat_stage_log
	bsr	forbid
...
g2v10_map_loaded
	;
	lea	g2combat_stage_initmap_begin,a0
	move.l	#g2combat_stage_initmap_begin_len,d3
	jsr	g2combat_stage_log
	bsr	initmap
	lea	g2combat_stage_initmap_done,a0
	move.l	#g2combat_stage_initmap_done_len,d3
	jsr	g2combat_stage_log
	bsr	loadtxts
	move	#$a3f7,d0
	jsr	seedrnd
	moveq	#1,d0
	lea	g2combat_stage_execevent_begin,a0
	move.l	#g2combat_stage_execevent_begin_len,d3
	jsr	g2combat_stage_log
	jsr	execevent
	lea	g2combat_stage_execevent_done,a0
	move.l	#g2combat_stage_execevent_done_len,d3
	jsr	g2combat_stage_log
	tst.l	player1
	bne.s	g2zcs_player1_present_before_forbid
	lea	g2combat_stage_player1_missing,a0
	move.l	#g2combat_stage_player1_missing_len,d3
	jsr	g2combat_stage_log
	bra.s	g2zcs_after_player1_stage
g2zcs_player1_present_before_forbid
	lea	g2combat_stage_player1_ok,a0
	move.l	#g2combat_stage_player1_ok_len,d3
	jsr	g2combat_stage_log
g2zcs_after_player1_stage
	jsr	g2v190cx_build_g1_tables	; v190cx: synthetic palette/remap for original Gloom
	lea	g2combat_stage_g1tables_done,a0
	move.l	#g2combat_stage_g1tables_done_len,d3
	jsr	g2combat_stage_log
	lea	g2combat_stage_before_forbid,a0
	move.l	#g2combat_stage_before_forbid_len,d3
	jsr	g2combat_stage_log
	bsr	forbid
	lea	g2combat_stage_after_forbid,a0
	move.l	#g2combat_stage_after_forbid_len,d3
	jsr	g2combat_stage_log_safe_forbid
	;
	...
	bra	gameover
g2v10_player1_ok
	lea	g2combat_stage_player_guard_ok,a0
	move.l	#g2combat_stage_player_guard_ok_len,d3
	jsr	g2combat_stage_log_safe_forbid
	;
...
.g2v190cj_no_game_pal
	lea	g2combat_stage_after_pokepal,a0
	move.l	#g2combat_stage_after_pokepal_len,d3
	jsr	g2combat_stage_log_safe_forbid
	bsr	calcpalettes
	lea	g2combat_stage_after_calcpalettes,a0
	move.l	#g2combat_stage_after_calcpalettes_len,d3
	jsr	g2combat_stage_log_safe_forbid
	bsr	dispoff
	lea	g2combat_stage_after_dispoff,a0
	move.l	#g2combat_stage_after_dispoff_len,d3
	jsr	g2combat_stage_log_safe_forbid
	;
	;
	;init player vars...
...
.p1hok	move	p1health(pc),ob_hitpoints(a5)
	...
	;
.psk	jsr	resetplayer
	lea	g2combat_stage_after_p1_reset,a0
	move.l	#g2combat_stage_after_p1_reset_len,d3
	jsr	g2combat_stage_log_safe_forbid
...
.psk2	jsr	resetplayer
	lea	g2combat_stage_after_p2_reset,a0
	move.l	#g2combat_stage_after_p2_reset_len,d3
	jsr	g2combat_stage_log_safe_forbid
	jsr	trainer_maintain_one	;v115: apply persistent trainer at level start
.p1	;
	bsr	linkswap
	lea	g2combat_stage_after_linkswap,a0
	move.l	#g2combat_stage_after_linkswap_len,d3
	jsr	g2combat_stage_log_safe_forbid
	;
...
.nolmed	;
	...
	clr	framecnt
	clr	paused
	lea	g2combat_stage_before_predrawall,a0
	move.l	#g2combat_stage_before_predrawall_len,d3
	jsr	g2combat_stage_log_safe_forbid
	jsr	predrawall
	lea	g2combat_stage_after_predrawall,a0
	move.l	#g2combat_stage_after_predrawall_len,d3
	jsr	g2combat_stage_log_safe_forbid
	clr	g2combat_cx_drawlog
	clr	g2combat_post8_log
	bsr	dispon
	lea	g2combat_stage_after_dispon,a0
	move.l	#g2combat_stage_after_dispon_len,d3
	jsr	g2combat_stage_log_safe_forbid
	bsr	chaton
	lea	g2combat_stage_after_chaton,a0
	move.l	#g2combat_stage_after_chaton_len,d3
	jsr	g2combat_stage_log_safe_forbid
	;
mainloop	; c86zda: log the first eight TWO PLAYER COMBAT drawall iterations.
	cmp	#2,gametype
	bne	g2combat_cz_before_drawlog_done
	move	g2combat_cx_drawlog,d0
	cmp	#0,d0
	beq	g2combat_cz_before_draw1
	cmp	#2,d0
	beq	g2combat_cz_before_draw2
	cmp	#4,d0
	beq	g2combat_cz_before_draw3
	cmp	#6,d0
	beq	g2combat_cz_before_draw4
	cmp	#8,d0
	beq	g2combat_cz_before_draw5
	cmp	#10,d0
	beq	g2combat_cz_before_draw6
	cmp	#12,d0
	beq	g2combat_cz_before_draw7
	cmp	#14,d0
	beq	g2combat_cz_before_draw8
	bra	g2combat_cz_before_drawlog_done
g2combat_cz_before_draw1
	move	#1,g2combat_cx_drawlog
	lea	g2combat_stage_before_first_drawall,a0
	move.l	#g2combat_stage_before_first_drawall_len,d3
	jsr	g2combat_stage_log_safe_forbid
	bra	g2combat_cz_before_drawlog_done
g2combat_cz_before_draw2
	move	#3,g2combat_cx_drawlog
	lea	g2combat_stage_before_second_drawall,a0
	move.l	#g2combat_stage_before_second_drawall_len,d3
	jsr	g2combat_stage_log_safe_forbid
	bra	g2combat_cz_before_drawlog_done
g2combat_cz_before_draw3
	move	#5,g2combat_cx_drawlog
	lea	g2combat_stage_before_third_drawall,a0
	move.l	#g2combat_stage_before_third_drawall_len,d3
	jsr	g2combat_stage_log_safe_forbid
	bra	g2combat_cz_before_drawlog_done
g2combat_cz_before_draw4
	move	#7,g2combat_cx_drawlog
	lea	g2combat_stage_before_fourth_drawall,a0
	move.l	#g2combat_stage_before_fourth_drawall_len,d3
	jsr	g2combat_stage_log_safe_forbid
	bra	g2combat_cz_before_drawlog_done
g2combat_cz_before_draw5
	move	#9,g2combat_cx_drawlog
	lea	g2combat_stage_before_fifth_drawall,a0
	move.l	#g2combat_stage_before_fifth_drawall_len,d3
	jsr	g2combat_stage_log_safe_forbid
	bra	g2combat_cz_before_drawlog_done
g2combat_cz_before_draw6
	move	#11,g2combat_cx_drawlog
	lea	g2combat_stage_before_sixth_drawall,a0
	move.l	#g2combat_stage_before_sixth_drawall_len,d3
	jsr	g2combat_stage_log_safe_forbid
	bra	g2combat_cz_before_drawlog_done
g2combat_cz_before_draw7
	move	#13,g2combat_cx_drawlog
	lea	g2combat_stage_before_seventh_drawall,a0
	move.l	#g2combat_stage_before_seventh_drawall_len,d3
	jsr	g2combat_stage_log_safe_forbid
	bra	g2combat_cz_before_drawlog_done
g2combat_cz_before_draw8
	move	#15,g2combat_cx_drawlog
	lea	g2combat_stage_before_eighth_drawall,a0
	move.l	#g2combat_stage_before_eighth_drawall_len,d3
	jsr	g2combat_stage_log_safe_forbid
g2combat_cz_before_drawlog_done
	jsr	drawall
	cmp	#2,gametype
	bne	g2combat_cz_after_drawlog_done
	move	g2combat_cx_drawlog,d0
	cmp	#1,d0
	beq	g2combat_cz_after_draw1
	cmp	#3,d0
	beq	g2combat_cz_after_draw2
	cmp	#5,d0
	beq	g2combat_cz_after_draw3
	cmp	#7,d0
	beq	g2combat_cz_after_draw4
	cmp	#9,d0
	beq	g2combat_cz_after_draw5
	cmp	#11,d0
	beq	g2combat_cz_after_draw6
	cmp	#13,d0
	beq	g2combat_cz_after_draw7
	cmp	#15,d0
	beq	g2combat_cz_after_draw8
	bra	g2combat_cz_after_drawlog_done
g2combat_cz_after_draw1
	move	#2,g2combat_cx_drawlog
	lea	g2combat_stage_after_first_drawall,a0
	move.l	#g2combat_stage_after_first_drawall_len,d3
	jsr	g2combat_stage_log_safe_forbid
	bra	g2combat_cz_after_drawlog_done
g2combat_cz_after_draw2
	move	#4,g2combat_cx_drawlog
	lea	g2combat_stage_after_second_drawall,a0
	move.l	#g2combat_stage_after_second_drawall_len,d3
	jsr	g2combat_stage_log_safe_forbid
	bra	g2combat_cz_after_drawlog_done
g2combat_cz_after_draw3
	move	#6,g2combat_cx_drawlog
	lea	g2combat_stage_after_third_drawall,a0
	move.l	#g2combat_stage_after_third_drawall_len,d3
	jsr	g2combat_stage_log_safe_forbid
	bra	g2combat_cz_after_drawlog_done
g2combat_cz_after_draw4
	move	#8,g2combat_cx_drawlog
	lea	g2combat_stage_after_fourth_drawall,a0
	move.l	#g2combat_stage_after_fourth_drawall_len,d3
	jsr	g2combat_stage_log_safe_forbid
	bra	g2combat_cz_after_drawlog_done
g2combat_cz_after_draw5
	move	#10,g2combat_cx_drawlog
	lea	g2combat_stage_after_fifth_drawall,a0
	move.l	#g2combat_stage_after_fifth_drawall_len,d3
	jsr	g2combat_stage_log_safe_forbid
	bra	g2combat_cz_after_drawlog_done
g2combat_cz_after_draw6
	move	#12,g2combat_cx_drawlog
	lea	g2combat_stage_after_sixth_drawall,a0
	move.l	#g2combat_stage_after_sixth_drawall_len,d3
	jsr	g2combat_stage_log_safe_forbid
	bra	g2combat_cz_after_drawlog_done
g2combat_cz_after_draw7
	move	#14,g2combat_cx_drawlog
	lea	g2combat_stage_after_seventh_drawall,a0
	move.l	#g2combat_stage_after_seventh_drawall_len,d3
	jsr	g2combat_stage_log_safe_forbid
	bra	g2combat_cz_after_drawlog_done
g2combat_cz_after_draw8
	move	#16,g2combat_cx_drawlog
	lea	g2combat_stage_after_eighth_drawall,a0
	move.l	#g2combat_stage_after_eighth_drawall_len,d3
	jsr	g2combat_stage_log_safe_forbid
g2combat_cz_after_drawlog_done
	moveq	#0,d0
	moveq	#1,d1
	lea	g2combat_stage_post8_before_escape,a0
	move.l	#g2combat_stage_post8_before_escape_len,d3
	jsr	g2combat_post8_log_stage
	move	escape,d0
	moveq	#1,d0
	moveq	#2,d1
	lea	g2combat_stage_post8_escape_read,a0
	move.l	#g2combat_stage_post8_escape_read_len,d3
	jsr	g2combat_post8_log_stage
	move	escape,d0
	beq.s	.noesc
	moveq	#2,d0
	moveq	#3,d1
	lea	g2combat_stage_post8_before_dogamemenu,a0
	move.l	#g2combat_stage_post8_before_dogamemenu_len,d3
	jsr	g2combat_post8_log_stage
	jsr	dogamemenu
	moveq	#3,d0
	moveq	#4,d1
	lea	g2combat_stage_post8_after_dogamemenu,a0
	move.l	#g2combat_stage_post8_after_dogamemenu_len,d3
	jsr	g2combat_post8_log_stage
	clr	escape
.noesc	; v190hy cleanup: log marker removed
	moveq	#4,d0
	moveq	#5,d1
	lea	g2combat_stage_post8_after_escape_path,a0
	move.l	#g2combat_stage_post8_after_escape_path_len,d3
	jsr	g2combat_post8_log_stage
	...
	moveq	#5,d0
	moveq	#6,d1
	lea	g2combat_stage_post8_tele_hold_active,a0
	move.l	#g2combat_stage_post8_tele_hold_active_len,d3
	jsr	g2combat_post8_log_stage
...
.g2no_tele_black_hold
	moveq	#5,d0
	moveq	#6,d1
	lea	g2combat_stage_post8_after_tele_check,a0
	move.l	#g2combat_stage_post8_after_tele_check_len,d3
	jsr	g2combat_post8_log_stage
	move	finished(pc),d0
	moveq	#6,d0
	moveq	#7,d1
	lea	g2combat_stage_post8_finished_read,a0
	move.l	#g2combat_stage_post8_finished_read_len,d3
	jsr	g2combat_post8_log_stage
	move	finished(pc),d0
	beq.s	g2combat_post8_loop_continue
	move	d0,d2
	and	#127,d2
	cmp	#1,d2
	beq	g2combat_zde_finished_quit
	cmp	#2,d2
	beq	g2combat_zde_finished_death
	cmp	#3,d2
	beq	g2combat_zde_finished_levelover
	cmp	#4,d2
	beq	g2combat_zde_finished_combatwon
	lea	g2combat_stage_finished_unknown,a0
	move.l	#g2combat_stage_finished_unknown_len,d3
	bra	g2combat_zde_finished_log
g2combat_zde_finished_quit
	lea	g2combat_stage_finished_quit,a0
	move.l	#g2combat_stage_finished_quit_len,d3
	bra	g2combat_zde_finished_log
g2combat_zde_finished_death
	lea	g2combat_stage_finished_death,a0
	move.l	#g2combat_stage_finished_death_len,d3
	bra	g2combat_zde_finished_log
g2combat_zde_finished_levelover
	lea	g2combat_stage_finished_levelover,a0
	move.l	#g2combat_stage_finished_levelover_len,d3
	bra	g2combat_zde_finished_log
g2combat_zde_finished_combatwon
	lea	g2combat_stage_finished_combatwon,a0
	move.l	#g2combat_stage_finished_combatwon_len,d3
g2combat_zde_finished_log
	moveq	#7,d0
	moveq	#8,d1
	jsr	g2combat_post8_log_stage
	bra.s	g2combat_post8_mainexit_go
g2combat_post8_loop_continue
	moveq	#7,d0
	moveq	#8,d1
	lea	g2combat_stage_post8_loop_continues,a0
	move.l	#g2combat_stage_post8_loop_continues_len,d3
	jsr	g2combat_post8_log_stage
	bra	mainloop
g2combat_post8_mainexit_go
	;
mainexit	st	paused
	bsr	chatoff
	bsr	dispoff
...
; c86zcs: minimal TWO PLAYER COMBAT stage logger.  It overwrites
; DH3:gloom_combat_stage.txt with the latest reached stage.  Diagnostic only.
; Call with a0 = message, d3 = message length.  Preserves all registers.
g2combat_stage_log	movem.l	d0-d7/a0-a6,-(a7)
	move.l	a0,a2
	move.l	d3,d4
	move.l	dosbase,d0
	beq	g2combat_stage_log_done
	move.l	d0,a6
	lea	g2combat_log_name(pc),a0
	move.l	a0,d1
	move.l	#1006,d2	; MODE_NEWFILE
	jsr	-30(a6)	; Open
	move.l	d0,d7
	...
	move.l	d7,d1
	jsr	-36(a6)	; Close
g2combat_stage_log_done	movem.l	(a7)+,d0-d7/a0-a6
	rts

; c86zcv: DOS stage logging while game is in Forbid region.
; Diagnostic only: briefly permits DOS, writes the stage, then forbids again.
; Call with a0 = message, d3 = message length. Preserves all registers.
g2combat_stage_log_safe_forbid	movem.l	d0-d7/a0-a6,-(a7)
	move.l	a0,a2
	move.l	d3,d4
	jsr	permit
	move.l	a2,a0
...
; c86zdc: after eight successful combat drawall frames, log the
; immediate post-draw mainloop path one step at a time.
; in: d0=expected post8 step, d1=new step, a0=message, d3=length.
; Preserves all registers.
g2combat_post8_log_stage	movem.l	d0-d7/a0-a6,-(a7)
	cmp	#2,gametype
	bne.s	g2combat_post8_log_done
	move	g2combat_cx_drawlog,d2
	cmp	#16,d2
	blt.s	g2combat_post8_log_done
	cmp	g2combat_post8_log,d0
	bne.s	g2combat_post8_log_done
	move	d1,g2combat_post8_log
	jsr	g2combat_stage_log_safe_forbid
g2combat_post8_log_done	movem.l	(a7)+,d0-d7/a0-a6
	rts

g2combat_log_name	dc.b	'DH3:gloom_combat_stage.txt',0
	even
g2combat_stage_pick_done	dc.b	'COMBAT: pickcombat returned',10
g2combat_stage_pick_done_len	equ	*-g2combat_stage_pick_done
g2combat_stage_mapload_begin	dc.b	'COMBAT: map load begin',10
g2combat_stage_mapload_begin_len	equ	*-g2combat_stage_mapload_begin
g2combat_stage_mapload_done	dc.b	'COMBAT: map load done',10
g2combat_stage_mapload_done_len	equ	*-g2combat_stage_mapload_done
g2combat_stage_mapload_fail	dc.b	'COMBAT: map load FAILED',10
g2combat_stage_mapload_fail_len	equ	*-g2combat_stage_mapload_fail
g2combat_stage_initmap_begin	dc.b	'COMBAT: initmap begin',10
g2combat_stage_initmap_begin_len	equ	*-g2combat_stage_initmap_begin
g2combat_stage_initmap_done	dc.b	'COMBAT: initmap done',10
g2combat_stage_initmap_done_len	equ	*-g2combat_stage_initmap_done
g2combat_stage_execevent_begin	dc.b	'COMBAT: execevent begin',10
g2combat_stage_execevent_begin_len	equ	*-g2combat_stage_execevent_begin
g2combat_stage_execevent_done	dc.b	'COMBAT: execevent done',10
g2combat_stage_execevent_done_len	equ	*-g2combat_stage_execevent_done
g2combat_stage_player1_missing	dc.b	'COMBAT: PLAYER1 MISSING after execevent',10
g2combat_stage_player1_missing_len	equ	*-g2combat_stage_player1_missing
g2combat_stage_player1_ok	dc.b	'COMBAT: player1 OK after execevent',10
g2combat_stage_player1_ok_len	equ	*-g2combat_stage_player1_ok
g2combat_stage_g1tables_done	dc.b	'COMBAT: g1tables done',10
g2combat_stage_g1tables_done_len	equ	*-g2combat_stage_g1tables_done
g2combat_stage_before_forbid	dc.b	'COMBAT: before forbid',10
g2combat_stage_before_forbid_len	equ	*-g2combat_stage_before_forbid
g2combat_stage_after_forbid	dc.b	'COMBAT: after forbid',10
g2combat_stage_after_forbid_len	equ	*-g2combat_stage_after_forbid
g2combat_stage_player_guard_ok	dc.b	'COMBAT: player guard OK',10
g2combat_stage_player_guard_ok_len	equ	*-g2combat_stage_player_guard_ok
g2combat_stage_after_pokepal	dc.b	'COMBAT: after pokepal',10
g2combat_stage_after_pokepal_len	equ	*-g2combat_stage_after_pokepal
g2combat_stage_after_calcpalettes	dc.b	'COMBAT: after calcpalettes',10
g2combat_stage_after_calcpalettes_len	equ	*-g2combat_stage_after_calcpalettes
g2combat_stage_after_dispoff	dc.b	'COMBAT: after dispoff',10
g2combat_stage_after_dispoff_len	equ	*-g2combat_stage_after_dispoff
g2combat_stage_after_p1_reset	dc.b	'COMBAT: after p1 resetplayer',10
g2combat_stage_after_p1_reset_len	equ	*-g2combat_stage_after_p1_reset
g2combat_stage_after_p2_reset	dc.b	'COMBAT: after p2 resetplayer',10
g2combat_stage_after_p2_reset_len	equ	*-g2combat_stage_after_p2_reset
g2combat_stage_after_linkswap	dc.b	'COMBAT: after linkswap',10
g2combat_stage_after_linkswap_len	equ	*-g2combat_stage_after_linkswap
g2combat_stage_before_predrawall	dc.b	'COMBAT: before predrawall',10
g2combat_stage_before_predrawall_len	equ	*-g2combat_stage_before_predrawall
g2combat_stage_after_predrawall	dc.b	'COMBAT: after predrawall',10
g2combat_stage_after_predrawall_len	equ	*-g2combat_stage_after_predrawall
g2combat_stage_after_dispon	dc.b	'COMBAT: after dispon',10
g2combat_stage_after_dispon_len	equ	*-g2combat_stage_after_dispon
g2combat_stage_after_chaton	dc.b	'COMBAT: after chaton',10
g2combat_stage_after_chaton_len	equ	*-g2combat_stage_after_chaton
g2combat_stage_before_first_drawall	dc.b	'COMBAT: before first drawall',10
g2combat_stage_before_first_drawall_len	equ	*-g2combat_stage_before_first_drawall
g2combat_stage_after_first_drawall	dc.b	'COMBAT: after first drawall',10
g2combat_stage_after_first_drawall_len	equ	*-g2combat_stage_after_first_drawall
g2combat_stage_before_second_drawall	dc.b	'COMBAT: before second drawall',10
g2combat_stage_before_second_drawall_len	equ	*-g2combat_stage_before_second_drawall
g2combat_stage_after_second_drawall	dc.b	'COMBAT: after second drawall',10
g2combat_stage_after_second_drawall_len	equ	*-g2combat_stage_after_second_drawall
g2combat_stage_before_third_drawall	dc.b	'COMBAT: before third drawall',10
g2combat_stage_before_third_drawall_len	equ	*-g2combat_stage_before_third_drawall
g2combat_stage_after_third_drawall	dc.b	'COMBAT: after third drawall',10
g2combat_stage_after_third_drawall_len	equ	*-g2combat_stage_after_third_drawall
g2combat_stage_before_fourth_drawall	dc.b	'COMBAT: before fourth drawall',10
g2combat_stage_before_fourth_drawall_len	equ	*-g2combat_stage_before_fourth_drawall
g2combat_stage_after_fourth_drawall	dc.b	'COMBAT: after fourth drawall',10
g2combat_stage_after_fourth_drawall_len	equ	*-g2combat_stage_after_fourth_drawall
g2combat_stage_before_fifth_drawall	dc.b	'COMBAT: before fifth drawall',10
g2combat_stage_before_fifth_drawall_len	equ	*-g2combat_stage_before_fifth_drawall
g2combat_stage_after_fifth_drawall	dc.b	'COMBAT: after fifth drawall',10
g2combat_stage_after_fifth_drawall_len	equ	*-g2combat_stage_after_fifth_drawall
g2combat_stage_before_sixth_drawall	dc.b	'COMBAT: before sixth drawall',10
g2combat_stage_before_sixth_drawall_len	equ	*-g2combat_stage_before_sixth_drawall
g2combat_stage_after_sixth_drawall	dc.b	'COMBAT: after sixth drawall',10
g2combat_stage_after_sixth_drawall_len	equ	*-g2combat_stage_after_sixth_drawall
g2combat_stage_before_seventh_drawall	dc.b	'COMBAT: before seventh drawall',10
g2combat_stage_before_seventh_drawall_len	equ	*-g2combat_stage_before_seventh_drawall
g2combat_stage_after_seventh_drawall	dc.b	'COMBAT: after seventh drawall',10
g2combat_stage_after_seventh_drawall_len	equ	*-g2combat_stage_after_seventh_drawall
g2combat_stage_before_eighth_drawall	dc.b	'COMBAT: before eighth drawall',10
g2combat_stage_before_eighth_drawall_len	equ	*-g2combat_stage_before_eighth_drawall
g2combat_stage_after_eighth_drawall	dc.b	'COMBAT: after eighth drawall',10
g2combat_stage_after_eighth_drawall_len	equ	*-g2combat_stage_after_eighth_drawall
g2combat_stage_post8_before_escape	dc.b	'COMBAT: post8 before escape check',10
g2combat_stage_post8_before_escape_len	equ	*-g2combat_stage_post8_before_escape
g2combat_stage_post8_escape_read	dc.b	'COMBAT: post8 escape read',10
g2combat_stage_post8_escape_read_len	equ	*-g2combat_stage_post8_escape_read
g2combat_stage_post8_before_dogamemenu	dc.b	'COMBAT: post8 before dogamemenu',10
g2combat_stage_post8_before_dogamemenu_len	equ	*-g2combat_stage_post8_before_dogamemenu
g2combat_stage_post8_after_dogamemenu	dc.b	'COMBAT: post8 after dogamemenu',10
g2combat_stage_post8_after_dogamemenu_len	equ	*-g2combat_stage_post8_after_dogamemenu
g2combat_stage_post8_after_escape_path	dc.b	'COMBAT: post8 after escape path',10
g2combat_stage_post8_after_escape_path_len	equ	*-g2combat_stage_post8_after_escape_path
g2combat_stage_post8_tele_hold_active	dc.b	'COMBAT: post8 teleport hold active',10
g2combat_stage_post8_tele_hold_active_len	equ	*-g2combat_stage_post8_tele_hold_active
g2combat_stage_post8_after_tele_check	dc.b	'COMBAT: post8 after teleport check',10
g2combat_stage_post8_after_tele_check_len	equ	*-g2combat_stage_post8_after_tele_check
g2combat_stage_post8_finished_read	dc.b	'COMBAT: post8 finished read',10
g2combat_stage_post8_finished_read_len	equ	*-g2combat_stage_post8_finished_read
g2combat_stage_post8_mainexit_begin	dc.b	'COMBAT: post8 mainexit begin',10
g2combat_stage_post8_mainexit_begin_len	equ	*-g2combat_stage_post8_mainexit_begin
g2combat_stage_finished_quit	dc.b	'COMBAT: finished=1 quit/exit',10
g2combat_stage_finished_quit_len	equ	*-g2combat_stage_finished_quit
g2combat_stage_finished_death	dc.b	'COMBAT: finished=2 death/allover',10
g2combat_stage_finished_death_len	equ	*-g2combat_stage_finished_death
g2combat_stage_finished_levelover	dc.b	'COMBAT: finished=3 levelover/exit',10
g2combat_stage_finished_levelover_len	equ	*-g2combat_stage_finished_levelover
g2combat_stage_finished_combatwon	dc.b	'COMBAT: finished=4 combatwon/player died',10
g2combat_stage_finished_combatwon_len	equ	*-g2combat_stage_finished_combatwon
g2combat_stage_finished_unknown	dc.b	'COMBAT: finished=unknown',10
g2combat_stage_finished_unknown_len	equ	*-g2combat_stage_finished_unknown
g2combat_stage_post8_loop_continues	dc.b	'COMBAT: post8 loop continues',10
g2combat_stage_post8_loop_continues_len	equ	*-g2combat_stage_post8_loop_continues
	even
g2combat_cx_drawlog	dc.w	0
g2combat_post8_log	dc.w	0
	even
