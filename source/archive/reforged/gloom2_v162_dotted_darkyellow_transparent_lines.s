...
; v162: draw two thin 80px dotted separators into the blank title-menu
; rows, 1px higher than v154: one between TWO PLAYER COMBAT and PLAYER 1,
; one between PLAYER 2 and REMOTE LINK OPTIONS.  Each dot is forced to the
; same dark-yellow colour as the earlier solid line, while every second
; pixel stays transparent so the title image remains visible in between.
...
.plane	move.l	a1,a2
	...
	beq.s	.plane0
	moveq	#9,d1
.clearbyte	andi.b	#$55,(a2)+	; clear every other pixel on higher planes
	dbf	d1,.clearbyte
	bra.s	.nextplane
.plane0	moveq	#9,d1
.setbyte	ori.b	#$AA,(a2)+	; set every other pixel on plane 0 -> dark yellow dots
	dbf	d1,.setbyte
.nextplane	addq	#1,d2
	dbf	d5,.plane
