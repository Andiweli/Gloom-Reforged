...
.gotdata	;
	...
	lea	combatmenu,a0
	bsr	qmenu
	bsr	g2zci_waitreleaseany	;c86zci: do not let title Fire auto-select Combat series
	;
.loop	bsr	selmenu
	cmp	#3,d0
	bcs	.play
...
; c86zci: debounce title/Combat carry-over input.
; The original menu code is fine when entered from a clean state, but after
; returning from gameplay the Fire/Return used to select TWO PLAYER COMBAT can
; still be seen by the following Combat-series/menu or map screen.  Keep these
; helpers small and independent: first variant only waits for release before a
; menu starts, second variant waits for release/new press/release on the Combat
; map/intermission picture.
g2zci_waitreleaseany	movem.l	d0-d7/a0-a6,-(a7)
g2zci_wr_loop	bsr	checkany
	bne.s	g2zci_wr_loop
	movem.l	(a7)+,d0-d7/a0-a6
	rts

g2zci_waitfreshany	movem.l	d0-d7/a0-a6,-(a7)
g2zci_wf_release1	bsr	checkany
	bne.s	g2zci_wf_release1
g2zci_wf_press	bsr	checkany
	beq.s	g2zci_wf_press
g2zci_wf_release2	bsr	checkany
	bne.s	g2zci_wf_release2
	movem.l	(a7)+,d0-d7/a0-a6
	rts
...
.loop	move.b	(a0)+,(a1)+
	...
	move.l	combatpal,a1
	jsr	showpic
	bsr	g2zci_waitfreshany	;c86zci: Combat map picture waits for a fresh Enter/Fire
