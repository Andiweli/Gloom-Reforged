...
; -----------------------------------------------------------------------------
; v137/v171 enemy blob shadows + floor-anchored multi-colour reflections.
; Blob shadows keep the confirmed hard-edged foot shadow path.  Reflections are
; palette/dither based: dark centre, lighter outside. Projectiles use a flatter
...
.up_min_ok
	...
	moveq	#16,d0
.pick_pulse
	; v170/v171: pickups do not follow their vertical bob.  They stay at a fixed
...
.rabs_ok	move	g2_shadow_rx(pc),d2
	cmp	d2,d1
	bgt	.rts
	; v171: projectiles get a flatter, more oval reflection directly under
	; the shot.  Stationary pickups keep the slightly thicker v170 floor oval.
	clr	g2_reflect_softedge
	tst	g2_reflect_pickup
	bne.s	.pickup_band
	moveq	#1,d5	;projectile outer edge: shallow oval
	moveq	#0,d6	;1 row
	move	d2,d3
	mulu	#3,d3
	lsr	#2,d3	;3/4 radius
	...
	bgt	.rsoft_outer
	moveq	#0,d5
	moveq	#1,d6	;2-row broad centre, no tall diamond peak
	move	d2,d3
	lsr	#2,d3	;inner 1/4 stays dense
	cmp	d3,d1
	bgt	.rsoft_mid
	moveq	#0,d5
	moveq	#1,d6
