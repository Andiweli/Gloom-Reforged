...
solidstrip	set	0
drawstrip2	; v193 chunky-safe transparent wall strip overlay with green glass tint
	...
	; texels keep the scene visible, but receive a darker green checker tint
	; for glass doors so the spaces read as translucent green instead of
	; flat bright stripes.
	movem.l	d0-d7/a0-a5,-(a7)
	move.l	vd_data(a4),d0
	beq	.rts
	move.l	d0,a0		; source texture column
	; v193: force a darker green glass tint for transparent texels.  The
...
.clipdone
	move.l	a1,d2
	and	#1,d2		; column parity for checkerboard glass tint
	move	hite(pc),d5
	sub	d2,d5
	ble	.rts
	cmp	d5,d4
...
.glass	tst	d6
	bmi.s	.next
	move	d4,d5
	eor	d2,d5		; row+column parity -> checkerboard instead of stripes
	btst	#0,d5
	bne.s	.next
	move.b	d6,(a1)
.next	addx.l	d3,d0
