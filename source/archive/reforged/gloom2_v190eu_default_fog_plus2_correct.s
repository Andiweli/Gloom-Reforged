...
g2deffogfar	equ	8<<grdshft	; v190eu: DEFAULT fog/sight cap +2 texture widths (was 6)
g2defshapez	equ	8<<grdshft	; v190eu: DEFAULT strips/objects match new fog cap
	;
exshft	equ	3
exone	equ	1<<exshft
exhalf	equ	exone>>1
...
initdarktable	;
	; v190eu: DEFAULT sight increase is done here, not only at the
	; renderer clip.  v190er-v190et changed the draw range, but the
	; darktable still reached full fog at six texture widths, so the
	; visible result looked unchanged.  Keep the v190eq Bayer renderer,
	; but move the DEFAULT fog cap from 6 to 8 texture widths.
	move	#maxz-1,d2
	move.l	sqr(pc),a0
...
.g2v190aq_farboost
	...
	;
	; v190eu: full far fog now starts at eight texture widths.
	; This is the actual visible DEFAULT sight increase; walls/floors
	; were already clipped later, but previously became fully fogged
	; at six texture widths.
	cmp	#g2deffogfar,d4
	blo.s	.g2v190aq_ramp
	moveq	#14,d3
