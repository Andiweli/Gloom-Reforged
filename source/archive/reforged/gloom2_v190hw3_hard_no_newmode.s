...
aga_	equ	-1
os_	equ	0	;v190hw3: force direct custom-chip/Copper display path, no Intuition/NewMode screen
g2_no_newmode_requester	equ	-1	;v190hw3 marker: hard NewMode requester bypass active
...
finitdisplay	push
	;
	; v190hw3: display was forced to direct custom-chip/Copper mode,
	; so cleanup must also avoid the legacy Intuition CloseScreen path.
	bra.w	.noos
	;
	tst	os
	beq.w	.noos
...
newscreen_v	dc	0	;viewmode
	dc	$14f	;type: CUSTOMSCREEN|CUSTOMBITMAP|SCREENQUIET if legacy OS path is ever re-enabled
	dc.l	0	;font
	dc.l	0	;title
	dc.l	0	;gadgets
...
initdisplay	;
	...
	move.l	d0,bitmaps2
	;
	; v190hw3: hard-bypass the legacy Intuition/OpenScreen path.
	; NewMode hooks OpenScreen/OpenWindow-style screen promotion; the direct
	; custom-chip/Copper path below gives it nothing to intercept.
	bra.w	.noos
	;
	tst	os
	beq.w	.noos
	...
	move	#$80,newscreen_v
	;
.agasc	bra.w	.noos	;v190hw3: never call Intuition OpenScreen/NewMode
	move.l	int(pc),a6
	lea	newscreen(pc),a0
	jsr	-198(a6)
	move.l	d0,a0
