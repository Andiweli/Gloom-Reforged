...
showstats	;a5=player, draw classic Gloom HUD after C2P
	; v190gd: The HUD from gloom.s is planar/blitter based, not chunky-shape
	; based.  Draw it onto drawbitmap after doc2p, otherwise the C2P conversion
	; overwrites it or the smallfont.bin planar shape data is interpreted wrong.
	movem.l	d0-d7/a0-a6,-(a7)
...
.g2hud_have_font
	...
	;
	; labels: same text/positions as gloom.s initstats
	moveq	#2,d7
	moveq	#2,d6
...
g2hud_blit_oldsmallfont	;a0=old smallfont shapetable, a1=bitmap, d0=X, d1=Y, d2=char
	; v190ge: exact old Black Magic smallfont.bin layout, based on uploaded gloom.s.
	; Gloom2 blit expects smallfont2/bigfont2 metadata (+10 image start / plane count),
	; while smallfont.bin uses the older +8 image layout.  Use this only for the top HUD.
	...
	add	d2,a1
	move.l	a0,a2
	add.l	(a0),a2		; cookie/mask
	lea	8(a0),a3		; old smallfont image data starts at +8
	addq	#4,a0
	and	#15,d0
	ror	#4,d0
	btst	#6,$dff002
