...
showstats	;a5=player
	; v190ga: top-left HUD follows gloom.s layout/logic.
	; The lower Gloom2/smallfont2 status panel remains completely unused.
	movem.l	d0-d7/a0-a4,-(a7)
	move.l	smallfont_(pc),d0
	bne.s	.g2hud_have_font
	move.l	panel(pc),d0	; last-resort fallback only; normal HUD uses smallfont_.
	beq.w	.g2hud_done
.g2hud_have_font
	; labels: same positions/text as gloom.s initstats
	moveq	#2,d7
	moveq	#2,d6
...
.g2hud_hploop
	...
	bra.s	.g2hud_hploop
.g2hud_hpskip
	move.l	smallfont_(pc),a0
	bne.s	.g2hud_hp_font_ok
	move.l	panel(pc),a0
	beq.s	.g2hud_hpdone
.g2hud_hp_font_ok
	move	d5,d0
...
g2draw_hud_text	;a4=zero-terminated text, d7=x, d6=y
...
g2draw_hud_shape_top	;a0=smallfont shapetable, d0=x, d1=y, d2=shape# absolute top HUD draw
	; v190gb: smallfont.bin HUD pixels can use dark/source-local colour indices
	; that disappear in the active Gloom2 palette.  Keep the smallfont shape
	; graphics, but draw every non-transparent HUD pixel with a safe bright
	; remapped colour so HEALTH/WEAPON are visible in all profiles.
	movem.l	d0-d7/a0-a4,-(a7)
	tst.l	a0
	beq.w	.g2hds_done
	cmp	#0,d0
	...
	beq.w	.g2hds_done
	cmp.l	#32768,d3	; guard against missing/invalid smallfont shape entries
	bhi.w	.g2hds_done
	add.l	d3,a0
	addq	#4,a0
	movem	(a0)+,d2-d3	;width,height
	...
	lea	0(a2,d0*4),a2
	move.l	palettes(pc),a4
	moveq	#15,d6		; stable visible HUD colour, palette-remapped
	move.b	0(a4,d6),d6
	moveq	#0,d0
.g2hds_xloop
	move	d3,d7
