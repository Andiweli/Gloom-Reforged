...
; -----------------------------------------------------------------------------
; v134 enemy blob shadows + floor-anchored reflections.
; Blob shadows keep the confirmed v132/v133 path.  Reflections are now kept out
; of the HUD/gun area, use a darker edge-like colour, and pickups pulse their
; floor ring while the ring stays anchored to the ground.
; -----------------------------------------------------------------------------
...
.up_min_ok
	...
	bra	.rx_ok
.bullet_size
	lsr	#2,d0	;projectile reflection: smaller
	cmp	#2,d0
	bge	.bul_min_ok
	moveq	#2,d0
.bul_min_ok
	...
	moveq	#10,d0
.rx_ok
	; v134: pickups pulse their floor ring with the bobbing phase, but the
	; Y anchor remains floor-based below.  Projectiles keep a fixed small size.
	cmp.l	#weaponlogic,d5
	bne	.store_rx
...
.store_rx	move	d0,g2_shadow_rx
	; v134: reflection belongs on the floor, not glued to the bottom of
	; the floating sprite.  Approximate the floor drop from object Y and
	; current projected Z.  Pickups get more allowed compensation so the
	; floor ring does not bob with the sprite.
	clr	g2_shadow_yoff
	move	ob_y(a2),d0
	neg	d0
	ext.l	d0
...
g2_reflection_colour
	; return current-palette colour index in d0, based on bullet shape or pickup weapon
	movem.l	d1-d2/a0,-(a7)
	moveq	#0,d2
...
.got_weapon
	...
	move	0(a0,d2*2),d1
	moveq	#2,d0	;v133 fallback: dark palette entry if remap misses
	move.l	planar_remap(pc),a0
	tst.l	a0
	beq	.done
	move.b	0(a0,d1.w),d0
	bne	.done
	moveq	#15,d0
.done	movem.l	(a7)+,d1-d2/a0
	rts

; RGB12 colours, remapped to the active Gloom palette at runtime.
; weapon 1..5 / projectile 1..5: yellow, green, red, cyan, magenta.
g2_reflect_rgb
	; v136: use the second-brighter colour family of projectile/upgrade art,
	; not the darkest rim and not the full bright core.
...
.rdraw	move	g2_shadow_col(pc),d4
	; v135 soft reflection edge: leave floor pixels in a small checker
	; pattern on the outer/mid bands.  This gives a feathered edge without
	; needing true alpha blending in the 8-bit chunky buffer.
