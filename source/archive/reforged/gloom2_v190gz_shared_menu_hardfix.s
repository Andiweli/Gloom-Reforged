...
.redrawmenu
	...
	; v190gr: all local profiles use the same Gloom Deluxe title menu path.
...
.notlinked
	...
	and	#$00ff,d0
	; v190gt: one local title-menu mapping only: all profiles use startmenu.
	; Keep the old compat/G3ZM handlers below unreachable for label stability.
	tst	d7
...
.g2v190f_shifted_newgame
	...
	bra	.newgame
.g2v190gq_notlinked
	; v190gz: fallback handler now uses the exact 13-row Gloom Deluxe layout.
	tst	d7
	beq.s	.g2v190gq_fire
	cmp	#0,d0
	beq	.g2v190f_level_lrsel
...
.g2v190gq_mode_copy
	...
	move	mode(pc),d0	; v190gz: keep compat duplicate menu mode text in sync too
	lea	modes,a0
	move.l	0(a0,d0*4),a0
	lea	modetxtc,a1
.g2v190gz_mode_copy_c
	move.b	(a0)+,(a1)+
	bne.s	.g2v190gz_mode_copy_c
	;
	bra	.sel
	;	;
.about	;about text...
...
startmenu_g3zm	dc.b	13		; v190gz: exact Gloom Deluxe menu layout for Gloom3/ZM fallback path
	dc.b	'START LEVEL '
g2v190gq_level_text	dc.b	'MAP1.1',0	; 0, synced with g2v190f_level_text
	...
	dc.b	'TWO PLAYER GAME',0	; 3
	dc.b	'TWO PLAYER COMBAT',0	; 4
	dc.b	0			; 5 separator above PLAYER 1
	dc.b	'PLAYER 1 '
p1ctypegq
	ifne	cd32
	dc.b	'CD32 PAD 1',0	; 6, fixed 10-char field
...
p2ctypegq
	...
	dc.b	'JOYSTICK 1 ',0		; 7, fixed 10-char field
	endc
	dc.b	0			; 8 separator above REMOTE LINK OPTIONS
	dc.b	'REMOTE LINK OPTIONS',0	; 9
modetxtgq	dc.b	'MEATY VIOLENCE MODE',0	; 10
	dc.b	'ABOUT GLOOM',0		; 11
	dc.b	'EXIT GAME',0,0		; 12
	even

compat_startmenu	dc.b	13		; v190gz: exact Gloom Deluxe menu layout for compatibility fallback path
	dc.b	'START LEVEL '
g2v190compat_level_text	dc.b	'MAP1.1',0	; 0, synced with g2v190f_level_text
	dc.b	0			; 1 spacer below level selector
	dc.b	'ONE PLAYER GAME',0	; 2
	dc.b	'TWO PLAYER GAME',0	; 3
	dc.b	'TWO PLAYER COMBAT',0	; 4
...
.g2v190gq_copy2
	...
	clr.b	(a1)
.g2v190gq_copy2_done
	lea	g2v190f_level_text(pc),a0	; v190gz: keep compat duplicate START LEVEL in sync too
	lea	g2v190compat_level_text(pc),a1
	moveq	#5,d1
.g2v190gz_copy3
	move.b	(a0)+,(a1)+
	beq.s	.g2v190gz_copy3_done
	dbf	d1,.g2v190gz_copy3
	clr.b	(a1)
.g2v190gz_copy3_done
	movem.l	(a7)+,d0-d2/a0-a1
	rts
...
initmenu2	;
	...
	bsr	initfontpal
	; v190gt: absolute single local title-menu safety net.
	; Even if an old profile branch hands initmenu the legacy G3/ZM or
	; compatibility menu address, draw the same Gloom Deluxe startmenu.
	cmp.l	#startmenu_g3zm,a4
	beq.s	.g2v190gt_force_startmenu
	cmp.l	#compat_startmenu,a4
	bne.s	.g2v190gt_menu_ok
.g2v190gt_force_startmenu
	lea	startmenu,a4
.g2v190gt_menu_ok
	;
	...
	cmp.l	#startmenu_g3zm+1,a4	; v190gz: if legacy pointer reaches here, draw exactly like Deluxe
	beq.s	.titlemenu_main
	cmp.l	#compat_startmenu+1,a4	; v190gz: if compat pointer reaches here, draw exactly like Deluxe
	beq.s	.titlemenu_main
...
.titlemenu_main	move	#-1,g2v154_titlemenu_lines
	...
	bra.s	.titlemenu_y_done
.titlemenu_g3zm	move	#-1,g2v154_titlemenu_lines	; v190gr legacy only: match Gloom Deluxe layout
	sub	fonth(pc),d6
	sub	fonth(pc),d6
	sub	fonth(pc),d6
	bra.s	.titlemenu_y_done
