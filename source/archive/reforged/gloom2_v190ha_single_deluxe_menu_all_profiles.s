...
.redrawmenu
	...
	; v190ha: every local game profile uses the exact Gloom Deluxe title menu.
	; Same START LEVEL row, separators, vertical position and row indices.
...
.notlinked
	...
	and	#$00ff,d0
	; v190ha: every local profile now returns the 13-row startmenu indices.
	; Do not jump into the old compatibility row mapping here.
	tst	d7
...
.vilesel	addq	#1,mode
	...
	lea	modes,a0
	move.l	0(a0,d0*4),a0
	; v190ha: all local profiles use the shared startmenu/modetxt field.
	lea	modetxt,a1
.g2v190co_mode_copy
	move.b	(a0)+,(a1)+
