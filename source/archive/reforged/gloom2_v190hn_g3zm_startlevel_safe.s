...
.redrawmenu
	...
	cmp	#2,g2_game_profile	; v190hn: Zombie Massacre gets START LEVEL compat menu
	beq.s	.use_g3zm_startmenu
	cmp	#3,g2_game_profile	; v190hn: Gloom3 gets START LEVEL compat menu
	beq.s	.use_g3zm_startmenu
	tst	g2_game_profile	; v190co: only true Gloom Deluxe keeps START LEVEL
	beq.s	.use_gloom2_startmenu
	lea	compat_startmenu,a4	; Classic Gloom: keep v190hm compatibility menu unchanged
	jsr	initmenu
	clr	curropt
	bra.s	.sel
.use_g3zm_startmenu
	lea	compat_startmenu_g3zm,a4	; v190hn: Gloom3/ZM START LEVEL without moving existing rows
	jsr	initmenu
	move	#2,curropt	; default title selection remains ONE PLAYER GAME
	bra.s	.sel
...
.g2v190f_shifted_newgame
	...
	bra	.newgame
.g2compat_notlinked
	cmp	#13,numopts	; v190hn: Gloom3/ZM START LEVEL compat menu has full row map
	beq	.g2compat13_notlinked
	tst	d7
	beq.s	.g2compat_fire
...
.g2compat_fire
	...
	beq	.exitgloom
	bra	.sel
.g2compat13_notlinked
	tst	d7
	beq.s	.g2compat13_fire
	cmp	#0,d0		; START LEVEL left/right
	beq	.g2v190f_level_lrsel
	cmp	#6,d0		; PLAYER 1 row
	beq	.g2compat_p1lrsel
	cmp	#7,d0		; PLAYER 2 row
	beq	.g2compat_p2lrsel
	bra	.sel
.g2compat13_fire
	cmp	#0,d0		; START LEVEL launch
	beq	.g2v190f_level_start
	cmp	#2,d0
	bcs	.sel		; spacer below START LEVEL
	cmp	#5,d0		; rows 2/3/4 -> gametype 0/1/2
	bcs	.g2compat13_newgame_shifted
	cmp	#6,d0
	beq	.g2compat_p1sel
	cmp	#7,d0
	beq	.g2compat_p2sel
...
.g2compat_p1_ptr
	cmp	#13,numopts
	beq.s	.g2compat_p1_ptr_g3zm
	lea	p1ctypec(pc),a1
	rts
.g2compat_p1_ptr_g3zm
	lea	p1ctypec_g3zm(pc),a1
	rts
.g2compat_p2_ptr
	cmp	#13,numopts
	beq.s	.g2compat_p2_ptr_g3zm
	lea	p2ctypec(pc),a1
	rts
.g2compat_p2_ptr_g3zm
	lea	p2ctypec_g3zm(pc),a1
	rts
.g2v190f_level_lrsel
...
.g2v190co_mode_copy
	...
	move	mode(pc),d0	; v190hn: keep G3/ZM compat menu violence text synced too
	lea	modes,a0
	move.l	0(a0,d0*4),a0
	lea	modetxtc_g3zm,a1
.g2v190hn_mode_copy2
	move.b	(a0)+,(a1)+
	bne.s	.g2v190hn_mode_copy2
	;
	bra	.sel
	;	;
.about	;about text...
...
compat_startmenu_g3zm	dc.b	13	; v190hn: Gloom3/ZM menu with START LEVEL, old rows stay in place
	dc.b	'START LEVEL '
g2v190hn_level_text	dc.b	'MAP1.1',0	;0 synced from dynamic level selector
	dc.b	0			;1 spacer below START LEVEL
	dc.b	'ONE PLAYER GAME',0	;2
	dc.b	'TWO PLAYER GAME',0	;3
	dc.b	'TWO PLAYER COMBAT',0	;4
	dc.b	0			;5 spacer/dotted line above PLAYER rows
	dc.b	'PLAYER 1 '
p1ctypec_g3zm
	ifne	cd32
	dc.b	'CD32 PAD 1',0	;6, fixed 10-char field
...
p2ctypec_g3zm
	...
	dc.b	'REMOTE LINK OPTIONS',0	;9
modetxtc_g3zm	dc.b	'MEATY VIOLENCE MODE',0	;10
	dc.b	'ABOUT GLOOM',0		;11
	dc.b	'EXIT GLOOM',0		;12
	even
...
.g2v190i_copy_done
	clr.b	(a1)
	lea	g2v190f_level_text(pc),a0	; v190hn: keep Gloom3/ZM START LEVEL text in sync
	lea	g2v190hn_level_text(pc),a1
	moveq	#5,d1
.g2v190hn_copy
	move.b	(a0)+,(a1)+
	beq.s	.g2v190hn_copy_done
	dbf	d1,.g2v190hn_copy
	clr.b	(a1)
.g2v190hn_copy_done
	movem.l	(a7)+,d0-d2/a0-a1
	rts
...
g2v190i_levelselect_loadscripts
	...
	clr	g2v190f_level_index
	bsr	permit
	; v190hn: profile-specific START LEVEL source.
	; Gloom/Gloom Deluxe/Gloom3 use misc/script; Zombie Massacre uses stuf/stages.
	cmp	#2,g2_game_profile
	beq.s	.g2v190hn_try_zm_stages
...
g2v190i_try_script
	; v190hn: use normal loadfile so CrM2-compressed script/stages files
	; are decrunched before parsing, matching the real game script path.
	movem.l	d0-d1/a0-a1,-(a7)
	moveq	#1,d1
	jsr	loadfile
	tst.l	d0
...
.titlemenu_main	move	#-1,g2v154_titlemenu_lines
	...
	bra.s	.titlemenu_y_done
.titlemenu_compat_level	move	#-1,g2v154_titlemenu_lines	; v190hn: 13-row G3/ZM menu uses Deluxe separator rows
	sub	fonth(pc),d6	; keep old ONE PLAYER row while adding START LEVEL above it
	sub	fonth(pc),d6
	sub	fonth(pc),d6
	bra.s	.titlemenu_y_done
