...
g2deffogfar	equ	8<<grdshft	; v190ey: shared darktable cap used by the confirmed v190ew 12-width fog
g2advviewfar	equ	12<<grdshft	; v190ez: ADVANCED uses the confirmed v190ew 12-texture-width view/fog range
g2advshapez	equ	12<<grdshft	; v190ez: ADVANCED strips/objects match the 12-width range
g2maxviewfar	equ	18<<grdshft	; v190ez: MAXIMUM uses same smooth path stretched to 18 texture widths
g2maxshapez	equ	18<<grdshft	; v190ez: MAXIMUM strips/objects match the 18-width range
	;
exshft	equ	3
exone	equ	1<<exshft
exhalf	equ	exone>>1
...
exittoos	jsr	inputoff	; v34: restore ciaa/rawkey vectors before OS exit/closewindow
	jsr	g2v190ep_stop_all_sfx	; v190ep: hard-stop Paula SFX before saving/freeing on exit
...
g2playeventsfx_force	; v190eg: hard-start critical local event SFX on reserved channel
	; a0=sfx file, d0=volume, d1=priority
	tst.l	a0
	beq.s	.rts
	movem.l	d2/a1-a2,-(a7)
	lea	sfx3(pc),a1	; keep one Paula channel for door/switch/teleport certainty
	move	d0,fx_vol(a1)
	move	d1,fx_priority(a1)
	bsr	sfxoff
	bsr	playsfxnow
...
trainer_toggle_visibility
	...
	cmp	#1,d0
	beq.s	.maximum
	move	#0,g2_visibility
	bra.s	.done
.advanced	move	#1,g2_visibility
	bra.s	.done
.maximum	move	#2,g2_visibility
...
.vloop	;
	...
	divs	d7,d6	;d6.w = Z
	move	g2_visibility,d5
	cmp	#2,d5
	bge	.g2v190ez_flat_max
	cmp	#1,d5
	beq	.g2v190ey_flat_adv
	cmp	#maxz,d6
	bcc	.rts
	move	d6,d5
...
.g2v190ey_flat_adv
	cmp	#g2advviewfar,d6	; v190ey: ADVANCED = confirmed smooth v190ew DEFAULT-12 range
	bcc	.rts
	move	d6,d5
	move	d6,d3
	lsr	#1,d5	; 12 actual widths map into the 8-width darktable cap
	move	d3,d0
	lsr	#3,d0
	add	d0,d5
	move	d3,d0
	...
	add	d0,d5
	cmp	#maxz-1,d5
	bls.s	.g2v190ey_flat_adv_scale_ok
	move	#maxz-1,d5
.g2v190ey_flat_adv_scale_ok
	bra.s	.g2v190dw_flat_shade
.g2v190ez_flat_max
	cmp	#g2maxviewfar,d6	; v190ez: MAXIMUM = same smooth path stretched to 18 widths
	bcc	.rts
	move	d6,d5
	move	d6,d3
	lsr	#1,d5
	...
	add	d0,d5
	cmp	#maxz-1,d5
	bls.s	.g2v190ez_flat_max_scale_ok
	move	#maxz-1,d5
.g2v190ez_flat_max_scale_ok
.g2v190dw_flat_shade
	;
...
doorsfxflag	dc	0
doorsfxplayed	dc	0	; v190ex: one guaranteed door/switch SFX per event, without end-of-event double play
...
exec_loop	move	(a6)+,d0
	...
	warn	#$f0f
	;
.rts	tst	doorsfxflag	; v190eg: restore original delayed door/switch event SFX path
	beq.s	.nodoor
	tst	doorsfxplayed	; v190ex: direct door sound already fired during this event
	bne.s	.nodoor
	clr	doorsfxflag
	movem.l	d0-d1/a0,-(a7)
	move.l	doorsfx(pc),a0
	moveq	#64,d0
	moveq	#100,d1	; v190ex: critical local door/switch event, above ordinary SFX
	jsr	g2playeventsfx_force
	st	doorsfxplayed
	movem.l	(a7)+,d0-d1/a0
.nodoor	rts
...
exec_changetxt	;
	...
	move	d0,changedtxt
	move.b	d0,zo_t(a1)
	st	doorsfxflag	; v190eg: switch/texture events use delayed guaranteed event SFX
	jsr	g2playdoorsfx_once	; v190ex: make switch/texture SFX audible immediately too
	bra	exec_loop

g2playdoorsfx_once	; v190ex: force one local door/switch SFX immediately, no ordinary SFX loss
	tst.b	doorsfxplayed
	bne.s	.rts
	st	doorsfxplayed
	st	doorsfxflag
	...
	beq.s	.done
	moveq	#64,d0
	moveq	#100,d1	; v190ex: nearby door/switch events must win over ordinary SFX
	jsr	g2playeventsfx_force
.done	movem.l	(a7)+,d0-d1/a0
.rts	rts

exec_opendoor	st	doorsfxflag	; v190eg: restore original flag path for reliable door sound
	...
	jsr	g2playdoorsfx_once	; v190ex: start audible door SFX at the actual trigger point
	bra	exec_loop
...
dotelesfx	move.l	telesfx(pc),a0
	moveq	#64,d0
	moveq	#120,d1	; v190eg: teleport is a forced local event SFX
	jsr	g2playeventsfx_force
	rts
...
.ok	st	doorsfxflag	; v190eg: rotating/morphing doors trigger delayed event sound
	jsr	g2playdoorsfx_once	; v190ex: make rotating/morphing door SFX audible at trigger point
...
.blok	sub	camy(pc),d1
	...
	move	g2_visibility,d5
	cmp	#2,d5
	bge	.g2v190ez_blood_max
	cmp	#1,d5
	beq	.g2v190dw_blood_far
	cmp	#maxz,d2	; v190ey: DEFAULT original cull
	bcc	.loop
	bra.s	.g2v190dw_blood_zok
.g2v190dw_blood_far
	cmp	#g2advviewfar,d2	; v190ey: ADVANCED uses confirmed 12 texture widths
	bcc	.loop
	bra.s	.g2v190dw_blood_zok
.g2v190ez_blood_max
	cmp	#g2maxviewfar,d2	; v190ez: MAXIMUM uses 18 texture widths
	bcc	.loop
.g2v190dw_blood_zok
	;
	ext.l	d0
...
.pix	;
	...
	move	g2_visibility,d6
	cmp	#2,d6
	bge	.g2v190ez_blood_shade_max
	cmp	#1,d6
	beq	.g2v190ey_blood_shade_adv
	cmp	#(4<<grdshft),d3
...
drawshape_q	;
	...
	move	g2_visibility,d6
	cmp	#2,d6
	bge	.g2v190ez_shape_max
	cmp	#1,d6
	beq	.g2v190dw_shape_far
	cmp	#maxz,d2	; v190ey: DEFAULT original object/sprite distance
	bcc	.rts
	bra.s	.g2v190dw_shape_zok
.g2v190dw_shape_far
	cmp	#g2advshapez,d2	; v190ey: ADVANCED objects/sprites = 12 texture widths
	bcc	.rts
	bra.s	.g2v190dw_shape_zok
.g2v190ez_shape_max
	cmp	#g2maxshapez,d2	; v190ez: MAXIMUM objects/sprites = 18 texture widths
	bcc	.rts
.g2v190dw_shape_zok
	;
	muls	cm1(pc),d0
...
dothezone2	;
	...
	move	g2_visibility,d5
	cmp	#1,d5
	bgt	.g2v190ez_zone2_max
	tst	d5
	ble.s	.g2v190dw_zone2_view_ok
	move	#g2advviewfar,d4	; v190ey: ADVANCED zone cull = 12 texture widths
	bra.s	.g2v190dw_zone2_view_ok
.g2v190ez_zone2_max
	move	#g2maxviewfar,d4	; v190ez: MAXIMUM zone cull = 18 texture widths
...
.zok	;
	...
	move	g2_visibility,d5
	cmp	#1,d5
	bgt	.g2v190ez_wall_view_max
	tst	d5
	ble.s	.g2v190dw_wall_view_ok
	move	#g2advviewfar,d4	; v190ey: ADVANCED wall/zone cull = 12 texture widths
	bra.s	.g2v190dw_wall_view_ok
.g2v190ez_wall_view_max
	move	#g2maxviewfar,d4	; v190ez: MAXIMUM wall/zone cull = 18 texture widths
...
.noov	lsr	#1,d0	;fraction -> unsigned
	...
	swap	d2
	;
	; v190fb: MAXIMUM extends past the signed 16-bit fixed-Z edge.
	; Treat high-bit Z as far distance only in MAXIMUM, then keep the
	; normal unsigned upper clip below.  DEFAULT/ADVANCED keep original
	; near/behind rejection.
	tst	d2
	bpl.s	.g2v190fb_cast_z_nonneg
	move	g2_visibility,d5
	cmp	#2,d5
	beq.s	.g2v190fb_cast_z_min_ok
	bra	.loop2
.g2v190fb_cast_z_nonneg
	cmp	#exone,d2
	blt	.loop2
.g2v190fb_cast_z_min_ok
	move	#maxz<<exshft,d6
	move	g2_visibility,d5
	cmp	#1,d5
	bgt	.g2v190ez_cast_view_max
	tst	d5
	ble.s	.g2v190dw_cast_view_ok
	move	#g2advviewfar<<exshft,d6	; v190ey: ADVANCED wall cast = 12 texture widths
	bra.s	.g2v190dw_cast_view_ok
.g2v190ez_cast_view_max
	move	#g2maxviewfar<<exshft,d6	; v190ez: MAXIMUM wall cast = 18 texture widths
...
.scdone	move.l	d0,d1
	...
	; stable 4x4 Bayer mask from 8..12 texture widths, so geometry
	...
	move	d2,d6
	sub	#(8<<grdshft),d6
	lsr	#6,d6	; v190ex: 0..15 over the 8..12 advanced far band
	cmp	#15,d6
...
.hiteok	;
	...
	move	vd_z(a0),d6
	move	d7,-(a7)
	move	g2_visibility,d7
	cmp	#1,d7
	bgt	.g2v190ez_wallblend_max
	tst	d7
...
.g2v190ez_wallblend_max
	...
	add	d7,d6
	cmp	#maxz-1,d6
	bls.s	.g2v190ez_wallblend_scale_ok
	move	#maxz-1,d6
.g2v190ez_wallblend_scale_ok
	move	(a7)+,d7
...
.drawloop	move.l	(a6),d0
	...
	move	g2_visibility,d5
	cmp	#1,d5
	bgt	.g2v190ez_shapelist_max
	tst	d5
	ble.s	.g2v190dw_shapelist_view_ok
	move	#g2advshapez,d0	; v190ey: ADVANCED strips/objects = confirmed 12 texture widths
	bra.s	.g2v190dw_shapelist_view_ok
.g2v190ez_shapelist_max
	move	#g2maxshapez,d0	; v190ez: MAXIMUM strips/objects = 18 texture widths
.g2v190dw_shapelist_view_ok
	...
	move	d1,d6
	sub	#(8<<grdshft),d6
	lsr	#6,d6	; v190ex: 8..12 texture-width far band
	cmp	#15,d6
...
.g2v190ez_obj_shade_max
	...
	add	d6,d7
	cmp	#maxz-1,d7
	bls.s	.g2v190ez_obj_shade_restore_max
	move	#maxz-1,d7
.g2v190ez_obj_shade_restore_max
	move	(a7)+,d6
...
g2_visibility	dc	0	;v190fb 0=DEFAULT(6), 1=ADVANCED(12), 2=MAXIMUM(18); saved in gloom.cfg
...
initdarktable	;
	; v190ey: shared darktable from the confirmed v190ew DEFAULT-12 test.
	; It reaches full fog at its 8-width table end; DEFAULT scales back
	; into the original short range, while ADVANCED scales 12 actual widths and MAXIMUM scales 18 actual widths
...
.no_onehit_old_cfg	move	#0,g2_visibility	; v190fb: old cfg defaults to DEFAULT visibility
	cmp	#3,d7
...
.refl_ok	move	g2_visibility,d0
	...
	move	#0,g2_visibility
	bra	.visibility_ok
.visibility_adv	move	#1,g2_visibility
	bra	.visibility_ok
.visibility_max	move	#2,g2_visibility
.visibility_ok
