...
solidstrip	set	0
drawstrip2	; v196 chunky-safe transparent wall strip overlay with true 50% green glass
	...
	; texels now blend the already-drawn background pixel 50/50 with a dark
	; green glass colour, matching the intended translucent look instead of
	; using stripes or a checkerboard fill.
	movem.l	d0-d7/a0-a5,-(a7)
	move.l	vd_data(a4),d0
	beq	.rts
	move.l	d0,a0		; source texture column
	move	#$060,d6		; dark green RGB12 glass tint for 50% transparency blend
	move.l	chunkymod(pc),d7
	move	vd_y(a4),d2
...
.hiteok
	...
	move	vd_pal(a4),d5
	move.l	0(a2,d5*4),a5
	move.l	map_rgbs(pc),a3	; index -> RGB12
	move.l	planar_remap(pc),a2	; RGB12 -> current palette index
.loop
	move.b	0(a0,d0),d5
	beq.s	.glass
	move.b	0(a5,d5),(a1)
	bra.s	.next
.glass	moveq	#0,d5
	move.b	(a1),d5		; current background palette index
	add		d5,d5
	move	0(a3,d5.w),d5	; current background RGB12
	move		d5,d1
	lsr		#8,d1
	and		#$000f,d1	; R
	lsr		#1,d1		; (R + 0) / 2
	...
	lsr		#4,d2
	and		#$000f,d2	; G
	addq		#6,d2		; + dark-green tint component
	lsr		#1,d2		; (G + 6) / 2
	and		#$000f,d5	; B
	lsr		#1,d5		; (B + 0) / 2
	lsl		#8,d1
