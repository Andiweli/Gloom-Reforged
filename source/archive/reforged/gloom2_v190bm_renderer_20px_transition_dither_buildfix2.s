...
.hiteok	;
	...
	swap	d3
	subq	#1,d4
	move	d6,d5	;prebuilt wall blend palette 0...15
	move.l	0(a2,d5*4),a4
	sub	d3,d2
	add.l	d3,d2
	;
...
; Build a broader 20px shade-transition dither map for solid wall columns.
; Base palette comes from vd_pal. Around a direct 1-step shade boundary,
; 20 columns on each side are progressively mixed with the neighbouring shade.
; This is intentionally broad and visible, matching the user's request for
; a large soft transition instead of a tiny subtle checker.
g2build_wallblend
	movem.l	d0-d7/a0-a6,-(a7)
	move.l	vertdraws(pc),a0
	lea	g2_wallblend(pc),a1
	move	width(pc),d7
	ble.w	.g2bwb_done
	subq	#1,d7
.g2bwb_init
	tst.l	vd_data(a0)
	beq.s	.g2bwb_init_empty
	move	vd_pal(a0),d0
	move.b	d0,(a1)+
	bra.s	.g2bwb_init_next
.g2bwb_init_empty
	move.b	#$ff,(a1)+
.g2bwb_init_next
	lea	vd_size(a0),a0
	...
	blt.w	.g2bwb_done
	move.l	vertdraws(pc),a0
	moveq	#0,d6	; boundary left-column x
.g2bwb_boundary
	tst.l	vd_data(a0)
	beq.s	.g2bwb_bnext
...
; d0 = left palette, d1 = right palette, d3 = boundary x (between x and x+1)
g2blend_boundary20
	movem.l	d0-d7/a0-a2,-(a7)
	lea	g2_wallblend(pc),a0
	move	d3,d4
	sub	#19,d4	; left start x
	bpl.s	.g2bb_left_ok
	moveq	#0,d4
.g2bb_left_ok
	move	d4,d5	; current x left side
.g2bb_left_loop
	cmp	d3,d5
