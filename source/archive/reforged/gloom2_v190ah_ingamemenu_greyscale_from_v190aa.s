...
dogamemenu	;
	...
	move	#$8020,$dff09a
	;
	jsr	g2v190ah_ingamemenu_grey_backdrop	; v190ah: draw grey game backdrop before ESC menu
...
; v190ah: In-game menu greyscale backdrop.
; Build a private grey palette in reserved high colour slots and render the
; current scene through a temporary object palette.  This keeps bigfont2 yellow
; in palette entries 0..3 without turning wall pixels yellow.
g2v190ah_ingamemenu_grey_backdrop
	movem.l	d0-d7/a0-a6,-(a7)
	jsr	g2v190ah_build_menu_palette
	jsr	g2v190ah_make_grey_tables
	clr.l	g2v190ah_savedpal1
	clr.l	g2v190ah_savedpal2
	move.l	player1(pc),d0
	beq.s	.draw
	move.l	d0,a5
	move.l	ob_palette(a5),g2v190ah_savedpal1
	move.l	#g2v190ah_palettesw,ob_palette(a5)
	move.l	player2(pc),d0
	beq.s	.draw
	tst	gametype
	beq.s	.draw
	move.l	d0,a5
	move.l	ob_palette(a5),g2v190ah_savedpal2
	move.l	#g2v190ah_palettesw,ob_palette(a5)
.draw	jsr	drawall_
	jsr	drawall_
	move.l	player1(pc),d0
	beq.s	.restore2
	move.l	d0,a5
	move.l	g2v190ah_savedpal1,d0
	beq.s	.restore2
	move.l	d0,ob_palette(a5)
.restore2
	move.l	g2v190ah_savedpal2,d0
	beq.s	.done
...
; Copy the current game palette, replace only a private 16-colour range with
; greys, and poke it directly via pokepal2.  lastpal is deliberately untouched,
; so finitmenu/pokelastpal restores the normal game palette afterwards.
g2v190ah_build_menu_palette
	movem.l	d0-d7/a0-a2,-(a7)
	move.l	lastpal,d0
	bne.s	.havepal
	move.l	planar_palette,d0
.havepal	move.l	d0,a0
	lea	g2v190ah_menu_palette,a1
	move	colours(pc),d6
	tst	aga
	beq.s	.copy_count_ok
	lsl	#1,d6		; AGA palette has high+low words per colour
.copy_count_ok
	subq	#1,d6
.copy_loop
	move	(a0)+,(a1)+
	dbf	d6,.copy_loop
	moveq	#16,d5		; ECS fallback: colours 16..31
	move	colours(pc),d0
	cmp	#64,d0
	bcs.s	.base_ok
	move	#240,d5		; AGA: colours 240..255
.base_ok
	lea	g2v190ah_menu_palette,a1
	moveq	#0,d6
.grey_loop
	...
	beq.s	.set_ecs
	move	d5,d2
	lsl	#2,d2		; AGA word-pair offset as byte offset
	lea	0(a1,d2.w),a2
	move	d0,(a2)+
	clr	(a2)
	bra.s	.next_grey
...
; Build 16 shade tables which map the original RGBs to the private grey range.
; The tables are used as a temporary ob_palette for players while drawing the
; ESC-menu background.
g2v190ah_make_grey_tables
	movem.l	d0-d7/a0-a4,-(a7)
	lea	g2v190ah_palettesw,a2
	lea	g2v190ah_paldat,a3
	moveq	#15,d7
.ptr_loop
	move.l	a3,(a2)+
	adda.l	#256,a3
...
refresh	jsr	dispoff
	jsr	finitmenu
	tst	game_menu_active	; v190ah: keep ESC menu backdrop greyscale after option redraws
	beq.s	.normal_redraw
	jsr	g2v190ah_ingamemenu_grey_backdrop
	bra.s	.menu_redrawn
.normal_redraw	jsr	predrawall
.menu_redrawn	lea	gamemenu,a4
	jsr	initmenu2
	jsr	dispon
	rts
...
g2v190i_level_offsets ds.l g2v190i_max_levels	; v190r: script command offsets for START LEVEL chain mode
	...
	even
; v190ah storage for ESC-menu greyscale backdrop.
g2v190ah_savedpal1	dc.l	0
g2v190ah_savedpal2	dc.l	0
g2v190ah_palettesw	ds.l	16
g2v190ah_paldat	ds.b	4096
g2v190ah_menu_palette	ds.w	512
