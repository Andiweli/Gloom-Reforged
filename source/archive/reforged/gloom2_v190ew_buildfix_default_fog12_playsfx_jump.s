...
g2deffogfar	equ	8<<grdshft	; v190ev: darktable table cap kept at maxz/8 widths, actual DEFAULT distance is scaled to it
g2defviewfar	equ	12<<grdshft	; v190ev: actual DEFAULT view/fog end = 12 texture widths
g2defshapez	equ	12<<grdshft	; v190ev: DEFAULT strips/objects match 12-width view cap
	;
exshft	equ	3
exone	equ	1<<exshft
exhalf	equ	exone>>1
...
.vloop	;
	...
	divs	d7,d6	;d6.w = Z
	tst	g2_visibility
	bgt.s	.g2v190dw_flat_far
	cmp	#g2defviewfar,d6	; v190ev: DEFAULT really draws to 12 texture widths
	bcc	.rts
	; v190ev: scale actual DEFAULT 0..12 widths into the existing
	; darktable 0..8-width range, so the proven v190eq/v190eu
	; Bayer-fog curve is stretched instead of overflowing darktable.
	move	d6,d5
	move	d6,d3
	lsr	#1,d5	; 1/2
	move	d3,d0
	...
	add	d0,d5
	move	d3,d0
	lsr	#6,d0	; +1/64 ~= 2/3 total
	add	d0,d5
	cmp	#maxz-1,d5
	bls.s	.g2v190ev_flat_scale_ok
	move	#maxz-1,d5
.g2v190ev_flat_scale_ok
...
.rts	tst	doorsfxflag	; v190eg: restore original delayed door/switch event SFX path
	...
	moveq	#64,d0
	moveq	#90,d1	; v190eg: critical local door/switch event
...
g2playdoorsfx_once	; v190ee: robust immediate door/switch SFX, one per event
	tst.b	doorsfxflag
	bne.s	.rts
	st	doorsfxflag
	movem.l	d0-d1/a0,-(a7)
	...
	beq.s	.done
	moveq	#64,d0
	moveq	#80,d1	; v190ef: nearby door/switch events must win over ordinary SFX
	bsr	playsfx
...
.blok	sub	camy(pc),d1
	...
	cmp	#g2defviewfar,d2	; v190ev: DEFAULT blood visibility follows 12-width range
	bcc	.loop
...
.pix	;
	...
	move	d2,d3
	tst	g2_visibility
	bgt.s	.g2v190dw_blood_shade_adv
	move	d6,-(a7)	; preserve blood colour mask while scaling DEFAULT shade distance
	move	d3,d6
	lsr	#1,d3
	lsr	#3,d6
	add	d6,d3
	...
	add	d6,d3
	cmp	#maxz-1,d3
	bls.s	.g2v190dw_blood_shade_restore
	move	#maxz-1,d3
.g2v190dw_blood_shade_restore
	move	(a7)+,d6
	bra.s	.g2v190dw_blood_shade_ok
.g2v190dw_blood_shade_adv
	lsr	#1,d3
...
drawshape_q	;
	...
	cmp	#g2defshapez,d2	; v190ev: DEFAULT objects/sprites follow 12-width range
	bcc	.rts
...
dothezone2	;
	...
	movem	camx(pc),d6-d7
	;
	move	#g2defviewfar,d4	; v190ev: DEFAULT zone cull to 12 texture widths
	tst	g2_visibility
...
dothezone3	move	d0,d4
	...
	ble	.skip2
.zok	;
	move	#g2defviewfar,d4	; v190ev: DEFAULT wall/zone cull to 12 texture widths
	tst	g2_visibility
...
.noov	lsr	#1,d0	;fraction -> unsigned
	...
	cmp	#exone,d2
	blt	.loop2
	move	#g2defviewfar<<exshft,d6	; v190ev: DEFAULT wall cast to 12 texture widths
	tst	g2_visibility
...
.scdone	move.l	d0,d1
	...
	move	d2,d6
	sub	#(8<<grdshft),d6
	lsr	#7,d6	; 0..15 over the advanced far band
	cmp	#15,d6
...
.g2v190dw_wall_shade_adv
	lsr	#1,d6
.g2v190dw_wall_shade_ok
	move.l	darktable(pc),a2
	move	0(a2,d6*2),d3
	; v190ec: ADVANCED keeps doubled visibility but never lets far
	; walls/doors pop as bright solid columns at the extended clip.
	...
	move	d2,d6
	sub	#(8<<grdshft),d6
	lsr	#7,d6	; 0..15 over the 8..16 texture-width advanced band
	cmp	#15,d6
...
.hiteok	;
	...
	move	vd_z(a0),d6
	tst	g2_visibility
	bgt.s	.g2v190ej_wallblend_dist_adv
	move	d7,-(a7)
	move	d6,d7
	lsr	#1,d6
	lsr	#3,d7
	...
	add	d7,d6
	cmp	#maxz-1,d6
	bls.s	.g2v190ej_wallblend_scale_ok
	move	#maxz-1,d6
.g2v190ej_wallblend_scale_ok
	move	(a7)+,d7
	bra.s	.g2v190ej_wallblend_dist_ok
.g2v190ej_wallblend_dist_adv
	lsr	#1,d6
...
.drawloop	move.l	(a6),d0
	...
	move	#g2defshapez,d0	; v190eu: DEFAULT strips/objects remain visible to the new fog end
	tst	g2_visibility
...
.g2v190dw_shapelist_view_ok
	...
	; with the same 8..16 texture-width Bayer band as solid walls,
...
.ycdone	;
	...
	add	d6,d7
	cmp	#maxz-1,d7
	bls.s	.g2v190dw_obj_shade_restore
	move	#maxz-1,d7
.g2v190dw_obj_shade_restore
	move	(a7)+,d6
	bra.s	.g2v190dw_obj_shade_ok
.g2v190dw_obj_shade_adv
	lsr	#1,d7
.g2v190dw_obj_shade_ok
	move.l	darktable(pc),a2
	move	0(a2,d7*2),d7
	; v190ec: keep ADVANCED far sprites/switch-like objects fogged
	; at the extended clip instead of letting them appear fully at once.
	tst	g2_visibility
...
.rite	add.l	playerxa(pc),d0
	...
	moveq	#32,d0
	moveq	#0,d1
	jmp	playsfx	; v190ew: GenAm range-safe tail jump
	;
.nofire	clr	deflbut
.rts	rts
...
g2_visibility	dc	-1	;v190em -1=DEFAULT, +1=ADVANCED; saved in gloom.cfg
...
initdarktable	;
	; v190ev: actual DEFAULT view/fog distance is 12 texture widths.
	; The renderer scales DEFAULT distances into this existing 0..8-width
	; darktable range, so the proven v190eq/v190eu fog+dither curve is
	; stretched safely without increasing maxz, darktable or sqr.bin.
	move	#maxz-1,d2
	move.l	sqr(pc),a0
...
.g2v190aq_farboost
	...
	;
	; v190ev: this table still reaches full fog at its 8-width end.
	; DEFAULT renderer distances are scaled from actual 12 widths into
	; this table, keeping the lookup safe and extending the visible fog.
	cmp	#g2deffogfar,d4
	blo.s	.g2v190aq_ramp
	moveq	#14,d3
...
.g2v190aq_ramp
	move	d4,d5
	sub	#(4<<grdshft),d5
	lsr	#7,d5	; v190eu: 0..7 extra darkness spread across 4..8 widths
	add	d5,d3
	cmp	#14,d3
