...
.rx_ok
	move	d0,g2_shadow_rx
	; v177: keep upgrade reflections on the floor all the time.  Like the
	; older v174 path, convert the pickup height back into a downward floor
	; offset instead of letting the reflection follow the bobbing sprite.
	; Downward movement still affects only the pulse/size above, not the
	; floor anchor itself.
	clr	g2_shadow_yoff
	move	ob_y(a2),d0
	neg	d0
