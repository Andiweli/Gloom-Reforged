	...
	;gloom2_c86zc8: P96 path still disabled; fix 2P combat restart cleanup and allow the visible other player to use enemy floor reflections in TWO PLAYER.
	;
...
.g2c57_player_try_blob
	; c86zc8: in TWO PLAYER the visible other player is drawn through the
	...
	tst	g2_reflections
	ble.s	.g2zc8_player_blob_only
	tst	ob_hitpoints(a2)
	ble.s	.g2zc8_player_blob_only
	cmp	#3<<grdshft,d2
	bgt.s	.g2zc8_player_blob_only
	bsr	g2_prepare_enemy_reflection_safe
.g2zc8_player_blob_only
	tst	g2_blobshadow
	ble	.done
...
.gotdata	;
	...
	;combat type game!
	;
	; c86zc8: a fresh title-menu TWO PLAYER COMBAT start must not
	; inherit the shuffled combat-map list from the previous combat run.
	; Continuing inside the same combat series jumps directly back to
	; execscript_med, so this reset only affects a new menu start.
...
; c86zc8: make TWO PLAYER COMBAT starts deterministic/clean again.
; pickcombat shuffles commaps in-place; if a complete combat run is left
; and another combat game is started from the title menu, restore the list
; instead of continuing with whatever the previous run left behind.
g2resetcombatseries
	movem.l	d0/a0,-(a7)
	lea	commaps(pc),a0
	moveq	#1,d0
...
; c86zc8: combat pictures are optional per combat run and freemem does not
; clear the owner variable.  Keep cleanup idempotent so a second combat start
; cannot inherit or later re-free a stale combat/combatpal pointer.
g2freecombatfiles
	movem.l	d0/a1,-(a7)
	move.l	combat(pc),d0
	beq.s	.g2zc8_no_combat
	clr.l	combat
	move.l	d0,a1
	freemem	combat
.g2zc8_no_combat
...
exitgame	cmp	#2,gametype
	bne.s	gameover
combatover	bsr	g2freecombatfiles
