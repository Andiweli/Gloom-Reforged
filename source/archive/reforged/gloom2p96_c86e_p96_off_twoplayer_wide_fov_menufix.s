	...
	;gloom2p96_c86e: P96 overlay off; TWO PLAYER window size uses wider FOV without aspect stretch.
	;
...
debugser	equ	0
debugmem	equ	0
g2p96_overlay_enabled	equ	0	; c86e: 0 hides/disables P96 overlay, -1 re-enables old c85 P96 preview path
	;
...
g2view_scale_x_d0
	; c86e: in TWO PLAYER the split height is fixed at 120.
...
g2view_scale_x_d2
	; c86e: same aspect-safe X scale for secondary wall endpoint.
	tst	g2twop_aspect_mode
	bne	.g2twop_aspect
	cmp	#320,width
	beq	.rts
...
g2view_scale_x_d3
	; c86e: sprite/shape widths must follow vertical scale in 2P,
...
; overwrote the entire frame.  c86d keeps each physical split half at 320x120
; and now uses the current WINDOW SIZE width dynamically: TINY stays narrow but
; keeps full split height, while FULLSCREEN uses the whole 320-pixel split
; width.  This brings back the useful two-player view-size behaviour without
; returning to the vertically squashed full-height projection.  c55 additionally
; draws the existing top HUD inside each split half; first-person gun placement
; is still intentionally left out.
g2twop_drawall_split
	...
	;
	jsr	g2twop_prepare_view_width	;c86e: use current WINDOW SIZE width for 2P split
	jsr	g2clearfullchunky	;clear both split halves from the real full-frame base
	jsr	g2twop_center_coloffs	;c86e: center selected split width inside each 320px half
	jsr	g2twop_set_half_view
	move	#-1,g2twop_aspect_mode	;c86e: width changes FOV, not object aspect
	;
...
.g2twop_restore_full
	clr	g2twop_aspect_mode	;c86e: normal full-frame C2P/menu code uses normal scale again
...
g2twop_draw_half_hud
	...
	move	#-1,g2hud_twop_mode	;c86d: keep right-side HUD inside selected split width
	jsr	showstats
	clr	g2hud_twop_mode
...
g2twop_prepare_view_width
	; c86e: WINDOW SIZE controls two-player field width; aspect stays height-locked.
	; Split height always stays at 120 pixels so the view is not vertically
	; squeezed.  Clamp the tiny end to the old gloom.s-style 132px minimum.
...
.min_ok	cmp	#320,d0
	bls.s	.max_ok
	move	#320,d0
.max_ok	and	#$fffe,d0	;keep an even render width
	move	d0,g2twop_view_width
	rts

g2twop_set_half_view
	; c86e: physical split half remains 320x120. Logical render width follows
	; WINDOW SIZE, while projection scale remains tied to the 120px split height.
...
castwalls	;process 'walls' list
	...
	; c86e: TWO PLAYER is different: height stays 120 and X scale is
...
p96prevlog_name	dc.b	'RAM:gloom_p96_preview.txt',0
	even
p96prevlog_ok	dc.b	'P96 preview: c86e p96 off twop wide fov active',10
...
g2v190aj_grey_menu_backdrop
	...
	jsr	g2v190aj_twop_full_c2p	;c86e: split preview must be C2P'ed as full 320x240
	bra	.g2grey_db
.g2grey_normal_c2p
	jsr	doc2p
.g2grey_db
...
; c86e: The ESC-menu grey backdrop works on the already rendered chunky
...
g2v190aj_twop_full_c2p
	movem.l	d0-d1/a0-a1,-(a7)
	move	width(pc),g2menu_saved_width
	move	hite(pc),g2menu_saved_hite
	move	chunkymodw(pc),g2menu_saved_chunkymodw
	move	minx(pc),g2menu_saved_minx
	move	maxx(pc),g2menu_saved_maxx
	move	miny(pc),g2menu_saved_miny
	move	maxy(pc),g2menu_saved_maxy
	move.l	offset(pc),g2menu_saved_offset
	...
	clr.l	offset
	jsr	doc2p
	move	g2menu_saved_width(pc),width
	move	g2menu_saved_hite(pc),hite
	move	g2menu_saved_chunkymodw(pc),chunkymodw
	move	g2menu_saved_minx(pc),minx
	move	g2menu_saved_maxx(pc),maxx
	move	g2menu_saved_miny(pc),miny
	move	g2menu_saved_maxy(pc),maxy
	move.l	g2menu_saved_offset(pc),offset
	movem.l	(a7)+,d0-d1/a0-a1
	rts
