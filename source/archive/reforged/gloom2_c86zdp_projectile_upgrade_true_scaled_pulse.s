...
.wp_pulse
	...
	; c86zdp: weapon-upgrade pulse grows/shrinks with a small envelope and tops
	; out at about 1.5x.  Width is handled by re-evaluated scaled columns,
	; not by stamping edge pixels outside the oval.
	move	ob_y(a2),d1
	cmp	#-32,d1
	blt	.pick_pulse_done
	cmp	#-20,d1
	...
	and	#31,d1
	; c86zdp: true pulse envelope instead of an instant max pop:
	; 1.0 -> 1.125 -> 1.25 -> 1.5 -> 1.25 -> 1.125 -> 1.0.
	cmp	#24,d1
	bgt	.pick_pulse_done
	cmp	#4,d1
	ble	.wp_pulse_l1
...
.wp_pulse_l3
	...
	move	d6,d1
	lsr	#1,d1
	add	d1,d0		;level 3: about 1.5x radius at pulse peak
.wp_pulse_distcap
	cmp	#(3<<grdshft),d2
...
.oval_v_done
	; c86zdp: weapon-upgrade floor-touch pulse swells smoothly up to 1.5x.
	; g2_reflect_pulse is 0..3 from prepare: 1 ~= 1.125x, 2 ~= 1.25x,
	; 3 ~= 1.5x.  Height is shaped here; width is handled by drawing
	; one real scaled oval column in the before-sprite wrapper.
	move	g2_reflect_pulse,d0
	beq	.oval_pulse_done
	tst	g2_reflect_pickup
	beq	.oval_pulse_done
