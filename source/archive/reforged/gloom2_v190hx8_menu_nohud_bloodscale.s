...
.v32_blood_color_ok
	...
	add.l	0(a2,d0*4),a1
	move.b	d3,0(a1,d7.l)
	; v190hx8: the projected blood position is already scaled by hx7, but
	; the old fixed 2x2 chunky block stayed full-size.  In smaller VIEW SIZE
	; modes draw a single pixel so splats scale with the render window.
	cmp	#320,width
	bne.w	.no_y2
	cmp	#240,hite
	bne.w	.no_y2
...
g2v190aj_grey_menu_backdrop
	...
	; v190hx8: the current chunky frame may already contain the fixed top HUD
	; from hx6/hx7.  Do not show that HUD inside the grey ESC-menu
	; preview, especially for small VIEW SIZE modes where it appears distorted.
	clr	g2hud_pending	; no deferred fixed-HUD pass while building the menu preview
	jsr	g2hud_clear_top_chunky
