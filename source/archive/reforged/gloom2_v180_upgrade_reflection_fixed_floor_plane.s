...
.rx_ok
	move	d0,g2_shadow_rx
	; v180: keep upgrade reflections on the same floor plane as projectile
	; reflections.  Start from a fixed bobbing-floor baseline and only use
	; current ob_y to counter the pickup bounce, so the reflection stays on
	; the floor texture instead of moving up/down with the upgrade sprite.
	clr	g2_shadow_yoff
	tst	g2_reflect_pickup
	beq.s	.projectile_floor_yoff
	move	#32,d0		; fixed pickup floor baseline in world-Y units
	sub	ob_y(a2),d0
	bra.s	.scale_yoff
