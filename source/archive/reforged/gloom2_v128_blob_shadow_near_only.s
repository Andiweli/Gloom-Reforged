...
; -----------------------------------------------------------------------------
; v126 enemy blob shadow, direct per-column writer.
; Earlier tests prepared a separate post-sprite oval and wrote via a generic span,
; but no visible pixels appeared.  This version writes from inside drawobjnorm,
; using the exact destination column pointer that just drew the enemy sprite.
; Colour is a hard very-dark palette-mapped value and the ellipse has hard edges.
; -----------------------------------------------------------------------------
...
.rx_max_ok
	...
	moveq	#2,d0	;fallback: dark but slightly lighter than v126
	move.l	planar_remap(pc),a0
	tst.l	a0
	beq	.col_ok
	move	#$222,d1	;v127: lighter dark palette tone
	move.b	0(a0,d1.w),d0
	bne	.col_ok
	move	#$333,d1
	move.b	0(a0,d1.w),d0
...
.abs_ok	move	g2_shadow_rx(pc),d2
	cmp	d2,d1
	bgt	.rts
	; v127: flatter, more elliptical hard-edged profile, glued to feet.
	; a4 is just below the drawn sprite, so shift the base up two rows.
	moveq	#2,d5	;outer edge vertical offset
	moveq	#0,d6	;dbf count: 1 row
	move	d2,d3
	mulu	#3,d3
	lsr	#2,d3	;3/4 radius: edge band
	cmp	d3,d1
	bgt	.have_band
	moveq	#1,d5
	moveq	#1,d6	;2 rows
	...
	bgt	.have_band
	moveq	#0,d5
	moveq	#2,d6	;3 rows centre/mid
	move	d2,d3
	lsr	#2,d3	;1/4 radius: slightly fuller centre
	cmp	d3,d1
	bgt	.have_band
	moveq	#0,d5
	moveq	#3,d6	;4 rows only in the centre
.have_band
	move.l	a4,a0
	move.l	chunkymod(pc),d7
	sub.l	d7,a0	;stick shadow to the feet
	sub.l	d7,a0
