...
; -----------------------------------------------------------------------------
; v132 enemy blob shadows + first reflections test, direct per-column writer.
; v126 finally proved this is the real visible draw path.  Blob shadows still
; use the confirmed enemy path, but are moved 1px back down from v131.  Reflec-
; tions use the same visible sprite-column hook for bullets and weapon pickups.
; -----------------------------------------------------------------------------
...
.bul_min_ok
	...
	ble	.rx_ok
	moveq	#10,d0
.rx_ok	move	d0,g2_shadow_rx
	; v133: reflection belongs on the floor, not glued to the bottom of
	; the floating sprite.  Approximate the floor drop from object Y and
	; current projected Z.  Negative object Y means the sprite is above
	; the floor, so move the reflection down by the projected height.
	clr	g2_shadow_yoff
	move	ob_y(a2),d0
	neg	d0
	ext.l	d0
...
g2_reflect_rgb
	; v133: darker edge-like tones from the projectile/upgrade palette
	; family, not the bright core colours.
...
.rabs_ok	move	g2_shadow_rx(pc),d2
	cmp	d2,d1
	bgt	.rts
	; Hard-edged coloured mini-ellipse/ring.  This is intentionally small so
	; bullets get a glow/reflection without becoming a second blob shadow.
	moveq	#1,d5	;outer: one row down
	moveq	#0,d6	;1 row
	move	d2,d3
	lsr	#1,d3
	cmp	d3,d1
	bgt	.rband_ok
	moveq	#0,d5
	moveq	#1,d6	;centre: 2 rows
.rband_ok
	move.l	a4,a0
	move.l	chunkymod(pc),d7
	sub.l	d7,a0
	sub.l	d7,a0
	; v133: floor-anchor reflection.  Move down/up from the sprite
