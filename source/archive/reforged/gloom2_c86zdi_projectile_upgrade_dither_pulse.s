...
exhalf	equ	exone>>1
	...
	jmp	entrypoint
	; c86zde / GitHub 1.3.3: early executable ASCII release marker for hex/text viewers.
g2release_marker	dc.b	'Gloom Reforged AGA (v1.3.3 28.06.2026) Andreas ',39,'Andiweli',39,' Stuermer',0
	even
...
.wp_min_ok
	...
	moveq	#16,d0
.wp_pulse
	; c86zdi: weapon-upgrade pulse now matches the c86zdf raised floor
	; anchor.  Since weaponlogic clamps the lowest bob to about -20px,
	; pulse only for a few phase frames while the upgrade is just reaching
	; that low point.  This keeps the reflection from staying oversized.
	move	ob_y(a2),d1
	cmp	#-24,d1
	blt.s	.pick_pulse_done
	cmp	#-20,d1
...
.rband_ok
	; c86zdh: close projectiles/upgrades should become vertically oval,
	; not wider.  The sparse outer rim stays one row; the mid/body bands
	; gain height only in the near range stored during preparation.
	...
	cmp	#1,d3
	bne.s	.oval_v_body
	moveq	#0,d5		;mid band starts one row higher
	moveq	#3,d6		;c86zdi near mid: 4 rows
	cmp	#2,d0
	bne.s	.oval_v_done
	moveq	#4,d6		;c86zdi very near mid: 5 rows
	bra.s	.oval_v_done
.oval_v_body
	moveq	#0,d5
	moveq	#6,d6		;c86zdi near centre: 7 rows
	cmp	#2,d0
	bne.s	.oval_v_done
	moveq	#8,d6		;c86zdi very near centre: 9 rows
.oval_v_done
	; c86zdi: weapon-upgrade floor-touch pulse swells the whole oval
	; briefly.  Projectiles keep the normal distance-based thickness.
	tst	g2_reflect_pulse
	beq.s	.oval_pulse_done
	tst	g2_reflect_pickup
	beq.s	.oval_pulse_done
	move	g2_reflect_softedge(pc),d3
	cmp	#2,d3
	...
	moveq	#0,d5
	moveq	#9,d6		;pulse centre: 10 rows
	bra.s	.oval_pulse_done
.oval_pulse_mid
	moveq	#0,d5
	moveq	#5,d6		;pulse mid: 6 rows
	bra.s	.oval_pulse_done
.oval_pulse_outer
	moveq	#1,d5
	moveq	#1,d6		;pulse outer: 2 rows, still sparse
.oval_pulse_done
	move.l	chunkymod(pc),d7
	tst	g2_reflect_pickup
	bne.s	.pick_anchor_a
...
.rdraw	move	g2_shadow_col(pc),d4
		; c86zdi: dither the complete projectile/upgrade reflection,
		; using the same 4x4 Bayer family as enemy reflections.
		; Centre/body stays denser, mid/outer bands become progressively
		; sparser instead of drawing a fully solid strip.
		moveq	#10,d3		; body threshold: dense but still dithered
...
dointro	;
	...
	jsr	g2v145_show_clean_title
	; c86zdf: draw the ZM g3-zm/g3-dc title overlay into the hidden
	; buffers before the title screen is made visible.  This avoids the
	; short flash of the original Zombie Massacre title image.
...
g2_reflect_pulse	dc	0	;c86zdi brief 1.5x weapon-upgrade floor-touch pulse
