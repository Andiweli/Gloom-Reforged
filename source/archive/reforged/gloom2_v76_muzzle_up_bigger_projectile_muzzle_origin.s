...
.g2dm_have_weapon
	...
	; v76: enlarge the normal flash by roughly another 25% over the old
	; 100% pass.  We still use the original gun.bin shapes, but stamp them
	; in a broader pattern so the visible result is closer to 125%.
	bsr	g2drawgun_coloffs_shape
	subq	#3,d1
	bsr	g2drawgun_coloffs_shape
	addq	#6,d1
	...
	addq	#2,d1
	subq	#2,d4
	; v76: the larger flicker frame should now read as roughly 150% of the
	; original flash (former 120% phase + another 25%).  Add a wider halo
	; and diagonals around the already enlarged 125% base pass.
	move	g2gun_firetimer(pc),d5
	btst	#0,d5
	bne.w	.g2dm_done		;125%,150%,125%,150% over the 4 fire frames
	subq	#7,d1
	bsr	g2drawgun_coloffs_shape
	addq	#7,d1
	addq	#7,d1
...
.g2amo_do
	...
	lea	0(a1,d0*8),a1
	move	#42,d0		;v76: a touch farther forward so the projectile begins from the flash core
	move	d0,d2
	muls	2(a1),d0
	add.l	d0,d0
	neg.l	d0
	...
	add.l	d2,d2
	add.l	d2,ob_z(a0)
	; v76: raise the projectile origin further so the shot visibly starts at
	; the muzzle flash instead of a little above the statusbar line.
	sub	#40,ob_y(a0)
.g2amo_done
	movem.l	(a7)+,d0-d2/a1
	rts
...
.nomega	bsr	shoot
	;
.shdone	move.l	(a7)+,a0
	move	#4,g2gun_firetimer	;v74: 4-frame 100/120/100/120 muzzle pulse
	move.l	(a0),a0
	moveq	#32,d0
	moveq	#0,d1
	bsr	playsfx
