...
.g2dm_have_weapon
	...
	; v74: Original-Gloom-Deluxe style flash pulse.  There is no cheap
	; scaler here, so the 120% frames are simulated by drawing the same
	; flash once normally and then a few pixels around the muzzle.
	bsr	g2drawgun_coloffs_shape
	move	g2gun_firetimer(pc),d5
	btst	#0,d5
	bne.s	.g2dm_done		;100%,120%,100%,120% over the 4 fire frames
	subq	#2,d1
	bsr	g2drawgun_coloffs_shape
	addq	#4,d1
	bsr	g2drawgun_coloffs_shape
...
.g2amo_do
	...
	add.l	d2,d2
	add.l	d2,ob_z(a0)
	; v74: raise the visible player projectile start to the gun muzzle again.
	; #36 was too high/near, #12 was still visibly below the muzzle flash;
	; this middle value keeps the first frame out of the statusbar area.
	sub	#24,ob_y(a0)
.g2amo_done
	movem.l	(a7)+,d0-d2/a1
	rts
