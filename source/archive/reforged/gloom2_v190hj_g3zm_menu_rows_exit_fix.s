...
.redrawmenu
	...
	tst	g2_game_profile	; v190hj: Gloom Deluxe keeps original START LEVEL menu
	beq.s	.use_gloom2_startmenu
	cmp	#1,g2_game_profile	; v190hj: Classic Gloom keeps confirmed v190hi compat menu
	beq.s	.use_compat_startmenu
	lea	compat_startmenu_g3zm,a4	; v190hj: Gloom3/ZM compact START LEVEL menu
	jsr	initmenu
	move	#1,curropt	; v190hj: ONE PLAYER GAME is default/blinking row
	bra.s	.sel
.use_compat_startmenu
	lea	compat_startmenu,a4	; v190hj: Classic Gloom confirmed menu
	jsr	initmenu
...
.g2v190f_shifted_newgame
	...
	bra	.newgame
.g2compat_notlinked
	cmp	#1,g2_game_profile	; v190hj: Classic Gloom keeps confirmed v190hi mapping
	bne	.g2compat_g3zm_notlinked
	tst	d7
	beq.s	.g2compat_fire
	cmp	#0,d0		; v190hf: START LEVEL left/right
	beq	.g2v190f_level_lrsel
	cmp	#6,d0		; v190hf: PLAYER 1 row
	beq	.g2compat_p1lrsel
	cmp	#7,d0		; v190hf: PLAYER 2 row
	beq	.g2compat_p2lrsel
	bra	.sel
.g2compat_fire
	cmp	#0,d0		; v190hf: START LEVEL launch
	beq	.g2v190f_level_start
	cmp	#2,d0
	bcs	.sel		; spacer below START LEVEL
	cmp	#5,d0		; rows 2/3/4 -> gametype 0/1/2
	bcs	.g2compat_newgame_shifted
	cmp	#6,d0
	beq	.g2compat_p1sel
	cmp	#7,d0
	beq	.g2compat_p2sel
	...
	beq	.exitgloom
	bra	.sel
.g2compat_newgame_shifted
	subq	#2,d0
	bra	.newgame
.g2compat_g3zm_notlinked
	tst	d7
	beq.s	.g2compat_g3zm_fire
	cmp	#0,d0		; v190hj: START LEVEL left/right
	beq	.g2v190f_level_lrsel
	cmp	#5,d0		; v190hj: PLAYER 1 row
	beq	.g2compat_g3zm_p1lrsel
	cmp	#6,d0		; v190hj: PLAYER 2 row
	beq	.g2compat_g3zm_p2lrsel
	bra	.sel
.g2compat_g3zm_fire
	cmp	#0,d0		; v190hj: START LEVEL launch
	beq	.g2v190f_level_start
	cmp	#1,d0
	bcs	.sel
	cmp	#4,d0		; rows 1/2/3 -> gametype 0/1/2
	bcs	.g2compat_g3zm_newgame
	cmp	#5,d0
	beq	.g2compat_g3zm_p1sel
	cmp	#6,d0
	beq	.g2compat_g3zm_p2sel
	cmp	#8,d0
	beq	.linksel
	cmp	#9,d0
	beq	.vilesel
	...
	beq	.exitgloom
	bra	.sel
.g2compat_g3zm_newgame
	subq	#1,d0
	bra	.newgame
.g2compat_g3zm_p1lrsel
	cmp	#$0100,d7
	beq.s	.g2compat_g3zm_p1prevsel
.g2compat_g3zm_p1sel	lea	p1ctypeg3zm(pc),a1
	lea	p1_ob_cntrl,a2
	lea	p2_ob_cntrl,a3
	bsr	inccntrl
	bra	.sel
.g2compat_g3zm_p1prevsel	lea	p1ctypeg3zm(pc),a1
	lea	p1_ob_cntrl,a2
	lea	p2_ob_cntrl,a3
	bsr	deccntrl
	bra	.sel
.g2compat_g3zm_p2lrsel
	cmp	#$0100,d7
	beq.s	.g2compat_g3zm_p2prevsel
.g2compat_g3zm_p2sel	lea	p2ctypeg3zm(pc),a1
	lea	p2_ob_cntrl,a2
	lea	p1_ob_cntrl,a3
	bsr	inccntrl
	bra	.sel
.g2compat_g3zm_p2prevsel	lea	p2ctypeg3zm(pc),a1
	lea	p2_ob_cntrl,a2
	lea	p1_ob_cntrl,a3
	bsr	deccntrl
	bra	.sel
...
.vilesel	addq	#1,mode
	...
	cmp	#1,g2_game_profile	; v190hj: Classic uses modetxtc, G3/ZM use modetxtg3zm
	beq.s	.g2v190co_mode_compat
	lea	modetxtg3zm,a1
	bra.s	.g2v190co_mode_copy
.g2v190co_mode_compat
	lea	modetxtc,a1
	bra.s	.g2v190co_mode_copy
.g2v190co_mode_main
	lea	modetxt,a1
...
compat_startmenu	dc.b	13		; v190hf: same visible structure as Deluxe, profile-specific control strings
	dc.b	'START LEVEL '
g2v190hc_level_text	dc.b	'MAP1.1',0	;0 synced from g2v190f_level_text
	dc.b	0			;1 spacer below level selector
	dc.b	'ONE PLAYER GAME',0	;2
	dc.b	'TWO PLAYER GAME',0	;3
	dc.b	'TWO PLAYER COMBAT',0	;4
	dc.b	0			;5 spacer/dotted line above PLAYER 1
	dc.b	'PLAYER 1 '
p1ctypec
	ifne	cd32
	dc.b	'CD32 PAD 1',0	;6, fixed 10-char field
...
p2ctypec
	...
	endc
	;
	dc.b	0			;8 spacer/dotted line above REMOTE LINK OPTIONS
	dc.b	'REMOTE LINK OPTIONS',0	;9
modetxtc	dc.b	'MEATY VIOLENCE MODE',0	;10
	dc.b	'ABOUT GLOOM',0		;11
	dc.b	'EXIT GLOOM',0		;12
	even

compat_startmenu_g3zm	dc.b	13		; v190hj: Gloom3/ZM compact START LEVEL menu, lines stay fixed
	dc.b	'START LEVEL '
g2v190hj_level_text	dc.b	'MAP1.1',0	;0 synced from g2v190f_level_text
	dc.b	'ONE PLAYER GAME',0	;1 default/blinking row
	dc.b	'TWO PLAYER GAME',0	;2
	dc.b	'TWO PLAYER COMBAT',0	;3
	dc.b	0			;4 spacer/dotted line above PLAYER 1
	dc.b	'PLAYER 1 '
p1ctypeg3zm
	ifne	cd32
...
p2ctypeg3zm
	...
	endc
	;
	dc.b	0			;7 spacer/dotted line above REMOTE LINK OPTIONS
	dc.b	'REMOTE LINK OPTIONS',0	;8
modetxtg3zm	dc.b	'MEATY VIOLENCE MODE',0	;9
	dc.b	'ABOUT GLOOM',0		;10
	dc.b	'EXIT GAME',0		;11
	dc.b	0			;12 trailing visual spacer keeps menu Y identical to 13-row title menu
	even
...
.g2v190i_copy_done
	clr.b	(a1)
	lea	g2v190f_level_text(pc),a0	; v190hf: keep Classic compat START LEVEL text in sync
	lea	g2v190hc_level_text(pc),a1
	moveq	#5,d1
.g2v190hf_copy
	move.b	(a0)+,(a1)+
	beq.s	.g2v190hf_copy_done
	dbf	d1,.g2v190hf_copy
	clr.b	(a1)
.g2v190hf_copy_done
	lea	g2v190f_level_text(pc),a0	; v190hj: keep Gloom3/ZM compat START LEVEL text in sync
	lea	g2v190hj_level_text(pc),a1
	moveq	#5,d1
.g2v190hj_copy
	move.b	(a0)+,(a1)+
	beq.s	.g2v190hj_copy_done
	dbf	d1,.g2v190hj_copy
	clr.b	(a1)
.g2v190hj_copy_done
	movem.l	(a7)+,d0-d2/a0-a1
	rts
...
g2v190i_levelselect_loadscripts
	...
	clr	g2v190f_level_index
	bsr	permit
	; v190hf: profile-specific START LEVEL source:
	; Gloom/Gloom Deluxe/Gloom3 -> misc/script, Zombie Massacre -> stuf/stages.
	cmp	#2,g2_game_profile
	beq.s	.g2v190hf_try_zm_stages
.g2v190hf_try_misc_script
...
g2v190i_try_script
	; v190hf: use the normal loadfile path so compressed CrM2 scripts
	; are decrunched before parsing, same behaviour as the normal game loader.
	movem.l	d0-d1/a0-a1,-(a7)
	moveq	#1,d1
	jsr	loadfile
	tst.l	d0
...
initmenu2	;
	...
	cmp.l	#compat_startmenu_g3zm+1,a4
...
.titlemenu_main	move	#-1,g2v154_titlemenu_lines
	...
	bra.s	.titlemenu_y_done
.titlemenu_compat	move	#-1,g2v154_titlemenu_lines	; v190hf: Classic START LEVEL compat menu uses Deluxe separator rows
	sub	fonth(pc),d6	; v190hf: add START LEVEL while preserving old ONE PLAYER row
	sub	fonth(pc),d6
	sub	fonth(pc),d6
	bra.s	.titlemenu_y_done
.titlemenu_g3zm	move	#3,g2v154_titlemenu_lines	; v190hj: G3/ZM lines at compact rows 4 and 7
	sub	fonth(pc),d6
	sub	fonth(pc),d6
	sub	fonth(pc),d6
.titlemenu_y_done
...
g2v154_draw_titlemenu_lines
	...
	bne.s	.g2v190hj_line1_not_g3zm
	moveq	#4,d3
	bra.s	.g2v190cp_line1row_ok
.g2v190hj_line1_not_g3zm
...
.g2v190cp_line1row_ok
	...
	moveq	#8,d3
	cmp	#3,g2v154_titlemenu_lines
	bne.s	.g2v190hj_line2_not_g3zm
	moveq	#7,d3
	bra.s	.g2v190cp_line2row_ok
.g2v190hj_line2_not_g3zm
	cmp	#2,g2v154_titlemenu_lines
...
g2v166_tplr_full
	move.l	menustrips(pc),d0	; v190hj: G3/ZM compact menu has PLAYER rows at 5/6
	lea	compat_startmenu_g3zm+1(pc),a0
	move.l	a0,d1
	cmp.l	d1,d0
