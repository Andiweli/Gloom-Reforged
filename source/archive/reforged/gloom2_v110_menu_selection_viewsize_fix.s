...
.pok	move	(a2),d0
	...
	bra.s	copycntrltext
	;
cntrlright_noloop	;v110: right arrow, no wrap, but KEYBMOUSE is always selectable
	move	(a2),d0
	cmp	#5,d0
	bge.s	.rts
	addq	#1,d0
.r_try	tst	d0		;both players may use KEYBMOUSE
	beq.s	.r_set
	cmp	(a3),d0		;avoid duplicate non-keymouse controls only
	bne.s	.r_set
	addq	#1,d0
	cmp	#5,d0
	bgt.s	.rts
...
.rts	rts
	;
cntrlleft_noloop	;v110: left arrow, no wrap, KEYBMOUSE can be selected again
	move	(a2),d0
	tst	d0
	ble.s	.rts
	subq	#1,d0
.l_try	tst	d0		;both players may use KEYBMOUSE
	beq.s	.l_set
	cmp	(a3),d0		;avoid duplicate non-keymouse controls only
	bne.s	.l_set
	subq	#1,d0
