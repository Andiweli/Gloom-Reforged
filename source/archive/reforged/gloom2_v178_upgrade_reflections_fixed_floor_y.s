...
.rx_ok
	move	d0,g2_shadow_rx
	; v178: pickups/upgrades use the opposite Y compensation from bullets.
	; Their sprite bobs around the floor point, so add the projected ob_y back
	; below the drawn sprite column.  That keeps the reflection on the floor;
	; bobbing/downward movement only changes the pulse size above.
	clr	g2_shadow_yoff
	move	ob_y(a2),d0
	tst	g2_reflect_pickup
	bne.s	.pickup_floor_yoff
	neg	d0
.pickup_floor_yoff
.scale_yoff	ext.l	d0
	asl.l	#7,d0
