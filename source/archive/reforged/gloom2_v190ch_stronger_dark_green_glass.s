...
.g2st_make_green
	; v190cg: realistic dark green glass.  The already-rendered background
	; remains at least 50% visible, then receives only a dark green tint.
	; Dark walls therefore remain dark; bright lamps/textures stay readable.
	move	d0,d1	; red
	lsr	#8,d1
	and	#$000f,d1
	move	d0,d2	; green
...
.g2st_max_g_ok
	...
	move	d6,d5
.g2st_max_b_ok
	; v190ch: stronger dark green glass.  v190cg often quantised to the
	; same palette entries as v190cf, so make the target colour more clearly
	; green while preserving the scene brightness.  Red/blue are strongly
	; suppressed; green follows the original brightness plus a very small
	; dark glass bias.  Dark walls remain dark, bright lamps stay readable.
	lsr	#2,d1	; red = 25% original
	lsr	#2,d6	; blue = 25% original
	move	d5,d2	; green from brightness, not fixed bright boost
	lsr	#1,d2	; green = 50% scene brightness
	addq	#2,d2	; small dark-green glass bias for black/dark areas
	cmp	#10,d2	; cap so glass never becomes neon/too bright
	bls.s	.g2st_green_cap_ok
	moveq	#10,d2
.g2st_green_cap_ok
	lsl	#8,d1
	lsl	#4,d2
