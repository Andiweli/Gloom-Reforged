...
exittoos	jsr	inputoff	; v34: restore ciaa/rawkey vectors before OS exit/closewindow
	; v190fj: removed undefined hard-stop Paula SFX call, v58 exit path
...
.p_loop	move.l	d0,(a0)+
	...
	clr	dispnest
	clr.l	font
	jsr	initsfx	; v190fj: GenAm range fix
	bsr	initvbint
	bsr	initdisplay
	;
...
.agaa	bsr	loadfiles
	...
	bsr	loadfiles
	jsr	g2v190fi_sfx_fallbacks	; v190fi: fill any missing SFX pointers from v58 paths without overriding loaded profile sounds
...
; -----------------------------------------------------------------------------
; v190fi: conservative SFX pointer fallback.
; The live mixer and all SFX call paths above are restored to v58/Gloom style.
; This only protects the newer multi-profile loader: if an active profile table
; leaves a standard SFX pointer at 0, try the original v58 sfxs/ file once.
; Already loaded profile sounds are never overwritten.
; -----------------------------------------------------------------------------
g2v190fi_sfx_fallbacks
	movem.l	d0-d2/a0-a2,-(a7)
	lea	g2v190fi_sfx_fallback_table(pc),a2
.loop	move.l	(a2)+,a1
	beq.s	.done
	tst.l	(a1)
	bne.s	.skipname
	move.l	a2,a0
	moveq	#2,d1	; original v58 SFX files are CHIP allocations via +1 table entries
	jsr	loadfile
	tst.l	d0
	beq.s	.skipname
	move.l	d0,(a1)
.skipname
	...
	exg	a2,d0
	bra.s	.loop
.done	movem.l	(a7)+,d0-d2/a0-a2
	rts

g2v190fi_sfx_fallback_table
	dc.l	shootsfx
	dc.b	'sfxs/shoot.bin',0
	even
	dc.l	shootsfx2
