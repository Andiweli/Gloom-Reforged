...
doc2p	;
	...
	move.l	c2p(pc),a2
	jsr	(a2)
	jsr	g2v15_clear_bottom16	;v15: clear PAL tail below 240-line game area
	move	panelcnt(pc),d0
	beq.s	.rts
	;
	subq	#1,panelcnt
...
.aga1	tst	os
	beq.s	.os1
	moveq	#40,d2	;linemod
	move.l	#40*256,d3	;bpmod ;v15: 256-line OS plane span
	lea	db_os,a0
	lea	pokepal_os,a1
.os1	;
	move	d0,bitplanes
...
g2v14_wrap_script_text	;auto-wrap long text_ script lines at a word boundary
	;in: d0.w = text length, buffer at text
	movem.l	d1-d4/a0-a1,-(a7)
	cmp	#38,d0
	ble.s	g2v14_wst_done
	lea	text,a0
	move.l	a0,a1
	moveq	#-1,d2	;last space offset
	moveq	#0,d3	;current offset
g2v14_wst_scan
	move.b	(a1)+,d4
	beq.s	g2v14_wst_done
	cmp.b	#'\',d4
	beq.s	g2v14_wst_done
	cmp.b	#' ',d4
	bne.s	g2v14_wst_next
	move	d3,d2
g2v14_wst_next
	addq	#1,d3
	cmp	#38,d3
	blt.s	g2v14_wst_scan
	tst	d2
	bpl.s	g2v14_wst_have
	move	#38,d2
g2v14_wst_have
	lea	text,a0
	move.b	#'\',0(a0,d2.w)
...
initbitmap	;a0=bitmap struct, d0=bitplane 0
	...
	move	bitplanes(pc),d1
	move	#40,(a0)	;linemod
	move	#256,2(a0)	;v15: OS BitMap rows cover visible PAL tail
	move	d1,4(a0)
	clr	6(a0)
	lea	8(a0),a1
	subq	#1,d1
...
newscreen	dc	16,0	;x,y ;v15: nudge OS/NewMode screen right for WinUAE centering
	dc	320,256	;w,h ;v15: allocate visible PAL-safe height
...
newwindow	dc	0,0	;x,y
	dc	320,256	;w,h ;v15: match 256-line OS screen
	dc.b	0,0	;pens
...
initdisplay	;
	...
	;allocate 2 bitmaps!
	;
	move	#40*256,d2	;1 bitplane (DB), v15 PAL-safe
...
copinit_ecs	dc	$096,$120
	;
	dc	$08e,$2c91,$090,$1cd1	;v15: 320X240 nudged right
...
copinit_aga	;copperlist for AGA amigas
	...
	dc	$1fc,15,$096,$120
	;
	dc	$08e,$2c91,$090,$1cd1	;v15: nudged right
