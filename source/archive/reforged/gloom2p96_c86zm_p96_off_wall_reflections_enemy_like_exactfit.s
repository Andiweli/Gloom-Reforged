...
.col_loop
	...
	; c86zm: mirror sizing now follows the enemy-reflection idea more closely.
	; First decide the desired visible reflection size from the visible wall
	; height itself (small far wall = small reflection, large near wall =
	; larger reflection).  Then fit that desired size on screen by moving the
	; destination start upward if needed, instead of shrinking the reflection
	; simply because little floor space is visible below the wall-foot.
	move	d1,d0		; preserve actual wall-foot row for the source pointer
	move	vd_h(a0),d6
	asr	#3,d6		; 1 row per ~8 visible wall pixels
	cmp	#1,d6
	bge.s	.depth_min_ok
	moveq	#1,d6
.depth_min_ok
	cmp	#7,d6
	ble.s	.depth_cap_ok
	moveq	#7,d6		; max 8 visible rows (0..7 in DBF)
.depth_cap_ok
	move	d0,d1
	addq	#1,d1		; default destination start just below the wall-foot
	move	d1,d2
	add	d6,d2		; wanted last destination row
	move	hite(pc),d3
	sub	#3,d3		; safe last row before HUD/gun edge
	cmp	d3,d2
	ble.s	.depth_fit_ok
	sub	d3,d2		; overflow rows
	sub	d2,d1		; shift destination start upward instead of shrinking height
	move	midy(pc),d2
	addq	#1,d2
	cmp	d2,d1
	bge.s	.depth_fit_ok
	move	d2,d1		; do not lift above the floor half start
.depth_fit_ok
	...
	move.l	a2,a3
	add.l	d5,a3
	add.l	d2,a3		; a3 = source at actual wall bottom
	move	d1,d2
	mulu	chunkymodw(pc),d2
	move.l	a2,a4
	add.l	d5,a4
	add.l	d2,a4		; a4 = first reflected destination row
