...
solidstrip	set	0
drawstrip2	; v200 chunky-safe transparent wall strip overlay with darker true green alpha
	...
	; texels use the same packed-RGB 50% blend method as drawobjtrans:
	; background RGB (from planar_palette) blended with a darker green RGB12
	; tint, then remapped through planar_remap.  Also preserve the X flag
	; across the transparent blend path so near-camera texture stepping does
	; not break/clamp away parts of the door strips.
	movem.l	d0-d7/a0-a5,-(a7)
	move.l	vd_data(a4),d0
	beq	.rts
	move.l	d0,a0		; source texture column
	move	#$070,d6		; much darker green glass RGB12 tint
	move.l	chunkymod(pc),d7
	move	vd_y(a4),d2
...
.loop
	...
	move.b	0(a5,d5),(a1)
	bra.s	.next
.glass	move	sr,d1		; preserve X for addx texture stepping
	move	d6,d5
	and	d2,d5		; glass tint RGB12 (dark green)
	moveq	#0,d6
	move.b	(a1),d6		; already-rendered background pixel
	move	0(a3,d6*4),d6	; background RGB12
	and	d2,d6		; safe packed-RGB mask like drawobjtrans
	add	d6,d5
	lsr	#1,d5		; 50% background + 50% green glass
	move.b	0(a2,d5.w),(a1)
	move	d1,ccr		; restore X/NZVC before addx
.next	addx.l	d3,d0
	add.l	d7,a1
	dbf	d4,.loop
