...
.zero
	tst	d6
	beq.s	.advance
	; v190ci: emulate the old planar glass as real transparency.
	; Only every second destination sample is green-filtered; the others keep
	; the already-rendered scene unchanged.  This gives much higher light
	; transmission while still reading as dark green glass.
	move.l	a1,d5
	eor	d4,d5
	btst	#2,d5
	bne.s	.advance
...
.g2st_make_green
	; v190ci: dark, translucent green glass.  The LUT only supplies the
	; tinted half of a 50% checker; the other half remains the original scene.
	; Keep luminance but avoid the bright/neon green from v190cb.
	move	d0,d1	; red
	lsr	#8,d1
	and	#$000f,d1
	move	d0,d2	; green
	...
	move	d0,d6	; blue
	and	#$000f,d6
	move	d1,d5	; brightness = max(r,g,b), preserves lamps
	cmp	d2,d5
	bhs.s	.g2st_max_g_ok
	move	d2,d5
.g2st_max_g_ok
	...
	move	d6,d5
.g2st_max_b_ok
	lsr	#3,d1	; red  = 12.5% original
	lsr	#3,d6	; blue = 12.5% original
	move	d5,d2
	lsr	#1,d2	; green = 50% scene brightness
	addq	#1,d2	; tiny dark-green bias
	cmp	#7,d2
	bls.s	.g2st_green_ok
	moveq	#7,d2
.g2st_green_ok
