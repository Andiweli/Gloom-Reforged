...
.notroof
	...
	bsr	opton		;v109: cheat rows redraw only their line, no full-screen flicker
	bra	.loop
.notinv
	cmp	#8,d0
	bne.s	.notbouncy
...
.pok	move	(a2),d0
	...
	bra.s	copycntrltext
	;
cntrlright_noloop	;v109: title menu right arrow, no wrap and KEYBMOUSE only initial
	move	(a2),d0
	cmp	#1,d0
	bge.s	.r_has_nonkey
	moveq	#1,d0
	bra.s	.r_try
.r_has_nonkey	cmp	#5,d0
	bge.s	.rts
	addq	#1,d0
.r_try	cmp	(a3),d0
	bne.s	.r_set
...
.rts	rts
	;
cntrlleft_noloop	;v109: title menu left arrow, no wrap and no return to KEYBMOUSE
	move	(a2),d0
	cmp	#1,d0
	ble.s	.rts
	subq	#1,d0
