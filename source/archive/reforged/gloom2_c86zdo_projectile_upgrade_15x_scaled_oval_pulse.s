...
.wp_pulse
	...
	; c86zdo: weapon-upgrade pulse grows with proximity, but tops out
	; at about 1.5x.  Horizontal swell is edge-only in the draw path,
	; so it no longer looks like the same reflection drawn again outside.
	move	ob_y(a2),d1
	cmp	#-32,d1
	blt	.pick_pulse_done
	cmp	#-20,d1
	...
	move	d6,d1
	lsr	#1,d1
	add	d1,d0		;level 3: about 1.5x radius when very near
...
.oval_v_done
	; c86zdo: weapon-upgrade floor-touch pulse swells smoothly up to 1.5x.
	; g2_reflect_pulse is 0..3 from prepare: 1 ~= 1.125x, 2 ~= 1.25x,
	; 3 ~= 1.5x.  Keep height moderate; width is handled edge-only below.
...
.ryloop
		...
		bge	.ryskip
		move.b	d4,(a0)
		; c86zdo: pulse width is edge-only.  Earlier versions painted
		; +/- pixels from every column, which looked like a duplicate reflection
		; stamped beside the original.  Now only the outer half of the oval
		; extends one pixel outward, preserving a single rounded shape.
		tst	g2_reflect_pickup
		beq	.ryskip
		move	g2_reflect_pulse,d0
		beq	.ryskip
		...
		move	d1,d0
		sub	g2_shadow_cx(pc),d0
		beq	.ryskip
		bmi	.pulse_left_abs
		add	d0,d0		;right half: only outer half extends
		cmp	d2,d0
		ble	.ryskip
		cmp	#318,d1
		bgt	.ryskip
...
.pulse_left_abs
		neg	d0
		add	d0,d0		;left half: only outer half extends
		cmp	d2,d0
		ble	.ryskip
		cmp	#1,d1
		blt	.ryskip
...
g2_reflect_pulse	dc	0	;c86zdo smooth 0..3 weapon-upgrade floor-touch pulse up to 1.5x
