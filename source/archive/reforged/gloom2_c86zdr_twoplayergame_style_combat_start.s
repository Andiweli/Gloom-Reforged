	...
	;gloom2_c86zdr: TWO PLAYER COMBAT starts from the same clean persistent player/game state as TWO PLAYER GAME, then applies combat-specific maps/lives/collision. No ESC/dogamemenu hacks.
	;
...
.play	add	#49,d0
	...
	move	d0,p2lives
	;
	; c86zdr: mirror the persistent player/game-state reset from
	; TWO PLAYER GAME before entering combat.  Combat keeps its
	; selected lives and combat collision types, but must not inherit
	; health/weapon/reload/finished/escape/teleport state from a
	; previously started one-player or two-player game.
	move	#25,p1health
	move	#25,p2health
	move	#0,p1weapon
	move	#0,p2weapon
...
scriptplay	;
	...
	;
	bsr	pickcombat
	; c86zdr: after the combat intermission/map picker, clear the same
	; volatile game-exit state before loading the map.  This keeps a
	; previous normal game from leaking stale finished/escape/teleport
	; state into the combat round while leaving manual ESC handling intact.
	clr	finished
	clr	finished2
	clr	escape
	clr	doneflag
...
.doit	lea	joyx,a0
	jsr	readjoy1
	lea	joyx0,a0
	jsr	readkeys	; c86zdr build safety: logger/init growth can put readkeys out of BSR range
	move.l	joyx0(pc),d0
	or.l	d0,joyx
	move.l	joyb0(pc),d0
	or.l	d0,joyb
