	...
	;gloom2p96_c86z: earlier/lighter grounded enemy reflections; flying enemies fall back to blob shadows; broken wall-reflection probe disabled.
	;
...
.g2v190dm_floor_fog
	...
	jsr	g2fill_disabled_flat_fog
.nofloor	;
	; c86z: temporary disable the first wall-reflection probe again.
	; The c86x/c86y post-process version was effectively invisible and could
	; introduce black flickering vertical lines.  Keep the routine in the file
	; for later reactivation, but skip calling it for now.
...
.rh_max_ok
	...
	cmp	d1,d0
	bgt.s	.no_ref
	; c86z: grounded enemies reflect well.  Airborne/floating enemies still
	; looked wrong on pointed/uneven lower silhouettes, so for now suppress
	; their mirror entirely and keep only the blob shadow.
	move	ob_y(a2),d1
	bpl.s	.airgap_done
	bra.s	.no_ref
.airgap_done
...
.yloop
	cmp.l	a1,a0
	bhi	.rts
	; c86z: fade earlier and more strongly downward.  Keep the top still
	; recognisable, but thin the reflection sooner so it looks lighter and
	; dies out faster toward the bottom.
