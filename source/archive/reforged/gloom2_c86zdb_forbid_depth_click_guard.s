	...
	;gloom2_c86zdb: final hidden-pointer depth-click guard: SCREENQUIET/ShowTitle plus Forbid while gameplay input is active; keep grey-screen OpenScreen/OpenWindow guards, no refront/vwait trick.
	;
...
swaphflags	movem.l	floorflag,d0-d1	;c86zdb: absolute access, avoids GenAm PC-range growth after guard code
	move.l	d1,floorflag
...
finitdisplay	push
	;
	bsr	g2c86zdb_permit_input	;c86zdb: safety release in case shutdown bypassed inputoff
	;
	ifne	g2p96_overlay_enabled
...
newscreen_v	dc	0	;viewmode
	dc	$14f	;type ;c86zdb: +SCREENQUIET, suppress screen title/depth gadget while hidden pointer is active
	dc.l	0	;font
	dc.l	0	;title
	dc.l	0	;gadgets
...
oswindow	dc.l	0
msgport	dc.l	0
g2c86zdb_forbid_active	dc	0	;c86zdb: Exec Forbid held while gameplay input is active, to stop hidden mouse depth-gadget clicks

newtask	dcb.b	92,0

g2c86zdb_forbid_input
	movem.l	d0/a6,-(a7)
	tst	g2c86zdb_forbid_active
	bne.s	.rts
	move.l	4.w,a6
	jsr	-132(a6)	;Exec Forbid -- Intuition can not process screen depth gadget while game owns input
	move	#1,g2c86zdb_forbid_active
.rts	movem.l	(a7)+,d0/a6
	rts

g2c86zdb_permit_input
	movem.l	d0/a6,-(a7)
	tst	g2c86zdb_forbid_active
	beq.s	.rts
	clr	g2c86zdb_forbid_active
	move.l	4.w,a6
	jsr	-138(a6)	;Exec Permit -- release OS when leaving gameplay input
.rts	movem.l	(a7)+,d0/a6
	rts
...
.loop	move.l	(a3)+,(a2)+
	...
	move	#$c000,$dff09a
	st	active
	bsr	g2c86zdb_forbid_input	;c86zdb: keep Intuition from seeing hidden top-right depth-gadget clicks during gameplay
	;
...
.loop	move.l	a3,(a2)+
	...
	clr	active
	bsr	g2c86zdb_permit_input	;c86zdb: release OS only after game input/pointer state was restored
	;
.rts	rts
...
windowtask	move.l	int(pc),a6	;intuition base
	...
	move.l	d0,oswindow
	beq.s	.rts	;c86zdb: do not dereference a failed OpenWindow (grey screen + pointer guard)
...
.agasc	move.l	int(pc),a6
	...
	move.l	d0,a0
	move.l	a0,screen
	bne.s	.g2c86zdb_screen_ok
	clr.w	os	;c86zdb: failed OS screen, avoid null-screen gray/pointer hang by falling back to custom display path
	bra.w	.noos
.g2c86zdb_screen_ok
	move.l	int(pc),a6
	move.l	screen(pc),a0
	moveq	#0,d0
	jsr	-282(a6)	;c86zdb: ShowTitle(screen,FALSE), hide screen title/depth gadget if SCREENQUIET alone is not enough
