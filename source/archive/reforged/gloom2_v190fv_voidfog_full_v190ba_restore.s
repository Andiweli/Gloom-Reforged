...
g2fill_void_fog
	; v190fv2/v190ba: safe real-Amiga void fog after flats.
	; Keep the corridor/room openings dark like neighbouring walls,
	; but do not copy whole textured rows from the neighbouring wall column.
	; Instead, sample one dark non-zero colour from the neighbouring wall
	; column and use that as a uniform fog fill for the empty column.  This
	; avoids the stray horizontal texture bars seen in the dark far range.
	movem.l	d0-d7/a0-a6,-(a7)
	move.l	chunky(pc),a0
	tst.l	a0
	beq.w	.g2fv_done
...
; a3 = neighbouring solid source column top
; d4 = chunkymod
; Samples one non-zero pixel from the source column and uses that single
; colour to fill zero pixels in the destination column.
