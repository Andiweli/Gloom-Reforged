...
.rx_ok
	...
	; v190hx11: do not clamp floor-anchored pickup reflections into the
	; visible 3D window.  If the projected floor row has already moved
	; above/below the current VIEW SIZE render area, hide the reflection
	; instead of letting it slide along the edge.
	cmp	#2,d0
	blt	.no_reflect
	move	hite(pc),d1
	sub	#12,d1
...
.pick_anchor_b
	...
	sub.l	d7,a0
.clamp_yoff_done
	; v190hx11: top/bottom clip for all floor reflections.  The previous
	; code only clipped the lower status/gun edge; when VIEW SIZE was small
	; a reflection could stay visible while its floor anchor was already
	; outside the 3D render area.
	move.l	chunky(pc),d0
	move	g2_shadow_curx(pc),d4
	lsl	#2,d4
	lea	coloffs(pc),a1
