...
solidstrip	set	0
drawstrip2	; v191 chunky-safe transparent wall strip overlay with colour tint
	...
	; texels keep the scene visible, but now receive a sparse colour tint based
	; on the strip colour selector byte at -1(a0), like the old planar path.
	movem.l	d0-d7/a0-a5,-(a7)
	move.l	vd_data(a4),d0
	beq	.rts
	move.l	d0,a0		; source texture column
	moveq	#-1,d6		; default: no transparent tint
	moveq	#0,d5
	move.b	-1(a0),d5	; legacy strip colour selector: 0..6 = red..white
	cmp	#6,d5
	bhi.s	.notint
	lea	g2_stripglass_rgb(pc),a3
	move	0(a3,d5*2),d5
	move.l	planar_remap(pc),a3
	move.b	0(a3,d5),d6	; palette index for the glass tint
.notint
	move.l	chunkymod(pc),d7
	move	vd_y(a4),d2
...
.glass	tst	d6
	bmi.s	.next
	btst	#0,d4		; sparse checker tint so the scene stays visible behind glass
	bne.s	.next
	move.b	d6,(a1)
.next	addx.l	d3,d0
	add.l	d7,a1
