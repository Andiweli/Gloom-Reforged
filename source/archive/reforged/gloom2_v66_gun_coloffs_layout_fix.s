...
; remaps it once at startup.  The original gun uses colour index 1 as
; transparency, so g2gun_prepare changes raw 1 -> 0 before remapanim.
; v63 draws the already-remapped chunky indices directly and draws the
; gun before the statusbar, so the bar correctly sits in front of it.
g2drawgun
	movem.l	d0-d7/a0-a5,-(a7)
	move.l	gunpic(pc),d0
	beq.w	.g2dg_done
	move.l	d0,a0
	moveq	#12,d6		;shape #0 offset table entry
	move	g2gun_firetimer(pc),d7
	beq.s	.g2dg_shape_sel_done
	subq	#1,g2gun_firetimer	;show firing frame for a few frames
	moveq	#16,d6		;shape #1 = firing/muzzle frame
.g2dg_shape_sel_done
	move.l	0(a0,d6.w),d0
	beq.w	.g2dg_done
	cmp.l	#$20000,d0
	...
	bhi.w	.g2dg_done
	;
	; ZGloom-style placement: x = screen centre - xhandle.
	; v65: Y is raised versus v64 so the weapon sits higher behind
	; the statusbar like the original.
	move	#160,d1
	sub	d0,d1		;x
	move	#245,d4
	sub	d3,d4		;v65: raise gun 20px behind statusbar
	;
	; v63: ZGloom-style weapon bob while moving.  ob_bounce is advanced
	; by moveplayer; convert it to a small horizontal sway and lift the
	; weapon slightly on the swing.  No idle movement when standing still.
...
.g2dg_nobob
	;
	; clamp/skip if off-screen.  Current gun is y=204/h=61 => 36 visible rows.
	cmp	#240,d4
	bge.w	.g2dg_done
	move	#240,d5
...
.g2dg_vhok
	...
	ext.l	d1
	lea	coloffs,a2	;v66: chunky buffer uses C2P column offsets
	lea	0(a2,d1*4),a2	;do not write gun columns linearly
	lea	8(a0),a0		;source pixels
	moveq	#0,d0
	;
.g2dg_xloop
	move.l	a1,a3
	add.l	(a2)+,a3	;v66: same layout as drawchunky/statusbar
	move	d5,d7
.g2dg_yloop
	move.b	(a0)+,d0
	beq.s	.g2dg_skip
	move.b	d0,(a3)		;v64: gunpic already remapped by remapanim
.g2dg_skip
	lea	320(a3),a3
	dbf	d7,.g2dg_yloop
	adda.w	d6,a0		;skip clipped bottom rows
	;v66: destination x advances through coloffs(a2), not a linear +1
	dbf	d2,.g2dg_xloop
.g2dg_done
...
g2gun_prepare
	...
	move.l	a0,a6
	movem	(a6),d6-d7	;rotshift, frames
	lsl	d6,d7
	subq	#1,d7
	lea	an_size(a6),a2
.g2gp_shape
...
.nomega	bsr	shoot
	;
.shdone	move.l	(a7)+,a0
	move	#3,g2gun_firetimer	;v65 show muzzle/firing frame briefly
	move.l	(a0),a0
	moveq	#32,d0
	moveq	#0,d1
	bsr	playsfx
