	...
	;gloom2_c86zdv: fix TWO PLAYER COMBAT restart-after-game crash by reloading core player/token object assets because Combat bypasses script loadobjs.
	;
...
.p2	;
	bsr	permit
	;
	; c86zdv: TWO PLAYER COMBAT bypasses the normal script parser,
	; therefore it also bypasses exec_loadobjs.  After any previous
	; gameplay/combat exit, freeobjlist has freed the player/tokens
	; object graphics and cleared their objinfo pointers.  Reload the
	; core objects here while DOS is permitted, exactly like the boot
	; preload / script loadobjs path would do.
	cmp	#2,gametype
	bne.s	g2zdv_no_combat_obj_reload
	moveq	#0,d0
	jsr	loadanobj	; player1
	moveq	#1,d0
	jsr	loadanobj	; player2
	moveq	#2,d0
	jsr	loadanobj	; tokens/health
g2zdv_no_combat_obj_reload
	;
