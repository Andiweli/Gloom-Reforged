...
smallfont	move	#6,fontw
	move	#8,fonth
	move.l	smallfont_,font	;v108b: keep original planar font pointer semantics
	rts
...
; v112: discrete view sizes.  Left/right no longer wraps; it clamps at
; TINY/FULLSCREEN.  ALMOST FULL is deliberately shorter to avoid the old
; bottom artefacts just above the statusbar.
newwsize	cmp	#320,width
	bne.s	.nf
	cmp	#224,hite
	bcc	sizedone		;already FULLSCREEN, stop
.nf	move	width(pc),d0
	cmp	#96,d0
	bcs.s	.s96
	cmp	#128,d0
...
.s256	move	#256,width
	...
	bra	sizedone
.s288	move	#288,width
	move	#160,hite		;v112: ALMOST FULL-safe height
	bra	sizedone
...
decwsize	move	width(pc),d0
	cmp	#96,d0
	bls.s	.s64		;already TINY or below, stop
	cmp	#128,d0
	bls.s	.s96
	cmp	#160,d0
	bls.s	.s128
...
.g2amo_do
	...
	lea	0(a1,d0*8),a1
	move	#28,d0		;v80: ZGloom MakeShotFromMuzzle forward offset
	move	d0,d2
	muls	2(a1),d0
	add.l	d0,d0
	neg.l	d0
...
playerhit	tst	ob_damage(a0)
	beq.s	.rts
	tst	trainer_invincible	;v112: unlimited health cancels the visual hit flash/HUD flicker
	beq.s	.normal
	move	#25,ob_hitpoints(a5)
	rts
...
playerdie	;v112: unlimited health cheat also blocks lethal damage paths without HUD flash
	tst	trainer_invincible
	beq.s	.die
	move	#25,ob_hitpoints(a5)
	rts
...
weapond0	tst	trainer_weapon	;v112: forced trainer weapon/upgrade must not flicker on pickups
	bne	.trainer_skip
	tst	trainer_boost
	bne	.trainer_skip
	st	ob_update(a5)
...
playerlogic	;
	...
	jsr	trainer_maintain_one	;v111: permanent trainer options, including level/pickup override
...
.p1hok	move	p1health(pc),ob_hitpoints(a5)
	...
	;
.psk	jsr	resetplayer
	jsr	trainer_maintain_one	;v111: apply persistent trainer at level start
	;
	tst	gametype
	beq	.p1
	;
...
inccntrl	;v113: advance control, only KEYBMOUSE/KEYBOARD are mutually exclusive
	bra	cntrlright_noloop
	;
cntrl_allowed_v113	;a2=current control candidate in d0, a3=other player control ptr
	; allowed unless both players would use the shared key controls
	; 0=KEYBMOUSE, 1=KEYBOARD. Joysticks/pads may be shared.
	cmp	#2,d0
	bge.s	.ok
	cmp	#2,(a3)
	bge.s	.ok
...
.ok	moveq	#-1,d1
	rts
	;
cntrlright_noloop	;v113: right stops at CD32 PAD 2, skips shared-key conflict only
	move	(a2),d2
	cmp	#5,d2
	bge.s	.rts
	move	d2,d0
...
.rts	rts
	;
cntrlleft_noloop	;v113: left stops at KEYBMOUSE, skips shared-key conflict only
	move	(a2),d2
	tst	d2
	ble.s	.rts
	move	d2,d0
...
.rts	rts
	;
g2_title_sync_controls	;v113: rebuild title control text, repair invalid shared-key state
	movem.l	d0/a1,-(a7)
	move	p1_ob_cntrl,d0
	cmp	#0,d0
	blt.s	.p1_def
	cmp	#5,d0
	ble.s	.p1_ok
.p1_def	clr	p1_ob_cntrl
.p1_ok	move	p2_ob_cntrl,d0
	cmp	#0,d0
	blt.s	.p2_def
	cmp	#5,d0
	ble.s	.p2_ok
.p2_def	move	#2,p2_ob_cntrl
.p2_ok	move	p1_ob_cntrl,d0
	cmp	#2,d0
	bge.s	.no_shared_conflict
	move	p2_ob_cntrl,d0
	cmp	#2,d0
	bge.s	.no_shared_conflict
	move	#2,p2_ob_cntrl	;PLAYER 2 default/fallback = JOYSTICK 1
.no_shared_conflict	lea	p1ctype,a1
	move	p1_ob_cntrl,d0
	bsr.s	copycntrltext
	lea	p2ctype,a1
	move	p2_ob_cntrl,d0
...
copycntrltext	lea	popts(pc),a0
	move.l	0(a0,d0*4),a0
	moveq	#9,d1		;fixed 10-char control slot in packed menu table
.cpy	move.b	(a0)+,d2
	beq.s	.pad
	move.b	d2,(a1)+
	dbf	d1,.cpy
...
dointro	;
	...
	move.l	gloompal(pc),a1
	jsr	showpic
	jsr	g2v37_clear_title_topline	;v37: remove stale sprite/plane junk after returning from game
	jsr	g2v58_clear_title_lastline	;v58: remove stale very bottom title/menu line
	jsr	g2v36_hide_pointer	;v37: re-apply chip-RAM blank pointer after title display
	;
	; v42d diagnostic/safe path: skip the AGA gloombrush overlay.
	; The title showpic is stable, but decoding pics/gloombrush into the
	; OS/NewMode bitmap causes a hard reset/no Guru on the current test setup.
	nop	; v42e: fall through to .noaga (GenAm rejects 0-byte bra.s)
	;
	; Disabled original overlay block for now:
	;move	aga(pc),d0
	;beq.s	.noaga
...
.noaga	;
	...
	bsr	chaton
	;
	; v09: auto-enter the title menu. The current OS/NewMode input
	; path may not have activated yet, so do not wait forever before
	; drawing the menu. Force input on, then build the menu immediately.
	jsr	inputon
	nop
	;
	tst	linked
	bne.s	.use_linked_menu
	jsr	g2_title_sync_controls	;v112: stable PLAYER 1/2 text and no duplicate controls
	lea	startmenu,a4
	bra.s	.use
.use_linked_menu	lea	startmenu2,a4
.use	move	fonth(pc),d0	;v108: title menu from gloom.s sits a little higher
	add	d0,d0
	neg	d0
	move	d0,menu_yadjust
	jsr	initmenu
	clr	menu_yadjust
	; v108b: footer disabled for safety. The previous smallfont footer used
	; smallfont2/panel data through the planar printmess2 font path and could
	; crash as soon as the title menu was drawn. Re-enable later via a proper
	; smallfont2 shape/chunky-safe title footer path.
	nop
	;
.sel	jsr	selmenu
	move	d0,d7
	and	#$0300,d7	;v109: title menu left/right option changes
	and	#$00ff,d0
	;
	tst	linked
	beq.s	.notlinked
...
.title_fire	cmp	#3,d0
	bcs	.newgame
	cmp	#4,d0		;v108: startmenu now matches gloom.s with spacer rows
	bne.s	.notp1
.p1_arrow	lea	p1ctype(pc),a1
	lea	p1_ob_cntrl,a2
	lea	p2_ob_cntrl,a3
	cmp	#$0100,d7
	beq.s	.p1_left
	bsr	cntrlright_noloop
	bra	.sel
.p1_left	bsr	cntrlleft_noloop
	bra	.sel
.notp1	cmp	#5,d0
	bne.s	.notp2
.p2_arrow	lea	p2ctype(pc),a1
	lea	p2_ob_cntrl,a2
	lea	p1_ob_cntrl,a3
	cmp	#$0100,d7
	beq.s	.p2_left
...
.vile_arrow	tst	d7
	...
	subq	#1,mode
	bra.s	.vile_copy
.vile_right	cmp	#1,mode
	bge.s	.vile_copy
	addq	#1,mode
.vile_copy	move	mode(pc),d0
	lea	modes,a0
	move.l	0(a0,d0*4),a0
	lea	modetxt,a1
.moloop	move.b	(a0)+,(a1)+
	bne.s	.moloop
	bra	.sel
.notvile	cmp	#10,d0		;v114 original title menu ABOUT index
	bne	.notabout
	;
.about	;about text...
	;
...
.noch	move	(a7)+,numopts
	...
	bra	dointro
	;
.notabout	cmp	#12,d0		;v114 original title menu EXIT index
	bne	.sel
	;
...
print_reforged_footer	;v108: footer from gloom.s, but smallfont + grey
	movem.l	d0-d7/a0-a6,-(a7)
	move	fontw(pc),-(a7)
	move	fonth(pc),-(a7)
	move.l	font(pc),-(a7)
...
g2footer_make_gray
	movem.l	d0-d7/a0-a6,-(a7)
	bsr	g2footer_poke_gray_pal
	cmp	#3,bitplanes
	bcs.s	.done
	move.l	showbitmap(pc),d0
	beq.s	.done
	move.l	d0,a0
	move	g2footer_y(pc),d0
	mulu	linemodw(pc),d0
	adda.l	d0,a0
	move.l	a0,a1
	move.l	a0,a2
	move	bpmod(pc),d0
	ext.l	d0
	adda.l	d0,a1	;plane 1
	adda.l	d0,a2
	adda.l	d0,a2	;plane 2
	move	fonth(pc),d7
	subq	#1,d7
.row	moveq	#9,d6		;320 pixels = 40 bytes = 10 longs
.long	move.l	(a0),d0
	or.l	(a1),d0
	or.l	d0,(a2)
	addq.l	#4,a0
...
g2footer_poke_gray_pal
	movem.l	d0/a2,-(a7)
	tst	aga
	beq.s	.ecs
	move.l	coplist(pc),d0
	beq.s	.done
	move.l	d0,a2
	lea	palette_aga-copinit_aga(a2),a2
	move	#$666,26(a2)
...
gamemenu	dc.b	13
	dc.b	'CONTINUE',0
	dc.b	92,0
	; v109: no RESOLUTION row in gloom2.s; ':' value columns shifted 5 chars left.
...
reffooter_txt	dc.b	'GLOOM REFORGED V0.5 BY ANDIWELI',0
	even
...
startmenu	dc.b	13			;v114: restore stable original 13-row title menu, EXIT visible
...
p1ctype
	...
	dc.b	'CD32 PAD 1',0	;4
	elseif
	dc.b	'KEYBMOUSE  ',0	;4 fixed 10-char slot
	endc
	;
	dc.b	'PLAYER 2 '
p2ctype
	...
	dc.b	'CD32 PAD 2',0	;5
	elseif
	dc.b	'JOYSTICK 1',0	;5 PLAYER 2 default
	endc
	;
	dc.b	92,0			;6 spacer
...
menuy	dc	0
menu_yadjust	dc	0	;v108 title menu vertical adjustment
...
.doit	lea	joyx(pc),a0
	...
	beq.s	.noesc
	moveq	#$20,d0	;v108 ESC cancels/back in game menu
	rts
.noesc	qkey	$44
	bne.s	.fire
	bra.s	.encode
...
.encode	lea	joyx(pc),a0
	...
	and	#$1f,d0	;v108 game menu: left/right/up/down/fire
	rts
.normal_menu_mask	and	#$1f,d0	;v109: title menu also accepts left/right for option rows
	rts
...
menuskip	;return EQ if current item is a visual spacer/empty row
	...
	move.l	0(a0,d0*8),a0
	move.b	(a0),d0
	beq.s	.rts		;v111: empty/corrupt row is not selectable
	cmp.b	#92,d0
.rts	rts
...
.joygot	move	d0,-(a7)
	...
	move	(a7)+,d0
.joygot2	;
	btst	#5,d0	;v108 ESC cancel/back in game menu
	beq.s	.noescsel
	moveq	#0,d0
	rts
.noescsel	btst	#0,d0
	...
	;selected!
	;
	bsr	menuskip	;v110: visual spacer rows are never selectable
	beq.s	.down
	move	curropt(pc),d0
	rts
	;
...
abouttext	dc.b	16
	...
	dc.b	0
	dc.b	'GLOOM REFORGED V0.5 BY ANDIWELI',0
	even
