...
.rit	move.l	a5,g2_shape_owner	;v122 remember object for shapelist shadow tag
	jsr	(a0)
	clr.l	g2_shape_owner
	move.l	#drawobjnorm,shaperender
	bra.s	.loop
.notinvs	move.l	a5,g2_shape_owner	;v122 remember object for shapelist shadow tag
	jsr	(a0)
	clr.l	g2_shape_owner
	bra.s	.loop
.done	;
...
.loop2	move.l	(a5),a5
	...
	move	#$200,d7
	clr.l	g2_shape_owner	;v122 gore/body chunks are not enemy-shadow owners
	bsr	drawshape_q
	;
	bra.s	.loop2
	;
...
.noroof	;
	...
	jsr	g2log_drawstep
.nofloor	;
	; v122: old pre-sprite/object-list shadow pass disabled.  It was stable
	; but invisible because it did not use the final clipped sprite screen box.
	; The visible blob is now emitted from drawshapes using the already-built
	; shapelist entry and original object owner.
	;jsr	g2_draw_enemy_blobshadows
...
drawshape_q	;
	...
	move.l	g2_shape_owner,sh_prev(a1)	;v122 original object owner for blob shadows
	move	d7,sh_scale(a1)
...
; -----------------------------------------------------------------------------
; v122 visible enemy blob shadows.
; Called from drawshapes after clipping/scaling is finished, so the blob uses
; the same screen box as the enemy sprite.  sh_prev stores the original object
; owner only for this purpose.  Bullets/pickups/gore are rejected by owner/type.
; -----------------------------------------------------------------------------
g2_draw_enemy_blob_from_shape
	movem.l	d0-d7/a0-a6,-(a7)
	tst	g2_blobshadow
	ble	.done
	move.l	sh_prev(a6),a4
	tst.l	a4
	beq	.done
	...
	cmp.l	d5,a4
	beq	.done
	move.l	ob_render(a4),d5
	move.l	#drawshape_8,d6
	cmp.l	d6,d5
	bne	.done
	tst	ob_hitpoints(a4)
	ble	.done
	; screen centre x = clipped start column + clipped width/2
	move	d7,d0
	move	d3,d5
	lsr	#1,d5
	add	d5,d0
	; screen base y = top row from a1 + clipped height - small lift
	move.l	a1,d1
	sub.l	chunky(pc),d1
	...
	add	d4,d1
	subq	#2,d1
	; half width follows final sprite width, clamped for tiny/far enemies
	move	d3,d3
	lsr	#1,d3
	cmp	#4,d3
	bge	.minok
...
.minok	cmp	#30,d3
	ble	.maxok
	moveq	#30,d3
.maxok	; map a stable dark-grey RGB to the current chunky/C2P palette
	move	#$222,d4
	move.l	planar_remap(pc),a0
	move.b	0(a0,d4.w),d4
	and	#$00ff,d4
	...
	move.b	0(a0,d4.w),d4
	and	#$00ff,d4
	bsr	g2_draw_blob_oval
.done	movem.l	(a7)+,d0-d7/a0-a6
	rts

; -----------------------------------------------------------------------------
; v121 optional enemy blob shadows.
; Conservative path: iterate live drawshape_8 enemies after floor rendering and
; draw a small z-checked dark-grey oval directly into the chunky buffer.  This
; does not store anything in sh_prev and does not hook bullets/pickups.
; -----------------------------------------------------------------------------
g2_draw_enemy_blobshadows
	movem.l	d0-d7/a0-a6,-(a7)
	tst	g2_blobshadow
	ble	.done
	tst	floorflag
...
.loop	move.l	(a5),a5
	...
	cmp.l	d0,a5
	beq	.loop
	move.l	ob_render(a5),d0
	move.l	#drawshape_8,d1
	cmp.l	d1,d0
	bne	.loop
	tst	ob_hitpoints(a5)
	ble	.loop
	move.l	ob_shape(a5),d0
	lea	bullet1,a0
	move.l	a0,d1
	cmp.l	d1,d0
	beq	.loop
	...
	cmp.l	d1,d0
	beq	.loop
	bsr	g2_draw_enemy_blob_one
	bra	.loop
.done	movem.l	(a7)+,d0-d7/a0-a6
	rts

g2_draw_enemy_blob_one
	; project object floor contact point into screen space
	move	ob_x(a5),d0
	move	ob_y(a5),d1
	move	ob_z(a5),d2
	sub	camx(pc),d0
	sub	camy(pc),d1
	sub	camz(pc),d2
	move	d0,d6		;relative X
	move	d2,d7		;relative Z
	move	d0,d3
	muls	cm3(pc),d3
	muls	cm4(pc),d2
	add.l	d3,d2
	add.l	d2,d2
	swap	d2		;camera depth
	tst	d2
	ble	.rts
	cmp	#maxz,d2
	bcc	.rts
	...
	lsl.l	#focshft,d0
	divs	d2,d0
	add	midx(pc),d0	;screen centre X
	move	d1,d4
	ext.l	d4
	lsl.l	#focshft,d4
	divs	d2,d4
	add	midy(pc),d4	;floor/base Y
	move	d4,d1
	cmp	#-4,d1
	blt	.rts
	move	maxy(pc),d3
	...
	cmp	d3,d1
	bgt	.rts
	; halfwidth roughly follows object scale and distance
	move	ob_scale(a5),d3
	mulu	#30,d3
	divu	d2,d3
	cmp	#3,d3
...
.minok	cmp	#24,d3
	ble	.maxok
	moveq	#24,d3
.maxok	moveq	#1,d4		;dark grey/near-black chunky colour index
	bsr	g2_draw_blob_oval
.rts	rts
...
.ycdone2	move.l	d6,d1
	lsr.l	#1,d1
.ycdone	;
	; v122: draw optional enemy blob shadow from final clipped sprite box.
	; This makes the blob use the same screen position/scale as the sprite,
	; unlike v121's separate object-list projection pass.
	bsr	g2_draw_enemy_blob_from_shape
	;
	;draw bit...
	;
...
g2_blobshadow	dc	-1	;v116c menu flag only, render code removed
g2_reflections	dc	-1	;v116c menu flag only, render code removed
g2_shape_owner	dc.l	0	;v122 object owner copied into sh_prev for blob shadows
