	...
	;gloom2p96_c86p: c86o + 2P teleport duration match + floor/ceiling cfg mirror/save fix.
	;
...
.hloop	move.l	a1,a4
	...
	bsr	g2_enemy_reflection_capture_column	;c86q: keep source column for mirrored enemy reflection
.vloop	move.b	0(a3,d1),d5
	beq.s	.skip
	;
...
.g2c57_not_player_owner
	...
	cmp.l	#invincgot,d0
	beq	.setup_reflection
	; c86q: REFLECTIONS=YES also enables real enemy floor reflections.
	; Keep bullets/pickups on the old oval path above; only living colltype-0
	; monsters reach this branch.  REFLECTIONS=NO falls through to the old blob path.
	tst	ob_colltype(a2)
	bne	.try_blob
	tst	ob_hitpoints(a2)
	ble	.try_blob
	cmp	#3<<grdshft,d2
	bgt	.try_blob
	bsr	g2_prepare_enemy_reflection_column
	bra	.done
	bra	.try_blob
	;
.setup_reflection
...
; prepare mirrored floor reflection under living enemies.
; active=3 means enemy reflection. It deliberately reuses the existing
; per-sprite-column draw point, but stores a separate full reflection radius so
; optional blob shadows can keep their old narrow foot-oval look.
g2_prepare_enemy_reflection_column
	; d7=start screen column, d3=visible clipped sprite width, d2=depth
	move	d7,g2_shadow_curx
	move	d7,d0
	move	d3,d1
	lsr	#1,d1
	add	d1,d0
	move	d0,g2_shadow_cx
	; full visible sprite width for the mirrored reflection columns
	move	d3,d0
	lsr	#1,d0
	cmp	#4,d0
	bge	.erx_min_ok
...
.erx_max_ok
	...
	; old blob-shadow radius remains narrower if BLOB SHADOWS is also enabled
	move	d3,d0
	lsr	#2,d0
	cmp	#3,d0
	bge	.esrx_min_ok
...
.esrx_max_ok
	move	d0,g2_shadow_rx
	; anchor to projected floor plane, same as confirmed blob-shadow path
	move	camy(pc),d0
	neg	d0
	ext.l	d0
	asl.l	#focshft,d0
	...
	add	midy(pc),d0
	cmp	#2,d0
	blt	.no_enemy_reflect
	move	hite(pc),d1
	sub	#10,d1
	cmp	d1,d0
	bgt	.no_enemy_reflect
	move	d0,g2_reflect_floorrow
	move	#3,g2_shadow_active
	rts
.no_enemy_reflect
	clr	g2_shadow_active
	rts
...
; active=2 means projectile/pickup reflection, active=3 enemy mirror reflection, active=1 enemy blob shadow.
...
g2_draw_reflection_column_after_sprite
	...
	rts
.enemy_reflect
	bsr	g2_draw_enemy_mirror_reflection_column
.rts	rts

; c86q: capture the currently drawn sprite column before the main sprite loop
; consumes d1/d4. Used only for active enemy mirror reflections.
g2_enemy_reflection_capture_column
	cmp	#3,g2_shadow_active
	bne.s	.rts
	move.l	a3,g2_enemy_ref_src
	move.l	d1,g2_enemy_ref_srcy
	move.l	d6,g2_enemy_ref_step
	move	d4,g2_enemy_ref_hcount
	move.l	a2,g2_enemy_ref_pal
.rts	rts

; c86q: draw one mirrored, heavily dithered enemy sprite column below the floor.
; This is deliberately conservative: it uses the already clipped visible source
; column, starts at the projected floor row, mirrors upward sprite pixels downward,
; and fades with sparse Bayer-like writes instead of true alpha.
g2_draw_enemy_mirror_reflection_column
...
.abs_ok
	...
	cmp	d2,d1
	bgt	.rts
	move.l	g2_enemy_ref_src(pc),a3
	move.l	g2_enemy_ref_pal(pc),a2
	tst.l	a3
	beq	.rts
	tst.l	a2
	beq	.rts
	move	g2_enemy_ref_hcount(pc),d4
	blt	.rts
	; reflection height: visible sprite height, capped, with lower rows fading out
	move	d4,d5
	addq	#1,d5
	cmp	#36,d5
	ble.s	.hcap_ok
	moveq	#36,d5
.hcap_ok
	; bottom source sample ~= start + step * original visible height
	move.l	g2_enemy_ref_srcy(pc),d6
	move.l	g2_enemy_ref_step(pc),d7
	move	g2_enemy_ref_hcount(pc),d3
	blt.s	.src_done
.src_loop
	add.l	d7,d6
	dbf	d3,.src_loop
.src_done
	...
	mulu	chunkymodw(pc),d3
	add.l	d3,a0
	move.l	chunkymod(pc),d2
	moveq	#0,d0	; reflection output row index
.yloop
	; vertical fade: top rows 1/2 dither, middle 1/4, bottom 1/8.
	move	d0,d3
	cmp	#8,d3
	blt.s	.dither_half
	cmp	#20,d3
	blt.s	.dither_quarter
	move	g2_shadow_curx(pc),d3
	add	d0,d3
	and	#7,d3
	bne.s	.next_row
...
.next_row
	...
	cmp	d5,d0
	bge.s	.rts
	; safety: do not write into lower status/gun area
	move	g2_reflect_floorrow(pc),d3
	add	d0,d3
	move	hite(pc),d2
	sub	#10,d2
...
g2_enemy_ref_rx	dc	0	;c86q full sprite half-width for mirrored enemy reflections
g2_enemy_ref_hcount	dc	0
g2_enemy_ref_src	dc.l	0
g2_enemy_ref_srcy	dc.l	0
g2_enemy_ref_step	dc.l	0
g2_enemy_ref_pal	dc.l	0
...
p96prevlog_name	dc.b	'RAM:gloom_p96_preview.txt',0
	even
p96prevlog_ok	dc.b	'P96 preview: c86p P96 off 2P teleport duration cfgfix active',10
