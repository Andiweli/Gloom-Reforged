...
g2drawgun_muzzle
	...
	beq.w	.g2dm_done
	move.l	d0,a0
	; v71: animate two or more muzzle frames behind the gun.
	; firetimer was already decremented by g2drawgun: 1 -> shape #2, 0 -> shape #3.
	move	g2gun_firetimer(pc),d6
	moveq	#3,d7
	sub	d6,d7
	cmp	#2,d7
	bge.s	.g2dm_frame_ok
...
.g2dm_frame_maxok
	...
	bcc.w	.g2dm_done
	add.l	d0,a0
	; ZGloom places flash centred and at the bottom of the surface.
	; Drawing it before the gun lets the gun cover the lower flash part.
	move	4(a0),d0		;width
	move	#320,d1
	sub	d0,d1
	...
	move	6(a0),d3		;height
	move	#240,d4
	sub	d3,d4		;bottom aligned behind weapon/status area
	bsr	g2drawgun_coloffs_shape
.g2dm_done
	movem.l	(a7)+,d0-d7/a0-a5
	rts
...
.g2amo_do
	...
	lea	0(a1,d0*8),a1
	move	#28,d0		;forward offset from ZGloom
	move	d0,d2
	muls	2(a1),d0
	add.l	d0,d0
	neg.l	d0
	...
	add.l	d2,d2
	add.l	d2,ob_z(a0)
	; v71: only lift projectiles to just below the weapon, not screen centre.
	sub	#24,ob_y(a0)
.g2amo_done
	movem.l	(a7)+,d0-d2/a1
	rts
...
.nomega	bsr	shoot
	;
.shdone	move.l	(a7)+,a0
	move	#2,g2gun_firetimer	;v71: two-frame recoil/muzzle animation
	move.l	(a0),a0
	moveq	#32,d0
	moveq	#0,d1
	bsr	playsfx
