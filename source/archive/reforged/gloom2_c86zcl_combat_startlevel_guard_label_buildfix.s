...
.skhit	;
	...
	endc
.gotdata	;
	; c86zck: Combat does not use script/gloomgame here. The guard below
	; belongs to normalgame only; after returning from another game the
	; compatibility/script state must not block COMBAT.
	cmp	#2,gametype
	beq.s	g2zck_skip_dataguard
...
g2zck_skip_dataguard
	...
	;combat type game!
	;
	; c86zck: force COMBAT to be independent from optional ONE PLAYER
	; START LEVEL state. Combat chooses its own comX_Y map below.
	move.l	#-1,g2v190i_start_offset
	clr	g2v190hx_level_select_active
	clr.l	map_test
	move	#2,twowins
	...
	lea	combatmenu,a0
	bsr	qmenu
	jsr	g2zck_wait_release_any	; avoid carrying menu Fire/Return into series menu
	;
.loop	bsr	selmenu
	cmp	#3,d0
	bcs	.play
...
; c86zck: input debounce helpers for COMBAT menu/picture paths.
; Uses checkany so RETURN/ENTER and fire are treated exactly like the menus.
g2zck_wait_release_any
	movem.l	d0-d7/a0-a6,-(a7)
g2zck_wra_loop
	jsr	checkany
	bne.s	g2zck_wra_loop
	movem.l	(a7)+,d0-d7/a0-a6
	rts

g2zck_wait_fresh_any
	movem.l	d0-d7/a0-a6,-(a7)
g2zck_wfa_release
	jsr	checkany
	bne.s	g2zck_wfa_release
g2zck_wfa_press
	jsr	checkany
	beq.s	g2zck_wfa_press
g2zck_wfa_release2
	jsr	checkany
	bne.s	g2zck_wfa_release2
	movem.l	(a7)+,d0-d7/a0-a6
	rts
...
.loop	move.b	(a0)+,(a1)+
	...
	move.l	combatpal,a1
	jsr	showpic
	jsr	g2zck_wait_fresh_any	; c86zck: keep combat picture until fresh Enter/Fire
...
combatover	move.l	combat,a1
	freemem	combat
	clr.l	combat		; c86zck: avoid stale freed COMBAT IFF pointer on next start
	move.l	combatpal,a1
	freemem	combatpal
	clr.l	combatpal	; c86zck: avoid stale freed COMBAT palette pointer
...
.newgame	clr.l	map_test	; v190f: normal title starts are not forced-map tests
	; c86zck: optional START LEVEL belongs to ONE PLAYER only.
	; Do not let stale level-select state redirect TWO PLAYER or COMBAT.
	tst	d0
	bne.s	g2zck_plain_newgame
...
.g2v190cw_ngofs_ok
g2zck_newgame_maptest	move	d0,gametype
	bsr	qsync2
	bsr	chatoff
	bra	finitpmenu
...
p96prevlog_name	dc.b	'RAM:gloom_p96_preview.txt',0
	even
p96prevlog_ok	dc.b	'P96 preview: c86zck combat startlevel guard fix active',10
