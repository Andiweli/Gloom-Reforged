...
solidstrip	set	0
drawstrip2	; v190cd chunky-safe transparent wall strip overlay with inverted green glass
...
; v190cd: transparent wall strip colour filter LUTs for chunky renderer.
...
.g2st_make_green
	; v190cd/v190ce: dark green glass now inverts the scene seen through it.
	; Extract the 12-bit RGB behind the pane, invert it, then feed that
	; inverted brightness through the same dark green glass tint as v190cc.
	move	d0,d1	; red
	lsr	#8,d1
	and	#$000f,d1
	moveq	#15,d5
	sub	d1,d5
	move	d5,d1	; inverted red
	move	d0,d2	; green
	lsr	#4,d2
	and	#$000f,d2
	moveq	#15,d5
	sub	d2,d5
	move	d5,d2	; inverted green
	move	d0,d6	; blue
	and	#$000f,d6
	moveq	#15,d5
	sub	d6,d5
	move	d5,d6	; inverted blue
	move	d1,d5	; inverted brightness
	add	d2,d5
	add	d6,d5
	divu	#3,d5
	; Keep the pane dark: red/blue remain tiny, green is capped so the
	; negative effect is visible but does not become neon bright.
	lsr	#3,d1
	lsr	#3,d6
	add	d5,d2
	lsr	#2,d2
