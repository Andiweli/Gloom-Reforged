...
g2drawgun
	...
	cmp	#1,g2_game_profile	; v190gi: Classic Gloom has no first-person gun overlay
	beq.w	.g2dg_done
	move.l	gunpic(pc),d0
	beq.w	.g2dg_done
...
showstats
	;a5=player
	; v190gk: Classic Gloom has no Gloom2 smallfont2/panel anim.
	; Keep Classic in the chunky/C2P render path, but draw a safe built-in
	; top HUD fallback instead of touching the old smallfont.bin blitter data.
	cmp	#1,g2_game_profile
	bne.s	.g2hud_normal
	jmp	g2v190gk_showstats_g1
.g2hud_normal	; v190gi: clean restart from v190fy.
...
.notopclip	;
	...
	moveq	#0,d5
	cmp	#1,g2_game_profile
	bne.s	.g2v190gk_topcls_col_ok
	moveq	#0,d5
	move.b	g2v190gk_g1_black_col,d5
.g2v190gk_topcls_col_ok
.topcls	move.b	d5,(a1)
	add.l	d0,a1
	dbf	d2,.topcls
.notopcls	;
...
.vertskip	;
	...
	moveq	#0,d5
	cmp	#1,g2_game_profile
	bne.s	.g2v190gk_botcls_col_ok
	moveq	#0,d5
	move.b	g2v190gk_g1_black_col,d5
.g2v190gk_botcls_col_ok
.botcls	move.b	d5,(a1)
	add.l	d0,a1
	dbf	d1,.botcls
.rts	;
...
dointro	;
	; v190gj: Classic Gloom is now allowed into the shared menu path.
	; Profile 1 keeps the BlackMagic picture only: no unsupported notice,
	; no exit-after-notice, no optional title/gloom overlay.
	jsr	g2v145_show_clean_title
	bsr	dispon
...
.g2v190cj_no_remap
	; v190gk: only fall back to identity when no remap exists at all.
	; Classic Gloom now builds a synthetic remap after map load so the normal
	; distance-shade/fog tables can run instead of endless full-bright rendering.
	lea	palettes(pc),a2
	moveq	#15,d7
.g2v190cj_shade_loop
	move.l	(a2)+,a3
...
g1progfiles	dc.l	gloomcfg
	...
	even
	; v190gi: Classic Gloom has no gun overlay; do not require/load misc/gun.bin here.
	dc.l	titlemed+1
	dc.b	'sfxs/med1',0
	even
	dc.l	loadingmed+1
...
.not_zm
	...
	jsr	g2setup_g1_profile	; Classic Gloom: BlackMagic-only menu profile
	rts
...
g2setup_g1_profile
	...
	; v190gj: Classic Gloom always uses only pics/blackmagic as
	; title/menu backdrop. Do not switch to optional pics/title or
	; pics/gloom even if repacked data accidentally contains them.
	rts
...
.g2v190cv_clearplanes
	...
	; v190gk: draw the glyph into an explicit colour index instead of
	; assuming plane 1.  BlackMagic/title defaults to index 4; after a
	; Classic-Gloom map has loaded, the colour is switched to the best red
	; palette entry found in the live map palette so the in-game menu stays visible.
	moveq	#0,d5
	move.b	g2v190cv_menu_col,d5
	moveq	#0,d6	; current bitplane number
	move.l	a1,a2
	move	bitplanes,d7
	subq	#1,d7
.g2v190gk_colour_loop
	btst	d6,d5
	beq.s	.g2v190gk_colour_skip
	or.b	d0,(a2)
.g2v190gk_colour_skip
	addq	#1,d6
	add.l	d4,a2
	dbf	d7,.g2v190gk_colour_loop
.g2v190cv_nextline
	lea	40(a0),a0
	adda.w	linemodw,a1
...
; -----------------------------------------------------------------------------
; v190dl legacy: original Gloom unsupported notice (unused since v190gj)
; -----------------------------------------------------------------------------
; Kept in the file to avoid unnecessary code/data movement. The v190gj start
; path no longer calls this screen; Classic Gloom enters the BlackMagic menu.
...
g2v190cx_build_g1_tables
	...
	bne.s	.g2v190cx_done
	movem.l	d0-d7/a0-a4,-(a7)
	jsr	g2v190gk_find_g1_colours
	lea	g2v190cx_g1_remap,a0
	moveq	#0,d0
	move.b	g2v190gk_g1_black_col,d0
	move	#4095,d7
.g2v190cx_clear
	move.b	d0,(a0)+
...
.g2v190cx_built
	...
	moveq	#0,d0
	move.b	g2v190gk_g1_red_col,d0
	move.b	d0,g2v190cv_menu_col	; in-game Classic menu: visible red text
	movem.l	(a7)+,d0-d7/a0-a4
.g2v190cx_done
	rts
...
; -----------------------------------------------------------------------------
; v190gk: Classic Gloom safe top HUD + palette/fog colour helpers
; -----------------------------------------------------------------------------
; These helpers are used only for profile 1. They deliberately stay in the
; chunky/C2P path and do not touch the SoundFX path, wall renderer, ADVANCED
; fog logic, or the Gloom2/Gloom3/ZM HUD/menu paths.

g2v190gk_showstats_g1	; a5=player, Classic Gloom only
	movem.l	d0-d7/a0-a5,-(a7)
	move.l	chunky,d0
	beq.w	.g2g1hud_done
	moveq	#2,d7
	...
	jsr	g2v190gk_draw_text_chunky
	;
	; HEALTH bar: compact Gloom-style bar beside the label.
	move	ob_hitpoints(a5),d7
	ble.s	.g2g1hud_nohp
	cmp	#25,d7
	ble.s	.g2g1hud_hp_ok
...
.g2g1hud_hploop
	...
	moveq	#5,d3
	moveq	#0,d5
	move.b	g2v190gk_g1_red_col,d5
	jsr	g2v190gk_fill_rect
	addq	#2,d6
	dbf	d7,.g2g1hud_hploop
.g2g1hud_nohp
	;
	; WEAPON/reload bar.
	moveq	#5,d7
...
.g2g1hud_wploop
	...
	jsr	g2v190gk_fill_rect
	add	#10,d6
	dbf	d7,.g2g1hud_wploop
.g2g1hud_lives
	move	ob_lives(a5),d7
	ble.s	.g2g1hud_done
	cmp	#5,d7
	ble.s	.g2g1hud_lives_ok
	moveq	#5,d7
.g2g1hud_lives_ok
	subq	#1,d7
	move	#276,d6
.g2g1hud_liveloop
	...
	jsr	g2v190gk_fill_rect
	addq	#8,d6
	dbf	d7,.g2g1hud_liveloop
.g2g1hud_done
	movem.l	(a7)+,d0-d7/a0-a5
	rts

g2v190gk_draw_text_chunky	; a4=text, d7=x, d6=y
	movem.l	d0-d7/a0-a4,-(a7)
.g2g1txt_loop
	move.b	(a4)+,d0
	beq.s	.g2g1txt_done
	...
	moveq	#0,d5
	move.b	g2v190gk_g1_red_col,d5
	jsr	g2v190gk_draw_chatglyph_chunky
.g2g1txt_space
	addq	#6,d7
	bra.s	.g2g1txt_loop
.g2g1txt_done
...
g2v190gk_draw_chatglyph_chunky	; d0=glyph, d7=x, d6=y, d5=colour
	movem.l	d0-d7/a0-a4,-(a7)
	lea	chatfont,a0
	adda.w	d0,a0
	move.l	chunky,a1
	tst.l	a1
	beq.w	.g2g1glyph_done
	move	d6,d1
	mulu	#320,d1
	add.l	d1,a1
	lea	coloffs,a2
...
.g2g1glyph_y
	moveq	#0,d2
	move.b	(a0),d2
	beq.s	.g2g1glyph_nextrow
	move	d7,d0	; current X
	moveq	#7,d4
.g2g1glyph_x
	btst	d4,d2
	beq.s	.g2g1glyph_skip
	cmp	#0,d0
	blt.s	.g2g1glyph_skip
	cmp	#319,d0
	bgt.s	.g2g1glyph_skip
	move.l	0(a2,d0*4),d3
	move.b	d5,0(a1,d3.l)
.g2g1glyph_skip
	addq	#1,d0
...
g2v190gk_fill_rect	; d0=x,d1=y,d2=w,d3=h,d5=colour
	movem.l	d0-d7/a0-a3,-(a7)
	tst	d2
	ble.w	.g2g1fr_done
	tst	d3
...
g2v190gk_find_g1_colours
	movem.l	d0-d7/a0-a1,-(a7)
	move.b	#0,g2v190gk_g1_black_col
	move.b	#2,g2v190gk_g1_red_col
	move	#999,d5	; best/darkest RGB sum
	moveq	#0,d6	; darkest palette index
	move	#-32768,d7	; best red score
	move.l	map_rgbs,a0
	move.l	map_rgbsat,a1
...
.g2g1col_loop
	...
	move	d0,d4	; B
	and	#$000f,d4
	move	d1,d0	; darkness score R+G+B, lower is better
	add	d2,d0
	add	d4,d0
	cmp	d5,d0
	bge.s	.g2g1col_notdark
	move	d0,d5
	move	d3,d6
.g2g1col_notdark
	move	d1,d0	; red score = 4R-G-B
	add	d0,d0
	add	d1,d0
	add	d1,d0
	sub	d2,d0
	...
	ble.s	.g2g1col_next
	move	d0,d7
	move.b	d3,g2v190gk_g1_red_col
.g2g1col_next
	addq	#1,d3
	bra.s	.g2g1col_loop
.g2g1col_done
	move.b	d6,g2v190gk_g1_black_col
	movem.l	(a7)+,d0-d7/a0-a1
	rts

	even
g2v190gk_g1_black_col	dc.b	0
g2v190gk_g1_red_col	dc.b	2
g2v190cv_menu_col	dc.b	4	; title BlackMagic default: try blue index; map changes it to red
	even
