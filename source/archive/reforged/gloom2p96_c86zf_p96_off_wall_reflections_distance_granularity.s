...
.row_loop
	; ordered fade: clearly visible near the wall-foot, then thinning out.
	; c86zd: also thin reflections with distance so fog covers them similarly
	; to the wall/enemy rendering instead of leaving distant mirrored columns
	; too readable.
	moveq	#10,d3		; top: 10/16 pixels kept
	cmp	#2,d5
	blt.s	.have_thr
	moveq	#7,d3		; upper mid: 7/16
	cmp	#5,d5
	blt.s	.have_thr
	moveq	#4,d3		; lower: 4/16
	cmp	#8,d5
	blt.s	.have_thr
	moveq	#2,d3		; tail: 2/16
.have_thr
	move	vd_z(a0),d1
	tst	g2_visibility
	bgt.s	.fog_adv
...
.fog_done
	...
	bra	.skip_px
.fog_keep
	; c86zf: make distant wall-reflection pixels coarser.  Near walls keep
	; the fine per-pixel Bayer pattern, medium distance groups into 2x2-ish
	; blocks, and far distance into larger blocks.  This avoids the "same tiny
	; dither everywhere" look that read as a graphics glitch in the distance.
	move	vd_z(a0),d2
	move	d4,d0
	move	d5,d7
	tst	g2_visibility
