...
pixelate	movem.l	d0-d7/a0-a5,-(a7)
	; v98: conservative crash-safe teleport/death pixelate.
	; v97 half-block/64px ZGloom clone was too risky in the real teleport path.
	; Keep the visual effect, but clamp hard and never write outside the active
	; 320x224 game viewport.  No NewMode/P96/LOD changes here.
	tst.l	chunky(pc)
	beq.w	.done
	move	width(pc),d0
	ble.w	.done
	...
	cmp	#2,d6
	blt.w	.done
	cmp	#16,d6		;small cap: stable, visible, avoids huge risky blocks
	ble.s	.sizeok
	move	#16,d6
...
.yfill	move	d1,d0
	...
	move	d2,d3		;current X inside this block
.xfill	move.l	a2,a3
	move.l	0(a1,d3*4),d6
	add.l	d6,a3
	move.b	d7,(a3)
