	...
	;gloom2p96_c86j: P96 overlay off; TWO PLAYER no-stretch width steps and screen-edge HUD.
	;
...
g2twop_prepare_view_width
	; c86j: remove the c86h/i wide stretch path again.  TWO PLAYER now
	; stays in the height-locked projection for every WINDOW SIZE step,
	; because that is the only path that kept the proportions correct.
	; Width is allowed to grow a little past c86g/c86c, but is capped at
	; the original gloom.s high-res 2P maximum of 180 physical pixels
	; (90 logical chixels at 2x).  Wider true FOV needs a larger castrots
	; table or a real low-res 2x render/expand path later.
	...
	bcs.s	.vhuge
	bra.s	.full
.tiny	move	#132,d0	;gloom.s 66*2 baseline
	bra.s	.store
.small	move	#144,d0
	bra.s	.store
.medium	move	#156,d0
...
.large	move	#168,d0
	bra.s	.store
.xlarge	move	#180,d0	;gloom.s high-res 2P maximum
	bra.s	.store
.huge	move	#180,d0
	bra.s	.store
.vhuge	move	#180,d0
...
g2twop_set_half_view
	; c86j: physical split half remains 320x120.  The selected 3D
	; view is centred inside it, but the projection is always tied to
	; the 120px split height.  This prevents the c86h/i stretch/squash
	; regression when WINDOW SIZE changes.
...
; c86d: temporarily map logical render columns 0..selected_width-1 to a
; centred physical region inside the 320-pixel split half.  FULLSCREEN maps
; to the whole width; smaller WINDOW SIZE values leave black side borders.
