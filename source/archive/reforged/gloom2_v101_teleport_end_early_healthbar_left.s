...
; into the weapon body; crop only the first two source columns for the gun.
g2drawgun_coloffs_shape_crop_left
	...
	cmp	#240,d4
	bge.w	.g2dgcl_done
	addq	#2,d1		;skip two transparent-padding columns on screen
	subq	#2,d2		;and in source width
	move	#240,d5
	sub	d4,d5
	ble.w	.g2dgcl_done
	cmp	d3,d5
...
.g2dgcl_vhok
	...
	move	d3,d0
	add	d0,d0
	adda.w	d0,a0		;skip two column-major source columns
	move.l	palettes(pc),a4
	moveq	#0,d0
.g2dgcl_xloop
	move.l	a1,a3
...
.notp	;
	...
	bra.s	.notpix
	;
.pixnz	cmp	#18,ob_pixsize(a5)	;v101: end while pixels are still visible, before full-blue
	bne.s	.notpix
	;
	...
	; v101: finish teleport before the effect reaches full blue.  The last
	; visible frame still has chunky pixels on screen, so the transition ends
	; while it is clearly teleporting instead of waiting for a solid blue wash.
	clr	ob_pixsize(a5)
	clr	ob_pixsizeadd(a5)
.notpix	;
	move	ob_hyper(a5),d0
