	...
	;gloom2_c86zcp: projectile/upgrade reflections draw behind their bitmap; health pickups also get reflections.
	;gloom2_c86zcq: robust token/pickup reflection classifier for health/thermo/invisi/invinc/bouncy.
	;gloom2_c86zcr: final pickup-reflection attempt: broad collectible classifier plus clamped floor anchor.
	;
...
swaphflags	movem.l	floorflag,d0-d1
	move.l	d1,floorflag
...
.g2c57_not_player_owner
	...
	beq	.setup_reflection
	; c86zcq: token pickups are not always safely covered by the narrow
	; old hit-routine list.  Use the shared classifier here so healthkits,
	; thermo/infra goggles, invisibility, invincibility and bouncy tokens all
	; enter the same behind-sprite reflection path as weapon pickups.
	bsr	g2_reflect_owner_is_pickup
	tst	d0
	bne	.setup_reflection
...
.rx_ok
	...
	; c86zcr: last pickup-reflection attempt.  Token pickups can project
	; very close to the lower VIEW SIZE edge; rejecting that anchor made
	; health/thermo/invisi/invinc reflections disappear.  Clamp the pickup
	; floor anchor into the drawable floor area instead of dropping it.
	cmp	#2,d0
	bge.s	.pick_floor_min_ok
	moveq	#2,d0
.pick_floor_min_ok
	move	hite(pc),d1
	sub	#12,d1
	cmp	d1,d0
	ble.s	.pick_floor_store
	move	d1,d0
.pick_floor_store
...
; v169/c86zcq: pickup classifier used only by the reflection path.
; Do not depend on one single field: weapon upgrades use weaponlogic, bouncy
; uses its own logic, token pickups often use plain rts logic and are most
; reliably recognized by hit/die routine or by the shared tokens shape.
g2_reflect_owner_is_pickup
	...
	beq.s	.yes
	move.l	ob_hit(a2),d0
	bsr.s	g2_reflect_hit_is_pickup
	tst	d0
	bne.s	.yes
	move.l	ob_die(a2),d0
	bsr.s	g2_reflect_hit_is_pickup
	tst	d0
	bne.s	.yes
	move.l	ob_shape(a2),d0
	cmp.l	#tokens,d0
	beq.s	.yes
	; c86zcr: broad collectible fallback.  Runtime ob_shape is often the
	; loaded animation pointer, not the original tokens descriptor.  Health,
	; thermo, invisibility, invincibility and similar pickups are passive
	; colltype-0 objects that collide with players (24) and have no HP.
	cmp	#24,ob_collwith(a2)
	bne.s	.no
	tst	ob_colltype(a2)
	bne.s	.no
	tst	ob_hitpoints(a2)
	bne.s	.no
	move.l	ob_render(a2),d0
	cmp.l	#drawshape_1,d0
	beq.s	.yes
.no	moveq	#0,d0
	rts
.yes	moveq	#1,d0
...
; c86zcq: shared hit/die routine test for token pickups.
g2_reflect_hit_is_pickup
	cmp.l	#healthgot,d0
	beq.s	.yes
	cmp.l	#bouncygot,d0
	beq.s	.yes
...
g2_reflection_colour
	...
	cmp.l	#bouncygot,d1
	beq	.got_bouncy
	; c86zcq: if the active object has had hit altered, fall back to die
	; routine and finally to generic token colouring.
	move.l	ob_die(a2),d1
	cmp.l	#healthgot,d1
	beq	.got_health
	cmp.l	#thermogot,d1
...
.got_bouncy	moveq	#1,d2	; green-ish
	bra	.got_weapon
.got_health	moveq	#5,d2	; warm health-kit glow
	bra	.got_weapon
...
.got_invinc	moveq	#0,d2	; yellow-ish
	bra	.got_weapon
.got_token	moveq	#2,d2	; generic token/powerup fallback
.got_weapon
	and	#7,d2
...
; c86zcp: projectile/upgrade/pickup reflections are drawn before the owning
; bitmap column, so pickup animation pixels and projectile sprites stay in
; front of their reflection.  For projectiles the existing reflection routine
; expects a4 one row below the sprite column, so temporarily synthesize that
; pointer from the current top pointer and the still-intact DBF height d4.
...
; c86zcp: coloured projectile/upgrade/pickup reflections now happen before
; sprites.  The after-sprite pass is kept only for true mirrored enemy/player
