...
.play	add	#49,d0
	...
	move	d0,p2lives
	;
	; c86zcm: HARD FIX from gloom.s comparison + current gloom2 memory model.
	; In this branch the Combat IFF/palette are loaded while the title art is
	; still resident.  After one previous game gloom2 reloads title assets before
	; dointro, then Combat loads an extra picture/palette on top of them.  That
	; can make the following Combat picture/tile/map load fail or leave invalid
	; state before pickcombat.  ONE/TWO PLAYER do not load this extra Combat IFF.
	; Free title art immediately after leaving the Combat menu, before loading
	; pics/combat.  The main .play path calls the same routine again and it is
	; already guarded, so this is idempotent.
	jsr	g2v190p_free_title_assets
	;
...
; c86zc9/c86zcm: Combat picture must wait for a fresh press, not the
; Fire/Return that selected the Combat menu or killed the previous player.
