	...
	;gloom2p96_c53: suspend P96 preview/window/buffers completely to diagnose TWO PLAYER split-screen/Guru.
	;
...
.play	;
	...
	tst	gametype
	bmi	.intro2
	;gloom2p96_c53: keep P96 preview inactive; normal AGA gameplay only.
...
.done	jsr	g2cfg_save	;v141: save ingame menu settings when leaving menu
	...
	jsr	predrawall
	jsr	dispon
	;gloom2p96_c53: CONTINUE keeps P96 suspended.
	clr	p96preview_gameactive
	bra.s	.findone
.finish_exit
	jsr	dispoff
...
.osskipz	;
	...
	;
	;gloom2p96_c53: P96 is suspended for this diagnostic build.  Do not
	;allocate any preview/RGB staging buffers so TWO PLAYER can be tested with
	;the normal game memory/layout as closely as possible.
...
p96prevlog_name	dc.b	'RAM:gloom_p96_preview.txt',0
	even
p96prevlog_ok	dc.b	'P96 preview: c53 P96 suspended twoplayer diagnose active',10
...
g2p96_preview_frame	push
	;gloom2p96_c53: hard off-switch.  The function is still callable from
	;drawall/vwait, but it must not touch the P96/Workbench window while
	;we diagnose TWO PLAYER split-screen and second-start Guru.
	bra.w	.done
...
initdisplay	;
	...
	jsr	-408(a6)
	move.l	d0,grbase
	;gloom2p96_c53: P96 library probe disabled for TWO PLAYER/splitscreen diagnosis.
	clr	p96present
	clr	p96found
	clr.l	p96base
	;
...
.agasc	move.l	int(pc),a6
	...
	move.l	d0,dbufinfo
	;gloom2p96_c53: no P96 preview window in this diagnostic build.
	clr.l	p96preview_window
	clr	p96preview_state
	bra	db
	;
