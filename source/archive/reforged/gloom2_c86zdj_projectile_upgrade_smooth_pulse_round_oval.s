...
.wp_min_ok
	...
	moveq	#16,d0
.wp_pulse
	; c86zdj: weapon-upgrade pulse ramps smoothly instead of jumping.
	; The bob is clamped at about -20px above the floor, so combine the
	; near-floor y range with the animation phase and grow the radius in
	; three short steps: about 1.25x, 1.5x, then 2x.
	move	ob_y(a2),d1
	cmp	#-32,d1
	...
	move	d0,d1
	add	d1,d0		;level 3: 2.0x radius
	bra.s	.wp_pulse_cap
.wp_pulse_l1
	move	#1,g2_reflect_pulse
	move	d0,d1
	lsr	#2,d1
	add	d1,d0		;level 1: about 1.25x radius
	bra.s	.wp_pulse_cap
.wp_pulse_l2
	move	#2,g2_reflect_pulse
	move	d0,d1
	lsr	#1,d1
	add	d1,d0		;level 2: about 1.5x radius
.wp_pulse_cap
	cmp	#32,d0
...
.oval_v_done
	; c86zdj: weapon-upgrade floor-touch pulse swells smoothly up to 2x.
	; g2_reflect_pulse is 0..3 from prepare: 1 ~= 1.25x, 2 ~= 1.5x,
	; 3 ~= 2x.  Apply height changes gradually to keep the oval round.
	...
	moveq	#0,d5
	moveq	#7,d6		;pulse core level 1: 8 rows
	cmp	#2,d0
	blt.s	.oval_pulse_done
	moveq	#9,d6		;pulse core level 2: 10 rows
	cmp	#3,d0
	blt.s	.oval_pulse_done
	moveq	#11,d6		;pulse core level 3: 12 rows
	bra.s	.oval_pulse_done
.oval_pulse_inner
	moveq	#0,d5
	moveq	#5,d6		;pulse inner level 1: 6 rows
	cmp	#2,d0
	blt.s	.oval_pulse_done
	moveq	#6,d6		;pulse inner level 2: 7 rows
	cmp	#3,d0
	blt.s	.oval_pulse_done
	moveq	#8,d6		;pulse inner level 3: 9 rows
	bra.s	.oval_pulse_done
.oval_pulse_mid
	moveq	#1,d5
	moveq	#3,d6		;pulse mid level 1: 4 rows
	cmp	#2,d0
	blt.s	.oval_pulse_done
	moveq	#4,d6		;pulse mid level 2: 5 rows
	cmp	#3,d0
	blt.s	.oval_pulse_done
	moveq	#5,d6		;pulse mid level 3: 6 rows
	bra.s	.oval_pulse_done
.oval_pulse_outer
	moveq	#1,d5
...
g2_reflect_pulse	dc	0	;c86zdj smooth 0..3 weapon-upgrade floor-touch pulse up to 2x
