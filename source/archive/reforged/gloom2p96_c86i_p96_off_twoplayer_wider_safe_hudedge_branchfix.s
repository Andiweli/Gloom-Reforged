	...
	;gloom2p96_c86i: P96 overlay off; TWO PLAYER wider safe mixed-FOV and screen-edge HUD branchfix.
	;
...
g2twop_prepare_view_width
	; c86h: map the normal one-player WINDOW SIZE steps to a wider
	; two-player split view.  Up to 160px we keep the proven 4:3-ish
	; height-locked projection.  Above 160px we switch to the safe
	; full-FOV caster path so castrots is never addressed outside its
	; original -160..+159 range.  This gives EXTRA LARGE..FULLSCREEN
	; visible width changes without bringing back the broken side-wall
	; zones from c86f.
...
.small	move	#156,d0
	bra.s	.store_narrow
.medium	move	#180,d0	;gloom.s high-res maximum
	bra.s	.store_wide
.large	move	#204,d0
	bra.s	.store_wide
.xlarge	move	#228,d0
...
.vhuge	move	#276,d0
	bra.s	.store_wide
.full	move	#320,d0
	; c86i: fall through to .store_wide; bra.s to next label is 0-byte and GenAm rejects it
.store_wide
	move	#-1,g2twop_wide_mode
.store_narrow
	move	d0,g2twop_view_width
	rts

g2twop_set_half_view
	; c86h: physical split half remains 320x120.  The 3D view width
	; follows WINDOW SIZE.  Narrow modes use the height-locked projection
	; that fixed the squeezed 4:3 look.  Wider modes use the safe normal
	; full-FOV caster path to avoid invalid castrots lookups at the sides.
	...
	move	#60,maxy
	clr.l	offset
	tst	g2twop_wide_mode
	bne.s	.wide
	move	#-1,g2twop_aspect_mode
	rts
.wide	clr	g2twop_aspect_mode
	rts
...
p96prevlog_name	dc.b	'RAM:gloom_p96_preview.txt',0
	even
p96prevlog_ok	dc.b	'P96 preview: c86i p96 off twop wider safe HUD edge branchfix active',10
