...
solidstrip	set	0
drawstrip2	; v190ac chunky transparent wall strip overlay using original strip colour selector
	; a1=top of destination column, a2=palettes base, a4=strip/vd data
	; Original gloom.s does not make every transparent wall strip green.  It
	; uses the colour selector byte stored directly before the column data
	; (-1(a0)) and the old stripands masks.  In chunky mode we mirror that
	; behaviour by only tinting zero texels with the selector colour. Selector
	; 6/invalid means clear glass/no extra green tint.
	movem.l	d0-d7/a0-a5,-(a7)
	move.l	vd_data(a4),d0
	beq	.rts
	move.l	d0,a0		; source texture column, -1(a0)=strip colour selector
	moveq	#0,d6
	move.b	-1(a0),d6
	cmp	#5,d6		; 0..5 = coloured glass, 6 = white/no tint like stripands $ffff
	bhi.s	.notint
	add	d6,d6
...
.hiteok
	...
	move	vd_pal(a4),d5
	move.l	0(a2,d5*4),a5
	move	#$0eee,d2		; packed-RGB even mask like drawobjtrans
...
.glass	addx.l	d3,d0		; advance before RGB blend clobbers CCR/X
	tst	d6
	bmi.s	.next		; selector 6/invalid: leave background untouched
	move	d6,d5
	and	d2,d5		; selector colour RGB12
	moveq	#0,d1
	move.b	(a1),d1		; already-rendered background pixel
	move	0(a3,d1*4),d1	; background RGB12
	and	d2,d1
	add	d1,d5
	lsr	#1,d5		; 50% background + 50% selector tint
	move.b	0(a2,d5.w),(a1)
.next	add.l	d7,a1
	dbf	d4,.loop
...
g2_stripglass_rgb	; red,green,blue,yellow,purple,cyan selector tints
	dc	$600,$060,$006,$660,$606,$066
	;
stripands	;red,green,blue,yel,pur,cyn,wht (legacy planar table removed from use)
...
finitfontpal	tst	g2_fontpal_active
	beq.s	.rts
	clr	g2_fontpal_active
	bra	pokelastpal
.rts	rts

g2_fontpal_active	dc	0
...
initmenu2	;
	...
	clr	g2_fontpal_active
	cmp.l	#gamemenu,a4
	beq.s	.g2_no_fontpal	; v190ac: do not recolour live game palette behind in-game menu
	bsr	initfontpal
	move	#-1,g2_fontpal_active
.g2_no_fontpal
	;
