	...
	;gloom2_c86zcc: clean Combat fix: fresh wait on Combat picture, other-player reflection, restore gloom.s ob_window setup.
	;
...
.g2c57_not_player_owner
	...
	tst	g2_reflections
	ble	g2c86zce_try_blob
	move.l	ob_logic(a2),d0
	cmp.l	#firelogic,d0
	beq	g2c86zce_setup_reflection
	...
	tst	ob_colltype(a2)
	bne	g2c86zce_try_blob
	tst	ob_hitpoints(a2)
	ble	g2c86zce_try_blob
	cmp	#3<<grdshft,d2
	bgt	g2c86zce_try_blob
	bsr	g2_prepare_enemy_reflection_safe
	bra	g2c86zce_try_blob
	;
g2c86zce_setup_reflection
	bsr	g2_prepare_reflection_column
	bra	g2c86zce_shadow_done
	;
.g2c57_player_try_blob
		; c86zcc: in TWO PLAYER the visible other player is drawn through the
		...
		tst	g2_reflections
		ble.s	g2zcc_player_blob_only
		tst	ob_hitpoints(a2)
		ble.s	g2zcc_player_blob_only
		cmp	#3<<grdshft,d2
		bgt.s	g2zcc_player_blob_only
		bsr	g2_prepare_enemy_reflection_safe
g2zcc_player_blob_only
		tst	g2_blobshadow
		ble	g2c86zce_shadow_done
		bra	g2c86zce_blob_after_colltype
	;
g2c86zce_try_blob
	tst	g2_blobshadow
	ble	g2c86zce_shadow_done
	...
	tst	ob_colltype(a2)
	bne	g2c86zce_shadow_done
g2c86zce_blob_after_colltype
	tst	ob_hitpoints(a2)
	ble	g2c86zce_shadow_done
...
g2c86zce_shadow_done	movem.l	(a7)+,d0-d7/a0-a2
	rts
...
.wait2	bsr	checkany
	...
	; c86zcc: combat intermission/death screens must not be consumed by the
	; Fire/Return press that selected the menu item or killed the player.
	; First wait for all inputs to be released, then require a new press/release.
g2waitfreshany	movem.l	d0-d7/a0-a6,-(a7)
g2zcc_wait_release	bsr	checkany
		bne.s	g2zcc_wait_release
g2zcc_wait_press	bsr	checkany
		beq.s	g2zcc_wait_press
g2zcc_wait_release2	bsr	checkany
		bne.s	g2zcc_wait_release2
		movem.l	(a7)+,d0-d7/a0-a6
		rts
...
.loop	move.b	(a0)+,(a1)+
	...
	move.l	combatpal,a1
	jsr	showpic
		bsr	g2waitfreshany	;c86zcc: require fresh press before Combat map load
...
.shit	;
	;init windows...
	;
		; c86zcc: restore the original gloom.s per-level player window
		; assignment.  The current gloom2 path no longer calls makewindow/showwindow,
		; but message/status helpers still dereference ob_window(a5).  After a
		; previous ONE/TWO PLAYER run this could be stale before entering COMBAT.
		move.l	player1(pc),a5
		move.l	#window1,ob_window(a5)
		tst	gametype
		beq.s	g2zcc_window_done
		move.l	player2(pc),a5
