...
.nocd	;
	...
	;
	cmp	#1,g2_game_profile	; v190dg: original Gloom title/menu uses smallfont.bin
	bne	g2v190dg_title_bigfont
	bsr	smallfont
	bra	g2v190dg_title_font_done
g2v190dg_title_bigfont
	bsr	bigfont
g2v190dg_title_font_done
...
.play	;
	...
	;
	cmp	#1,g2_game_profile	; v190dg: original Gloom HUD/messages use smallfont.bin in-game
	bne	g2v190dg_play_font_done
	bsr	smallfont
g2v190dg_play_font_done
	tst	twowins
	beq.s	.n2
	bsr	swaphflags
...
calcoffset	move	#320,d0
	...
	;
	move	#224,d1
	cmp	#1,g2_game_profile	; v190dg: original Gloom has no lower statusbar
	bne	g2v190dg_calcoff_224
	move	#240,d1
g2v190dg_calcoff_224
	sub	hite(pc),d1
	lsr	#1,d1
	mulu	linemodw(pc),d1
	add.l	d1,d0
...
drawchunky	;draw a chunky shape on a 320 wide chunkymap
	...
	;
	move.l	panel(pc),a0
	beq.w	g2v190dg_drawchunky_nopanel
...
g2drawgun
	...
	cmp	#1,g2_game_profile	; v190dg: original Gloom has no lower gun graphic
	beq.w	.g2dg_done
	move.l	gunpic(pc),d0
	beq.w	.g2dg_done
...
doc2p	;
	...
	bne	g2v190dg_doc2p_no_g1_hud
	jsr	g2g1_draw_classic_hud_planar
g2v190dg_doc2p_no_g1_hud
...
; v190dg: Black-Magic-engine data profile.
; 0=Gloom Deluxe/Gloom2, 1=original Gloom, 2=Zombie Massacre, 3=Gloom3.
g2_game_profile	dc	0
	even
...
.p_loop	move.l	d0,(a0)+
	...
	;
	jsr	g2detectprofile
	jsr	g2apply_profile_runtime
	jsr	g2select_magicfiles
	bsr	loadfiles
	jsr	g2select_magicpalfiles
	bsr	loadfiles
	;
	move.l	magic,a0
	move.l	magicpal,a1
	...
	move	#50,vbcounter
	;
	jsr	g2select_progfiles
	bsr	loadfiles
	jsr	g2select_titlefiles
	bsr	loadfiles
	cmp	#1,g2_game_profile
	beq	g2v190dg_no_gun_fallback
	jsr	g2loadgunfallback	;v61: optional misc/stuf gun.bin fallback
g2v190dg_no_gun_fallback
	;
...
g2_after_inline_c2p_init
	...
	move.l	panel,d0
	beq	g2v190dg_no_panel_remap
	move.l	d0,a0
	jsr	remapanim
g2v190dg_no_panel_remap
...
.dataok	bsr	permit
	...
	move.l	d0,oldlock
	;
g2v190di_load_datafiles
	jsr	g2select_datafiles
	bsr	loadfiles
	bra	forbid
...
.gotdata	;
	...
	tst.l	gloomgame
	bne	g2v190dg_gloomgame_ok
	tst	g2_game_profile
	beq	g2v10_datafail_initnewgame
	move.l	#g2v190dg_dummy_gloomgame,gloomgame
g2v190dg_gloomgame_ok
	bsr	qsync
	;
	move.l	medat,a1
	jsr	12(a1)
...
g2v10_player1_ok
	...
	jsr	g2log
	cmp	#1,g2_game_profile
	bne	g2v190dg_normal_gamepal
	jsr	g2g1_poke_dynamic_palette
	jsr	g2g1_calcpalettes
	bra	g2v190dg_gamepal_done
g2v190dg_normal_gamepal
	move.l	planar_palette(pc),a1
	jsr	pokepal
	bsr	calcpalettes
g2v190dg_gamepal_done
	bsr	dispoff
	;
	;
	;init player vars...
...
.redrawmenu
	...
	lea	startmenu,a4
	tst	linked
	bne	g2v190dg_use_linkmenu
	tst	g2_game_profile
	beq	g2v190dg_use_mainmenu
	lea	startmenu_compat,a4
	bra.s	.use
g2v190dg_use_linkmenu
	lea	startmenu2,a4
	bra.s	.use
g2v190dg_use_mainmenu
	lea	startmenu,a4
.use	jsr	initmenu
...
.g2v166_p1sel	tst	g2_game_profile
	beq	g2v190dg_p1_default
	lea	p1ctype_compat(pc),a1
	bra	g2v190dg_p1_have
g2v190dg_p1_default	lea	p1ctype(pc),a1
g2v190dg_p1_have	lea	p1_ob_cntrl,a2
	lea	p2_ob_cntrl,a3
	bsr	inccntrl
	bra	.sel
.g2v166_p1prevsel	tst	g2_game_profile
	beq	g2v190dg_p1p_default
	lea	p1ctype_compat(pc),a1
	bra	g2v190dg_p1p_have
g2v190dg_p1p_default	lea	p1ctype(pc),a1
g2v190dg_p1p_have	lea	p1_ob_cntrl,a2
	lea	p2_ob_cntrl,a3
	bsr	deccntrl
	bra	.sel
	;
...
.g2v166_p2sel	tst	g2_game_profile
	beq	g2v190dg_p2_default
	lea	p2ctype_compat(pc),a1
	bra	g2v190dg_p2_have
g2v190dg_p2_default	lea	p2ctype(pc),a1
g2v190dg_p2_have	lea	p2_ob_cntrl,a2
	lea	p1_ob_cntrl,a3
	bsr	inccntrl
	bra	.sel
.g2v166_p2prevsel	tst	g2_game_profile
	beq	g2v190dg_p2p_default
	lea	p2ctype_compat(pc),a1
	bra	g2v190dg_p2p_have
g2v190dg_p2p_default	lea	p2ctype(pc),a1
g2v190dg_p2p_have	lea	p2_ob_cntrl,a2
	lea	p1_ob_cntrl,a3
	bsr	deccntrl
	bra	.sel
	;
...
.moloop	move.b	(a0)+,(a1)+
	bne.s	.moloop
	tst	g2_game_profile
	beq	g2v190dg_mode_done
	move	mode(pc),d0
	lea	modes,a0
	move.l	0(a0,d0*4),a0
	lea	modetxt_compat,a1
g2v190dg_moloop2	move.b	(a0)+,(a1)+
	bne	g2v190dg_moloop2
g2v190dg_mode_done
	;
	bra	.sel
	;	;
.about	;about text...
	...
	bne	g2v190dg_about_no_zm_brush
	jsr	g2v142_draw_gloombrush_safe
	bsr	db
g2v190dg_about_no_zm_brush
	move	numopts(pc),-(a7)
	move	#1,numopts
	;
	bsr	selmenu
...
.noch	bsr	g2v147_finitmenu_soft
	...
	jsr	g2v145_show_clean_title
	cmp	#2,g2_game_profile
	bne	g2v190dg_about_return_show
	jsr	g2v142_draw_gloombrush_safe
g2v190dg_about_return_show
	bsr	dispon
	bra	.redrawmenu
	;
.notabout	subq	#1,d0
...
startmenu_compat	dc.b	13
	dc.b	0			;0 no START LEVEL for non-Gloom2 profiles
	dc.b	0			;1 spacer below missing selector
	dc.b	'ONE PLAYER GAME',0	;2
	dc.b	'TWO PLAYER GAME',0	;3
	dc.b	'TWO PLAYER COMBAT',0	;4
	dc.b	0			;5 spacer above PLAYER 1
	dc.b	'PLAYER 1 '
p1ctype_compat
	ifne	cd32
	dc.b	'CD32 PAD 1',0
...
p2ctype_compat
	...
	dc.b	'JOYSTICK 1 ',0
	endc
	dc.b	0			;8 spacer above REMOTE LINK OPTIONS
	dc.b	'REMOTE LINK OPTIONS',0	;9
modetxt_compat	dc.b	'MEATY VIOLENCE MODE',0	;10
	dc.b	'ABOUT GLOOM',0		;11
	dc.b	'EXIT GLOOM',0		;12
	even
...
loadfile_	;a0=name, d1=memtype
	;
	jsr	g2profile_translate_loadname
...
; v190dg: profile detection/selection and original-Gloom/ZM compatibility helpers.

g2detectprofile
	movem.l	d0-d2/a0-a1/a6,-(a7)
	clr	g2_game_profile
	lea	g2prof_zm_probe,a0
	jsr	g2fileexists
	tst.l	d0
	beq	g2prof_not_zm
	...
	bra	g2prof_done
g2prof_not_zm
	lea	g2prof_g3_probe,a0
	jsr	g2fileexists
	tst.l	d0
	beq	g2prof_not_g3
	move	#3,g2_game_profile
	bra	g2prof_done
g2prof_not_g3
	lea	g2prof_title_probe,a0
	jsr	g2fileexists
	tst.l	d0
	bne	g2prof_done
...
g2fileexists	;a0=name, returns d0 nonzero if file/lock exists
...
g2select_magicfiles
	...
	rts
g2sel_magic_zm
	lea	g2zm_magicfiles,a0
	rts

g2select_magicpalfiles
	cmp	#2,g2_game_profile
	beq	g2sel_magicpal_zm
	tst	aga
	bne	g2sel_magicpal_aga
...
g2sel_title_aga
	...
	rts
g2sel_title_g1
	lea	g2g1_titlefiles,a0
	rts
g2sel_title_zm
	lea	g2zm_titlefiles,a0
	rts
...
g2select_progfiles
	...
	rts
g2sel_prog_g1
	lea	g2g1_progfiles,a0
	rts
g2sel_prog_zm
	lea	g2zm_progfiles,a0
	rts
...
g2select_datafiles
	...
	rts
g2sel_data_zm
	lea	g2zm_datafiles,a0
	rts

; --- profile probes ---
g2prof_zm_probe	dc.b	'stuf/stages',0
	even
g2prof_g3_probe	dc.b	'fonts/microscopic.font',0
	even
g2prof_title_probe	dc.b	'pics/title',0
	even

; --- profile-specific load tables ---
g2zm_magicfiles
	dc.l	magic
...
; --- original Gloom dynamic palette/Fog LUTs ---
g2g1_poke_dynamic_palette
	movem.l	d0-d7/a0-a2,-(a7)
	move.l	map_rgbs,a1
	tst	aga
	beq	g2g1_poke_go
...
g2g1_nearest_colour	; d4=RGB12, returns d0=index in current dynamic map_rgbs
	movem.l	d1-d7/a0-a2,-(a7)
	clr	g2g1_best_index
	move.l	#$7fffffff,d7
	moveq	#0,d6
	move.l	map_rgbs,a0
	move.l	map_rgbsat,a2
	move	d4,d5
	and	#$0f00,d5
	lsr	#8,d5
	move	d4,d2
...
g2g1_near_next
	...
	bra	g2g1_near_loop
g2g1_near_done
	move	g2g1_best_index,d0
	movem.l	(a7)+,d1-d7/a0-a2
	rts

; --- original Gloom HUD drawn after chunky->planar conversion ---
g2g1_draw_classic_hud_planar
	movem.l	d0-d7/a0-a6,-(a7)
	move.l	player_,a5
	tst.l	a5
	beq.w	g2g1_hud_done
	...
	tst.l	a0
	beq.w	g2g1_hud_done
	lea	g2g1_hud_health,a4
	moveq	#2,d7
	moveq	#2,d6
	jsr	g2g1_blit_hud_text
	lea	g2g1_hud_weapon,a4
	moveq	#2,d7
	moveq	#12,d6
	jsr	g2g1_blit_hud_text
	; health bar: x=44,y=2, 25 two-pixel cells, same shape ids as gloom.s
	moveq	#44,d5
	moveq	#40,d4
	moveq	#0,d6
	move	ob_hitpoints(a5),d7
...
g2g1_hp_draw
	...
	bra	g2g1_hp_loop
g2g1_hp_done
	; lives at top right, as original Gloom did
	move	ob_lives(a5),d7
	beq	g2g1_life_done
	subq	#1,d7
	move	width,d5
...
g2g1_life_loop
	...
	moveq	#43,d2
	jsr	blit
	dbf	d7,g2g1_life_loop
g2g1_life_done
	; weapon pips: x=44,y=12, 5 pips, shape 49-ob_weapon / empty 50
	moveq	#44,d5
	moveq	#49,d4
	sub	ob_weapon(a5),d4
	moveq	#1,d7
...
g2g1_hud_health	dc.b	'HEALTH',0
g2g1_hud_weapon	dc.b	'WEAPON',0
	even

; --- Zombie Massacre load-name compatibility ---
g2profile_translate_loadname
...
g2pt_prefix_loop
	...
	jsr	g2pt_startswith
	tst	d0
	bne	g2pt_prefix_loop
	lea	g2loadname_tmp,a4
	move.l	a2,a5
g2pt_cp_pref
	move.b	(a5)+,d0
	beq	g2pt_cp_suffix
...
g2pt_cp_suffix
	...
	move.b	d0,(a4)+
	bne	g2pt_cp_suffix
	lea	g2loadname_tmp,a0

g2pt_done
	movem.l	(a7)+,d0-d7/a1-a5
	rts

g2pt_streq	;a0 src, a1 cmp, d0=0 equal
	movem.l	a0-a1,-(a7)
g2pt_se_loop
	move.b	(a0)+,d0
	cmp.b	(a1)+,d0
...
g2pt_startswith	;a0 src, a1 prefix, returns a0 advanced after prefix on match; original a0 on miss
	movem.l	a1-a2,-(a7)
	move.l	a0,a2
g2pt_sw_loop
	move.b	(a1)+,d0
...
g2pt_copy_tmp	;a1 source full replacement -> a0 tmp
	lea	g2loadname_tmp,a4
g2pt_ct_loop
	move.b	(a1)+,(a4)+
	bne	g2pt_ct_loop
...
; Full filename remap for Zombie Massacre object names.
g2pt_fullmap
	dc.l	g2pt_o_player,g2pt_z_kforee
	dc.l	g2pt_o_player2,g2pt_z_kforee2
	dc.l	g2pt_o_tokens,g2pt_z_pwrups
	dc.l	g2pt_o_marine,g2pt_z_troopr
	dc.l	g2pt_o_marine2,g2pt_z_troopr2
	dc.l	g2pt_o_baldy,g2pt_z_zombi
	dc.l	g2pt_o_baldy2,g2pt_z_zombi2
	dc.l	g2pt_o_terra,g2pt_z_zocom
	dc.l	g2pt_o_terra2,g2pt_z_zocom2
	dc.l	g2pt_o_ghoul,g2pt_z_ghost
	dc.l	g2pt_o_demon,g2pt_z_fatzo
	dc.l	g2pt_o_demon2,g2pt_z_fatzo2
	dc.l	g2pt_o_phantom,g2pt_z_zomboid
	dc.l	g2pt_o_phantom2,g2pt_z_zomboid2
	dc.l	g2pt_o_lizard,g2pt_z_skinny
	dc.l	g2pt_o_lizard2,g2pt_z_skinny2
	dc.l	g2pt_o_death,g2pt_z_dows
	dc.l	g2pt_o_dragon,g2pt_z_james
	dc.l	g2pt_o_dragon2,g2pt_z_james2
	dc.l	g2pt_o_troll,g2pt_z_zombie
	dc.l	g2pt_o_troll2,g2pt_z_zombie2
	dc.l	0

g2pt_prefixmap
	dc.l	g2pt_p_maps,g2pt_p_lvls
	dc.l	g2pt_p_pics,g2pt_p_pixs
	dc.l	g2pt_p_misc,g2pt_p_stuf
	dc.l	0

g2pt_o_player	dc.b	'objs/player',0
g2pt_o_player2	dc.b	'objs/player2',0
g2pt_o_tokens	dc.b	'objs/tokens',0
g2pt_o_marine	dc.b	'objs/marine',0
g2pt_o_marine2	dc.b	'objs/marine2',0
g2pt_o_baldy	dc.b	'objs/baldy',0
g2pt_o_baldy2	dc.b	'objs/baldy2',0
g2pt_o_terra	dc.b	'objs/terra',0
g2pt_o_terra2	dc.b	'objs/terra2',0
g2pt_o_ghoul	dc.b	'objs/ghoul',0
g2pt_o_demon	dc.b	'objs/demon',0
g2pt_o_demon2	dc.b	'objs/demon2',0
g2pt_o_phantom	dc.b	'objs/phantom',0
g2pt_o_phantom2	dc.b	'objs/phantom2',0
g2pt_o_lizard	dc.b	'objs/lizard',0
g2pt_o_lizard2	dc.b	'objs/lizard2',0
g2pt_o_death	dc.b	'objs/deathhead',0
g2pt_o_dragon	dc.b	'objs/dragon',0
g2pt_o_dragon2	dc.b	'objs/dragon2',0
g2pt_o_troll	dc.b	'objs/troll',0
g2pt_o_troll2	dc.b	'objs/troll2',0

g2pt_z_kforee	dc.b	'char/kforee',0
g2pt_z_kforee2	dc.b	'char/kforee2',0
g2pt_z_pwrups	dc.b	'char/pwrups',0
g2pt_z_troopr	dc.b	'char/troopr',0
g2pt_z_troopr2	dc.b	'char/troopr2',0
g2pt_z_zombi	dc.b	'char/zombi',0
g2pt_z_zombi2	dc.b	'char/zombi2',0
g2pt_z_zocom	dc.b	'char/zocom',0
g2pt_z_zocom2	dc.b	'char/zocom2',0
g2pt_z_ghost	dc.b	'char/ghost',0
g2pt_z_fatzo	dc.b	'char/fatzo',0
g2pt_z_fatzo2	dc.b	'char/fatzo2',0
g2pt_z_zomboid	dc.b	'char/zomboid',0
g2pt_z_zomboid2	dc.b	'char/zomboid2',0
g2pt_z_skinny	dc.b	'char/skinny',0
g2pt_z_skinny2	dc.b	'char/skinny2',0
g2pt_z_dows	dc.b	'char/dows-head',0
g2pt_z_james	dc.b	'char/james',0
g2pt_z_james2	dc.b	'char/james2',0
g2pt_z_zombie	dc.b	'char/zombie',0
g2pt_z_zombie2	dc.b	'char/zombie2',0

g2pt_p_maps	dc.b	'maps/',0
g2pt_p_lvls	dc.b	'lvls/',0
g2pt_p_pics	dc.b	'pics/',0
g2pt_p_pixs	dc.b	'pixs/',0
g2pt_p_misc	dc.b	'misc/',0
g2pt_p_stuf	dc.b	'stuf/',0
	even
...
g2v190dg_dummy_gloomgame	dc.l	'game'
	dc.l	'game'
	dc.l	'game'
	dc.l	'game'
	dc.l	'game'
...
g2loadname_tmp	ds.b	96
	even

g2g1_best_index	dc	0
	even
g2g1_aga_pal	ds.w	512
	even
