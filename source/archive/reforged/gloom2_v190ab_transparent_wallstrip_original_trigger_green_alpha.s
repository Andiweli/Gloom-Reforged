...
solidstrip	set	0
drawstrip2	; v190ab chunky transparent wall strip overlay, original-style green glass
	; a1=top of destination column, a2=palettes base, a4=strip/vd data
	; Original gloom.s marks these wall textures by the non-zero byte directly
	; before the column data.  That byte makes the wall a strip via makestrip
	; and is then used by drawstrip_ as the transparency colour selector.
	; In chunky mode we keep the same trigger, but render zero texels as a
	; continuous dark-green transparent blend.  The texture step is kept as a
	; normal 16.16 value and advanced with ADD.L, so the RGB blend cannot break
	; the old ADDX/X-flag stepping and make near-door elements vanish.
	movem.l	d0-d7/a0-a5,-(a7)
	move.l	vd_data(a4),d0
	beq	.rts
	move.l	d0,a0		; source texture column, -1(a0)=strip colour selector
	move	#$060,d6		; dark green glass tint, matching the original maps' doors
	move.l	chunkymod(pc),d7
	move	vd_y(a4),d2
	add	midy(pc),d2	; screen Y start
	move.l	vd_ystep(a4),d3
	move	vd_h(a4),d4
	moveq	#0,d0		; texture Y start, standard 16.16 fixed point
	tst	d2
	bpl.s	.notopclip
	add	d2,d4
	ble	.rts
...
.hiteok
	...
	blt	.rts
	move	vd_pal(a4),d5
	move.l	0(a2,d5*4),a5	; wall palette for non-transparent texels
	move	#$0eee,d2		; even RGB mask like drawobjtrans, avoids packed carry
	move.l	planar_palette(pc),a3	; chunky palette index -> RGB12
	move.l	planar_remap(pc),a2	; RGB12 -> chunky palette index
.loop
	move.l	d0,d1
	swap	d1
	and	#63,d1		; source texture Y, clamped to the 64 pixel column
	moveq	#0,d5
	move.b	0(a0,d1.w),d5
	beq.s	.glass
	move.b	0(a5,d5.w),(a1)
	bra.s	.next
.glass	move	d6,d5
	and	d2,d5		; dark green glass RGB12
	moveq	#0,d1
	move.b	(a1),d1		; already-rendered background pixel
	move	0(a3,d1*4),d1	; background RGB12
	and	d2,d1
	add	d1,d5
	lsr	#1,d5		; 50% background + 50% dark green glass
	move.b	0(a2,d5.w),(a1)
.next	add.l	d3,d0		; normal fixed-point step, independent of X flag
	add.l	d7,a1
	dbf	d4,.loop
