...
.rit	move.l	a5,g2_shape_owner	;v123 remember object owner for blob shadow
	jsr	(a0)
	clr.l	g2_shape_owner
	move.l	#drawobjnorm,shaperender
	bra.s	.loop
.notinvs	move.l	a5,g2_shape_owner	;v123 remember object owner for blob shadow
	jsr	(a0)
	clr.l	g2_shape_owner
	bra.s	.loop
.done	;
...
.loop2	move.l	(a5),a5
	...
	move	#$200,d7
	clr.l	g2_shape_owner	;v123 gore/body chunks are not enemy-shadow owners
	bsr	drawshape_q
	;
	bra.s	.loop2
	;
...
drawshape_q	;
	...
	move.l	g2_shape_owner,sh_prev(a1)	;v123 object owner for blob shadow
	move	d7,sh_scale(a1)
...
.ycdone2	move.l	d6,d1
	lsr.l	#1,d1
.ycdone	;
	bsr	g2_prepare_enemy_blob_from_shape	;v124 ZGloom-style sprite-box blob
	;
	;draw bit...
	;
	...
	bsr	g2_draw_pending_blob_after_sprite	;v124 draw shadow after sprite using saved box
	;
	move.l	(a7)+,a6
	bra	.drawloop
	;
...
; -----------------------------------------------------------------------------
; v125 ZGloom-style enemy blob shadow, palette-safe floor darken test.
; The previous tests wrote a fixed grey chunky colour.  Depending on the active
; level palette that can collapse to an invisible/near-identical index.  v125
; instead reads the already rendered floor pixel below the enemy, darkens its
; RGB through planar_palette/planar_remap, and writes that palette-valid result
; back.  First test deliberately ignores the per-column wall Z check so we can
; verify that the hook and colour path are visible at all.
; -----------------------------------------------------------------------------
g2_prepare_enemy_blob_from_shape
	movem.l	d0-d7/a0-a2,-(a7)
	clr	g2_blob_pending
	tst	g2_blobshadow
	ble	.done
	move.l	sh_prev(a6),a2
	tst.l	a2
	beq	.done
	move.l	player1(pc),d5
	cmp.l	d5,a2
	beq	.done
	move.l	player2(pc),d5
	cmp.l	d5,a2
	beq	.done
	move.l	ob_render(a2),d5
	move.l	#drawshape_8,d6
	cmp.l	d6,d5
	bne	.done
	tst	ob_hitpoints(a2)
	ble	.done
	; v124: do not whitelist by ob_logic here.  Several monsters temporarily
	; switch logic while hurt/attacking, which made v123 prepare no shadow at all.
	; drawshape_8 + positive hitpoints + not player is enough to target enemies.
	; centre X = clipped screen start + clipped width/2
	move	d7,d0
	move	d3,d5
	lsr	#1,d5
	add	d5,d0
	move	d0,g2_blob_cx
	; base Y = current dest top row + clipped height + small visible lift
	move.l	a1,d1
	sub.l	chunky(pc),d1
	divu	chunkymodw(pc),d1
	and.l	#$0000ffff,d1
	add	d4,d1
	addq	#2,d1
	move	d1,g2_blob_cy
	move	d2,g2_blob_z
	; radius follows visible width, stronger than v122 so first test is obvious
	move	d3,d0
	lsr	#1,d0
	cmp	#5,d0
	bge	.rxminok
...
.rymaxok
	move	d1,g2_blob_ry
	; v125: colour is no longer a fixed grey index.  The span routine
	; darkens the existing floor/background pixels through the active palette,
	; so this stored colour is only kept for fallback/debug compatibility.
	moveq	#7,d4
	move	d4,g2_blob_col
	move	#1,g2_blob_pending
.done	movem.l	(a7)+,d0-d7/a0-a2
	rts

g2_draw_pending_blob_after_sprite
	movem.l	d0-d7/a0-a5,-(a7)
	tst	g2_blob_pending
	beq	.done
	clr	g2_blob_pending
	move	g2_blob_cx(pc),d0
	move	g2_blob_cy(pc),d1
	move	g2_blob_z(pc),d2
	move	g2_blob_rx(pc),d3
	move	g2_blob_col(pc),d4
	bsr	g2_draw_blob_oval5
.done	movem.l	(a7)+,d0-d7/a0-a5
	rts

g2_draw_blob_oval5
	; d0 centre x, d1 centre/base y, d2 depth, d3 halfwidth, d4 colour
	movem.l	d0-d4,-(a7)
	move	d3,d5
	lsr	#1,d5
	move	d5,d3
	subq	#2,d1
	bsr	g2_draw_blob_span
	movem.l	(a7),d0-d4
	move	d3,d5
	lsr	#2,d5
	sub	d5,d3
	...
	move	d5,d3
	addq	#2,d1
	bra	g2_draw_blob_span

g2_draw_blob_span
	; d0 centre x, d1 y, d2 depth, d3 halfwidth, d4 colour
	; v124: the Gloom2 chunky buffer is not simple row-major for X.
	; X must go through coloffs, just like sprite/blood/pixelate drawing.
	tst	d3
	ble	.rts
	tst	d1
	bmi	.rts
...
.xeok	sub	d5,d6
	...
	add.l	d7,a0		;row base
	lea	coloffs(pc),a2	;v124 display-X -> chunky column offset
	move.l	planar_palette(pc),a4
	move.l	planar_remap(pc),a5
	tst.l	a4
	beq.s	.flatcolour
	tst.l	a5
	beq.s	.flatcolour
	move	d5,d7		;current X
.loop	move.l	a0,a3
	add.l	0(a2,d7*4),a3
	moveq	#0,d0
	move.b	(a3),d0		;current floor/background chunky colour
	beq.s	.skip
	move	0(a4,d0*4),d0	;chunky index -> 12-bit RGB
	and	#$ccc,d0		;palette-safe darken: keep nibble boundaries
	lsr	#2,d0		;roughly 25% brightness, still remapped to palette
	move.b	0(a5,d0.w),(a3)
.skip	addq	#1,d7
	dbf	d6,.loop
	rts
.flatcolour	move	d5,d7		;fallback only, should rarely be used
.floop	move.l	a0,a3
	add.l	0(a2,d7*4),a3
	move.b	d4,(a3)
	addq	#1,d7
...
game_ceil	dc.b	'            CEILING: YES                        ',0
	dc.b	92,0
game_blob	dc.b	'        BLOB SHADOW: NO                         ',0
...
g2_blobshadow	dc	-1	;v116c menu flag, v124 enables enemy blob rendering
g2_reflections	dc	-1	;v116c menu flag only, render code removed
g2_shape_owner	dc.l	0	;v123 owner copied into shape list for blob shadow
g2_blob_pending	dc	0	;v123 prepared shadow flag
g2_blob_cx	dc	0
g2_blob_cy	dc	0
g2_blob_z	dc	0
g2_blob_rx	dc	0
