...
; v115: discrete view sizes from v114 ingame menu. Left/right clamps at
; TINY/FULLSCREEN.  ALMOST FULL keeps current width but uses VERY HUGE height.
...
drawshape_1sc	;draw shape with one frame, scaled!
	...
	add.l	12(a0,d0*4),a0
	move	ob_scale(a5),d7
	movem.l	d0-d2/d6/a0,-(a7)	;v116 mark optional floor FX for this object
	jsr	g2_make_floorfx_flags
	move.l	d6,g2currentfx
...
drawshape_q	;
	...
	move.l	g2currentfx(pc),sh_prev(a1)	;v116 optional floor FX from object render entry
	clr.l	g2currentfx
	;
	lea	shapelist(pc),a2
.loop	move.l	(a2),d0
...
.shape	move.l	d0,a0
	...
	;
	add	midx(pc),d0
	move	d0,g2fx_sx	;v116 projected sprite bounds before clipping
	move	d3,g2fx_sw
	bpl.s	.xcskip
...
.xcdone	;
	...
	;
	add	midy(pc),d1
	move	d1,g2fx_sy	;v116 projected sprite bounds before clipping
	move	d4,g2fx_sh
	bpl.s	.ycskip
	add	d1,d4	;hite
	ble	.drawloop
...
.ycdone2	move.l	d6,d1
	lsr.l	#1,d1
.ycdone	;
	;v116 optional floor effects are drawn as small chunky spans below the sprite.
	move.l	sh_prev(a6),d0
	beq.s	.g2_no_floorfx
	movem.l	d0-d7/a0-a6,-(a7)
	jsr	g2_draw_sprite_floorfx
	movem.l	(a7)+,d0-d7/a0-a6
.g2_no_floorfx
	;draw bit...
	;
...
; -----------------------------------------------------------------------------
; v116 optional projected floor effects.
; These are deliberately simple chunky spans, so no object/collision/list logic is
; changed.  sh_prev is unused by the original shapelist and carries flags here:
; bit0 = blob shadow, bit1 = reflection glow, bit2 = pulsing pickup glow,
; bits 8..15 = chunky colour index for the glow.
; -----------------------------------------------------------------------------
g2_make_floorfx_flags
	moveq	#0,d6
	; v116b: hard gate. If both effects are off, do not inspect object data at all.
	tst	g2_blobshadow
	bgt.s	.active
	tst	g2_reflections
	bgt.s	.active
	rts
.active	; blob shadow: only real living monsters, not players, pickups or bullets
	tst	g2_blobshadow
	ble	.no_blob
	tst	ob_colltype(a5)
	bne	.no_blob
	...
	tst	ob_hitpoints(a5)
	ble	.no_blob
	; monster shots may use colltype 0, so exclude bullet shapes from shadows
	...
	moveq	#1,d6
.no_blob
	; reflections/glows: flying shots and map pickups/upgrades
	tst	g2_reflections
	ble	.done
	move.l	ob_shape(a5),d0
	lea	bullet1,a0
	...
	cmp.l	d1,d0
	beq	.pick_wht
	; token powerups: harmless map item, collides only with players
	tst	ob_colltype(a5)
	bne	.done
	cmp	#24,ob_collwith(a5)
	bne	.done
	tst	ob_damage(a5)
	bne	.done
	or	#$0f06,d6	;white-ish pulsing token ring
	rts
.bul_red	or	#$0c02,d6
	rts
.bul_yel	or	#$0e02,d6
...
g2_draw_sprite_floorfx
	; d0 = flags from sh_prev
	move.l	d0,d6
	beq	.rts
	move	g2fx_sx(pc),d0
	move	g2fx_sw(pc),d2
	...
	move	g2fx_sy(pc),d1
	add	g2fx_sh(pc),d1
	subq	#2,d1		;bottom y / floor contact
	btst	#0,d6
	beq	.no_shadow
	move	g2fx_sw(pc),d2
	lsr	#2,d2
...
.sh_min_ok	cmp	#20,d2
	ble	.sh_max_ok
	moveq	#20,d2
.sh_max_ok	moveq	#1,d3
	bsr	g2_draw_soft_flatspan
.no_shadow
	btst	#1,d6
	beq	.rts
	move	g2fx_sw(pc),d2
...
.no_pulse	cmp	#15,d2
	ble	.rf_max_ok
	moveq	#15,d2
.rf_max_ok	move.l	d6,d3
	lsr.l	#8,d3
	and	#$00ff,d3
	bsr	g2_draw_ring_flatspan
.rts	rts

g2_draw_soft_flatspan
	; d0 centre, d1 y, d2 halfwidth, d3 colour
	movem	d0-d3,-(a7)
	subq	#1,d1
	lsr	#1,d2
	bsr	g2_draw_chunky_span
	movem	(a7),d0-d3
	bsr	g2_draw_chunky_span
	movem	(a7)+,d0-d3
	addq	#1,d1
	lsr	#1,d2
	bra	g2_draw_chunky_span

g2_draw_ring_flatspan
	; d0 centre, d1 y, d2 halfwidth, d3 colour
	movem	d0-d3,-(a7)
	subq	#1,d1
	lsr	#1,d2
...
g2_draw_chunky_span
	; d0 centre x, d1 y, d2 half width, d3 byte colour
	tst	d2
	ble	.rts
	tst	d1
	bmi	.rts
...
g2fx_sx	dc	0	;v116 projected floor-fx screen bounds
g2fx_sy	dc	0
g2fx_sw	dc	0
g2fx_sh	dc	0
g2currentfx	dc.l	0	;v116 per-object floor effect flags for next shapelist entry
...
gamemenu	dc.b	16
	dc.b	'CONTINUE',0
	dc.b	92,0
	; v116: ingame effects menu only; title menu remains v107.
...
g2_blobshadow	dc	-1	;v116b default off: 1=on, -1=off
g2_reflections	dc	-1	;v116b default off: 1=on, -1=off
