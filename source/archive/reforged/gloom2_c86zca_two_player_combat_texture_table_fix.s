	...
	;gloom2_c86zc9: P96 path still disabled; fix TWO PLAYER COMBAT re-entry, require a fresh press on combat intermission image, guard missing combat IFF, keep other-player reflection in TWO PLAYER.
	;gloom2_c86zca: clear stale texture pointer table after freetxts; fixes TWO PLAYER COMBAT crash when started after any previous game/combat run.
	;
...
.g2c57_player_try_blob
	; c86zc9: in TWO PLAYER the visible other player is drawn through the
	; same shape path as monsters, but the old player-owner branch jumped
	; straight to blob shadows and never prepared the reflection state.
	; Keep the current-camera guard above, then allow the other player to use
	; the same safe floor-anchored reflection helper as enemies.
	tst	g2_reflections
	ble.s	.g2zc9_player_blob_only
	tst	ob_hitpoints(a2)
	ble.s	.g2zc9_player_blob_only
	cmp	#3<<grdshft,d2
	bgt.s	.g2zc9_player_blob_only
	bsr	g2_prepare_enemy_reflection_safe
.g2zc9_player_blob_only
	tst	g2_blobshadow
	ble	.done
...
.gotdata	;
	...
	;combat type game!
	;
	; c86zc9: a fresh title-menu TWO PLAYER COMBAT start must not
	; inherit the shuffled combat-map list or stale per-run state from the
	; previous combat run.  Continuing inside the same combat series jumps
	; directly back to execscript_med, so this reset only affects a new menu
	; start.
	bsr	g2resetcombatseries
...
; c86zc9: combat intermission/death screens must not be consumed by the
; Fire/Return press that killed the previous player or selected the menu item.
; First wait for all inputs to be released, then require a new press and release.
g2waitfreshany	movem.l	d0-d7/a0-a6,-(a7)
.release	bsr	checkany
	bne.s	.release
.press	bsr	checkany
	beq.s	.press
.release2	bsr	checkany
	bne.s	.release2
	movem.l	(a7)+,d0-d7/a0-a6
	rts
...
.loop	move.b	(a0)+,(a1)+
	bne.s	.loop
	;
	; c86zc9: guard the optional combat IFF/palette before showpic.
	; On a failed or fragmented reload, showpic with a null pointer can guru;
	; the map itself can still load without the picture.
	move.l	combat(pc),d0
	beq.s	.g2zc9_no_combat_pic
	move.l	combatpal(pc),d1
	beq.s	.g2zc9_no_combat_pic
	move.l	d0,a0
	move.l	d1,a1
	jsr	showpic
	; Do not auto-skip this combat intermission image with a stale button.
	bsr	g2waitfreshany
.g2zc9_no_combat_pic
...
; c86zc9: restore deterministic combat map order for a fresh title-menu start.
; pickcombat shuffles commaps in-place while a combat series is running.
g2resetcombatseries
	movem.l	d0/a0,-(a7)
	lea	commaps(pc),a0
	moveq	#1,d0
...
exitgame	cmp	#2,gametype
	bne.s	gameover
combatover	move.l	combat(pc),d0
	beq.s	.g2zc9_no_combat_free
	move.l	d0,a1
	freemem	combat
	clr.l	combat
.g2zc9_no_combat_free
	move.l	combatpal(pc),d0
	beq.s	.g2zc9_no_combatpal_free
	move.l	d0,a1
	freemem	combatpal
	clr.l	combatpal
.g2zc9_no_combatpal_free
...
.loop	move.l	(a6)+,d0
	...
	clr.l	-4(a6)
.skip	dbf	d7,.loop
	; c86zca: loadtxts only overwrites texture slots that are present in the
	; new map.  If a previous level had more/other texture screens, stale
	; pointers to freed texture data could remain in textures[] and the next
	; combat map could Guru as soon as it rendered.  Normal ONE/TWO starts
	; effectively come through a clean table on first run; TWO PLAYER COMBAT
	; as a second action exposed the stale entries.
	lea	textures,a6
	move	#160-1,d7
.clrtex	clr.l	(a6)+
	dbf	d7,.clrtex
