...
; -----------------------------------------------------------------------------
; v190dc: Original Gloom classic top-left HUD
; -----------------------------------------------------------------------------
; Classic Gloom does not use the Gloom2 bottom statusbar or first-person gun.
; Draw compact HEALTH / WEAPON labels and bars directly into the chunky frame
; after the 3D scene, using the active generated Gloom palette/remap.

g2g1_draw_classic_hud
	movem.l	d0-d7/a0-a5,-(a7)
	move.l	player_(pc),a5
	tst.l	a5
	beq	.g2g1_hud_done
	move	#$04f,d0
	bsr	g2g1_rgb_to_chunky
	move	d0,g2g1_hud_col_text
	move	#$0f0,d0
	bsr	g2g1_rgb_to_chunky
	move	d0,g2g1_hud_col_green
	move	#$ff0,d0
	bsr	g2g1_rgb_to_chunky
	move	d0,g2g1_hud_col_yellow
	move	#$f00,d0
	bsr	g2g1_rgb_to_chunky
	move	d0,g2g1_hud_col_red
	move	#$0cf,d0
	bsr	g2g1_rgb_to_chunky
	move	d0,g2g1_hud_col_blue
	lea	g2g1_hud_health(pc),a4
	moveq	#8,d0
	moveq	#6,d1
	move	g2g1_hud_col_text(pc),d2
	bsr	g2g1_draw_hud_text
	lea	g2g1_hud_weapon(pc),a4
	moveq	#8,d0
	moveq	#17,d1
	move	g2g1_hud_col_text(pc),d2
	bsr	g2g1_draw_hud_text
	; health bar: 25 cells, two pixels each
	move	ob_hitpoints(a5),d7
	ble.s	.g2g1_no_health
	cmp	#25,d7
	bls.s	.g2g1_hp_ok
	moveq	#25,d7
.g2g1_hp_ok
...
.g2g1_hp_loop
	...
	moveq	#2,d2
	moveq	#5,d3
	move	g2g1_hud_col_red(pc),d4
	cmp	#10,d6
	blt.s	.g2g1_hp_col_ok
	move	g2g1_hud_col_yellow(pc),d4
	cmp	#18,d6
	blt.s	.g2g1_hp_col_ok
	move	g2g1_hud_col_green(pc),d4
.g2g1_hp_col_ok
	bsr	g2g1_fill_rect
	addq	#1,d6
	cmp	d7,d6
	blt.s	.g2g1_hp_loop
.g2g1_no_health
	; weapon bar: current upgrade as five classic blocks
	move	ob_weapon(a5),d7
	addq	#1,d7
	cmp	#5,d7
	bls.s	.g2g1_wp_ok
...
g2g1_rgb_to_chunky	; d0.w = RGB12, returns d0.w = active chunky index
	movem.l	d1/a0,-(a7)
	move	d0,d1
...
g2g1_draw_hud_text	; a4=0-term text, d0=x, d1=y, d2=colour
	movem.l	d0-d7/a0-a4,-(a7)
	move	d0,d6
	move	d1,d7
	move	d2,d5
...
g2g1_draw_hud_glyph	; d2=glyph, d6=x, d7=y, d5=colour
...
g2g1_fill_rect	; d0=x,d1=y,d2=w,d3=h,d4=colour
...
g2g1_hud_weapon	dc.b	'WEAPON',0
	even
g2g1_hud_col_text	dc	0
g2g1_hud_col_green	dc	0
g2g1_hud_col_yellow	dc	0
g2g1_hud_col_red	dc	0
g2g1_hud_col_blue	dc	0
	even
...
.g2bs_not_blackout
	...
	cmp	#1,g2_game_profile	; v190dc: original Gloom has no bottom statusbar/gun
	beq.s	.g2bs_g1_classic_hud
	...
	rts
.g2bs_g1_classic_hud
	bsr	g2g1_draw_classic_hud
	move	#2,panelcnt	; convert the black lower 16 lines, but no statusbar art
	rts
...
pickcombat	;pick combat zone...
	...
	;
.doit	move	$dff006,d0
	jsr	seedrnd	; v190dd: far call buildfix
	;
	move	comsleft(pc),d0
	bne.s	.pick
	moveq	#7,d0
...
; v190dc: original Gloom menu text is drawn by the safe chatfont path,
; so provide a stable blue 4-colour font palette instead of relying on
; whichever title/intermission/game palette happens to be active.
g2g1_initfontpal_blue
	tst	aga
	beq.s	.ecs
	lea	g2g1_fontpal_blue_aga(pc),a1
	moveq	#4,d0
	bsr	pokepal2
	rts
.ecs	lea	g2g1_fontpal_blue_ecs(pc),a1
	moveq	#4,d0
	bsr	pokepal2
	rts

g2g1_fontpal_blue_ecs
	dc	$000,$04f,$08f,$0cf
g2g1_fontpal_blue_aga
	dc	$000,0,$04f,0,$08f,0,$0cf,0
...
initmenu2	;
	...
	bsr	swapshow
	cmp	#1,g2_game_profile	; v190dc: classic Gloom uses forced blue menu font palette
	beq.s	.g2v190dc_g1_fontpal
	bsr	initfontpal
	bra.s	.g2v190dc_fontpal_done
.g2v190dc_g1_fontpal
	jsr	g2g1_initfontpal_blue
.g2v190dc_fontpal_done
	;
...
.g2v190cj_no_remap
	; v190dc: only use identity when no remap exists at all.  Original Gloom
	; now builds a synthetic nearest-colour remap, so it must go through the
	; normal shade-palette generator to regain distance fog/darkening.
	lea	palettes(pc),a2
	moveq	#15,d7
.g2v190cj_shade_loop
	move.l	(a2)+,a3
...
.g2v190cv_clearplanes
	...
	; v190da: classic Gloom smallfont colour is the low palette bit.
	; v190cw used plane 1 and produced orange on real data; plane 0 maps
	; to the blue smallfont-style menu colour.
	move.l	a1,a2
	or.b	d0,(a2)
.g2v190cv_nextline
	lea	40(a0),a0
...
g2v190cx_build_g1_tables
	cmp	#1,g2_game_profile
	bne	.g2v190cx_done	; v190dd: branch can exceed short range
	movem.l	d0-d7/a0-a4,-(a7)
...
.g2v190cx_clear
	...
	clr	(a0)		; v190dc: classic clear/texture index 0 must be black, not old -1 sentinel
...
.g2v190cx_loop
	...
	move	(a0)+,d0
	and	#$0fff,d0
	; v190dc: map_rgbs starts with the old transparent sentinel (-1).
	; Do not expose that as hardware colour 0, otherwise cleared pixels,
	; view borders and the lower no-statusbar strip become white in Gloom 1.
	tst	d3
	bne.s	.g2v190dc_not_zero
	moveq	#0,d0
.g2v190dc_not_zero
	cmp	#256,d3
	bcc.s	.g2v190cx_skip_store
	move	d0,(a2)+
	clr	(a2)+
.g2v190cx_skip_store
	addq	#1,d3
...
.g2v190cx_built
	cmp	#256,d3
	bls.s	.g2v190cx_count_ok
	move	#256,d3
.g2v190cx_count_ok
	move	d3,g2v190cx_g1_count
	jsr	g2v190cx_fill_g1_remap_nearest
...
g2v190cx_fill_g1_remap_nearest
	movem.l	d0-d7/a0-a6,-(a7)
	lea	g2v190cx_g1_remap,a1
	moveq	#0,d7
.g2v190cx_rgb_loop
	move	d7,d4
	lsr	#8,d4
	and	#$000f,d4
	move	d4,g2v190cx_tar_r
	move	d7,d4
	lsr	#4,d4
	and	#$000f,d4
	move	d4,g2v190cx_tar_g
	move	d7,d4
	and	#$000f,d4
	move	d4,g2v190cx_tar_b
	lea	g2v190cx_g1_palette,a0
	move	g2v190cx_g1_count,d3
	beq.s	.g2v190cx_store_zero
	subq	#1,d3
	move	#32767,d2	; best score
	moveq	#0,d0
	movea.l	d0,a5		; current index
	movea.l	d0,a6		; best index
.g2v190cx_pal_loop
	move	(a0),d0
	addq.l	#4,a0
	move	d0,d4
	lsr	#8,d4
	and	#$000f,d4
	sub	g2v190cx_tar_r,d4
	bpl.s	.g2v190cx_abs_r
	neg	d4
.g2v190cx_abs_r
	move	d4,d1
	move	d0,d4
	lsr	#4,d4
	and	#$000f,d4
	sub	g2v190cx_tar_g,d4
	bpl.s	.g2v190cx_abs_g
	neg	d4
.g2v190cx_abs_g
	add	d4,d1
	move	d0,d4
	and	#$000f,d4
	sub	g2v190cx_tar_b,d4
	bpl.s	.g2v190cx_abs_b
	neg	d4
.g2v190cx_abs_b
	add	d4,d1
	cmp	d2,d1
	bcc.s	.g2v190cx_not_better
	move	d1,d2
	movea.l	a5,a6
.g2v190cx_not_better
	addq.l	#1,a5
	dbf	d3,.g2v190cx_pal_loop
	move.l	a6,d0
	move.b	d0,0(a1,d7.w)
	bra.s	.g2v190cx_next_rgb
.g2v190cx_store_zero
	clr.b	0(a1,d7.w)
.g2v190cx_next_rgb
	addq	#1,d7
	cmp	#4096,d7
	bcs	.g2v190cx_rgb_loop
	movem.l	(a7)+,d0-d7/a0-a6
	rts
