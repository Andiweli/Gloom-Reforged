	...
	;gloom2p96_c86zc: c86zb per-column wall reflection lite branchfix.
	;gloom2p96_c86zh: wall reflections softened about 33 percent, enemies unchanged.
	;gloom2p96_c86zk: wall reflections inverted near/far height and half-depth.
	;
...
; c86zb: wall reflection version A.
; Instead of a full-screen horizon post-process, reflect each rendered wall
; column individually into the floor below its own bottom edge.  This avoids
; the black flickering lines seen in the first probe and keeps the effect tied
; to real wall columns.  It is still intentionally stylised and lightweight,
; not a second full physical wall renderer.
g2_draw_wall_reflection_lite
	...
	beq.w	.done
	subq	#1,d7
	clr	d4		; screen x counter for Bayer phase
.col_loop
	tst.l	vd_data(a0)
	beq	.col_next
	move	vd_y(a0),d0
	add	midy(pc),d0		; unclipped wall top on screen
	move	vd_h(a0),d1
	ble	.col_next
	add	d0,d1
	subq	#1,d1		; wall bottom on screen
	cmp	midy(pc),d1
	blt	.col_next		; do not mirror rows that never reach the floor half
	; c86zk: force the visible wall-reflection height the other way round:
	; near walls get the longest visible tail, far walls get a very short one.
	; Also reduce the total wall-reflection depth to roughly half of the old
	; 12-row probe.  Do not let the free space below the wall-foot dominate
	; the result; when a near wall-foot is low on screen, move the visual
	; reflection start slightly upward instead of shrinking it away.
	moveq	#5,d6		; near: 6 rows visible max, half of old 12-row depth
	move	vd_z(a0),d3
	tst	g2_visibility
	bgt.s	.depth_adv
	cmp	#(5<<grdshft),d3
	...
	cmp	#(8<<grdshft),d3
	blo.s	.depth_mid
	moveq	#1,d6		; far: 2 rows only
	bra.s	.depth_done
.depth_mid
	moveq	#3,d6		; medium: 4 rows
	bra.s	.depth_done
.depth_adv
	cmp	#(8<<grdshft),d3
	blo.s	.depth_done
	cmp	#(12<<grdshft),d3
	blo.s	.depth_mid_adv
	moveq	#1,d6		; far advanced: 2 rows only
	bra.s	.depth_done
.depth_mid_adv
	moveq	#3,d6		; medium advanced: 4 rows
.depth_done
	; never mirror more rows than roughly half the actually visible wall column
	move	vd_h(a0),d3
	lsr	#1,d3
	subq	#1,d3
	cmp	d3,d6
	ble.s	.depth_wallh_ok
	move	d3,d6
.depth_wallh_ok
	cmp	#1,d6
	blt	.col_next
	; Keep destination inside the 3D window.  If the wall foot is too low,
	; shift the reflected strip upward so near walls do not become shorter
	; than distant walls merely because the screen bottom cuts them off.
	move	d1,d0
	addq	#1,d0
	add	d6,d0		; wanted last destination row
	move	hite(pc),d2
	sub	#3,d2		; safe last destination row before HUD/gun edge
	cmp	d2,d0
	ble.s	.depth_fit_ok
	sub	d2,d0		; overflow rows
	sub	d0,d1		; move source/foot upward by overflow
	cmp	midy(pc),d1
	blt	.col_next
.depth_fit_ok
	move.l	(a1),d5		; x byte offset into chunky buffer
	move	d1,d2
	mulu	chunkymodw(pc),d2
	move.l	a2,a3
	add.l	d5,a3
	add.l	d2,a3		; a3 = source at wall bottom
	move.l	a3,a4
	add.l	chunkymod(pc),a4	; a4 = first reflected destination row below wall
	moveq	#0,d5		; reflection row index
.row_loop
	; ordered fade: visible near the wall-foot, then thinning out.
	; c86zh: soften wall reflections by roughly one third.  The same fog and
	; distance-grain logic stays active, but the base keep thresholds are lower
	; so reflections read less intense and less solid on shiny floors.
	moveq	#7,d3		; top: was 10/16, now ~33% weaker
	cmp	#2,d5
	blt.s	.have_thr
	moveq	#5,d3		; upper mid: was 7/16
	cmp	#5,d5
	blt.s	.have_thr
	moveq	#3,d3		; lower: was 4/16
	cmp	#8,d5
	blt.s	.have_thr
	moveq	#1,d3		; tail: was 2/16
.have_thr
	move	vd_z(a0),d1
	tst	g2_visibility
	bgt.s	.fog_adv
...
.fog_done
	...
	bra	.skip_px
.fog_keep
	; c86zg: make distant wall-reflection pixels coarser, but do not touch
	; d7.  c86zf used d7 as a temporary inside this row loop, which also is
	; the outer column DBF counter; that could make the reflection flicker and
	; disappear while moving.  Keep the grain phase in d1 instead.
	move	vd_z(a0),d2
	move	d4,d0
	move	d5,d1
	tst	g2_visibility
...
.g2v190dm_floor_fog
	...
	jsr	g2fill_disabled_flat_fog
.nofloor	;
	; c86zb: wall reflection version A.  Mirror each real wall column into the
	; floor below its own bottom edge with a light ordered-dither fade.
	; REFLECTIONS NO still disables the effect.
	tst	g2_reflections
	ble.s	.g2c86zb_no_wall_refl
	move	floorflag(pc),d0
	ble.s	.g2c86zb_no_wall_refl
	jsr	g2_draw_wall_reflection_lite
.g2c86zb_no_wall_refl
...
.yloop
	cmp.l	a1,a0
	bhi	.rts
	; c86y: 4x4 ordered fade. Keep the reflected sprite pixels recognisable
	; near the feet, but fade earlier and more aggressively toward the lower
	; end.  This avoids c86x becoming too invisible while still reducing tail.
	moveq	#11,d4		; top: 11/16 pixels kept - still recognisable
	cmp	#2,d5
	blt.s	.dither_pick
	moveq	#7,d4		; early mid: 7/16
	cmp	#6,d5
	blt.s	.dither_pick
	moveq	#4,d4		; lower: 4/16
	cmp	#12,d5
	blt.s	.dither_pick
	moveq	#2,d4		; tail: 2/16
.dither_pick
	move	g2_enemy_ref_curx(pc),d0
	and	#3,d0
	move	d5,d1
...
p96prevlog_name	dc.b	'RAM:gloom_p96_preview.txt',0
	even
p96prevlog_ok	dc.b	'P96 preview: c86y P96 off clearer enemy and visible wall reflection lite active',10
