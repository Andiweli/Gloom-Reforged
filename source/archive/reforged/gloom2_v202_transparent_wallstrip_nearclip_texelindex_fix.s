...
.pixnz	cmp	#22,ob_pixsize(a5)	;v105c: keep teleport animation visible 10 frames longer before black hold
	...
	move	#8,g2teleport_black_hold	;v197: half-length black hold after teleport sequence
...
solidstrip	set	0
drawstrip2	; v201 chunky-safe transparent wall strip overlay crashfix + darker alpha
	; a1=top of destination column, a2=palettes base, a4=strip/vd data
	; Non-zero texels are drawn through the current wall palette. Transparent
	; texels now use the same packed-RGB 50% blend method as drawobjtrans:
	; background RGB (from planar_palette) blended with a darker green RGB12
	; tint, then remapped through planar_remap. This keeps the glass darker
	; and advances texture stepping before the blend so near-camera strips do not lose parts.
	movem.l	d0-d7/a0-a5,-(a7)
	move.l	vd_data(a4),d0
	beq	.rts
	move.l	d0,a0		; source texture column
	move	#$060,d6		; much darker green glass RGB12 tint
	move.l	chunkymod(pc),d7
	move	vd_y(a4),d2
...
.hiteok
	...
	move	vd_pal(a4),d5
	move.l	0(a2,d5*4),a5
	move	#$0eee,d2		; packed-RGB even mask for safe 50% average like drawobjtrans
	move.l	planar_palette(pc),a3	; chunky palette index -> RGB12
	move.l	planar_remap(pc),a2	; RGB12 -> chunky palette index
.loop
	moveq	#0,d5		; v202: clear texel index each row to avoid stale high bits
	move.b	0(a0,d0),d5
	beq.s	.glass
	move.b	0(a5,d5.w),(a1)
	addx.l	d3,d0		; advance texture while X is still intact
	bra.s	.next
.glass	addx.l	d3,d0		; v201: advance before RGB blend clobbers CCR/X
	move	d6,d5
	and	d2,d5		; glass tint RGB12 (dark green)
	moveq	#0,d1
	move.b	(a1),d1		; already-rendered background pixel
	move	0(a3,d1*4),d1	; background RGB12
	and	d2,d1		; safe packed-RGB mask like drawobjtrans
	add	d1,d5
	lsr	#1,d5		; 50% background + 50% green glass
	move.b	0(a2,d5.w),(a1)
.next	add.l	d7,a1
	dbf	d4,.loop
.rts	movem.l	(a7)+,d0-d7/a0-a5
	...
	dc	$ffff
	;
g2_stripglass_rgb	; v191 RGB12 tint colours for transparent wall strips
	dc	$f00,$0f0,$00f,$ff0,$f0f,$0ff,$fff
	;
