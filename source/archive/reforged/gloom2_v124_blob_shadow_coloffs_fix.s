...
; -----------------------------------------------------------------------------
; v124 ZGloom-style enemy blob shadow, fixed coloffs X writes.
; The Android ZGloom renderer draws the shadow from the already projected sprite
; box: cx = sprite centre, cy = sprite bottom, rx/ry from visible sprite width.
; On Amiga we prepare that box before drawobjnorm/invs, then draw the small dark
; oval after the sprite.  The later draw makes this first test deliberately
; visible, while the hard object checks keep bullets/pickups/gore out.
; -----------------------------------------------------------------------------
...
.rymaxok
	move	d1,g2_blob_ry
	; map visible dark grey into the chunky palette.  v124 uses the same
	; RGB->chunky lookup style as the chunky blood particles; do not apply
	; paladjust here or the colour can collapse to an invisible index.
	move	#$555,d4
	move.l	planar_remap(pc),a0
	tst.l	a0
	beq.s	.colfallback
	...
	and	#$00ff,d4
	bra.s	.colok
.colfallback	moveq	#7,d4
.colok	move	d4,g2_blob_col
