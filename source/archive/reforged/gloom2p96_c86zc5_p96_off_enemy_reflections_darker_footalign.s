	...
	;gloom2p96_c86zc5: wall reflections removed; enemy reflections refined with denser darker foot-seam reflection, c86zb3 per-column coverblocking, stronger distance build-up and coarser downward fade.
	;
...
.yloop
	cmp.l	a1,a0
	bhi	.rts
	; c86zc5: make the seam at the floor less obviously dithered.  Keep a
	; denser band directly under the enemy, but darken the colours so the
	; reflection still feels subdued.  Downward it gets coarser and sparser.
	moveq	#10,d4		; top seam: 10/16 pixels kept, but darkened
	cmp	#3,d5
	blt.s	.dither_pick
...
.dither_pick
	...
	move.b	(a3),d0
	beq.s	.next
	bsr	g2_enemy_reflection_dark_remap
	move.b	d0,(a0)
.next
	add.l	d7,a0
	sub.l	d7,a3
...
; c86zc5: remap one already-rendered chunky pixel to a darker reflected
; version.  This reduces the harsh dither look right at the floor seam by
; letting us keep more pixels there while still darkening the reflection.
; in: d0.b = source chunky colour index, out: d0.b = darker remapped colour.
g2_enemy_reflection_dark_remap
	movem.l	d1-d3/a2-a3,-(a7)
	move.l	planar_palette(pc),a2
	move.l	planar_remap(pc),a3
	tst.l	a2
	beq.s	.keep
	tst.l	a3
	beq.s	.keep
