...
newwsize	cmp	#320,width
	bne.s	.nf
	cmp	#224,hite
	bcc.s	sizedone		;already FULLSCREEN, stop
.nf	move	width(pc),d0
	cmp	#96,d0
	bcs.s	.s96
	cmp	#128,d0
...
decwsize	move	width(pc),d0
	...
	cmp	#288,d0
	bls.s	.s256
	bra.s	.s288		;FULLSCREEN/320 -> ALMOST FULL
.s64	move	#64,width
	move	#64,hite
	bra.s	sizedone
.s96	move	#96,width
...
.pok	move	(a2),d0
	...
	bra.s	copycntrltext
	;
cntrlright_noloop	;v111: no duplicate control option between PLAYER 1/2
	move	(a2),d0
	cmp	#5,d0
	bge.s	.rts
	addq	#1,d0
.r_try	cmp	(a3),d0		;skip the other player's current option, including KEYBMOUSE
	bne.s	.r_set
	addq	#1,d0
	cmp	#5,d0
	bgt.s	.rts
...
.rts	rts
	;
cntrlleft_noloop	;v111: no duplicate control option between PLAYER 1/2
	move	(a2),d0
	tst	d0
	ble.s	.rts
	subq	#1,d0
.l_try	cmp	(a3),d0		;skip the other player's current option, including KEYBMOUSE
	bne.s	.l_set
	subq	#1,d0
	blt.s	.rts
	bra.s	.l_try
...
.rts	rts
	;
g2_title_sync_controls	;v112: rebuild title control text from actual values every time
	movem.l	d0/a1,-(a7)
	move	p1_ob_cntrl,d0
	cmp	p2_ob_cntrl,d0
	bne.s	.ok
	...
	clr	p2_ob_cntrl
	bra.s	.ok
.p2_keyboard	move	#1,p2_ob_cntrl
.ok	lea	p1ctype,a1
	move	p1_ob_cntrl,d0
	bsr.s	copycntrltext
	lea	p2ctype,a1
...
p1ctype
	...
	dc.b	'CD32 PAD 1',0	;4
	elseif
	dc.b	'KEYBMOUSE  ',0	;4 v112 fixed 10-char slot
	endc
	;
	dc.b	'PLAYER 2 '
p2ctype
	...
	dc.b	'CD32 PAD 2',0	;5
	elseif
	dc.b	' KEYBOARD ',0	;5 v112 fixed 10-char slot
	endc
	;
	dc.b	92,0			;6 spacer
...
p2_ob_cntrl
	...
	elseif
	dc	1	;v111 KEYBOARD default, never duplicate PLAYER 1 KEYBMOUSE
	endc
