	...
	;gloom2p96_c86zc2: experimental full weak reflections with distance-building reflection height; c86zb3 enemy coverblock kept.
	;
...
.depth_min_ok
	...
	cmp	#(8<<grdshft),d5
	blo.s	.depth_mid
	moveq	#3,d5		; far: max 4 rows
	bra.s	.depth_cap_apply
.depth_mid
	moveq	#11,d5		; medium: max 12 rows
	bra.s	.depth_cap_apply
.depth_near
	moveq	#31,d5		; near: max 32 rows
	bra.s	.depth_cap_apply
.depth_adv
	cmp	#(8<<grdshft),d5
	blo.s	.depth_near
	cmp	#(12<<grdshft),d5
	blo.s	.depth_mid
	moveq	#3,d5		; far advanced: max 4 rows
.depth_cap_apply
	cmp	d5,d6
	ble.s	.depth_cap_ok
	move	d5,d6
...
.row_loop
	...
	moveq	#3,d3		; top: 3/16 pixels kept
	cmp	#8,d5
	blt.s	.have_thr
	moveq	#2,d3		; mid: 2/16
	cmp	#20,d5
	blt.s	.have_thr
	moveq	#1,d3		; tail: 1/16
.have_thr
...
.nofloor	;
	...
	tst	g2_reflections
	ble.s	.g2c86zc1_no_wall_reflect
	move	floorflag(pc),d0
	ble.s	.g2c86zc1_no_wall_reflect
	jsr	g2_draw_wall_reflection_lite
.g2c86zc1_no_wall_reflect
...
.rh_min_ok
	...
	cmp	#(2<<grdshft),d1
	blo.s	.rh_mid
	moveq	#8,d1		; far within reflection range: max 8 rows
	bra.s	.rh_cap_apply
.rh_mid
	moveq	#20,d1		; medium: max 20 rows
	bra.s	.rh_cap_apply
.rh_near
	moveq	#40,d1		; close: max 40 rows
.rh_cap_apply
	cmp	d1,d0
	ble.s	.rh_max_ok
	move	d1,d0
...
.yloop
	...
	moveq	#4,d4		; top: 4/16 pixels kept
	cmp	#4,d5
	blt.s	.dither_pick
	moveq	#3,d4		; early mid: 3/16
	cmp	#10,d5
	blt.s	.dither_pick
	moveq	#2,d4		; lower: 2/16
	cmp	#20,d5
	blt.s	.dither_pick
