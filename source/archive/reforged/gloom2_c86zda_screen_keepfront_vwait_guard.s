	...
	;gloom2_c86zcx: add SCREENQUIET to the OS custom screen so hidden mouse clicks can not hit the screen-depth gadget; add minimal OpenScreen/OpenWindow null guards.
	;gloom2_c86zcy: remove WFLG_BACKDROP from the game window so the fullscreen borderless window can sit above the screen depth gadget/hot corner; keep SCREENQUIET and null guards.
	;gloom2_c86zcz: additionally hide the screen title/depth bar with ShowTitle(FALSE), and immediately refocus the game screen on IDCMP_INACTIVE.
	;gloom2_c86zda: add throttled ScreenToFront watchdog from OS vwait so hidden pointer depth-clicks cannot leave game on Workbench.
	;
...
.osvwait	movem.l	d0-d1/a0-a1/a6,-(a7)
	...
	jsr	g2c86zda_keep_screen_front_vwait	;c86zda: recover immediately if hidden mouse depth-click pushed the screen back
	ifne	g2p96_overlay_enabled
...
newscreen_v	dc	0	;viewmode
	dc	$14f	;type ;c86zcx: +SCREENQUIET, disables screen bar/depth gadget clicks while OS mouse is hidden
	dc.l	0	;font
	dc.l	0	;title
	dc.l	0	;gadgets
...
newwindow	dc	0,0	;x,y
	...
	dc.l	$11840	;flags! c86zcy: ACTIVATE|BORDERLESS|SIMPLE_REFRESH|RMBTRAP, no BACKDROP so screen depth gadget is covered
	dc.l	0	;gadgets
	dc.l	0	;checkmark
	dc.l	0	;title
...
g2c86zcz_refront_screen	;keep hidden-pointer screen-depth clicks from landing on Workbench
	movem.l	d0/a0/a6,-(a7)
	move.l	int,d0
	beq.s	.rts
	move.l	d0,a6
	...
	move.l	d0,a0
	moveq	#0,d0
	jsr	-282(a6)	;ShowTitle(screen,FALSE) -- hide title/depth bar again
	move.l	screen,a0
	jsr	-252(a6)	;ScreenToFront
.input	movem.l	(a7)+,d0/a0/a6
	bsr	inputon		;keeps raw input active if a depth-click caused IDCMP_INACTIVE
	rts
.rts	movem.l	(a7)+,d0/a0/a6
	rts

g2c86zda_keep_screen_front_vwait	;c86zda: lightweight front watchdog from OS vwait task context
	; Screen/window flags alone cannot stop every AmigaOS depth-gadget click when
	; the Intuition pointer is invisible.  If such a click pushes us behind, pull
	; the game screen back to front on the next vwait.  Throttled to avoid doing
	; an Intuition call on every frame.
	movem.l	d0/a0/a6,-(a7)
	tst	active
	beq.s	.rts
	addq.w	#1,g2c86zda_front_tick
	and.w	#3,g2c86zda_front_tick
	bne.s	.rts
	move.l	int,d0
	beq.s	.rts
	move.l	screen,d0
...
g2c86zda_front_tick	dc.w	0

windowtask	move.l	int(pc),a6	;intuition base
	lea	newwindow(pc),a0
	jsr	-204(a6)	;openwindow
	move.l	d0,oswindow
	beq.s	.rts	;c86zcx: do not dereference a failed OpenWindow
...
.loop	move.l	msgport(pc),a0
	...
	cmp.l	#$80000,d0
	bne.s	.skip
	; c86zcz: a hidden mouse click on the screen-depth corner can still deactivate
	; the game screen on some setups.  Do not release input here; pull the
	; game screen/window back to front and keep the invisible pointer active.
	bsr	g2c86zcz_refront_screen
	bra.s	.skip
	;
...
.agasc	move.l	int(pc),a6
	...
	move.l	d0,a0
	move.l	a0,screen
	bne.s	.g2c86zcx_screen_ok
	clr.w	os	;c86zcx: failed OS screen, avoid null-screen gray/pointer hang by falling back to custom display path
	bra.w	.noos
.g2c86zcx_screen_ok
	move.l	int(pc),a6
	move.l	screen(pc),a0
	moveq	#0,d0
	jsr	-282(a6)	;c86zcz: ShowTitle(screen,FALSE), hide screen title/depth gadget
	jsr	g2v36_hide_pointer	;v39: hide OS mouse sprite immediately after custom screen opens
	move.l	screen(pc),a0	;c86zcz: ShowTitle may clobber a0, restore Screen pointer
	lea	44(a0),a0
	;
	move.l	a0,viewport
	move.l	grbase(pc),a6
