...
g2fill_void_fog
	; v190bb: corridor/room openings borrow only the neighbouring wall span,
	; and use one sampled dark colour instead of copying full texture rows.
	; This avoids both the horizontal texture bars and the black far portals.
	movem.l	d0-d7/a0-a6,-(a7)
	move.l	chunky(pc),a0
	tst.l	a0
	beq.w	.g2fv_done
	...
	;
	; pass 1: left -> right, use previous solid wall column as source.
	lea	coloffs(pc),a1
	move	width(pc),d7
	ble.w	.g2fv_pass2
	subq	#1,d7
...
.g2fv_lx
	...
	ble.s	.g2fv_lnext
	bsr	g2fv_fill_wallspan_uniform
.g2fv_lnext
	lea	vd_size(a6),a6
	addq.l	#4,a1
	addq	#1,d6
	dbf	d7,.g2fv_lx
	;
	; pass 2: right -> left, use next solid wall column as source.
.g2fv_pass2
	move	width(pc),d7
	ble.w	.g2fv_done
	subq	#1,d7
...
; a2 = destination column top (empty portal column)
; a3 = source column top (neighbouring solid wall column)
; d0 = wall top Y on screen, d4 = chunkymod, d5 = wall height
; Fills only the clipped wall span with one sampled source colour.
g2fv_fill_wallspan_uniform
	tst	d0
