...
solidstrip	set	0
drawstrip2	; v198 chunky-safe transparent wall strip overlay with true green alpha
	...
	; texels are blended channel-wise 50/50 against the already-rendered chunky
	; pixel using planar_palette + planar_remap, matching the safe drawobjtrans
	; RGB path without packed-RGB carry artefacts.
	movem.l	d0-d7/a0-a5,-(a7)
	move.l	vd_data(a4),d0
	beq	.rts
	move.l	d0,a0		; source texture column
	move	#$0f0,d6		; pure green glass RGB12 tint before 50% channel blend
	move.l	chunkymod(pc),d7
	move	vd_y(a4),d2
...
.loop
	...
	bra.s	.next
.glass	moveq	#0,d5
	move.b	(a1),d5		; already-rendered background pixel
	move	0(a3,d5*4),d5	; background RGB12, same table as drawobjtrans
	move	d5,d1
	lsr	#8,d1
	and	#$000f,d1	; R
	lsr	#1,d1		; (R + 0) / 2
	...
	lsl	#4,d2
	or	d2,d1
	or	d5,d1		; blended RGB12 without packed-nibble carry
	move.b	0(a2,d1.w),(a1)
.next	addx.l	d3,d0
	add.l	d7,a1
	dbf	d4,.loop
