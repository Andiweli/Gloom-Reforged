...
.g2st_make_green
	; Convert current RGB to a dark green glass tint.  Even black behind glass
	; gets a small green component so green windows do not disappear to black.
	move	d0,d1	; red
	lsr	#8,d1
	and	#$000f,d1
	move	d0,d2	; green
	...
	move	d0,d6	; blue
	and	#$000f,d6
	move	d1,d5	; brightness
	add	d2,d5
	add	d6,d5
	divu	#3,d5
	; v190cc: darker green glass.  v190cb made the transparent
	; windows too bright/grell.  Keep only a tiny red/blue component and
	; cap the green channel in the dark range so the pane reads as tinted
	; shadow instead of a luminous overlay.
	lsr	#3,d1
	lsr	#3,d6
	add	d5,d2
	lsr	#2,d2
