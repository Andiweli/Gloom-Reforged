...
.g2st_max_g_ok
	...
	move	d6,d5
.g2st_max_b_ok
	; v190cg: darker, at-least-50%-transparent green glass.
	; Keep half of the original background colour, then add only a dark
	; green component based on the original brightness.  This keeps dark
	; walls dark and lets bright lamps/textures remain visible through the pane.
	lsr	#1,d1	; red = 50% original
	lsr	#1,d6	; blue = 50% original
	lsr	#1,d2	; green = 50% original green
	lsr	#2,d5	; plus dark green overlay, max 25% brightness
	add	d5,d2
	cmp	#15,d2
