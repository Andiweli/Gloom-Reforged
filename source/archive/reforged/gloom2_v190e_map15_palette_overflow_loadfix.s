...
.no	;colour doesn't exist!
	; v190e: chunky pixels are 8-bit palette indices.  Later maps can push
	; the accumulated RGB list past 256 colours during texture loading; the
	; old code still appended and then truncated the index, corrupting the
	; palette/remap tables and crashing during the intermission -> map1_5 load.
	; Keep the original large RGB scratch pool, but clamp new colours once the
	; 256 visible chunky entries are full.  Overflow colours reuse the last
	; valid entry instead of writing past the generated palette table.
	move.l	a2,d3
	sub.l	map_rgbs(pc),d3
	cmp.l	#256*2,d3
	bcs.s	.addnewrgb
	move.l	a2,a1
	bra.s	.yes
...
map_rgbs_	ds.w	256*16	;v190e restore original-sized RGB pool; avoids map palette overflow on later maps
