...
pixelate	movem.l	d0-d7/a0-a5,-(a7)
	...
	move	#64,d6
.sizeok
	; v97: ZGloom-style teleport/death pixelate.
	; ApplyTeleportPixelate() in ZGloom uses the current fade/pixel size as
	; block size, clamps it, and starts the first X/Y block at half size.
	; That half-block border keeps the effect centred and avoids the coarse
	; top-left anchored chunking from v93.
	moveq	#0,d4		;Y block start
.yloop	cmp	hite(pc),d4
	bge.w	.done
	move	d6,d5		;block height
	tst	d4
	bne.s	.yfull
	lsr	#1,d5		;first block row is half height, ZGloom-style
	bne.s	.yfull
	moveq	#1,d5
.yfull	move	hite(pc),d0
	sub	d4,d0		;remaining rows
	cmp	d0,d5
	ble.s	.bhok
	move	d0,d5
.bhok	move.l	chunky(pc),a5
	move	d4,d3
...
.xloop	cmp	width(pc),d2
	bge.s	.nexty
	move	d6,d1		;block width
	tst	d2
	bne.s	.xfull
	lsr	#1,d1		;first block column is half width, ZGloom-style
	bne.s	.xfull
	moveq	#1,d1
.xfull	move	width(pc),d0
	sub	d2,d0		;remaining columns
	cmp	d0,d1
	ble.s	.bwok
	move	d0,d1
.bwok	lea	coloffs(pc),a1
	move.l	a5,a0
	add.l	0(a1,d2*4),a0
	moveq	#0,d7
	move.b	(a0),d7		;sample colour from block origin
	move.l	a5,a2		;current destination row base
	move	d5,d3
	subq	#1,d3		;DBF row count
.yfill	move	d1,d0
	subq	#1,d0		;DBF column count
	move	d2,d5		;current X inside this block
.xfill	move.l	a2,a3
	add.l	0(a1,d5*4),a3
	move.b	d7,(a3)
	addq	#1,d5
	...
	dbf	d3,.yfill
	add	d1,d2		;advance by actual/clipped block width
	bra.s	.xloop
.nexty	move	d6,d5
	tst	d4
	bne.s	.yaddfull
