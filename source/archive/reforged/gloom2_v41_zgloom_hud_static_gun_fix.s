...
g2drawchunky_a0	;a0=shapetable, d0=x,d1=y,d2=shape#
	add	#224,d1
	;
g2drawchunky_abs_a0	;a0=shapetable, d0=x,d1=y,d2=shape# absolute Y
...
; v41: absolute chunky draw helper for ZGloom-style top HUD.
; Unlike drawchunky, this does not force y += 224.
g2drawchunky_abs	;draw panel shape at absolute chunky coordinates
	move.l	panel(pc),a0
	bra.w	g2drawchunky_abs_a0

; v41: print smallfont2/panel text into chunky at absolute coordinates.
; a4=zero-terminated text, d7=x, d6=y
g2drawhudtext
...
; v41: top HUD modelled after the stable gloom.s / ZGloom style.
; This replaces the broken bottom statusbar texture path for now.
g2showhud_top	;a5=player
	movem.l	d0-d7/a4,-(a7)
	lea	g2hud_health(pc),a4
	moveq	#2,d7
	moveq	#2,d6
	bsr	g2drawhudtext
	lea	g2hud_weapon(pc),a4
	moveq	#2,d7
	moveq	#12,d6
	bsr	g2drawhudtext
	;
...
g2hud_health	dc.b	'HEALTH',0
g2hud_weapon	dc.b	'WEAPON',0
	even
...
; v20: optional first-person gun overlay.
; The original public master does not reference a gun file, but some game
; data sets have misc/gun. If it is absent or not a sane shape table, skip.
g2drawgun
	;v40: draw optional first-person gun from Gloom/Deluxe gun.bin format.
	;Format matches ZGloom ObjectGraphics::LoadGraphic:
	; word shift, word framebase, word maxw, word maxh, long paletteoff,
	; then long frame offsets.  Each frame is xh/yh/w/h + column-major pixels.
	movem.l	d0-d7/a0-a6,-(a7)
	move.l	gunpic(pc),d0
	beq.w	.g2dg_done
	move.l	d0,a0
	cmp.l	#'CrM2',(a0)
	beq.w	.g2dg_done	;should have been decrunched by loadfile
	cmp.l	#'CrM!',(a0)
	beq.w	.g2dg_done
	;
	moveq	#0,d5
	move	2(a0),d5	;frame base count
	moveq	#0,d0
	move	(a0),d0		;shift
	cmp	#5,d0
	bhi.w	.g2dg_done
	lsl.l	d0,d5		;frame count
	beq.w	.g2dg_done
	cmp.l	#64,d5
	bhi.w	.g2dg_done
	;
	move.l	8(a0),d4	;palette offset
	beq.w	.g2dg_done
	cmp.l	#$20000,d4
	bhi.w	.g2dg_done
	lea	0(a0,d4.l),a3	;palette
	;
	moveq	#0,d6		;frame 0 normally
	...
	cmp.l	#1,d5
	bls.s	.g2dg_frameok
	moveq	#1,d6		;frame 1 while firing/reloading
.g2dg_frameok
	move.l	12(a0,d6.l*4),d4
	beq.w	.g2dg_done
	cmp.l	#$20000,d4
	bhi.w	.g2dg_done
	lea	0(a0,d4.l),a2	;frame
	;
	move	(a2),d0		;xh
	move	2(a2),d1	;yh
	move	4(a2),d2	;width
	...
	bhi.w	.g2dg_done
	;
	;v41: destination bottom-centred like ZGloom, no bottom statusbar.
	move	#160,d4
	sub	d0,d4		;x = centre - xhandle
	bpl.s	.g2dg_xok
	moveq	#0,d4
.g2dg_xok
	move	#240,d7
	sub	d3,d7		;y = screen bottom - height
	bpl.s	.g2dg_yok
	moveq	#0,d7
.g2dg_yok
	;v41: no idle/movement bob yet.  The previous ob_movspeed/vbcounter
	;bob made the weapon animate even while standing still on some builds.
	;
	lea	8(a2),a2	;pixel data
	move.l	chunky(pc),a1
	move.l	d7,d0
	mulu	#320,d0
	add.l	d0,a1
	add	d4,a1
	move.l	planar_remap(pc),a4
	;
	subq	#1,d2		;width loop count
	subq	#1,d3		;height loop count template
	move.l	#320,d4
	sub	d2,d4
	subq	#1,d4		;dest advance after each column = 320-height? no, column uses +320, then next column back up and +1
	;
.g2dg_xloop
	move	d3,d7
	move.l	a1,a5
.g2dg_yloop
	...
	beq.s	.g2dg_skip
	lsl	#1,d0
	move	0(a3,d0.w),d0	;12-bit RGB from gun palette
	and	#$0fff,d0
	move.b	0(a4,d0.w),(a5)	;RGB -> active chunky palette index
.g2dg_skip
	add.l	#320,a5
	dbf	d7,.g2dg_yloop
	addq.l	#1,a1
...
g2loadgun
	;v41: try the real gun.bin locations used by ZGloom/data sets.
	;Ignore the earlier optional progfiles misc/gun load so the .bin path
	;is preferred and stale/non-bin data can not lock the renderer.
	movem.l	d0-d1/a0,-(a7)
	clr.l	gunpic
	tst.l	gunpic
	bne.s	.g2lg_done
	moveq	#1,d1
	lea	g2gun_misc_bin,a0
	jsr	loadfile
	move.l	d0,gunpic
	bne.s	.g2lg_done
	moveq	#1,d1
	lea	g2gun_stuf_bin,a0
	jsr	loadfile
	move.l	d0,gunpic
	bne.s	.g2lg_done
	moveq	#1,d1
	lea	g2gun_misc_plain,a0
	jsr	loadfile
	move.l	d0,gunpic
	bne.s	.g2lg_done
	moveq	#1,d1
	lea	g2gun_stuf_plain,a0
	jsr	loadfile
	move.l	d0,gunpic
.g2lg_done
	movem.l	(a7)+,d0-d1/a0
...
g2gun_misc_bin	dc.b	'misc/gun.bin',0
g2gun_stuf_bin	dc.b	'stuf/gun.bin',0
g2gun_misc_plain	dc.b	'misc/gun',0
g2gun_stuf_plain	dc.b	'stuf/gun',0
	even
...
showstats	;
	;a5=player
	;
	;v40: keep the textured green statusbar background (#47) visible.
	;The old middle-clear shape #46 overwrote the base strip, so only
	;dynamic status contents are drawn here now.
	;
	;hitpoints
	move	ob_hitpoints(a5),d7
	ble.s	.nohp
...
blitscene	;
	...
	bclr	#7,ob_update(a5)
	bsr	g2clearpanelchunky	;v41: bottom statusbar disabled, keep strip deterministic/black
	bsr	g2drawgun		;weapon overlay remains bottom-centred
	bsr	g2showhud_top		;ZGloom/stable-gloom style top HUD
	move	#2,panelcnt		;convert lower 16 lines for gun tail/black strip
	rts
...
progfiles	dc.l	gloomcfg
	...
	even
	dc.l	gunpic
	dc.b	'misc/gun',0	;v20 optional, skipped if missing
	even
	dc.l	titlemed+1
	dc.b	'sfxs/med1',0
	even
...
.agaa	bsr	loadfiles
	...
	lea	ecsfiles,a0
.lf	bsr	loadfiles
	jsr	g2loadgun	;v40: optional misc/gun.bin / stuf/gun.bin loader
	;
...
scripttext	;print text on iff
	...
	lea	text,a4
	move	bmaphite(pc),d6
	sub	#7,d6	;v15: match gloom.s lower text baseline
	;
	move.l	a4,a0
	moveq	#0,d1
.loop	move.b	(a0)+,d2
...
; v22: very small OS/DOS file logger for first-level crash hunting.
; Writes to DH3:gloom2.log.  It is intentionally marker-only, not per-pixel
; or per-object, to avoid turning the logger itself into the next boss enemy.
g2log_open
...
g2logname	dc.b	'DH3:gloom2.log',0
g2log_nl	dc.b	10
g2loghand	dc.l	0
g2logframe	dc	0
g2log_msg_open	dc.b	'G2LOG OPEN v28 DRAWSCENE DH3',0
...
newscreen	dc	40,0	;x,y ;v20: halfway center between v18(32) and v19(48)
