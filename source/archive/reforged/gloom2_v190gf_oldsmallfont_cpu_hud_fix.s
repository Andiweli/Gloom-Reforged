...
g2hud_clear_top_area
	; Clear a small stable HUD rectangle before drawing old smallfont shapes.
	; Works for both interleaved AGA (bpmod=40, linemod=320) and plane-major RTG/OS
	; (bpmod=40*240, linemod=40), because it uses bpmod/linemodw directly.
...
.g2hudc_plane
	...
	moveq	#21,d6
.g2hudc_y
	moveq	#7,d5		; 128 pixels = 16 bytes = 8 words per row
.g2hudc_x
	clr.w	(a1)+
	dbf	d5,.g2hudc_x
	adda.w	linemodw(pc),a1
	dbf	d6,.g2hudc_y
	add.l	bpmod(pc),a0
	dbf	d7,.g2hudc_plane
.g2hudc_done
...
g2hud_blit_oldsmallfont	;a0=old smallfont shapetable, a1=bitmap, d0=X, d1=Y, d2=char
	; v190gf: CPU copy for old Gloom smallfont.bin shape layout.
	; The old Gloom blitter assumes 7 planes interleaved per scanline.  This routine
	; reads that source layout and writes into the active Gloom2 bitmap layout using
	; bpmod/linemodw, so it is safe on RTG/OS plane-major buffers too.
	movem.l	d0-d7/a0-a6,-(a7)
	tst.l	a0
	beq.w	.g2hudb_done
	tst.l	a1
	...
	move.l	a0,a2
	add.l	(a0),a2		; a2 = mask/cookie rows
	lea	8(a0),a3		; a3 = image rows
	move	4(a0),d2		; width in words
	ble.w	.g2hudb_done
	cmp	#8,d2
	bhi.w	.g2hudb_done	; defensive: HUD glyphs/cells are small
	move	6(a0),d3		; blitter size: rows<<6 | widthwords
	lsr	#6,d3			; total source rows = oldHeight * 7 planes
	ble.w	.g2hudb_done
	moveq	#7,d4
	...
	mulu	linemodw(pc),d1
	add.l	d1,a4
	lsr	#3,d0			; byte-align x.  HUD caller uses byte-aligned positions.
	add.w	d0,a4
