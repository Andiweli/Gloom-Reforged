...
; v164: PLAYER control selector.  PLAYER 1/2 may choose any input
; method, but never the method currently used by the other player.
; KEYBMOUSE and KEYBOARD count as the same keyboard method.
inccntrl	addq	#1,(a2)
	cmp	#6,(a2)
	bcs.s	.pok
	clr	(a2)
.pok	move	(a2),d0
	bsr.s	g2v164_cntrl_conflict
	beq.s	inccntrl
	bra.s	g2v164_cntrl_copy
...
g2v164_cntrl_conflict	; in: d0=candidate, a3=other player's cntrl. EQ=blocked
	move	(a3),d1
	cmp	#2,d0
	bcc.s	.nonkeyboard_candidate
	cmp	#2,d1
	bcs.s	.blocked	; KEYBMOUSE/KEYBOARD are one shared method
	bra.s	.ok
.nonkeyboard_candidate
	cmp	d1,d0
	beq.s	.blocked
.ok	moveq	#1,d1
	rts
...
.notlinked
	move	d0,d7
	and	#$0300,d7	; v164: $0100=left, $0200=right for PLAYER rows
	and	#$00ff,d0
	tst	d7
	beq.s	.notlinked_fire
	cmp	#4,d0
	beq	.p1lrsel
	cmp	#5,d0
	beq	.p2lrsel
...
p1ctype
	...
	dc.b	'CD32 PAD 1',0	;4
	elseif
	dc.b	'KEYBMOUSE',0,0,0	;4 v164: reserve room for 10-char control names
	endc
	;
	dc.b	'PLAYER 2 '
p2ctype
	...
	dc.b	'CD32 PAD 2',0	;5
	elseif
	dc.b	'JOYSTICK 2',0		;5 v164: non-conflicting default versus P1 KEYBMOUSE
	endc
	;
...
.encode	lea	joyx(pc),a0
	...
	rts
.normal_menu_mask	and	#$1f,d0	;v164 allow title PLAYER rows to see left/right
	rts
...
.noescsel	btst	#0,d0
	...
	bne.s	g2v165_sel_up
	btst	#3,d0
	bne.s	g2v165_sel_down
	;
	;selected!
	;
	bsr	menuskip	;v115 visual spacer rows are never selectable
	beq.s	g2v165_sel_down
	move	curropt(pc),d0
	rts
	;
.left	tst	game_menu_active
	bne.s	.leftret
	bsr.s	g2v164_title_player_lr
	beq	selmenu
.leftret	move	curropt(pc),d0
	or	#$0100,d0
	rts
.right	tst	game_menu_active
	...
	bsr.s	g2v164_title_player_lr
	beq	selmenu
.rightret	move	curropt(pc),d0
	or	#$0200,d0
	rts

g2v164_title_player_lr	; NE if normal title menu is on PLAYER 1/2 row
	move	numopts(pc),d1
	cmp	#11,d1
	bne.s	.no
	move	curropt(pc),d1
...
.upchk	bsr	menuskip
	...
	bra	selmenu
	;
g2v165_sel_down	addq	#1,curropt
	move	curropt(pc),d0
	cmp	numopts(pc),d0
	bcs.s	.downchk
	clr	curropt
...
p2_ob_cntrl
	...
	elseif
	dc	3	;v164 JOYSTICK 2 default, avoids sharing keyboard method with P1
	endc
