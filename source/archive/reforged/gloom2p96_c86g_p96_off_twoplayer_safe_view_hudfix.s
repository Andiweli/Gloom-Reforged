	...
	;gloom2p96_c86g: P96 overlay off; TWO PLAYER safe variable view width and fixed HUD.
	;
...
debugser	equ	0
debugmem	equ	0
g2p96_overlay_enabled	equ	0	; c86g: 0 hides/disables P96 overlay, -1 re-enables old c85 P96 preview path
	;
...
g2twop_drawall_split
	...
	move	#-1,g2twop_aspect_mode	;c86g: keep 2P projection height-locked
	;
...
g2twop_draw_half_hud
	...
	; c86g: keep the HUD at the normal split-half positions, independent
	; of the selected 3D view width.  HEALTH/WEAPON stay left, LIVES
	; stay right like ONE PLAYER GAME, instead of following the black
	; side borders of tiny views.
	jsr	showstats
...
g2twop_prepare_view_width
	; c86g: WINDOW SIZE controls the selected two-player view width, but
	; the wall caster is only trusted up to the old LARGE/160px range.
	; Earlier c86f allowed 192..320 and the side regions showed objects but
	; broken/missing walls.  Use distinct safe steps and clamp XL..FULL to
	; the largest stable width.
	...
	bcs.s	.medium
	bra.s	.large_or_more
.tiny	move	#132,d0	;gloom.s-style 66*2 baseline
	bra.s	.store
.small	move	#144,d0
	bra.s	.store
.medium	move	#152,d0
	bra.s	.store
.large_or_more
	move	#160,d0	;safe max for c86g wall casting
.store	move	d0,g2twop_view_width
	rts

g2twop_set_half_view
	; c86g: physical split half remains 320x120. Logical render width follows
	; the safe WINDOW SIZE map, while projection scale remains tied to the 120px split height.
...
p96prevlog_name	dc.b	'RAM:gloom_p96_preview.txt',0
	even
p96prevlog_ok	dc.b	'P96 preview: c86f p96 off twop wide fov active',10
