...
smallfont	move	#6,fontw
	...
	bne.s	.sfok
	move.l	panel(pc),d0	;v108: smallfont2.bin is loaded as panel/statusbar data
.sfok	move.l	d0,font
	rts
...
trainer_apply_one
	move	trainer_weapon(pc),d0
	beq.s	.weapon_default
	subq	#1,d0
	move	d0,ob_weapon(a5)
.weapon_default
	move	trainer_boost(pc),d0
	beq.s	.boost_default
	moveq	#6,d0
	sub	trainer_boost(pc),d0
...
trainer_update_wsize_text
	lea	game_wsize,a0
	lea	26(a0),a0
	lea	trainer_txt_custom(pc),a1
	cmp	#320,width
	bne.s	.copy
	cmp	#224,hite
	bne.s	.copy
	lea	trainer_txt_fullscreen(pc),a1
.copy	moveq	#15,d1
.blank	move.b	#' ',(a0)+
	dbf	d1,.blank
	lea	game_wsize,a0
...
.use	move	fonth(pc),d0	;v108: title menu from gloom.s sits a little higher
	...
	jsr	initmenu
	clr	menu_yadjust
	jsr	print_reforged_footer	;v108: smallfont grey footer
	;
.sel	jsr	selmenu
	;
	tst	linked
