...
dogamemenu	;
	...
	move	#$8020,$dff09a
	;
	; v190ag: Gloom-style in-game menu backdrop for the gloom2 renderer.
	; gloom2 has no plotwbmap/initstats path, so keep the original idea
	; but use the live renderer itself: switch player window palettes to
	; palettesw, render two clean frames, then draw the yellow menu font on
	; top.  The real palettes are restored before returning to gameplay.
	move.l	player1,a5
...
solidstrip	set	0
drawstrip2	; v190ae chunky transparent wall strip with map1 green-glass fallback
	; a1=top of destination column, a2=palettes base, a4=strip/vd data
	; Original gloom.s handles transparent wall strips by ANDing the already
	; drawn background with a selector mask stored in the byte before the
	; texture column.  In the 8-bit chunky path the direct planar AND is not
	; available, so zero texels normally stay transparent.  For first-episode
	; map1_x doors the original look is dark green glass; enable that only
	; while a map1_x level is active, not globally for later episodes.
	movem.l	d0-d7/a0-a5,-(a7)
	move.l	vd_data(a4),d0
	beq	.rts
	move.l	d0,a0		; source texture column
	moveq	#-1,d6		; default: pure transparency/no tint
	moveq	#0,d5
	move.b	-1(a0),d5	; original strip selector byte
	ext	d5
	cmp	#-7,d5
	blt.s	.sel_invalid
	cmp	#-2,d5
	bgt.s	.sel_check_white
	addq	#7,d5		; -7..-2 -> 0..5
	lea	g2_stripglass_rgb(pc),a3
	move	0(a3,d5*2),d6
	bra.s	.sel_done
.sel_check_white
	cmp	#-1,d5
	bne.s	.sel_invalid
	; selector -1 is the original white/no-colour mask.  Keep it clean except
	; in map1_x where the original visible glass is the well-known dark green.
	tst	g2v190ae_map1_green_glass
	beq.s	.sel_done
	move	#$060,d6		; dark green fallback only for map1_x glass
	bra.s	.sel_done
.sel_invalid
	tst	g2v190ae_map1_green_glass
	beq.s	.sel_done
	move	#$060,d6		; map1_x safety: transparent door holes become green glass
.sel_done
	move.l	chunkymod(pc),d7
	move	vd_y(a4),d2
...
.hiteok
	...
	move	vd_pal(a4),d5
	move.l	0(a2,d5*4),a5
	move	#$0eee,d2		; packed-RGB even mask for safe 50% average
	move.l	planar_palette(pc),a3	; chunky palette index -> RGB12
	move.l	planar_remap(pc),a2	; RGB12 -> chunky palette index
.loop
	moveq	#0,d5
	move.b	0(a0,d0),d5
	beq.s	.glass
	move.b	0(a5,d5.w),(a1)
	addx.l	d3,d0		; advance while X flag is still intact
	bra.s	.next
.glass	addx.l	d3,d0		; advance before RGB blend clobbers CCR/X
	tst	d6
	bmi.s	.next
	move	d6,d5
	and	d2,d5		; tint RGB12
	moveq	#0,d1
	move.b	(a1),d1		; already-rendered background pixel
	move	0(a3,d1*4),d1	; background RGB12
	and	d2,d1
	add	d1,d5
	lsr	#1,d5		; 50% background + 50% selector/map1 tint
	move.b	0(a2,d5.w),(a1)
.next	add.l	d7,a1
	dbf	d4,.loop
.rts	movem.l	(a7)+,d0-d7/a0-a5
	rts
	...
	dc	$ffff
	;
g2_stripglass_rgb	; dark RGB12 tints for original signed selectors -7..-2
	dc	$600,$060,$006,$660,$606,$066
	;
...
palettesw	ds.l	16	;v190ag 16 greyscale shade palettes for in-game menu
			;each 256 bytes long.
map_map	dc.l	0
map_grid	dc.l	0
map_poly	dc.l	0
...
.p_loop	move.l	d0,(a0)+
	...
	dbf	d1,.p_loop
	;
	move.l	#256*16,d0	;v190ag greyscale menu shade palettes
	moveq	#1,d1
	allocmem	palettesw
	lea	palettesw(pc),a0
	moveq	#15,d1
.pw_loop	move.l	d0,(a0)+
	add.l	#256,d0
	dbf	d1,.pw_loop
	;
...
.g2v190s_noskip
	bsr	fetchrest
.gotname	;
	bsr	g2v190ae_update_map1_glass
...
.g2v22_use_again
	...
	jsr	g2v10_showmsg
	bra	gameover
g2v190ae_update_map1_glass
	; v190ae: original planar transparency often leaves selector -1 as pure
	; glass, while first episode maps show green glass because of their green
	; lighting/door setup.  The chunky path does not reproduce that indirect
	; tint, so enable a map1_x-only green fallback for transparent strip holes.
	clr	g2v190ae_map1_green_glass
	lea	mapname,a0
	cmp.b	#'m',(a0)+
	bne.s	.rts
	cmp.b	#'a',(a0)+
	...
	cmp.b	#'_',(a0)+
	bne.s	.rts
	move	#-1,g2v190ae_map1_green_glass
.rts	rts
...
g2v190ae_map1_green_glass	dc	0	;v190ae map1_x transparent strips get dark green glass fallback
...
.bok	;
	...
	bcs.s	.loop
	;
	; v190ag: build a second 16-shade table for the in-game menu
	; background.  It maps every active map colour to a dim greyscale
	; equivalent through the same planar_remap/paladjust path, so the
	; yellow bigfont2 palette can be poked on top without recolouring the
	; visible wall pixels.
	lea	paladjust,a5
	move.l	map_rgbsat(pc),a4
	move.l	planar_remap(pc),a1
	lea	palettesw(pc),a2
	moveq	#0,d7
.gw_loop	move.l	(a2)+,a3
	move.l	map_rgbs(pc),a0
.gw_loop2	move	(a0)+,d0
	move	d0,d1
	move	d0,d2
	...
	add	d2,d1
	add	d3,d1
	lsr	#2,d1	;dim average, keeps menu backdrop subdued
	move	d7,d0
	lsr	#1,d0
	sub	d0,d1
	bpl.s	.gw_ok
