	...
	;gloom2_c86zcg: TWO PLAYER COMBAT restart fix: restore gloom.s window-template reset before each level, keep fresh Combat wait and other-player reflection; P96 disabled.
	;
...
g2c57_not_player_owner
	...
	bra	g2c57_try_blob
	;
g2c57_setup_reflection
	bsr	g2_prepare_reflection_column
	bra	g2c57_blob_done
	;
g2c57_player_try_blob
	; c86zcg: the visible other player in TWO PLAYER should use the same
	; floor-anchored reflection preparation as living enemies. Keep the
	; current-camera guard above intact.
	tst	g2_reflections
	ble.s	g2zcg_player_blob_only
	tst	ob_hitpoints(a2)
	ble.s	g2zcg_player_blob_only
	cmp	#3<<grdshft,d2
	bgt.s	g2zcg_player_blob_only
	bsr	g2_prepare_enemy_reflection_safe
g2zcg_player_blob_only
	tst	g2_blobshadow
	ble	g2c57_blob_done
...
.rx_max_ok
	...
	bne	g2c57_col_ok
	moveq	#1,d0
g2c57_col_ok	move	d0,g2_shadow_col
	move	#1,g2_shadow_active
g2c57_blob_done	movem.l	(a7)+,d0-d7/a0-a2
	rts
...
; c86zcg: Combat selection/intermission pictures must not be consumed by
; the Fire/Return press that selected the menu item or ended the previous
; round. First wait for release, then require a fresh press and release.
g2waitfreshany	movem.l	d0-d7/a0-a6,-(a7)
g2zcg_wait_release	bsr	checkany
	bne.s	g2zcg_wait_release
g2zcg_wait_press	bsr	checkany
	beq.s	g2zcg_wait_press
g2zcg_wait_release2	bsr	checkany
	bne.s	g2zcg_wait_release2
	movem.l	(a7)+,d0-d7/a0-a6
	rts
...
; c86zcg: restore the gloom.s per-level window template reset.
; Even though this gloom2 fullscreen path does not call makewindow/showwindow,
; message/status helpers still dereference window1/window2 fields. After a
; previous ONE/TWO PLAYER run those structs may be left in the wrong shape for
; TWO PLAYER COMBAT, so copy the proper defaults before assigning ob_window.
g2zcg_getwindow	tst	twowins
	bne.s	g2zcg_getwindow_p2
	lea	defwindow1_1p,a0
	lea	window1,a1
	bsr	copywin
	rts
g2zcg_getwindow_p2	lea	defwindow1_2p,a0
	lea	window1,a1
	bsr	copywin
...
.loop	move.b	(a0)+,(a1)+
	...
	move.l	combatpal,a1
	jsr	showpic
	bsr	g2waitfreshany	;c86zcg: require fresh press before Combat map load
...
.shit	;
	;init windows...
	;
	; c86zcg: match gloom.s here: reset window1/window2 from the
	; one-player/two-player templates, then attach fresh window pointers to the
	; player objects before any message/status/render helper can use them.
	bsr	g2zcg_getwindow
	move.l	player1(pc),a5
	move.l	#window1,ob_window(a5)
	tst	gametype
	beq.s	g2zcg_window_done
	move.l	player2(pc),a5
