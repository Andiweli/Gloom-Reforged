	...
	;gloom2_c86zcl: from clean c86zc7; add other-player mirror reflections in TWO PLAYER modes and keep immediate-near enemy/player reflections from fading out.
	;
...
.g2c57_player_try_blob
	; c86zcl: in TWO PLAYER modes the other player should get the same
	; safe mirrored floor reflection as grounded enemies.  Keep the current
	; camera player guarded out above; this only affects the visible opponent.
	tst	g2_reflections
...
.rh_mid
	...
	bra.s	.rh_cap_apply
.rh_near
	; c86zcl: immediate-near enemies/players should not visibly shrink/fade
	; away.  Allow a fuller near reflection than c86zc7, but keep a sane cap.
	moveq	#40,d1		; close: fuller near reflection, still bounded
.rh_cap_apply
	cmp	d1,d0
	ble.s	.rh_max_ok
	move	d1,d0
...
.yloop
	...
	; c86zcl: do not let immediate-near enemy/player mirror reflections
	; dissolve away.  Beyond one map width keep the confirmed c86zc7 fade.
	move	g2_enemy_ref_z(pc),d0
	cmp	#(1<<grdshft),d0
	bhs.s	.dist_dither
	moveq	#16,d4		; very near feet/body: solid, no Bayer dropout
	cmp	#12,d5
	blt.s	.dither_pick
	moveq	#12,d4		; near lower body: still strong
	cmp	#24,d5
	blt.s	.dither_pick
	moveq	#8,d4		; near tail: visible, not vanished
	bra.s	.dither_pick
.dist_dither
...
readmenujoy	;encode to d0!
	...
	rts
.doit	lea	joyx,a0
	jsr	readjoy1	;c86zcl build-safe absolute call after added reflection code
	lea	joyx0,a0
	bsr	readkeys
	move.l	joyx0(pc),d0
	or.l	d0,joyx
