	...
	;gloom2p96_c86zc1: experimental full weak reflections; c86zb3 enemy coverblock kept, enemy/wall reflections drawn more complete but about 80 percent weakened by sparse dither.
	;
...
.col_loop
	...
	; desired reflection depth: more complete than the old probe, but capped
	; for safety and then clipped to the visible floor area.
	move	vd_h(a0),d6
	subq	#1,d6		; DBF count for full-ish mirrored height
	cmp	#47,d6
	ble.s	.depth_cap_ok
	moveq	#47,d6		; hard safety cap: max 48 rows
.depth_cap_ok
	move	d2,d5
...
.rx_max_ok
	...
	; c86zc1: more complete enemy reflection silhouette.  Use most of the
	; visible sprite height instead of the older half-height mirror, then make
	; it very weak through sparse dither in the draw path.
	move	d4,d0
	cmp	#8,d0
	bge.s	.rh_min_ok
	moveq	#8,d0
