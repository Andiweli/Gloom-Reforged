...
g2drawchunky_a0	;a0=shapetable, d0=x,d1=y,d2=shape#
	add	#224,d1
	;
	; v43: panel/smallfont2 shapes have their own 12-bit palette at an_pal.
	; The old code treated the already-remapped panel bytes as level palette
	; indices via palettes(pc), which made the green statusbar background go
	; black in the chunky/C2P path. Keep the panel data unremapped and map
	; its own RGB palette directly through planar_remap/paladjust here.
	movem.l	d2-d7/a2-a6,-(a7)
	;
	move.l	a0,a5	;shape table base
	move.l	chunky(pc),a1
	mulu	#320,d1
	add.l	d1,a1	;left
	lea	coloffs,a2
	...
	;
	move.l	a5,a4
	add.l	an_pal(a4),a4	;palette block
	move	(a4)+,d7	;number of colours in source palette
	move.l	planar_remap,a6
	lea	paladjust,a5
	;
...
.vloop	move.b	(a0)+,d0
	...
	cmp	d7,d6
	bhi.s	.raw
	move	0(a4,d6.w*2),d6	;source RGB12
	tst.l	a6
	beq.s	.raw
	moveq	#0,d0
	move.b	0(a6,d6.w),d0	;RGB12 -> chunky LUT
	move.b	0(a5,d0.w),(a3)	;C2P colour adjust
	bra.s	.skip
.raw	move.b	d0,(a3)
.skip	add.l	d5,a3
	dbf	d4,.vloop
...
g2drawgun
	movem.l	d0-d4/a0-a2/a5,-(a7)
	move.l	gunpic(pc),d0
	beq.w	.g2dg_done
	move.l	d0,a0
	move.l	12(a0),d0	;shape #0 offset in same table format as panel
	beq.w	.g2dg_done
	cmp.l	#$20000,d0
	bcc.w	.g2dg_done
	move.l	a0,a2
	add.l	d0,a2
	move	4(a2),d3	;width after handles
	move	6(a2),d4	;height after handles
	tst	d3
	ble.w	.g2dg_done
	cmp	#320,d3
	bhi.w	.g2dg_done
	...
	lsr	#1,d0		;x centered
	move	d4,d1
	neg	d1		;bottom sits just above the status strip
...
showstats	;
	;a5=player
	;
	;v45: keep the textured statusbar base (#47) visible.
	;Shape #46 is a middle clear/cover shape and made the lower bar look
	;black in the chunky/C2P path, so do not draw it here.
	;
	;hitpoints
...
g2_after_inline_c2p_init
	...
	;
	; v43: do not remap smallfont2/panel in-place.  drawchunky now maps
	; panel colours from smallfont2's own RGB palette at draw time.
	;move.l	panel,a0
	;jsr	remapanim
	lea	bullet1,a0
	jsr	remapanim
