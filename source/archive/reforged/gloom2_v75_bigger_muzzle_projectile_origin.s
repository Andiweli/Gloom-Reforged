...
.g2dm_have_weapon
	...
	move	6(a0),d3		;height
	move	#219,d4
	sub	d3,d4		;visible flash behind/above the gun muzzle
	; v75: make every selected upgrade flash visibly larger.  This keeps
	; the per-upgrade gun.bin shape selection from v73, but draws a small
	; expanded cross around it so even the 100% frames are bigger.
	bsr	g2drawgun_coloffs_shape
	subq	#2,d1
	bsr	g2drawgun_coloffs_shape
	addq	#4,d1
	...
	bsr	g2drawgun_coloffs_shape
	subq	#2,d4
	; v75: the short 120% flicker frames get a wider halo, including
	; diagonals, so the original two-step bigger flash is more obvious.
	move	g2gun_firetimer(pc),d5
	btst	#0,d5
	bne.w	.g2dm_done		;100%,120%,100%,120% over the 4 fire frames
	subq	#5,d1
	bsr	g2drawgun_coloffs_shape
	addq	#5,d1
	addq	#5,d1
...
.g2amo_do
	...
	lea	0(a1,d0*8),a1
	move	#40,d0		;v75: farther forward so first projectile appears at the muzzle
	move	d0,d2
	muls	2(a1),d0
	add.l	d0,d0
	neg.l	d0
	...
	add.l	d2,d2
	add.l	d2,ob_z(a0)
	; v75: keep the projectile vertically at the muzzle, while the larger
	; forward offset above prevents the first frame from being projected down
	; near the statusbar.
	sub	#24,ob_y(a0)
.g2amo_done
	movem.l	(a7)+,d0-d2/a1
