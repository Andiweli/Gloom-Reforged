	...
	;gloom2p96_c86x: enemy reflections stronger early fade + wall reflection lite.
	;
...
; c86x: very simple wall reflection version A.
; Called after wall/floor/ceiling rendering and before objects.  It mirrors
; only the lower wall colour samples into the floor with sparse ordered dither.
; This gives a subtle glossy-floor hint without changing castwalls/flat maths.
g2_draw_wall_reflection_lite
	...
	beq.w	.done
	move	maxy(pc),d0
	add	midy(pc),d0	; absolute floor start row
	cmp	#2,d0
	blt.w	.done
	move	hite(pc),d1
	sub	#14,d1
	...
	move	#11,d6		; 12-pixel maximum reflection depth
.yloop
	move	d6,d5
	neg	d5
	add	#11,d5		; y = 0..11 from floor downward
	; fade thresholds: very light and quickly vanishing
	moveq	#3,d4		; top: 3/16
	cmp	#3,d5
	blt.s	.have_thr
	moveq	#2,d4		; middle: 2/16
	cmp	#7,d5
	blt.s	.have_thr
	moveq	#1,d4		; tail: 1/16
.have_thr
	move	d0,d2
	subq	#1,d2
	sub	d5,d2		; source row above floor
	blt.s	.next_y
	move	d0,d3
	add	d5,d3		; destination row below floor
	move	hite(pc),d1
	sub	#10,d1
	cmp	d1,d3
	bgt.s	.next_y
	...
	ble.s	.next_y
	subq	#1,d2
	clr	d5		; x counter for dither only
.xloop
	move.l	(a1)+,d1
	move.l	a3,a5
	add.l	d1,a5
	...
	move.b	(a5),d3
	beq.s	.xnext
	; Do not mirror HUD/void black; only real non-zero wall-ish pixels.
	move	d5,d1
	and	#3,d1
	move	d6,d3
	and	#3,d3
...
.g2v190dm_floor_fog
	...
	jsr	g2fill_disabled_flat_fog
.nofloor	;
	; c86x: simple wall-reflection lite.  This is intentionally stylised, not
	; a second physical wall renderer: after flats are drawn, mirror a few lower
	; wall pixels sparsely into the floor area.  REFLECTIONS NO disables it.
	tst	g2_reflections
	ble.s	.g2c86x_no_wall_refl
	move	floorflag(pc),d0
	ble.s	.g2c86x_no_wall_refl
...
; c86t: safe mirrored enemy floor reflection, lighter with air-gap floor start.
...
.rh_max_ok
	...
	cmp	d1,d0
	bgt.s	.no_ref
	; c86x: do not add an artificial rectangular air-gap here.  The mirror
	; starts at the projected floor row, and the draw routine now masks each
	; reflected row against the real source sprite pixel.  That means flying
	; enemies naturally keep the correct visual gap and uneven transparent
	; undersides do not turn into reflected background/floor pixels.
	move	hite(pc),d1
	sub	#10,d1
	cmp	d1,d0
	bgt.s	.no_ref
...
g2_draw_enemy_mirror_reflection_safe
	...
	ble	.rts
	; c86x: use the real source sprite as an alpha mask.  Earlier versions
	; sampled the already-rendered chunky column only; transparent bottom pixels
	; then reflected floor/wall background and made flying/uneven enemies look
	; wrong.  a3/d1/d6 are still the source base/index/ystep from drawobj*.
	move.l	a3,a6		; source sprite column base
	move.l	d1,g2_enemy_ref_srcidx
	move.l	d6,g2_enemy_ref_srcstep
...
.abs_ok
	...
	move.l	chunkymod(pc),d7
	move.l	a4,a3
	sub.l	d7,a3	; first source colour sample: bottom visible sprite pixel
	move.l	chunky(pc),d0
...
.yloop
	cmp.l	a1,a0
	bhi	.rts
	; First walk one source row upwards and test the original sprite pixel.
	; If this row/column is transparent, leave the reflection transparent too.
	move.l	g2_enemy_ref_srcidx(pc),d2
	sub.l	g2_enemy_ref_srcstep(pc),d2
	move.l	d2,g2_enemy_ref_srcidx
	tst.l	d2
	bmi.s	.next
	moveq	#0,d0
	move.b	0(a6,d2),d0
	beq.s	.next
	; c86x: earlier and stronger fade.  Top is already light, lower rows
	; become sparse very quickly so the mirror reads like transparent dither,
	; not a solid copied sprite.
	moveq	#8,d4		; top: 8/16 pixels kept
	cmp	#2,d5
	blt.s	.dither_pick
	moveq	#5,d4		; early mid: 5/16
	cmp	#5,d5
	blt.s	.dither_pick
	moveq	#3,d4		; mid/lower: 3/16
	cmp	#9,d5
	blt.s	.dither_pick
	moveq	#1,d4		; tail: 1/16
	cmp	#14,d5
	blt.s	.dither_pick
	bra.s	.next		; hard transparent tail
.dither_pick
	move	g2_enemy_ref_curx(pc),d0
	and	#3,d0
	move	d5,d1
...
; c86w/c86x 4x4 ordered threshold table for enemy-reflection transparency.
g2_enemy_ref_bayer4
...
p96prevlog_name	dc.b	'RAM:gloom_p96_preview.txt',0
	even
p96prevlog_ok	dc.b	'P96 preview: c86x P96 off enemy plus wall reflection lite active',10
