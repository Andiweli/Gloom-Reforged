...
dointro	;
	...
	bsr	dispon
	jsr	g2v145_draw_menu_gloombrush
	jsr	g2v159_draw_title_credit_boot
	;
	bsr	chaton
	;
...
.titlemenu_y	move	#-1,g2v158_titlemenu_credit
	sub	fonth(pc),d6
	sub	fonth(pc),d6
	bra.s	.titlemenu_y_done
.titlemenu_main	move	#-1,g2v154_titlemenu_lines
	move	#-1,g2v158_titlemenu_credit
	sub	fonth(pc),d6
	sub	fonth(pc),d6
.titlemenu_y_done
	move	d6,menuy
...
; v159: draw two thin 80px medium-yellow dotted separators into the
; blank title-menu rows, 1px higher than v154: one between TWO PLAYER
; COMBAT and PLAYER 1, one between PLAYER 2 and REMOTE LINK OPTIONS.
; Use colour index 2 (plane 1 only), which is a little brighter than the
; darker v155 tone but not as bright as the main yellow highlight.
...
.plane	move.l	a1,a2
	...
	moveq	#0,d0
	bra.s	.fill
.mediumyellowplane	moveq	#-86,d0		; $AA dotted pattern
.fill	moveq	#9,d1
.byte	move.b	d0,(a2)+
	dbf	d1,.byte
	addq	#1,d2
...
.g2v158_len	addq	#1,d0
	...
	move	bmaphite(pc),d6
	sub	fonth(pc),d6
	addq	#1,d6		; v159: move footer one scanline lower
	jsr	printmess2
	movem.l	(a7)+,d0-d1/a4
	rts

; v159: initial title-screen footer draw before the menu opens.
g2v159_draw_title_credit_boot
	bsr	g2v158_draw_title_credit
	bsr	db
	bra	vwait
