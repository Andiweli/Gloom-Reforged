...
debugser	equ	0
debugmem	equ	0
g2p96_overlay_enabled	equ	0	; c86c: 0 hides/disables P96 overlay, -1 re-enables old c85 P96 preview path
	;
...
; overwrote the entire frame.  c86c keeps the physical split halves at 320x120,
; but uses the widest non-squashed 4:3 view that fits inside each half:
; 160x120 pixels centred in the 320-wide split area.  This keeps the
; corrected vertical proportions from c86b, but shows more field than the
; strict gloom.s-style 132x120 framing.  c55 additionally draws the
...
g2twop_drawall_split
	...
	jsr	g2twop_center_coloffs	;c86c: render widest 160px 4:3 view centered in each 320px split half
...
g2twop_set_half_view
	; c86c: physical split half remains 320x120, but the visible 3D
	; projection is the widest non-squashed 4:3 area that fits: 160x120.
	move	#160,width
	move	#120,hite
...
; c86c: temporarily map logical render columns 0..159 to physical screen
; columns 80..239.  This is the widest 4:3 area possible inside a 320x120
; split half.  The remaining side areas stay black from g2clearfullchunky.
