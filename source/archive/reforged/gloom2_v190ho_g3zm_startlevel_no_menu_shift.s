...
.redrawmenu
	...
	cmp	#2,g2_game_profile	; v190ho: ZM gets START LEVEL without moving old rows
	beq.s	.use_compat_startlevel_menu
	cmp	#3,g2_game_profile	; v190ho: Gloom3 gets START LEVEL without moving old rows
	beq.s	.use_compat_startlevel_menu
	lea	compat_startmenu,a4	; Gloom Original: classic menu without START LEVEL
	jsr	initmenu
	clr	curropt
	bra.s	.sel
.use_compat_startlevel_menu
	lea	compat_startmenu_sl,a4
	jsr	initmenu
	move	#2,curropt	; keep ONE PLAYER GAME as default/blinking row
	bra.s	.sel
...
.g2v190f_shifted_newgame
	...
	bra	.newgame
.g2compat_notlinked
	cmp	#13,numopts	; v190ho: Gloom3/ZM START LEVEL overlay table uses full row indices
	beq	.g2compat_sl_notlinked
	tst	d7
	beq.s	.g2compat_fire
...
.g2compat_fire
	...
	beq	.exitgloom
	bra	.sel
.g2compat_sl_notlinked
	tst	d7
	beq.s	.g2compat_sl_fire
	cmp	#0,d0
	beq	.g2v190f_level_lrsel
	cmp	#6,d0		; shifted visual table, PLAYER 1 still at old screen Y
	beq	.g2compat_p1lrsel
	cmp	#7,d0		; shifted visual table, PLAYER 2 still at old screen Y
	beq	.g2compat_p2lrsel
	bra	.sel
.g2compat_sl_fire
	cmp	#0,d0
...
.g2compat_p1sel	bsr	g2v190ho_get_p1ctypec_ptr
	lea	p1_ob_cntrl,a2
	lea	p2_ob_cntrl,a3
	bsr	inccntrl
	bra	.sel
.g2compat_p1prevsel	bsr	g2v190ho_get_p1ctypec_ptr
	lea	p1_ob_cntrl,a2
	lea	p2_ob_cntrl,a3
	bsr	deccntrl
	bra	.sel
...
.g2compat_p2sel	bsr	g2v190ho_get_p2ctypec_ptr
	lea	p2_ob_cntrl,a2
	lea	p1_ob_cntrl,a3
	bsr	inccntrl
	bra	.sel
.g2compat_p2prevsel	bsr	g2v190ho_get_p2ctypec_ptr
	lea	p2_ob_cntrl,a2
	lea	p1_ob_cntrl,a3
	bsr	deccntrl
	bra	.sel

g2v190ho_get_p1ctypec_ptr
	cmp	#13,numopts
	bne.s	.old
	lea	p1ctypecs(pc),a1
	rts
.old	lea	p1ctypec(pc),a1
	rts

g2v190ho_get_p2ctypec_ptr
	cmp	#13,numopts
	bne.s	.old
	lea	p2ctypecs(pc),a1
	rts
.old	lea	p2ctypec(pc),a1
	rts
.g2v190f_level_lrsel
...
.vilesel	addq	#1,mode
	...
	cmp	#13,numopts	; v190ho: Gloom3/ZM START LEVEL menu has its own mode row
	bne.s	.g2v190co_mode_compat
	lea	modetxtcs,a1
...
compat_startmenu_sl	dc.b	13	; v190ho: Gloom3/ZM only, START LEVEL drawn above unchanged classic rows
	dc.b	'START LEVEL ['
g2v190f_level_textc	dc.b	'MAP1.1]',0	; fixed 6-char display plus bracket
	dc.b	0			;1 spacer below START LEVEL, keeps one blank row before ONE PLAYER
	dc.b	'ONE PLAYER GAME',0	;2 - same screen position as compat row 0
	dc.b	'TWO PLAYER GAME',0	;3
	dc.b	'TWO PLAYER COMBAT',0	;4
	dc.b	0			;5 original dotted separator position
	dc.b	'PLAYER 1 '
p1ctypecs
	ifne	cd32
	dc.b	'CD32 PAD 1',0	;6, fixed 10-char field
...
p2ctypecs
	...
	endc
	;
	dc.b	0			;8 original dotted separator position
	dc.b	'REMOTE LINK OPTIONS',0	;9
modetxtcs	dc.b	'MEATY VIOLENCE MODE',0	;10
	dc.b	'ABOUT GLOOM',0		;11
	dc.b	'EXIT GLOOM',0		;12
	even
...
.g2v190i_copy_done
	clr.b	(a1)
	move.l	a2,a0
	lea	g2v190f_level_textc(pc),a1
	moveq	#5,d1		; v190ho: bracketed Gloom3/ZM title row
.g2v190ho_copyc
	move.b	(a0)+,d2
	beq.s	.g2v190ho_copyc_done
	move.b	d2,(a1)+
	dbf	d1,.g2v190ho_copyc
.g2v190ho_copyc_done
	move.b	#']',(a1)+
	clr.b	(a1)
...
g2v190i_levelselect_loadscripts
	...
	clr	g2v190f_level_index
	bsr	permit
	cmp	#2,g2_game_profile	; v190ho: Zombie Massacre level order comes from stuf/stages
	beq.s	.g2v190ho_profile_zm
	cmp	#3,g2_game_profile	; v190ho: Gloom3 level order comes from misc/script
	beq.s	.g2v190ho_profile_g3
.g2v190ho_common_scripts
	...
	tst	g2v190i_level_count
	bne	.doneio
	bra	.g2v190ho_gd_scripts
.g2v190ho_profile_g3
	lea	g2v190i_script_misc(pc),a0
	bsr	g2v190i_try_script
	tst	g2v190i_level_count
	bne	.doneio
	bra	.g2v190ho_common_scripts
.g2v190ho_profile_zm
	lea	g2v190i_script_stages(pc),a0
	bsr	g2v190i_try_script
	tst	g2v190i_level_count
...
initmenu2	;
	...
	cmp.l	#compat_startmenu_sl+1,a4
	beq.s	.titlemenu_compat_sl
...
.titlemenu_main	move	#-1,g2v154_titlemenu_lines
	...
	bra.s	.titlemenu_y_done
.titlemenu_compat_sl	move	#-1,g2v154_titlemenu_lines	; v190ho: use row 5/8 lines, old rows stay in place
	sub	fonth(pc),d6
	sub	fonth(pc),d6
	sub	fonth(pc),d6	; compensate START LEVEL+blank so ONE PLAYER keeps old compat Y
	bra.s	.titlemenu_y_done
