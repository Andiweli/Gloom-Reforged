...
darklevels	equ	32	;v85: finer depth shading to reduce visible banding
darkmax	equ	darklevels-1
...
vd_pal	rs.w	1	;palette# (0...31)
vd_dith	rs.w	1	;v88: 0..7 column dither amount towards next darker shade
vd_y	rs.w	1
vd_h	rs.w	1
vd_data	rs.l	1
vd_ystep	rs.l	1
...
showstats	;
	...
	ble.s	.nohp
	move	#268,d6	;v53: healthbar 1px further left, final slot alignment
	moveq	#0,d3
	subq	#1,d7
...
flatyadd	dc	0
flatyadd2	dc	0
flatphase	dc	0	;v89: per-scanline dither threshold for floor/roof crossfade
flatdith	dc	0	;v89: 0..7 floor/roof dither amount toward next darker shade
flatpal2	dc.l	0	;v89: next darker flat palette for crossfade
render_dithx	dc	0	;v90: current wall-column dither x phase 0..7
render_diththr	dc	0	;v90: current wall-column dither threshold 0..7

dithpat8	dc.b	0,4,1,5,2,6,3,7	;dispersed 8-step dither pattern
even
...
.vloop	;
	...
	and	#63,d4
	lsr	#3,d4
	move	d4,flatdith		;v89 0..7 crossfade amount for this floor/roof depth
	move	d7,d4
	and	#7,d4
	lea	dithpat8(pc),a6
	moveq	#0,d3
	move.b	0(a6,d4.w),d3
	move	d3,flatphase
	move.l	palette(pc),a6
	move.l	0(a6,d5*4),a5
	clr.l	flatpal2
	cmp	#darkmax,d5
	bge.s	.palset
	move.l	4(a6,d5*4),flatpal2
.palset	move	flatdith(pc),d4
	cmp	flatphase(pc),d4
	ble.s	.palchoose_done
	tst.l	flatpal2
	beq.s	.palchoose_done
	move.l	flatpal2(pc),a5	;use next darker palette on selected scanlines
.palchoose_done
	;
	;Find leftmost X...
	;
...
.solid	move.l	a3,vd_data(a0)	;start column
	;
	;fill in vd struct...
	; v88: compute a 32-step base shade plus an extra 0..7 dither amount.
	; renderwalls then crossfades adjacent depth shades across columns, which
	; gives a ZGloom-like softer transition instead of hard depth bands.
	move	d2,d3
	lsr	#6,d3
	cmp	#darkmax,d3
	bls.s	.palok
...
.palok	move	d2,d6
	and	#63,d6
	lsr	#3,d6		;0..7 dither amount towards next darker shade
	move	d2,vd_z(a0)
	move	d3,vd_pal(a0)
	move	d6,vd_dith(a0)
	;
...
.hiteok	;
	...
	swap	d3
	subq	#1,d4
	move	vd_pal(a0),d5	;0...31
	move.l	0(a2,d5*4),a4	;base palette
	move.l	#0,a6		;v90a GenAm/68000 buildfix: CLR is not valid for address registers
	move	render_diththr(pc),d6
	beq.s	.nodithpal
	cmp	#darkmax,d5
	bge.s	.nodithpal
	move.l	4(a2,d5*4),a6	;next darker palette
.nodithpal	lea	dithpat8(pc),a5
	sub	d3,d2
	add.l	d3,d2
	;
.vertloop	move.b	0(a3,d2),d5
	...
	tst.l	a6
	beq.s	.baseplot
	move	render_dithx(pc),d6
	add	d4,d6		;mix x and y so shading becomes grainy, not vertical ridges
	and	#7,d6
	moveq	#0,d7
	move.b	0(a5,d6.w),d7
	move	render_diththr(pc),d6
	...
	move.b	0(a6,d5),(a1)
	bra.s	.plotted
.baseplot	move.b	0(a4,d5),(a1)
.plotted
.skipplot	addx.l	d3,d2
	add.l	d0,a1
	dbf	d4,.vertloop
.vertskip	;
	subq	#1,d1
...
renderwalls	;
	...
	move	width(pc),d7
	subq	#1,d7
	moveq	#0,d6		;v90 0..7 x phase, but the actual palette crossfade now happens
				;per pixel row inside drawsolidstrip to avoid dark wall ridges
	;
.loop	move.l	(a6)+,a1
	add.l	chunky(pc),a1
	move	d6,render_dithx
	move	vd_dith(a0),render_diththr
	drawsolidstrip
	;
	lea	vd_size(a0),a0
	addq	#1,d6
...
palettes	ds.l	darklevels	;v85: 32 shade palettes 
			;each 256 bytes long.
map_map	dc.l	0
map_grid	dc.l	0
map_poly	dc.l	0
...
.osskipz	;
	...
	;
	move.l	#256*darklevels,d0
	moveq	#1,d1
	allocmem	palettes
	;
	;v85: 32 shade palettes for smoother depth falloff.
	lea	palettes(pc),a0
	moveq	#darkmax,d1
.p_loop	move.l	d0,(a0)+
	add.l	#256,d0
...
initdarktable	;
	...
	;
.loop	move	#maxz-1,d3
	sub	d2,d3		;v89: actual distance 0..maxz-1 for this table entry
	lsr	#6,d3		;base 0..31 depth shade index across the visible range
	move	d3,d4
	lsr	#1,d4		;base/2
	move	d4,d5
	lsr	#2,d5		;base/8
	sub	d4,d3
	add	d5,d3		;~= 5/8 of the old darkness -> about 35-40% more visibility
	cmp	#darkmax,d3
	bls.s	.ok
	move	#darkmax,d3
.ok	move	d3,(a1)+
...
calcpalettes	;v87 generate 32 stronger far-depth palettes
	;
	lea	paladjust,a5
	move.l	map_rgbsat(pc),a4	;end
	move.l	planar_remap(pc),a1	;remap table
	lea	palettes(pc),a2
	moveq	#0,d7		;v85 half-step darkness 0..31
	;
.loop	move.l	(a2)+,a3
...
.loop2	move	(a0)+,d0	;RGB value 0,1,2...
	...
	lsr	#8,d0
	and	#$00f,d0	;R
	add	d0,d0		;v86: keep the smoother half-step dimming, but let far
				;distances reach full black instead of leaving a red remainder
	sub	d7,d0
	bpl.s	.rok
	moveq	#0,d0
.rok	lsr	#1,d0
