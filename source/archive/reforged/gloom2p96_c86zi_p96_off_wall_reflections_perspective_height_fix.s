...
.depth_from_screen_ok
	; c86zi: reflection height should feel more like enemy reflections:
	; far walls get a shorter mirrored tail, near walls may keep a taller one.
	; This also helps the previous inversion where distant wall reflections
	; looked higher than near ones.
	moveq	#11,d3
	move	vd_z(a0),d0
	tst	g2_visibility
	bgt.s	.depth_adv
	cmp	#(5<<grdshft),d0
	blo.s	.depth_cap_done
	cmp	#(8<<grdshft),d0
	blo.s	.depth_mid
	moveq	#3,d3
	bra.s	.depth_cap_done
.depth_mid
	moveq	#6,d3
	bra.s	.depth_cap_done
.depth_adv
