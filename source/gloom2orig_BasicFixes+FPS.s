	;orig-runtimefix3c: restore CIA-A/raw keyboard vectors on clean exit.
	;                   Prevents dead CLI keyboard after leaving the game.
	;orig-runtimefix3b: exact gloom2 c87a7 presented-FPS logic,
	;                   both 5x7 digits fixed, transparent white planar
	;                   overlay after HUD; no black box or background bar.
	;orig-runtimefix3a: GenAm 3.18 short-branch range fix only.
	;                   Three CLI FPS scanner branches changed .s -> .w.
	;orig-runtimefix3: CLI FPS token + fixed top-centre FPS overlay,
	;                  proven gloom2 muzzle/projectile path and 320x240 default.
	;orig-runtimefix2: bare FPS Workbench ToolType with bottom-right counter,
	;                  plus centred bobbing first-person gun and muzzleflash.
	;                  C2P converters remain external in PROGDIR:c2p/.
	;orig-runtimefix1: intermission wrap, ESC-menu release debounce,
	;                  hidden game pointer, KEYBMOUSE held-fire,
	;                  fixed 320px top HUD and PLAYER1 KEYBMOUSE default.
	;orig-startfix2a: Title input activation fix. The working Reforged path
	;                  explicitly calls inputon before waitany and accepts
	;                  RETURN, keypad ENTER, SPACE, ESC and left mouse.
	;orig-startfix2: Complete post-BlackMagic startup safety recovery.
	;                 C2P remains EXTERNAL: configured c2p file first,
	;                 c2p/blackmagic_1 fallback second. Only the small
	;                 coloffs/paladjust table builder is internal.
	;orig-startfix1a: GenAm 3.18 range buildfix only. Five references
	;                 in the appended start-screen helpers now use
	;                 absolute relocatable addressing/jump.
	;orig-startfix1: Port proven BlackMagic/title fixes from Gloom Reforged.
	;          ECS uses a matching six-plane BlackMagic image/palette,
	;          decodeiff cannot overrun a six-plane target, and missing
	;          static assets fall back safely. Title brush loads from
	;          pics/gloom and uses the correct per-plane line offset.
	; Gloom original-source recovery + minimal usability patch.
	; Build-fixed original renderer with top HUD and selectable KEYBMOUSE only.
...
testw	equ	320	;orig-runtimefix3: FULLSCREEN default
testh	equ	240	;orig-runtimefix3: FULLSCREEN default
...
entrypoint	;
	...
	bra	wb
cli	;
	move.l	a0,g2orig_cli_ptr	;orig-runtimefix3: preserve CLI tokens
	cmp.b	#'@',(a0)+
	bne.s	wb
	lea	tempfile,a1
	move.l	a1,map_test
...
wb	;
	...
	jsr	-126(a6)
.nocd	;
	; orig-runtimefix2: read the bare FPS Workbench ToolType before initmain.
	jsr	g2orig_fps_tooltype_probe
	move.l	4.w,a6
	moveq	#1,d1
	jsr	-216(a6)
...
.intro	move.l	medat(pc),a1
	move.l	titlemed(pc),d0
	beq.s	.g2orig_no_title_music
	move.l	d0,a0
	jsr	8(a1)	;start title music!
.g2orig_no_title_music
	;
...
.n22	bsr	bigfont
	bra	.intro
	;
exittoos	jsr	inputoff	;runtimefix3c: restore CIA-A/rawkey vectors before OS exit
	jsr	freeobjlist2
	jsr	permit
	jsr	finitdisplay
	jsr	finitvbint
...
dogamemenu	;
	...
	lea	gamemenu,a4
	jsr	initmenu
	; orig-runtimefix1: ESC is still held when the menu first appears.
	; Wait for release so CONTINUE/exit is not selected immediately.
	jsr	g2orig_wait_menu_release
	;
.loop	jsr	selmenu
	beq	.done
	;
...
largewin	move	#320,width
	move	#240,hite	;orig-runtimefix3: true fullscreen
	bra.s	sizedone
...
calcoffset	move	#320,d0
	...
	ext.l	d0
	;
	move	#240,d1	;orig-runtimefix3: centre inside 240-row display
	sub	hite(pc),d1
	lsr	#1,d1
	mulu	linemodw(pc),d1
	add.l	d1,d0
...
predrawall	;draw up everything....
	;
	; Top HUD is drawn over the 3D image; the old lower panel is disabled.
	;
	bsr	clspic
	bsr	clspic
	bsr	calcoffset
...
doc2p	;
	...
	move.l	c2p(pc),a2
	jsr	(a2)
	jsr	g2orig_hud_post_c2p
	jsr	g2orig_fps_post_c2p
	; FPS is overlaid at the fixed physical top centre.
.rts	rts
...
showstats	jmp	orig_top_hud_impl
...
blitscene	;
	...
	;
	; orig-runtimefix2: weapon/muzzle are world overlays, HUD stays in front.
	jsr	g2orig_drawgun
	; The top HUD shares the 3D chunky frame and must be redrawn every frame.
	bclr	#7,ob_update(a5)
	bsr	showstats
	clr	panelcnt
	rts
...
joytable_end
joytable2	dc.l	readjoy1,readjoy0,readkeys,readcd321,readcd320,readkeymouse
...
vbhandler	movem.l	d2-d7/a2-a6,-(a7)
	...
	addq	#1,(a1)
	;
	; KEYBMOUSE mouse yaw is applied directly once per VBlank.
...
shoot	;
	...
	jsr	g2orig_apply_player_muzzle_origin
	...
	jsr	g2orig_player_bullet_visual_prestep
	;add.l	d0,ob_x(a0)
	;add.l	d1,ob_z(a0)
	;
	move	#32,ob_rad(a0)
...
checkfireb	move	joyb(pc),d0
	beq.s	.nofire
	cmp	#5,ob_cntrl(a5)	;orig-runtimefix1: KEYBMOUSE held-fire
	beq.s	.g2orig_kmfire
	tst	ob_lastbut(a5)
	bne.s	.skip
...
.skip3	rts
	...
	;elseif	; buildfix: inactive alternate marker
...
.solid	move.l	a3,vd_data(a0)	;start column
	...
	movem	d3-d4,vd_y(a0)
	;
	;elseif	; buildfix: select this original wall-step variant
	;
	moveq	#64,d5
	swap	d5
	clr	d5
	...
	divu.l	d4,d5
	;
	;elseif	; buildfix: abandoned alternate disabled
	;
;	neg	d3
;	neg	d5
;	cmp	#-128,camy
...
defwindow1_1p	;
	...
	dc.l	copstop
	;
	;elseif	; buildfix: select first original 1P layout
	dc	160-126 ;45*2
	dc	cy-120 ;45*2
...
.loop5	move	d2,d5	;depth
	...
	;
.loop4	move.l	a1,a2
	; STARTFIX: consume all source planes, but discard planes that do not
	; exist in the active target. This prevents 8-plane source data from
	; overwriting memory behind a six-plane ECS bitmap.
	move	d2,d6
	sub	d5,d6
	cmp	bitplanes(pc),d6
	bcs.s	.g2orig_decode_dest_ok
	lea	g2orig_decode_sink(pc),a2
.g2orig_decode_dest_ok
...
.skip	subq	#1,d4
	bgt.s	.loop
	;
	; Advance only for source planes represented by the destination bitmap.
	move	d2,d6
	sub	d5,d6
	cmp	bitplanes(pc),d6
	bcc.s	.g2orig_decode_no_advance
	add.l	d7,a1
.g2orig_decode_no_advance
	dbf	d5,.loop4
	;
...
; One complete source row is at most 40 bytes for the normal 320px images.
; Keep extra space for malformed/test assets while safely consuming ByteRun1.
g2orig_decode_sink	ds.b	128
	even
...
showpic	;a0=trimmed IFF file, a1=palette
	...
	movem.l	(a7),a0-a1
	tst.l	a0
	beq.s	.g2orig_showpic_nopic
	move.l	drawbitmap(pc),a1
	bsr	decodeiff
.g2orig_showpic_nopic
	movem.l	(a7)+,a0-a1
	tst.l	a1
	beq.s	.g2orig_showpic_nopal
	jsr	g2orig_pokepal_picture
.g2orig_showpic_nopal
	bsr	db
	bra	vwait
...
.noreloc	move.l	medat(pc),a1
	...
	;
	move.l	titlemed(pc),d0
	beq.s	.g2orig_initmed_no_title
	move.l	d0,a0
	move.l	medat(pc),a1
	jsr	4(a1)
.g2orig_initmed_no_title
	;
...
agafiles	dc.l	gloom
	...
	even
	dc.l	gloombrush
	dc.b	'pics/gloom',0	;STARTFIX: real retail title brush path
	even
	dc.l	planar_palette
...
; STARTFIX: ECS must not decode the retail eight-plane BlackMagic image into
; a six-plane bitmap. Prefer the matching editor-generated Fast-EHB image.
...
.os1	;
	...
	move.l	d0,temppal
.osskipz	;
	; orig-startfix2: complete blank 16x16 pointer image in CHIP RAM.
	move.l	#128,d0
	move.l	#$10002,d1
	allocmem	chipzero
	move.l	d0,chipzero
...
.p_loop	move.l	d0,(a0)+
	...
	bsr	initdisplay
	;
	; STARTFIX: load a depth-matched BlackMagic bitmap.
	lea	magicfiles,a0
	tst	aga
	bne.s	.g2orig_magic_image_ready
	lea	ecsmagicimagefiles,a0
.g2orig_magic_image_ready
	bsr	loadfiles
	;
	; If the optional ECS image is absent, try the retail path only when its
	; trimmed-IFF header proves that it is a safe 320x240x6 image.
	tst	aga
	bne.s	.g2orig_magic_image_done
	tst.l	magic
	bne.s	.g2orig_magic_validate
	lea	magicfiles,a0
	bsr	loadfiles
.g2orig_magic_validate
	move.l	magic,d0
	beq.s	.g2orig_magic_image_done
	move.l	d0,a0
	cmp	#320,(a0)
	bne.s	.g2orig_magic_reject
	cmp	#240,2(a0)
	blo.s	.g2orig_magic_reject
	cmp	#6,4(a0)
	beq.s	.g2orig_magic_image_done
.g2orig_magic_reject
	clr.l	magic
.g2orig_magic_image_done
	;
	lea	agamagicfiles,a0
	tst	aga
	bne.s	.g2orig_magic_palette_ready
	lea	ecsmagicfiles,a0
.g2orig_magic_palette_ready
	bsr	loadfiles
	;
	move.l	magic,a0
	move.l	magicpal,a1
	; Missing/mismatched ECS assets produce a clean black frame rather than
	; stale colours or a NULL/overflow decode.
	tst	aga
	bne.s	.g2orig_magic_show
	tst.l	a0
	beq.s	.g2orig_magic_black
	tst.l	a1
	bne.s	.g2orig_magic_show
.g2orig_magic_black
	sub.l	a0,a0
	lea	g2orig_black_palette(pc),a1
.g2orig_magic_show
	jsr	showpic
	bsr	dispon
	;
	...
	lea	ecsfiles,a0
.lf	bsr	loadfiles
	; orig-runtimefix2: optional external first-person weapon artwork.
	jsr	g2orig_loadgunfallback
	;
	; orig-startfix2: use EXTERNAL C2P code only.
	; The helper tries the filename stored in gloomcfg and then the external
	; c2p/blackmagic_1 fallback. It validates the returned pointer before
	; selecting 36(a2)/40(a2). The crashing external inittables entry is not
	; called; only coloffs/paladjust are initialized locally.
	jsr	g2orig_setup_external_c2p
	tst	d0
	beq.s	.g2orig_c2p_ready
.g2orig_c2p_missing
	; A renderer cannot run without an external C2P. Stop visibly instead of
	; dereferencing address 0 or jumping into invalid memory.
	move	#$f00,$dff180
	bra.s	.g2orig_c2p_missing
.g2orig_c2p_ready
	;
	bsr	initmed
	bsr	initser
	...
	bsr	initdarktable
	;
	tst.l	remapped	; buildfix: memory BSET #15 is illegal
	bne	.noremap
	move.l	#-1,remapped
	;
	...
	;
	move.l	panel,d0
	beq.s	.g2orig_no_panel_remap
	move.l	d0,a0
	jsr	remapanim
.g2orig_no_panel_remap
	move.l	gunpic,d0
	beq.s	.g2orig_no_gun_remap
	move.l	d0,a0
	jsr	remapanim
.g2orig_no_gun_remap
	lea	bullet1,a0
...
.skip	;
	...
	bne.s	.skhit
	;
	tst	gloomdata	; buildfix: memory BSET #15 is illegal
	bne.s	.gotdata
	move	#-1,gloomdata
.skhit	;
	ifne	cd32
...
scripttext	;print text on iff
	...
	lea	text,a1
	bsr	fetchrest
	; orig-runtimefix1: create a two-line word wrap before either line reaches
	; the screen edge. Existing script '\' separators remain authoritative.
	jsr	g2orig_wrap_script_text
	;
	lea	text,a4
	move	bmaphite(pc),d6
	sub	#11,d6
...
checkany	movem.l	d0-d7/a0-a6,-(a7)
	jsr	vwait
	jsr	readmenusel	;joystick/fire and normal RETURN path
	and	#$10,d0
	bne.s	.g2orig_any_pressed
	; orig-startfix2a: same title/static-screen keys as the proven gloom2 path.
	; Rawkey codes: $44 RETURN, $43 keypad ENTER, $40 SPACE, $45 ESC.
	qkey	$44
	bne.s	.g2orig_any_set
	qkey	$43
	bne.s	.g2orig_any_set
	qkey	$40
	bne.s	.g2orig_any_set
	...
	beq.s	.g2orig_any_set
	moveq	#0,d0
	bra.s	.g2orig_any_pressed
.g2orig_any_set
	moveq	#$10,d0
.g2orig_any_pressed
	tst	d0		;preserve EQ/NE after register restoration
...
.pok	move	(a2),d0
	move	(a3),d1
	cmp	d1,d0
	beq.s	inccntrl	; never assign the same control twice
	; KEYBOARD (2) and KEYBMOUSE (5) share the same physical keyboard.
	cmp	#2,d0
	beq.s	.keyboard_candidate
	cmp	#5,d0
	bne.s	.there
.keyboard_candidate
	cmp	#2,d1
...
dointro	;
	...
	;
	bsr	dispon
	; STARTFIX: Fast-EHB title is already precomposed. Legacy AGA loads
	; pics/gloom and draws it safely at Y=168 into both display buffers.
	tst	aga
	beq.s	.noaga
	jsr	g2orig_draw_gloombrush_safe
.noaga	;
	;
	bsr	chaton
	;
	; orig-startfix2a: do not depend on a delayed Intuition ACTIVENEW event.
	; With active=0 readmenujoy deliberately returns zero forever.
	jsr	inputon
	jsr	waitany
	;
	lea	startmenu,a4
...
startmenu	dc.b	9
	...
	dc.b	'PLAYER 1 '
p1ctype
	dc.b	'KEYBMOUSE',0		;orig-runtimefix1 default control index 5
	;
	dc.b	'PLAYER 2 '
p2ctype
	ifne	cd32
...
; window is defined once in the main runtime globals.
msgport	dc.l	0

newtask	dcb.b	92,0

inputon	jsr	g2orig_hide_pointer
	tst	active
	bne.s	.rts
	;
	move	#$4000,$dff09a
...
windowtask	move.l	int(pc),a6	;intuition base
	...
	move.l	d0,g2orig_oswindow
	beq.s	.g2orig_window_open_failed
	jsr	g2orig_hide_pointer
	move.l	g2orig_oswindow,a0
...
p1_ob_cntrl
	dc	5	;orig-runtimefix1: PLAYER1 defaults to KEYBMOUSE
.ob_damage	dc	1
.ob_hitpoints	dc	25
.ob_think	dc	0
.ob_frame	dc.l	0
...
map_rgbs_	ds.w	256	;4bit RGB palette for a game
	...
	even
; -----------------------------------------------------------------------------
; Minimal recovered top HUD and KEYBMOUSE extension.
; Appended deliberately: existing original code/data ranges stay nearly intact.
; -----------------------------------------------------------------------------
...
orig_top_hud_impl
	...
	move.l	panel,d0
	beq.w	.oth_done
	; Fullscreen can draw directly into the normal chunky frame. Smaller
	; WINDOW SIZE modes defer to a fixed 320x32 post-C2P overlay.
	tst	g2orig_hud_force_draw
	bne.s	.oth_draw_now
	cmp	#320,width
	beq.s	.oth_draw_now
	move.l	a5,g2orig_hud_pending_player
	move	#-1,g2orig_hud_pending
	bra.w	.oth_done
.oth_draw_now
	moveq	#2,d7
	moveq	#2,d6
	...
	lea	oth_weapon,a4
	bsr	oth_draw_text
	move	#320-84,d7	;fixed screen-right HUD position
	moveq	#2,d6
	lea	oth_lives,a4
	bsr	oth_draw_text
	;
...
.oth_lives_ok
	subq	#1,d7
	move	#320-44,d6	;fixed screen-right life icons
.oth_lives_loop
	move	d6,d0
	moveq	#2,d1
	moveq	#44,d2
...
.oth_weapon_loop
	...
	bsr	oth_draw_shape
	add	#10,d6
	dbf	d7,.oth_weapon_loop
.oth_done
	movem.l	(a7)+,d0-d7/a0-a4
	rts

oth_health	dc.b	'HEALTH',0
oth_weapon	dc.b	'WEAPON',0
oth_lives	dc.b	'LIVES',0
	even

oth_draw_text	;a4=text, d7=x, d6=y
	movem.l	d0-d2/d6-d7/a4,-(a7)
.oth_text_loop
	move.b	(a4)+,d2
	beq.s	.oth_text_done
	cmp.b	#' ',d2
	beq.s	.oth_text_space
	cmp.b	#'0',d2
	bcs.s	.oth_text_letter
	cmp.b	#'9',d2
	bhi.s	.oth_text_letter
	sub.b	#'0',d2
	ext	d2
	bra.s	.oth_text_draw
.oth_text_letter
	cmp.b	#'A',d2
	bcs.s	.oth_text_space
	and	#31,d2
	add	#9,d2
.oth_text_draw
	move	d7,d0
...
oth_draw_shape	;d0=x,d1=y,d2=panel shape
	movem.l	d0-d7/a0-a4,-(a7)
	move.l	panel,a0
	tst.l	a0
	beq.w	.oth_shape_done
	tst	d0
	blt.w	.oth_shape_done
	tst	d1
	blt.w	.oth_shape_done
	cmp	#49,d2
	bhi.w	.oth_shape_done
	...
	movem	(a0)+,d2-d3
	tst	d2
	ble.w	.oth_shape_done
	tst	d3
	ble.w	.oth_shape_done
	move	d0,d6
	add	d2,d6
	cmp	width,d6
	bgt.w	.oth_shape_done
	move	d1,d6
	add	d3,d6
	cmp	hite,d6
	bgt.w	.oth_shape_done
...
.oth_shape_yloop
	move.b	(a0)+,d0
	beq.s	.oth_shape_skip
	move.b	0(a4,d0),(a3)
.oth_shape_skip
	adda.w	chunkymodw,a3
	dbf	d7,.oth_shape_yloop
	dbf	d2,.oth_shape_xloop
.oth_shape_done
	movem.l	(a7)+,d0-d7/a0-a4
	rts

readkeymouse	;control index 5: keyboard movement/strafe plus mouse turn/fire
	movem.l	d1-d3/a1,-(a7)
	move.l	rawtable,a1
	clr	(a0)
	clr	2(a0)
	...
	bne.s	.rkm_forward
	keya1	$11	; W
	beq.s	.rkm_check_back
.rkm_forward
	moveq	#-1,d0
.rkm_check_back
	keya1	$4d	; cursor down
	bne.s	.rkm_back
	keya1	$21	; S
	bne.s	.rkm_back
	keya1	$32	; X
...
.rkm_store_y
	...
	bne.s	.rkm_left
	keya1	$20	; A
	beq.s	.rkm_check_right
.rkm_left
	moveq	#-1,d0
.rkm_check_right
	keya1	$4e	; cursor right
	bne.s	.rkm_right
	keya1	$22	; D
	beq.s	.rkm_store_x
.rkm_right
...
.rkm_no_strafe
	moveq	#0,d0
	keya1	$66	; existing keyboard fire
	beq.s	.rkm_mouse_fire
	moveq	#-1,d0
.rkm_mouse_fire
	btst	#6,$bfe001	; left mouse button
	bne.s	.rkm_store_fire
	moveq	#-1,d0
.rkm_store_fire
	move	d0,4(a0)
	movem.l	(a7)+,d1-d3/a1
...
sample_keymouse_vb	;mouse X turns whichever player selected control index 5
	movem.l	d0-d2/a5,-(a7)
	move	$dff00a,d1
	move.b	d1,d0
	move.l	player1,a5
...
; ---------------------------------------------------------------------------
; STARTFIX helpers ported from the proven Gloom Reforged static-screen path.
; ---------------------------------------------------------------------------

g2orig_pokepal_picture
	; a0=trimmed IFF or 0, a1=palette. Program only as many entries as the
	; source depth actually owns, capped to the active display palette.
	move.l	a1,lastpal
	move	colours,d0	;startfix1a: GenAm PC-range buildfix
	tst.l	a0
	beq.s	.g2opp_store
	moveq	#0,d1
	move	4(a0),d1
	...
	moveq	#1,d0
	lsl	d1,d0
	cmp	colours,d0	;startfix1a: GenAm PC-range buildfix
	bls.s	.g2opp_store
	move	colours,d0	;startfix1a: GenAm PC-range buildfix
.g2opp_store
	jmp	pokepal2	;startfix1a: absolute tail jump

g2orig_draw_gloombrush_safe
	movem.l	d0-d4/a0-a2,-(a7)
	move.l	gloombrush,d0
	beq	.g2ogb_done
	move.l	d0,a2
	;
	; Validate trimmed-IFF dimensions and depth before touching a bitmap.
	move	(a2),d0
	beq	.g2ogb_done
	cmp	#320,d0
	bhi	.g2ogb_done
	...
	bhi	.g2ogb_done
	;
	; The footer area is 72 rows high (Y 168..239).
	move	d2,-(a7)
	move	#72,d0
	cmp	d0,d2
	bls.s	.g2ogb_height_ok
	move	d0,2(a2)
.g2ogb_height_ok
	;
	move.l	showbitmap,d0
	beq.s	.g2ogb_skip_show
	move.l	d0,a1
	bsr.s	.g2ogb_draw_one
.g2ogb_skip_show
...
.g2ogb_draw_one
	movem.l	d0/a0-a1,-(a7)
	move	#168,d0
	; linemod is a longword; its low word at linemodw is bytes per row.
	; The old 168*40*bitplanes calculation stepped outside the title row.
	mulu	linemodw,d0	;startfix1a: GenAm PC-range buildfix
	add.l	d0,a1
...
; ===========================================================================
; orig-startfix2 - safe EXTERNAL C2P setup
; ===========================================================================
;
; The actual converter remains in the loaded file from PROGDIR:c2p/.
; This helper only:
; - builds a bounded filename from gloomcfg;
; - falls back to external c2p/blackmagic_1;
; - validates the load result before using offsets 36/40;
; - creates coloffs and paladjust without calling the crashing external
;   inittables entry at 32(a2).
;

g2orig_setup_external_c2p
	movem.l	d1-d7/a0-a3,-(a7)
	clr.l	planar_c2p
	clr.l	c2p
	;
	; Prefer the filename stored in gloomcfg when the file was loaded.
	move.l	gloomcfg,d0
	beq.s	.g2osc_try_default
	move.l	d0,a0
	addq.l	#2,a0
	lea	text,a1
	move.l	#'c2p/',(a1)+
	;
	; text/picname is 64 bytes. Four bytes are already occupied by "c2p/".
	; Copy at most 59 filename characters and accept LF, CR or NUL terminators.
	moveq	#58,d7
.g2osc_copy_cfg
	move.b	(a0)+,d0
	beq.s	.g2osc_cfg_done
	cmp.b	#10,d0
	beq.s	.g2osc_cfg_done
	cmp.b	#13,d0
	beq.s	.g2osc_cfg_done
	move.b	d0,(a1)+
	dbf	d7,.g2osc_copy_cfg
.g2osc_cfg_done
	clr.b	(a1)
	lea	text,a0
	moveq	#1,d1
	...
	;
.g2osc_try_default
	lea	g2orig_default_c2p_name(pc),a0
	moveq	#1,d1
	jsr	loadfile
	move.l	d0,planar_c2p
	beq.s	.g2osc_fail
	;
.g2osc_loaded
	move.l	planar_c2p,d0
	beq.s	.g2osc_fail
	move.l	d0,a2
	;
	; Original external file layout:
	; 36(a2) = eight-plane AGA converter
	; 40(a2) = six-plane ECS converter
	lea	36(a2),a0
	tst	aga
	bne.s	.g2osc_converter_ready
	lea	40(a2),a0
.g2osc_converter_ready
	move.l	a0,c2p
	;
	; Build only the support tables locally. The loaded external converter is
	; still the routine used by doc2p during every rendered frame.
	lea	coloffs,a0
	move	#320,d0
	lea	paladjust,a1
	jsr	g2orig_inline_c2p_tables
	;
	moveq	#0,d0
	bra.s	.g2osc_done
.g2osc_fail
...
g2orig_inline_c2p_tables
	; in: a0=coloffs, d0=number of columns, a1=paladjust
	move	#$ff,d1
.g2oct_pal
	move.b	d1,0(a1,d1)
	dbf	d1,.g2oct_pal
...
.g2oct_odd
	...
	dbf	d2,.g2oct_odd
	subq.l	#1,d1
	dbf	d0,.g2oct_col_group
	rts

g2orig_default_c2p_name
	dc.b	'c2p/blackmagic_1',0
	even

; ===========================================================================
; orig-runtimefix1 helpers
; ===========================================================================

; Auto-wrap a long intermission text into at most two balanced lines.
; d0.w = text length, buffer = text. Existing '\' separator is preserved.
g2orig_wrap_script_text
	movem.l	d1-d7/a0,-(a7)
	cmp	#38,d0
	ble.s	.g2ow_done
	move	d0,d4
...
; Prevent the ESC/menu fire used to open the menu from immediately closing it.
g2orig_wait_menu_release
	movem.l	d0,-(a7)
.g2owmr_loop
	jsr	vwait
	jsr	readmenujoy
...
; Separate Intuition window pointer. The old recovered source had two symbols
; named 'window'; overloading the gameplay window pointer was unsafe.
g2orig_oswindow	dc.l	0

g2orig_hide_pointer
	movem.l	d0-d3/a0-a1/a6,-(a7)
	move	#$0020,$dff096
	move.l	g2orig_oswindow,d0
	beq.s	.g2ohp_done
	move.l	int,d1
	beq.s	.g2ohp_done
	move.l	chipzero,a1
...
; Small-window HUD overlay state.
g2orig_hud_pending	dc	0
g2orig_hud_force_draw	dc	0
g2orig_hud_pending_player	dc.l	0
g2orig_hud_saved_width	dc	0
g2orig_hud_saved_hite	dc	0
g2orig_hud_saved_chunkymodw	dc	0
	even
g2orig_hud_saved_top_chunky	ds.b	320*32
	even

; Main C2P converts only the centred small view. Draw the HUD into a temporary
; 320x32 chunky strip, convert it to the top of the planar screen, then restore
; the original compact chunky image for the next frame/menu backdrop.
g2orig_hud_post_c2p
	tst	g2orig_hud_pending
	beq	.g2ohud_done
	clr	g2orig_hud_pending
	movem.l	d0-d7/a0-a6,-(a7)
	;
	move	width,g2orig_hud_saved_width
	move	hite,g2orig_hud_saved_hite
	move	chunkymodw,g2orig_hud_saved_chunkymodw
	;
	move.l	chunky,a0
	lea	g2orig_hud_saved_top_chunky,a1
	move	#(320*32)/4-1,d0
.g2ohud_save
	move.l	(a0)+,(a1)+
	dbf	d0,.g2ohud_save
...
.g2ohud_clear
	...
	move	#32,hite
	move	#320,chunkymodw
	move.l	g2orig_hud_pending_player,a5
	tst.l	a5
	beq.s	.g2ohud_convert
	move	#-1,g2orig_hud_force_draw
	jsr	showstats
	clr	g2orig_hud_force_draw
.g2ohud_convert
	move.l	chunky,a0
...
.g2ohud_restore
	move.l	(a1)+,(a0)+
	dbf	d0,.g2ohud_restore
	;
	move	g2orig_hud_saved_width,width
	move	g2orig_hud_saved_hite,hite
	move	g2orig_hud_saved_chunkymodw,chunkymodw
	movem.l	(a7)+,d0-d7/a0-a6
.g2ohud_done
	rts

; ===========================================================================
; orig-runtimefix2 - FPS ToolType and first-person weapon overlay
; ===========================================================================

; ---------------------------------------------------------------------------
; Bare Workbench ToolType "FPS"
; ---------------------------------------------------------------------------

g2orig_cli_ptr	dc.l	0
g2orig_fps_enabled	dc	0
g2orig_fps_value	dc	0
g2orig_fps_window_start	dc	0
g2orig_fps_window_count	dc	0
g2orig_fps_window_valid	dc	0
g2orig_fps_refresh_hz	dc	50
g2orig_iconbase	dc.l	0
g2orig_iconname	dc.b	'icon.library',0
	even

g2orig_fps_tooltype_probe
	movem.l	d0-d7/a0-a6,-(a7)
	clr	g2orig_fps_enabled
	;
	; Workbench icon ToolType.
	move.l	wbmess,d0
	beq.w	.g2oft_scan_cli
	move.l	4.w,a6
	lea	g2orig_iconname(pc),a1
	jsr	-408(a6)
	move.l	d0,g2orig_iconbase
	beq.w	.g2oft_scan_cli
	move.l	wbmess,a0
	move.l	$24(a0),d0
	beq.s	.g2oft_close
...
.g2oft_loop
	...
	beq.s	.g2oft_free
	move.l	d0,a0
	jsr	g2orig_is_bare_fps
	tst	d0
	beq.s	.g2oft_loop
	move	#-1,g2orig_fps_enabled
.g2oft_free
	move.l	d6,a0
	move.l	g2orig_iconbase,a6
	jsr	-90(a6)
.g2oft_close
	move.l	g2orig_iconbase,d0
	beq.s	.g2oft_scan_cli
	move.l	d0,a1
	move.l	4.w,a6
	jsr	-414(a6)
	clr.l	g2orig_iconbase
	;
.g2oft_scan_cli
	move.l	g2orig_cli_ptr,d0
	beq.s	.g2oft_done
	move.l	d0,a0
	jsr	g2orig_fps_scan_cli
.g2oft_done
	movem.l	(a7)+,d0-d7/a0-a6
	rts

; a0 = zero/LF terminated CLI line. Recognize standalone FPS exactly like the
; working gloom2 token scanner (case-insensitive; space/TAB/comma separators).
g2orig_fps_scan_cli
	movem.l	d0-d4/a0-a1,-(a7)
.g2ofc_skip
	move.b	(a0),d0
	beq.w	.g2ofc_done	;runtimefix3a: GenAm branch range
	cmp.b	#10,d0
	beq.w	.g2ofc_done	;runtimefix3a: GenAm branch range
	cmp.b	#13,d0
	beq.s	.g2ofc_advance
...
.g2ofc_compare
	...
	jsr	g2orig_upper_d0
	cmp.b	#'S',d0
	bne.w	.g2ofc_skip	;runtimefix3a: GenAm branch range
	move	#-1,g2orig_fps_enabled
.g2ofc_done
	movem.l	(a7)+,d0-d4/a0-a1
	rts

; a0=one ToolType string, d0=-1 only for active bare FPS, otherwise 0.
g2orig_is_bare_fps
	movem.l	d1-d3/a0,-(a7)
.g2oif_skip
	move.b	(a0)+,d0
...
; ---------------------------------------------------------------------------
; Presented FPS measurement and two 5x7 digits in the free bottom panel area.
; ---------------------------------------------------------------------------

g2orig_fps_reset
	; Proven gloom2/c87a7 logic: initialize to the engine's normal capped
	; PAL/NTSC value until the first complete one-second sample is available.
	movem.l	d0-d2/a0,-(a7)
	moveq	#50,d1
	move.l	4.w,a0
	moveq	#0,d0
	move.b	530(a0),d0		;ExecBase VBlankFrequency
	cmp	#40,d0
	bcs.s	.g2ofr_fallback
	cmp	#100,d0
	bhi.s	.g2ofr_fallback
	move	d0,d1
.g2ofr_fallback
	move	d1,g2orig_fps_refresh_hz
	lsr	#1,d1			;Gloom: one present per two VBlanks
	move	d1,g2orig_fps_value
	clr	g2orig_fps_window_start
	clr	g2orig_fps_window_count
	clr	g2orig_fps_window_valid
	movem.l	(a7)+,d0-d2/a0
	rts

g2orig_fps_restart_window
	; Keep the visible value, but discard time spent inside the in-game menu.
	clr	g2orig_fps_window_count
	clr	g2orig_fps_window_valid
	rts

g2orig_fps_update_present
	; Exact gloom2/c87a7 presented-frame average:
	; presents * VBlankFrequency / elapsed VBlanks, rounded to nearest integer.
	tst	g2orig_fps_enabled
	beq.w	.g2ofu_done
	movem.l	d0-d5,-(a7)
.g2ofu_retry
	moveq	#0,d0
	...
	cmp	d0,d1
	bne.s	.g2ofu_retry
	tst	g2orig_fps_window_valid
	bne.s	.g2ofu_have_window
	move	#-1,g2orig_fps_window_valid
	move	d0,g2orig_fps_window_start
	clr	g2orig_fps_window_count
	bra.s	.g2ofu_exit
.g2ofu_have_window
	addq	#1,g2orig_fps_window_count
	move	d0,d1
	sub	g2orig_fps_window_start,d1
	and.l	#$0000ffff,d1
	moveq	#0,d2
	move	g2orig_fps_refresh_hz,d2
	cmp.l	d2,d1
	bcs.s	.g2ofu_exit		;hold displayed value for a full second
	;
	moveq	#0,d3
	move	g2orig_fps_window_count,d3
	mulu	d2,d3
	move.l	d1,d4
	lsr.l	#1,d4
	add.l	d4,d3
...
; Select the whitest neutral logical palette colour and map it through paladjust.
g2orig_fps_find_white
	movem.l	d0-d3/d5-d7/a0-a1,-(a7)
	moveq	#0,d4
	move.l	lastpal,d0
	bne.s	.g2off_have_pal
...
; Transparent white FPS overlay at the physical top centre.
; Only glyph pixels are changed. Existing world/HUD pixels remain untouched.
; Called after g2orig_hud_post_c2p, so the number visibly covers the HUD.
g2orig_fps_post_c2p
	tst	g2orig_fps_enabled
	beq.w	.g2ofp_done
	movem.l	d0-d7/a0-a6,-(a7)
	move.l	drawbitmap,d0
	beq.w	.g2ofp_exit
	jsr	g2orig_fps_find_white
	move	#154,d2			;11px digits centred in physical 320px
	moveq	#1,d3
	jsr	g2orig_fps_draw_digits_planar
.g2ofp_exit
	movem.l	(a7)+,d0-d7/a0-a6
.g2ofp_done
	rts

; Exact two-digit sequencing from gloom2/c87a7.
g2orig_fps_draw_digits_planar
	jsr	g2orig_fps_split_digits
	move	d6,d0
	lsl	#3,d0
	sub	d6,d0			;digit * 7 bytes
	ext.l	d0
	lea	g2orig_fps_digits(pc),a0
	adda.l	d0,a0
	jsr	g2orig_fps_draw_glyph_planar
	move	d7,d0
	lsl	#3,d0
	sub	d7,d0
	ext.l	d0
...
g2orig_fps_split_digits
	moveq	#0,d0
	move	g2orig_fps_value,d0
	cmp	#99,d0
	bls.s	.g2ofs_range
	moveq	#99,d0
.g2ofs_range
	...
	divu	d1,d0
	moveq	#0,d6
	move	d0,d6			;quotient/tens
	swap	d0
	moveq	#0,d7
	move	d0,d7			;remainder/ones
	rts

; Seven rows, five columns, bit 4=left pixel ... bit 0=right pixel.
; d2/d3 are preserved so the second digit remains on the same baseline.
g2orig_fps_draw_glyph_planar
	movem.l	d0-d3/d6-d7/a1-a2,-(a7)
	moveq	#6,d6
.g2ofg_row
	moveq	#0,d7
	move.b	(a0)+,d7
...
.g2ofg_next
	addq	#1,d3
	dbf	d6,.g2ofg_row
	movem.l	(a7)+,d0-d3/d6-d7/a1-a2
	rts

; Plot one colour-indexed pixel into the completed planar draw bitmap.
; d0=x, d1=y, d4=adjusted white palette index.
g2orig_fps_plot_planar
	...
	adda.w	d2,a1
	and	#7,d0
	eor	#7,d0			;Amiga planar bit number in destination byte
	moveq	#0,d6
	moveq	#0,d7
	move	bitplanes,d7
	subq	#1,d7
...
g2orig_fps_digits
	dc.b	$0e,$11,$13,$15,$19,$11,$0e
	dc.b	$04,$0c,$04,$04,$04,$04,$0e
	dc.b	$0e,$11,$01,$02,$04,$08,$1f
	dc.b	$1e,$01,$01,$0e,$01,$01,$1e
	dc.b	$02,$06,$0a,$12,$1f,$02,$02
	dc.b	$1f,$10,$10,$1e,$01,$01,$1e
	dc.b	$0e,$10,$10,$1e,$11,$11,$0e
	dc.b	$1f,$01,$02,$04,$08,$08,$08
	dc.b	$0e,$11,$11,$0e,$11,$11,$0e
	dc.b	$0e,$11,$11,$0f,$01,$01,$0e
	even

; ---------------------------------------------------------------------------
; Centred first-person weapon from external misc/gun.bin / stuf/gun.bin.
; ---------------------------------------------------------------------------

gunpic	dc.l	0
g2orig_gun_firetimer	dc	0
g2orig_gun_recoilflag	dc.b	0
	even
g2orig_gun_name_miscbin	dc.b	'misc/gun.bin',0
g2orig_gun_name_stufbin	dc.b	'stuf/gun.bin',0
g2orig_gun_name_miscraw	dc.b	'misc/gun',0
g2orig_gun_name_stufraw	dc.b	'stuf/gun',0
	even

g2orig_loadgunfallback
	movem.l	d0-d1/a0,-(a7)
	tst.l	gunpic
	bne.s	.g2olg_done
	lea	g2orig_gun_name_miscbin(pc),a0
	moveq	#1,d1
	jsr	loadfile
	move.l	d0,gunpic
	bne.s	.g2olg_done
	lea	g2orig_gun_name_stufbin(pc),a0
	moveq	#1,d1
	jsr	loadfile
	move.l	d0,gunpic
	bne.s	.g2olg_done
	lea	g2orig_gun_name_miscraw(pc),a0
	moveq	#1,d1
	jsr	loadfile
	move.l	d0,gunpic
	bne.s	.g2olg_done
	lea	g2orig_gun_name_stufraw(pc),a0
	moveq	#1,d1
	jsr	loadfile
	move.l	d0,gunpic
.g2olg_done
...
g2orig_drawgun
	...
	ble.w	.g2odg_done
	move.l	d0,a0
	clr.b	g2orig_gun_recoilflag
	move	g2orig_gun_firetimer,d7
	beq.s	.g2odg_normal
	st	g2orig_gun_recoilflag
	subq	#1,g2orig_gun_firetimer
	moveq	#1,d6			;shape 1 = recoil gun
	bra.s	.g2odg_shape_index
.g2odg_normal
	moveq	#0,d6			;shape 0 = idle gun
.g2odg_shape_index
	lsl	#2,d6
	add	#12,d6
	move.l	0(a0,d6.w),d0
	...
	move	width,d1
	asr	#1,d1
	sub	(a0),d1			;centre - xhandle
	move	6(a0),d3
	move	hite,d4
	add	#26,d4
	sub	d3,d4
	;
	moveq	#0,d6			;horizontal bob
	moveq	#0,d7			;vertical bob
	tst.b	g2orig_gun_recoilflag
	bne.s	.g2odg_recoil
	move	ob_bounce(a5),d0
	beq.s	.g2odg_bob_done
	lsr	#1,d0
	and	#255,d0
	move.l	camrots,a2
	lea	0(a2,d0*8),a2
...
.g2odg_bob_abs
	lsr	#1,d7
	sub	d7,d4
	bra.s	.g2odg_bob_done
.g2odg_recoil
	addq	#2,d4
.g2odg_bob_done
	;
	tst.b	g2orig_gun_recoilflag
	beq.s	.g2odg_no_flash
	movem.l	d0-d7/a0-a5,-(a7)
	jsr	g2orig_drawgun_muzzle
	movem.l	(a7)+,d0-d7/a0-a5
.g2odg_no_flash
	jsr	g2orig_drawgun_shape
.g2odg_done
	movem.l	(a7)+,d0-d7/a0-a5
	rts

; Muzzle shape = 2 + ((weapon + 1) / 2), matching the working gloom2 path.
g2orig_drawgun_muzzle
	movem.l	d0-d7/a0-a5,-(a7)
	move.l	gunpic,d0
	beq.w	.g2odm_done
	move.l	d0,a0
	;
	; Working gloom2 grouping: shape = 2 + ((weapon + 1) / 2).
	move.l	player_,a5
	moveq	#0,d7
	tst.l	a5
	beq.s	.g2odm_have_weapon
	move	ob_weapon(a5),d7
	cmp	#4,d7
	bls.s	.g2odm_weapon_ok
	moveq	#4,d7
.g2odm_weapon_ok
	addq	#1,d7
	asr	#1,d7
...
.g2odm_have_weapon
	...
	add.l	d0,a0
	;
	; Centre by flash width. Draw flash before the gun so the weapon is in front.
	move	4(a0),d0
	move	width,d1
	sub	d0,d1
	asr	#1,d1
	move	6(a0),d3
	move	hite,d4
	subq	#3,d4			;same relation as proven fullscreen Y=237
	sub	d3,d4
	;
	; Proven three-frame size sequence: 1.0x, 1.6x, 2.4x.
	move	g2orig_gun_firetimer,d5
	cmp	#2,d5
	beq.s	.g2odm_scale1
	cmp	#1,d5
	beq.s	.g2odm_scale16
.g2odm_scale24
	...
	moveq	#12,d6
	moveq	#5,d7
	jsr	g2orig_drawgun_shape_scalefrac
	bra.s	.g2odm_done
.g2odm_scale16
	move	d0,d6
	moveq	#8,d7
...
; a0=shape, d1=x, d4=y. Robust clipping for every WINDOW SIZE.
g2orig_drawgun_shape
	movem.l	d0-d7/a0-a6,-(a7)
	move	4(a0),d2			;source width
	move	6(a0),d3			;source height
	tst	d2
	ble.w	.g2ods_done
	cmp	#160,d2
	bhi.w	.g2ods_done
	...
	cmp	#128,d3
	bhi.w	.g2ods_done
	lea	8(a0),a5			;source column-major pixels
	;
	moveq	#0,d6				;source x start
	tst	d1
	bpl.s	.g2ods_x_nonneg
	move	d1,d6
	neg	d6
	moveq	#0,d1
.g2ods_x_nonneg
	...
	bge.w	.g2ods_done
	move	d2,d7
	sub	d6,d7				;remaining source columns
	move	width,d0
	sub	d1,d0				;available destination columns
	ble.w	.g2ods_done
	cmp	d0,d7
	bls.s	.g2ods_x_count_ok
	move	d0,d7
.g2ods_x_count_ok
	;
	moveq	#0,d0				;source y start
	tst	d4
	bpl.s	.g2ods_y_nonneg
	move	d4,d0
	neg	d0
	moveq	#0,d4
.g2ods_y_nonneg
	...
	bge.w	.g2ods_done
	move	d3,d5
	sub	d0,d5				;remaining source rows
	move	hite,d2
	sub	d4,d2
	ble.w	.g2ods_done
	cmp	d2,d5
	bls.s	.g2ods_y_count_ok
	move	d2,d5
.g2ods_y_count_ok
	;
	move	d6,d2
...
.g2ods_skip
	adda.w	chunkymodw,a3
	dbf	d6,.g2ods_yloop
	adda.w	d3,a5			;next complete source column
	dbf	d7,.g2ods_xloop
.g2ods_done
	movem.l	(a7)+,d0-d7/a0-a6
	rts
...
; ---------------------------------------------------------------------------
; orig-runtimefix3: fractional nearest-neighbour flash scaler from gloom2.
; a0=shape, d1=x, d4=y, d6=numerator, d7=denominator.
; ---------------------------------------------------------------------------
g2orig_drawgun_shape_scalefrac
	movem.l	d0-d7/a0-a6,-(a7)
	move	4(a0),d2
	move	6(a0),d3
	tst	d2
...
; Player shots begin in front of the first-person muzzle, not inside the player.
g2orig_apply_player_muzzle_origin
	movem.l	d0-d2/a1,-(a7)
	move.l	a5,d1
	move.l	player1,d0
	cmp.l	d1,d0
...
; After the real vector is known, move player projectiles three visual steps
; forward. This is the proven gloom2 fix for large projectile sprites.
g2orig_player_bullet_visual_prestep
	movem.l	d0-d1,-(a7)
	move.l	a5,d0
	cmp.l	player1,d0
	beq.s	.g2opbp_do
